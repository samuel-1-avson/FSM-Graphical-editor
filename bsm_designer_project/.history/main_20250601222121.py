...
class MainWindow(QMainWindow):
    ...
    def _create_actions(self):
        ...
        try:
            ...
            def _safe_get_style_enum(attr_name, fallback_attr_name=None):
                try:
                    if isinstance(attr_name, QStyle.StandardPixmap):
                        return attr_name
                    return getattr(QStyle, str(attr_name))
                except AttributeError:
                    if fallback_attr_name:
                        try:
                            if isinstance(fallback_attr_name, QStyle.StandardPixmap):
                                return fallback_attr_name
                            return getattr(QStyle, str(fallback_attr_name))
                        except AttributeError:
                            pass
            ...
            # Find action (needs to be created after other actions)
            self.find_item_action = QAction(
                get_standard_icon(QStyle.SP_FileDialogContentsView, "Find"),
                "&Find State...", self,
                shortcut=QKeySequence.Find,
                statusTip="Find a state by name",
                triggered=self.on_show_find_item_dialog
            )
    ...
    def _create_mode_actions(self, _safe_get_style_enum):
        ...
        # Create mode actions with proper icons
        self.select_mode_action = QAction(
            get_standard_icon(QStyle.SP_ArrowCursor, "Select"),
            "Select Mode", self,
            shortcut="Esc",
            statusTip="Switch to selection mode",
            triggered=lambda: self.diagram_scene.set_mode(DiagramScene.Mode.SELECT)
        )
        self.select_mode_action.setCheckable(True)
        self.select_mode_action.setChecked(True)
        
        self.state_mode_action = QAction(
            get_standard_icon(QStyle.SP_FileIcon, "State"),
            "Add State", self,
            shortcut="S",
            statusTip="Switch to state creation mode",
            triggered=lambda: self.diagram_scene.set_mode(DiagramScene.Mode.ADD_STATE)
        )
        self.state_mode_action.setCheckable(True)
        
        self.transition_mode_action = QAction(
            get_standard_icon(QStyle.SP_ArrowRight, "Trans"),
            "Add Transition", self,
            shortcut="T",
            statusTip="Switch to transition creation mode",
            triggered=lambda: self.diagram_scene.set_mode(DiagramScene.Mode.ADD_TRANSITION)
        )
        self.transition_mode_action.setCheckable(True)
        
        self.comment_mode_action = QAction(
            get_standard_icon(QStyle.SP_MessageBoxInformation, "Comment"),
            "Add Comment", self,
            shortcut="C",
            statusTip="Switch to comment creation mode",
            triggered=lambda: self.diagram_scene.set_mode(DiagramScene.Mode.ADD_COMMENT)
        )
        self.comment_mode_action.setCheckable(True)
    ...
    def _create_ide_actions(self, _safe_get_style_enum):
        ...
        for action_id, text, short_name, icon_style, slot in ide_configs:
            try:
                icon = get_standard_icon(icon_style, short_name)
                action = QAction(icon, text, self, triggered=slot)
                setattr(self, f"ide_{action_id}_action", action)
            except Exception as e:
                logger.error(f"Error creating IDE action {action_id}: {e}")
                # Create action without icon as fallback
    ...
    def _create_edit_menu(self, menu_bar):
        ...
        # Find
        if hasattr(self, 'find_item_action'):
        ...
        mode_menu.addAction(self.state_mode_action)
        mode_menu.addAction(self.transition_mode_action)
        mode_menu.addAction(self.comment_mode_action)
        edit_menu.addSeparator()
    ...
    def _create_toolbars(self):
        """Create and initialize all application toolbars with proper error handling."""
        try:
            # Create main toolbar
            ...
            edit_toolbar.addAction(self.select_mode_action)
            edit_toolbar.addAction(self.state_mode_action)
            edit_toolbar.addAction(self.transition_mode_action)
            edit_toolbar.addAction(self.comment_mode_action)
            ...
            logger.error(f"Error creating toolbars: {e}", exc_info=True)
            QMessageBox.critical(self, "Error", f"Failed to initialize application toolbars: {str(e)}")

    def _create_docks(self):
        """Create and initialize all dock widgets with proper error handling."""
        try:
            # Create toolbox dock
            ...
            self.toolbox_select_button.setToolButtonStyle(Qt.ToolButtonTextUnderIcon)
            
            self.toolbox_state_button = QToolButton()
            self.toolbox_state_button.setDefaultAction(self.state_mode_action)
            self.toolbox_state_button.setToolButtonStyle(Qt.ToolButtonTextUnderIcon)
            
            self.toolbox_transition_button = QToolButton()
            self.toolbox_transition_button.setDefaultAction(self.transition_mode_action)
            self.toolbox_transition_button.setToolButtonStyle(Qt.ToolButtonTextUnderIcon)
            
            self.toolbox_comment_button = QToolButton()
            self.toolbox_comment_button.setDefaultAction(self.comment_mode_action)
            self.toolbox_comment_button.setToolButtonStyle(Qt.ToolButtonTextUnderIcon)
            
            # Add buttons to layout
            toolbox_layout.addWidget(self.toolbox_select_button)
            toolbox_layout.addWidget(self.toolbox_state_button)
            toolbox_layout.addWidget(self.toolbox_transition_button)
            toolbox_layout.addWidget(self.toolbox_comment_button)
            ...
            logger.error(f"Error creating docks: {e}", exc_info=True)
            QMessageBox.critical(self, "Error", f"Failed to initialize application docks: {str(e)}")

    def get_standard_icon(self, style_enum_value, short_name):
        """Get a standard icon with proper error handling."""
        try:
            if isinstance(style_enum_value, str):
                # Try to get the enum value from QStyle
                try:
                    style_enum_value = getattr(QStyle, style_enum_value)
                except AttributeError:
                    logger.warning(f"Could not find style enum {style_enum_value}, using fallback")
                    return QIcon()
            
            if not isinstance(style_enum_value, QStyle.StandardPixmap):
                logger.warning(f"Invalid type for style_enum_value in get_standard_icon. Expected QStyle.StandardPixmap, got {type(style_enum_value)} (value: {style_enum_value}). Alt: {short_name}")
                return QIcon()
            
            return self.style().standardIcon(style_enum_value)
        except Exception as e:
            logger.error(f"Error getting standard icon: {e}")
            return QIcon()
    ...
    def _handle_py_sim_global_ui_enable_by_manager(self, enable: bool):
        ...
        diagram_editing_actions = [
            ...
            self.state_mode_action, self.transition_mode_action, self.comment_mode_action
        ]
        for action in diagram_editing_actions:
            if hasattr(action, 'setEnabled'): action.setEnabled(is_editable)

        if hasattr(self, 'toolbox_dock'): self.toolbox_dock.setEnabled(is_editable)
    ...
    def set_ui_enabled_for_matlab_op(self, enabled: bool):
        ...
        for dock_name in ["ToolboxDock", "PropertiesDock", "LogDock", "PySimDock", "AIChatbotDock", "IDEDock"]:
