...
class MainWindow(QMainWindow):
    ...
    def on_trigger_py_event(self): # Likely the next one to be called if not defined
        if not self.py_fsm_engine:
        ...
        # If no event name is entered, treat it as an internal step
        if not event_name:
            # self.log_message("No event name entered, performing internal step.")
            self.on_step_py_simulation() # Call the internal step handler
            return

        # TODO: Add GUI elements to input event_payload if needed. For now, it's None.
