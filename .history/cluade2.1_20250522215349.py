...
class DiagramScene(QGraphicsScene):
    ...
    def __init__(self, undo_stack, parent_window=None):
        ...
        self.log_function = print # Default, can be overridden
    ...
    def set_mode(self, mode: str):
        ...
        if self.parent_window:
            ...
            for m, action in actions_map.items():
                if m == mode and action and not action.isChecked(): # Added 'action and' for safety
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            if self.current_mode == "state":
                ...
                if isinstance(top_item_at_pos, GraphicsStateItem):
                ...
                else: # Clicked on non-state or empty space
                    ...
                    if self._temp_transition_line:
                        self.removeItem(self._temp_transition_line)
                        self._temp_transition_line = None
            else: # This 'else' implies self.current_mode == "select" or other default mode
                self._mouse_press_items_positions.clear()
                
                # STEP 1: Absolutely ensure the variable is initialized.
                # Using a new name 'movable_items_cache' to avoid any stale references.
                movable_items_cache = []

                # STEP 2: Try to populate it.
                try:
                    if self.log_function: # Defensive check
                        self.log_function(f"DIAGNOSTIC: In mousePressEvent 'else' block (select mode). Current mode: {self.current_mode}")
                    
                    # Explicitly check self.selectedItems() before comprehension
                    selected_items_list = self.selectedItems()
                    if self.log_function:
                        self.log_function(f"DIAGNOSTIC: self.selectedItems() returned: {type(selected_items_list)}, length: {len(selected_items_list) if selected_items_list is not None else 'None'}")

                    if selected_items_list is not None: # Ensure it's iterable
                        movable_items_cache = [item for item in selected_items_list if item.flags() & QGraphicsItem.ItemIsMovable]
                    else:
                        if self.log_function:
                            self.log_function("DIAGNOSTIC: self.selectedItems() was None. movable_items_cache remains empty.")
                
                except Exception as e_populate:
                    if self.log_function:
                        self.log_function(f"DIAGNOSTIC: Error populating movable_items_cache: {type(e_populate).__name__}: {e_populate}")
                    # movable_items_cache remains [] if an error occurred
                
                # STEP 3: Check if it's really a list and log its state before the loop
                if self.log_function:
                    self.log_function(f"DIAGNOSTIC: movable_items_cache is type: {type(movable_items_cache)}, "
                                      f"len: {len(movable_items_cache) if isinstance(movable_items_cache, list) else 'N/A (not a list)'}")

                # STEP 4: The problematic loop
                try:
                    for item_to_process in movable_items_cache: # This is likely around your line 1126
                        self._mouse_press_items_positions[item_to_process] = item_to_process.pos()
                except Exception as e_loop:
                    if self.log_function:
                         self.log_function(f"DIAGNOSTIC: Error IN the loop for _mouse_press_items_positions: {type(e_loop).__name__}: {e_loop}")

                super().mousePressEvent(event) # Let the base class handle item selection/movement start
        ...
        if event.button() == Qt.LeftButton and self.current_mode == "select":
            if self._mouse_press_items_positions:
                ...
                for item, old_pos in self._mouse_press_items_positions.items():
                    ...
                    if (new_pos - old_pos).manhattanLength() > 0.1: # Item actually moved
    ...
    def edit_item_properties(self, item):
        ...
        if dialog_executed_and_accepted and new_props_from_dialog is not None:
            ...
            final_new_props.update(new_props_from_dialog) # Merge
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        ...
        if item_type == "State":
            ...
            state_name_from_input = default_name # Default if dialog is skipped or name not changed
            ...
            # Make sure StatePropertiesDialog exists and is correctly imported/defined
            ...
            if props_dialog.exec_() == QDialog.Accepted:
                ...
                # Check for duplicate name, but allow original default if it's somehow chosen again by user
                if self.get_state_by_name(final_props['name']) and final_props['name'] != state_name_from_input :
                    ...
                    # Do not return, but the state will not be added if current_item remains None
                else: # Only create item if name is unique or unchanged default
            ...
            # Handle mode change if this was triggered by clicking tool button
            if self.current_mode == "state":
                 self.set_mode("select")
            if not current_item: return # If dialog cancelled or name conflict, exit
        ...
        # Ensure mode reset after drag-drop as well (if it changed implicitly)
        # If current_mode was state/comment via button, it was already reset above.
        # If current_mode was "select" and item was drag-dropped, mode is fine.
    ...
    def _handle_transition_click(self, clicked_state_item: GraphicsStateItem, click_pos: QPointF):
        if not self.transition_start_item: # Starting a new transition
            ...
            center_start = self.transition_start_item.sceneBoundingRect().center()
            self._temp_transition_line.setLine(QLineF(center_start, click_pos)) # Start drawing line
            ...
            # Ensure TransitionPropertiesDialog exists
        ...
        if event.key() == Qt.Key_Delete or event.key() == Qt.Key_Backspace:
            ...
            if self.current_mode == "transition" and self.transition_start_item:
                ...
                self.set_mode("select") # Revert to select mode
                 ...
                 self.set_mode("select") # Revert other modes to select mode
    ...
    def dropEvent(self, event: QGraphicsSceneDragDropEvent):
        pos = event.scenePos()
        if event.mimeData().hasFormat("application/x-bsm-tool"):
            item_type_data_str = event.mimeData().text() # was .data("application/x-bsm-tool").data().decode()
            
            grid_x = round(pos.x() / self.grid_size) * self.grid_size
            grid_y = round(pos.y() / self.grid_size) * self.grid_size
            
            if "State" in item_type_data_str: # Adjust for state item center
                grid_x -= 60 # Roughly half width
                grid_y -= 30 # Roughly half height
            ...
            # Use the existing _add_item_interactive which handles dialogs and undo stack
    ...
    def load_diagram_data(self, data):
        ...
        self.set_dirty(False) # Loading a file should clear dirty status
        self.undo_stack.clear() # Reset undo stack for newly loaded file
    ...
    def drawBackground(self, painter: QPainter, rect: QRectF):
        ...
        # Get integer boundaries for loop ranges
        ...
        # Calculate the first lines to draw, aligned to grid_size
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.MiddleButton or \
           (self._is_panning_with_space and event.button() == Qt.LeftButton):
            ...
            event.accept() # We fully handled panning here, so accept and don't pass to super.
        else:
            self._is_panning_with_mouse_button = False
            # WHEN NOT PANNING, THE EVENT MUST BE PASSED TO SUPER()
            # This allows QGraphicsView to process it, convert it to
            # QGraphicsSceneMouseEvent, and dispatch it to the scene.
            super().mousePressEvent(event)
    def mouseMoveEvent(self, event: QMouseEvent):
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        if self.py_fsm_engine:
            self.py_fsm_engine.register_mock_function("set_digital_output", mock_set_digital_output_from_gui_context)
            self.py_fsm_engine.register_mock_function("start_software_timer", mock_start_software_timer_from_gui_context)
    ...
    def _update_py_simulation_actions_enabled_state(self):
        is_matlab_op_running = hasattr(self,'progress_bar') and not self.progress_bar.isHidden()

    # Python sim actions are only available if no MATLAB op is running
        can_start_py_sim = not self.py_sim_active and not is_matlab_op_running
        can_interact_with_py_sim = self.py_sim_active and not is_matlab_op_running

        self.start_py_sim_action.setEnabled(can_start_py_sim)
        if hasattr(self, 'py_sim_start_btn'): # Check if button exists
            self.py_sim_start_btn.setEnabled(can_start_py_sim)

        self.stop_py_sim_action.setEnabled(can_interact_with_py_sim)
        if hasattr(self, 'py_sim_stop_btn'):
            self.py_sim_stop_btn.setEnabled(can_interact_with_py_sim)
    
        self.reset_py_sim_action.setEnabled(can_interact_with_py_sim)
        if hasattr(self, 'py_sim_reset_btn'):
            self.py_sim_reset_btn.setEnabled(can_interact_with_py_sim)
    
        if hasattr(self, 'py_sim_step_btn'):
            self.py_sim_step_btn.setEnabled(can_interact_with_py_sim)
        if hasattr(self, 'py_sim_event_name_edit'):
            self.py_sim_event_name_edit.setEnabled(can_interact_with_py_sim)
        if hasattr(self, 'py_sim_trigger_event_btn'):
            self.py_sim_trigger_event_btn.setEnabled(can_interact_with_py_sim)
    ...
    # File menu actions related to opening/saving
        file_actions = [self.new_action, self.open_action, self.save_action, self.save_as_action]
        for action in file_actions:
            action.setEnabled(is_editable)

    # Edit menu actions
        edit_actions = [self.undo_action, self.redo_action, self.delete_action, self.select_all_action]
        for action in edit_actions:
            action.setEnabled(is_editable)

    # Mode actions for diagram editing (only if they exist)
        mode_editing_actions = [self.add_state_mode_action, self.add_transition_mode_action, self.add_comment_mode_action]
        for action in mode_editing_actions:
            if hasattr(self, 'mode_action_group') and action in self.mode_action_group.actions(): # Ensure part of group
                action.setEnabled(is_editable)
    
        if is_editable and hasattr(self, 'select_mode_action'): # If returning to editable, ensure select mode is available
            self.select_mode_action.setEnabled(True) 
        elif hasattr(self, 'select_mode_action'): # If sim running, select_mode might still be "enabled" visually but diagram frozen
            self.select_mode_action.setEnabled(True) # Keep select enabled for scene interaction like panning if allowed
        ...
        if hasattr(self, 'properties_edit_button'):
            can_edit_props = is_editable and (True if not hasattr(self, 'scene') else len(self.scene.selectedItems())==1)
            self.properties_edit_button.setEnabled(can_edit_props)
    
        matlab_connected = hasattr(self, 'matlab_connection') and self.matlab_connection.connected
        matlab_actions = [self.export_simulink_action, self.run_simulation_action, self.generate_code_action]
        for action in matlab_actions:
            action.setEnabled(is_editable and matlab_connected)
        self.matlab_settings_action.setEnabled(is_editable)

    # This will correctly set enable states for sim control buttons
    ...
    def _highlight_sim_active_state(self, state_name_to_highlight: str | None):
        if hasattr(self, '_py_sim_currently_highlighted_item') and self._py_sim_currently_highlighted_item:
            self._py_sim_currently_highlighted_item.set_py_sim_active_style(False)
            self._py_sim_currently_highlighted_item = None

        if state_name_to_highlight and hasattr(self, 'scene'):
    ...
    def _append_to_py_simulation_log(self, log_entries: list[str]):
        if not hasattr(self, 'py_sim_action_log_output'):
        return
    ...
    for entry in log_entries:
        ...
        elif "ERROR" in entry.upper() or "CRITICAL" in entry.upper(): # General error coloring
            cleaned_entry = f"<span style='color:#D32F2F; font-weight:bold;'>{cleaned_entry}</span>" # Red for errors
        
        self.py_sim_action_log_output.append(cleaned_entry)
    
    # Auto-scroll
    scrollbar = self.py_sim_action_log_output.verticalScrollBar()
    if scrollbar:
        scrollbar.setValue(scrollbar.maximum())
    
    if log_entries:
        last_log_short = log_entries[-1].split('\n')[0][:100]
        significant_keywords = ["Transitioned from", "No eligible transition", "ERROR", "CRITICAL", "Reset.", "Started."]
        if any(keyword in last_log_short for keyword in significant_keywords):
    ...
    # In class MainWindow (inside cluade2.1.py)

    # ... (other methods like __init__, on_stop_py_simulation etc.) ...

    # --- NEW PYTHON SIMULATION METHODS --- # Make sure this whole section is present
    # ... (other py sim methods like _update_py_sim_status_display) ...
    ...
    def on_start_py_simulation(self):
        ...
        # Check for unsaved changes before starting simulation based on current diagram
        # This uses isWindowModified(), which should be set by scene.set_dirty()
        if self.isWindowModified(): # Using the standard Qt window modified flag
             reply = QMessageBox.question(self, "Unsaved Changes", 
                                          "The diagram has unsaved changes that won't be reflected in this simulation session unless you save. "
                                          "Start simulation with current in-memory diagram anyway?", 
        ...
        try:
            ...
            # Optionally register default/common mock functions here if needed globally
            # self.register_default_py_sim_mocks() # Example if you have such a helper

            self.set_ui_enabled_for_py_sim(True) # This sets self.py_sim_active and updates UI states
            
            initial_log = self.py_fsm_engine.get_last_executed_actions_log() # Get log from initial reset
            if initial_log: # Only append if there's something (e.g., from entry actions of initial state)
                 self._append_to_py_simulation_log(initial_log)
            
            self._update_py_simulation_dock_ui() # Update dock displays (current state, vars)
            self.log_message("Python FSM Simulation started.")
            QMessageBox.information(self, "Simulation Started", "Python FSM simulation has been started.")

        except FSMError as e:
            QMessageBox.critical(self, "FSM Initialization Error", f"Failed to start Python FSM simulation:\n{e}")
            if self.py_fsm_engine: # Should be None if instantiation failed, but for safety
    ...
    # ... (other methods like on_stop_py_simulation, on_reset_py_simulation, etc.) ...         

    # In class MainWindow (inside cluade2.1.py)

    # ... (other methods like __init__, on_start_py_simulation, on_stop_py_simulation, etc.) ...

    # --- NEW PYTHON SIMULATION METHODS ---
    # ... (other py sim methods) ...

    def on_reset_py_simulation(self):
        if not self.py_fsm_engine: # Check if simulation is running/engine exists
            QMessageBox.warning(self, "Simulation Not Active", 
                                "Python simulation is not running or has not been initialized. Start it first.")
            return
        try:
            self.py_fsm_engine.reset()
            if hasattr(self, 'py_sim_action_log_output'): # Check if UI element exists
                self.py_sim_action_log_output.clear() # Clear previous log on reset
            
            initial_log = self.py_fsm_engine.get_last_executed_actions_log()
            if initial_log:
                self._append_to_py_simulation_log(initial_log)
            
            self._update_py_simulation_dock_ui() # Update dock with new current state and cleared vars
            self.log_message("Python FSM Simulation reset to initial state.")
            QMessageBox.information(self, "Simulation Reset", "Python FSM simulation has been reset to its initial state.")

        except FSMError as e:
            QMessageBox.critical(self, "FSM Reset Error", f"Failed to reset Python FSM simulation:\n{e}")
            # Optionally stop simulation or try to recover
            self.on_stop_py_simulation(silent=True) # Stop sim if reset failed catastrophically
        except Exception as e:
            QMessageBox.critical(self, "Error", f"An unexpected error occurred during FSM reset:\n{e}")
            self.on_stop_py_simulation(silent=True)

    # ... (other methods like on_step_py_simulation, on_trigger_py_event, etc.) ...



    def on_stop_py_simulation(self, silent=False):
        if not self.py_sim_active and not silent :
            # Optional: Show message only if not called silently and sim is already stopped.
            # If you want a popup when the user tries to stop an already stopped simulation (and it wasn't a silent call)
            # if not silent: # Only show message if not a silent stop
            #     QMessageBox.information(self, "Simulation Not Active", "Python simulation is not currently running.")
            return # Silently return if already stopped or called silently and stopped

        self.py_fsm_engine = None # Crucial: Release the engine instance
        self.set_ui_enabled_for_py_sim(False) # This will set self.py_sim_active = False and update UI
        
        # These updates should be part of set_ui_enabled_for_py_sim or called by it.
        # Forcing them here as well to be certain after UI state change.
        self._update_py_simulation_dock_ui() # Clears displays like current state, variables
        if hasattr(self, 'py_sim_action_log_output'): # Check if UI element exists
            self.py_sim_action_log_output.clear() # Clear the sim log in the PySimDock
        self._highlight_sim_active_state(None) # Ensure no state remains highlighted

        if not silent:
            self.log_message("Python FSM Simulation stopped.")
            # Consider if a pop-up is always needed, or if log message is enough
            # QMessageBox.information(self, "Simulation Stopped", "Python FSM simulation has been stopped.")
