...
class AIChatUIManager(QObject): # This is the class we are modifying
    ...
    def _connect_actions_to_manager_slots(self):
        ...
        if hasattr(self.mw, 'openai_settings_action'): # This is the action name in main.py (via UIManager)
            logger.debug("AIChatUI: Found 'openai_settings_action', connecting to on_gemini_settings.") # ADD THIS LINE
            self.mw.openai_settings_action.triggered.connect(self.on_gemini_settings)
        else:
            logger.error("AIChatUIManager: Could not find 'openai_settings_action' on MainWindow to connect.") # Ensure this isn't printed

        if hasattr(self.mw, 'ask_ai_to_generate_fsm_action'):
            logger.debug("AIChatUI: Found 'ask_ai_to_generate_fsm_action', connecting to on_ask_ai_to_generate_fsm.") # ADD THIS LINE
            self.mw.ask_ai_to_generate_fsm_action.triggered.connect(self.on_ask_ai_to_generate_fsm)
        else:
            logger.error("AIChatUIManager: Could not find 'ask_ai_to_generate_fsm_action' on MainWindow to connect.")

        if hasattr(self.mw, 'clear_ai_chat_action'):
            logger.debug("AIChatUI: Found 'clear_ai_chat_action', connecting to on_clear_ai_chat_history.") # ADD THIS LINE
            self.mw.clear_ai_chat_action.triggered.connect(self.on_clear_ai_chat_history)
        else:
            logger.error("AIChatUIManager: Could not find 'clear_ai_chat_action' on MainWindow to connect.")

    @pyqtSlot()
    def on_gemini_settings(self): 
        logger.info("AIChatUI: SLOT on_gemini_settings CALLED!") # Make it more prominent
        if not self.mw.ai_chatbot_manager:
            logger.warning("AIChatUI: AI Chatbot Manager not available for settings within on_gemini_settings.") # ADD THIS LINE
