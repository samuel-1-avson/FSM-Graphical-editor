...
class UIManager(QObject): 
    ...
    def _create_toolbars(self):
        ...
        mw.edit_toolbar.addAction(mw.select_all_action) 
        ...
        mw.design_toolbar.addAction(mw.align_left_action); mw.design_toolbar.addAction(mw.align_center_h_action); mw.design_toolbar.addAction(mw.align_right_action)
        mw.design_toolbar.addAction(mw.align_top_action); mw.design_toolbar.addAction(mw.align_middle_v_action); mw.design_toolbar.addAction(mw.align_bottom_action)
        mw.design_toolbar.addSeparator()
        mw.design_toolbar.addAction(mw.distribute_h_action); mw.design_toolbar.addAction(mw.distribute_v_action)

        mw.view_control_toolbar = mw.addToolBar("View Controls") 
        ...
        mw.view_control_toolbar.addAction(mw.zoom_in_action); mw.view_control_toolbar.addAction(mw.zoom_out_action); mw.view_control_toolbar.addAction(mw.reset_zoom_action)
        mw.view_control_toolbar.addSeparator()
        mw.view_control_toolbar.addAction(mw.zoom_to_selection_action); mw.view_control_toolbar.addAction(mw.fit_diagram_action)
        
        mw.simulation_main_toolbar = mw.addToolBar("Simulation") 
        ...
        mw.simulation_main_toolbar.addAction(mw.start_py_sim_action); mw.simulation_main_toolbar.addAction(mw.stop_py_sim_action); mw.simulation_main_toolbar.addAction(mw.reset_py_sim_action)
        mw.simulation_main_toolbar.addSeparator()
        mw.simulation_main_toolbar.addAction(mw.run_simulation_action)

        mw.export_main_toolbar = mw.addToolBar("Export & Code Generation") 
        ...
        mw.export_main_toolbar.addAction(mw.generate_c_code_action); mw.export_main_toolbar.addAction(mw.generate_matlab_code_action)
        mw.export_main_toolbar.addSeparator()
        mw.export_main_toolbar.addAction(mw.export_plantuml_action); mw.export_main_toolbar.addAction(mw.export_mermaid_action)
    ...
    def _create_docks(self):
        ...
        if hasattr(mw, 'view_menu'): 
            if not hasattr(mw, 'toolbars_menu'): 
                 mw.view_menu.addSeparator() 
    ...
    def _load_and_display_templates(self):
        ...
        for template_info in templates_to_load:
            ...
            template_btn.setIconSize(QSize(24,24)) 
    ...
    def _create_status_bar(self):
        ...
        # Main operational status label (stretchable)
        mw.main_op_status_label = QLabel("Ready"); 
        mw.main_op_status_label.setObjectName("MainOpStatusLabel") # For potential specific styling
        mw.status_bar.addWidget(mw.main_op_status_label, 1) # Stretch factor 1

        # IDE file status (permanent, fixed size)
        mw.ide_file_status_label = QLabel("IDE: Idle")
        mw.ide_file_status_label.setObjectName("IdeFileStatusLabel")
        mw.ide_file_status_label.setMinimumWidth(120) # Adjust as needed
        ...
        logger.debug("UIManager: Status bar created with new layout.")
