...
from resource_monitor import ResourceMonitorManager # Corrected import
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.resource_monitor_manager = ResourceMonitorManager(self) # Instantiate
        ...
        if self.resource_monitor_manager: # Start monitor after signals are connected
    ...
    def _connect_signals(self):
        ...
        if hasattr(self, 'properties_edit_dialog_button'):
            # Ensure only one connection or define behavior if multiple items could trigger it
            # Disconnect first if re-connecting to be safe, though usually not necessary if connected once.
            try: self.properties_edit_dialog_button.clicked.disconnect() 
            except TypeError: pass # No connections to disconnect
    ...
    def _update_resource_display(self, cpu_usage, ram_usage, gpu_util, gpu_name):
        ...
        if hasattr(self, 'gpu_status_label'):
            if gpu_util == -1.0: self.gpu_status_label.setText(f"GPU: {gpu_name}") # N/A or pynvml not available
            elif gpu_util == -2.0: self.gpu_status_label.setText(f"GPU: NVML Err") # Error reading from NVML
            elif gpu_util == -3.0: self.gpu_status_label.setText(f"GPU: Mon Err") # General monitoring error
            ...
            else: # Fallback if NVML init failed but was expected
    ...
    def _handle_py_sim_global_ui_enable_by_manager(self, enable: bool):
        ...
        if hasattr(self, 'properties_apply_button'): self.properties_apply_button.setEnabled(False) # Always disable on global UI change
        if hasattr(self, 'properties_revert_button'): self.properties_revert_button.setEnabled(False) # Always disable
        for item in self.scene.items():
    ...
    def _on_apply_dock_properties(self):
        ...
        if hasattr(self._current_edited_item_in_dock, 'get_data'): # Refresh original props for next revert
    ...
    def _on_revert_dock_properties(self):
        ...
        # Block signals from editors during revert to prevent _on_dock_property_changed_mw from firing
