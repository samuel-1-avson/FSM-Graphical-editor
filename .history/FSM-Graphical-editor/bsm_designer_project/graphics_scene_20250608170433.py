...
from config import ( # Keep specific constants if used as fallbacks or for non-themed parts
...
from settings_manager import SettingsManager # For visual property defaults


logger = logging.getLogger(__name__)

SNAP_THRESHOLD_PIXELS = 8
# GUIDELINE_PEN_COLOR will now be read from config.COLOR_SNAP_GUIDELINE
...
class DiagramScene(QGraphicsScene):
    ...
    def __init__(self, undo_stack, parent_window=None, custom_snippet_manager: CustomSnippetManager | None = None):
        ...
        self.grid_size = 20 # Could also be a setting from SettingsManager

        # Pens and background brush are now initialized using dynamic colors from config
        ...
        # These will be updated by MainWindow based on settings
    ...
    def log_function(self, message: str, level: str = "ERROR"): # Keep for SubFSMEditorDialog
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        # ... (Updated in previous response to pass visual properties to item constructors)
        ...
        if item_type == "State":
            ...
            initial_dialog_props = {
                ...
                'shape_type': initial_data.get('shape_type', config.DEFAULT_STATE_SHAPE),
                'font_family': initial_data.get('font_family', config.APP_FONT_FAMILY),
                'font_size': initial_data.get('font_size', 10),
                'font_bold': initial_data.get('font_bold', True),
                'font_italic': initial_data.get('font_italic', False),
                'border_style_str': SettingsManager.QT_PEN_STYLE_TO_STRING.get(initial_data.get('border_style_qt', config.DEFAULT_STATE_BORDER_STYLE), "Solid"),
                'border_width': initial_data.get('border_width', config.DEFAULT_STATE_BORDER_WIDTH),
            ...
            initial_comment_props = {
                'text': initial_text,
                'font_family': initial_data.get('font_family', config.APP_FONT_FAMILY),
                'font_size': initial_data.get('font_size', 9),
                'font_italic': initial_data.get('font_italic', True)
            }
    ...
    def _handle_transition_click(self, clicked_state_item: GraphicsStateItem, click_pos: QPointF):
        ...
        if not self.transition_start_item:
            ...
            initial_props = {
                ...
                'line_width': config.DEFAULT_TRANSITION_LINE_WIDTH,
                'arrowhead_style': config.DEFAULT_TRANSITION_ARROWHEAD,
                'label_font_family': config.APP_FONT_FAMILY,
                'label_font_size': 8
            }
    ...
    def get_diagram_data(self):
        # ... (remains the same, relies on item.get_data() which was updated) ...
    ...
    def load_diagram_data(self, data):
        # ... (Updated in previous response to handle new visual properties from file) ...
        self.clear() 
        self._problematic_items.clear() 
        self._validation_issues = []
        ...
        for state_data in data.get('states', []):
            state_item = GraphicsStateItem(
                ...
                shape_type=state_data.get('shape_type'),
                font_family=state_data.get('font_family'),
                font_size=state_data.get('font_size'),
                font_bold=state_data.get('font_bold'),
                font_italic=state_data.get('font_italic'),
                border_style_qt=SettingsManager.STRING_TO_QT_PEN_STYLE.get(state_data.get('border_style_str')), 
                custom_border_width=state_data.get('border_width'),
        ...
        for trans_data in data.get('transitions', []):
            ...
            if src_item and tgt_item:
                trans_item = GraphicsTransitionItem(
                    ...
                    line_style_qt=SettingsManager.STRING_TO_QT_PEN_STYLE.get(trans_data.get('line_style_str')),
                    custom_line_width=trans_data.get('line_width'),
                    arrowhead_style=trans_data.get('arrowhead_style'),
                    label_font_family=trans_data.get('label_font_family'),
                    label_font_size=trans_data.get('label_font_size')
                )
        ...
        for comment_data in data.get('comments', []):
            comment_item = GraphicsCommentItem(
                ...
                font_family=comment_data.get('font_family'),
                font_size=comment_data.get('font_size'),
                font_italic=comment_data.get('font_italic')
            )
    ...
    def copy_selected_items(self):
        # ... (remains the same, relies on item.get_data()) ...
        ...
        for i, item_info in enumerate(items_to_paste_data):
            ...
            if item_type == "State":
                ...
                new_item = GraphicsStateItem(
                    ...
                    border_style_qt=SettingsManager.STRING_TO_QT_PEN_STYLE.get(data.get('border_style_str')),
                    custom_border_width=data.get('border_width'),
