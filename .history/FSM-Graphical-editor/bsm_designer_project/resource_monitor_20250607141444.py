            ...
            if (current_loop_time_ms - last_data_emit_time_ms) >= self.data_collection_interval_ms or last_data_emit_time_ms == 0:
                ...
                try:
                    ...
                    if self._nvml_initialized and self._gpu_handle and PYNVML_AVAILABLE and pynvml:
                        ...
                        self._attempt_nvml_init(from_worker_loop=True) # Attempt init
...
class ResourceMonitorManager(QObject):
    def __init__(self, main_window_ref, parent=None):
        ...
        # setup_and_start_monitor will be called by MainWindow
    ...
    def setup_and_start_monitor(self):
        ...
        self.worker = ResourceMonitorWorker(interval_ms=2000)
        self.worker.moveToThread(self.thread)
        
        self.thread.started.connect(self.worker.start_monitoring)
        # Connect finished_signal from worker to QThread.quit and then to manager's cleanup
        self.worker.finished_signal.connect(self.thread.quit)
        self.worker.finished_signal.connect(self._handle_worker_thread_finished) # For potential additional cleanup
