...
class AIChatUIManager(QObject): # This is the class we are modifying
    ...
    def _connect_actions_to_manager_slots(self):
        ...
        if hasattr(self.mw, 'openai_settings_action'): # This is the action name in main.py (via UIManager)
            logger.debug("AIChatUI: Found 'openai_settings_action', connecting to on_gemini_settings.")
            self.mw.openai_settings_action.triggered.connect(self.on_gemini_settings)
        else:
            logger.error("AIChatUIManager: Could not find 'openai_settings_action' on MainWindow to connect.")
    ...
    def _markdown_to_html_basic(self, text_part: str) -> str:
        ...
        for line in text_part.splitlines(keepends=True):
            ...
            if match:
                if not in_list: # Start of a new list
                    if current_buffer: processed_text_parts_for_list.append(current_buffer)
                    current_buffer = ""
                    processed_text_parts_for_list.append("<ul>")
                    in_list = True
                list_items_html.append(f"<li>{match.group(1).strip()}</li>")
            else: # Not a list item
                if in_list: # End of a list
        ...
        if in_list: # Close any open list at the end
            processed_text_parts_for_list.append("".join(list_items_html))
            processed_text_parts_for_list.append("</ul>")
        elif current_buffer: # Append any remaining non-list text
