...
if __name__ == '__main__' and __package__ is None:
    # This ensures that when main.py is run directly, Python can find other modules
    # in the bsm_designer_project package using relative imports.
    
    # Get the absolute path to the directory containing main.py (bsm_designer_project)
    current_script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Get the parent directory of current_script_dir (e.g., FSM-Graphical-editor)
    # This is the directory that should be on sys.path for bsm_designer_project to be importable.
    project_root = os.path.dirname(current_script_dir)
    
    if project_root not in sys.path:
        sys.path.insert(0, project_root)
        # print(f"DEBUG: Added to sys.path: {project_root}") # For debugging

    # Now, we can re-import main as part of the package, which allows relative imports
    # within the package to work correctly.
    # This effectively restarts the script's context as if it were loaded as a module.
    import bsm_designer_project.main # Re-import self
    sys.exit(bsm_designer_project.main.main_entry_point()) # Call the defined entry point
...
# --- Custom Modules (use relative imports now that main is part of a package context) ---
...
from . import config  # This makes 'config' available as the module object.
from .config import ( # Import specific non-dynamic constants and functions
    ...
    # Removed all COLOR_XXX constants and the problematic 'config' name itself from this list.
    # Dynamic colors will be accessed via `config.COLOR_XXX`.
)
...
class MainWindow(QMainWindow):
    # ... (The entire MainWindow class content remains unchanged from the previous version) ...
    # ... (It's quite long, so I'm omitting it here for brevity, but it's the same as before) ...
    ...
    def _apply_theme(self, theme_name: str):
        """Applies the selected theme by updating dynamic colors and stylesheet."""
        ...
        if app_instance: # Ensure app instance exists
            app_instance.setStyleSheet(new_stylesheet)
        else:
            logger.error("Cannot apply theme: QApplication instance not found.")
            return # Cannot proceed without app instance
        
        self.update()
        self.repaint()
        
        # Force children to re-evaluate style (more robust)
        all_widgets = self.findChildren(QWidget) # Find all QWidgets
        if self.menuBar(): all_widgets.append(self.menuBar()) # Include menubar
        if self.statusBar(): all_widgets.append(self.statusBar()) # Include statusbar

        for child_widget in all_widgets:
            if child_widget: # Check if widget still exists
                # Forcing a re-polish can help QSS changes propagate more reliably
                child_widget.style().unpolish(child_widget)
                child_widget.style().polish(child_widget)
                child_widget.update() # Request a repaint
    ...
    def _apply_initial_settings(self):
        ...
        if self.resource_monitor_manager:
            if self.resource_monitor_manager.worker: # Check if worker exists
    ...
    def _handle_setting_changed(self, key: str, value: object):
        ...
        if key == "appearance_theme":
            ...
            theme_related_change = True # Mark that theme has changed
        ...
        if theme_related_change:
            # No need to re-apply stylesheet here directly if _apply_theme was called
            # _apply_theme already handles it.
            # If only canvas colors changed without a full theme name change,
            # we might need to ensure config colors are updated and scene repaints.
            if key != "appearance_theme": # If it wasn't a full theme change that already did this
                new_stylesheet = GET_CURRENT_STYLE_SHEET() # Ensure QSS reflects latest config colors
                app_instance = QApplication.instance()
...
# This function will be called by the sys.path modification block if main.py is run directly
def main_entry_point():
    if hasattr(Qt, 'AA_EnableHighDpiScaling'): QApplication.setAttribute(Qt.AA_EnableHighDpiScaling, True)
    if hasattr(Qt, 'AA_UseHighDpiPixmaps'): QApplication.setAttribute(Qt.AA_UseHighDpiPixmaps, True)
    
    app = QApplication(sys.argv)
    # SettingsManager is now created and attached to app in MainWindow.__init__
    # (or here if MainWindow isn't guaranteed to be the first to access it)
    if not hasattr(QApplication.instance(), 'settings_manager'):
         QApplication.instance().settings_manager = SettingsManager(app_name=APP_NAME)
    
    app.setApplicationName(APP_NAME)
    app.setApplicationVersion(APP_VERSION)
    # Stylesheet is applied dynamically in MainWindow._apply_theme after theme colors are set
...
if __name__ == '__main__':
    # The sys.path modification block at the top handles the direct execution case by
    # re-importing and calling main_entry_point.
    # If that block is not present (e.g., when run with `python -m`),
    # this traditional __main__ block would execute.
    # However, with the current setup, this block is effectively bypassed when run directly.
    # If you *only* wanted to support `python -m bsm_designer_project.main`, you would remove
    # the sys.path hack and just call main_entry_point() here.
    pass