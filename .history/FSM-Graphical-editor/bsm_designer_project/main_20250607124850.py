...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ide_manager = IDEManager(self) # Handles IDE dock content and logic
        ...
        self.ai_chat_ui_manager = AIChatUIManager(self) # UI manager for AI Chat (lives on MainWindow)
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        # This method is called by AIChatUIManager, so it remains in MainWindow
        ...
        if clear_current_diagram:
            if not self.action_handler.on_new_file(silent=True): logger.warning("MW: Clearing diagram cancelled by user (save prompt). Cannot add AI FSM."); return
    ...
    def _fit_view_to_new_ai_items(self):
        # This method is called after AI FSM generation, so it remains in MainWindow
    ...
    def _update_window_title(self):
        ...
        if self.ide_manager: # Check if ide_manager exists
            ...
            if hasattr(self, 'ide_dock'): self.ide_dock.setWindowTitle(ide_dock_title) # ide_dock is on self.mw
    ...
    def _update_properties_dock(self): # Called by scene selectionChanged
        ...
        if hasattr(self.ui_manager, 'properties_editor_layout') and hasattr(self.ui_manager, '_dock_property_editors'):
            while self.ui_manager.properties_editor_layout.count():
                child_layout_item = self.ui_manager.properties_editor_layout.takeAt(0)
            ...
            self.ui_manager._dock_property_editors.clear()
            self.ui_manager._current_edited_item_in_dock = None
            if len(selected_items) == 1:
                self.ui_manager._current_edited_item_in_dock = selected_items[0]
                item_data = self.ui_manager._current_edited_item_in_dock.get_data() if hasattr(self.ui_manager._current_edited_item_in_dock, 'get_data') else {}
                self.ui_manager.properties_editor_container.setVisible(True)
                self.ui_manager.properties_placeholder_label.setVisible(False)
                self.ui_manager.properties_edit_dialog_button.setEnabled(True)
                if isinstance(self.ui_manager._current_edited_item_in_dock, GraphicsStateItem):
                    name_edit = QLineEdit(item_data.get('name', '')); name_edit.textChanged.connect(self._on_dock_property_changed_mw)
                    self.ui_manager.properties_editor_layout.addRow("Name:", name_edit); self.ui_manager._dock_property_editors['name'] = name_edit
                elif isinstance(self.ui_manager._current_edited_item_in_dock, GraphicsTransitionItem):
                    event_edit = QLineEdit(item_data.get('event', '')); event_edit.textChanged.connect(self._on_dock_property_changed_mw)
                    self.ui_manager.properties_editor_layout.addRow("Event:", event_edit); self.ui_manager._dock_property_editors['event'] = event_edit
                elif isinstance(self.ui_manager._current_edited_item_in_dock, GraphicsCommentItem):
                    ...
                    self.ui_manager.properties_editor_layout.addRow("Text:", text_edit); self.ui_manager._dock_property_editors['text'] = text_edit
                else:
                     self.ui_manager.properties_placeholder_label.setText(f"<i>Editing: {type(self.ui_manager._current_edited_item_in_dock).__name__}.<br>Dock editor UI needs full implementation. Use 'Advanced Edit...'</i>")
                     self.ui_manager.properties_editor_container.setVisible(False); self.ui_manager.properties_placeholder_label.setVisible(True)
            elif len(selected_items) > 1:
                self.ui_manager.properties_placeholder_label.setText(f"<i><b>{len(selected_items)} items selected.</b><br><span style='font-size:{APP_FONT_SIZE_SMALL}; color:{COLOR_TEXT_SECONDARY};'>Select a single item to edit properties.</span></i>")
                self.ui_manager.properties_editor_container.setVisible(False); self.ui_manager.properties_placeholder_label.setVisible(True); self.ui_manager.properties_edit_dialog_button.setEnabled(False)
            else:
                self.ui_manager.properties_placeholder_label.setText(f"<i>No item selected.</i><br><span style='font-size:{APP_FONT_SIZE_SMALL}; color:{COLOR_TEXT_SECONDARY};'>Click an item or use tools to add elements.</span>")
                self.ui_manager.properties_editor_container.setVisible(False); self.ui_manager.properties_placeholder_label.setVisible(True); self.ui_manager.properties_edit_dialog_button.setEnabled(False)
            self.ui_manager.properties_apply_button.setEnabled(False)
            self.ui_manager.properties_revert_button.setEnabled(False)

    def _on_dock_property_changed_mw(self): # Connects to dock editor signals
        self.ui_manager.properties_apply_button.setEnabled(True)
        self.ui_manager.properties_revert_button.setEnabled(True)
