...
class UIManager(QObject): 
    ...
    def _create_status_bar_segment(self, icon_enum, icon_alt, initial_text, tooltip, object_name=None, min_width=60) -> tuple[QWidget, QLabel, QLabel]:
        ...
        segment_layout.setContentsMargins(2, 0, 2, 0) # Compact margins
        segment_layout.setSpacing(2) # Compact spacing

        icon_label = QLabel()
        icon = get_standard_icon(icon_enum, icon_alt)
        if not icon.isNull():
            icon_label.setPixmap(icon.pixmap(QSize(12,12))) # Small icon
        ...
        text_label.setMinimumWidth(min_width - 16) # Adjust for icon
        text_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        # Styling will be handled by global QSS or specific object name QSS
    ...
    def _create_status_bar(self):
        ...
        zoom_segment, mw.zoom_icon_label, mw.zoom_status_label = self._create_status_bar_segment(
            QStyle.SP_FileDialogContentsView, "Zoom", "Zoom: 100%", "Current Zoom Level", "ZoomStatusLabel", 70)
        mw.status_bar.addPermanentWidget(zoom_segment)
        
        # Resource Monitors (wrapper widget to hold them together if desired)
        mw.resource_monitor_widget = QWidget()
        resource_layout = QHBoxLayout(mw.resource_monitor_widget)
        resource_layout.setContentsMargins(0,0,0,0); resource_layout.setSpacing(4)

        cpu_segment, mw.cpu_icon_label, mw.cpu_status_label = self._create_status_bar_segment(
            QStyle.SP_ComputerIcon, "CPU", "CPU: --%", "CPU Usage", "CpuStatusLabel", 60)
        resource_layout.addWidget(cpu_segment)
        
        ram_segment, mw.ram_icon_label, mw.ram_status_label = self._create_status_bar_segment(
            QStyle.SP_DriveHDIcon, "RAM", "RAM: --%", "RAM Usage", "RamStatusLabel", 60) # Using DriveHDIcon as a placeholder
        resource_layout.addWidget(ram_segment)

        gpu_segment, mw.gpu_icon_label, mw.gpu_status_label = self._create_status_bar_segment(
             _safe_get_style_enum("SP_MediaVolume", "SP_ComputerIcon"), "GPU", "GPU: N/A", "GPU Usage", "GpuStatusLabel", 60) # SP_MediaVolume is a placeholder
        ...
        pysim_segment, mw.pysim_icon_label, mw.py_sim_status_label = self._create_status_bar_segment(
            QStyle.SP_MediaPlay, "PySim", "PySim: Idle", "Python Simulation Status", "PySimStatusLabel", 100)
        ...
        matlab_segment, mw.matlab_icon_label, mw.matlab_status_label = self._create_status_bar_segment(
            QStyle.SP_ComputerIcon, "MATLAB", "MATLAB: Init...", "MATLAB Connection Status", "MatlabStatusLabel", 110)
        ...
        net_segment, mw.net_icon_label, mw.internet_status_label = self._create_status_bar_segment(
            QStyle.SP_DriveNetIcon, "Net", "Net: Init...", "Internet Connectivity", "InternetStatusLabel", 80)
        ...
        logger.debug("UIManager: Status bar created with enhanced segments.")
