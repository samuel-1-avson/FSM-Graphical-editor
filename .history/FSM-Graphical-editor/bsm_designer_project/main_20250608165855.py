...
try:
    import resources_rc # Keep this import at this level
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        # Ensure settings_manager is available globally via QApplication instance for other modules
        ...
        self._apply_initial_settings() # Applies theme and other settings
        ...
        # self.setWindowTitle already called by _update_window_title via _apply_initial_settings

    def _apply_theme(self, theme_name: str):
        """Applies the selected theme by updating dynamic colors and stylesheet."""
        logger.info(f"Applying theme: {theme_name}")
        DYNAMIC_UPDATE_COLORS_FROM_THEME(theme_name) # This updates colors in config.py
        
        # Update scene related colors directly as QSS doesn't easily affect QGraphicsScene drawing
        if hasattr(self, 'scene') and self.scene:
            ...
            # Snap guideline color is now read from config.COLOR_SNAP_GUIDELINE in scene's drawForeground
            self.scene.update()
        
        new_stylesheet = GET_CURRENT_STYLE_SHEET() # This gets the dynamically generated QSS
        QApplication.instance().setStyleSheet(new_stylesheet)
        
        # To ensure all widgets correctly pick up the new stylesheet,
        # sometimes a more forceful update is needed.
        self.style().unpolish(self)
        self.style().polish(self)
        self.update()
        # Force children to re-evaluate style
    ...
    def _connect_signals(self): 
        ...
        # Connect to settings_manager's signal
    ...
    def _apply_initial_settings(self):
        ...
        # Theme (must be applied first to get correct dynamic colors for other settings)
        initial_theme = self.settings_manager.get("appearance_theme")
        self._apply_theme(initial_theme) # This will also call DYNAMIC_UPDATE_COLORS & setStyleSheet

        # View settings
        if hasattr(self.scene, 'setBackgroundBrush'):
            show_grid = self.settings_manager.get("view_show_grid")
            # Action's checked state will be connected to the setting directly by ActionHandler
        ...
        # Resource Monitor
        if self.resource_monitor_manager:
            enabled = self.settings_manager.get("resource_monitor_enabled")
            ...
            # Visibility handled by __init__ based on settings

        self._update_window_title() # Ensure title is correct after initial settings
    ...
    def _handle_setting_changed(self, key: str, value: object):
        ...
        if key == "appearance_theme":
            ...
            # The DYNAMIC_UPDATE_COLORS_FROM_THEME is primarily for global QSS variables.
            # For these specific canvas colors, we need to ensure config module is updated,
            # then scene is told to re-read/repaint.
            current_theme = self.settings_manager.get("appearance_theme") # Get current theme
            DYNAMIC_UPDATE_COLORS_FROM_THEME(current_theme) # Re-evaluate all dynamic colors based on current theme
            theme_related_change = True # Treat as theme related to force QSS re-application
            if self.scene: self.scene.update() # Force scene repaint

        if theme_related_change:
            # Re-apply the global stylesheet because underlying config colors changed
            new_stylesheet = GET_CURRENT_STYLE_SHEET()
            QApplication.instance().setStyleSheet(new_stylesheet)
            self.update()
            self.repaint()
            # Propagate style changes to children more reliably
            for child_widget in self.findChildren(QWidget):
                child_widget.style().unpolish(child_widget)
                child_widget.style().polish(child_widget)
                child_widget.update()
        ...
        if key == "view_show_grid":
            ...
            if self.scene: self.scene.update() # Scene drawBackground will use its internal flag now
        ...
        self._update_window_title() # Update title in case theme affects it or status messages change
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str | None = None):
        ...
        for i, state_data in enumerate(states_data):
            ...
            # Use dynamic config colors for defaults
            state_item = GraphicsStateItem(
                ...
                # Visual properties will be set by item's __init__ using settings or defaults
            )
        ...
        for trans_data in transitions_data:
            ...
            if src_item and tgt_item:
                # Use dynamic config color for default
                trans_item = GraphicsTransitionItem(
                    ...
                    # Visual properties from item's __init__
                )
        ...
        for i, comment_data in enumerate(comments_data):
            ...
            comment_item = GraphicsCommentItem(pos_x, pos_y, text) # Visuals from item's __init__
...
if __name__ == '__main__':
    ...
    # Stylesheet is now applied dynamically in MainWindow after theme colors are set
