...
if __name__ == '__main__' and __package__ is None:
    ...
    import bsm_designer_project.main 
...
from PyQt5.QtWidgets import (
    ...
    QSpinBox, QComboBox, QGraphicsRectItem, QGraphicsPathItem, QDialogButtonBox, # Added QSpinBox, QComboBox
...
import pygraphviz as pgv # Keep for future potential use
...
from .utils import get_standard_icon # Import for icon usage
...
class MainWindow(QMainWindow):
    ...
    def _update_properties_dock(self):
        ...
        if len(selected_items) == 1:
            ...
            if isinstance(self._current_edited_item_in_dock, GraphicsStateItem):
                ...
                # --- MODIFIED SECTION for StateItem Properties Dock ---
                font_size_spin = QSpinBox()
                font_size_spin.setRange(6, 72)
                font_size_spin.setValue(item_data.get('font_size', 10))
                font_size_spin.valueChanged.connect(self._on_dock_property_changed_mw)
                self.properties_editor_layout.addRow("Font Size:", font_size_spin)
                self._dock_property_editors['font_size'] = font_size_spin

                border_width_spin = QDoubleSpinBox()
                border_width_spin.setRange(0.5, 10.0)
                border_width_spin.setSingleStep(0.1)
                border_width_spin.setDecimals(1)
                border_width_spin.setValue(item_data.get('border_width', config.DEFAULT_STATE_BORDER_WIDTH))
                border_width_spin.valueChanged.connect(self._on_dock_property_changed_mw)
                self.properties_editor_layout.addRow("Border Width:", border_width_spin)
                self._dock_property_editors['border_width'] = border_width_spin
                # --- END MODIFIED SECTION for StateItem ---
                ...
                # --- MODIFIED SECTION for TransitionItem Properties Dock ---
                control_offset_x_spin = QSpinBox()
                control_offset_x_spin.setRange(-1000, 1000)
                control_offset_x_spin.setValue(int(item_data.get('control_offset_x', 0)))
                control_offset_x_spin.valueChanged.connect(self._on_dock_property_changed_mw)
                self.properties_editor_layout.addRow("Curve Bend (Perp):", control_offset_x_spin) # Changed Label
                self._dock_property_editors['control_offset_x'] = control_offset_x_spin

                control_offset_y_spin = QSpinBox()
                control_offset_y_spin.setRange(-1000, 1000)
                control_offset_y_spin.setValue(int(item_data.get('control_offset_y', 0)))
                control_offset_y_spin.valueChanged.connect(self._on_dock_property_changed_mw)
                self.properties_editor_layout.addRow("Curve Shift (Tang):", control_offset_y_spin) # Changed Label
                self._dock_property_editors['control_offset_y'] = control_offset_y_spin
                # --- END MODIFIED SECTION for TransitionItem ---
    ...
    def _on_apply_dock_properties(self):
        ...
        if current_item_type is GraphicsStateItem:
            ...
            # --- MODIFIED: Read from new SpinBoxes for StateItem ---
            if 'font_size' in self._dock_property_editors: new_props_from_dock['font_size'] = self._dock_property_editors['font_size'].value()
            if 'border_width' in self._dock_property_editors: new_props_from_dock['border_width'] = self._dock_property_editors['border_width'].value()
            # --- END MODIFIED ---
            ...
            # --- MODIFIED: Read from new SpinBoxes for TransitionItem ---
            if 'control_offset_x' in self._dock_property_editors: new_props_from_dock['control_offset_x'] = self._dock_property_editors['control_offset_x'].value()
            if 'control_offset_y' in self._dock_property_editors: new_props_from_dock['control_offset_y'] = self._dock_property_editors['control_offset_y'].value()
            # --- END MODIFIED ---
            if 'color_button' in self._dock_property_editors: new_props_from_dock['color'] = self._dock_property_editors['color_button'].property("currentColorHex")
        ...
        for key, editor_widget in self._dock_property_editors.items():
            ...
            elif isinstance(editor_widget, (QSpinBox, QDoubleSpinBox)): new_val = editor_widget.value() # MODIFIED: Read value from SpinBox
    ...
    def _on_revert_dock_properties(self):
        ...
        if current_item_type is GraphicsStateItem:
            ...
            # --- MODIFIED: Set values for new SpinBoxes in StateItem ---
            if 'font_size' in self._dock_property_editors: self._dock_property_editors['font_size'].setValue(original_props.get('font_size', 10))
            if 'border_width' in self._dock_property_editors: self._dock_property_editors['border_width'].setValue(original_props.get('border_width', config.DEFAULT_STATE_BORDER_WIDTH))
            # --- END MODIFIED ---
            ...
            # --- MODIFIED: Set values for new SpinBoxes in TransitionItem ---
            if 'control_offset_x' in self._dock_property_editors: self._dock_property_editors['control_offset_x'].setValue(int(original_props.get('control_offset_x', 0)))
            if 'control_offset_y' in self._dock_property_editors: self._dock_property_editors['control_offset_y'].setValue(int(original_props.get('control_offset_y', 0)))
            # --- END MODIFIED ---
            if 'color_button' in self._dock_property_editors:
    ...
    def __init__(self):
        ...
        # self.setWindowTitle(f"{APP_NAME} - Untitled") # Title is set in _update_window_title with placeholder
        ...
        if self.settings_manager.get("resource_monitor_enabled"):
            ...
            # Resource monitor labels visibility is handled in _update_resource_display and _handle_setting_changed
            if hasattr(self, 'resource_monitor_widget'): self.resource_monitor_widget.setVisible(True)
        else:
    ...
    def _connect_signals(self): 
        ...
        self.scene.interaction_mode_changed.connect(self._on_interaction_mode_changed_by_scene) # New connection
        ...
        if hasattr(self, 'save_perspective_action'):
            self.save_perspective_action.triggered.connect(self.save_current_perspective_as)
        if hasattr(self, 'reset_perspectives_action'):
    ...
    def _set_status_label_object_names(self):
        ...
        # Icon labels are not directly updated with text, but their parent segment widgets are
        # No need to set object names for icon_labels if they aren't styled or accessed by name
        if hasattr(self, 'mode_status_label'): self.mode_status_label.setObjectName("InteractionModeStatusLabel") # Added for new mode label
    ...
    def _update_matlab_status_display(self, connected, message):
        ...
        if hasattr(self, 'matlab_status_label') and self.matlab_status_label:
            ...
            # Style is now better handled by QSS targeting objectName, but can be overridden if needed
            # self.matlab_status_label.setStyleSheet(f"color:{text_color};")
    ...
    def update_zoom_status_display(self, scale_factor: float):
        if hasattr(self, 'zoom_status_label') and self.zoom_status_label:
            ...
            self.zoom_status_label.setText(f"{zoom_percentage}%") # Icon handled separately now
    ...
    def _update_window_title(self):
        ...
        if self.ide_manager:
            ...
            if self.ide_manager.current_ide_file_path:
                ...
                ide_status_bar_text = f"{ide_fn}{ide_dirty_char_dock}"
                ...
                ide_status_bar_text = f"Untitled Script{ide_dirty_char_dock}"
            else: 
            ...
            if hasattr(self, 'ide_file_status_label') and self.ide_file_status_label:
                 self.ide_file_status_label.setText(ide_status_bar_text)
                 self.ide_file_status_label.parentWidget().setToolTip(f"IDE Script: {ide_status_bar_text}")
        ...
        final_title_string = " ".join(title_parts)
        self.setWindowTitle(f"{final_title_string}[*]") 
        self.setWindowModified(main_window_is_dirty) 

        main_op_text = f"File: {diagram_file_name}" 
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        full_status_text = f"Net: {message_detail}" # Text for the label
        if hasattr(self, 'internet_status_label') and self.internet_status_label:
            self.internet_status_label.setText(message_detail) # Only show detail
            host_for_tooltip = "8.8.8.8:53 (Google DNS)"
            self.internet_status_label.parentWidget().setToolTip(f"Internet Status: {message_detail} (Checks {host_for_tooltip})") # Tooltip on parent widget
            # Style is now primarily handled by QSS with objectName="InternetStatusLabel"
            # However, icon can be updated here
            if hasattr(self, 'net_icon_label') and self.net_icon_label:
    ...
    def _update_py_sim_status_display(self): 
        if hasattr(self, 'py_sim_status_label') and self.py_sim_status_label:
            ...
            # QSS will handle the text/background color styling based on PySimStatusLabel objectName
    ...
    def _populate_perspectives_menu(self):
        ...
        for action in self.perspectives_action_group.actions(): # Remove old actions from group
        ...
        if user_perspective_names:
            for p_name in sorted(user_perspective_names): # Sort custom names for consistency
                action = self.perspectives_menu.addAction(p_name)
                action.setCheckable(True)
                action.triggered.connect(lambda checked=False, name=p_name: self.apply_perspective(name))
                self.perspectives_action_group.addAction(action)
    ...
    def _update_current_perspective_check(self):
        ...
        if not found_current and self.perspectives_action_group.actions():
             # If current_perspective_name isn't in the menu (e.g., after reset), uncheck all
             pass
        ...
        if saved_state_hex and isinstance(saved_state_hex, str):
            try:
                ...
                if self.restoreState(state_data_bytes):
                    ...
                    logger.warning(f"Failed to restore layout from saved state for perspective: {perspective_name}. Falling back to default arrangement if known.")
            except ValueError:
                logger.error(f"Invalid hex string for perspective '{perspective_name}'. Using default arrangement.")
    ...
    def _apply_default_perspective_layout(self, perspective_name: str):
        ...
        # Make all docks non-floating before rearranging
        all_docks = [
        ...
        for dock in all_docks:
            if dock: 
                ...
                # Important: Ensure dock is added to main window if it wasn't already
                # This can happen if a saved state previously removed it entirely.
                # We only call addDockWidget if we know its intended area.
                # A simpler way is to hide all first, then show and place desired ones.
                dock.setVisible(False)


        if perspective_name == self.PERSPECTIVE_DESIGN_FOCUS:
            self.addDockWidget(Qt.LeftDockWidgetArea, self.elements_palette_dock); self.elements_palette_dock.setVisible(True)
            self.addDockWidget(Qt.RightDockWidgetArea, self.properties_dock); self.properties_dock.setVisible(True)
            self.addDockWidget(Qt.BottomDockWidgetArea, self.problems_dock); self.problems_dock.setVisible(True)
            self.addDockWidget(Qt.BottomDockWidgetArea, self.log_dock); self.log_dock.setVisible(True)
            self.tabifyDockWidget(self.problems_dock, self.log_dock) # Log next to problems
            self.properties_dock.raise_()

        elif perspective_name == self.PERSPECTIVE_SIMULATION_FOCUS:
            self.addDockWidget(Qt.RightDockWidgetArea, self.py_sim_dock); self.py_sim_dock.setVisible(True)
            self.addDockWidget(Qt.RightDockWidgetArea, self.properties_dock); self.properties_dock.setVisible(True) # Keep properties accessible
            self.tabifyDockWidget(self.py_sim_dock, self.properties_dock)
            self.addDockWidget(Qt.BottomDockWidgetArea, self.log_dock); self.log_dock.setVisible(True) # PySim outputs to main log for now
            self.py_sim_dock.raise_()

        elif perspective_name == self.PERSPECTIVE_IDE_FOCUS:
            self.addDockWidget(Qt.RightDockWidgetArea, self.ide_dock); self.ide_dock.setVisible(True)
            # It might be better to make IDE take most of the space or be central for this perspective.
            # For now, keeping it as a right dock.
            self.addDockWidget(Qt.BottomDockWidgetArea, self.problems_dock); self.problems_dock.setVisible(True)
            self.addDockWidget(Qt.BottomDockWidgetArea, self.log_dock); self.log_dock.setVisible(True)
            ...
            self.addDockWidget(Qt.RightDockWidgetArea, self.ai_chatbot_dock); self.ai_chatbot_dock.setVisible(True)
            self.addDockWidget(Qt.BottomDockWidgetArea, self.log_dock); self.log_dock.setVisible(True)
            self.ai_chatbot_dock.raise_()
        else:
            logger.warning(f"Unknown default perspective name: {perspective_name}. Applying Design Focus.")
            self._apply_default_perspective_layout(self.PERSPECTIVE_DESIGN_FOCUS)


    def save_current_perspective_as(self):
        current_layout_name = self.current_perspective_name
        name, ok = QInputDialog.getText(self, "Save Perspective", "Enter name for current layout:", QLineEdit.Normal, current_layout_name)
        if ok and name:
            name = name.strip()
            if not name:
                QMessageBox.warning(self, "Invalid Name", "Perspective name cannot be empty.")
                return
            
            is_default_name = name in self.DEFAULT_PERSPECTIVES_ORDER
            
            if is_default_name and name != current_layout_name: # Renaming to a default name
                 reply = QMessageBox.question(self, "Overwrite Default?", f"'{name}' is a default perspective. Overwrite its layout with the current one?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                 if reply == QMessageBox.No: return
            elif name != current_layout_name and name in self.settings_manager.get("user_perspective_names", []): # Renaming to an existing custom name
                 reply = QMessageBox.question(self, "Overwrite Custom?", f"A custom perspective named '{name}' already exists. Overwrite it?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                 if reply == QMessageBox.No: return


            state_data_bytes = self.saveState()
            state_data_hex = state_data_bytes.hex()
            self.settings_manager.set(f"perspective_{name}", state_data_hex)

            user_perspective_names = self.settings_manager.get("user_perspective_names", [])
            if name not in user_perspective_names and not is_default_name:
                user_perspective_names.append(name)
                self.settings_manager.set("user_perspective_names", sorted(user_perspective_names)) # Keep sorted
    ...
    def reset_all_custom_perspectives(self):
        ...
        if reply == QMessageBox.Yes:
            # Remove settings for all custom perspectives
            user_perspective_names = self.settings_manager.get("user_perspective_names", [])
            for p_name in user_perspective_names:
                self.settings_manager.remove_setting(f"perspective_{p_name}", save_immediately=False)
            
            # Remove any saved states for default perspectives (to revert them to programmatic defaults)
            for p_name_default in self.DEFAULT_PERSPECTIVES_ORDER:
                 self.settings_manager.remove_setting(f"perspective_{p_name_default}", save_immediately=False)
    ...
    def _on_interaction_mode_changed_by_scene(self, mode_name: str):
        ...
        # Update cursor for the view (if it was being managed by scene/view directly)
        if hasattr(self, 'view') and hasattr(self.view, '_restore_cursor_to_scene_mode'):
