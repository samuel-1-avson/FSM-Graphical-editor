...
class UIManager(QObject): 
    ...
    def setup_ui(self):
        self.mw.setGeometry(50, 50, 1650, 1050) 
        self.mw.setWindowIcon(get_standard_icon(QStyle.SP_DesktopIcon, "BSM"))

        self._create_central_widget() 
        self._create_actions()        
        self._create_menus()
        self._create_toolbars() # This will be modified
        self._create_docks()          
    ...
    def _create_actions(self):
        ...
        logger.debug("UIManager: Actions created and assigned to MainWindow.")
    ...
    def _create_menus(self):
        ...
        mw.toolbars_menu = mw.view_menu.addMenu("Toolbars") # Store reference if needed later
    ...
    def _create_toolbars(self):
        ...
        # Clear existing toolbars if any (important if this method is called multiple times, though usually not)
        for tb_child in mw.findChildren(QToolBar):
            mw.removeToolBar(tb_child)
            tb_child.deleteLater()

        # 1. FileToolbar
        mw.file_toolbar = mw.addToolBar("File")
        mw.file_toolbar.setObjectName("FileToolbar")
        mw.file_toolbar.setIconSize(icon_size); mw.file_toolbar.setToolButtonStyle(tb_style)
        mw.file_toolbar.addAction(mw.new_action)
        mw.file_toolbar.addAction(mw.open_action)
        mw.file_toolbar.addAction(mw.save_action)

        # 2. EditToolbar
        mw.edit_toolbar = mw.addToolBar("Edit")
        mw.edit_toolbar.setObjectName("EditToolbar")
        mw.edit_toolbar.setIconSize(icon_size); mw.edit_toolbar.setToolButtonStyle(tb_style)
        mw.edit_toolbar.addAction(mw.undo_action)
        mw.edit_toolbar.addAction(mw.redo_action)
        mw.edit_toolbar.addSeparator()
        mw.edit_toolbar.addAction(mw.delete_action)
        mw.edit_toolbar.addAction(mw.select_all_action) # Added Select All
        mw.edit_toolbar.addAction(mw.find_item_action)

        # 3. DesignToolbar (Interaction Modes, Align, Distribute)
        mw.design_toolbar = mw.addToolBar("Design")
        mw.design_toolbar.setObjectName("DesignToolbar")
        mw.design_toolbar.setIconSize(icon_size); mw.design_toolbar.setToolButtonStyle(tb_style)
        mw.design_toolbar.addAction(mw.select_mode_action)
        mw.design_toolbar.addAction(mw.add_state_mode_action)
        mw.design_toolbar.addAction(mw.add_transition_mode_action)
        mw.design_toolbar.addAction(mw.add_comment_mode_action)
        mw.design_toolbar.addSeparator()
        mw.design_toolbar.addAction(mw.align_left_action)
        mw.design_toolbar.addAction(mw.align_center_h_action)
        mw.design_toolbar.addAction(mw.align_right_action)
        mw.design_toolbar.addAction(mw.align_top_action)
        mw.design_toolbar.addAction(mw.align_middle_v_action)
        mw.design_toolbar.addAction(mw.align_bottom_action)
        mw.design_toolbar.addSeparator()
        mw.design_toolbar.addAction(mw.distribute_h_action)
        mw.design_toolbar.addAction(mw.distribute_v_action)

        # 4. ViewToolbar
        mw.view_control_toolbar = mw.addToolBar("View Controls") # Renamed to avoid conflict with view_menu
        mw.view_control_toolbar.setObjectName("ViewControlToolBar")
        mw.view_control_toolbar.setIconSize(icon_size); mw.view_control_toolbar.setToolButtonStyle(tb_style)
        mw.view_control_toolbar.addAction(mw.zoom_in_action)
        mw.view_control_toolbar.addAction(mw.zoom_out_action)
        mw.view_control_toolbar.addAction(mw.reset_zoom_action)
        mw.view_control_toolbar.addSeparator()
        mw.view_control_toolbar.addAction(mw.zoom_to_selection_action)
        mw.view_control_toolbar.addAction(mw.fit_diagram_action)
        
        # 5. SimulationToolbar
        mw.simulation_main_toolbar = mw.addToolBar("Simulation") # Renamed to avoid conflict with sim_menu
        mw.simulation_main_toolbar.setObjectName("SimulationMainToolBar")
        mw.simulation_main_toolbar.setIconSize(icon_size); mw.simulation_main_toolbar.setToolButtonStyle(tb_style)
        mw.simulation_main_toolbar.addAction(mw.start_py_sim_action)
        mw.simulation_main_toolbar.addAction(mw.stop_py_sim_action)
        mw.simulation_main_toolbar.addAction(mw.reset_py_sim_action)
        mw.simulation_main_toolbar.addSeparator()
        mw.simulation_main_toolbar.addAction(mw.run_simulation_action) # MATLAB Sim

        # 6. ExportToolbar
        mw.export_main_toolbar = mw.addToolBar("Export & Code Generation") # Renamed
        mw.export_main_toolbar.setObjectName("ExportMainToolBar")
        mw.export_main_toolbar.setIconSize(icon_size); mw.export_main_toolbar.setToolButtonStyle(tb_style)
        mw.export_main_toolbar.addAction(mw.export_simulink_action)
        mw.export_main_toolbar.addSeparator()
        mw.export_main_toolbar.addAction(mw.generate_c_code_action)
        mw.export_main_toolbar.addAction(mw.generate_matlab_code_action)
        mw.export_main_toolbar.addSeparator()
        mw.export_main_toolbar.addAction(mw.export_plantuml_action)
        mw.export_main_toolbar.addAction(mw.export_mermaid_action)

        # Add toggle actions for these new toolbars to the View > Toolbars menu
        if hasattr(mw, 'toolbars_menu'): # Ensure the submenu exists
            mw.toolbars_menu.clear() # Clear any old toolbar toggles
            mw.toolbars_menu.addAction(mw.file_toolbar.toggleViewAction())
            mw.toolbars_menu.addAction(mw.edit_toolbar.toggleViewAction())
            mw.toolbars_menu.addAction(mw.design_toolbar.toggleViewAction())
            mw.toolbars_menu.addAction(mw.view_control_toolbar.toggleViewAction())
            mw.toolbars_menu.addAction(mw.simulation_main_toolbar.toggleViewAction())
            mw.toolbars_menu.addAction(mw.export_main_toolbar.toggleViewAction())
            
        logger.debug("UIManager: Consolidated toolbars created and View menu updated.")
    ...
    def _create_docks(self):
        ...
        mw.tools_dock = QDockWidget("Elements & Modes", mw); mw.tools_dock.setObjectName("ToolsDock")
        ...
        mw.toolbox_select_button = QToolButton(); mw.toolbox_select_button.setDefaultAction(mw.select_mode_action)
        mw.toolbox_add_state_button = QToolButton(); mw.toolbox_add_state_button.setDefaultAction(mw.add_state_mode_action)
        mw.toolbox_transition_button = QToolButton(); mw.toolbox_transition_button.setDefaultAction(mw.add_transition_mode_action)
        mw.toolbox_add_comment_button = QToolButton(); mw.toolbox_add_comment_button.setDefaultAction(mw.add_comment_mode_action)
        for btn in [mw.toolbox_select_button, mw.toolbox_add_state_button, mw.toolbox_transition_button, mw.toolbox_add_comment_button]:
        ...
        templates_layout = QVBoxLayout(); templates_layout.setSpacing(6)
        mw.template_buttons_container = QWidget() 
        mw.template_buttons_layout = QVBoxLayout(mw.template_buttons_container)
        mw.template_buttons_layout.setContentsMargins(0,0,0,0); mw.template_buttons_layout.setSpacing(4)
        ...
        tools_main_layout.addStretch(); mw.tools_dock.setWidget(tools_widget_main)
        mw.addDockWidget(Qt.LeftDockWidgetArea, mw.tools_dock)
        ...
        mw.addDockWidget(Qt.BottomDockWidgetArea, mw.log_dock); mw.log_dock.setVisible(True)
        ...
        mw.problems_dock.setWidget(mw.problems_list_widget); mw.addDockWidget(Qt.BottomDockWidgetArea, mw.problems_dock); mw.problems_dock.setVisible(True)
        ...
        if hasattr(mw, 'view_menu'): 
            # Add Docks to View Menu (if not already handled by toolbars_menu)
            if not hasattr(mw, 'toolbars_menu'): # If toolbars_menu isn't the place for these
                mw.view_menu.addSeparator() # Separate from zoom/snap
            mw.view_menu.addAction(mw.tools_dock.toggleViewAction())
            mw.view_menu.addAction(mw.properties_dock.toggleViewAction())
            mw.view_menu.addAction(mw.log_dock.toggleViewAction())
            mw.view_menu.addAction(mw.problems_dock.toggleViewAction())
            mw.view_menu.addAction(mw.py_sim_dock.toggleViewAction())
            mw.view_menu.addAction(mw.ai_chatbot_dock.toggleViewAction())
            mw.view_menu.addAction(mw.ide_dock.toggleViewAction())
    ...
    def _load_and_display_templates(self):
        ...
        logger.debug("UIManager: FSM Templates loaded and displayed in ToolsDock.")
        
    def _create_status_bar(self):
        mw = self.mw
        mw.status_bar = QStatusBar(mw); mw.setStatusBar(mw.status_bar)
        mw.status_label = QLabel("Ready"); mw.status_bar.addWidget(mw.status_label, 1)
        mw.zoom_status_label = QLabel("Zoom: 100%"); mw.zoom_status_label.setMinimumWidth(80); mw.zoom_status_label.setAlignment(Qt.AlignCenter); mw.status_bar.addPermanentWidget(mw.zoom_status_label)
        mw.cpu_status_label = QLabel("CPU: --%"); mw.cpu_status_label.setToolTip("CPU Usage")
        mw.ram_status_label = QLabel("RAM: --%"); mw.ram_status_label.setToolTip("RAM Usage")
        mw.gpu_status_label = QLabel("GPU: N/A"); mw.gpu_status_label.setToolTip("GPU Usage (NVIDIA only, if pynvml installed)")
        for label in [mw.cpu_status_label, mw.ram_status_label, mw.gpu_status_label]:
            label.setMinimumWidth(80); label.setAlignment(Qt.AlignCenter); mw.status_bar.addPermanentWidget(label)
        mw.py_sim_status_label = QLabel("PySim: Idle"); mw.py_sim_status_label.setToolTip("Internal Python FSM Simulation Status."); mw.py_sim_status_label.setMinimumWidth(120); mw.py_sim_status_label.setAlignment(Qt.AlignCenter); mw.status_bar.addPermanentWidget(mw.py_sim_status_label)
        mw.matlab_status_label = QLabel("MATLAB: Init..."); mw.matlab_status_label.setToolTip("MATLAB connection status."); mw.matlab_status_label.setMinimumWidth(140); mw.matlab_status_label.setAlignment(Qt.AlignCenter); mw.status_bar.addPermanentWidget(mw.matlab_status_label)
        mw.internet_status_label = QLabel("Net: Init..."); mw.internet_status_label.setToolTip("Internet connectivity. Checks periodically."); mw.internet_status_label.setMinimumWidth(90); mw.internet_status_label.setAlignment(Qt.AlignCenter); mw.status_bar.addPermanentWidget(mw.internet_status_label)
        mw.progress_bar = QProgressBar(mw); mw.progress_bar.setRange(0,0); mw.progress_bar.setVisible(False); mw.progress_bar.setMaximumWidth(160); mw.progress_bar.setTextVisible(False); mw.status_bar.addPermanentWidget(mw.progress_bar)
    ...
    def populate_dynamic_docks(self):
        ...
        docks_to_tabify = [
            (mw.properties_dock, mw.ai_chatbot_dock),
            (mw.ai_chatbot_dock, mw.py_sim_dock),
            (mw.py_sim_dock, getattr(mw, 'ide_dock', None)), 
            (mw.log_dock, mw.problems_dock)
        ]
        for dock1_ref, dock2_ref in docks_to_tabify:
            if dock1_ref and dock2_ref:
                try:
                    mw.tabifyDockWidget(dock1_ref, dock2_ref)
                except Exception as e: 
                    logger.error(f"Error tabifying docks {dock1_ref.objectName()} and {dock2_ref.objectName()}: {e}")
            else:
                obj_name1 = getattr(dock1_ref, 'objectName', lambda: 'N/A')() if dock1_ref else "None"
                obj_name2 = getattr(dock2_ref, 'objectName', lambda: 'N/A')() if dock2_ref else "None"
                logger.warning(f"UIManager: Skipping tabify for docks: {obj_name1} and {obj_name2} as one or both might be missing/None.")
        logger.debug("UIManager: Dynamic docks populated and tabified.")