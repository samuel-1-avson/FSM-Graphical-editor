...
from PyQt5.QtCore import QObject, pyqtSignal, QThread, QTime, QTimer, Qt, QMetaObject, pyqtSlot, Q_ARG, QSize, QUrl # Added QUrl
import json
import re
import logging
from enum import Enum, auto
import html # For AIChatUIManager
import uuid # For unique IDs for copy buttons
...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QTextEdit, QHBoxLayout, QLineEdit,
                             QPushButton, QLabel, QStyle, QMessageBox, QInputDialog, QAction, QApplication) # Added QApplication for clipboard
...
class AIChatUIManager(QObject):
    def __init__(self, main_window, parent=None):
        ...
        self._code_snippet_cache: Dict[str, str] = {} # Stores {unique_id: raw_code_string}
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        self.ai_chat_display.anchorClicked.connect(self.on_chat_anchor_clicked) # Connect anchor click
    ...
    def _format_code_block(self, code_content: str, language: str = "") -> str:
        ...
        code_id = str(uuid.uuid4()) # Generate a unique ID for this code block
        self._code_snippet_cache[code_id] = code_content # Store raw code
        ...
        # Basic copy button style - can be improved
        copy_button_style = (
            f"float: right; margin-left: 10px; padding: 1px 4px; font-size: 7pt; "
            f"background-color: {QColor(COLOR_BORDER_LIGHT).lighter(110).name()}; color: {COLOR_TEXT_PRIMARY}; "
            f"border: 1px solid {COLOR_BORDER_LIGHT}; border-radius: 3px; text-decoration: none;"
        )
        copy_button_hover_style = f"background-color: {COLOR_ACCENT_PRIMARY_LIGHT}; color: {COLOR_TEXT_ON_ACCENT};"
        # Note: QTextEdit doesn't support :hover directly in this way. The visual change on hover needs JS, not possible here.
        # We can make the button always visible or use a simpler character.
        
        # Using a simple text link for copy for broader compatibility with QTextEdit's HTML
        copy_button_html = (
            f'<a href="copycode:{code_id}" '
            f'style="float:right; margin-left:8px; padding:1px 5px; font-size:7.5pt; text-decoration:none; '
            f'background-color:{QColor(COLOR_BACKGROUND_MEDIUM).name()}; color:{COLOR_TEXT_PRIMARY}; '
            f'border:1px solid {COLOR_BORDER_MEDIUM}; border-radius:3px;" '
            f'title="Copy code to clipboard">Copy</a>'
        )

        escaped_code = html.escape(code_content)
        return (f'<div style="position:relative; margin: 8px 0; padding: 10px; background-color:{bg_color}; color:{text_color}; ' # position:relative for potential absolute positioned button later
                f'border:1px solid {border_color}; border-radius:4px; font-family: Consolas, monospace; white-space:pre-wrap; overflow-x:auto;">'
                f'{copy_button_html}' # Add button HTML
                f'{lang_display}'
                f'<div style="clear:both;"></div>' # Clear float for the code itself
                f'{escaped_code}</div>')

    def _markdown_to_html_basic(self, text_part: str) -> str:
        temp_bold_start = "_TEMP_BOLD_START_"
        temp_bold_end = "_TEMP_BOLD_END_"
        temp_em_start = "_TEMP_EM_START_"
        temp_em_end = "_TEMP_EM_END_"
        temp_ul_start = "_TEMP_UL_START_"
        temp_ul_end = "_TEMP_UL_END_"
        temp_li_start = "_TEMP_LI_START_"
        temp_li_end = "_TEMP_LI_END_"
        temp_inline_code_start = "_TEMP_INLINE_CODE_START_"
        temp_inline_code_end = "_TEMP_INLINE_CODE_END_"

        text_part = re.sub(r'`(.*?)`', f'{temp_inline_code_start}\\1{temp_inline_code_end}', text_part, flags=re.DOTALL)
        text_part = re.sub(r'\*\*(.*?)\*\*', f'{temp_bold_start}\\1{temp_bold_end}', text_part, flags=re.DOTALL)
        text_part = re.sub(r'__(.*?)__', f'{temp_bold_start}\\1{temp_bold_end}', text_part, flags=re.DOTALL)
        text_part = re.sub(r'(?<![a-zA-Z0-9])\*([^* \n](?:[^*]*[^* \n])?)\*(?![a-zA-Z0-9])', f'{temp_em_start}\\1{temp_em_end}', text_part)
        text_part = re.sub(r'(?<![a-zA-Z0-9])_([^_ \n](?:[^_]*[^_ \n])?)_(?![a-zA-Z0-9])', f'{temp_em_start}\\1{temp_em_end}', text_part)
            ...
            if match:
                if not in_list_block: 
                    if current_buffer.strip(): 
                        processed_text_parts_for_list.append(current_buffer)
                    current_buffer = "" 
                    processed_text_parts_for_list.append(temp_ul_start)
                    in_list_block = True
                processed_text_parts_for_list.append(f"{temp_li_start}{match.group(1).strip()}{temp_li_end}")
            else: 
                if in_list_block: 
                    processed_text_parts_for_list.append(temp_ul_end)
                    in_list_block = False
                current_buffer += line
        ...
        text_part_intermediate = "".join(processed_text_parts_for_list)
        text_part_escaped = html.escape(text_part_intermediate).replace("\n", "<br>")
        text_part_escaped = text_part_escaped.replace(html.escape(temp_bold_start), "<strong>").replace(html.escape(temp_bold_end), "</strong>")
        text_part_escaped = text_part_escaped.replace(html.escape(temp_em_start), "<em>").replace(html.escape(temp_em_end), "</em>")
        text_part_escaped = text_part_escaped.replace(html.escape(temp_ul_start), "<ul>").replace(html.escape(temp_ul_end), "</ul>")
        text_part_escaped = text_part_escaped.replace(html.escape(temp_li_start), "<li>").replace(html.escape(temp_li_end), "</li>")
        ...
        text_part_escaped = text_part_escaped.replace(html.escape(temp_inline_code_start), f"<code style='{inline_code_style}'>").replace(html.escape(temp_inline_code_end), "</code>")
    ...
    def _append_to_chat_display(self, sender: str, message: str):
        ...
        for match in code_block_regex.finditer(message):
            ...
            code_content = match.group(2).strip('\n') # Keep original newlines in raw code
    ...
    def on_chat_anchor_clicked(self, url: QUrl):
        if url.scheme() == 'copycode':
            code_id = url.path() # The part after "copycode:"
            if code_id in self._code_snippet_cache:
                ...
                # Simple feedback in status label
                if self.ai_chat_status_label:
                    original_text = self.ai_chat_status_label.text()
                    original_stylesheet = self.ai_chat_status_label.styleSheet()
                    
                    self.ai_chat_status_label.setText("Status: Code copied to clipboard!")
                    self.ai_chat_status_label.setStyleSheet(f"font-size: {APP_FONT_SIZE_SMALL}; padding: 1px 3px; border-radius: 2px; color: {COLOR_TEXT_ON_ACCENT}; background-color: {COLOR_ACCENT_SUCCESS};")
                    
                    # Remove previous timer if any to prevent multiple restores
                    ...
                    self._last_copy_feedback_timer.timeout.connect(lambda: (
                        self.ai_chat_status_label.setText(original_text),
                        self.ai_chat_status_label.setStyleSheet(original_stylesheet)
                    ))
                    self._last_copy_feedback_timer.start(2000) # Revert after 2 seconds
            else:
                logger.warning(f"Could not find code snippet for ID: {code_id}")
        else:
            # Handle other types of links if necessary, or open external links
    ...
    def on_clear_ai_chat_history(self):
        ...
        if self.mw.ai_chatbot_manager:
            ...
            if reply == QMessageBox.Yes:
                ...
                self._code_snippet_cache.clear() # Clear code cache on history clear
            else:
...
class AIChatbotManager(QObject):
    ...
    def clear_conversation_history(self):
        ...
        if self.chatbot_worker and self.chatbot_thread and self.chatbot_thread.isRunning():
            ...
            # Clear the UI manager's code cache as well
