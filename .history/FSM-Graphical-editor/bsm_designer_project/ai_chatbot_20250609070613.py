...
from PyQt5.QtWidgets import (
    ...
    QGraphicsItem,QTabWidget, QWidget, QGraphicsDropShadowEffect, # Added QGraphicsDropShadowEffect
...
from .settings_manager import SettingsManager # Assuming in same package
from .graphics_items import GraphicsStateItem, GraphicsTransitionItem, GraphicsCommentItem # Assuming in same package
from .config import ( # Assuming in same package
    APP_NAME,
    COLOR_ITEM_STATE_DEFAULT_BG, COLOR_ITEM_TRANSITION_DEFAULT, COLOR_ITEM_COMMENT_BG, COLOR_TEXT_PRIMARY,
...
from .code_editor import CodeEditor # Assuming in same package
from .utils import get_standard_icon # Assuming in same package
from .matlab_integration import MatlabConnection # Assuming in same package
from .snippet_manager import CustomSnippetManager # Assuming in same package
...
try:
    ...
    try:
        from graphics_scene import DiagramScene, ZoomableView # Fallback for direct execution
        IMPORTS_SUCCESSFUL = True
    except ImportError:
...
class SubFSMEditorDialog(QDialog):
    def __init__(self, sub_fsm_data_initial: dict, parent_state_name: str, parent_window_ref=None):
        ...
        if IMPORTS_SUCCESSFUL and DiagramScene is not None and ZoomableView is not None:
    ...
    def accept_changes(self):
        if IMPORTS_SUCCESSFUL and hasattr(self, 'sub_scene') and DiagramScene is not None:
...
class SettingsDialog(QDialog):
    ...
    def _update_default_item_previews(self):
        ...
        if self._preview_state_item and self.state_preview_label:
            state_color_hex = self.settings_manager.get("state_default_color", COLOR_ITEM_STATE_DEFAULT_BG)
        ...
        if self._preview_transition_item and self.transition_preview_label:
            trans_color_hex = self.settings_manager.get("transition_default_color", COLOR_ITEM_TRANSITION_DEFAULT)
        ...
        if self._preview_comment_item and self.comment_preview_label:
            comment_color_hex = self.settings_manager.get("comment_default_background_color", COLOR_ITEM_COMMENT_BG) # If you add color for comments
            ...
            # self._preview_comment_item.background_brush = QBrush(QColor(comment_color_hex)) # Example if comments get color
