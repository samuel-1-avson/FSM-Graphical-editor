...
from config import (
    ...
    # New defaults from config
    DEFAULT_STATE_SHAPE, DEFAULT_STATE_BORDER_STYLE, DEFAULT_STATE_BORDER_WIDTH
)
from settings_manager import SettingsManager # To get current application settings
...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
        ...
        # --- Apply settings or defaults for visual properties ---
        ...
        # Handle border_style_qt: needs mapping from string if string is stored/passed
        ...
        if self.icon_path:
            ...
            if self._q_icon.isNull():
                logger.warning(f"Failed to load custom icon for state '{text}' from path: {self.icon_path}")
                self._q_icon = None
        # --- End visual properties init ---
        ...
        self._text_color = QColor(COLOR_TEXT_PRIMARY) # Could be themed later
        # self._font is now set above
        # self._border_pen_width is now self.custom_border_width
        self._superstate_border_pen_width_multiplier = 1.3 # Superstates have slightly thicker border

        # Original pen now more complex due to styles
        ...
        # ... (flags, tooltip, shadow effect remain the same) ...
    ...
    def _determine_current_pen(self) -> QPen:
        ...
        if self.is_py_sim_active:
            ...
            pen_to_use.setWidthF(max(current_base_width, COLOR_PY_SIM_STATE_ACTIVE_PEN_WIDTH)) # Use thicker of sim or custom
            pen_to_use.setStyle(Qt.SolidLine) # Sim active always solid
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        border_radius = 12 # For rounded rect
        ...
        if not self._is_editing_inline:
            painter.setPen(self._text_color) # Use dynamic text color from theme if possible
            painter.setFont(self._font) # Use custom font
            text_rect = current_rect.adjusted(10, 10, -10, -10)
            if self.is_superstate or self._q_icon: # Make space for icons
                text_rect.setRight(text_rect.right() - 18)
            
            # Adjust text rect for ellipse to be more centered
            if self.shape_type == "ellipse":
        ...
        # Custom Icon (if any, drawn before superstate icon if both exist)
        if self._q_icon and not self._q_icon.isNull():
            icon_size = 16; margin = 5
            # Position top-right, adjust if superstate icon is also there
            icon_x = current_rect.right() - icon_size - margin
            if self.is_superstate:
                icon_x -= (icon_size + margin / 2) # Shift left to make space for superstate icon
        ...
        # Initial State Marker (remains mostly the same)
        # ... (Initial State Marker paint code) ...
        if self.is_initial:
            marker_radius = 7; line_length = 20; marker_color = QColor(COLOR_TEXT_PRIMARY) # Use dynamic text color
            start_marker_center_x = current_rect.left() - line_length - marker_radius / 2
            start_marker_center_y = current_rect.center().y()
            painter.setBrush(marker_color); painter.setPen(QPen(marker_color, self.custom_border_width + 0.5)) # Use custom width base
        ...
        if self.is_final:
            ...
            inner_border_pen = QPen(inner_border_color, self.custom_border_width) # Use custom base width
        ...
        # Superstate Icon (remains mostly the same)
        if self.is_superstate:
            icon_size = 16; margin = 5
        ...
        if self.isSelected() and not self.is_py_sim_active:
            selection_pen_width = current_pen_for_drawing.widthF() + 0.5
    ...
    def set_properties(self, name, is_initial, is_final, color_hex=None,
        ...
        if icon_path_prop != self.icon_path: # Handles None vs path and path changes
            ...
            if self.icon_path:
                self._q_icon = QPixmap(self.icon_path)
                if self._q_icon.isNull():
                    logger.warning(f"Failed to load custom icon (set_properties) for state '{self.text_label}' from path: {self.icon_path}")
                    self._q_icon = None
            else:
                self._q_icon = None
            changed = True
        
        # Rebuild original_pen if border properties changed
        if changed: # Optimization: only if any border prop changed
            self.original_pen = QPen(self.border_color, self.custom_border_width, self.border_style_qt)
    ...
    # ... (start_inline_edit, _handle_editor_key_press, _finish_inline_edit, keyPressEvent, itemChange, set_text remain mostly same)
    def start_inline_edit(self): # Copied from previous, check for conflicts
        ...
        editor = QLineEdit(self.text_label); editor.setFont(self._font); editor.setAlignment(Qt.AlignCenter) # Use custom font
...
# GraphicsTransitionItem and GraphicsCommentItem would have similar additions
# for their respective visual properties.
# For brevity, I'll skip fully implementing them here, but the pattern is similar:
# 1. Add new properties to __init__ and self.
# 2. Load from settings or use defaults.
# 3. Modify get_data() and set_properties().
# 4. Update paint() method to use the new properties.
# 5. Update their respective PropertiesDialogs.

# --- GraphicsTransitionItem (Conceptual changes) ---
class GraphicsTransitionItem(QGraphicsPathItem):
    ...
    def __init__(self, start_item, end_item, event_str="", condition_str="", action_str="", color=None, description="", action_language=DEFAULT_EXECUTION_ENV,
        ...
        self._font = QFont(_label_font_family, _label_font_size) # Font for the label
        ...
        if line_style_qt is not None:
            ...
            self.line_style_qt = SettingsManager.STRING_TO_QT_PEN_STYLE.get(style_str, Qt.SolidLine)
        else:
            self.line_style_qt = Qt.SolidLine
        ...
        self._text_color = QColor(COLOR_TEXT_PRIMARY) # Could be themed
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        if self.isSelected() and not self.is_py_sim_active:
            # ... (selection highlight code remains, uses current_pen_for_drawing.widthF()) ...
        ...
        painter.setPen(current_pen_for_drawing)
        painter.setBrush(Qt.NoBrush) # Transitions are lines, not filled
        painter.drawPath(self.path())

        # Arrowhead drawing
        if self.path().elementCount() < 1 or self.arrowhead_style == "none": return # Skip if no arrowhead
        ...
        painter.setPen(current_pen_for_drawing) # Use same pen for arrowhead outline
        ...
        if current_label:
            # ... (label drawing code remains mostly the same, but uses self._font) ...
            from PyQt5.QtGui import QFontMetrics; painter.setFont(self._font); fm = QFontMetrics(self._font) # Use custom font
    ...
    def set_properties(self, event_str="", condition_str="", action_str="", color_hex=None, description="", offset=None, action_language=DEFAULT_EXECUTION_ENV,
        ...
        if changed: # Rebuild original_pen if base color or style changed
            self.original_pen = QPen(self.base_color, self.custom_line_width, self.line_style_qt, Qt.RoundCap, Qt.RoundJoin)
            self.prepareGeometryChange()
            self.update_path() # update_path itself calls prepareGeometryChange
            self.update()
    ...
    # ... (other methods like _get_actual_control_point_scene_pos, _compose_label_string, shape, update_path, mouse events remain similar)
    def set_problematic_style(self, is_problematic: bool, problem_description: str = ""):
    ...
    def _get_intersection_point(self, item: QGraphicsRectItem, line: QLineF): 
        ...
        # For ellipse shape, calculate intersection with ellipse
        if isinstance(item, GraphicsStateItem) and item.shape_type == "ellipse":
            # Simplified ellipse intersection (assuming axis-aligned)
            # More robust intersection math would be needed for arbitrary ellipses
            # This approximation finds intersections with the bounding box lines,
            # then picks the one closest to the line's start that's "towards" the line's end.
            # A true ellipse-line intersection is more complex.
            # For now, using bounding box for ellipse too, for simplicity of this step.
            # TODO: Implement proper ellipse-line intersection if precision is critical.
            pass # Fall through to bounding box for now

        rect_path = QPainterPath(); 
        if isinstance(item, GraphicsStateItem) and item.shape_type == "rectangle":
            rect_path.addRoundedRect(item_rect, 12, 12) # Use rounded rect for intersection
        else: # Ellipse or other, use bounding rect
        ...
        for edge in edges:
            ...
            # intersect_type = line.intersect(edge, intersection_point_var) # Qt 5.15
            # For older Qt, use:
            intersect_type = QLineF.IntersectType(line.intersect(edge, intersection_point_var))
...
# --- GraphicsCommentItem (Conceptual changes) ---
class GraphicsCommentItem(QGraphicsTextItem):
    ...
    def __init__(self, x, y, text="Comment",
        ...
        self.setFont(self._custom_font) # Apply custom font
        ...
        self.setDefaultTextColor(QColor(COLOR_TEXT_PRIMARY).darker(110)) # Could be themed
    ...
    def set_properties(self, text, width=None,
        ...
        current_font = self.font() # QGraphicsTextItem provides font()
        new_font_family = font_family_prop if font_family_prop is not None else current_font.family()
        new_font_size = font_size_prop if font_size_prop is not None else current_font.pointSize()
        new_font_italic = font_italic_prop if font_italic_prop is not None else current_font.italic()
    ...
    # ... (other methods like _on_contents_changed, start_inline_edit, eventFilter, _finish_inline_edit, keyPressEvent, itemChange remain similar)
    def set_problematic_style(self, is_problematic: bool, problem_description: str = ""):
        if self._is_problematic == is_problematic and self._problem_tooltip_text == problem_description: return
        self._is_problematic = is_problematic; self._problem_tooltip_text = problem_description if is_problematic else ""
    ...
    def start_inline_edit(self): 
        if self._is_editing_inline or not self.scene(): return
        self._is_editing_inline = True; self._inline_edit_aborted = False; self.update()
        editor = QTextEdit(self.toPlainText()); editor.setFont(self.font()) # Use current font
    ...
    def _finish_inline_edit(self, editor_widget: QTextEdit | None = None): 
        ...
        if new_text.strip() != old_text.strip():
            ...
            if self.textWidth() < 0 : self.setTextWidth(-1) # Recalculate width if it was auto
            new_props = self.get_data()
            if self.scene() and hasattr(self.scene(), 'undo_stack'):
                from undo_commands import EditItemPropertiesCommand # Local import
