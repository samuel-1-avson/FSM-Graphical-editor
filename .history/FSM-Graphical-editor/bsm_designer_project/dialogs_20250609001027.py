...
from PyQt5.QtGui import QColor, QIcon, QPalette, QKeyEvent, QFont, QPixmap, QPainter 
from PyQt5.QtCore import Qt, QDir, QSize, QPointF, pyqtSignal, QVariant, QRectF # <--- ADDED QRectF HERE
...
class SettingsDialog(QDialog):
    ...
    def _create_preview_items(self):
        ...
        self._preview_transition_item_start.setVisible(False) # These are conceptual, not drawn directly
        self._preview_transition_item_end = GraphicsStateItem(0,0,1,1, "tgt")
        self._preview_transition_item_end.setVisible(False)
    ...
    def _render_preview(self, item: QGraphicsItem, label: QLabel, width: int, height: int, is_transition=False):
        ...
        if is_transition and isinstance(item, GraphicsTransitionItem):
            padding = 15 # Increase padding for transition preview to ensure arrow is visible
            item.start_item.setPos(padding, height / 2) # Position dummy start state
            item.end_item.setPos(width - padding, height / 2) # Position dummy end state
            item.update_path() 
            
            # Render the item: try to render path and label at center of pixmap
            path_to_draw = item.path()
            if not path_to_draw.isEmpty():
                path_rect = path_to_draw.boundingRect()
                # Translate painter so the path is roughly centered
                dx = (width - path_rect.width()) / 2 - path_rect.left()
                dy = (height - path_rect.height()) / 2 - path_rect.top()
                painter.translate(dx, dy)
                item.paint(painter, None, None) # Let the item's paint handle details
                painter.translate(-dx, -dy) # Reset translation
            else: # Fallback if path is empty (should not happen if start/end items are set)
                 painter.drawText(QRectF(0,0,width,height), Qt.AlignCenter, "Transition Preview")
            ...
            original_pos = item.pos() # Store original pos
            item.setPos(target_x - item_rect.x(), target_y - item_rect.y()) # Adjust for rendering
            ...
            item.setPos(original_pos) # Restore original pos
    ...
    def _update_default_item_previews(self):
        ...
        if self._preview_state_item and self.state_preview_label:
            # Use a generic name, no need for initial/final, etc. for preview of defaults
            self._preview_state_item.set_properties(
                name="State", is_initial=False, is_final=False, 
                color_hex=self.settings_manager.get("state_default_color", COLOR_ITEM_STATE_DEFAULT_BG),
            ...
            # Adjust preview item size to fit if necessary, or keep fixed for consistency
            self._preview_state_item.setRect(0,0, self.PREVIEW_WIDTH - 20, self.PREVIEW_HEIGHT - 20)
        ...
        if self._preview_transition_item and self.transition_preview_label:
            self._preview_transition_item.set_properties(
                event_str="Event", 
                color_hex=self.settings_manager.get("transition_default_color", COLOR_ITEM_TRANSITION_DEFAULT), 
        ...
        if self._preview_comment_item and self.comment_preview_label:
            ...
            self._preview_comment_item.setTextWidth(self.PREVIEW_WIDTH - 15) # Give a bit of padding
