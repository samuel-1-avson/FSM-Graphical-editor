...
STYLE_SHEET_GLOBAL = f"""
    ...
    QTabBar::tab {{
        ...
        color: {COLOR_TEXT_SECONDARY};
        border: 1px solid {COLOR_BORDER_LIGHT};
        border-bottom-color: {COLOR_BACKGROUND_DIALOG}; /* Make bottom border match pane for selected */
        border-top-left-radius: 4px;
        border-top-right-radius: 4px;
        padding: 8px 18px;
        margin-right: 2px;
    ...
    QTabBar::tab:!selected:hover {{
        ...
        color: {COLOR_TEXT_PRIMARY};
        border-bottom-color: {COLOR_BORDER_LIGHT}; /* Keep original bottom border for non-selected hover */
    }}
    QCheckBox {{
        spacing: 10px;
    }}
    ...
    QPushButton#DraggableToolButton {{
        ...
        text-align: left; /* Align text to the left, useful if icon is also left */
        font-weight: 500;
        min-height: 40px; /* Consistent with UIManager */
        /* icon-size: 24px 24px; /* This can be set via setIconSize in code */
    }}
    ...
    /* Specific style for color button in properties dock to override general if needed */
