...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QTextBrowser, QHBoxLayout, QLineEdit, # Changed QTextEdit to QTextBrowser
                             QPushButton, QLabel, QStyle, QMessageBox, QInputDialog, QAction, QApplication)

from .config import ( # Assuming config is in the same package
    ...
    COLOR_TEXT_ON_ACCENT, COLOR_ACCENT_SUCCESS, COLOR_BACKGROUND_DIALOG, # Added COLOR_BACKGROUND_DIALOG
    COLOR_BORDER_MEDIUM # Added for copy button
)
from .utils import get_standard_icon # Assuming utils is in the same package
...
class AIChatUIManager(QObject):
    def __init__(self, main_window, parent=None):
        ...
        self.ai_chat_display: QTextBrowser = None # Changed from QTextEdit
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        self.ai_chat_display = QTextBrowser() # Changed from QTextEdit
        self.ai_chat_display.setReadOnly(True)
        self.ai_chat_display.setAcceptRichText(True) # QTextBrowser does this by default
        self.ai_chat_display.setTextInteractionFlags(Qt.TextBrowserInteraction | Qt.TextSelectableByKeyboard) # Allow keyboard selection
        self.ai_chat_display.setOpenLinks(False) # Crucial for handling links via anchorClicked only
    ...
    def _format_code_block(self, code_content: str, language: str = "") -> str:
        ...
        # --- MODIFIED SECTION for AI Chat Copy Button ---
        ...
        copy_button_html = (
            ...
            f'background-color:{copy_button_bg_color}; color:{copy_button_text_color}; ' # Use new theme-aware colors
            f'border:1px solid {copy_button_border_color}; border-radius:3px;" '
            f'title="Copy code to clipboard"> Copy</a>' # Consider replacing  with a more common icon like a clipboard symbol if fonts vary
        )
