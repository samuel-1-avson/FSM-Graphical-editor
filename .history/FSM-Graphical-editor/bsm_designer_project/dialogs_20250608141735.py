...
from PyQt5.QtWidgets import (
    ...
    QGraphicsItem,QTabWidget, QWidget # Added QWidget here
)
...
from settings_manager import SettingsManager # Import the new manager
...
class SubFSMEditorDialog(QDialog):
    ...
    def __init__(self, sub_fsm_data_initial: dict, parent_state_name: str, parent_window_ref=None):
        ...
        if IMPORTS_SUCCESSFUL: 
            ...
            toolbar.setToolButtonStyle(Qt.ToolButtonIconOnly) # Icon only for minimality
            ...
            for mode, text, icon_enum, icon_alt in actions_data:
                ...
                action.setToolTip(text) # Add tooltip for icon only buttons
...
class StatePropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_state=False, scene_ref=None, custom_snippet_manager: CustomSnippetManager | None = None): # Added custom_snippet_manager
        ...
        id_layout.setSpacing(8) # Reduced spacing
        ...
        appearance_layout.setSpacing(8) # Reduced spacing
        ...
        actions_layout.setSpacing(8) # Reduced spacing
        ...
        self.entry_action_edit = CodeEditor(); self.entry_action_edit.setPlainText(p.get('entry_action', "")); self.entry_action_edit.setFixedHeight(80); self.entry_action_edit.setObjectName("ActionCodeEditor") # Reduced height
        self.during_action_edit = CodeEditor(); self.during_action_edit.setPlainText(p.get('during_action', "")); self.during_action_edit.setFixedHeight(80); self.during_action_edit.setObjectName("ActionCodeEditor") # Reduced height
        self.exit_action_edit = CodeEditor(); self.exit_action_edit.setPlainText(p.get('exit_action', "")); self.exit_action_edit.setFixedHeight(80); self.exit_action_edit.setObjectName("ActionCodeEditor") # Reduced height
        ...
        self.description_edit = QTextEdit(p.get('description', "")); self.description_edit.setFixedHeight(60) # Reduced height
...
class TransitionPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_transition=False, custom_snippet_manager: CustomSnippetManager | None = None): # Added custom_snippet_manager
        ...
        logic_layout.setSpacing(8) # Reduced spacing
        ...
        action_form_layout.setSpacing(8) # Reduced spacing
        ...
        self.action_edit = CodeEditor(); self.action_edit.setPlainText(p.get('action', "")); self.action_edit.setFixedHeight(80); self.action_edit.setObjectName("ActionCodeEditor") # Reduced height
        ...
        appearance_desc_layout.setSpacing(8) # Reduced spacing
        self.color_button = QPushButton("Choose Color..."); self.color_button.setObjectName("ColorButton")
...
class CommentPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None):
        ...
        layout.setSpacing(8); layout.setContentsMargins(12,12,12,12) # Reduced spacing/margins
        self.text_edit = QTextEdit(p.get('text', "Comment"))
        self.text_edit.setMinimumHeight(100); self.text_edit.setPlaceholderText("Enter your comment or note here.") # Slightly reduced height
        self.text_edit.setFontPointSize(10) 
        layout.addWidget(QLabel("Comment Text:")); layout.addWidget(self.text_edit)
        buttons = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        buttons.accepted.connect(self.accept); buttons.rejected.connect(self.reject)
        layout.addWidget(buttons)
        self.setMinimumWidth(350); self.text_edit.setFocus(); self.text_edit.selectAll() # Reduced min width
    def get_properties(self):
...
class MatlabSettingsDialog(QDialog):
    def __init__(self, matlab_connection: MatlabConnection, parent=None):
        ...
        self.setMinimumWidth(550) # Reduced width
        ...
        main_layout = QVBoxLayout(self); main_layout.setSpacing(10); main_layout.setContentsMargins(10,10,10,10) # Reduced
        path_group = QGroupBox("MATLAB Executable Path"); path_form_layout = QFormLayout()
        path_form_layout.setSpacing(6) # Reduced
        ...
        btn_layout = QHBoxLayout(); btn_layout.setSpacing(6) # Reduced
        ...
        path_v_layout = QVBoxLayout(); path_v_layout.setSpacing(8) # Reduced
        ...
        test_group = QGroupBox("Connection Test"); test_layout = QVBoxLayout(); test_layout.setSpacing(8) # Reduced
        self.test_status_label = QLabel("Status: Unknown"); self.test_status_label.setObjectName("TestStatusLabel")
        self.test_status_label.setWordWrap(True); self.test_status_label.setTextInteractionFlags(Qt.TextSelectableByMouse); self.test_status_label.setMinimumHeight(30) # Reduced
...
class SettingsDialog(QDialog):
    def __init__(self, settings_manager: SettingsManager, parent=None):
        ...
        self.setWindowIcon(get_standard_icon(QStyle.SP_FileDialogDetailedView, "Prefs")) # Or SP_ComputerIcon
        ...
        self.original_settings_on_open = {} # To track changes for apply/reset
        ...
        # --- Behavior Tab (Example - add more later) ---
        ...
        self.resource_monitor_interval_spin.setRange(500, 60000) # 0.5s to 1 minute
        self.resource_monitor_interval_spin.setSingleStep(500)
        self.resource_monitor_interval_spin.setSuffix(" ms")
        ...
        restart_note.setStyleSheet(f"font-size: {APP_FONT_SIZE_SMALL};")
    ...
    def load_settings_to_ui(self):
        """Load current settings from SettingsManager into the dialog's UI elements."""
    ...
    def apply_settings(self):
        """Apply current UI settings to the SettingsManager without closing."""
        ...
        self.settings_manager.save_settings() # Save all changes at once
        self.load_settings_to_ui() # Refresh original_settings_on_open
    ...
    def on_reset_to_defaults(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            self.load_settings_to_ui() # Reload UI with defaults
    ...
    def reject(self):
        # Check if any settings were changed compared to when dialog opened
        changed = False
        if self.show_grid_cb.isChecked() != self.original_settings_on_open.get("view_show_grid"): changed = True
        if self.snap_to_grid_cb.isChecked() != self.original_settings_on_open.get("view_snap_to_grid"): changed = True
        if self.snap_to_objects_cb.isChecked() != self.original_settings_on_open.get("view_snap_to_objects"): changed = True
        if self.show_snap_guidelines_cb.isChecked() != self.original_settings_on_open.get("view_show_snap_guidelines"): changed = True
        if self.resource_monitor_enabled_cb.isChecked() != self.original_settings_on_open.get("resource_monitor_enabled"): changed = True
        if self.resource_monitor_interval_spin.value() != self.original_settings_on_open.get("resource_monitor_interval_ms"): changed = True
