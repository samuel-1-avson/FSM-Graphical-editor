...
class MainWindow(QMainWindow):
    ...
    def _update_window_title(self):
        ...
        diagram_dirty_char = "*" if self.scene.is_dirty() else ""
        ...
        sim_status_text = ""
        main_op_text_for_title = "" # Specific status for window title regarding PySim
        if self.py_sim_active:
            sim_status_text = " [PySim Active" # For status bar
            main_op_text_for_title = " [PySim Active" # For window title
            if self.py_fsm_engine and self.py_fsm_engine.paused_on_breakpoint:
                sim_status_text += " (Paused)"
                main_op_text_for_title += " (Paused)"
            sim_status_text += "]"
            main_op_text_for_title += "]"
        
        main_window_is_dirty = self.scene.is_dirty() or ide_is_dirty
        self.setWindowModified(main_window_is_dirty)
        
        window_title_str = f"{APP_NAME} - {file_name_part}{diagram_dirty_char}{main_op_text_for_title} [*]"
        self.setWindowTitle(window_title_str)

        current_main_op_text = f"File: {file_name_part}{diagram_dirty_char}"
        if self.py_sim_active: # PySim status takes precedence for main_op_status_label
            current_main_op_text = f"PySim: Active"
            if self.py_fsm_engine and self.py_fsm_engine.paused_on_breakpoint:
                current_main_op_text += " (Paused)"
        elif hasattr(self, 'progress_bar') and self.progress_bar.isVisible():
            # If main_op_status_label already shows a MATLAB operation, keep it.
            if hasattr(self, 'main_op_status_label') and "MATLAB:" not in self.main_op_status_label.text():
                 current_main_op_text = "Processing..." # Generic processing if not MATLAB
        elif not self.current_file_path and not self.scene.is_dirty(): # New, clean file
            current_main_op_text = "Ready"
        elif self.current_file_path and not self.scene.is_dirty() and not ide_is_dirty : # Saved, clean file
            current_main_op_text = f"Saved: {file_name_part}"


        if hasattr(self, 'main_op_status_label'):
            self.main_op_status_label.setText(current_main_op_text)
        
        self._update_py_sim_status_display() # Update dedicated PySim label
    ...
    # --- Properties Dock Logic (from previous step, confirmed) ---
