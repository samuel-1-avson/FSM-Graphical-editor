...
class AIChatUIManager(QObject):
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        # Important: Enable link interaction for anchorClicked to work
        self.ai_chat_display.setTextInteractionFlags(Qt.TextBrowserInteraction)
        self.ai_chat_display.setOpenLinks(False) # We handle custom links ourselves
    ...
    def update_status_display(self, status_enum: AIStatus, status_text: str):
        ...
        # Don't let status updates overwrite "Copied!" message immediately
        if self._last_copy_feedback_timer and self._last_copy_feedback_timer.isActive():
            # Store the new status to be applied after the "Copied!" message timeout
            self._pending_status_update = (status_enum, status_text)
            return
        self._pending_status_update = None # Clear any pending update
    ...
    def _format_code_block(self, code_content: str, language: str = "") -> str:
        ...
        copy_button_html = (
            ...
            f'style="float:right; margin-left:8px; margin-top:-2px; padding:1px 5px; font-size:7.5pt; text-decoration:none; ' # Adjusted margin-top
            ...
            f'title="Copy code to clipboard"> Copy</a>' # Using clipboard emoji 📋
        )
        ...
        if url.scheme() == 'copycode':
            ...
            if code_id in self._code_snippet_cache:
                ...
                if self.ai_chat_status_label:
                    if self._last_copy_feedback_timer and self._last_copy_feedback_timer.isActive():
                        self._last_copy_feedback_timer.stop() # Stop previous timer
                    else: # Store original only if no timer is active
                    ...
                    def revert_status():
                        if hasattr(self, '_pending_status_update') and self._pending_status_update:
                            self.update_status_display(*self._pending_status_update)
                            self._pending_status_update = None
                        elif hasattr(self, '_original_status_text'): # Check if attributes exist
                        ...
                        # If no pending and no original (e.g. rapid clicks), it will keep "Copied!" briefly.
                        # Or default to a "Ready" state if that's more desirable.
