...
from PyQt5.QtCore import QObject, pyqtSlot, QDir, QUrl, QPointF
...
class ActionHandler(QObject):
    ...
    def on_align_items(self, mode: str):
        mw = self.mw
        selected_items = [item for item in mw.scene.selectedItems() if isinstance(item, (type(mw.scene.items()[0] if mw.scene.items() else None) if not hasattr(mw.scene.items()[0] if mw.scene.items() else None, 'GraphicsStateItem') else mw.scene.items()[0].__class__.__bases__[0])) and (type(item).__name__ in ["GraphicsStateItem", "GraphicsCommentItem"])] # Adjusted for potential dynamic types
    ...
    def on_distribute_items(self, mode: str):
        mw = self.mw
        selected_items = [item for item in mw.scene.selectedItems() if isinstance(item, (type(mw.scene.items()[0] if mw.scene.items() else None) if not hasattr(mw.scene.items()[0] if mw.scene.items() else None, 'GraphicsStateItem') else mw.scene.items()[0].__class__.__bases__[0])) and (type(item).__name__ in ["GraphicsStateItem", "GraphicsCommentItem"])]
