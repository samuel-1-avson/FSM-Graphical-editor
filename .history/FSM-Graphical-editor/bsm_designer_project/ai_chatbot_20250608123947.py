...
from PyQt5.QtCore import QObject, pyqtSignal, QThread, QTime, QTimer, Qt, QMetaObject, pyqtSlot, Q_ARG, QSize # Added QSize
import json
import re
import logging
...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QTextEdit, QHBoxLayout, QLineEdit,
                             QPushButton, QLabel, QStyle, QMessageBox, QInputDialog, QAction) # Added QAction for type checking
...
class AIChatUIManager(QObject):
    ...
    def _connect_actions_to_manager_slots(self):
        ...
        if settings_action is not None and isinstance(settings_action, QAction):
            ...
            try:
                # Ensure any previous connections are removed to avoid duplicates if this is called multiple times
        ...
        if ask_fsm_action and isinstance(ask_fsm_action, QAction):
            ...
            try: ask_fsm_action.triggered.disconnect()
        ...
        if clear_chat_action and isinstance(clear_chat_action, QAction):
            ...
            try: clear_chat_action.triggered.disconnect()
            except TypeError: pass
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        ai_chat_layout.setContentsMargins(4,4,4,4) # Reduced margins
        ai_chat_layout.setSpacing(4) # Reduced spacing
        ...
        input_layout.setSpacing(4) # Reduced spacing
        ...
        self.ai_chat_send_button.setIconSize(QSize(16,16)) # Smaller icon for send button

        self.ai_chat_send_button.setObjectName("AIChatSendButton")
        self.ai_chat_send_button.setToolTip("Send message to AI")
        self.ai_chat_send_button.setFixedWidth(32) # Adjusted for smaller icon
    ...
    def update_status_display(self, status_enum: AIStatus, status_text: str):
        ...
        base_style = f"font-size: {APP_FONT_SIZE_SMALL}; padding: 1px 3px; border-radius: 2px;" # Reduced padding
    ...
    def _markdown_to_html_basic(self, text_part: str) -> str:
        ...
        for line in text_part.splitlines(keepends=True):
            ...
            if match:
                if not in_list_block: # Start of a new list block
                    if current_buffer.strip(): # Append buffered non-list text
                        # Before appending, process buffer for strong/em if needed, or ensure it's escaped later
                ...
                if in_list_block: # End of a list block
                    processed_text_parts_for_list.append(temp_ul_end)
                    in_list_block = False
                current_buffer += line

        if in_list_block: # Close any open list at the end
            processed_text_parts_for_list.append(temp_ul_end)
        elif current_buffer.strip(): # Append any remaining non-list text
            processed_text_parts_for_list.append(current_buffer)

        text_part_intermediate = "".join(processed_text_parts_for_list)

        # Escape HTML special characters from the intermediate string, then replace placeholders
    ...
    def _append_to_chat_display(self, sender: str, message: str):
        ...
        html_to_append = (f"<div style='margin-bottom: 10px; padding: 6px 8px; border-left: 3px solid {sender_color_str}; background-color: {bg_msg_color}; border-radius: 4px;'>" # Reduced padding/margins
                          f"<div style='margin-bottom: 3px;'>" # Reduced margin
                          f"<strong style='color:{sender_color_str}; font-size: 9pt;'>{sender_name_html}</strong>"
                          f"<span style='font-size:7pt; color:{COLOR_TEXT_SECONDARY}; margin-left: 6px;'>[{timestamp}]</span> " # Reduced margin
                          f"</div>"
                          f"<div style='padding-left: 2px; line-height:1.4; font-size: 9pt;'>{final_message_html}</div></div>") # Adjusted line-height
