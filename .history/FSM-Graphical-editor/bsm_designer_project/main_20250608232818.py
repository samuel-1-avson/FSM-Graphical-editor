...
class MainWindow(QMainWindow):
    ...
    # Define default perspective names as class constants for easier reference
    ...
    def __init__(self):
        ...
        self._populate_perspectives_menu() # Populate menu after UI setup
        QTimer.singleShot(50, lambda: self.apply_perspective(self.current_perspective_name)) # Apply initial perspective
    ...
    def _apply_initial_settings(self):
        ...
        self.current_perspective_name = self.settings_manager.get("last_used_perspective", self.PERSPECTIVE_DESIGN_FOCUS)
        # Initial perspective application is now deferred to QTimer in __init__
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        # Save the current perspective name
    ...
    def _populate_perspectives_menu(self):
        ...
        self.perspectives_menu.clear() # Clear previous items except save/reset
        self.perspectives_action_group.actions().clear() # Clear action group
    ...
    def apply_perspective(self, perspective_name: str):
        ...
        if not applied_from_saved_state:
            # Apply default programmatic layouts if no saved state or restore failed
            self._apply_default_perspective_layout(perspective_name)

        self.current_perspective_name = perspective_name
        self._update_current_perspective_check()
        self._update_window_title() # Reflect perspective if needed
    ...
    def _apply_default_perspective_layout(self, perspective_name: str):
        ...
        # Ensure all docks are potentially floatable and then add them to avoid tab issues
        docks_to_manage = [
        ...
        for dock in docks_to_manage:
            if dock: dock.setFloating(False) # Ensure they are dockable

        if perspective_name == self.PERSPECTIVE_DESIGN_FOCUS:
            self.elements_palette_dock.setVisible(True); self.addDockWidget(Qt.LeftDockWidgetArea, self.elements_palette_dock)
            self.properties_dock.setVisible(True); self.addDockWidget(Qt.RightDockWidgetArea, self.properties_dock)
            self.problems_dock.setVisible(True); self.addDockWidget(Qt.BottomDockWidgetArea, self.problems_dock)
            self.log_dock.setVisible(True); self.tabifyDockWidget(self.problems_dock, self.log_dock)
            ...
            self.py_sim_dock.setVisible(True); self.addDockWidget(Qt.RightDockWidgetArea, self.py_sim_dock)
            self.properties_dock.setVisible(True); self.tabifyDockWidget(self.py_sim_dock, self.properties_dock)
            self.log_dock.setVisible(True); self.addDockWidget(Qt.BottomDockWidgetArea, self.log_dock) # Or use PySim's internal log

            self.elements_palette_dock.setVisible(False)
            self.ai_chatbot_dock.setVisible(False)
            self.ide_dock.setVisible(False)
            ...
            self.ide_dock.setVisible(True); self.addDockWidget(Qt.RightDockWidgetArea, self.ide_dock)
            self.problems_dock.setVisible(True); self.addDockWidget(Qt.BottomDockWidgetArea, self.problems_dock)
            self.log_dock.setVisible(True); self.tabifyDockWidget(self.problems_dock, self.log_dock)
            ...
            self.ai_chatbot_dock.setVisible(True); self.addDockWidget(Qt.RightDockWidgetArea, self.ai_chatbot_dock)
            self.log_dock.setVisible(True); self.addDockWidget(Qt.BottomDockWidgetArea, self.log_dock)
    ...
    def save_current_perspective_as(self):
        ...
        if ok and name:
            ...
            if not name:
                ...
                return
            if name in self.DEFAULT_PERSPECTIVES_ORDER and name != current_layout_name:
            ...
            user_perspective_names = self.settings_manager.get("user_perspective_names", [])
            if name not in user_perspective_names and name not in self.DEFAULT_PERSPECTIVES_ORDER:
                user_perspective_names.append(name)
                self.settings_manager.set("user_perspective_names", user_perspective_names)
            
            logger.info(f"Saved current layout as perspective: {name}")
            self._populate_perspectives_menu()
            self.apply_perspective(name) # Switch to it to update checkmark

    def reset_all_custom_perspectives(self):
        reply = QMessageBox.question(self, "Reset Layouts", 
                                     "Are you sure you want to reset all custom perspectives to their defaults? This will remove any user-saved layouts.",
        ...
        if reply == QMessageBox.Yes:
            ...
            # Also clear any potentially overwritten default perspective saved states
            ...
            self.settings_manager.set("user_perspective_names", [], save_immediately=True) # Save changes
            
            logger.info("All custom perspectives have been reset.")
            self._populate_perspectives_menu()
            self.apply_perspective(self.PERSPECTIVE_DESIGN_FOCUS) # Go to default
            QMessageBox.information(self, "Layouts Reset", "All custom perspectives have been reset.")
