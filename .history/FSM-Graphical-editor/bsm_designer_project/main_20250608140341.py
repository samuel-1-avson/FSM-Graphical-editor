...
from settings_manager import SettingsManager # Added missing import
from config import (
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Instantiate SettingsManager early
        ...
        # Pass settings_manager to ResourceMonitorManager if it needs to check settings
        ...
        self._apply_initial_settings() # New method to apply settings on startup
        ...
        if self.settings_manager.get("resource_monitor_enabled"):
            ...
            # Ensure resource labels are hidden if disabled
    ...
    def _apply_initial_settings(self):
        ...
        # View settings
        if hasattr(self.scene, 'setBackgroundBrush'): # Check if scene is DiagramScene or generic
            # Grid visibility is handled by drawBackground based on a flag,
            # here we update the flag and actions.
            show_grid = self.settings_manager.get("view_show_grid")
            # self.scene.grid_visible = show_grid # Scene needs this property or a way to toggle drawing
            # For now, assuming drawBackground checks a flag that actions toggle
            if hasattr(self, 'show_grid_action'): # If we create such an action
        ...
        self.scene.update() # Trigger redraw of background with new grid state

        # Resource Monitor (visibility handled in __init__ based on setting)
        if self.resource_monitor_manager:
            ...
            # Hiding/showing labels also handled in __init__
    ...
    def _handle_setting_changed(self, key: str, value: object):
        ...
        if key == "view_show_grid":
            # Scene's drawBackground should ideally check a flag that this toggles.
            # Or, have a scene.setGridVisible(bool) method.
            # For now, if there's an action, update it.
            if hasattr(self, 'show_grid_action'): self.show_grid_action.setChecked(bool(value))
            self.scene.update()
        elif key == "view_snap_to_grid":
            self.scene.snap_to_grid_enabled = bool(value)
            if hasattr(self, 'snap_to_grid_action'): self.snap_to_grid_action.setChecked(bool(value))
        elif key == "view_snap_to_objects":
            self.scene.snap_to_objects_enabled = bool(value)
            ...
            if self.resource_monitor_manager:
                if is_enabled and (not self.resource_monitor_manager.thread or not self.resource_monitor_manager.thread.isRunning()):
                    self.resource_monitor_manager.setup_and_start_monitor() # (Re)start it
                    if self.resource_monitor_manager.worker:
                        # Reconnect if setup_and_start_monitor was called again
        ...
        # Add more handlers as settings are implemented
    ...
    def on_show_preferences_dialog(self):
        ...
        dialog.exec_()
        # Settings are applied via the dialog's Apply/OK buttons and signals
    ...
    def set_ui_enabled_for_matlab_op(self, enabled: bool):
        ...
        # Only affect the main_toolbar, not all toolbars (like potentially IDE's)
        ...
        # Make this more specific to avoid disabling IDE toolbar if IDE is separate
        for dock_name in ["ElementsPaletteDock", "PropertiesDock", "LogDock", "PySimDock", "AIChatbotDock", "ProblemsDock"]: # "IDEDock" handled by its manager
    ...
    def _handle_py_sim_global_ui_enable_by_manager(self, enable: bool):
        ...
        # Only disable the ElementsPaletteDock if simulation is active
    ...
    def update_problems_dock(self, issues_with_items: list):
        ...
        if issues_with_items:
            ...
            # Show the dock only if there are issues and it's currently hidden.
            ...
            # Optionally hide if it was shown and now there are no issues
            # if not self.problems_dock.isHidden(): self.problems_dock.hide()
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str | None = None):
        """
        Adds FSM data (states, transitions, comments) to the scene.
        Can clear the existing diagram before adding.
        """
        ...
        if clear_current_diagram:
            if not self.action_handler.on_new_file(silent=True): # Use silent new file to clear scene and reset state
        ...
        # Check for positions in comments
        ...
        for i, state_data in enumerate(states_data):
            ...
            state_item = GraphicsStateItem(
                ...
                state_data.get('properties', {}).get('color', COLOR_ITEM_STATE_DEFAULT_BG),
