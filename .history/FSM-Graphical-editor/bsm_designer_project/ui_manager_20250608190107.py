...
class UIManager(QObject): 
    ...
    def setup_ui(self):
        self.mw.setGeometry(50, 50, 1600, 1000) # Slightly smaller default
    ...
    def _create_actions(self):
        ...
        # These actions are now primarily for the toolbar and menu, not the ToolsDock
        ...
        # Modify Snap Actions initialization (their checked state will be set by _apply_initial_settings)
        ...
        # The actual setChecked will happen in MainWindow._apply_initial_settings based on loaded settings
        ...
        logger.debug("UIManager: Actions created and assigned to MainWindow, including Preferences.")
    ...
    def _create_menus(self):
        ...
        edit_menu = menu_bar.addMenu("&Edit") # Keep for now, Preferences moved here
        
        edit_menu.addAction(mw.undo_action)
        edit_menu.addAction(mw.redo_action)
        edit_menu.addSeparator()
        # ... (other edit menu items like delete, select all, find) ...
        edit_menu.addSeparator()
        edit_menu.addAction(mw.preferences_action) # Preferences in Edit menu
        ...
        logger.debug("UIManager: Menus created, Preferences added to Edit.")

    def _create_toolbars(self):
        mw = self.mw
        icon_size = QSize(20,20) # Slightly smaller icons for toolbars
        tb_style = Qt.ToolButtonIconOnly # More minimal
        ...
        # Align actions (grouped in a menu on the toolbar for minimality)
        ...
        # Export/Generate actions (grouped in a menu on the toolbar)
        ...
        if hasattr(mw, 'toolbars_menu'): 
            ...
            mw.toolbars_menu.addAction(mw.main_toolbar.toggleViewAction()) # Only one main toolbar now
    ...
    def _create_docks(self):
        ...
        # Renamed ToolsDock to ElementsPaletteDock
        ...
        tools_main_layout.setSpacing(6) # Reduced spacing
        tools_main_layout.setContentsMargins(6,6,6,6) # Reduced margins
        
        # Interaction Mode buttons are now on the main toolbar, so this group is removed.
        
        draggable_group_box = QGroupBox("Drag New Elements")
        draggable_layout = QVBoxLayout()
        draggable_layout.setSpacing(4) # Reduced spacing
        ...
        templates_layout.setSpacing(4) # Reduced spacing
        ...
        mw.template_buttons_layout.setSpacing(3) # Reduced spacing
        ...
        # Hide problems dock by default if empty initially, can be shown on first issue.
        ...
        if hasattr(mw, 'view_menu'): 
            if not hasattr(mw, 'toolbars_menu'): 
                 mw.view_menu.addSeparator() 
            mw.view_menu.addAction(mw.elements_palette_dock.toggleViewAction()) # Changed name
    ...
    def _load_and_display_templates(self):
        ...
        templates_to_load = []
        # FSM_TEMPLATES_BUILTIN is already a Python dict where values are template dicts
        for template_key, template_dict_value in FSM_TEMPLATES_BUILTIN.items():
            # template_dict_value is already a Python dictionary, no need for json.loads here
            templates_to_load.append({
                ...
                # For DraggableToolButton, we need the JSON string of this specific template
        ...
        for template_info in templates_to_load:
            ...
            template_btn.setIconSize(QSize(20,20)) # Slightly smaller icon for draggable button
    ...
    def _create_status_bar(self):
        ...
        mw.ide_file_status_label.setMinimumWidth(100) # Adjusted
        ...
        # Group resource monitors for a more minimal look, or remove them if not critical for "easy to use"
        # For now, let's keep them but make them smaller.
        ...
        for label in [mw.cpu_status_label, mw.ram_status_label, mw.gpu_status_label]:
            label.setAlignment(Qt.AlignCenter); mw.status_bar.addPermanentWidget(label)
        ...
        # Resource monitor labels will be added conditionally in MainWindow based on settings
        resource_label_min_width = 60
        mw.cpu_status_label = QLabel("CPU: --%"); mw.cpu_status_label.setToolTip("CPU Usage"); mw.cpu_status_label.setMinimumWidth(resource_label_min_width);
        mw.ram_status_label = QLabel("RAM: --%"); mw.ram_status_label.setToolTip("RAM Usage"); mw.ram_status_label.setMinimumWidth(resource_label_min_width);
        mw.gpu_status_label = QLabel("GPU: N/A"); mw.gpu_status_label.setToolTip("GPU Usage (NVIDIA only, if pynvml installed)"); mw.gpu_status_label.setMinimumWidth(resource_label_min_width);
        
        # Do not add them here directly, MainWindow will manage their visibility
        # for label in [mw.cpu_status_label, mw.ram_status_label, mw.gpu_status_label]:
        #     label.setAlignment(Qt.AlignCenter); mw.status_bar.addPermanentWidget(label)
    ...
    def populate_dynamic_docks(self):
        ...
        if mw.ide_manager and hasattr(mw, 'ide_dock') and mw.ide_dock:
            pass # IDE dock content is set up within IDEManager constructor
        else:
            logger.error("UIManager: Could not populate IDE Dock: IDEManager or ide_dock missing.")

        # Adjusted tabification for better grouping
        # Group related tools on the right
        mw.tabifyDockWidget(mw.properties_dock, mw.py_sim_dock)
        mw.tabifyDockWidget(mw.py_sim_dock, mw.ai_chatbot_dock)
        if hasattr(mw, 'ide_dock') and mw.ide_dock:
            mw.tabifyDockWidget(mw.ai_chatbot_dock, mw.ide_dock)
            
        # Group output/info docks at the bottom
        mw.tabifyDockWidget(mw.log_dock, mw.problems_dock)
        
        # Set a default active tab for the right-side group if desired
        if hasattr(mw, 'properties_dock'):
            mw.properties_dock.raise_() 
