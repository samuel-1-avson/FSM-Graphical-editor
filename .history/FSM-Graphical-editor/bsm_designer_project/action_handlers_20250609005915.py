...
from PyQt5.QtCore import QObject, pyqtSlot, QDir, QUrl, QPointF, Qt, QRectF 
from PyQt5.QtGui import QDesktopServices, QImage, QPainter # Added QImage, QPainter
...
class ActionHandler(QObject):
    ...
    def connect_actions(self):
        ...
        self.mw.export_png_action.triggered.connect(self.on_export_png) # New
        self.mw.export_svg_action.triggered.connect(self.on_export_svg) # New
    ...
    def on_export_png(self):
        ...
        if file_path:
            try:
                ...
                image.fill(Qt.transparent) # or Qt.white or self.mw.scene.backgroundBrush().color()

                painter = QPainter(image)
                painter.setRenderHint(QPainter.Antialiasing)
                
                # Clear selection before rendering for a clean export
                selected_items_backup = self.mw.scene.selectedItems()
                self.mw.scene.clearSelection()

                self.mw.scene.render(painter, QRectF(QPointF(0,0), image.size()), padded_rect)
                painter.end()
    ...
    def on_export_svg(self):
        ...
        if file_path:
            try:
                ...
                svg_generator.setViewBox(padded_rect) # Render this part of the scene
                ...
                painter.setRenderHint(QPainter.Antialiasing) # SVG supports this

                # Clear selection before rendering
                selected_items_backup = self.mw.scene.selectedItems()
                self.mw.scene.clearSelection()
                
                # Render only the padded_rect area of the scene
                self.mw.scene.render(painter, source=padded_rect) # Only render the source rect
                
                # Restore selection
                for item in selected_items_backup:
                    item.setSelected(True)
                
                painter.end() # Finalizes SVG generation
