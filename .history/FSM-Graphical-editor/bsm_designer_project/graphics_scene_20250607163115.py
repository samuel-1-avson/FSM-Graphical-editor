--- START OF FILE bsm_designer_project/graphics_scene.py ---
...
# Ensure correct relative import if undo_commands is in the same directory
try:
    from .undo_commands import AddItemCommand, MoveItemsCommand, RemoveItemsCommand, EditItemPropertiesCommand
except ImportError:
    from undo_commands import AddItemCommand, MoveItemsCommand, RemoveItemsCommand, EditItemPropertiesCommand

from snippet_manager import CustomSnippetManager # Added for type hinting
...
class DiagramScene(QGraphicsScene):
    ...
    validation_issues_updated = pyqtSignal(list) # list of (str: issue_description, QGraphicsItem: problematic_item_or_None)


    def __init__(self, undo_stack, parent_window=None, custom_snippet_manager: CustomSnippetManager | None = None): # parent_window is typically MainWindow
        ...
        self._validation_issues = [] # Stores tuples: (description_str, item_reference_or_None)
        self._problematic_items = set() # Stores QGraphicsItem instances with issues
    ...
    def _update_transitions_for_renamed_state(self, old_name:str, new_name:str):
        ...
        self.run_all_validations("StateRenamed") # Run validation after rename
    ...
    def set_mode(self, mode: str):
        ...
        if self.parent_window and hasattr(self.parent_window, 'mode_action_group'): # For MainWindow
            actions_map = {
        ...
        elif self.parent_window and hasattr(self.parent_window, 'sub_mode_action_group'): # For SubFSMEditorDialog
            actions_map_sub = {
    ...
    def _handle_item_moved_visual_update(self, moved_item):
        ...
        # No need to call run_all_validations() here for simple moves,
        # as it's about structural consistency. Call it after move is *committed* (mouseRelease).
    ...
    def _mark_item_as_problematic(self, item, problem_description="Validation Issue"):
        if hasattr(item, 'set_problematic_style'):
            # Pass True to indicate it's problematic, and the description
    ...
    def _clear_all_visual_validation_warnings(self):
        # Iterate over a copy of the set, as set_problematic_style(False) might modify the original set implicitly
        for item in list(self._problematic_items):
            if hasattr(item, 'set_problematic_style'):
                item.set_problematic_style(False) # Pass False to clear the problematic style
        self._problematic_items.clear()
        # self.update() # update() will be called at the end of run_all_validations
    ...
    def run_all_validations(self, trigger_source="unknown_source"):
        ...
        self._clear_all_visual_validation_warnings() # Clear previous visual warnings and _problematic_items set
        current_validation_issues = [] # Local list for this run
        ...
        if not states:
            ...
            self._validation_issues = current_validation_issues # Update scene's main list
        ...
        if not initial_states:
            ...
            current_validation_issues.append((issue_msg, None)) # Global issue
        ...
        unreachable_states_set = set() # To avoid double-flagging
        ...
        for state in states:
            if not state.is_final and state not in unreachable_states_set: # Only check reachable non-final states
                ...
                if not has_outgoing:
                    ...
                    if state.is_superstate and state.sub_fsm_data and state.sub_fsm_data.get('states'):
                        is_superstate_with_content = True # Simplification: if superstate has sub-states, assume not dead-end
        ...
        self.validation_issues_updated.emit(self._validation_issues)
        self.update() # Ensure scene redraws to show visual warnings
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            ...
            # Context menu logic moved to contextMenuEvent
            pass
        else:
            super().mousePressEvent(event)
    ...
    def mouseReleaseEvent(self, event: QGraphicsSceneMouseEvent):
        super().mouseReleaseEvent(event) # Call super first to ensure selection updates
            ...
            if self._mouse_press_items_positions:
                ...
                for item, old_pos in self._mouse_press_items_positions.items():
                    ...
                    if (final_snapped_pos - old_pos).manhattanLength() > 0.1 :
                        item.setPos(final_snapped_pos) # Set pos before adding to command for correct 'new_pos'
                        moved_items_data_for_command.append((item, old_pos, final_snapped_pos))
                         ...
                         # Item was dragged but object/grid snapping brought it back to original drag pos (not original press pos)
        ...
        self._clear_dynamic_guidelines()
        # super().mouseReleaseEvent(event) # Moved to top
    ...
    def _show_context_menu(self, item, global_pos): # This is now a local method for SubFSMEditorDialog scenes
        menu = QMenu()
    ...
    def edit_item_properties(self, item):
        ...
        if self.parent_window and self.parent_window.__class__.__name__ != "MainWindow":
            # This means self.parent_window is likely the SubFSMEditorDialog itself
            pass
        elif self.parent_window and self.parent_window.__class__.__name__ == "MainWindow":
            pass # Standard case
        elif self.views(): # Fallback if no direct parent_window link (e.g. test setup)
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if isinstance(item, GraphicsStateItem):
                ...
                if current_new_name != old_props.get('name') and existing_state_with_new_name and existing_state_with_new_name != item:
                    ...
                    return # Prevent applying changes if name conflict

        if dialog_executed_and_accepted and new_props_from_dialog is not None:
            final_new_props = old_props.copy()
            final_new_props.update(new_props_from_dialog) # Apply changes from dialog
        ...
        # self.update() # Not strictly needed here as EditItemPropertiesCommand redo will call item.update()
        # self.run_all_validations("EditItemProperties") # Called by EditItemPropertiesCommand.redo
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        ...
        if item_type == "State":
            ...
            if props_dialog.exec() == QDialog.Accepted:
                ...
                # Check for duplicate name *again* after dialog, as user might have changed it
            ...
            if self.current_mode == "state": # Revert mode after adding
                self.set_mode("select")
            if not current_item: return # If dialog cancelled or name conflict
            ...
            if comment_props_dialog.exec() == QDialog.Accepted:
                ...
                else: # User cleared text and clicked OK
        ...
        if current_item:
            ...
            self.undo_stack.push(cmd) # This will trigger redo, which adds item and runs validation
        ...
        # self.run_all_validations("AddItemInteractive") # Called by AddItemCommand.redo
    ...
    def _handle_transition_click(self, clicked_state_item: GraphicsStateItem, click_pos: QPointF):
        ...
        if not self.transition_start_item:
        ...
        else: # This is the second click (target state)
            if self._temp_transition_line:
                self.removeItem(self._temp_transition_line)
                self._temp_transition_line = None

            # Default properties for a new transition
            initial_props = {
                'event': "", 'condition': "",
                'action_language': DEFAULT_EXECUTION_ENV, 'action': "",
                'color': COLOR_ITEM_TRANSITION_DEFAULT, 'description':"",
                'control_offset_x':0, 'control_offset_y':0 # Default to straight line initially
            }
            ...
            if dialog.exec() == QDialog.Accepted:
                ...
                new_transition = GraphicsTransitionItem(
                    self.transition_start_item, clicked_state_item, # start_item, end_item
                ...
                self.undo_stack.push(cmd) # This will call redo, which runs validation
            ...
            self.set_mode("select") # Revert to select mode after transition attempt
        # self.run_all_validations("TransitionClick") # Called by AddItemCommand.redo
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if event.key() == Qt.Key_Delete or (event.key() == Qt.Key_Backspace and sys.platform != 'darwin'):
            ...
            for item in self.items(): # Check if an inline editor is active
            ...
            if active_editor_item: # If an inline editor is active, escape should cancel it
                editor_widget = active_editor_item._inline_editor_proxy.widget()
                if editor_widget:
                    active_editor_item._inline_edit_aborted = True
                    editor_widget.clearFocus() # Triggers editingFinished or focusOutEvent
                event.accept()
                return

            if self.current_mode == "transition" and self.transition_start_item:
                self.transition_start_item = None # Cancel drawing transition
                if self._temp_transition_line:
                    self.removeItem(self._temp_transition_line)
                    self._temp_transition_line = None
                ...
                event.accept()
                return
            elif self.current_mode != "select": # If in any other mode than select, revert to select
                self.set_mode("select")
                event.accept()
                return
    ...
    def delete_selected_items(self):
        ...
        for item in selected:
            ...
            if isinstance(item, GraphicsStateItem): # If deleting a state, also mark its transitions
        ...
        if items_to_delete_with_related:
            ...
            self.undo_stack.push(cmd) # Redo of this command will run validation
            self._log_to_parent("INFO", f"Queued deletion of {len(items_to_delete_with_related)} item(s).")
            self.clearSelection() # Clear selection after queuing for delete
        # self.run_all_validations("DeleteSelectedItems") # Called by RemoveItemsCommand.redo
    ...
    def dropEvent(self, event: QGraphicsSceneDragDropEvent):
        ...
        if mime_data.hasFormat(MIME_TYPE_BSM_TEMPLATE):
            ...
            try:
                ...
                event.ignore()
            # Validation will be run by AddItemCommands within _add_template_to_scene
            return

        elif mime_data.hasFormat("application/x-bsm-tool"):
            item_type_data_str = mime_data.text() # For simple tool drops

            grid_x = round(pos.x() / self.grid_size) * self.grid_size
            grid_y = round(pos.y() / self.grid_size) * self.grid_size

            if "State" in item_type_data_str: # Adjust drop pos for states
            ...
            # Determine type and initial properties from dragged tool
            ...
            event.acceptProposedAction()
            # Validation will be run by AddItemCommand within _add_item_interactive
        else:
    ...
    def get_diagram_data(self):
        ...
        for item in self.items():
            if isinstance(item, GraphicsStateItem):
                ...
                if item.start_item and item.end_item: # Ensure transition is valid before saving
    ...
    def load_diagram_data(self, data):
        self.clear() # Clears items, also selection
        self._problematic_items.clear() # Clear validation markers
        self._validation_issues = []
        self.set_dirty(False) # Start clean
        state_items_map = {}
        ...
        for comment_data in data.get('comments', []):
            ...
            comment_item.setTextWidth(comment_data.get('width', 150)) # default width if not specified
            self.addItem(comment_item)

        self.set_dirty(False) # Mark as clean after load
        if self.undo_stack: self.undo_stack.clear()
        self.run_all_validations("LoadDiagramData") # Validate after loading
        self.scene_content_changed_for_find.emit()
    ...
    def drawBackground(self, painter: QPainter, rect: QRectF):
        ...
        for x in range(left, right, self.grid_size):
            if x % (self.grid_size * 5) != 0: # Minor grid lines
                painter.drawLine(x, top, x, bottom)
        for y in range(top, bottom, self.grid_size):
            if y % (self.grid_size * 5) != 0: # Minor grid lines
    ...
    def copy_selected_items(self):
        ...
        for item in selected_items:
            if isinstance(item, (GraphicsStateItem, GraphicsCommentItem)): # Transitions not copied this way for simplicity
        ...
        if items_to_copy_data:
            ...
            try:
                ...
                mime_data_obj.setText(f"{len(items_to_copy_data)} BSM items copied") # For external paste as text
    ...
    def paste_items_from_clipboard(self):
        ...
        # Determine paste position (e.g., near cursor or offset from original)
        paste_center_pos = QPointF(100, 100) # Default fallback
        if self.views():
            view = self.views()[0]
        ...
        offset_delta = QPointF(self.grid_size, self.grid_size) # Small offset for multiple pasted items
        ...
        for i, item_info in enumerate(items_to_paste_data):
            ...
            if item_type == "State":
                ...
                new_name = self._generate_unique_state_name(original_name) # Ensure unique name
                # Snap pasted item to grid
                ...
                new_item = GraphicsStateItem(
                    ...
                    text=new_name, is_initial=False, # Pasted states are not initial by default
                    is_final=data.get('is_final', False),
                    color=data.get('color', COLOR_ITEM_STATE_DEFAULT_BG),
                    ...
                    # Deepcopy sub_fsm_data to avoid shared references if multiple pasted
            ...
            if new_item:
                ...
                self.undo_stack.push(cmd) # This will run validation via AddItemCommand.redo()
        ...
        if pasted_graphic_items:
            ...
            if self.views() and pasted_graphic_items: # Ensure pasted items are visible
                self.views()[0].ensureVisible(pasted_graphic_items[-1], 50, 50)
            # self.run_all_validations("PasteItems") # Called by AddItemCommand.redo()
            self.scene_content_changed_for_find.emit()
    ...
    def _generate_unique_state_name(self, base_name: str) -> str:
        ...
        # If base_name itself ends with _Copy<number>, try incrementing that number
        match = re.match(r"^(.*?)_Copy(\d+)$", base_name)
        if match:
            prefix = match.group(1)
            num = int(match.group(2))
        else: # If not, just append _Copy
            prefix = base_name
            num = 0 # Start copy numbering from 1

        # If base_name already exists, but doesn't fit the _Copy pattern,
        # or if it's something like "State1" and "State2" exists, we need a more robust way.
        # For now, this handles simple "Name" -> "Name_Copy1" or "Name_Copy1" -> "Name_Copy2"
        # It might also convert "State1" to "State1_Copy1" if "State1" exists.

        # A more robust approach if base_name is "State1" and "State1" exists:
        # try "State2", "State3", etc. before resorting to "_Copy".
        # This is a simplification for now.
        base_match_num = re.match(r"^(.*?)(\d+)$", base_name)
        if not match and base_match_num: # e.g. "State1"
            prefix_base = base_match_num.group(1)
            num_base = int(base_match_num.group(2))
            # Try incrementing the number part first
            next_num_base = num_base + 1
            while self.get_state_by_name(f"{prefix_base}{next_num_base}"):
                next_num_base += 1
            return f"{prefix_base}{next_num_base}"
        else: # Default to _Copy suffix
            next_num_copy = num + 1
            while self.get_state_by_name(f"{prefix}_Copy{next_num_copy}"):
                next_num_copy += 1
            return f"{prefix}_Copy{next_num_copy}"

    def _add_template_to_scene(self, template_data: dict, drop_pos: QPointF):
        """Adds items from a template to the scene."""
        ...
        state_items_map = {} # Maps original template name to new scene item instance

        # Determine offset: align top-left of template's bounding box with drop_pos
        ...
        # Handle potential name clashes by suffixing if needed
        template_instance_suffix = ""
        # Check if any state from the template would clash if added directly
        if any(self.get_state_by_name(s_data.get('name', "State")) for s_data in template_data.get('states', [])):
            i = 1
            # Find a suffix that makes all state names from the template unique in the current scene
        ...
        for state_data in template_data.get('states', []):
            original_name = state_data.get('name', "State") # Original name from template
            unique_name_base = f"{original_name}{template_instance_suffix}"
            # Ensure this specific derived name is unique, _generate_unique_state_name might add _CopyN if needed
            unique_name = self._generate_unique_state_name(unique_name_base)


            # Calculate new position based on template's relative position and drop point
            ...
            state_item = GraphicsStateItem(
                ...
                text=unique_name,
                is_initial=state_data.get('is_initial', False) if not self.items() else False, # Only first template makes initial state if scene empty
                is_final=state_data.get('is_final', False),
                color=state_data.get('color', COLOR_ITEM_STATE_DEFAULT_BG),
                action_language=state_data.get('action_language', DEFAULT_EXECUTION_ENV),
                ...
                sub_fsm_data=json.loads(json.dumps(state_data.get('sub_fsm_data', {'states':[], 'transitions':[], 'comments':[]}))) # Deep copy
            )
            if self.parent_window and hasattr(self.parent_window, 'connect_state_item_signals') and self.parent_window.__class__.__name__ == "MainWindow":
                self.parent_window.connect_state_item_signals(state_item)

            cmd = AddItemCommand(self, state_item, f"Add State from Template: {unique_name}")
            self.undo_stack.push(cmd) # This will run validation
            newly_created_scene_items.append(state_item)
            state_items_map[original_name] = state_item # Map original template name to new scene item
        ...
        for trans_data in template_data.get('transitions', []):
            ...
            src_item = state_items_map.get(src_original_name) # Use map to get correct instance
            ...
            if src_item and tgt_item:
                ...
                self.undo_stack.push(cmd) # Runs validation
        ...
        if newly_created_scene_items:
            ...
            # self.run_all_validations("AddTemplate") # Called by AddItemCommands
            self.scene_content_changed_for_find.emit()
            if self.views() and newly_created_scene_items:
                # Ensure the newly added group of items is visible
...
class ZoomableView(QGraphicsView):
    ...
    def __init__(self, scene, parent=None):
        ...
        self.zoom_level_steps = 0 # To track discrete zoom steps
        self.setTransformationAnchor(QGraphicsView.AnchorUnderMouse) # Zoom towards mouse
        self.setResizeAnchor(QGraphicsView.AnchorViewCenter) # Resize around center
        self._is_panning_with_space = False
        self._is_panning_with_mouse_button = False
        self._last_pan_point = QPoint()
        self._emit_current_zoom() # Emit initial zoom level

    def _emit_current_zoom(self): # Helper to emit current zoom factor
        current_scale_factor = self.transform().m11() # m11 is horizontal scale
    ...
    def wheelEvent(self, event: QWheelEvent):
        if event.modifiers() & Qt.ControlModifier: # Zoom with Ctrl + Mouse Wheel
            ...
            # Limit zoom range
            min_zoom_level_steps = -15 # Corresponds to roughly 10% zoom
            max_zoom_level_steps = 15  # Corresponds to roughly 800% zoom

            if min_zoom_level_steps <= new_zoom_level_steps <= max_zoom_level_steps:
                self.scale(factor, factor)
                self.zoom_level_steps = new_zoom_level_steps
                self._emit_current_zoom() # Emit signal after zoom
            event.accept()
        else:
    ...
    def zoom_in(self):
        ...
        if new_zoom_level_steps <= 15: # Max zoom limit
            self.scale(factor, factor)
            self.zoom_level_steps = new_zoom_level_steps
    ...
    def zoom_out(self):
        factor = 1 / 1.12
        new_zoom_level_steps = self.zoom_level_steps - 1
        if new_zoom_level_steps >= -15: # Min zoom limit
            self.scale(factor, factor)
            self.zoom_level_steps = new_zoom_level_steps
            self._emit_current_zoom()

    def reset_view_and_zoom(self):
        self.resetTransform() # Resets zoom and pan
        self.zoom_level_steps = 0
        if self.scene():
            content_rect = self.scene().itemsBoundingRect()
            if not content_rect.isEmpty():
                self.centerOn(content_rect.center()) # Center on content
            elif self.scene().sceneRect(): # Fallback to scene rect if no items
    ...
    def fitInView(self, rect: QRectF, aspectRadioMode: Qt.AspectRatioMode = Qt.IgnoreAspectRatio):
        ...
        self._emit_current_zoom() # Update zoom status after fitInView

    def setTransform(self, matrix: QTransform, combine: bool = False):
        super().setTransform(matrix, combine)
        self._emit_current_zoom() # Update zoom status after any transform change
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_Space and not self._is_panning_with_space and not event.isAutoRepeat():
            ...
            event.accept()
        elif event.key() == Qt.Key_Plus or event.key() == Qt.Key_Equal: # Numpad Plus or regular Plus/Equal
            self.zoom_in()
        elif event.key() == Qt.Key_Minus: # Numpad Minus or regular Minus
            self.zoom_out()
        elif event.key() == Qt.Key_0 or event.key() == Qt.Key_Asterisk: # Numpad 0 or Asterisk for reset
    ...
    def keyReleaseEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_Space and self._is_panning_with_space and not event.isAutoRepeat():
            self._is_panning_with_space = False
            if not self._is_panning_with_mouse_button: # Only restore if not also middle-mouse panning
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        if self._is_panning_with_mouse_button and \
           (event.button() == Qt.MiddleButton or (self._is_panning_with_space and event.button() == Qt.LeftButton)):
            self._is_panning_with_mouse_button = False
            if self._is_panning_with_space: # If space was held, revert to OpenHand
                self.setCursor(Qt.OpenHandCursor)
            else: # Otherwise, revert to normal mode cursor
                self._restore_cursor_to_scene_mode()
            event.accept()
        else:
    ...
    def _restore_cursor_to_scene_mode(self):
        # Get current mode from the scene and set cursor accordingly
        ...
        if current_scene_mode == "select":
            ...
            self.setCursor(Qt.PointingHandCursor) # Or a custom transition cursor
        else:
    ...
    def zoom_to_rect(self, target_rect: QRectF, padding_factor: float = 0.1):
        ...
        self.fitInView(padded_rect, Qt.KeepAspectRatio) # fitInView also updates transform and emits zoomChanged
    ...
    def fit_diagram_in_view(self):
        ...
        else: # If no items, fit the whole sceneRect (if defined) or default view
