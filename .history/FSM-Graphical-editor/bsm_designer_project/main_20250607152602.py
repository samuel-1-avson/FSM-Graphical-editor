...
class MainWindow(QMainWindow):
    ...
    def _update_window_title(self):
        ...
        # PySim status (for window title and potentially main_op_status_label)
        pysim_title_suffix = ""
        main_op_pysim_text = ""
        if self.py_sim_active:
        ...
        window_title_str = f"{APP_NAME} - {file_name_part}{diagram_dirty_char}{pysim_title_suffix} [*]"
        self.setWindowTitle(window_title_str)

        # Update main_op_status_label
        main_op_text = f"File: {file_name_part}{diagram_dirty_char}" # Default to file status
        
        if hasattr(self, 'progress_bar') and self.progress_bar.isVisible(): # MATLAB op takes precedence
        ...
        elif self.py_sim_active: # Then PySim status
            main_op_text = main_op_pysim_text
        elif not self.current_file_path and not self.scene.is_dirty(): # New, clean diagram
            main_op_text = "Ready"
        # Else, it remains the "File: ..." status

        if hasattr(self, 'main_op_status_label'):
            self.main_op_status_label.setText(main_op_text)
        
        self._update_py_sim_status_display() # Keep dedicated PySim label updated
    ...
    # ... (Methods _update_properties_dock, _update_dock_color_button_style, _on_dock_color_button_clicked, _on_dock_property_changed_mw, _on_apply_dock_properties, _on_revert_dock_properties remain as previously provided)
    # ...
