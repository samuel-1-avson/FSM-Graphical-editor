...
import sys
import os # Added os for icon path check
import json
...
from config import (
    ...
    DEFAULT_TRANSITION_LINE_STYLE, DEFAULT_TRANSITION_LINE_WIDTH, DEFAULT_TRANSITION_ARROWHEAD, # For transition defaults
...
class SubFSMEditorDialog(QDialog):
    def __init__(self, sub_fsm_data_initial: dict, parent_state_name: str, parent_window_ref=None):
        ...
        if IMPORTS_SUCCESSFUL: 
            ...
            try:
                self.json_text_edit.set_language("JSON") # Assuming CodeEditor supports "JSON"
            except Exception: 
...
class TransitionPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_transition=False, custom_snippet_manager: CustomSnippetManager | None = None):
        ...
        tabs = QTabWidget() # Use tabs for better organization
        main_layout.addWidget(tabs)
...
class SettingsDialog(QDialog):
    def __init__(self, settings_manager: SettingsManager, parent=None):
        ...
        state_defaults_form_layout = QFormLayout(state_defaults_group)
        ...
        # Transition Defaults Group
        transition_defaults_group = QGroupBox("Default Transition Visuals")
        transition_defaults_form_layout = QFormLayout(transition_defaults_group)
        ...
        # Comment Defaults Group
        comment_defaults_group = QGroupBox("Default Comment Visuals")
        comment_defaults_form_layout = QFormLayout(comment_defaults_group)
    ...
    def _connect_change_signals_for_apply_button(self):
        # Connect signals from UI elements to enable Apply button
        # View Tab
        self.show_grid_cb.toggled.connect(self._on_setting_ui_changed)
        self.snap_to_grid_cb.toggled.connect(self._on_setting_ui_changed)
        self.snap_to_objects_cb.toggled.connect(self._on_setting_ui_changed)
        self.show_snap_guidelines_cb.toggled.connect(self._on_setting_ui_changed)
        # Appearance Tab
        self.theme_combo.currentTextChanged.connect(self._on_setting_ui_changed)
        # Color buttons already handled via _pick_color_for_button
        # Item Defaults Tab
        self.state_default_shape_combo.currentTextChanged.connect(self._on_setting_ui_changed)
        self.state_default_font_family_combo.currentFontChanged.connect(self._on_setting_ui_changed)
        self.state_default_font_size_spin.valueChanged.connect(self._on_setting_ui_changed)
        self.state_default_font_bold_cb.toggled.connect(self._on_setting_ui_changed)
        self.state_default_font_italic_cb.toggled.connect(self._on_setting_ui_changed)
        self.state_default_border_style_combo.currentTextChanged.connect(self._on_setting_ui_changed)
        self.state_default_border_width_spin.valueChanged.connect(self._on_setting_ui_changed)
        self.transition_default_line_style_combo.currentTextChanged.connect(self._on_setting_ui_changed)
        self.transition_default_line_width_spin.valueChanged.connect(self._on_setting_ui_changed)
        self.transition_default_arrowhead_combo.currentTextChanged.connect(self._on_setting_ui_changed)
        self.transition_default_font_family_combo.currentFontChanged.connect(self._on_setting_ui_changed)
        self.transition_default_font_size_spin.valueChanged.connect(self._on_setting_ui_changed)
        self.comment_default_font_family_combo.currentFontChanged.connect(self._on_setting_ui_changed)
        self.comment_default_font_size_spin.valueChanged.connect(self._on_setting_ui_changed)
        self.comment_default_font_italic_cb.toggled.connect(self._on_setting_ui_changed)
        # Behavior Tab
        self.resource_monitor_enabled_cb.toggled.connect(self._on_setting_ui_changed)
        self.resource_monitor_interval_spin.valueChanged.connect(self._on_setting_ui_changed)
    ...
    def _pick_color_for_button(self, button: QPushButton, setting_key: str):
        current_color_hex = button.property("pendingColorHex") # Use pending color as initial
        if not current_color_hex: # Fallback if property not set yet (should be on load)
        ...
        if dialog.exec_():
            ...
            if new_color.isValid() and new_color.name() != initial_color.name(): # Compare hex names
    ...
    def load_settings_to_ui(self):
        ...
        # Block signals while loading to prevent premature enabling of Apply button
        ...
        # Color buttons don't emit signals directly, handled by property and _pick_color_for_button
