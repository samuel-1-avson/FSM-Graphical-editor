...
class JSONHighlighter(QSyntaxHighlighter):
    def __init__(self, parent=None):
        ...
        # Key (string in quotes followed by a colon)
        keyFormat = QTextCharFormat()
        keyFormat.setForeground(QColor("#9CDCFE")) # Light Blue
        key_pattern = r'"([^"]*)"\s*:'
        self.highlightingRules.append(HighlightingRule(key_pattern, keyFormat, 1))

        # String values
        stringFormat = QTextCharFormat()
        stringFormat.setForeground(QColor("#CE9178")) # Orange
        self.highlightingRules.append(HighlightingRule(r'":\s*"(.*?)(?<!\\)"', stringFormat, 1, minimal=True))

        # Number values
        numberFormat = QTextCharFormat()
        numberFormat.setForeground(QColor("#B5CEA8")) # Light Green
        self.highlightingRules.append(HighlightingRule(r'\b-?[0-9]+\.?[0-9]*([eE][-+]?[0-9]+)?\b', numberFormat))

        # Boolean values
        booleanFormat = QTextCharFormat()
        booleanFormat.setForeground(QColor("#569CD6")) # Blue
        self.highlightingRules.append(HighlightingRule(r'\b(true|false)\b', booleanFormat))

        # Null value
        nullFormat = QTextCharFormat()
        nullFormat.setForeground(QColor("#C586C0")) # Purple
        nullFormat.setFontItalic(True)
        self.highlightingRules.append(HighlightingRule(r'\bnull\b', nullFormat))
        ...
        for rule in self.highlightingRules:
            ...
            if rule.nth > 0:
                ...
                while index >= 0:
                    ...
                    if new_offset <= offset : new_offset = offset + 1
                    offset = new_offset
                    if offset >= len(text) or expression.matchedLength() == 0: break
                    index = expression.indexIn(text, offset)
            else:
                index = expression.indexIn(text, offset)
                while index >= 0:
                    length = expression.matchedLength()
                    if length > 0: self.setFormat(index, length, rule.format)
                    new_offset = index + length
                    if new_offset <= offset: new_offset = offset + 1
                    offset = new_offset
                    if offset >= len(text) or length == 0: break
                    index = expression.indexIn(text, offset)
