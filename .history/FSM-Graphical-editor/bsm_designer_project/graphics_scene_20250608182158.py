...
class DiagramScene(QGraphicsScene):
    ...
    def drawBackground(self, painter: QPainter, rect: QRectF):
        ...
        if settings:
            ...
            # Update pens based on current theme settings if not done elsewhere dynamically
            self.grid_pen_light.setColor(QColor(settings.get("canvas_grid_minor_color")))
            self.grid_pen_dark.setColor(QColor(settings.get("canvas_grid_major_color")))
            self._guideline_pen.setColor(QColor(settings.get("canvas_snap_guideline_color")))
            self.setBackgroundBrush(QColor(config.COLOR_BACKGROUND_LIGHT))
    ...
    def set_mode(self, mode: str):
        ...
        if self.parent_window and hasattr(self.parent_window, 'mode_action_group'):
            ...
            actions_map_sub = {
                ...
                "state": getattr(self.parent_window, 'sub_state_action', None), # Corrected from 'sub_add_state_action'
                "transition": getattr(self.parent_window, 'sub_transition_action', None),# Corrected
                "comment": getattr(self.parent_window, 'sub_comment_action', None) # Corrected
            }
    ...
    def run_all_validations(self, trigger_source="unknown_source"):
        ...
        if not states:
            if not current_validation_issues: # Only add this if no other specific issue was found for empty diagram
        ...
        elif not initial_states: # If no initial state, all states are technically unreachable by definition
            for s_state in states:
                unreachable_states_set.add(s_state)
        ...
        for t in transitions:
            ...
            elif t.start_item not in states or t.end_item not in states: # Check if they are actually in our current list of states
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            if self.current_mode == "state":
                ...
                super().mousePressEvent(event) # Important for item selection/move handling

        elif event.button() == Qt.RightButton:
            pass # Context menu handled by contextMenuEvent
        else:
    ...
    def contextMenuEvent(self, event: QGraphicsSceneContextMenuEvent):
        ...
        if item_at_pos and isinstance(item_at_pos, (GraphicsStateItem, GraphicsTransitionItem, GraphicsCommentItem)):
            if self.parent_window and hasattr(self.parent_window, '_show_context_menu_for_item_from_scene'):
                ...
                event.accept()
                return
            else: # Fallback for SubFSMEditorDialog context
                if hasattr(self, '_show_context_menu'): # Ensure the method exists (it should be defined in DiagramScene)
            ...
            if self.parent_window and self.parent_window.__class__.__name__ == "MainWindow": # Check if parent is MainWindow
                 menu.addSeparator()
            ...
            for moving_ref_name, moving_x_val in moving_item_refs_x.items():
                for other_x_val in other_item_snap_points_x:
                    ...
                    if abs(diff_x) < min_offset_x:
                        ...
                        pass # Could add logic for tie-breaking if needed
                ...
                for item, old_pos in self._mouse_press_items_positions.items():
                    ...
                    if (final_snapped_pos - old_pos).manhattanLength() > 0.1 :
                        ...
                        moved_items_data_for_command.append((item, old_pos, final_snapped_pos))
                         ...
                         (final_snapped_pos - current_item_pos_after_drag).manhattanLength() < 0.1: # Moved but snapped back to original or near
                         # This case means it moved but snapping brought it back to a non-dirty position relative to original
                         # but if it moved at ALL from its press pos, we consider it for undo
    ...
    def _show_context_menu(self, item, global_pos): 
        ...
        if isinstance(item, GraphicsStateItem) and item.is_superstate:
            # Option to edit sub-machine could be here if needed,
            # but usually done via properties dialog.
            pass
    ...
    def edit_item_properties(self, item):
        ...
        if self.parent_window and self.parent_window.__class__.__name__ != "MainWindow" and \
           self.parent_window.__class__.__name__ != "SubFSMEditorDialog": # Allow SubFSMEditorDialog as parent
            pass # It's some other parent context (e.g., test)
            ...
            pass # Correct parent type
        elif self.views(): # Fallback if no explicit parent_window but view exists
            dialog_parent = self.views()[0]
        else:
            dialog_parent = None # No suitable parent found
