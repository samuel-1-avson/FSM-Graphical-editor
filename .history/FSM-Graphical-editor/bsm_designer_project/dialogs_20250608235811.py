...
class SettingsDialog(QDialog):
    # Constants for preview item sizes
    PREVIEW_WIDTH = 150
    PREVIEW_HEIGHT = 80
    ...
    def __init__(self, settings_manager: SettingsManager, parent=None):
        ...
        self.setMinimumWidth(600) # Increased width to accommodate previews
        ...
        # Temporary graphics items for previews
        self._preview_state_item = None
        self._preview_transition_item_start = None # Need two dummy states for transition
        ...
        state_defaults_outer_layout = QHBoxLayout(state_defaults_group) # Outer layout for form + preview
        state_defaults_form_layout = QFormLayout() # Form for controls
        ...
        state_defaults_outer_layout.addLayout(state_defaults_form_layout, 2) # Controls take more space
        ...
        state_defaults_outer_layout.addWidget(self.state_preview_label, 1) # Preview takes less space
        ...
        self._create_preview_items() # Create items after UI is built
        self.load_settings_to_ui() # This will also trigger initial preview rendering
    ...
    def _create_preview_items(self):
        ...
        self._preview_state_item.setFlag(QGraphicsItem.ItemIsMovable, False)
        self._preview_state_item.setFlag(QGraphicsItem.ItemIsSelectable, False)

        # Transition Preview Items
        # Create dummy start/end states for the transition preview. These won't be added to any scene.
        self._preview_transition_item_start = GraphicsStateItem(0,0,1,1, "src")
        self._preview_transition_item_end = GraphicsStateItem(0,0,1,1, "tgt")
        ...
        self._preview_transition_item.setFlag(QGraphicsItem.ItemIsMovable, False)
        self._preview_transition_item.setFlag(QGraphicsItem.ItemIsSelectable, False)

        # Comment Preview Item
        self._preview_comment_item = GraphicsCommentItem(0, 0, "Comment text...")
        self._preview_comment_item.setFlag(QGraphicsItem.ItemIsMovable, False)
        self._preview_comment_item.setFlag(QGraphicsItem.ItemIsSelectable, False)


    def _render_preview(self, item: QGraphicsItem, label: QLabel, width: int, height: int, is_transition=False):
        if not item: return
        ...
        # For transitions, we need to set up dummy start/end item positions for the path calculation
        if is_transition and isinstance(item, GraphicsTransitionItem):
            padding = 10
            item.start_item.setPos(padding, height / 2)
            item.end_item.setPos(width - padding - item.start_item.rect().width(), height / 2) # Small width for dummy states
            item.update_path() # Recalculate path based on new dummy positions
             # Render the item centered within the pixmap
            item_rect = item.boundingRect()
            target_rect = QRectF(0,0, width, height)
            # We need to translate the painter so the item is drawn correctly
            # For transitions, bounding rect can be tricky. Let's try to render its path more directly.
            painter.translate(-item_rect.topLeft() + target_rect.center() - item_rect.center())

        else: # For states and comments
            item_rect = item.boundingRect()
            # Center the item in the pixmap
            target_x = (width - item_rect.width()) / 2
            target_y = (height - item_rect.height()) / 2
            # Temporarily set item position for rendering if it's not (0,0)
            original_pos = item.pos()
            item.setPos(target_x - item_rect.x(), target_y - item_rect.y())


        # Temporarily disable selection style for preview
        was_selected = item.isSelected()
        if was_selected: item.setSelected(False)
        item.paint(painter, None, None) # Render item
        if was_selected: item.setSelected(True) # Restore selection state
        
        if not is_transition:
            item.setPos(original_pos) # Restore original pos if it was state/comment
    ...
    def _update_default_item_previews(self):
        ...
        if self._preview_state_item and self.state_preview_label:
            self._preview_state_item.set_properties(
                name="State", is_initial=False, is_final=False, # Keep simple text for preview
                color_hex=self.settings_manager.get("state_default_color", COLOR_ITEM_STATE_DEFAULT_BG), # Placeholder if needed
        ...
        if self._preview_transition_item and self.transition_preview_label:
            self._preview_transition_item.set_properties(
                event_str="Event", # Simple text
                color_hex=self.settings_manager.get("transition_default_color", COLOR_ITEM_TRANSITION_DEFAULT), # Placeholder
        ...
        if self._preview_comment_item and self.comment_preview_label:
            ...
            # Adjust comment width for preview
        ...
        self._on_setting_ui_changed() # Enable apply button
    ...
    def _pick_color_for_button(self, button: QPushButton, setting_key: str):
        ...
        if dialog.exec_():
            ...
            if new_color.isValid() and new_color.name() != initial_color.name(): 
                ...
                self._update_default_item_previews() # Update previews on color change
    ...
    def load_settings_to_ui(self):
        ...
        all_widgets_with_signals = [
            ...
            self.comment_default_font_italic_cb,
            self.resource_monitor_enabled_cb, self.resource_monitor_interval_spin,
            self.grid_minor_color_button, self.grid_major_color_button, self.snap_guideline_color_button # Add color buttons
        ]
        ...
        self._update_default_item_previews() # Render initial previews
