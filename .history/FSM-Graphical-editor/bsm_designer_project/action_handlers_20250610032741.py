...
class ActionHandler(QObject):
    ...
    def connect_actions(self):
        ...
        self.mw.generate_python_code_action.triggered.connect(self.on_generate_python_code)
    ...
    def on_generate_python_code(self):
        ...
        if mw.current_file_path: default_classname_base = os.path.splitext(os.path.basename(mw.current_file_path))[0]
        default_classname_base = "".join(c if c.isalnum() else '_' for c in default_classname_base.title().replace("_", ""))
        if not default_classname_base or not default_classname_base[0].isalpha(): default_classname_base = "Fsm" + (default_classname_base if default_classname_base else "Model")
        
        output_dir = QFileDialog.getExistingDirectory(mw, "Select Output Directory for Python Code", QDir.homePath())
        if output_dir:
            classname, ok = QInputDialog.getText(mw, "Python Class Name", "Enter a name for the FSM class:", QLineEdit.Normal, default_classname_base)
            if ok and classname.strip():
                ...
                diagram_data = mw.scene.get_diagram_data()
                try:
                    py_file_path = generate_python_fsm_file(diagram_data, output_dir, classname)
                    QMessageBox.information(mw, "Python Code Generation Successful", f"Generated Python FSM file:\n{py_file_path}")
