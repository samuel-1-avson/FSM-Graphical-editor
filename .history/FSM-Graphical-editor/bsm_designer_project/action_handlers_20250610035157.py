...
from .python_code_generator import generate_python_fsm_file # Added for Python export
...
class ActionHandler(QObject):
    ...
    def connect_actions(self):
        ...
        self.mw.generate_python_code_action.triggered.connect(self.on_generate_python_code) # Added
    ...
    def on_save_file(self) -> bool:
        ...
        if self.mw.scene.is_dirty():
             return self.mw._save_to_path(self.mw.current_file_path)
        return True
    ...
    @pyqtSlot()
    def on_generate_python_code(self):
        mw = self.mw
        if not mw.scene.items():
            QMessageBox.information(mw, "Empty Diagram", "Cannot generate code for an empty diagram.")
            return

        default_classname_base = "MyFSM"
        if mw.current_file_path:
            default_classname_base = os.path.splitext(os.path.basename(mw.current_file_path))[0]
            default_classname_base = "".join(c if c.isalnum() else '_' for c in default_classname_base)
            default_classname_base = default_classname_base.strip('_')
            if default_classname_base and default_classname_base[0].isdigit():
                default_classname_base = "FSM_" + default_classname_base
            if default_classname_base:
                default_classname_base = default_classname_base[0].upper() + default_classname_base[1:]
            else:
                default_classname_base = "MyFSM"

        output_dir = QFileDialog.getExistingDirectory(mw, "Select Output Directory for Python Code", QDir.homePath())
        if output_dir:
            class_name, ok = QInputDialog.getText(
                mw, "Python Class Name", "Enter a name for the FSM class:",
                QLineEdit.Normal, default_classname_base)
            if ok and class_name.strip():
                class_name = class_name.strip()
                if not class_name.isidentifier():
                    QMessageBox.warning(mw, "Invalid Class Name", "The provided name is not a valid Python identifier.")
                    return
                diagram_data = mw.scene.get_diagram_data()
                try:
                    py_file_path = generate_python_fsm_file(diagram_data, output_dir, class_name)
                    QMessageBox.information(
                        mw, "Python Code Generation Successful",
                        f"Generated file:\n{py_file_path}")
                    logger.info(f"Python FSM code generated successfully to {py_file_path}")
                except Exception as e:
                    QMessageBox.critical(mw, "Python Code Generation Error", f"Failed to generate Python code: {e}")
                    logger.error(f"Error generating Python code: {e}", exc_info=True)
            elif ok:
