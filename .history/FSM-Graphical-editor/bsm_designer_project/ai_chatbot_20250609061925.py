...
class AIChatUIManager(QObject):
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        self.ai_chat_display.setTextInteractionFlags(Qt.TextBrowserInteraction | Qt.LinksAccessibleByMouse) # Ensure links are interactive
        # self.ai_chat_display.setOpenLinks(False) # This line caused the AttributeError
    ...
    def on_chat_anchor_clicked(self, url: QUrl):
        if url.scheme() == 'copycode':
                ...
                if self.ai_chat_status_label:
                    ...
                    def revert_status():
                        if hasattr(self, '_pending_status_update') and self._pending_status_update:
                            # Apply the pending status update first
                            pending_enum, pending_text = self._pending_status_update
                            self._pending_status_update = None # Clear it
                            self.update_status_display(pending_enum, pending_text) # This will call update_status_display again
            ...
            if url.scheme() in ['http', 'https', 'file']:
                # Check if QDesktopServices is available and callable
