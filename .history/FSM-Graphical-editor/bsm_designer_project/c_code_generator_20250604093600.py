...
def parse_c_function_name_to_action_details(c_func_name_full: str) -> tuple[str | None, str | None, str | None, str | None]:
    ...
    if name_part.startswith("entry_action_"):
        ...
        # Expecting Source_to_Target or Source_to_Target_Event
        parts = remainder.split("_to_")
        if len(parts) == 2:
            source_name_c_safe = parts[0]
            target_and_event_part = parts[1]
            
            # Try to find the longest possible event name by iterating common prefixes
            # This is heuristic because event names can also contain underscores.
            # A more robust way would be to know all state names during parsing.
            # For now, assume event is the last part if multiple underscores exist after target.
            
            possible_event_idx = -1
            # Try to identify event part by looking for an underscore separating target from event
            # This assumes target names don't typically end in "_eventpart" if eventpart exists
            # and event names are usually shorter or single words.
            
            # A simple split: if more than one underscore, assume the last part is event
            ...
            if len(target_event_split) > 1:
                 # This is tricky. If target name itself has underscores.
                 # Let's assume event is simple for now or the last segment.
                 # Reconstruct target if it had underscores, assuming event is the last part.
                 potential_event = target_event_split[-1]
                 potential_target = "_".join(target_event_split[:-1])

                 # This heuristic needs improvement if state names can be "A_B" and event "C_D"
                 # For now, let's assume if there are multiple parts, the last one is the event.
                 # If only one part, it's the target, no event string was in the FSM for this transition.
                 if len(target_event_split) > 1 : #  e.g. Target_Event or Target_Part1_Event_Part2
                     # This logic is still imperfect for complex names.
                     # A better way: when generating function names, use a clear separator like "__EVT__"
                     # For now, assume simple event or last part if underscores.
                     
                     # Try matching known event names first (if we had a list of sanitized event names)
                     # As a fallback, assume last segment is event if multiple parts
                     event_name_c_safe = target_event_split[-1]
                     target_name_c_safe = "_".join(target_event_split[:-1])
                     if not target_name_c_safe: # Case like "_to_EventName" (no target) - unlikely
                         target_name_c_safe = event_name_c_safe # Assume it was target, no event
                         event_name_c_safe = None
    ...
    for state_data in states:
        ...
        for trans_data in transitions:
            if trans_data['source'] == original_state_name:
                ...
                if event_name_str and event_name_str in event_name_to_id:
                    ...
                    # Eventless transitions might be evaluated if no event is given OR if a given event doesn't match others
                    # For simplicity now, let's assume they are only checked if FSM_NO_EVENT,
                    # or they could be the final 'else' in an event processing chain.
                    # Current logic: if event_id is not FSM_NO_EVENT, this event_check will be false.
                    # This means eventless transitions are only checked on internal steps.
                    # This might need refinement based on desired semantics.
                     event_check = "(event_id == FSM_NO_EVENT)" # Only for internal step
                     if not first_trans_for_state_event_block: if_keyword = "else if" # Could be an else if to a during action block
                     else: if_keyword = "if"
        ...
        for c_func_signature, original_py_code, action_type_orig, name1_orig, name2_orig, event_orig in sorted(list(action_prototypes_details), key=lambda x: x[0]):
            ...
            c_code.append(f"{c_func_signature[:-1]} {{ // Original action source: {action_type_orig} for {name1_orig}" + 
                          (f" to {name2_orig} on {event_orig}" if name2_orig else "") + 
