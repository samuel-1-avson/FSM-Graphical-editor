...
class MainWindow(QMainWindow):
    ...
    # --- NEW PYTHON SIMULATION METHODS ---
# (Ensure this section is part of your MainWindow class and correctly indented)
...
def _update_py_simulation_dock_ui(self):
    if not (hasattr(self, 'py_fsm_engine') and self.py_fsm_engine):
        if hasattr(self, 'py_sim_current_state_label'):
            self.py_sim_current_state_label.setText("<i>Not Running</i>")
        if hasattr(self, 'py_sim_variables_table'):
            self.py_sim_variables_table.setRowCount(0)
        self._highlight_sim_active_state(None)
        return
...
def on_start_py_simulation(self):
    if hasattr(self, 'py_sim_active') and self.py_sim_active:
    ...
    if not hasattr(self, 'scene'):
        QMessageBox.critical(self, "Error", "Diagram scene not available.")
        return
    ...
    try:
        ...
        self.py_fsm_engine = None # Ensure engine is None on failure
...
def on_stop_py_simulation(self, silent=False):
    if not (hasattr(self, 'py_sim_active') and self.py_sim_active) and not silent :
        return 
    ...
    self._update_py_simulation_dock_ui() 
    if hasattr(self, 'py_sim_action_log_output'):
...
def on_step_py_simulation(self):
    ...
    try:
        new_state_name, log_entries = self.py_fsm_engine.step(event_name=None, event_payload=None) 
        ...
        QMessageBox.warning(self, "Simulation Step Error", 
                            f"An error occurred during the FSM step:\n{e}")
        self._append_to_py_simulation_log([f"ERROR DURING INTERNAL STEP: {e}"])
    except Exception as e:
        QMessageBox.critical(self, "Unexpected Error", 
                             f"An unexpected error occurred during FSM step:\n{e}")
        self._append_to_py_simulation_log([f"CRITICAL ERROR DURING INTERNAL STEP: {e}"])
        self.on_stop_py_simulation(silent=True)

def on_trigger_py_event(self):
    if not (hasattr(self, 'py_fsm_engine') and self.py_fsm_engine):
        QMessageBox.warning(self, "Simulation Not Active", 
                            "Python simulation is not running or has not been initialized.")
        return
    ...
    event_payload_dict = None # For future extension: allow payload input from GUI

    try:
        new_state_name, log_entries = self.py_fsm_engine.step(event_name=event_name, 
        ...
        QMessageBox.warning(self, "Simulation Event Error", 
                            f"An error occurred processing event '{event_name}':\n{e}")
        self._append_to_py_simulation_log([f"ERROR DURING EVENT '{event_name}': {e}"])
    except Exception as e:
        QMessageBox.critical(self, "Unexpected Error", 
                             f"An unexpected error occurred processing event '{event_name}':\n{e}")
