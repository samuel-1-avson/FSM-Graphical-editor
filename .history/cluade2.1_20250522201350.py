...
# In class DiagramScene:
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if isinstance(event, QMouseEvent) and not isinstance(event, QGraphicsSceneMouseEvent):
            # This is a fallback if the view somehow didn't convert the event.
            # This situation is unusual and points to an issue in event propagation from the view.
            # Assuming the view is self.views()[0] if available.
            if self.views():
                view = self.views()[0]
                pos_in_scene_coords = view.mapToScene(event.pos()) # event.pos() is QPoint for QMouseEvent
            else:
                # Cannot map without a view, might have to ignore or use a default
                print("Warning: Received QMouseEvent in scene without a view to map coordinates.")
                super().mousePressEvent(event) # Or handle error
                return
            # Now use pos_in_scene_coords for your logic
            pos = pos_in_scene_coords
