...
# Initialize Pygame
pygame.init()
pygame.mixer.init(frequency=22050, size=-16, channels=2, buffer=512) # For sound
...
GRID_X_OFFSET = 50
GRID_Y_OFFSET = 50
SIDE_PANEL_WIDTH = 250 # Increased for Hold Piece and better layout
PANEL_PADDING = 10
...
class Tetris:
    def __init__(self):
        ...
        self.sfx_rotate = self.load_sound("rotate.wav")
        self.sfx_land = self.load_sound("land.wav")
        self.sfx_clear = self.load_sound("clear.wav")
        self.sfx_gameover = self.load_sound("gameover.wav")
        self.sfx_hard_drop = self.load_sound("hard_drop.wav")
        self.sfx_hold = self.load_sound("hold.wav")
    ...
    def new_piece(self):
        ...
        # Spawn pieces near the top, slightly above visible grid if piece shape allows
        # The y=0 for a piece refers to the top of its 5x5 matrix.
        # Some pieces might need y=-1 or y=-2 to appear correctly at the top.
        # For simplicity, let's spawn such that the first solid block aims for y=0.
        # The current PIECES definition has padding, so y=0 for the matrix is fine.
        return {
            'type': piece_type,
            'rotation': 0,
            'x': GRID_WIDTH // 2 - 2, # Center based on 5x5 matrix
            'y': 0, 
            ...
            for c, cell_char in enumerate(row_str):
                if cell_char == '#':
                    ...
                    if not (0 <= new_x < GRID_WIDTH and 0 <= new_y < GRID_HEIGHT):
                        # If part of piece is outside grid horizontally, or below grid
                        if new_x < 0 or new_x >= GRID_WIDTH or new_y >= GRID_HEIGHT:
                            return False
                        # If part of piece is above grid (new_y < 0), it's fine for now
                    elif new_y >= 0 and self.grid[new_y][new_x] != 0: # Collision with existing blocks
                        return False
        return True
            ...
            for c, cell_char in enumerate(row_str):
                if cell_char == '#':
                    ...
                    if 0 <= grid_y < GRID_HEIGHT and 0 <= grid_x < GRID_WIDTH:
                        self.grid[grid_y][grid_x] = self.current_piece['type'] + 1 # Store color index + 1
        self.sfx_land.play()
    ...
    def clear_lines(self):
        ...
        if full_lines:
            ...
            # Score and level up (do this when lines are identified, not after animation)
            num_cleared = len(full_lines)
            self.lines_cleared += num_cleared
            score_values = [0, 40, 100, 300, 1200] # 0, 1, 2, 3, 4 lines
            self.score += score_values[min(num_cleared, 4)] * self.level
            
            new_level = self.lines_cleared // 10 + 1
            if new_level > self.level:
                self.level = new_level
                self.fall_speed = max(100, 500 - (self.level - 1) * 40) # Faster speed reduction
    ...
    def move_piece(self, dx, dy):
        if self.is_valid_position(self.current_piece, dx, dy):
            ...
            if dy > 0 : # Moved down
                self.is_on_ground = False # No longer definitively on ground
                self.lock_timer = 0
            elif dx != 0 and self.is_on_ground: # Moved sideways while on ground
                 self.lock_timer = LOCK_DELAY_DURATION # Reset lock timer
            if dx != 0: self.sfx_move.play()
            return True
        return False
    ...
    def rotate_current_piece(self):
        ...
        # Wall kick attempts: 0 (no kick), +1, -1, +2, -2
        # For I-piece, SRS has different kicks, but this is a simpler approach
        ...
        for dx_kick in kick_offsets:
            if self.is_valid_position(piece, dx=dx_kick, dy=0, rotation=new_rotation):
                ...
                if self.is_on_ground: # Reset lock timer if rotated on ground
                    self.lock_timer = LOCK_DELAY_DURATION
                self.sfx_rotate.play()
                return True
        return False # Rotation failed

    def drop_piece(self): # Hard drop
        dropped_rows = 0
        while self.move_piece(0, 1):
            dropped_rows +=1
        self.score += dropped_rows * 1 # 1 point per row for hard drop
        self.place_piece() # Immediately place after hard drop
        self.process_after_lock()
        self.sfx_hard_drop.play()
    ...
    def hold_current_piece(self):
        ...
        # Reset new current_piece position and state
        self.current_piece['x'] = GRID_WIDTH // 2 - 2
        self.current_piece['y'] = 0
        self.current_piece['rotation'] = 0 # Or its default rotation if pieces had one
        self.current_piece['shape'] = self.get_piece_shape(self.current_piece)

        self.is_on_ground = False
        self.lock_timer = 0
        self.can_swap_hold = False # Prevent repeated swapping until next piece locks

        if not self.is_valid_position(self.current_piece):
            self.game_over = True # Game over if swapped piece can't fit

    def process_after_lock(self):
        self.clear_lines() # This will set up animation if lines are cleared
        if not self.is_clearing_lines_animation: # If no lines to clear, proceed
            self.current_piece = self.next_piece
            self.next_piece = self.new_piece()
            self.can_swap_hold = True # Allow hold swap again
            self.is_on_ground = False
            if not self.is_valid_position(self.current_piece):
                self.game_over = True
    ...
    def update(self, dt):
        ...
        if self.is_clearing_lines_animation:
            ...
            if self.line_clear_anim_timer <= 0:
                ...
                # After clearing lines, set up next piece
                self.current_piece = self.next_piece
                self.next_piece = self.new_piece()
                self.can_swap_hold = True
                self.is_on_ground = False
                if not self.is_valid_position(self.current_piece):
                    self.game_over = True
                    self.sfx_gameover.play()
            return # Game logic paused during line clear animation

        self.fall_time += dt
        
        # Check if piece is on ground (cannot move down further)
        if not self.is_valid_position(self.current_piece, 0, 1):
            ...
            if self.is_on_ground:
                ...
                if self.lock_timer <= 0:
                    self.place_piece()
                    self.process_after_lock()
                    # Fall time reset will occur when new piece active or by gravity logic
        else: # Piece can move down
            self.is_on_ground = False
            self.lock_timer = 0 # Not on ground, so no lock timer
            if self.fall_time >= self.fall_speed:
                self.move_piece(0, 1) # Gravity
                self.fall_time = 0
    
    def draw_styled_cell(self, surface, rect, cell_color, for_ghost=False):
        if for_ghost:
            s = pygame.Surface((rect.width, rect.height), pygame.SRCALPHA)
            s.fill((cell_color[0], cell_color[1], cell_color[2], GHOST_COLOR[3])) # Use GHOST_COLOR alpha
            surface.blit(s, rect.topleft)
            pygame.draw.rect(surface, (200,200,200,100), rect, 1) # Light border for ghost
            return
        ...
        # Top and Left highlight lines (inside)
        ...
        # Bottom and Right shadow lines (inside)
        ...
        pygame.draw.rect(surface, BLACK, rect, 1) # Outer border

    def draw_grid(self):
        # Draw grid background (slightly darker than main window)
    ...
    def draw_piece(self, piece_data, screen_x_offset, screen_y_offset, cell_size, is_ghost=False, is_preview=False):
        ...
        base_x = piece_data['x'] if not is_preview else 0 # Preview pieces drawn relative to their box
        base_y = piece_data['y'] if not is_preview else 0
        
        # For previews, find true top-left of the piece for centering
        if is_preview:
            min_r, min_c = 5, 5
            max_r, max_c = 0, 0
            has_blocks = False
            ...
            if has_blocks:
                ...
                # Center the piece in a 4x4 cell preview area (standard for Tetris previews)
                preview_area_width = 4 * cell_size 
                preview_area_height = 4 * cell_size
                
                # Adjust base_x, base_y to center the piece shape
                # These are offsets from the top-left of the preview drawing box
            ...
            else: # Empty shape (should not happen for valid pieces)
    ...
    def draw_panel(self, x, y, width, height, title):
        ...
        pygame.draw.rect(self.screen, LIGHT_GRAY, panel_rect, border_radius=5) # Panel background
        pygame.draw.rect(self.screen, DARK_GRAY, panel_rect, 2, border_radius=5) # Panel border
        ...
        return panel_rect # Return rect for content placement
    ...
    def draw_side_panel_content(self):
        ...
        # Hold Piece Panel
        hold_panel_height = 4 * PREVIEW_CELL_SIZE + 2 * PANEL_PADDING + 30 # 30 for title
        ...
        # Next Piece Panel
        next_panel_height = 4 * PREVIEW_CELL_SIZE + 2 * PANEL_PADDING + 30 # 30 for title
        ...
        # Info Panel
        info_panel_height = 150 # Adjust as needed
        ...
        controls_panel_height = len(controls_text_list) * 20 + 2 * PANEL_PADDING + 30 # 30 for title
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_r: # Restart works anytime
                        self.reset_game()
                        continue

                    if event.key == pygame.K_p: # Pause works anytime unless game over
                        if not self.game_over:
                           self.paused = not self.paused
                        continue
                    
                    if self.game_over or self.paused or self.is_clearing_lines_animation:
                        continue # No input processing if game over, paused, or animating line clear
                    ...
                    if event.key == pygame.K_LEFT:
                        ...
                        if self.move_piece(0, 1):
                            ...
                            self.fall_time = 0 # Reset fall timer for soft drop
