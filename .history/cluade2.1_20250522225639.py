...
# --- Configuration ---
APP_VERSION = "1.7.1" # Fixed PySim log color bug
...
COLOR_TEXT_SECONDARY = "#757575" # Used as string in HTML
COLOR_TEXT_ON_ACCENT = "#FFFFFF"
COLOR_ACCENT_PRIMARY = "#1976D2" # Used as string in HTML
COLOR_ACCENT_PRIMARY_LIGHT = "#BBDEFB"
COLOR_ACCENT_SECONDARY = "#FF8F00" # Used as string in HTML
...
COLOR_PY_SIM_STATE_ACTIVE = QColor("#4CAF50")  # Green for active state (as QColor)
...
class GraphicsTransitionItem(QGraphicsPathItem):
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        if self.isSelected():
            ...
            stroker.setWidth(current_pen.widthF() + 6) 
...
class GraphicsCommentItem(QGraphicsTextItem):
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.ItemPositionHasChanged and self.scene():
            self.scene().item_moved.emit(self)
        if change == QGraphicsItem.ItemUsersettedData and self.document(): 
             self.prepareGeometryChange()
        return super().itemChange(change, value)
...
class StatePropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_state=False):
        ...
        def add_field(lbl, te, btn): 
            ...
            h.addWidget(te,1) 
            v = QVBoxLayout(); v.addWidget(btn); v.addStretch() 
            h.addLayout(v)
            layout.addRow(lbl,h)
...
class MainWindow(QMainWindow):
    ...
    def on_about(self):
        QMessageBox.about(self, "About " + APP_NAME,
            ...
            f"<li>Persistent storage in JSON format ({FILE_EXTENSION}).</li>"
    ...
    def _append_to_py_simulation_log(self, log_entries: list[str]):
        for entry in log_entries:
            ...
            if "[Condition]" in entry or "[Eval Error]" in entry or "ERROR" in entry.upper():
                # CORRECTED: Use string constants directly for HTML colors
                ...
                # CORRECTED: Use string constants directly for HTML colors (was already correct)
