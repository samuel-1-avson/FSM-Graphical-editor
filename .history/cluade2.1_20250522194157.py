...
from PyQt5.QtWidgets import (
    ...
    QHeaderView, QTableWidget, QTableWidgetItem # <-- Added for Variables Table
)
...
class StatePropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_state=False):
        super().__init__(parent); self.setWindowTitle("State Properties"); self.setWindowIcon(get_standard_icon(QStyle.SP_DialogApplyButton, "Props")); self.setMinimumWidth(480)
        ...
        self.current_color = QColor(p.get('color', COLOR_ITEM_STATE_DEFAULT_BG)); self._update_color_button_style(); self.color_button.clicked.connect(self._choose_color)
        ...
        def add_field_with_button(label_text, text_edit, button): h_layout = QHBoxLayout(); h_layout.setSpacing(5); h_layout.addWidget(text_edit, 1); v_button_layout = QVBoxLayout(); v_button_layout.addWidget(button); v_button_layout.addStretch(); h_layout.addLayout(v_button_layout); layout.addRow(label_text, h_layout)
        add_field_with_button("Entry Action:", self.entry_action_edit, entry_action_btn); add_field_with_button("During Action:", self.during_action_edit, during_action_btn); add_field_with_button("Exit Action:", self.exit_action_edit, exit_action_btn)
    ...
    def _create_insert_snippet_button(self, target_text_edit: QTextEdit, snippets_dict: dict, button_text="Insert..."):
        button = QPushButton(button_text); button.setObjectName("SnippetButton"); button.setToolTip(f"Insert common snippets into the text field."); button.setIcon(get_standard_icon(QStyle.SP_FileDialogContentsView,"Ins")); button.setIconSize(QSize(14,14))
        menu = QMenu(self)
        for name, snippet in snippets_dict.items(): action = QAction(name, self); action.triggered.connect(lambda checked=False, text_edit=target_text_edit, s=snippet: text_edit.insertPlainText(s + "\n")); menu.addAction(action)
    ...
    def _update_color_button_style(self): l = self.current_color.lightnessF(); text_color = COLOR_TEXT_PRIMARY if l > 0.5 else COLOR_TEXT_ON_ACCENT; self.color_button.setStyleSheet(f"background-color: {self.current_color.name()}; color: {text_color};")
...
class TransitionPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_transition=False):
        ...
        self.offset_perp_spin = QSpinBox(); self.offset_perp_spin.setRange(-1000, 1000); self.offset_perp_spin.setSingleStep(10); self.offset_perp_spin.setValue(int(p.get('control_offset_x', 0))); self.offset_perp_spin.setToolTip("Perpendicular bend of curve (0 for straight).")
        self.offset_tang_spin = QSpinBox(); self.offset_tang_spin.setRange(-1000, 1000); self.offset_tang_spin.setSingleStep(10); self.offset_tang_spin.setValue(int(p.get('control_offset_y', 0))); self.offset_tang_spin.setToolTip("Tangential shift of curve midpoint.")
        self.description_edit = QTextEdit(p.get('description', "")); self.description_edit.setFixedHeight(75); self.description_edit.setPlaceholderText("Optional notes for this transition")
        def add_field_with_button(label_text, edit_widget, button): h_layout = QHBoxLayout(); h_layout.setSpacing(5); h_layout.addWidget(edit_widget, 1); v_button_layout = QVBoxLayout(); v_button_layout.addWidget(button); v_button_layout.addStretch(); h_layout.addLayout(v_button_layout); layout.addRow(label_text, h_layout)
        add_field_with_button("Event Trigger:", self.event_edit, event_btn); add_field_with_button("Condition (Guard):", self.condition_edit, condition_btn); add_field_with_button("Transition Action:", self.action_edit, action_btn)
        layout.addRow("Color:", self.color_button); curve_layout = QHBoxLayout(); curve_layout.addWidget(QLabel("Bend (Perp):")); curve_layout.addWidget(self.offset_perp_spin); curve_layout.addSpacing(10); curve_layout.addWidget(QLabel("Mid Shift (Tang):")); curve_layout.addWidget(self.offset_tang_spin); curve_layout.addStretch(); layout.addRow("Curve Shape:", curve_layout); layout.addRow("Description:", self.description_edit)
        buttons = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel); buttons.accepted.connect(self.accept); buttons.rejected.connect(self.reject); layout.addRow(buttons)
    ...
    def _create_insert_snippet_button_lineedit(self, target_line_edit: QLineEdit, snippets_dict: dict, button_text="Insert..."):
        button = QPushButton(button_text); button.setObjectName("SnippetButton"); button.setIcon(get_standard_icon(QStyle.SP_FileDialogContentsView,"Ins")); button.setIconSize(QSize(14,14)); button.setToolTip("Insert common snippets.")
        menu = QMenu(self)
