...
class FSMSimulator:
    ...
    def _evaluate_expression(self, expr_str: str, event_payload: dict | None = None):
        original_expr_for_log = expr_str # Keep original for logging
        expr_str = expr_str.split('%', 1)[0].strip()
        if not expr_str:
            return True

        # Convert common non-Python syntax to Python syntax
        # Boolean literals
        ...
        # Important: Use word boundaries (\b) for 'or' to avoid accidental replacement in words like 'error'.
        # Handle multiple spaces around operators as well.
        py_expr_str = re.sub(r'\s*&&\s*', ' and ', py_expr_str)
        py_expr_str = re.sub(r'\s*\|\|\s*', r' or ', py_expr_str) # Using r' or ' for word boundary with 'or'
                                                               # More robust: \b(or)\b ->  or (with careful handling)

        # For 'or', specifically handle potential word boundary issues. Simpler is direct replace then space check
        # Simpler replacement first for ||
        py_expr_str = py_expr_str.replace("||", " or ") # Then ensure spacing if it became "foo orbar"
        
        # Handle logical NOT: !var or !(expr) -> not var or not (expr)
        # Be careful not to replace '!=' (not equals)
        # Regex for '!' not immediately followed by '='
        py_expr_str = re.sub(r'!(?!=)', ' not ', py_expr_str)


        # Normalize spacing around operators after replacements (optional but good for consistency)
        py_expr_str = re.sub(r'\s*([=<>&|!+*/%-])\s*', r' \1 ', py_expr_str).strip()
        py_expr_str = py_expr_str.replace('  ', ' ') # Reduce double spaces
        ...
        try:
            ...
            err_msg = f"NameError evaluating '{original_expr_for_log}' (became '{py_expr_str}'): {ne}. Variable not defined in FSM state, event payload, or placeholders."
            self._action_log.append(f"[Eval Error] {err_msg}")
            raise FSMError(err_msg) from ne
        except SyntaxError as se:
            # Provide more context for syntax errors.
            err_msg = f"SyntaxError evaluating '{original_expr_for_log}' (became '{py_expr_str}'): {se}. Ensure Python-compatible syntax (e.g., 'and' instead of '&&')."
            self._action_log.append(f"[Eval Error] {err_msg}")
            raise FSMError(err_msg) from se
        except Exception as e:
            err_msg = f"Error evaluating expression '{original_expr_for_log}' (became '{py_expr_str}'): {type(e).__name__}: {e}"
            self._action_log.append(f"[Eval Error] {err_msg}")
