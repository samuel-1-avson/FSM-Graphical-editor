...
# --- Configuration ---
APP_VERSION = "1.4.0" # Incremented for new features
...
class DiagramScene(QGraphicsScene):
    ...
    def set_mode(self, mode: str):
        ...
        if self.parent_window:
            ...
            # Add for comment if such a mode button exists later
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        ...
        if item_type == "State" or item_type == "Initial State" or item_type == "Final State":
            ...
            state_name_from_input = default_name # Placeholder for now
            if not initial_data: # If not from drag-drop prefill, ask user
                ...
                if not (ok and temp_name and temp_name.strip()):
                    self.log_function(f"State addition cancelled by user or empty name.")
                    if self.current_mode == "state": self.set_mode("select")
                    return
                state_name_from_input = temp_name.strip()

            if self.get_state_by_name(state_name_from_input) and (not initial_data or state_name_from_input != default_name): # Check for name collision
                QMessageBox.warning(self.parent_window, "Duplicate Name", f"A state named '{state_name_from_input}' already exists.")
                if self.current_mode == "state": self.set_mode("select")
                return

            # Initial properties dialog to confirm/set initial, final, color, actions etc.
            ...
            if props_dialog.exec_() == QDialog.Accepted:
                ...
                if final_props['name'] != state_name_from_input and self.get_state_by_name(final_props['name']): # Re-check after dialog edit
            ...
            initial_text = initial_data.get('text', "Comment") if initial_data else "Comment"
        ...
        # Revert to select mode after adding an item usually.
        if self.current_mode == "state" or self.current_mode == "comment": 
    ...
    def dropEvent(self, event: QGraphicsSceneDragDropEvent): 
        pos = event.scenePos()
        if event.mimeData().hasFormat("application/x-bsm-tool"):
            item_type_data = event.mimeData().text() # e.g., "State", "Initial State", "Comment"
            
            # Snap drop position to grid
            grid_x = round(pos.x() / self.grid_size) * self.grid_size
            grid_y = round(pos.y() / self.grid_size) * self.grid_size
            # Adjust for typical item center if it's a state
            if "State" in item_type_data: 
    ...
    def drawBackground(self, painter: QPainter, rect: QRectF):
        ...
        # Draw minor grid points (optional, can be lines too)
        painter.setPen(self.grid_pen_light)
        for x in range(first_left, right, self.grid_size):
            # Only draw points if not on a major grid line intersection
...
class TransitionPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_transition=False):
        ...
        self.event_edit.setPlaceholderText("e.g., mouseClick")
        self.condition_edit = QLineEdit(p.get('condition', ""))
        self.condition_edit.setPlaceholderText("e.g., data > 10")
        self.action_edit = QTextEdit(p.get('action', ""))
        self.action_edit.setPlaceholderText("e.g., counter++; flag = true;")
        ...
        self.offset_perp_spin.setToolTip("Perpendicular curve bend.")
        ...
        self.offset_tang_spin.setToolTip("Tangential curve midpoint shift.")
...
class CommentPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None):
        ...
        # Optional: Width control for comments. Usually managed by resizing in scene.
    ...
    def get_properties(self):
        return {
            ...
            # If width control is added: 'width': self.width_spin.value()
        }
...
class MainWindow(QMainWindow):
    ...
    def _update_properties_dock(self):
        ...
        if len(selected_items) == 1:
            ...
            props = item.get_data()
            item_info = f"<b>Type:</b> {type(item).__name__}<br><hr>"
            
            if isinstance(item, GraphicsStateItem):
                item_info += f"<b>Name:</b> {props['name']}<br>"
                item_info += f"<b>Initial:</b> {'Yes' if props['is_initial'] else 'No'}<br>"
                item_info += f"<b>Final:</b> {'Yes' if props['is_final'] else 'No'}<br>"
                item_info += f"<b>Color:</b> <span style='background-color:{props.get('color','#FFFFFF')};'>&nbsp;&nbsp;&nbsp;</span> {props.get('color','N/A')}<br>"
                if props.get('entry_action'): item_info += f"<b>Entry:</b> {props['entry_action'][:30]}...<br>"
                if props.get('during_action'): item_info += f"<b>During:</b> {props['during_action'][:30]}...<br>"
                if props.get('exit_action'): item_info += f"<b>Exit:</b> {props['exit_action'][:30]}...<br>"
                if props.get('description'): item_info += f"<b>Desc:</b> {props['description'][:40]}...<br>"

            elif isinstance(item, GraphicsTransitionItem):
                label_parts = []
                if props.get('event'): label_parts.append(props['event'])
                if props.get('condition'): label_parts.append(f"[{props['condition']}]")
                if props.get('action'): label_parts.append(f"/{{{props['action']}}}")
                full_label = " ".join(label_parts) if label_parts else "<i>(No Label)</i>"
                item_info += f"<b>Label:</b> {full_label}<br>"
                item_info += f"<b>From:</b> {props['source']}<br>"
                item_info += f"<b>To:</b> {props['target']}<br>"
                item_info += f"<b>Color:</b> <span style='background-color:{props.get('color','#FFFFFF')};'>&nbsp;&nbsp;&nbsp;</span> {props.get('color','N/A')}<br>"
                if props.get('description'): item_info += f"<b>Desc:</b> {props['description'][:40]}...<br>"
                item_info += f"<b>Curve:</b> Bend={props['control_offset_x']:.0f}, Shift={props['control_offset_y']:.0f}"
            
            elif isinstance(item, GraphicsCommentItem):
                item_info += f"<b>Text:</b> {props['text'][:60]}{'...' if len(props['text']) > 60 else ''}<br>"
    ...
    def _update_save_actions_enable_state(self):
        ...
        self.save_as_action.setEnabled(True) # Save As is always enabled if there's content (even if not dirty)
