        ...
        if event.button() == Qt.LeftButton:
            if self.current_mode == "state":
                ...
                if isinstance(top_item_at_pos, GraphicsStateItem):
                    ...
                    if self.transition_start_item: # Only log/reset if a transition was in progress
                ...
                # The line below was unconditional, moved cleanup of _temp_transition_line into the else
                # if self._temp_transition_line: self.removeItem(self._temp_transition_line); self._temp_transition_line = None
                # self.log_function("Transition drawing cancelled (clicked empty space/non-state).")
            else: # This 'else' implies self.current_mode == "select" (or other unexpected modes)
                self._mouse_press_items_positions.clear()
                
                # Initialize selected_movable to ensure it's defined
                selected_movable = []  # Ensure it's at least an empty list
                try:
                    # Attempt to populate selected_movable
                    selected_movable = [item for item in self.selectedItems() if item.flags() & QGraphicsItem.ItemIsMovable]
                except Exception as e:
                    # Log any error during the list comprehension and ensure selected_movable is empty
                    if self.log_function: # Check if log_function is set
                        self.log_function(f"ERROR during selected_movable computation: {type(e).__name__}: {e}")
                    # selected_movable remains an empty list in case of error
                
                # This loop should now be safe as selected_movable is guaranteed to be a list (possibly empty)
            ...
            super().mousePressEvent(event) # Handle other mouse buttons if necessary
