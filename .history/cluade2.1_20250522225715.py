...
class DraggableToolButton(QPushButton):
    def __init__(self, text, mime_type, item_type_data, parent=None):
        ...
        self.setText(text) # Already set by QPushButton constructor, but explicit for clarity
        self.setMinimumHeight(40)
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed) # Takes available width
        self.drag_start_position = QPoint() # Initialize

    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.LeftButton:
            self.drag_start_position = event.pos()
        super().mousePressEvent(event) # Important for button's own behavior

    def mouseMoveEvent(self, event: QMouseEvent):
        if not (event.buttons() & Qt.LeftButton): # Only if left button is pressed
            return
        if (event.pos() - self.drag_start_position).manhattanLength() < QApplication.startDragDistance():
            # Don't start drag if mouse hasn't moved enough
            return

        drag = QDrag(self)
        mime_data = QMimeData()
        mime_data.setText(self.item_type_data) # General text data
        mime_data.setData(self.mime_type, self.item_type_data.encode()) # Specific MIME type
        drag.setMimeData(mime_data)

        # Create a more visually appealing pixmap for the drag
        pixmap_size = QSize(max(150, self.width()), max(40, self.height())) # Ensure minimum width
        ...
        # Use theme colors if available, fallback
        bg_color = QColor(self.palette().color(self.backgroundRole())).lighter(110)
        if not bg_color.isValid() or bg_color.alpha() == 0 : # If transparent or invalid
            bg_color = QColor(COLOR_ACCENT_PRIMARY_LIGHT) # Fallback background
        ...
        # Draw icon and text
        icon_pixmap = self.icon().pixmap(QSize(20, 20), QIcon.Normal, QIcon.On) # Standard size
        text_x_offset = 10 # Left padding for text/icon
        ...
        painter.setFont(self.font()) # Use button's font
        ...
        drag.setHotSpot(QPoint(pixmap.width() // 4, pixmap.height() // 2)) # Offset hotspot a bit

        # Execute drag operation (Copy or Move are fine, scene handles the type)
...
class GraphicsCommentItem(QGraphicsTextItem):
    ...
    def paint(self, painter, option, widget):
        ...
        rect = self.boundingRect() # The rect of the text item itself
        painter.drawRoundedRect(rect.adjusted(0.5,0.5,-0.5,-0.5), 4, 4) # Slight adjust for pen

        # Let base class paint the text content
        self.setDefaultTextColor(QColor(COLOR_TEXT_PRIMARY)) # Ensure text color is set before super paint
        super().paint(painter, option, widget) # This draws the text

        if self.isSelected():
            selection_pen = QPen(QColor(COLOR_ACCENT_PRIMARY), 1.5, Qt.DashLine)
            painter.setPen(selection_pen)
            painter.setBrush(Qt.NoBrush) # Selection is just an outline
            painter.drawRect(self.boundingRect())

    def get_data(self):
        # textWidth() is -1 if no fixed width (auto-sizes), document().idealWidth() preferred then
    ...
    def set_properties(self, text, width=None):
        self.setPlainText(text)
        if width and width > 0 : # Apply a specific width if provided
            self.setTextWidth(width)
        else: # Revert to default if no width provided or invalid width
    ...
    def itemChange(self, change, value):
        ...
        # When text content changes, bounding rect might change
        if change == QGraphicsItem.ItemUsersettedData and self.document(): # Or track document contentsChanged
             self.prepareGeometryChange()
        return super().itemChange(change, value)
...
class DiagramScene(QGraphicsScene):
    item_moved = pyqtSignal(QGraphicsItem) # Signal emitted when an item has been moved
    modifiedStatusChanged = pyqtSignal(bool) # Signal for dirtiness state

    def __init__(self, undo_stack, parent_window=None):
        super().__init__(parent_window)
        self.parent_window = parent_window # Reference to MainWindow
        self.setSceneRect(0, 0, 6000, 4500) # Large canvas
        self.current_mode = "select" # Default interaction mode
        self.transition_start_item = None # For drawing transitions
        self.log_function = print # Placeholder, overridden by MainWindow
        self.undo_stack: QUndoStack = undo_stack
        self._dirty = False # Tracks if the diagram has unsaved changes
        self._mouse_press_items_positions = {} # Stores item positions at mouse press for move command
        self._temp_transition_line = None # Visual feedback for transition drawing
        ...
        self.setBackgroundBrush(QColor(COLOR_BACKGROUND_LIGHT)) # Light grey background
        self.snap_to_grid_enabled = True


    def _update_connected_transitions(self, state_item: GraphicsStateItem):
        """Updates paths of transitions connected to the given state_item."""
    ...
    def _update_transitions_for_renamed_state(self, old_name:str, new_name:str):
        """
        Called when a state is renamed.
        Transitions store source/target names in their get_data().
        If a user manually inspects the JSON or if other logic relies on these names,
        this notification can be useful. Actual transition graphics link to item instances directly.
        """
        self.log_function(f"State '{old_name}' renamed to '{new_name}'. Dependent transitions may need data update if name was key in external system.")

    def get_state_by_name(self, name: str) -> GraphicsStateItem | None:
        """Finds a state item by its text_label (name)."""
        for item in self.items():
    ...
    def set_dirty(self, dirty=True):
        if self._dirty != dirty:
            ...
            # MainWindow will connect to modifiedStatusChanged to update window title and save actions
        if self.parent_window: # Main window can update save action states
    ...
    def set_mode(self, mode: str):
        ...
        if self.parent_window and self.parent_window.view: # view is ZoomableView
            self.parent_window.view._restore_cursor_to_scene_mode()

        # Enable/disable item movability based on mode
        for item in self.items():
            movable_flag = mode == "select" # Only movable in select mode
            if isinstance(item, (GraphicsStateItem, GraphicsCommentItem)): # Transitions aren't directly moved
        ...
        if self.parent_window:
            ...
            for m, action in actions_map.items():
                if m == mode and action and not action.isChecked():
                    action.setChecked(True)
                    # No break here; let it cycle through in case actions aren't exclusive (though they are in an QActionGroup)
    ...
    def _handle_item_moved(self, moved_item):
        """Called when an item signals it has moved."""
        if isinstance(moved_item, GraphicsStateItem):
            self._update_connected_transitions(moved_item)
            # Move command would be pushed by mouseReleaseEvent after assessing if move happened.
            # This is more of a real-time update hook.

    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        pos = event.scenePos()
        items_at_pos = self.items(pos) # Get all items at this position

        # Try to find a "primary" item at this position (State > Comment/Transition > any other)
        ...
        if not top_item_at_pos:
            ...
            if not top_item_at_pos and items_at_pos: # If still nothing, but there ARE items...
                top_item_at_pos = items_at_pos[0] # pick the topmost one.

        if event.button() == Qt.LeftButton:
            if self.current_mode == "state":
                # Snap new state's top-left to grid (item size is 120x60)
                grid_x = round(pos.x() / self.grid_size) * self.grid_size - 60 # Adjust for item width center
                grid_y = round(pos.y() / self.grid_size) * self.grid_size - 30 # Adjust for item height center
                ...
                else: # Clicked on non-state or empty space during transition draw
                    if self.transition_start_item: # If a transition was being drawn
                        self.log_function("Transition drawing cancelled (clicked non-state/empty space).")
                        self.transition_start_item = None
                    if self._temp_transition_line:
                        self.removeItem(self._temp_transition_line)
                        self._temp_transition_line = None
            else: # self.current_mode == "select" (or any other unhandled mode)
                self._mouse_press_items_positions.clear()
                selected_items_list = self.selectedItems() # This returns a QList<QGraphicsItem*>

                # Filter for movable items among the selected ones
                movable_items_cache = []
                if selected_items_list is not None: # Should always be a list, even if empty
                    movable_items_cache = [item for item in selected_items_list if item.flags() & QGraphicsItem.ItemIsMovable]

                # Store initial positions of items that will be moved
                for item_to_process in movable_items_cache:
                     self._mouse_press_items_positions[item_to_process] = item_to_process.pos()

                super().mousePressEvent(event) # Let base class handle selection/movement start
            ...
            else: # Right-click on empty space clears selection
                self.clearSelection()
        else:
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent):
        if self.current_mode == "transition" and self.transition_start_item and self._temp_transition_line:
            # Update temporary line endpoint to follow mouse
    ...
    def mouseReleaseEvent(self, event: QGraphicsSceneMouseEvent):
        if event.button() == Qt.LeftButton and self.current_mode == "select":
            if self._mouse_press_items_positions:
                moved_items_data = [] # List of (item, new_pos)
                for item, old_pos in self._mouse_press_items_positions.items():
                    new_pos = item.pos()
                    if self.snap_to_grid_enabled: # Apply snapping if enabled
                        ...
                        if new_pos.x() != snapped_x or new_pos.y() != snapped_y:
                            item.setPos(snapped_x, snapped_y) # Update item's visual position
                            new_pos = QPointF(snapped_x, snapped_y) # Use snapped pos for command

                    if (new_pos - old_pos).manhattanLength() > 0.1: # Check if item actually moved
                        moved_items_data.append((item, new_pos))
                
                if moved_items_data: # If any items were actually moved
                    cmd = MoveItemsCommand(moved_items_data)
                    self.undo_stack.push(cmd)
                self._mouse_press_items_positions.clear() # Clear for next move operation
    ...
    def _show_context_menu(self, item, global_pos):
        ...
        if action == edit_action:
            ...
            if not item.isSelected(): # Ensure item is selected if deleting from context menu on unselected
                self.clearSelection()
                item.setSelected(True)
            self.delete_selected_items()

    def edit_item_properties(self, item):
        old_props = item.get_data() # Get current properties for dialog and undo command
        ...
        if isinstance(item, GraphicsStateItem):
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                # Check for duplicate name (unless it's the same item keeping its name)
                if new_props_from_dialog['name'] != old_props['name'] and self.get_state_by_name(new_props_from_dialog['name']):
                    ...
                    return # Prevent property change if name collides
        ...
        if dialog_executed_and_accepted and new_props_from_dialog is not None:
            final_new_props = old_props.copy() # Start with old properties
            final_new_props.update(new_props_from_dialog) # Update with new ones from dialog
        ...
        self.update() # Redraw scene potentially


    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        """Handles adding items, including showing property dialogs and pushing to undo stack."""
        current_item = None
        ...
        if item_type == "State":
            ...
            initial_dialog_props = { # Pre-fill dialog
                'name': default_name,
            ...
            if props_dialog.exec_() == QDialog.Accepted:
                ...
                if self.get_state_by_name(final_props['name']) and final_props['name'] != default_name :
                    ...
                    # Don't add item if name conflicts (and it wasn't the default name unchanged)
                else:
            ...
            if self.current_mode == "state": # If added by clicking "Add State" tool button
                 self.set_mode("select") # Revert to select mode after adding
            if not current_item: return # Dialog cancelled or name conflict

        elif item_type == "Comment":
            initial_text = (initial_data.get('text', "Comment") if initial_data
                             else (name_prefix if name_prefix != "Item" else "Comment")) # Handle dragged name
            # Using CommentPropertiesDialog for consistency, even if it's just text for now
            ...
            if comment_props_dialog.exec_() == QDialog.Accepted:
                ...
                if final_comment_props['text']: # Only add if text is not empty
        ...
        # If current_mode implies tool button use, it's reset inside specific type logic.
        # If item was drag-dropped, current_mode is likely "select", which is fine.
    ...
    def _handle_transition_click(self, clicked_state_item: GraphicsStateItem, click_pos: QPointF):
        if not self.transition_start_item: # Starting a new transition
            ...
            if not self._temp_transition_line:
                self._temp_transition_line = QGraphicsLineItem() # Simple line for feedback
            ...
            # Start drawing temporary line from center of start item to current mouse pos
            center_start = self.transition_start_item.sceneBoundingRect().center()
        ...
        else: # Finishing a transition (second click)
            if self._temp_transition_line: # Remove temporary line
                self.removeItem(self._temp_transition_line)
                self._temp_transition_line = None

            # Show transition properties dialog
            initial_props = {'event': "", 'condition': "", 'action': "",
                             'color': COLOR_ITEM_TRANSITION_DEFAULT, 'description':"",
                             'control_offset_x':0, 'control_offset_y':0} # Default control offset is 0,0 (straight)
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                new_transition = GraphicsTransitionItem(self.transition_start_item, clicked_state_item,
                                                        ...
                                                        action_str=props['action'], color=props.get('color'),
                                                        description=props.get('description', ""))
                # Set curve offset from dialog
            ...
            self.set_mode("select") # Go back to select mode


    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_Delete or (event.key() == Qt.Key_Backspace and sys.platform != 'darwin'): # Backspace for delete on Win/Linux
            ...
            if self.current_mode == "transition" and self.transition_start_item:
                ...
                self.set_mode("select") # Revert to select mode
                 ...
                 self.set_mode("select") # Revert other tool modes to select mode
    ...
    def delete_selected_items(self):
        ...
        items_to_delete_with_related = set() # Use a set to collect all items for deletion
        for item in selected:
            items_to_delete_with_related.add(item)
            # If a state is selected for deletion, also mark its connected transitions
            if isinstance(item, GraphicsStateItem):
                for scene_item in self.items(): # Iterate all scene items to find connected transitions
        ...
        if items_to_delete_with_related:
            ...
            # Selection should clear after items are removed via the command's redo.
            # Explicitly clear selection here for immediate visual feedback.
            self.clearSelection()


    def dragEnterEvent(self, event: QGraphicsSceneDragDropEvent):
        if event.mimeData().hasFormat("application/x-bsm-tool"):
            event.setAccepted(True) # We understand this MIME type
    ...
    def dropEvent(self, event: QGraphicsSceneDragDropEvent):
        pos = event.scenePos()
        if event.mimeData().hasFormat("application/x-bsm-tool"):
            item_type_data_str = event.mimeData().text() # text() used in DraggableButton

            # Snap drop position to grid
            grid_x = round(pos.x() / self.grid_size) * self.grid_size
            grid_y = round(pos.y() / self.grid_size) * self.grid_size
            
            if "State" in item_type_data_str: # Adjust drop point for state to be centered
                grid_x -= 60 # Approx half-width of default state item
                grid_y -= 30 # Approx half-height

            initial_props_for_add = {}
            actual_item_type_to_add = "Item" # Fallback, should be overridden
            name_prefix_for_add = "Item" # Fallback for _add_item_interactive default naming

            # Determine item type and initial properties from dragged data
            if item_type_data_str == "State":
                actual_item_type_to_add = "State"; name_prefix_for_add = "State"
            elif item_type_data_str == "Initial State":
                actual_item_type_to_add = "State"; name_prefix_for_add = "InitialState"
                initial_props_for_add['is_initial'] = True
            elif item_type_data_str == "Final State":
                actual_item_type_to_add = "State"; name_prefix_for_add = "FinalState"
    ...
    def get_diagram_data(self):
        ...
        for item in self.items():
            if isinstance(item, GraphicsStateItem):
                ...
                if item.start_item and item.end_item: # Only save validly connected transitions
    ...
    def load_diagram_data(self, data):
        self.clear() # Clear existing items before loading
        self.set_dirty(False) # Loading a file clears dirty status until modified
        state_items_map = {} # To link transitions by name

        for state_data in data.get('states', []):
            state_item = GraphicsStateItem(state_data['x'], state_data['y'],
                                           state_data.get('width', 120), state_data.get('height', 60), # Defaults for older files
        ...
        for trans_data in data.get('transitions', []):
            ...
            if src_item and tgt_item:
                ...
                # Log if a transition can't be linked
        ...
        for comment_data in data.get('comments', []):
            ...
            comment_item.setTextWidth(comment_data.get('width', 150)) # Apply stored width
            self.addItem(comment_item)
            
        self.set_dirty(False) # Explicitly ensure not dirty after load
        self.undo_stack.clear() # Reset undo stack for the newly loaded file

    def drawBackground(self, painter: QPainter, rect: QRectF): # rect is exposed area
        super().drawBackground(painter, rect) # Draw the basic background (e.g., brush color)

        # Determine the visible portion of the scene for optimized grid drawing
        # Fallback to `rect` if no views are available (headless, etc.)
        if self.views():
             view_rect = self.views()[0].viewport().rect()
             visible_scene_rect = self.views()[0].mapToScene(view_rect).boundingRect()
        else:
             visible_scene_rect = rect # Fallback to entire exposed area

        # Get integer boundaries for loop ranges, adjusted by grid size
        ...
        # Draw minor grid lines (lighter color, dotted)
        painter.setPen(self.grid_pen_light)
        for x in range(left, right, self.grid_size):
            if x % (self.grid_size * 5) != 0: # Don't draw over major lines
                 painter.drawLine(x, top, x, bottom)
        for y in range(top, bottom, self.grid_size):
            if y % (self.grid_size * 5) != 0: # Don't draw over major lines
                 painter.drawLine(left, y, right, y)

        # Draw major grid lines (darker/solid color)
...
class ZoomableView(QGraphicsView):
    def __init__(self, scene, parent=None):
        ...
        self.setViewportUpdateMode(QGraphicsView.BoundingRectViewportUpdate) # Optimize redraws
        self.zoom_level = 0 # Tracks current zoom factor for limits
        self.setTransformationAnchor(QGraphicsView.AnchorUnderMouse) # Zoom towards mouse cursor
        self.setResizeAnchor(QGraphicsView.AnchorViewCenter) # How view behaves on resize

        self._is_panning_with_space = False # True if spacebar is held for panning
        self._is_panning_with_mouse_button = False # True if middle mouse or space+left is active for pan
        self._last_pan_point = QPoint() # Last mouse position during pan

    def wheelEvent(self, event: QWheelEvent):
        if event.modifiers() & Qt.ControlModifier: # Zoom with Ctrl + Mouse Wheel
            delta = event.angleDelta().y()
            factor = 1.12 if delta > 0 else 1 / 1.12 # Zoom factor
            new_zoom_level = self.zoom_level + (1 if delta > 0 else -1)

            # Limit zoom levels
            if -15 <= new_zoom_level <= 25: # Min/Max zoom
                self.scale(factor, factor)
                self.zoom_level = new_zoom_level
            event.accept() # Event handled
        else:
            super().wheelEvent(event) # Default wheel behavior (e.g., scrolling)

    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_Space and not self._is_panning_with_space and not event.isAutoRepeat():
            self._is_panning_with_space = True
            self._last_pan_point = self.mapFromGlobal(QCursor.pos()) # Start pan from current cursor
            self.setCursor(Qt.OpenHandCursor)
            event.accept()
        elif event.key() == Qt.Key_Plus or event.key() == Qt.Key_Equal: # Zoom in with +/=
            self.scale(1.12, 1.12); self.zoom_level +=1
        elif event.key() == Qt.Key_Minus: # Zoom out with -
            self.scale(1/1.12, 1/1.12); self.zoom_level -=1
        elif event.key() == Qt.Key_0 or event.key() == Qt.Key_Asterisk: # Reset zoom and view
             ...
             if self.scene():
                 ...
                 else: self.centerOn(self.scene().sceneRect().center()) # Center on scene if empty
        else:
            super().keyPressEvent(event) # Pass to scene or QGraphicsView base
    ...
    def keyReleaseEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_Space and self._is_panning_with_space and not event.isAutoRepeat():
            ...
            if not self._is_panning_with_mouse_button: # If not also middle-mouse panning
    ...
    def mousePressEvent(self, event: QMouseEvent):
        # Start panning with Middle Mouse or Space + Left Mouse
        if event.button() == Qt.MiddleButton or \
           (self._is_panning_with_space and event.button() == Qt.LeftButton):
            self._last_pan_point = event.pos() # Viewport coordinates
            self.setCursor(Qt.ClosedHandCursor)
            self._is_panning_with_mouse_button = True # Indicates active mouse panning
            event.accept() # We handle panning, don't pass to scene for item selection
        else:
            self._is_panning_with_mouse_button = False
            # This is crucial: for selection and item interaction, event must go to superclass/scene
            super().mousePressEvent(event)


    def mouseMoveEvent(self, event: QMouseEvent):
        if self._is_panning_with_mouse_button: # If panning mode is active
            ...
            super().mouseMoveEvent(event) # For rubber band selection etc.
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        ...
        if self._is_panning_with_mouse_button and \
           (event.button() == Qt.MiddleButton or (self._is_panning_with_space and event.button() == Qt.LeftButton)):
            self._is_panning_with_mouse_button = False
            if self._is_panning_with_space: # If space is still held, revert to open hand
                self.setCursor(Qt.OpenHandCursor)
            else: # Otherwise, restore cursor based on scene mode
                self._restore_cursor_to_scene_mode()
            event.accept()
        else:
            super().mouseReleaseEvent(event) # Pass to scene for selection release etc.
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.last_generated_model_path = None # For remembering last SLX for sim/codegen
        self.matlab_connection = MatlabConnection()
        self.undo_stack = QUndoStack(self)

        self.scene = DiagramScene(self.undo_stack, self) # Pass undo_stack and self as parent
        self.scene.set_log_function(self.log_message)
        self.scene.modifiedStatusChanged.connect(self.setWindowModified) # Qt's way to track modified
        self.scene.modifiedStatusChanged.connect(self._update_window_title) # For [*] in title

        # --- NEW PYTHON SIMULATION ---
        self.py_fsm_engine: FSMSimulator | None = None
        self.py_sim_active = False # Is Python simulation currently running?
        self._py_sim_currently_highlighted_item: GraphicsStateItem | None = None

        self.init_ui() # Calls _create_... methods

        # Make status labels findable by QSS if needed, or for direct reference
        ...
        self.on_new_file(silent=True) # Start with a clean, new diagram without prompts

        # Connect scene selection change to update properties dock
        self.scene.selectionChanged.connect(self._update_properties_dock)
        self._update_properties_dock() # Initial call for empty selection
        self._update_py_simulation_actions_enabled_state()

    def init_ui(self):
        self.setGeometry(50, 50, 1650, 1050) # Adjust default size as needed
        self.setWindowIcon(get_standard_icon(QStyle.SP_DesktopIcon, "BSM")) # Generic app icon
        
        self._create_central_widget() # Main diagram view
    ...
    def _create_actions(self):
        # Helper for robustly getting QStyle enums
        def _safe_get_style_enum(attr_name, fallback_attr_name=None):
            try: return getattr(QStyle, attr_name)
                ...
                return QStyle.SP_CustomBase # A generic fallback if needed
        ...
        # Mode actions (mutually exclusive)
    ...
    def _create_menus(self):
        ...
        file_menu.addAction(self.export_simulink_action) # Export directly here for convenience
        ...
        py_sim_menu = sim_menu.addMenu(get_standard_icon(QStyle.SP_MediaPlay, "PyS"), "Python Simulation (Internal)") # Distinguish icon
        ...
        self.view_menu = menu_bar.addMenu("&View") # Docks will add actions here
        help_menu = menu_bar.addMenu("&Help"); help_menu.addAction(self.about_action)

    def _create_toolbars(self):
        icon_size = QSize(22,22) # Standard toolbar icon size
        tb_style = Qt.ToolButtonTextBesideIcon

        file_toolbar = self.addToolBar("File"); file_toolbar.setObjectName("FileToolBar")
        file_toolbar.setIconSize(icon_size); file_toolbar.setToolButtonStyle(tb_style)
        ...
        edit_toolbar.setIconSize(icon_size); edit_toolbar.setToolButtonStyle(tb_style)
        edit_toolbar.addAction(self.undo_action); edit_toolbar.addAction(self.redo_action); edit_toolbar.addSeparator(); edit_toolbar.addAction(self.delete_action)

        tools_tb = self.addToolBar("Interaction Tools"); tools_tb.setObjectName("ToolsToolBar")
        tools_tb.setIconSize(icon_size); tools_tb.setToolButtonStyle(tb_style)
        ...
        sim_toolbar.setIconSize(icon_size); sim_toolbar.setToolButtonStyle(tb_style)
        # Python Sim controls first for easy access
        sim_toolbar.addAction(self.start_py_sim_action)
        sim_toolbar.addAction(self.stop_py_sim_action) # Adding stop to toolbar as well
        sim_toolbar.addAction(self.reset_py_sim_action) # And reset
    ...
    def _create_status_bar(self):
        ...
        self.status_bar.addWidget(self.status_label, 1) # Stretchable
        ...
        self.progress_bar = QProgressBar(self) # For MATLAB operations
    ...
    def _create_docks(self):
        ...
        # Tools Dock (Interaction Modes & Draggable Elements)
        ...
        # Use QToolButtons linked to existing QActions for modes in dock
        ...
        drag_initial_state_btn.setIcon(get_standard_icon(QStyle.SP_ToolBarHorizontalExtensionButton, "ISt")) # Icon more indicative
        drag_final_state_btn = DraggableToolButton(" Final State", "application/x-bsm-tool", "Final State")
        drag_final_state_btn.setIcon(get_standard_icon(QStyle.SP_DialogOkButton, "FSt")) # Icon for final
        ...
        self.log_dock.setAllowedAreas(Qt.BottomDockWidgetArea | Qt.TopDockWidgetArea | Qt.LeftDockWidgetArea | Qt.RightDockWidgetArea) # Flexible
        self.log_output = QTextEdit(); self.log_output.setObjectName("LogOutputWidget") # For QSS
        self.log_output.setReadOnly(True); self.log_dock.setWidget(self.log_output)
        ...
        self.properties_editor_label = QLabel("<i>No item selected.</i>") # Display summary here
        ...
        self.properties_edit_button.setEnabled(False); self.properties_edit_button.clicked.connect(self._on_edit_selected_item_properties_from_dock)
    ...
    def _create_central_widget(self):
        self.view = ZoomableView(self.scene, self) # Main canvas
    ...
    def _create_py_simulation_dock(self):
        ...
        self.py_sim_dock.setObjectName("PySimDock") # For QSS
        ...
        # Control Row 1: Start, Stop, Reset (Buttons use existing actions)
        ...
        for btn in [self.py_sim_start_btn, self.py_sim_stop_btn, self.py_sim_reset_btn]:
            btn.setToolButtonStyle(Qt.ToolButtonTextBesideIcon); btn.setIconSize(QSize(18,18))
            control_row1_layout.addWidget(btn)
        py_sim_main_layout.addLayout(control_row1_layout)

        # Control Row 2: Step, Trigger Event
        ...
        control_row2_layout.addWidget(self.py_sim_event_name_edit, 1) # Line edit stretches
        control_row2_layout.addWidget(self.py_sim_trigger_event_btn)
        py_sim_main_layout.addLayout(control_row2_layout)

        # Display Area: Current State & Variables
        ...
        self.py_sim_variables_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Interactive) # Allow resizing first column
        ...
        self.py_sim_variables_table.setAlternatingRowColors(True) # Enhances readability
        self.py_sim_variables_table.setFixedHeight(150) # Or use size policy for stretch
        ...
        self.py_sim_action_log_output.setMinimumHeight(100) # Ensure it's visible
        log_layout.addWidget(self.py_sim_action_log_output)
        py_sim_main_layout.addWidget(log_group, 1) # Let log stretch vertically

        py_sim_widget.setLayout(py_sim_main_layout)
        self.py_sim_dock.setWidget(py_sim_widget)
        self.addDockWidget(Qt.RightDockWidgetArea, self.py_sim_dock) # Add to right by default
    ...
    def _update_properties_dock(self):
        ...
        if len(selected_items) == 1:
            ...
            props = item.get_data()
            item_type_name = type(item).__name__.replace("Graphics", "").replace("Item", "") # "State", "Transition", "Comment"
            ...
            def format_prop_text(text_content, max_chars=25):
                ...
                if len(first_line) > max_chars or '\n' in escaped:
                    return first_line[:max_chars] + "&hellip;" # Ellipsis for overflow
                return first_line

            rows_html = ""
            if isinstance(item, GraphicsStateItem):
                color_val = props.get('color', COLOR_ITEM_STATE_DEFAULT_BG)
                color_obj = QColor(color_val)
                text_on_color = 'black' if color_obj.lightnessF() > 0.5 else 'white'
                color_style = f"background-color:{color_val}; color:{text_on_color}; padding: 1px 4px; border-radius:2px;"
    ...
    def log_message(self, message: str, is_py_sim_log_source=False):
        if not is_py_sim_log_source: # Avoid double-logging if already in PySim log
            timestamp = QTime.currentTime().toString('hh:mm:ss.zzz')
            # Use html.escape for user-provided parts of message, style manually for timestamp
            self.log_output.append(f"<span style='color:{COLOR_TEXT_SECONDARY};'>[{timestamp}]</span> {html.escape(message)}")
            # Auto-scroll to bottom
            self.log_output.verticalScrollBar().setValue(self.log_output.verticalScrollBar().maximum())
        
        # Update status bar with concise message, unless it's a detailed technical log
        if not message.startswith("MATLAB Conn:") and not message.startswith("PySim Action:") and "worker:" not in message:
    ...
    def _update_window_title(self):
        ...
        # Standard Qt way to indicate modified: add "[*]" which setWindowModified fills
        title += "[*]"
        self.setWindowTitle(title)

    def _update_save_actions_enable_state(self):
        self.save_action.setEnabled(self.isWindowModified())
        # save_as_action is always enabled if there's a diagram.
    ...
    def _update_undo_redo_actions_enable_state(self):
        ...
        # Update text to show command name
    ...
    def _update_matlab_status_display(self, connected, message):
        ...
        style_sheet += f"color: {COLOR_PY_SIM_STATE_ACTIVE.name()};" if connected else f"color: #C62828;" # Use same green or red
        self.matlab_status_label.setStyleSheet(style_sheet)
        self.log_message(f"MATLAB Conn: {message}") # Log full message
        self._update_matlab_actions_enabled_state()

    def _update_matlab_actions_enabled_state(self):
        # MATLAB actions depend on connection AND not being in PySim mode.
        can_run_matlab = self.matlab_connection.connected and not self.py_sim_active
        self.export_simulink_action.setEnabled(can_run_matlab)
        self.run_simulation_action.setEnabled(can_run_matlab)
        self.generate_code_action.setEnabled(can_run_matlab)
        # MatlabSettings can always be accessed
    ...
    def set_ui_enabled_for_matlab_op(self, enabled: bool):
        """Disable/enable UI parts during long MATLAB operations."""
        self.menuBar().setEnabled(enabled)
        # Disable all toolbars except perhaps a "Cancel" one if implemented
        for child in self.findChildren(QToolBar):
            child.setEnabled(enabled)
        if self.centralWidget(): self.centralWidget().setEnabled(enabled)
        # Docks can be disabled as well
        for dock_name in ["ToolsDock", "PropertiesDock", "LogDock", "PySimDock"]:
            dock = self.findChild(QDockWidget, dock_name)
            if dock: dock.setEnabled(enabled)
        # Python sim actions also need to respect MATLAB op state
        self._update_py_simulation_actions_enabled_state()


    def _handle_matlab_modelgen_or_sim_finished(self, success, message, data):
        self._finish_matlab_operation() # UI cleanup
        self.log_message(f"MATLAB Result ({('Success' if success else 'Failure')}): {message}")
        if success:
            if "Model generation" in message and data: # Data here is model path
            ...
            elif "Simulation" in message: # Data is a success message string
    ...
    def _handle_matlab_codegen_finished(self, success, message, output_dir): # data is output_dir
    ...
    def _prompt_save_if_dirty(self) -> bool:
        if not self.isWindowModified(): return True # Not dirty, proceed

        if self.py_sim_active: # Check if Python simulation is running
    ...
    def on_open_file(self):
        ...
        if file_path:
            ...
            if self._load_from_path(file_path):
                ...
                self.last_generated_model_path = None # Reset SLX path context
                self.undo_stack.clear(); self.scene.set_dirty(False) # Reset modified for new file
                self._update_window_title(); self._update_undo_redo_actions_enable_state()
    ...
    def on_save_file_as(self) -> bool:
        ...
        if file_path:
            if not file_path.lower().endswith(FILE_EXTENSION): file_path += FILE_EXTENSION # Ensure extension
            if self._save_to_path(file_path):
                self.current_file_path = file_path # Update current path
                self.scene.set_dirty(False) # Clear modified status
                self._update_window_title()
                return True
        return False

    def _save_to_path(self, file_path) -> bool:
        # Use QSaveFile for atomic saves (prevents data loss if save fails mid-way)
        ...
        try:
            ...
            if bytes_written == -1 : # Error in writing
                error_str = save_file.errorString()
                ...
                save_file.cancelWriting(); return False # Discard temp file
            if not save_file.commit(): # Finalize save (rename temp to actual)
                error_str = save_file.errorString()
            ...
            self.scene.set_dirty(False) # Saved, so not dirty anymore
            return True
        except Exception as e:
    ...
    def on_export_simulink(self):
        ...
        # Suggest model name based on current file
        model_name_default = "BSM_SimulinkModel"
        if self.current_file_path:
            base_name = os.path.splitext(os.path.basename(self.current_file_path))[0]
            model_name_default = "".join(c if c.isalnum() or c=='_' else '_' for c in base_name) # Sanitize for MATLAB
    ...
    def on_run_simulation(self):
        ...
        # Default to dir of last generated model, or current file, or home
        ...
        sim_time, ok = QInputDialog.getDouble(self, "Simulation Time", "Simulation stop time (seconds):", 10.0, 0.001, 86400.0, 3) # min, max, decimals
        if not ok: return
    ...
    def on_generate_code(self):
        ...
        default_output_base = os.path.dirname(model_path) # Codegen output typically goes relative to model
    ...
    def on_about(self):
        QMessageBox.about(self, "About " + APP_NAME,
            ...
            "and integrates with MATLAB/Simulink for simulation and C/C++ code generation. It also supports basic internal Python FSM simulation.</p>"
            ...
            "<li><b>Internal Python Simulation:</b> Allows stepping through the FSM logic, observing state changes and basic variable manipulation directly within the application.</li>"
            ...
            f"<p style='font-size:8pt; color:{COLOR_TEXT_SECONDARY};'>This tool is intended for research and educational purposes.</p>")

    def closeEvent(self, event: QCloseEvent):
        self.on_stop_py_simulation(silent=True) # Ensure Python Sim is stopped first
        if self._prompt_save_if_dirty():
            # Check for active MATLAB threads and inform user
            ...
            if active_threads:
                ...
                # Ideally, implement a more graceful shutdown for these threads if needed,
                # but for now, a message is better than nothing.
            event.accept()
        else:
    ...
    def _update_py_simulation_actions_enabled_state(self):
        # Check if any MATLAB operation is ongoing (progress bar visible)
        is_matlab_op_running = self.progress_bar.isVisible()

        # Python sim main actions in menu/toolbar
    ...
    def set_ui_enabled_for_py_sim(self, is_sim_running: bool):
        ...
        self._update_window_title() # Reflects "[PySim Running]"
        
        is_editable = not is_sim_running # Editing is inverse of sim running

        # Diagram interaction
        if is_editable: # If becoming editable, ensure select mode is set
             if self.scene.current_mode != "select": self.scene.set_mode("select")
        else: # If simulation starts, force select mode and disable item movement flags.
            self.scene.set_mode("select") # Avoids being stuck in add/transition
            for item_view in self.scene.items():
                if isinstance(item_view, (GraphicsStateItem, GraphicsCommentItem)):
                    item_view.setFlag(QGraphicsItem.ItemIsMovable, False)
        ...
        self.tools_dock.setEnabled(is_editable) # Whole ToolsDock
        # PropertiesDock edit button
        self.properties_edit_button.setEnabled(is_editable and len(self.scene.selectedItems())==1)
        
        # Update MATLAB actions and Python simulation specific controls
        self._update_matlab_actions_enabled_state() # Reflects py_sim_active indirectly
        self._update_py_simulation_actions_enabled_state()
        self._update_py_sim_status_display()
        ...
        if state_name_to_highlight:
            for item in self.scene.items():
                if isinstance(item, GraphicsStateItem) and item.text_label == state_name_to_highlight:
                    ...
                    # Optional: Scroll to ensure the newly active item is visible
                    # self.view.ensureVisible(item, 50, 50) # 50px margin around item
                    break
        self.scene.update() # Ensure scene repaints

    def _update_py_simulation_dock_ui(self):
        if not self.py_fsm_engine or not self.py_sim_active: # Ensure sim is active
            self.py_sim_current_state_label.setText("<i>Not Running</i>")
            self.py_sim_variables_table.setRowCount(0) # Clear table
            self._highlight_sim_active_state(None) # Clear visual highlight
            return
        ...
        row = 0
        for name, value in sorted(variables.items()): # Sort for consistent display
...
# ...existing code...
    ...
    def on_start_py_simulation(self):
        if self.py_sim_active:
            ...
            return
        if self.scene.is_dirty(): # Check for unsaved changes relevant to simulation
             reply = QMessageBox.question(self, "Unsaved Changes",
                                          "The diagram has unsaved changes that won't be reflected in this simulation session if you don't save.\nStart simulation with current in-memory state anyway?",
        ...
        try:
            ...
            self.set_ui_enabled_for_py_sim(True) # This sets self.py_sim_active
            self.py_sim_action_log_output.clear() # Clear log from previous session
            self._append_to_py_simulation_log(self.py_fsm_engine.get_last_executed_actions_log()) # Initial entry actions
    ...
    def on_stop_py_simulation(self, silent=False):
        if not self.py_sim_active:
            if not silent: # QMessageBox.information(self, "Simulation Not Active", "Python simulation is not currently running.")
                pass # Do nothing if already stopped, unless being called silently for cleanup
            return

        self.py_fsm_engine = None # Release engine
        self.set_ui_enabled_for_py_sim(False) # This updates py_sim_active and UI
        self._update_py_simulation_dock_ui() # Clear displays (current state, variables)
        self._highlight_sim_active_state(None) # Remove any state highlight
        # Log is usually kept for review until next start, or can be cleared here:
        # self.py_sim_action_log_output.clear()
        if not silent: self.log_message("Python FSM Simulation stopped.")
    ...
    def on_reset_py_simulation(self):
        ...
        try:
            ...
            self.py_sim_action_log_output.clear() # Clear log on reset
            self._append_to_py_simulation_log(self.py_fsm_engine.get_last_executed_actions_log()) # Entry actions of new initial state
            self._update_py_simulation_dock_ui()
            ...
            self._append_to_py_simulation_log([f"ERROR DURING RESET: {e}"])
        except Exception as e:
            QMessageBox.critical(self, "Error", f"An unexpected error occurred during reset:\n{e}")
            self._append_to_py_simulation_log([f"CRITICAL ERROR DURING RESET: {e}"])


    def on_step_py_simulation(self): # Step without explicit event (for time-based or during actions)
        if not self.py_fsm_engine or not self.py_sim_active:
            QMessageBox.warning(self, "Simulation Not Active", "Python simulation is not running.")
            return
        try:
            _, log_entries = self.py_fsm_engine.step(event_name=None) # Internal step evaluates conditions/during actions
    ...
    def on_trigger_py_event(self):
        ...
        if not event_name: # If no event entered, treat as an internal step.
        ...
        try:
            ...
            self._append_to_py_simulation_log(log_entries)
            self._update_py_simulation_dock_ui()
            self.py_sim_event_name_edit.clear() # Clear input field after successful trigger
        except FSMError as e:
            QMessageBox.warning(self, "Simulation Event Error", str(e))
            self._append_to_py_simulation_log([f"ERROR DURING EVENT '{html.escape(event_name)}': {e}"])
        except Exception as e:
            QMessageBox.critical(self, "Error", f"An unexpected error occurred during event trigger:\n{e}")
            self._append_to_py_simulation_log([f"CRITICAL ERROR DURING EVENT '{html.escape(event_name)}': {e}"])
...
if __name__ == '__main__':
    ...
    # Path for icons used in QSS (e.g., combobox arrow)
    # This assumes 'dependencies/icons' is relative to the script's location
    dependencies_dir = os.path.join(app_dir, "dependencies", "icons")
    if not os.path.exists(dependencies_dir):
        try:
            os.makedirs(dependencies_dir, exist_ok=True)
            print(f"Info: Created directory for QSS icons: {dependencies_dir}")
        except OSError as e:
            print(f"Warning: Could not create directory {dependencies_dir}: {e}")
    # An alternative for QSS icons is using Qt Resource System (qrc files compiled with pyrcc5)
    # For example: QDir.addSearchPath('qss_icons', dependencies_dir)
    # then in QSS: url(qss_icons:arrow_down.png)
    # However, the provided QSS uses relative path directly: ./dependencies/icons/...
    # This relies on the application's current working directory.
    # For robustness, ensure CWD is app_dir if using such relative paths in QSS or load them explicitly.
