...
class MatlabConnection(QObject):
    ...
    def generate_simulink_model(self, states, transitions, output_dir, model_name="BrainStateMachine"):
        ...
        for i, state in enumerate(states):
            ...
            state_label_parts = []
            action_types = {'entry': state.get('entry_action'), 
                'during': state.get('during_action'), 
                'exit': state.get('exit_action')}
            for action_name, action_code in action_types.items():
                if action_code:
                    escaped_action_code = action_code.replace("'", "''").replace('%', '%%').replace('\\', '\\\\')
                    state_label_parts.append(f"{action_name}: {escaped_action_code.replace(chr(10), '; ')}") # Newline to ;
            s_label_string_parts_for_sprintf = []
            
            
            if state.get('entry_action'): s_label_string_parts_for_sprintf.append(f"entry: {state['entry_action'].replace('%','%%').replace(chr(10),'; ')}") # Escape % for sprintf
            if state.get('during_action'): s_label_string_parts_for_sprintf.append(f"during: {state['during_action'].replace('%','%%').replace(chr(10),'; ')}")
            if state.get('exit_action'): s_label_string_parts_for_sprintf.append(f"exit: {state['exit_action'].replace('%','%%').replace(chr(10),'; ')}")
            s_label_string_for_sprintf = "\\n".join(s_label_string_parts_for_sprintf) if s_label_string_parts_for_sprintf else ""
            s_label_string_matlab_literal = s_label_string_for_sprintf.replace("'", "''") # Escape for the outer MATLAB string
    ...
    # Use the correctly escaped string for sprintf
                f"if ~isempty('{s_label_string_matlab_literal}'), {s_id_matlab_safe}.LabelString = sprintf('{s_label_string_matlab_literal}'); end",
                f"stateHandles('{s_name_matlab}') = {s_id_matlab_safe};"
                ])
            
            
            t_label_parts_for_sprintf = []
            if trans.get('event'): t_label_parts_for_sprintf.append(trans['event'].replace('%','%%'))
            if trans.get('condition'): t_label_parts_for_sprintf.append(f"[{trans['condition'].replace('%','%%')}]")
            if trans.get('action'): t_label_parts_for_sprintf.append(f"/{{{trans['action'].replace('%','%%')}}}")
            t_label_for_sprintf = " ".join(t_label_parts_for_sprintf).strip()
            t_label_matlab_literal = t_label_for_sprintf.replace("'", "''")
            if t_label_matlab_literal:
                script_lines.append(f"    t{i}.LabelString = sprintf('{t_label_matlab_literal}');")
...
class MatlabCommandWorker(QObject):
    ...
    def run_command(self):
        ...
        try:
            ...
            if "MATLAB_SCRIPT_FAILURE:" in stdout_str:
                success = False
                extracted_error_detail = "Details not found in script output." # Default
                ...
                # Append more from stdout/stderr if available and not redundant
                if stderr_str and extracted_error_detail not in stderr_str: # Avoid duplicating error
                    message += f"\nMATLAB Stderr: {stderr_str[:500]}"
            elif stdout_str and extracted_error_detail not in stdout_str:
            # Capture surrounding lines from stdout for context if error detail is brief
                stdout_context_for_failure = "\n".join([line for line in stdout_str.splitlines() if "ERROR" in line.upper() or "WARNING" in line.upper() or model_name_orig in line][:10]) # model_name_orig might not be available here, consider passing
    ...
    # ...
            elif process.returncode != 0: # MATLAB process itself had an error (non-zero exit code)
                success = False
                error_output_detail = stderr_str or stdout_str # Prefer stderr if available
        # Try to find a specific MATLAB error message
                ...
                if matlab_error_lines:
                    specific_error = " ".join(matlab_error_lines[:2]) # Take first few lines of specific error
