...
class AIChatUIManager(QObject):
    ...
    def _markdown_to_html_basic(self, text_part: str) -> str:
        # Placeholder for inline code `text`
        text_part = re.sub(r'`(.*?)`', r'‹‹‹ICODE›››\1‹‹‹/ICODE›››', text_part, flags=re.DOTALL)
        # Placeholder for bold **text** or __text__
        ...
        # Placeholder for italics *text* or _text_
        text_part = re.sub(r'(?<!\w)\*([^*\s](?:[^*]*[^*\s])?)\*(?!\w)', r'‹‹‹EM›››\1‹‹‹/EM›››', text_part)
        text_part = re.sub(r'(?<!\w)_([^_ \s](?:[^_]*[^_ \s])?)_(?!\w)', r'‹‹‹EM›››\1‹‹‹/EM›››', text_part)
        ...
        for line in lines:
            ...
            if match:
                list_item_content = match.group(1).strip()
                if not in_list:
                    processed_lines.append("<ul>")
                    in_list = True
                
                # Apply basic formatting placeholders to list item content before escaping
                list_item_content_md = list_item_content
                list_item_content_md = re.sub(r'`(.*?)`', r'‹‹‹ICODE›››\1‹‹‹/ICODE›››', list_item_content_md, flags=re.DOTALL)
                list_item_content_md = re.sub(r'\*\*(.*?)\*\*', r'‹‹‹STRONG›››\1‹‹‹/STRONG›››', list_item_content_md, flags=re.DOTALL)
                list_item_content_md = re.sub(r'__(.*?)__', r'‹‹‹STRONG›››\1‹‹‹/STRONG›››', list_item_content_md, flags=re.DOTALL)
                list_item_content_md = re.sub(r'(?<!\w)\*([^*\s](?:[^*]*[^*\s])?)\*(?!\w)', r'‹‹‹EM›››\1‹‹‹/EM›››', list_item_content_md)
                list_item_content_md = re.sub(r'(?<!\w)_([^_ \s](?:[^_]*[^_ \s])?)_(?!\w)', r'‹‹‹EM›››\1‹‹‹/EM›››', list_item_content_md)
                
                processed_lines.append(f"<li>{html.escape(list_item_content_md)}</li>") # Escape after applying placeholders
            else:
                if in_list:
                    processed_lines.append("</ul>")
                    in_list = False
        ...
        text_part_escaped = "<br>".join(processed_lines)
        ...
        text_part_escaped = text_part_escaped.replace(html.escape("‹‹‹ICODE›››"), f"<code style='{inline_code_style}'>").replace(html.escape("‹‹‹/ICODE›››"), "</code>")
        
        # Unescape list tags and their content if they were escaped
        text_part_escaped = text_part_escaped.replace(html.escape("<ul>"), "<ul>").replace(html.escape("</ul>"), "</ul>")
        # For li, we need to unescape the content that now has placeholders, then re-apply markdown conversion to that content
        
        def unescape_li_content(match_obj):
            escaped_li_content_with_placeholders = match_obj.group(1)
            # Since the content inside <li> was already processed for placeholders and then escaped,
            # we need to re-apply the placeholder-to-HTML conversion here for the content of <li> only.
            unescaped_li_content = escaped_li_content_with_placeholders # It's already unescaped by html.escape above
            unescaped_li_content = unescaped_li_content.replace(html.escape("‹‹‹STRONG›››"), "<strong>").replace(html.escape("‹‹‹/STRONG›››"), "</strong>")
            unescaped_li_content = unescaped_li_content.replace(html.escape("‹‹‹EM›››"), "<em>").replace(html.escape("‹‹‹/EM›››"), "</em>")
            unescaped_li_content = unescaped_li_content.replace(html.escape("‹‹‹ICODE›››"), f"<code style='{inline_code_style}'>").replace(html.escape("‹‹‹/ICODE›››"), "</code>")
            return f"<li>{unescaped_li_content}</li>"

        text_part_escaped = re.sub(html.escape("<li>") + "(.*?)" + html.escape("</li>"), unescape_li_content, text_part_escaped)
