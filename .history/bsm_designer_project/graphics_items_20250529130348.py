...
# For GraphicsStateItem and GraphicsTransitionItem that emit signals:
...
# GraphicsStateItem: QObject first is correct for QGraphicsRectItem (not a QGraphicsObject)
class GraphicsStateItem(QObject, QGraphicsRectItem):
    Type = QGraphicsItem.UserType + 1
    nameChanged = pyqtSignal(QGraphicsItem, str, str) # item, old_name, new_name
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
        ...
        QGraphicsRectItem.__init__(self, x, y, w, h, parent_item)
    ...
    def set_properties(self, name, is_initial, is_final, color_hex=None,
        ...
        changed = False
        old_name_prop = self.text_label # Store current name before any change
        if self.text_label != name:
            self.text_label = name
            changed = True
            # Emit nameChanged signal if it exists and the name actually changed
            if hasattr(self, 'nameChanged'):
                self.nameChanged.emit(self, old_name_prop, self.text_label)
        # Ensure old_name_prop reflects the name *before* this properties update,
        # even if it wasn't changed by the 'name' parameter in *this* call.
        # This is important if nameChanged needs to be emitted for other reasons or if
        # the name was changed by set_text prior to set_properties in an undo operation.
        # However, the current logic correctly emits only if `self.text_label != name`.
...
# GraphicsTransitionItem: QObject first is correct for QGraphicsPathItem (not a QGraphicsObject)
class GraphicsTransitionItem(QObject, QGraphicsPathItem):
    ...
    def __init__(self, start_item, end_item, event_str="", condition_str="", action_str="",
        ...
        self.setAcceptHoverEvents(True)
        self.update_path()
    # ... (rest of GraphicsTransitionItem methods)
...
# GraphicsCommentItem already inherits from QGraphicsObject (via QGraphicsTextItem),
# so it can emit signals if needed without explicitly adding QObject here.
# If it didn't and needed signals, then QObject would be first.
class GraphicsCommentItem(QGraphicsTextItem): # No QObject needed here as QGraphicsTextItem -> QGraphicsObject -> QObject
    Type = QGraphicsItem.UserType + 3
    def type(self): return GraphicsCommentItem.Type

    def __init__(self, x, y, text="Comment", parent_item=None): # QGraphicsItem can have a parent_item
        super().__init__(parent_item) # Call QGraphicsTextItem constructor
