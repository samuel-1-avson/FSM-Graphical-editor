...
# Removed basicConfig setup from here; it's handled by logging_setup.py or main.py's fallback.
# Ensure logger level is set if not inheriting appropriately from root.
if not logger.hasHandlers() and not logging.getLogger().hasHandlers(): # Only add if no handlers at all
    LOGGING_DATE_FORMAT = "%H:%M:%S"
    handler = logging.StreamHandler()
    formatter = logging.Formatter("--- FSM_SIM (%(asctime)s.%(msecs)03d): %(message)s", datefmt=LOGGING_DATE_FORMAT)
    handler.setFormatter(formatter)
    logger.addHandler(handler)
    logger.setLevel(logging.INFO) # Set default for this logger
...
class BasicSafetyVisitor(ast.NodeVisitor):
    def __init__(self, allowed_variable_names=None):
        ...
        self.allowed_call_names = {
            ...
            'isinstance', 'hasattr',
            # Common math functions (consider adding `import math` to SAFE_BUILTINS if these are needed often)
            'sqrt', 'sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'atan2', 'degrees', 'radians',
            'exp', 'log', 'log10', 'pow', 'fabs', 'floor', 'ceil',
        }
        self.allowed_dunder_attrs = {
            ...
            '__enter__', '__exit__', # For `with` statements if ever allowed (currently not)
        }
        # Attributes that could be dangerous if accessed or modified.
        self.dangerous_attributes = {
            ...
            '_as_parameter_', '_fields_', '_length_', '_type_', # ctypes related
            '__annotations__', '__qualname__', '__module__',
            '__slots__', '__weakref__', '__set_name__',
            'format_map', 'mro', 'with_traceback', # Exception methods
            'cr_frame', 'cr_running', 'cr_await', 'cr_origin', # Coroutine internals
            'ag_running', 'ag_frame', 'ag_await', # Async generator internals
            'tb_frame', 'tb_lineno', 'tb_next', # Traceback object attributes
        }
        # Truly dangerous are those not explicitly allowed (even if dunder)
    ...
    def visit_Import(self, node):
        ...
        super().generic_visit(node) # Continue visiting children, though the import itself is disallowed
    ...
    def visit_Call(self, node):
        # Check for disallowed direct function calls
        if isinstance(node.func, ast.Name):
            func_name = node.func.id
            # Disallowed built-in functions that are dangerous
            if func_name in ('eval', 'exec', 'compile', 'open', 'input',
                             'getattr', 'setattr', 'delattr', # Direct use of these can bypass checks
                             'globals', 'locals', 'vars', # Accessing execution scopes
                             '__import__', # The import function itself
                             'memoryview', 'bytearray', 'bytes' # Low-level memory manipulation
                             ):
                self.violations.append(f"SecurityError: Calling the function '{func_name}' is not allowed.")
            # Check if the function is in the allowed list or provided variables/safe builtins
                ...
                # This path implies a call to a function not explicitly whitelisted or made available.
                # This could be a user-defined function not passed in the initial context,
                # or a more obscure built-in. For stricter security, this should be a violation.
                self.violations.append(f"SecurityError: Calling the unknown/disallowed function '{func_name}'.")
        # Also check for calls on attributes (e.g., some_object.dangerous_method())
        # This is partially covered by visit_Attribute, but specific method calls could be added here.
        super().generic_visit(node)
    ...
    def visit_Attribute(self, node):
        ...
        if isinstance(node.attr, str):
            ...
            # Check for dunder attributes not in the allowed list
    ...
    def visit_Exec(self, node): # Python 2.x specific, ast.parse in Py3 for 'exec' statement will error
        self.violations.append("SecurityError: The 'exec' statement/function is not allowed.")
        super().generic_visit(node)
    
    # Add checks for other potentially dangerous nodes if needed, e.g., ast.Delete for `del` on sensitive items.
    # For now, the focus is on preventing arbitrary code execution and scope manipulation.

def check_code_safety_basic(code_string: str, fsm_variables: set) -> tuple[bool, str]:
    if not code_string.strip():
        return True, "" # Empty code is safe
    try:
        tree = ast.parse(code_string, mode='exec') # Parse as a module/script
    ...
    except Exception as e: # Catch other parsing/visiting errors
        return False, f"Unexpected error during code safety check: {type(e).__name__} - {e}"

# Restricted set of builtins available in the `exec` and `eval` contexts
SAFE_BUILTINS = {
    ...
    # Consider adding math module or specific functions if commonly needed
    # "math": math, # If you add 'math', ensure 'import math' is also handled or math functions are individually whitelisted
}
...
class StateMachinePoweredSimulator:
    def __init__(self, states_data, transitions_data, parent_simulator=None, log_prefix="", halt_on_action_error=False):
        ...
        self._variables = {} # Holds variables for the FSM's execution scope
        self._action_log = [] # Log of actions and events
        self.FSMClass = None # Dynamically created StateMachine class
        self.sm: StateMachine | None = None # Instance of the FSMClass
        self._initial_state_name = None # Name of the initial state
        self.parent_simulator: StateMachinePoweredSimulator | None = parent_simulator
        self.log_prefix = log_prefix # For hierarchical logging

        self.active_sub_simulator: StateMachinePoweredSimulator | None = None # For superstates
        self.active_superstate_name: str | None = None # Name of the active superstate
        self._halt_simulation_on_action_error = halt_on_action_error # Whether to stop on error
        self.simulation_halted_flag = False # Flag to indicate if simulation is halted
        ...
        try:
            ...
            logger.error(f"{self.log_prefix}FSM Definition Error during Initialization: {e}", exc_info=False) # exc_info=False to avoid redundant traceback for known def errors
    ...
    def _log_action(self, message, level_prefix_override=None):
        """Logs an action message to internal log and global logger."""
        ...
        logger.info(full_message) # Use global logger

    def _create_dynamic_callback(self, code_string, callback_type="action", original_name="dynamic_callback"):
        """Creates a callback function from a code string with safety checks."""
        ...
        if not is_safe:
            ...
            if callback_type == "condition":
                def unsafe_condition_wrapper(*args, **kwargs):
                    ...
                    return False # Conditions that fail safety check should evaluate to False
        ...
        simulator_self = self # Closure for the wrapper

        def dynamic_callback_wrapper(*args, **kwargs_from_sm_call):
            # Extract the StateMachine instance if passed (it's usually the first arg or in kwargs)
            ...
            if not sm_instance_arg: # Should ideally not happen with python-statemachine
                ...
                return # No return for actions

            # Use the simulator's _variables dictionary as the local scope for exec/eval
            ...
            # Determine current state for logging context
            ...
            try:
                if callback_type == "action":
                    ...
                    return None # Actions typically don't return a meaningful value to the FSM
                elif callback_type == "condition":
                    # For eval, ensure we don't modify the _variables dict; pass a copy for safety if needed.
                    # However, conditions *should not* modify variables. If they do, it's bad practice.
                ...
                if callback_type == "condition": return False # Failed condition
            ...
            except NameError as e: # Variable not found
                ...
                simulator_self._log_action(f"[Code Error] {err_msg}")
                logger.warning(f"{simulator_self.log_prefix}{err_msg}") # Warning, as it might be intentional logic
                if callback_type == "condition": return False
                if simulator_self._halt_simulation_on_action_error and callback_type == "action": simulator_self.simulation_halted_flag = True; raise FSMError(err_msg)
            except TypeError as e: # e.g. calling non-callable, wrong num args for whitelisted func
                ...
                if callback_type == "condition": return False
                if simulator_self._halt_simulation_on_action_error and callback_type == "action": simulator_self.simulation_halted_flag = True; raise FSMError(err_msg)
            except (AttributeError, IndexError, KeyError, ValueError, ZeroDivisionError) as e: # Common runtime errors
                ...
                if callback_type == "condition": return False
                if simulator_self._halt_simulation_on_action_error and callback_type == "action": simulator_self.simulation_halted_flag = True; raise FSMError(err_msg)
            return None # Default for actions if no error
        
        # Give the wrapper a unique name for debugging and to satisfy python-statemachine internals
    ...
    def _master_on_enter_state_impl(self, sm_instance: StateMachine, target: State, **kwargs):
        """Master callback for state entry, handles sub-machine initialization."""
        ...
        if target_state_name in self._states_input_data:
            ...
            if state_def.get('is_superstate', False):
                ...
                if sub_fsm_data and sub_fsm_data.get('states'): # Ensure sub-machine has states
                    ...
                    try:
                        self.active_sub_simulator = StateMachinePoweredSimulator(
                            ...
                            parent_simulator=self, # Pass self as parent
                            log_prefix=self.log_prefix + "  [SUB] ", # Indent log prefix
                        ...
                        for sub_log_entry in self.active_sub_simulator.get_last_executed_actions_log():
                            self._action_log.append(sub_log_entry) # Avoid double logging
                    except Exception as e:
                    ...
                    self._log_action(f"Superstate '{target_state_name}' has no defined sub-machine data or states. It will act as a simple state.")

    def _master_on_exit_state_impl(self, sm_instance: StateMachine, source: State, **kwargs):
        """Master callback for state exit, handles sub-machine termination."""
    ...
    def _sm_before_transition_impl(self, sm_instance: StateMachine, event: str, source: State, target: State, **kwargs):
        """Generic callback executed before any transition."""
        event_data_obj = kwargs.get('event_data') # event_data is passed by python-statemachine
    ...
    def _sm_after_transition_impl(self, sm_instance: StateMachine, event: str, source: State, target: State, **kwargs):
        """Generic callback executed after any transition."""
    ...
    def _build_fsm_class_and_instance(self):
        """Dynamically builds the StateMachine class and an instance of it."""
        fsm_class_attrs = {}
        sm_states_obj_map = {} # Maps state name to State object
        initial_state_name_from_data = None
        simulator_self = self # For lambdas to close over

        # Create State objects and map them
        for s_name, s_data in self._states_input_data.items():
            is_initial = s_data.get('is_initial', False)
            if is_initial:
                if initial_state_name_from_data: # Check for multiple initial states
                ...
                self._initial_state_name = s_name # Store for reset logic

            state_obj = State(name=s_name, value=s_name, initial=is_initial, final=s_data.get('is_final', False))
            
            # Dynamically create on_enter_<state_name> methods using lambdas
            ...
            # Dynamically create on_exit_<state_name> methods
            ...
            fsm_class_attrs[s_name] = state_obj # Add State object to class attributes
            sm_states_obj_map[s_name] = state_obj

        # Determine initial state if not explicitly set
        if not initial_state_name_from_data and self._states_input_data:
            ...
            if first_state_name_from_data in sm_states_obj_map:
                ...
                sm_states_obj_map[first_state_name_from_data]._initial = True # Set flag on State object
        ...
        elif not self._states_input_data: # No states at all
        ...
        for t_idx, t_data in enumerate(self._transitions_input_data):
            ...
            if event_name_str not in defined_events:
                event_obj = SMEvent(event_name_str) # Use SMEvent from import
        ...
        # Add master hooks for state entry/exit and transitions
        ...
        if self.sm and not self.sm.current_state and self._states_input_data:
    ...
    def get_current_state_name(self):
        """Returns the name of the current state, including sub-state if active."""
        if not self.sm: return "Uninitialized" if not self.parent_simulator else "EmptySubFSM"
        name = self.sm.current_state.id
        if self.active_sub_simulator and self.active_sub_simulator.sm and self.active_sub_simulator.sm.current_state:
            name += f" ({self.active_sub_simulator.get_current_state_name()})"
        return name

    def get_current_leaf_state_name(self):
        """Returns the name of the current leaf state in the hierarchy."""
        if self.active_sub_simulator and self.active_sub_simulator.sm and self.active_sub_simulator.sm.current_state:
    ...
    def step(self, event_name=None):
        """Executes a step in the FSM simulation."""
        ...
        try:
            ...
            if self.active_sub_simulator:
                ...
                _, sub_log = self.active_sub_simulator.step(event_name=None) # Internal step
            ...
            if self.simulation_halted_flag or "HALTED due to error" in str(e_halt).upper():
            ...
            if event_name and hasattr(self.sm, event_name):
                if not callable(getattr(self.sm, event_name)): log_msg = f"Event name '{event_name}' conflicts with state or non-event attribute."
                else: log_msg = f"AttributeError processing event '{event_name}': {e}. Internal setup/callback issue?"
    ...
    def get_possible_events_from_current_state(self) -> list[str]:
        """Returns a list of possible event names from the current state(s)."""
        if not self.sm or not self.sm.current_state: return []
        
        possible_events_set = set()
        
        # If a sub-machine is active, its events are primary for its context
        if self.active_sub_simulator and self.active_sub_simulator.sm and self.active_sub_simulator.sm.current_state:
            possible_events_set.update(str(evt.id) for evt in self.active_sub_simulator.sm.allowed_events)
        
        # Also add events from the parent FSM, as parent transitions might be triggered
        # by events while in a superstate (these are transitions *from* the superstate itself).
        if self.sm and self.sm.current_state:
             possible_events_set.update(str(evt.id) for evt in self.sm.allowed_events)

        # Filter out internal/dummy events if they follow a pattern
        return sorted([evt for evt in list(possible_events_set) if not evt.startswith("_internal_")])


FSMSimulator = StateMachinePoweredSimulator

if __name__ == "__main__":
    # Configure basic logging for the __main__ test block
    ...
    try:
        ...
        # To trigger sub-machine event, send it to the main simulator.
        # The main simulator's step method will delegate to active_sub_simulator if appropriate.
        print("\n>>> Trigger 'start_sub_work' (sub-machine event) <<<")
        ...
        print("\n>>> Trigger 'finish_sub_work' (sub-machine event) <<<")
        ...
        try:
            ...
            unsafe_sim_s.step("dummy_event_unsafe") # Trigger entry action if not already
            print_status(unsafe_sim_s, "Unsafe Sim Test After Event (check logs)")
        ...
        error_states = [{"name": "ErrState", "is_initial": True, "entry_action": "my_undefined_var = 1 / 0"}] # Should be NameError or ZeroDivisionError
        ...
        try:
            ...
            print_status(error_sim, "Error Sim Start (should log error in entry action)")
            error_sim.step("dummy_event_error")
            print_status(error_sim, "Error Sim After Event")
        ...
        halt_sim = None 
        try:
            # Instantiation itself might trigger the error if initial state has faulty entry action
            halt_sim = FSMSimulator(halt_error_states, halt_error_trans, halt_on_action_error=True)
            print_status(halt_sim, "Halt Error Sim Init (MAY NOT REACH if error in init path)")
            # If not halted on init, try triggering an event
            halt_sim.step("dummy_event_halt") 
            print_status(halt_sim, "Halt Error Sim After Event (SHOULD BE HALTED)")
            ...
            if halt_sim: print("Halt sim halted_flag:", halt_sim.simulation_halted_flag)
