...
class TransitionPropertiesDialog(QDialog):
    ...
    def _create_insert_snippet_button_qtextedit(self, target_text_edit: QTextEdit, snippets_dict: dict, button_text="Insert..."):
        button = QPushButton(button_text); button.setObjectName("SnippetButton"); button.setIcon(get_standard_icon(QStyle.SP_FileDialogContentsView,"Ins")); button.setIconSize(QSize(16,16)); button.setToolTip("Insert common snippets.")        menu = QMenu(self)
        for name, snippet in snippets_dict.items():
            action = QAction(name, self)
    ...
    def _choose_color(self):
        ...
        if color.isValid():
            ...
            self._update_color_button_style()    def _update_color_button_style(self):
