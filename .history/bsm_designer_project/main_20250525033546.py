...
# from matlab_integration import MatlabConnection # Removed
...
# THIS WAS MISSING OR MISPLACED - NOW CORRECTED
...
# Remove the duplicate ResourceMonitorWorker class and keep this single version:
        ...
        if PYNVML_AVAILABLE and pynvml:
            try:
                ...
                if device_count > 0:
                    ...
                    self._gpu_name_cache = gpu_name.decode() if isinstance(gpu_name, bytes) else str(gpu_name)
            except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    # ... (ALL MainWindow methods from the previous correct response should be here) ...
    # For brevity, I'm not repeating them all again.
    # Ensure that _init_ui, _create_actions, _create_menus, _create_toolbars, _create_docks, _create_status_bar,
    # all the on_... handlers, _update_... handlers, AI methods, Properties methods,
    # Window/File management, Python Sim methods, and closeEvent are copied from the previous version.
    # The `on_matlab_settings` method was specifically updated above to use the embedded connection.
    ...
    def on_matlab_settings(self):
        """Handle MATLAB settings dialog and auto-detection."""
        dialog = MatlabSettingsDialog(self, self.matlab_connection)
        if dialog.exec_() == QDialog.Accepted:
            if dialog.auto_detect_clicked:
                logger.info("MATLAB: Starting auto-detection...")
                if self.matlab_connection.detect_matlab():
                    logger.info("MATLAB: Auto-detection successful.")
                else:
                    logger.warning("MATLAB: Auto-detection failed.")
            else:
                new_path = dialog.matlab_path_edit.text().strip()
                logger.info("MATLAB: Setting path manually to: '%s'", new_path)
                self.matlab_connection.set_matlab_path(new_path)
