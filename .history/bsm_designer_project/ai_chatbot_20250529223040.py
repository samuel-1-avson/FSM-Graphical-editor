...
class ChatbotWorker(QObject):
    ...
    def process_message_slot(self, user_message: str, force_fsm_generation: bool):
        ...
        history_context_limit = -6 # Keep last 3 user/model pairs
        if self.conversation_history:
            for msg in self.conversation_history[history_context_limit:]:
                gemini_contents.append(msg)
        
        # Combine system prompt with current user message for this turn
        # This is a common workaround if `system_instruction` is not available or reliably used at the call level.
        effective_user_message = f"{system_prompt_content}\n\nHuman: {user_message}"
        current_turn_parts = [{"text": effective_user_message}]
        
        # Rebuild gemini_contents for this API call only
        call_contents = []
        if self.conversation_history: # Add history before the current (modified) user message
        ...
        try:
            ...
            generation_args = {
                "contents": call_contents, # Use the specially prepared contents for this call
            ...
            ai_response_content = ""
            # Accessing response content safely
            ...
            # Keep conversation history from growing indefinitely
            max_history_len = 20 # Store last 10 user/model pairs
