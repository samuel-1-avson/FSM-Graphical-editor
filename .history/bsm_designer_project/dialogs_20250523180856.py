...
class BasePropertiesDialog(QDialog):
    ...
    def create_color_button(self, initial_color: str) -> tuple[QPushButton, QColor]:
        ...
        color_obj = QColor(initial_color) # Renamed to avoid conflict in some contexts
...
class StatePropertiesDialog(BasePropertiesDialog):
    ...
    def _choose_color(self):
        ...
        # 'color' here is the result from QColorDialog, distinct from self.current_color
...
class PreferencesDialog(QDialog):
    ...
    def _create_ai_settings_tab(self):
        ...
        # Corrected: Use ai_chatbot_manager.config.api_key if available, otherwise default ""
        current_api_key = ""
        if self.main_window.ai_chatbot_manager and self.main_window.ai_chatbot_manager.config:
             current_api_key = self.main_window.ai_chatbot_manager.config.api_key or ""
        current_api_key = settings_qt.value("api_key", current_api_key, type=str)
        ...
        self.model_name_combo_tab.addItems(["gpt-3.5-turbo", "gpt-4", "gpt-4-turbo", "gpt-4o"]) # Updated models
        
        default_model_from_manager = "gpt-3.5-turbo" # Fallback
        if self.main_window.ai_chatbot_manager and self.main_window.ai_chatbot_manager.config:
    ...
    def apply_all_settings(self):
        ...
        settings_appr_qt = QSettings(APP_NAME, "AppearanceSettings") # Persist to correct section
        ...
        current_manager_api_key = self.main_window.ai_chatbot_manager.config.api_key if self.main_window.ai_chatbot_manager.config else None
        if (current_manager_api_key or "") != new_api_key :
