...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.scene = DiagramScene(self.undo_stack, self) # DiagramScene needs undo_stack
        self.view: ZoomableView | None = None # Will be initialized in _create_central_widget
        
        # Call init_ui EARLIER, so actions exist before managers try to connect to them
        # Also, self.view is created inside init_ui (via _create_central_widget)
        # and some managers (like ViewManager) might need self.view to exist.
        self.init_ui() # Creates UI elements, actions, menus, toolbars, docks (self.view also created here)
        ...
        # Managers - Now created AFTER init_ui, so actions like self.openai_settings_action exist
        ...
        self.view_manager = ViewManager(self) # ViewManager connects to actions created in init_ui
        self.ai_chatbot_manager = AIChatbotManager(self) 
        self.ai_chat_ui_manager = AIChatUIManager(self)   # AIChatUIManager connects to actions
        self.py_sim_ui_manager = PySimulationUIManager(self) # PySimUIManager connects to actions
        ...
        # Logging setup (needs self.log_output which is created in _create_docks called by init_ui)
        try:
            if not hasattr(self, 'log_output') or not self.log_output: 
                # This fallback might be problematic if _create_docks hasn't run fully.
                # log_output is created in _create_docks. init_ui calls _create_docks.
                # Since init_ui is now called before this, log_output should exist.
                self.log_output = QTextEdit() # Should ideally not be needed if init_ui is complete
                logger.warning("MainWindow: log_output fallback used. Check init_ui call order.")
    ...
    def init_ui(self):
        ...
        # ViewManager and AlignmentManager might connect to actions in their __init__
        # or have dedicated connection methods called after actions are created.
        # For ViewManager, actions are connected after its __init__, so ensure actions exist.
        # self.view_manager = ViewManager(self) # Moved to __init__
    ...
    def _create_actions(self):
        ...
        self.gemini_settings_action = QAction(get_standard_icon(QStyle.SP_ComputerIcon, "AISet"), "AI Assistant Settings (Gemini)...", self) # Renamed
        ...
        # Alignment & Distribution Actions (connections now to AlignmentManager)
        ...
        # MainWindow still owns the QAction lists for menu creation, but manager controls enable state
        ...
        # Enablement is now handled by AlignmentManager's update_align_distribute_actions_enable_state
        # for action in self.align_actions: action.setEnabled(False) # Remove this
        # for action in self.distribute_actions: action.setEnabled(False) # Remove this
    ...
    def _create_menus(self):
        ...
        if hasattr(self, 'gemini_settings_action'): ai_menu.addAction(self.gemini_settings_action) # Renamed
    ...
    def on_show_find_item_dialog(self):
        ...
        if hasattr(self.find_item_dialog, 'search_input') and self.find_item_dialog.search_input:
            self.find_item_dialog.selectAll()
