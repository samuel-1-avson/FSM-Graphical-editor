...
from ai_chatbot import AIStatus # <--- CORRECTED IMPORT
...
from config import ( 
    ...
    COLOR_TEXT_ON_ACCENT, COLOR_BACKGROUND_DIALOG 
)
...
class AIChatUIManager(QObject):
    ...
    def on_send_ai_chat_message(self):
        ...
        if self.mw.ai_chatbot_manager:
            ...
            if self.mw.ai_chatbot_manager: # Try to set status even if error occurred before send
                self.mw.ai_chatbot_manager._update_current_ai_status(AIStatus.ERROR, "Status: Manager Not Initialized.")
