...
class ResourceMonitorWorker(QObject):
    ...
    def stop_monitoring(self):
        ...
        self._monitoring = False # This flag will be checked by the loop to exit
        if self._nvml_initialized and PYNVML_AVAILABLE and pynvml:
            try:
                pynvml.nvmlShutdown()
                logger.info("ResourceMonitorWorker: NVML shutdown.")
            except Exception as e: # Catch specific NVMLError if possible
                logger.warning(f"Error shutting down NVML: {e}")
        self._nvml_initialized = False # Ensure it's marked as not initialized
        self._gpu_handle = None
    ...
    def _monitor_resources(self):
        ...
        short_sleep_ms = 100 
        # No need for cycles_per_update if we always collect and emit at self.interval_ms

        while self._monitoring: 
            # It's important that this loop can exit quickly when _monitoring is False
            # If a long operation happens here *before* checking _monitoring, it can delay shutdown.
            ...
            try:
                ...
                if self._monitoring: # Double check before emitting, in case flag changed during data collection
                    self.resourceUpdate.emit(cpu_usage, ram_percent, gpu_util, gpu_name_to_emit)
            
            except psutil.Error as e_psutil: # More specific psutil errors
                 logger.error(f"psutil error in resource monitoring: {e_psutil}", exc_info=False) # Less verbose for common psutil errors
            ...
            # Sleep for the defined interval, but break it into smaller chunks
            # to check the _monitoring flag more frequently.
            # This makes the thread more responsive to the stop signal.
            for _ in range(self.interval_ms // short_sleep_ms):
                if not self._monitoring:
                    break
                QThread.msleep(short_sleep_ms)
            if not self._monitoring: # Check again after the sleep loop
                break
...
class MainWindow(QMainWindow):
    ...
    def _create_actions(self):
        ...
        # View Actions
        # Conditional connection for view actions as self.view is created in _create_central_widget
