...
class CodeEditor(QPlainTextEdit):
    def __init__(self, parent=None):
        ...
        print(f"CodeEditor {id(self)} __init__ isReadOnly (before explicit set): {self.isReadOnly()}") # DEBUG
        self.setReadOnly(False) # Explicitly ensure it's not read-only
        print(f"CodeEditor {id(self)} __init__ isReadOnly (after explicit set): {self.isReadOnly()}") # DEBUG
    ...
    def set_language(self, language: str):
        ...
        # Temporarily disable completer setup for debugging
        if self.completer: 
            self.completer.setWidget(None)
            self.completer = None
        ...
        if language == "Python":
            ...
            # self._setup_python_completer() # <<< TEMPORARILY COMMENTED OUT
        ...
        if self.current_highlighter: 
            ...
            default_format.setForeground(Qt.black) # Use a known, visible color
            cursor.setCharFormat(default_format) 
            cursor.clearSelection()
            self.setTextCursor(cursor) 
            # doc.markContentsDirty(0, doc.toPlainText().length()) # Generally not needed here
            # self.viewport().update() # Generally not needed here
    ...
    def keyPressEvent(self, event: QKeyEvent):
        # --- SIMPLIFIED keyPressEvent for DEBUGGING ---
        print(f"CodeEditor KeyPress: Key={event.key()}, Text='{event.text()}', ReadOnly={self.isReadOnly()}, IsEnabled={self.isEnabled()}, HasFocus={self.hasFocus()}") # Diagnostic
        super().keyPressEvent(event) 
        return 
        # --- ORIGINAL Completer Logic (commented out for now) ---
