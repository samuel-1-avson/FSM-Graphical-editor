    ...
    def process_message_slot(self, user_message: str, force_fsm_generation: bool):
        ...
        try:
            ...
            if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
                ...
                logger.error(error_msg); logger.error(f"Full Gemini response object on block/empty: {response}")
                self.errorOccurred.emit(error_msg); self._current_processing_had_error = True; return
            ...
            self.errorOccurred.emit(f"Invalid request to Gemini: {str(e)[:200]}"); self._current_processing_had_error = True
...
class AIChatbotManager(QObject):
    ...
    def _cleanup_existing_worker_and_thread(self):
        ...
        if self.chatbot_thread and self.chatbot_thread.isRunning():
            ...
            if not self.chatbot_thread.wait(200): # Reduced wait time for faster cleanup if stuck
    ...
    def _validate_fsm_schema(self, data: dict) -> tuple[bool, list[str]]:
        ...
        if 'states' not in data or not isinstance(data['states'], list):
            ...
            state_names = set() # Cannot proceed to check transitions if states are fundamentally wrong
        else:
            state_names = set()
            if not data['states']: # FSM with no states might be valid in some contexts but usually not for generation
            ...
            for i, s_obj in enumerate(data['states']):
                ...
                # ... Add more checks for other state properties like color format, other actions, etc.
        ...
        if 'transitions' not in data or not isinstance(data['transitions'], list):
            ...
            for i, t_obj in enumerate(data['transitions']):
                ...
                # ... Add more checks for event (string), condition (string), action (string), offsets (number), color format etc.
    ...
    def _handle_worker_response(self, ai_response_content: str, was_fsm_generation_attempt: bool):
        ...
        if was_fsm_generation_attempt:
            ...
            try:
                ...
                if is_valid_schema: # Check schema first
                    ...
                    logger.warning(f"MGR_HANDLE_WORKER_RESPONSE: AI FSM Schema validation failed: {error_summary}. Treating as plain text. Data: {fsm_data}")
                    self.errorOccurred.emit(f"AI returned JSON, but it's not a valid FSM: {error_summary[:100]}... Displaying as text.")
            ...
            # Fall through to plainResponseReady if JSON parsing/validation failed for FSM attempt
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        if not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning():
            ...
            if not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning(): # Check again after setup attempt
    ...
    def clear_conversation_history(self):
        ...
        if self.chatbot_worker and self.chatbot_thread and self.chatbot_thread.isRunning():
            ...
            # If worker isn't running, history is effectively gone with the worker or was never there.
            # Manager might not even have a worker if API key was never set.
