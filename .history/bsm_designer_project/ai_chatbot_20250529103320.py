    ...
    def process_message_slot(self, user_message: str, force_fsm_generation: bool):
        ...
        if is_fsm_generation_attempt:
            system_prompt_parts.append(
                ...
                "'action_language' (string, e.g., 'Python (Generic Simulation)', 'Arduino (C++)'), " # Added action_language
        ...
        try:
            ...
            if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
                ...
                finish_reason_str = genai.types.FinishReason(finish_reason_obj).name if finish_reason_obj else "Unknown reason" # Use an explicit string if enum conversion fails
            ...
            # InvalidArgument can be due to bad requests (e.g., malformed JSON request if mime_type was text/plain)
            # or other issues like invalid characters in the prompt not handled by safety settings.
        ...
        except google.api_core.exceptions.GoogleAPIError as e: # Catch-all for other Google API errors
            error_detail = str(e);
        ...
        except (genai.types.BlockedPromptException, genai.types.StopCandidateException) as e: # Specific exceptions for blocked content
            ...
            # If an error occurred, the status might have already been set by the error handler or will be set by the caller.
...
class AIChatbotManager(QObject):
    ...
    def _cleanup_existing_worker_and_thread(self):
        ...
        if self.chatbot_thread and self.chatbot_thread.isRunning():
            ...
            if self.chatbot_worker:
                # Ensure stop_processing_slot is invoked reliably.
                # QueuedConnection is fine, it will be processed by the worker's event loop.
            ...
            if not self.chatbot_thread.wait(300): # Slightly increased wait time
                logger.warning("MGR_CLEANUP: Thread did not quit gracefully. Terminating.")
                self.chatbot_thread.terminate() # Force stop
                self.chatbot_thread.wait(200) # Wait for termination
        ...
        if self.chatbot_worker:
            ...
            # Safe disconnect: Check if connected before disconnecting
            try: self.chatbot_worker.responseReady.disconnect(self._handle_worker_response)
            except (TypeError, RuntimeError): pass # Signal might not be connected or already disconnected
            ...
            self.chatbot_worker.deleteLater() # Ensure Qt cleans up the QObject
    ...
    def set_api_key(self, api_key: str | None):
        ...
        # Condition to re-create worker:
        # 1. Key changed (old != new)
        # 2. Key is set AND (worker doesn't exist OR thread not running)
        if old_api_key_val != self.api_key or \
           (self.api_key and (not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning())):
            ...
            if self.api_key:
             ...
             # If worker exists and key is re-confirmed (e.g. same key entered again), ensure worker knows.
        ...
        elif not self.api_key: # Key was cleared and was already None, or some other edge case.
    ...
    def _setup_worker(self):
        ...
        # Defensive cleanup if something went wrong previously
        ...
        self.chatbot_thread = QThread(self) # Parent to self for lifetime management with manager
        ...
        self.chatbot_worker.errorOccurred.connect(self.errorOccurred) # Direct connection
        self.chatbot_worker.statusUpdate.connect(self.statusUpdate) # Direct connection

        self.chatbot_thread.start()
        logger.info("MGR_SETUP_WORKER: New AI Chatbot worker thread started.")
        self.statusUpdate.emit("Status: AI Assistant Ready.") # Initial status after setup

    def _validate_fsm_schema(self, data: dict) -> tuple[bool, list[str]]:
        """Validates the FSM data structure against the expected schema."""
        errors = []
        if not isinstance(data, dict):
            errors.append("AI Response: Root must be a JSON object.")
            return False, errors
        
        # Check for mandatory 'description' key
        ...
        # Validate 'states'
        state_names = set() # Collect valid state names for transition validation
        if 'states' not in data or not isinstance(data['states'], list):
            ...
            for i, s_obj in enumerate(data['states']):
                ...
                if not s_name or not isinstance(s_name, str) or not s_name.strip():
                    ...
                    state_names.add(s_name) # Add to set of valid names
                ...
                if 'action_language' in s_obj and not isinstance(s_obj['action_language'], str): # Check action_language
                ...
                elif 'sub_fsm_data' in s_obj: # If sub_fsm_data exists, validate its basic structure
        ...
        if 'transitions' not in data or not isinstance(data['transitions'], list):
            ...
            for i, t_obj in enumerate(data['transitions']):
                ...
                elif src not in state_names: # Check against collected valid state names
                ...
                elif tgt not in state_names: # Check against collected valid state names
                ...
                if 'action_language' in t_obj and not isinstance(t_obj['action_language'], str): # Check action_language
        ...
        if 'comments' in data and not isinstance(data['comments'], list):
            ...
            for i, c_obj in enumerate(data['comments']):
                ...
                # Optional layout hints for comments
        ...
        if was_fsm_generation_attempt:
            ...
            try:
                ...
                if match:
                    ...
                    # It's also possible the JSON is just not wrapped in markdown.
                    # We also need to check if the response starts with extraneous text before a JSON object.
                    # Try to find the first '{' that likely starts the JSON object.
                ...
                if is_valid_schema:
                    ...
                    # Status update: Handled by worker's finally block or specific error handlers.
                    return # Successfully handled as FSM
                else:
                    ...
                    # Fall through to plainResponseReady to display the (invalid) JSON as text
                ...
                # Fall through to plainResponseReady to display the original non-JSON response
            # Fall through intended for cases where FSM generation attempt failed parsing/validation

        # If it wasn't an FSM attempt, or if FSM attempt failed validation/parsing, treat as plain response.
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        if not self.api_key:
            ...
            if self.parent_window and hasattr(self.parent_window, 'ai_chat_ui_manager') and self.parent_window.ai_chat_ui_manager:
                # Accessing _append_to_chat_display directly is okay for system messages from within the manager's scope
        ...
        if not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning():
            ...
            if self.api_key and (not self.chatbot_thread or not self.chatbot_thread.isRunning()):
                 ...
                 self._setup_worker() # This will also cleanup if necessary
        ...
        if is_fsm_gen_specific:
            ...
            self.last_fsm_request_description = None # Clear if not an FSM specific generation
        ...
        if self.parent_window and hasattr(self.parent_window, 'scene') and hasattr(self.parent_window.scene, 'get_diagram_data'):
            try:
                ...
                diagram_json_str = json.dumps({"error": "Could not retrieve diagram context."}) # Send error as JSON
        else:
             # If parent_window or scene is not available, send a specific error message.
        ...
        if self.chatbot_worker: # Should be true after checks and potential setup
            effective_diagram_json_str = diagram_json_str if diagram_json_str is not None else ""
            # Invoke methods on the worker thread
            ...
            # Update status in main window UI via its manager
            if self.parent_window and hasattr(self.parent_window, 'ai_chat_ui_manager') and self.parent_window.ai_chat_ui_manager:
                self.parent_window.ai_chat_ui_manager.update_status_display("Status: Sending to AI...")
        else: # Should ideally not be reached if checks above are correct
    ...
    def set_online_status(self, is_online: bool):
        ...
        if self.api_key:
            if is_online:
                ...
                # If we are online and have a key, ensure worker is running
                if not self.chatbot_thread or not self.chatbot_thread.isRunning():
                    ...
                    self._setup_worker() # This also handles cleanup if needed
            else:
                self.statusUpdate.emit("Status: Offline. Gemini AI features unavailable.")
                # Optionally, could stop the worker thread if offline to save resources,
                # but it might be simpler to let it stay and fail API calls if attempted.
                # For now, just update status. If worker tries to call API, it will fail.
        else: # No API Key
            if is_online:
                self.statusUpdate.emit("Status: Online, Gemini API Key required.")
            else: # Offline and no API key
