...
class ChatbotWorker(QObject):
    ...
    def __init__(self, api_key, model_name="gemini-2.5-flash-latest", parent=None):
    ...
    def process_message_slot(self, user_message: str, force_fsm_generation: bool):
        ...
        if system_prompt_content:
            ...
            # Optional dummy model response if needed for strict alternating roles.
            # For Gemini with generate_content, this is often not strictly necessary.
            # Test without it first. If issues arise, add:
            # api_contents.append({"role": "model", "parts": [{"text": "Understood."}]})
