...
import re
import logging
from PyQt5.QtCore import QTime, QTimer, QPointF, QMetaObject, QFile, QTemporaryFile, QDir, QIODevice, QFileInfo # Added QFileInfo
...
# Updated dialogs import to include FindItemDialog
...
from PyQt5.QtCore import (
    Qt, QRectF, QPointF, QMimeData, QPoint, QLineF, QObject, pyqtSignal, QThread,
    QEvent, QSize, QUrl,
    QSaveFile,
    pyqtSlot
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.scene.selectionChanged.connect(self._update_properties_dock) # Connect here
        ...
        self.find_item_dialog: FindItemDialog | None = None # For Find Item dialog
    ...
    def _create_actions(self):
        ...
        # Find Item Action (already defined above)
    ...
    def _create_menus(self):
        ...
        edit_menu.addSeparator()
        edit_menu.addAction(self.find_item_action) # Add Find Item to Edit menu
        edit_menu.addSeparator()
    ...
    def _create_toolbars(self):
        ...
        edit_toolbar.addAction(self.find_item_action) # Add Find to Edit Toolbar
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        for i, state_data in enumerate(fsm_data.get('states', [])):
            ...
            try:
                ...
                # Connect signals for FindItemDialog updates if state is created here
                if self.find_item_dialog and hasattr(state_item, 'signals'):
        ...
        if items_to_add_for_undo_command:
            ...
            else: # If clearing diagram, AddItemCommand handles dirty state itself
                for item_to_add in items_to_add_for_undo_command:
                     self.undo_stack.push(AddItemCommand(self.scene, item_to_add, f"Add AI {type(item_to_add).__name__}"))


            logger.info("MW: Added %d AI-generated items to diagram.", len(items_to_add_for_undo_command))
            self.scene.set_dirty(True) # Ensure dirty is set even if macro isn't used for clear_current
        ...
        if self.find_item_dialog and not self.find_item_dialog.isHidden(): # Refresh find dialog if open
    ...
    def on_new_file(self, silent=False):
        ...
        self.view.centerOn(self.scene.sceneRect().center())
        if hasattr(self, 'select_mode_action'): self.select_mode_action.trigger()
        if self.find_item_dialog and not self.find_item_dialog.isHidden(): # Refresh find dialog
            self.find_item_dialog.refresh_list()
        return True
    ...
    def _load_from_path(self, file_path):
        try:
            ...
            # --- Connect State Name Change Signals During Load ---
            temp_state_items_map = {} # Used temporarily during load
            
            # First pass: Create states and connect signals
            for state_data in data.get('states', []):
                state_item = GraphicsStateItem( # Assume constructor is defined elsewhere
                    state_data['x'], state_data['y'],
                    state_data.get('width', 120), state_data.get('height', 60),
                    state_data['name'],
                    # ... other properties ...
                )
                # Before adding to scene, connect the signal
                if hasattr(state_item, 'signals') and hasattr(state_item.signals, 'textChangedViaInlineEdit'):
                    try: state_item.signals.textChangedViaInlineEdit.disconnect(self._handle_state_renamed_inline)
                    except TypeError: pass # Was not connected
                    state_item.signals.textChangedViaInlineEdit.connect(self._handle_state_renamed_inline)
                
                # self.scene.addItem(state_item) # This would be part of scene.load_diagram_data
                temp_state_items_map[state_data['name']] = state_item # Store for transition linking

            # Now call the main load_diagram_data which internally adds items
            self.scene.load_diagram_data(data) # This will create new items
            
            # After scene.load_diagram_data, items are new instances. Re-connect signals.
            for item in self.scene.items():
                if isinstance(item, GraphicsStateItem):
                    if hasattr(item, 'signals') and hasattr(item.signals, 'textChangedViaInlineEdit'):
                        try: item.signals.textChangedViaInlineEdit.disconnect(self._handle_state_renamed_inline)
                        except TypeError: pass
                        item.signals.textChangedViaInlineEdit.connect(self._handle_state_renamed_inline)
    ...
    def on_show_find_item_dialog(self):
        if not self.find_item_dialog:
            ...
            self.find_item_dialog.item_selected_for_focus.connect(self.focus_on_state_by_name)
            # Connect scene modification signals if available and FindItemDialog is kept open
            # e.g., self.scene.itemAdded.connect(self.find_item_dialog.refresh_list)
            # For now, refresh_list is called when dialog is shown.
    ...
    def focus_on_state_by_name(self, state_name: str):
        ...
        if item_to_focus and isinstance(item_to_focus, GraphicsStateItem):
            ...
            if self.find_item_dialog and not self.find_item_dialog.isHidden():
                 self.find_item_dialog.hide() # Optionally hide after selection
        else:
    ...
    def _handle_state_renamed_inline(self, old_name: str, new_name: str):
        # This slot is connected to each GraphicsStateItem's textChangedViaInlineEdit signal
        logger.debug(f"MainWindow: State renamed inline from '{old_name}' to '{new_name}'.")
        if self.find_item_dialog and not self.find_item_dialog.isHidden():
            self.find_item_dialog.refresh_list()
        # Also update properties dock if the renamed item was selected
        if self.scene.selectedItems() and self.scene.selectedItems()[0].text_label == new_name:
            self._update_properties_dock()

    # Add this method to connect signals for newly created/loaded state items
    ...
    # Modify AddItemCommand.redo() in undo_commands.py to call this:
    # ...
    ...
    # ...
    # And in DiagramScene._add_item_interactive after current_item is created and before pushing to undo_stack:
    # ...
    # if isinstance(current_item, GraphicsStateItem) and self.parent_window:
    #     self.parent_window.connect_state_item_signals(current_item)
    # ...
    # And in DiagramScene.load_diagram_data, after creating state_item:
    # ...
