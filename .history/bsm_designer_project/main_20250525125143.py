...
logger = logging.getLogger(__name__) # Define logger early if used by ResourceMonitorWorker
# --- Custom Modules ---
                ...
                if self._nvml_initialized and self._gpu_handle and PYNVML_AVAILABLE and pynvml:
                    try: gpu_util = pynvml.nvmlDeviceGetUtilizationRates(self._gpu_handle).gpu
                        ...
                        if e_nvml_util.value == pynvml.NVML_ERROR_GPU_IS_LOST or \
                           e_nvml_util.value == pynvml.NVML_ERROR_INVALID_ARGUMENT:
                            self._gpu_handle = None 
                            self._nvml_initialized = False
...
class MainWindow(QMainWindow):
    ...
    def _create_actions(self):
        ...
        def _safe_get_style_enum(attr_name, fallback_attr_name=None): # Keep this helper
        ...
        self.openai_settings_action.triggered.connect(self.ai_chat_ui_manager.on_openai_settings) # Make sure this line exists and is correct  
        ...
        self.ask_ai_to_generate_fsm_action = QAction(
            get_standard_icon(QStyle.SP_ArrowRight, "AIGen"), # Using an arrow to suggest sending the description
        ...
        print(f"DEBUG MW: self.openai_settings_action created: {self.openai_settings_action}")
        print(f"DEBUG MW: self.clear_ai_chat_action created: {self.clear_ai_chat_action}")
        print(f"DEBUG MW: self.ask_ai_to_generate_fsm_action created: {self.ask_ai_to_generate_fsm_action}")
