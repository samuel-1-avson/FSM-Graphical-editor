from PyQt5.QtCore import (
    QObject, pyqtSignal, QThread, QTime, QTimer, Qt, 
    QMetaObject, pyqtSlot, QGenericArgument # Import QGenericArgument
)
import openai
import json
import re 
...
class ChatbotWorker(QObject):
    ...
    def __init__(self, api_key, model_name="gpt-3.5-turbo", parent=None):
        ...
        self.current_diagram_context_json_str = None # This can be str or None
    ...
    # Keep the slot signature flexible with Python type hints
    # but pyqtSlot might be more specific. Let's try object for diagram_json_str
    # to see if it gives more flexibility for None.
    # Or, we could make two slots if absolutely necessary, one for str, one for no-arg to clear.
    # For now, let's try to make the Python type hint match what we send via QGenericArgument.
    @pyqtSlot(str) 
    def set_current_diagram_context(self, diagram_json_str: str): 
        # This slot now expects a string. If it was None, an empty string "" will be passed.
        ...
        if old_api_key_val != self.api_key or (self.api_key and (not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning())):
            ...
            if self.api_key:
             ...
             QMetaObject.invokeMethod(self.chatbot_worker, "set_api_key", Qt.QueuedConnection, 
                                      QGenericArgument("QString", self.api_key)) 
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        if self.chatbot_worker: 
            ...
            QMetaObject.invokeMethod(self.chatbot_worker, "set_current_diagram_context", Qt.QueuedConnection, 
                                     QGenericArgument("QString", effective_diagram_json_str))

            QMetaObject.invokeMethod(self.chatbot_worker, "process_message", Qt.QueuedConnection,
                                     QGenericArgument("QString", user_message_text), 
                                     QGenericArgument("bool", is_fsm_gen_specific))
