...
class CodeEditor(QPlainTextEdit):
    ...
    def set_language(self, language: str):
        ...
        if self.current_highlighter:
            ...
            # For QPlainTextEdit, clearing formatting is slightly different.
            # Iterating through blocks and resetting format might be more direct if needed,
            # but for simple cases, text manipulation can work.
            # The self.setPlainText("") then self.setPlainText(text) approach
            # might have side effects like clearing undo stack if the document
            # is complex or part of a larger undo system.
            # For this CodeEditor as used in dialogs, clearing undo is likely fine.
            cursor = self.textCursor()
            cursor.select(QTextCursor.Document)
            default_format = QTextCharFormat() # Create a default format
            # Reset font potentially if syntax highlighter changed it
            editor_font_size = 10 
            try:
                editor_font_size_val = int(APP_FONT_SIZE_EDITOR.replace("pt", ""))
                if 8 <= editor_font_size_val <= 24: editor_font_size = editor_font_size_val
            except ValueError: pass
            default_font = QFont("Consolas, 'Courier New', monospace", editor_font_size)
            default_font.setStyleHint(QFont.Monospace)
            default_format.setFont(default_font)
            default_format.setForeground(QColor(COLOR_TEXT_EDITOR_DARK_PRIMARY)) # Ensure text color resets
            cursor.setCharFormat(default_format)
            cursor.clearSelection()
            # The above manual format reset is more targeted than clearing all text.
            # If doc.clearUndoRedoStacks() is specifically desired for switching to plain text:
            # text = self.toPlainText()
            # self.setPlainText("")
...
class PythonHighlighter(QSyntaxHighlighter):
    def __init__(self, parent=None): 
        ...
        operators_regex = (
            r"(\+|\-|\*|/|%|=|==|!=|<|>|<=|>=|&|\||\^|~|<<|>>|"
            r"\bnot\b|\band\b|\bor\b|\bis\b|\bin\b)" 
        )
