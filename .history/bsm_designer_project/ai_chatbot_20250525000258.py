...
logger = logging.getLogger(__name__) # or logging.getLogger("bsm_designer.ai_chatbot")
...
class ChatbotWorker(QObject):
    ...
    def set_api_key(self, api_key: str):
        logger.info(f"set_api_key called (new key {'SET' if api_key else 'NOT SET'}).")
        self.api_key = api_key
        self._initialize_client()

    @pyqtSlot(str)
    def set_current_diagram_context(self, diagram_json_str: str | None):
        logger.debug(f"Setting diagram context. Length: {len(diagram_json_str) if diagram_json_str else 'None'}")
        self.current_diagram_context_json_str = diagram_json_str

    @pyqtSlot(str, bool)
    def process_message(self, user_message: str, force_fsm_generation: bool = False):
        logger.info(f"WORKER_PROCESS: process_message CALLED for: '{user_message[:50]}...' (force_fsm_generation={force_fsm_generation})")
        ...
        if self.current_diagram_context_json_str:
            try:
                if self.current_diagram_context_json_str is not None: # Check for None explicitly
                    ...
                    system_prompt_content += " No diagram context was provided for this request (context string was None)."
        ...
        history_context_limit = -6 # Keep last 3 user/assistant pairs
...
class AIChatbotManager(QObject):
    ...
    def set_api_key(self, api_key: str | None):
        ...
        old_api_key_val = self.api_key # Store actual old value for comparison
        self.api_key = api_key
        ...
        if old_api_key_val != self.api_key or (self.api_key and (not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning())):
            ...
            if self.api_key:
             ...
             # Use QMetaObject.invokeMethod for thread-safe call if worker is on another thread
    ...
    def _setup_worker(self):
        ...
        self.chatbot_thread = QThread(self) # Parent to manager for lifecycle
        self.chatbot_worker = ChatbotWorker(self.api_key) # No parent, will be moved
        ...
        self.chatbot_worker.errorOccurred.connect(self.errorOccurred) # Forward directly
        self.chatbot_worker.statusUpdate.connect(self.statusUpdate) # Forward directly
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        if not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning():
            ...
            if self.api_key and (not self.chatbot_thread or not self.chatbot_thread.isRunning()):
                 ...
                 self._setup_worker() # This will re-create worker and thread if API key is present
        ...
        if is_fsm_gen_specific:
            ...
            self.last_fsm_request_description = None # Clear for general messages
        ...
        # Use QMetaObject.invokeMethod for thread-safe calls
        if diagram_json_str is not None:
            QMetaObject.invokeMethod(self.chatbot_worker, "set_current_diagram_context", Qt.QueuedConnection, 
                                     Q_ARG(str, diagram_json_str))

        QMetaObject.invokeMethod(self.chatbot_worker, "process_message", Qt.QueuedConnection,
                                 Q_ARG(str, user_message_text), Q_ARG(bool, is_fsm_gen_specific))

        logger.debug("MGR_PREP_SEND: set_current_diagram_context and process_message calls queued for worker.")
    ...
    def set_online_status(self, is_online: bool):
        ...
        if self.api_key:
            if is_online:
                ...
                # If worker isn't running but we have a key and are online, try to set it up
                ...
                # Optionally, you could stop the worker thread here if offline to save resources,
                # but it might try to restart when online again. Current logic is okay.
        else: # No API Key
            if is_online:
