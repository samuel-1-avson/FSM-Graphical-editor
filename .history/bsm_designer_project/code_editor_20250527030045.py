...
class CodeEditor(QPlainTextEdit):
    ...
    def insert_completion(self, completion):
        tc = self.textCursor()
        # Calculate how many characters of the prefix are already typed
        prefix_len = len(self.completer.completionPrefix())
        # Insert only the remaining part of the completion
        tc.insertText(completion[prefix_len:])
        self.setTextCursor(tc)
        self.completer.popup().hide() # Hide after insertion
    ...
    def set_language(self, language: str):
        ...
        if self.completer: # Detach previous completer
            self.completer.setWidget(None)
            self.completer = None
    ...
    def textUnderCursor(self) -> str:
        tc = self.textCursor()
        # Select from the start of the current word to the cursor position
        # This makes it behave more like a prefix for completion
        ...
        tc.setPosition(tc.block().position() + start_of_word)
        tc.movePosition(QTextCursor.Right, QTextCursor.KeepAnchor, pos_in_block - start_of_word)
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if self.completer and self.completer.popup().isVisible():
            if event.key() in [Qt.Key_Enter, Qt.Key_Return, Qt.Key_Escape, Qt.Key_Tab, Qt.Key_Backtab]:
                event.ignore() 
                return 
            # Arrow keys are handled by QCompleter if popup is visible. If not ignored, they move the cursor.
            # We only want to pass them to super if the completer is not handling them.
            # QCompleter's event filter usually handles this. If not, we might need explicit check.

        super().keyPressEvent(event) 

        if self.completer:
            ctrlOrShift = event.modifiers() & (Qt.ControlModifier | Qt.ShiftModifier)
            # Don't complete on space, or if popup is already visible and an ignored key was pressed
            if event.text() and not event.text().isspace() and not ctrlOrShift:
                ...
                if len(prefix) >= 2: 
                    ...
                    # Check if there are any completions for the current prefix
                    # This check is important because model.rowCount() can be slow on very large models
                    # For static model, it's fine. For dynamic, might need optimization.
                    if self.completer.model().rowCount(self.completer.popup().currentIndex()) > 0 :
                        popup = self.completer.popup()
                        # popup.setCurrentIndex(self.completer.completionModel().index(0,0)) # Reset to first item
                        cr = self.cursorRect()
                    ...
                    self.completer.popup().hide() 
            elif event.key() == Qt.Key_Backspace or event.key() == Qt.Key_Delete: # Hide on backspace/delete
                 self.completer.popup().hide()
