...
from config import (
    ...
    COLOR_DRAGGABLE_BUTTON_PRESSED_BG, APP_FONT_SIZE_SMALL, APP_FONT_SIZE_STANDARD, # Added APP_FONT_SIZE_STANDARD
...
class DraggableToolButton(QPushButton):
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        bg_color = QColor(COLOR_DRAGGABLE_BUTTON_HOVER_BG) 
        border_color_qcolor = QColor(COLOR_DRAGGABLE_BUTTON_HOVER_BORDER)

        painter.setBrush(bg_color)
        painter.setPen(QPen(border_color_qcolor, 1.8)) 
        ...
        if not icon_pixmap.isNull():
            ...
            text_x_offset += icon_pixmap.width() + 10 
...
class MainWindow(QMainWindow):
    ...
    def _setup_ide_dock_widget(self):
        ...
        ide_toolbar.addWidget(QLabel(" Language: ")) # Add some padding around "Language:" using QSS on QToolBar or by inserting spacers
        ide_language_combo_container = QWidget() # Container to manage spacing for combobox
        ide_language_combo_layout = QHBoxLayout(ide_language_combo_container)
        ide_language_combo_layout.setContentsMargins(2,0,2,0) # Minimal margins
    ...
    def _update_resource_display(self, cpu_usage, ram_usage, gpu_util, gpu_name):
        ...
        if hasattr(self, 'gpu_status_label'):
            ...
            elif PYNVML_AVAILABLE and self.resource_monitor_worker and self.resource_monitor_worker._nvml_initialized: # Check worker here too
    ...
    def _update_properties_dock(self):
        ...
        font_family_css = f"font-family: '{APP_FONT_FAMILY.split(',')[0].strip()}', sans-serif;" # Use first font in family
        base_font_size_css = f"font-size: {APP_FONT_SIZE_STANDARD};"
        ...
        if len(selected_items) == 1:
            ...
            def fmt(txt, max_chars=30, is_code=False): # Increased max_chars
                if not txt: return f"<i style='color:{COLOR_TEXT_SECONDARY}; {small_font_size_css}'>(none)</i>"
                txt_str = str(txt)
                first_line = txt_str.split('\n')[0]
                
                if is_code:
                    # For code, use a monospaced font and a specific style
                    style = f"font-family: Consolas, 'Courier New', monospace; {small_font_size_css} background-color: {QColor(COLOR_BACKGROUND_MEDIUM).lighter(105).name()}; padding: 1px 3px; border-radius: 2px;"
                else:
                    style = small_font_size_css # Use small font for property values by default
                
                escaped_first_line = html.escape(first_line)
                ellipsis = "&hellip;" if len(first_line) > max_chars or '\n' in txt_str else ""
                return f"<span style='{style}'>{escaped_first_line[:max_chars]}{ellipsis}</span>"
            
            def bool_fmt(val):
                 color = COLOR_ACCENT_SUCCESS if val else QColor(COLOR_TEXT_SECONDARY).darker(110).name()
                 text = "Yes" if val else "No"
                 return f"<span style='color:{color}; font-weight:bold; {small_font_size_css}'>{text}</span>"


            rows = ""
            table_style = f"width:100%; border-collapse:collapse; {base_font_size_css} {font_family_css}"
            th_style = f"padding:5px 8px; text-align:left; font-weight:bold; color:{COLOR_TEXT_PRIMARY}; background-color:{QColor(COLOR_BACKGROUND_MEDIUM).lighter(103).name()}; border-bottom:1px solid {COLOR_BORDER_MEDIUM};"
            ...
            td_val_style = f"padding:5px 8px; border-bottom:1px solid {COLOR_BORDER_LIGHT}; vertical-align:top;"
            ...
            if isinstance(item, GraphicsStateItem):
                ...
                rows += f"<tr><td style='{td_key_style}'>Action Lang:</td><td style='{td_val_style}'><span style='font-family:monospace; font-size:{APP_FONT_SIZE_SMALL}'>{html.escape(props.get('action_language','N/A'))}</span></td></tr>"
                for act_key in ['entry_action', 'during_action', 'exit_action']:
                    act_label = act_key.replace('_action','').capitalize()
                    rows += f"<tr><td style='{td_key_style}'>{act_label}:</td><td style='{td_val_style}'>{fmt(props.get(act_key, ''), 40, is_code=True)}</td></tr>"
                ...
                if props.get('condition'): label_parts.append(f"<span style='font-family:monospace; color:{COLOR_TEXT_SECONDARY};'>[{html.escape(props.get('condition'))}]</span>")
                if props.get('action'): label_parts.append(f"<span style='font-family:monospace;color:{QColor(COLOR_ACCENT_SECONDARY).darker(110).name()};'>/{{{fmt(props.get('action'),20,is_code=True)}}}</span>")
                full_label = " ".join(p for p in label_parts if p) or f"<i style='color:{COLOR_TEXT_SECONDARY}; {small_font_size_css}'>(No Label)</i>"

                rows += f"<tr><td style='{td_key_style}'>Label:</td><td style='{td_val_style}{small_font_size_css}'>{full_label}</td></tr>"
            ...
            html_content = f"<div style='{font_family_css} {base_font_size_css} padding:10px;text-align:center;'><i><b>{len(selected_items)} items selected.</b><br><span style='{small_font_size_css} color:{COLOR_TEXT_SECONDARY};'>Select a single item to view or edit its properties.</span></i></div>"
            item_type_tooltip = f"{len(selected_items)} items"
        else:
            html_content = f"<div style='{font_family_css} {base_font_size_css} padding:10px;text-align:center;'><i>No item selected.</i><br><span style='{small_font_size_css} color:{COLOR_TEXT_SECONDARY};'>Click an item in the diagram or use the tools to add new elements.</span></div>"
