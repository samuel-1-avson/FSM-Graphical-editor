...
class DiagramScene(QGraphicsScene):
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        ...
        if item_type == "State":
            ...
            initial_dialog_props = {
                ...
                'is_superstate': initial_data.get('is_superstate', False), 
    ...
    def dropEvent(self, event: QGraphicsSceneDragDropEvent):
        ...
        if event.mimeData().hasFormat("application/x-bsm-tool"):
            ...
            elif item_type_data_str == "Superstate": # Added for potential draggable superstate
                item_to_add_type = "State"; name_prefix = "Superstate"; initial_props['is_superstate'] = True
    ...
    def load_diagram_data(self, data, is_sub_machine_load=False):
        if not is_sub_machine_load: 
            self.clear(); self.set_dirty(False) # Clear only for top-level loads
        ...
        for state_data in data.get('states', []):
            state_item = GraphicsStateItem(state_data['x'], state_data['y'], 
                                           ...
                                           sub_fsm_data=state_data.get('sub_fsm_data', None))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.fsm_view_stack = [] 
        self.current_superstate_context_item: GraphicsStateItem | None = None
        ...
        self.on_new_file(silent=True) # This will also call _update_navigation_actions
    ...
    def init_ui(self):
        ...
        self._update_navigation_actions() # Ensure nav actions are updated after UI creation
    ...
    def _create_toolbars(self):
        ...
        self.go_up_button = QToolButton(self) 
        self.go_up_button.setDefaultAction(self.go_up_action) 
        self.go_up_button.setToolButtonStyle(tb_style) 
        self.go_up_button.setIconSize(icon_size) 
        edit_toolbar.addWidget(self.go_up_button) 
    ...
    def _create_docks(self):
        ...
        # drag_super = DraggableToolButton(" Superstate", "application/x-bsm-tool", "Superstate"); drag_super.setIcon(get_standard_icon(QStyle.SP_DirIcon, "SupSt")) # Example
    ...
    def _update_properties_dock(self):
        ...
        if len(selected) == 1:
            ...
            if isinstance(item, GraphicsStateItem):
                ...
                rows += f"<tr><td><b>Superstate:</b></td><td>{'Yes' if props.get('is_superstate') else 'No'}</td></tr>"
    ...
    def log_message(self, message: str, type_hint: str = "GENERAL"):
        ...
        elif type_hint == "ERROR": formatted_log_entry += f"<span style='color:red;font-weight:bold;'><i>(ERR!)</i> {display_message}</span>"
    ...
    def enter_superstate_view(self, superstate_item: GraphicsStateItem):
        ...
        self.current_superstate_context_item = superstate_item 
    ...
    def go_up_to_parent_fsm(self):
        ...
        parent_context_to_restore = self.fsm_view_stack[-1] 
        ...
        for state_data_in_parent in parent_scene_data_to_modify.get('states', []):
            if state_data_in_parent.get('name') == parent_gsi_name_to_update:
                ...
                state_data_in_parent['is_superstate'] = True 
        ...
        self.scene.clear(); self.undo_stack.clear()
        self.scene.load_diagram_data(restored_context['scene_data'], is_sub_machine_load=True) 
        ...
        else: self.current_superstate_context_item = None
        ...
        self.setWindowModified(parent_is_dirty) 
        self._update_window_title(); self._update_navigation_actions()
    ...
    def _update_navigation_actions(self):
        ...
        if hasattr(self, 'go_up_action'): self.go_up_action.setEnabled(can_go_up)
        if hasattr(self, 'go_up_button') and self.go_up_button is not None: # Check for None
             self.go_up_button.setEnabled(can_go_up)
    ...
    def _update_window_title(self):
        ...
        title_fsm_part = f"{file_name_part} > " + " > ".join(nav_path_parts) if nav_path_parts else file_name_part
    ...
    def _update_matlab_actions_enabled_state(self):
        ...
        # For now, MATLAB ops are allowed even in sub-machine view, operating on the currently viewed FSM.
        # This might need refinement if MATLAB export should *always* use the top-level.
    ...
    def _prompt_save_if_dirty(self) -> bool:
        ...
        fsm_level_name = f"'{self.current_superstate_context_item.text_label}' (sub-machine)" if self.current_superstate_context_item else f"'{os.path.basename(self.current_file_path) if self.current_file_path else 'Untitled'}'"
        ...
        if reply == QMessageBox.Save:
            return self.on_save_file() # on_save_file now handles hierarchy
        return reply != QMessageBox.Cancel

    def _save_top_level_fsm(self) -> bool:
        if not self.fsm_view_stack: return self.on_save_file() # Should be called by on_save_file
        QMessageBox.information(self, "Saving Hierarchy", "To save changes to the main file, please navigate to the top-level FSM view using 'Go to Parent FSM' (Ctrl+Up), then save. Changes in sub-machines are saved to their parent when navigating up.")
        return False 
    ...
    def on_open_file(self):
        ...
        if file_path:
            ...
            if self._load_from_path(file_path): 
                ...
                self.undo_stack.clear(); # scene.load_diagram_data sets dirty to False
                self._update_window_title(); self._update_undo_redo_actions_enable_state()
    ...
    def on_save_file(self) -> bool:
        if self.fsm_view_stack: return self._save_top_level_fsm()
    ...
    def on_save_file_as(self) -> bool:
        ...
        if file_path:
            ...
            if self._save_to_path(file_path):
                ...
                self._update_window_title() # Dirty status handled by _save_to_path
                return True
        return False

    def _save_to_path(self, file_path) -> bool:
        if self.fsm_view_stack: 
            self.log_message("Internal Error: _save_to_path called for sub-machine. Use on_save_file.", type_hint="ERROR")
    ...
    def on_export_simulink(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            diagram_data_to_export = self.scene.get_diagram_data() 
    ...
    def _get_bundled_file_path(self, filename: str) -> str | None:
        try: base_path = sys._MEIPASS # type: ignore
            ...
            base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "examples")) # Default if not found by other relative paths
    ...
    def _open_example_file(self, example_filename: str):
        ...
        if self._load_from_path(file_path):
            self.current_file_path = file_path; self.setWindowModified(False) 
    ...
    def _update_py_simulation_actions_enabled_state(self):
        ...
        can_start_py_sim = sim_inactive and not is_matlab_running and not self.fsm_view_stack 
