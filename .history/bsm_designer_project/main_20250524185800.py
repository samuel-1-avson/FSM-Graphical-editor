...
class MainWindow(QMainWindow):
    ...
    def on_ask_ai_to_generate_fsm(self):
        ...
        if ok and description.strip():
            ...
            # Calls manager's method
            self.ai_chatbot_manager.generate_fsm_from_description(description) # <--- This should set is_fsm_gen_specific=True
    ...
    def _handle_fsm_data_from_ai(self, fsm_data: dict, source_message: str):
        ...
        clear_button = msg_box.addButton("Clear and Add", QMessageBox.AcceptRole) # Returns 0 if clicked
        add_to_existing_button = msg_box.addButton("Add to Existing", QMessageBox.AcceptRole) # Returns 1 if clicked
        cancel_button = msg_box.addButton("Cancel", QMessageBox.RejectRole) # Returns 2 if clicked
        
        msg_box.setDefaultButton(cancel_button) # Make "Cancel" the default
        
        msg_box.exec_()
        
        clicked_button = msg_box.clickedButton()
        reply = -1 # Default to an invalid reply
