...
from ai_chatbot import AIChatbotManager, AIStatus # Added AIStatus
    ...
    def _monitor_resources(self):
        ...
        while self._monitoring:
            ...
            if PYNVML_AVAILABLE and pynvml and self._nvml_initialized and not self._gpu_handle:
                ...
                except Exception as e_poll: # Catch other potential errors
                    if self._nvml_reinit_attempts < self.MAX_NVML_REINIT_ATTEMPTS + 2:
                        logger.debug(f"NVML: Unexpected error getting GPU handle: {e_poll}")
                    self._gpu_name_cache = "NVML Handle Unexp. Err"
                    self._gpu_handle = None
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # AIChatUIManager is now defined in ai_chatbot.py, so import it there if MainWindow needs direct access,
        # or rely on signals/slots from AIChatbotManager.
        # For now, assuming direct instantiation here if the class definition remains separate.
        # If AIChatUIManager is moved *into* ai_chatbot.py, this line needs adjustment or removal
        # if MainWindow doesn't directly create an instance of it.
        self.ai_chat_ui_manager = AIChatUIManager(self) # This class is now expected to be in ai_chatbot.py
        ...
        # Initial AI status based on manager's current status
        if self.ai_chat_ui_manager and self.ai_chatbot_manager:
            ...
            # Try to get a more specific initial text if possible
    ...
    def _create_actions(self):
        ...
        self.openai_settings_action = QAction(get_standard_icon(QStyle.SP_ComputerIcon, "AISet"), "AI Assistant Settings (Gemini)...", self) # Name updated in UIManager logic
    ...
    def _prompt_save_if_dirty(self) -> bool:
        if not self.isWindowModified(): # This now considers IDE dirty state via _update_window_title
            return True
    ...
    def on_new_file(self, silent=False):
        ...
        self.scene.set_dirty(False) # Diagram is no longer dirty
        # If IDE was dirty, it remains so until explicitly saved/cleared.
        # The main window modified status will update based on both.
        self.setWindowModified(self.ide_editor_is_dirty) # Update based on IDE state too
        self._update_window_title()
        self._update_undo_redo_actions_enable_state()
        if not silent:
    ...
    def on_open_file(self):
        if not self._prompt_save_if_dirty(): # This now considers IDE dirty state
            return
        if self.py_sim_ui_manager:
            self.py_sim_ui_manager.on_stop_py_simulation(silent=True)
        ...
        if file_path:
            if self._load_from_path(file_path):
                ...
                self.undo_stack.clear()
                self.scene.set_dirty(False) # Diagram is no longer dirty
                # If IDE was dirty, it remains so. Main window modified status updates.
    ...
    def on_save_file(self) -> bool:
        ...
        # Check if only diagram is dirty or if IDE is also dirty
        # If IDE is dirty, it should be saved via its own mechanism if we want separate dirty flags
        # For now, main save saves the diagram.
        if self.isWindowModified() and not self.ide_editor_is_dirty: # Only diagram is dirty
            return self._save_to_path(self.current_file_path)
        elif self.isWindowModified() and self.ide_editor_is_dirty: # Both dirty
            # Decide save order or prompt. For now, just save diagram.
            # User has to save IDE script separately if they want.
            return self._save_to_path(self.current_file_path)
        elif not self.isWindowModified() and self.ide_editor_is_dirty: # Only IDE is dirty
            # Main diagram isn't dirty, so this save op is for diagram.
            # Let user save IDE via IDE's save action.
            return True # Indicate success for diagram save (nothing to save)
        return self._save_to_path(self.current_file_path) # Default: save diagram
...
if __name__ == '__main__':
    ...
    # Apply the global stylesheet defined in config.py
    # If STYLE_SHEET_GLOBAL is empty or causes parsing errors, Qt will print a warning.
