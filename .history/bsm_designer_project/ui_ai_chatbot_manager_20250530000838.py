...
from config import ( # Ensure all necessary colors are imported
...
class AIChatUIManager(QObject):
    ...
    def update_status_display(self, status_text: str):
        ...
        is_general_error = "error" in status_text.lower() or "failed" in status_text.lower()
        is_offline = "offline" in status_text.lower()
        is_ready = "ready" in status_text.lower() and not is_key_req and not is_thinking and not is_general_error and not is_offline
        ...
        elif is_general_error: # General errors not related to API key or offline status
            self.ai_chat_status_label.setStyleSheet(f"{base_style} color: white; background-color: {COLOR_ACCENT_ERROR}; font-weight: bold;")
        elif is_thinking: 
        ...
        else: # Default/Neutral (e.g., "Chat history cleared", "Initializing...")
            self.ai_chat_status_label.setStyleSheet(f"{base_style} color: {COLOR_TEXT_SECONDARY}; background-color: {COLOR_BACKGROUND_MEDIUM};")

        can_send_message = not is_thinking and not is_key_req and not is_offline and not is_general_error
    ...
    def _append_to_chat_display(self, sender: str, message: str):
        ...
        for i, part in enumerate(parts):
            if part.startswith("```") and part.endswith("```"):
                ...
                # Handle single asterisks for italics, ensuring not to mess up **
        ...
        if sender == "System Error":
            bg_msg_color = QColor(COLOR_ACCENT_ERROR).lighter(180).name()
        elif sender == "System":
            bg_msg_color = QColor(COLOR_BACKGROUND_MEDIUM).lighter(105).name()
    ...
    def handle_ai_error(self, error_message: str):
        ...
        self.update_status_display(f"Error: {display_error}")
