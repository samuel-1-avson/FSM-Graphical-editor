...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
        ...
        self.contained_history_pseudo_states: list[GraphicsHistoryPseudoStateItem] = [] # New attribute
...
class GraphicsTransitionItem(QGraphicsPathItem):
    ...
    def __init__(self, start_item, end_item, event_str="", condition_str="", action_str="", color=None, description="", action_language=DEFAULT_EXECUTION_ENV):
        super().__init__()
        self.start_item: QGraphicsItem | None = start_item # Can be StateItem or HistoryPseudoStateItem
    ...
    def _determine_current_pen(self) -> QPen:
        if self.is_py_sim_active:
            ...
            pen_to_use = QPen(self.original_pen)
            pen_to_use.setColor(self.base_color) # Ensure color is current base
            return pen_to_use
    ...
    def get_data(self):
        start_label = "None"
        if isinstance(self.start_item, GraphicsStateItem): start_label = self.start_item.text_label
        elif isinstance(self.start_item, GraphicsHistoryPseudoStateItem): start_label = f"H:{self.start_item.parent_superstate_item.text_label if self.start_item.parent_superstate_item else 'OrphanedH'}"

        return {'source': start_label,
...
class GraphicsHistoryPseudoStateItem(QGraphicsEllipseItem):
    ...
    def __init__(self, x_relative: float, y_relative: float,
        ...
        self.setParentItem(parent_superstate_item) # Child of the superstate
        parent_superstate_item.add_history_pseudo_state(self) # Register with parent
        ...
        # Ensure scene exists before updating transition
        if self.scene():
    ...
    def update_default_history_transition(self):
        ...
        if self.default_target_substate_item and self.scene():
            ...
            transition = GraphicsTransitionItem(start_item=self, end_item=self.default_target_substate_item, color=transition_pen)
    ...
    def get_data(self) -> dict:
        ...
        # Create a unique ID for saving, as name "H" is not unique
        _unique_id_for_save = f"history_{parent_name}_{self.history_type}_{int(relative_pos.x())}_{int(relative_pos.y())}"
        return {
            'id': _unique_id_for_save,
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.ItemPositionChange and self.scene():
            if self.parent_superstate_item:
                ...
                new_rect_in_parent = self.boundingRect().translated(value)
                margin = self.RADIUS + 2
                parent_bounds = parent_rect_local_coords
                constrained_x = max(parent_bounds.left() + margin, min(value.x(), parent_bounds.right() - self.RADIUS -2)) # Adjust for item width
                constrained_y = max(parent_bounds.top() + margin, min(value.y(), parent_bounds.bottom() - self.RADIUS -2)) # Adjust for item height
            ...
            if not self.parentItem() and self.scene():
                 ...
                 self._cleanup_default_transition() # Clean its own transition
                 # The scene's RemoveItemsCommand should handle removing this item when its parent is deleted

        elif change == QGraphicsItem.ItemSceneChange: # When added/removed from scene
            if value and self.scene(): # Added to a scene
                if self.parent_superstate_item not in self.scene().items():
                ...
                self.update_default_history_transition()
            elif not value: # Removed from scene
