...
from PyQt5.QtCore import Qt, QDir, QSize, QTime, QObject, pyqtSlot # <--- ADD QObject and pyqtSlot HERE
from PyQt5.QtGui import QIcon, QColor # <--- Added QColor for COLOR_ACCENT_ERROR
...
class IDEManager(QObject): # Now QObject is defined
    ...
    def on_ide_run_python_script(self):
        ...
        try:
            ...
            if err_output:
                # Ensure COLOR_ACCENT_ERROR is a string if it's a QColor
            ...
            self.ide_output_console.append(f"<hr style='border-color:{error_color_hex}; font-size: {APP_FONT_SIZE_SMALL};'><i>Execution failed.</i></div>") # Corrected font-size for error message
        finally:
