...
class GraphicsCommentItem(QGraphicsTextItem):
    ...
    def itemChange(self, change, value):
        ...
        if change == QGraphicsItem.ItemUserType +6 and self.document(): # QGraphicsItem.ItemSceneHasChanged for when text content changed? No, it's likely through the document. Let's re-verify proper signal for text changes for geometry
            # Using internal text document content change tracking can also work.
            # ItemSceneChange, ItemMatrixChange, ItemVisibleHasChanged, ItemSelectedHasChanged, ItemEnabledHasChanged
            # A common way is to check document().isModified() or connect to document().contentsChanged()
            self.prepareGeometryChange()
        return super().itemChange(change, value)