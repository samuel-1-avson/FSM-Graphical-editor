...
class MainWindow(QMainWindow):
    ...
    def log_message(self, message: str, type_hint: str = "GENERAL"):
        ...
        if type_hint == "NETWATCH":
            ...
            formatted_log_entry += f"<span style='color:DimGray;'><i>(MATLAB)</i> {display_message}</span>"
        elif type_hint == "PYSIM_STATUS_UPDATE":
             # Specific styling for Python simulation status updates that might go to the main log
             formatted_log_entry += f"<span style='color:{QColor(100,100,180).name()} ;'><i>(PySim)</i> {display_message}</span>"
        ...
        # Update the main status_label (the one with stretch) only for general/important messages,
        # not for background chatter like periodic internet checks or MATLAB connection status updates.
