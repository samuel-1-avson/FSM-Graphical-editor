...
from PyQt5.QtCore import QDir, QFile, QIODevice, QSaveFile, QUrl, QTimer
from PyQt5.QtGui import QDesktopServices

from config import FILE_FILTER, FILE_EXTENSION
from utils import get_bundled_file_path # Assuming _get_bundled_file_path is here
...
class FileOperationsManager(QObject):
    ...
    def _open_example_file(self, filename: str):
        ...
        # Diagram dirty check happens first in MainWindow before this is called by action
        # IDE dirty check also happens in MainWindow

        # This method in FileOperationsManager expects that MainWindow has already cleared
        # the way (checked IDE dirty, checked diagram dirty with its own _prompt_save_if_dirty).
        # However, for consistency, let's ensure a diagram dirty check here as well if invoked directly.
        ...
        example_path = get_bundled_file_path(filename, resource_prefix="examples") # Use utility function
        if example_path:
            if self._load_from_path(example_path):
                ...
                self.mw.setWindowModified(False) # Directly set on main window
                self.mw._update_window_title()
                self.mw._update_undo_redo_actions_enable_state()
