...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ide_find_action: QAction | None = None # Added
        self.ide_replace_action: QAction | None = None # Added
    ...
    def _update_ai_features_enabled_state(self, is_online_and_key_present: bool):
        ...
        if hasattr(self, 'ide_analyze_action') and hasattr(self, 'ide_language_combo'):
            ...
            if not is_online_and_key_present:
                ...
                else: # Should not happen if is_online_and_key_present is false
    ...
    def _on_ide_text_changed(self):
        if not self.ide_editor_is_dirty: # Set dirty only once per change session until saved
            self.ide_editor_is_dirty = True
        # Always update save actions and title in case content becomes empty/non-empty
        self._update_ide_save_actions_enable_state() 
        self._update_window_title() 
        self._update_ide_related_actions()
    ...
    def _on_ide_language_changed(self, language: str):
        ...
        self._update_ide_related_actions() # Re-evaluates run/analyze button states
    ...
    def _update_ide_related_actions(self):
        ...
        selected_language = self.ide_language_combo.currentText() if self.ide_language_combo else "Python"
        ...
        can_analyze_with_ai = (selected_language == "Python" or selected_language.startswith("C/C++")) and \
                              ...
                              self._internet_connected # Also check internet
        
        has_text_in_ide = self.ide_code_editor is not None and bool(self.ide_code_editor.toPlainText().strip())
    ...
    def on_ide_analyze_with_ai(self):
        if not self.ide_code_editor or not self.ide_output_console:
            ...
            return
        if not self.ai_chatbot_manager or not self.ai_chatbot_manager.api_key: # Corrected
        ...
        self.ai_chat_ui_manager._append_to_chat_display("IDE", f"Requesting AI analysis for the current script ({selected_language}).") # Corrected
        self.ai_chatbot_manager.send_message(prompt) # Corrected

    def on_ide_find(self):
        if not self.ide_code_editor: return
        # We'll create the FindReplaceDialog in dialogs.py next
        # from dialogs import FindReplaceDialog # Already imported at the top
        # Check if a dialog is already open for this editor? Or allow multiple? For now, allow new.
        
        # Check if an instance already exists and is visible, if so, just activate it
        # This requires storing the dialog instance, e.g., self.find_replace_dialog
        if not hasattr(self, '_find_replace_dialog_instance') or not self._find_replace_dialog_instance or not self._find_replace_dialog_instance.isVisible():
            self._find_replace_dialog_instance = FindReplaceDialog(self.ide_code_editor, parent=self)
        else: # If it exists, make sure it's in "find" mode
            self._find_replace_dialog_instance.prepare_for_mode(replace_mode=False)

        self._find_replace_dialog_instance.show()
        self._find_replace_dialog_instance.activateWindow() # Bring to front
    ...
    def on_ide_replace(self):
        ...
        # from dialogs import FindReplaceDialog # Already imported at the top
        if not hasattr(self, '_find_replace_dialog_instance') or not self._find_replace_dialog_instance or not self._find_replace_dialog_instance.isVisible():
            self._find_replace_dialog_instance = FindReplaceDialog(self.ide_code_editor, parent=self, replace_mode=True)
        else:
            # If already visible, just ensure it's in replace mode
