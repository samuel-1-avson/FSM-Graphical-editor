...
class ResourceMonitorWorker(QObject):
    ...
    WORKER_LOOP_CHECK_INTERVAL_MS = 100 # How often to wake up and check flags/event

    def __init__(self, interval_ms=2000, parent=None):
        super().__init__(parent)
        self.data_collection_interval_ms = interval_ms # User-defined interval for emitting data
    ...
    def _monitor_resources(self):
        ...
        while self._monitoring:
            ...
            # --- Perform data collection and emit only at the user-defined interval ---
            if (current_loop_time - last_data_emit_time) >= self.data_collection_interval_ms or last_data_emit_time == 0:
                # --- NVML Init/Handle Acquisition (attempt only if needed before data collection) ---
                ...
                if PYNVML_AVAILABLE and pynvml and self._nvml_initialized and not self._gpu_handle:
                    try:
                        if pynvml.nvmlDeviceGetCount() > 0:
                            ...
                            # logger.info(f"NVML: GPU handle re-acquired for {self._gpu_name_cache} in worker loop.") # Can be noisy
                        else:
                ...
                try:
                    ...
                    if self._nvml_initialized and self._gpu_handle and PYNVML_AVAILABLE and pynvml:
                        ...
                        except Exception as e_util_other: # Catch other errors during util fetch
                            if self._nvml_reinit_attempts < self.MAX_NVML_REINIT_ATTEMPTS + 2:
            ...
            # --- Short, interruptible wait ---
            if self._stop_requested_event.wait(timeout=self.WORKER_LOOP_CHECK_INTERVAL_MS / 1000.0):
                logger.debug("ResourceMonitorWorker: Stop event was set during short wait, breaking loop.")
                break
            
            if not self._monitoring: # Final check if _monitoring was set directly
                logger.debug("ResourceMonitorWorker: _monitoring flag became false, breaking loop.")
                break
...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        worker_instance_to_shutdown_nvml = None 
        ...
        if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
            ...
            if self.resource_monitor_worker:
                worker_instance_to_shutdown_nvml = self.resource_monitor_worker 
        ...
        if worker_instance_to_shutdown_nvml: 
             worker_instance_to_shutdown_nvml._shutdown_nvml()
        elif self.resource_monitor_worker: # Fallback if thread wasn't running but worker somehow exists
