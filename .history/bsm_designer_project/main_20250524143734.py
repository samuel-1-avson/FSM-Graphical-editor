...
# ...
    ...
    # Example FSM data processing
        for state in fsm_data.get("states", []):
            item = GraphicsStateItem(state["id"], state["label"])
            self.scene.addItem(item)
    
        for transition in fsm_data.get("transitions", []):
            item = GraphicsTransitionItem(transition["from"], transition["to"])
            self.scene.addItem(item)
    
        self.scene.set_dirty(True)
        self.undo_stack.push(...)  # Add undo command, implementation depends on your setup
        self.log_message("Added FSM data to scene")
