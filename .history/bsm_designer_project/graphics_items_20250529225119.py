...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
        ...
        self._text_color = self._determine_text_color(self.base_color)
    ...
    def _determine_text_color(self, background_color: QColor) -> QColor:
        """Determines if black or white text is more readable on the given background color."""
        luminance = background_color.lightnessF()
        return QColor(COLOR_TEXT_PRIMARY) if luminance > 0.5 else QColor(COLOR_TEXT_ON_ACCENT)
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        if self.is_superstate:
            icon_size = 16  # Slightly larger for clarity
            icon_margin = 6
            # Position to top-right, slightly inset
            outer_rect_x = current_rect.right() - icon_size - icon_margin
            outer_rect_y = current_rect.top() + icon_margin
            
            painter.setPen(QPen(self.border_color.darker(150), 1.2))
            painter.setBrush(Qt.NoBrush) # No fill for the icon rectangles
            
            # Draw outer rectangle (slightly larger)
            painter.drawRect(QRectF(outer_rect_x, outer_rect_y, icon_size, icon_size))
            # Draw inner rectangle (slightly smaller and offset)
            inner_offset = 3
            painter.drawRect(QRectF(outer_rect_x + inner_offset, outer_rect_y + inner_offset,
                                    icon_size - (2*inner_offset), icon_size - (2*inner_offset)))
    ...
    def set_properties(self, name, is_initial, is_final, color_hex=None,
        ...
        if self.base_color != new_base_color:
            ...
            self._text_color = self._determine_text_color(self.base_color) # Update text color
...
class GraphicsTransitionItem(QGraphicsPathItem):
    ...
    def _get_intersection_point(self, item: QGraphicsRectItem, line: QLineF):
        ...
        for edge in edges:
            ...
            # QLineF.intersect is deprecated, use QLineF.intersects
            intersect_type, intersection_point_var = line.intersects(edge)
            if intersect_type == QLineF.BoundedIntersection: # Check if QLineF.intersects returns this or equivalent
