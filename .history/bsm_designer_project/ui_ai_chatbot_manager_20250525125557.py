...
class AIChatUIManager(QObject):
    ...
    def on_ask_ai_to_generate_fsm(self):
        print("DEBUG: AIChatUIManager.on_ask_ai_to_generate_fsm CALLED!")
    ...
    def on_clear_ai_chat_history(self):
        print("DEBUG: AIChatUIManager.on_clear_ai_chat_history CALLED!")
        logger.info("AIChatUI: on_clear_ai_chat_history CALLED!")    

    @pyqtSlot()
    def on_openai_settings(self):
        print("DEBUG: AIChatUIManager.on_openai_settings CALLED!")
        logger.info("AIChatUI: on_openai_settings CALLED!")
        logger.debug("AIChatUI: on_openai_settings triggered.") # Add a log here
        if not self.mw.ai_chatbot_manager: # Check the main window's manager
        ...
        key, ok = QInputDialog.getText(
            self.mw, # Parent for the dialog
            "OpenAI API Key",
            "Enter your OpenAI API Key (leave blank to clear):",
            QLineEdit.PasswordEchoOnEdit, # Changed from Normal to PasswordEchoOnEdit for privacy
            current_key
        )
        if ok:
            new_key_value = key.strip() # No need to check if key is None, getText returns empty string
            logger.info(f"AIChatUI: OpenAI API Key dialog returned. OK: {ok}, Key: {'SET' if new_key_value else 'EMPTY'}")
            # Pass None if the key is empty after stripping
            ...
            if new_key_value:
                ...
                self.mw.log_message("INFO","OpenAI API Key has been set/updated.")
            else:
                self.update_status_display("Status: API Key Cleared. AI Inactive.")
                self.mw.log_message("INFO","OpenAI API Key has been cleared.")
        else:
