...
# Full content for graphics_items.py with Improvement 3 applied.
...
from config import (COLOR_ITEM_STATE_DEFAULT_BG, COLOR_ITEM_STATE_DEFAULT_BORDER, APP_FONT_FAMILY,
                    ...
                    COLOR_TEXT_EDITOR_DARK_PRIMARY, COLOR_BACKGROUND_EDITOR_DARK, COLOR_ACCENT_SECONDARY) # Added COLOR_ACCENT_SECONDARY
...
class GraphicsTransitionItem(QGraphicsPathItem): 
    ...
    def set_py_sim_active_style(self, active: bool): 
        ...
        if active:
            ...
            highlight_color = QColor(COLOR_PY_SIM_STATE_ACTIVE).lighter(120) 
            if highlight_color == self.base_color:
                 highlight_color = QColor(COLOR_ACCENT_SECONDARY) 
            
            active_pen = QPen(highlight_color, self._pen_width + 1.2, Qt.SolidLine)
            self.setPen(active_pen)
        else:
    ...
    def hoverLeaveEvent(self, event: QGraphicsSceneMouseEvent): 
        if not self.is_py_sim_active: # Don't revert pen if sim highlight is active
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        current_pen_to_use = QPen(self.pen()) # Use the current pen (might be original or sim_active)

        if self.isSelected() and not self.is_py_sim_active: # Don't show selection highlight if sim active
            stroker = QPainterPathStroker(); stroker.setWidth(current_pen_to_use.widthF() + 8); stroker.setCapStyle(Qt.RoundCap); stroker.setJoinStyle(Qt.RoundJoin)
        ...
        painter.setPen(current_pen_to_use) # Apply the (possibly modified) pen
        painter.setBrush(Qt.NoBrush)
        painter.drawPath(self.path())
        
        # Arrowhead drawing (remains mostly the same, uses current_pen_to_use.color() for fill)
        ...
        painter.setBrush(current_pen_to_use.color()); painter.drawPolygon(QPolygonF([line_end_point, arrow_p1, arrow_p2]))
        
        # Label drawing (remains the same)
        current_label = self._compose_label_string()
        if current_label:
    ...
    def set_properties(self, event_str="", condition_str="", action_str="", color_hex=None, description="", offset=None, action_language=DEFAULT_EXECUTION_ENV): 
        ...
        if self.base_color != new_color: 
            ...
            # Update pen only if not actively highlighted by simulation
            if not self.is_py_sim_active:
                self.setPen(QPen(self.base_color, self._pen_width))
            self.original_pen_for_py_sim_restore = QPen(self.base_color, self._pen_width) # Keep this updated
            changed = True
...
class GraphicsCommentItem(QGraphicsTextItem): 
        ...
        if self._is_editing_inline and self._inline_editor_proxy and watched_object == self._inline_editor_proxy.widget():
            if event.type() == QEvent.FocusOut:
                ...
                if key_event.key() == Qt.Key_Escape:
                    self._inline_edit_aborted = True
                    self._inline_editor_proxy.widget().clearFocus() 
                    return True 
    ...
    def _finish_inline_edit(self, editor_widget: QTextEdit | None = None):
        ...
        if not new_text.strip() and old_text.strip(): 
             ...
             self.update()
             return
        elif not new_text.strip() and not old_text.strip(): 
            self.update()
            return
