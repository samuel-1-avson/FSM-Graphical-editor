from PyQt5.QtCore import QObject, pyqtSignal, QThread, QTime, QTimer, Qt, QMetaObject, pyqtSlot # Removed Q_ARG as it's not used
import openai
import json
...
class ChatbotWorker(QObject):
    ...
    @pyqtSlot(str)
    def set_api_key(self, api_key: str): # Ensure api_key type hint matches the call
        logger.info(f"WORKER: set_api_key called (new key {'SET' if api_key else 'NOT SET'}).")
        self.api_key = api_key
        self._initialize_client()

    @pyqtSlot(str)
    def set_current_diagram_context(self, diagram_json_str: str | None): # Ensure type hint matches
        logger.debug(f"WORKER: Setting diagram context. Length: {len(diagram_json_str) if diagram_json_str else 'None'}")
        self.current_diagram_context_json_str = diagram_json_str

    @pyqtSlot(str, bool)
    def process_message(self, user_message: str, force_fsm_generation: bool = False): # Ensure type hints match
...
class AIChatbotManager(QObject):
        ...
        if old_api_key_val != self.api_key or (self.api_key and (not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning())):
            ...
            if self.api_key:
             ...
             QMetaObject.invokeMethod(self.chatbot_worker, "set_api_key", Qt.QueuedConnection, 
                                      str(self.api_key)) # Pass argument directly
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        if self.chatbot_worker: # Check if worker exists before invoking method
            if diagram_json_str is not None:
                ...
                QMetaObject.invokeMethod(self.chatbot_worker, "set_current_diagram_context", Qt.QueuedConnection, 
                                         str(diagram_json_str if diagram_json_str is not None else ""))


            # Pass arguments directly
            QMetaObject.invokeMethod(self.chatbot_worker, "process_message", Qt.QueuedConnection,
                                     str(user_message_text), bool(is_fsm_gen_specific))
