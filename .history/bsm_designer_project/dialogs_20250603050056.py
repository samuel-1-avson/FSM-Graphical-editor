...
class StatePropertiesDialog(QDialog):
    ...
    def _choose_color(self): color = QColorDialog.getColor(self.current_color, self, "Select State Color");_choose_color(self): color = QColorDialog.getColor(self.current_color, self, "Select State Color");
    ...
    def _update_snippet_button_menu(self, button: QPushButton, target_widget, language_mode: str, snippet_category: str):
        ...
        if not snippets_dict: action = QAction(f"(No '{snippet_category}' snippets for {language_mode})", self); action.setEnabled(False); menu.addAction(action)
            ...
            for name, snippet in snippets_dict.items():
                ...
                if isinstance(target_widget, QLineEdit): action.triggered.connect(lambda checked=False, line_edit=target_widget, s=snippet: (line_edit.setText(line_edit.text()[:line_edit.cursorPosition()] + s + line_edit.text()[line_edit.cursorPosition():]), line_edit.setCursorPosition(line_edit.cursorPosition() + len(s) - (len(line_edit.text()) - line_edit.cursorPosition())))) # Corrected insert
                elif isinstance(target_widget, CodeEditor): action.triggered.connect(lambda checked=False, text_edit=target_widget, s=snippet: text_edit.insertPlainText(s + "\n"))
                menu.addAction(action)
        button.setEnabled(bool(snippets_dict))
