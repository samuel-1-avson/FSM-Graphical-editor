...
from PyQt5.QtWidgets import (
    ...
    QMainWindow, # Added QMainWindow for type hint
    QListWidget, QListWidgetItem # Added for FindItemDialog
)
from PyQt5.QtGui import QColor, QIcon, QPalette, QKeyEvent # Added QKeyEvent
from PyQt5.QtCore import Qt, QDir, QSize, QPointF, pyqtSignal # Added pyqtSignal
from PyQt5.QtCore import Qt, QDir, QSize, QPointF, pyqtSignal, QVariant # Added QVariant for item data
from graphics_items import GraphicsStateItem, GraphicsTransitionItem, GraphicsCommentItem # Ensure all are imported
from config import (
    ...
    APP_FONT_SIZE_SMALL, COLOR_TEXT_SECONDARY, COLOR_BACKGROUND_DIALOG,
    COLOR_BACKGROUND_LIGHT, COLOR_ACCENT_PRIMARY_LIGHT
)
from code_editor import CodeEditor
from utils import get_standard_icon
from matlab_integration import MatlabConnection
from graphics_items import GraphicsStateItem # Added for FindItemDialog type hint

# Ensure logger is available if needed for dialogs, though often they report via QMessageBox
import logging
logger = logging.getLogger(__name__)

try:
    from graphics_scene import DiagramScene, ZoomableView
    IMPORTS_SUCCESSFUL = True # Set to True by default
except ImportError as e:
    print(f"SubFSMEditorDialog: Could not import DiagramScene/ZoomableView: {e}. Visual sub-editor will be disabled.")
    DiagramScene = None
    ZoomableView = None
    IMPORTS_SUCCESSFUL = False # Set to False if imports fail
...
class SubFSMEditorDialog(QDialog):
    def __init__(self, sub_fsm_data_initial: dict, parent_state_name: str, parent_window_ref=None):
        ...
        if IMPORTS_SUCCESSFUL: # This global variable determines which editor to show
            ...
            self.sub_scene.set_dirty(False) # Sub-editor starts clean with loaded data
        ...
        else: # Fallback if IMPORTS_SUCCESSFUL is False
    ...
    def accept_changes(self):
        if IMPORTS_SUCCESSFUL and hasattr(self, 'sub_scene'): # Visual editor was used
        ...
        elif hasattr(self, 'json_text_edit'): # JSON Editing mode was used
            try:
                ...
                if isinstance(parsed_new_data, dict) and all(k in parsed_new_data for k in ['states', 'transitions', 'comments']):
                    ...
                    if sub_states_list_json:
                        ...
                        if not has_initial_json:
                            ...
                            if reply == set_first_btn:
                                if sub_states_list_json:
                                    sub_states_list_json[0]['is_initial'] = True # Modification happens here
                                    self.log_message("INFO", f"Set state '{sub_states_list_json[0].get('name', 'Unnamed')}' as initial in sub-machine (JSON).")
                                    # Update the editor text only if modification happened
                                    self.json_text_edit.setPlainText(json.dumps(parsed_new_data, indent=2, ensure_ascii=False)) 
                    
                    # Assign the potentially modified parsed_new_data to self.current_sub_fsm_data
...
class StatePropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_state=False, scene_ref=None):
        ...
        self.setStyleSheet(f"QDialog {{ background-color: {COLOR_BACKGROUND_DIALOG}; }} QLabel#SafetyNote {{ font-size: {APP_FONT_SIZE_SMALL}; color: {COLOR_TEXT_SECONDARY}; }} QGroupBox {{ background-color: {QColor(COLOR_BACKGROUND_LIGHT).lighter(102).name()}; }}")
        ...
        def add_field_with_note(form_layout, label_text, code_editor_widget, snippet_button):
            ...
            v_btn_container.addWidget(safety_note_label); v_btn_container.addStretch(1)
            h_editor_btn_layout.addLayout(v_btn_container)
            form_layout.addRow(label_text, h_editor_btn_layout)

        add_field_with_note(actions_layout, "Entry Action:", self.entry_action_edit, self.entry_action_snippet_btn)
        add_field_with_note(actions_layout, "During Action:", self.during_action_edit, self.during_action_snippet_btn)
        add_field_with_note(actions_layout, "Exit Action:", self.exit_action_edit, self.exit_action_snippet_btn)
    ...
    def _on_edit_sub_fsm(self):
        ...
        while dialog_parent and not isinstance(dialog_parent, QMainWindow) and hasattr(dialog_parent, 'parent') and callable(dialog_parent.parent):
            if dialog_parent.parent() == dialog_parent: break # Avoid infinite loop if parent() returns self
...
class TransitionPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_transition=False):
        ...
        def add_lineedit_with_snippet(form_layout, label_text, edit_widget, snippet_button, is_code_field=True):
            ...
            if is_code_field:
                safety_note_label = QLabel("<small><i>Note: Guard conditions are evaluated.</i></small>")
                safety_note_label.setObjectName("SafetyNote")
                safety_note_label.setToolTip("Conditions are evaluated as Python expressions. Ensure safety.")
        ...
        def add_codeeditor_with_snippet(form_layout, label_text, code_editor_widget, snippet_button):
            h_editor_btn_layout = QHBoxLayout(); h_editor_btn_layout.setSpacing(6)
            h_editor_btn_layout.addWidget(code_editor_widget, 1)
            v_btn_container = QVBoxLayout(); v_btn_container.setSpacing(3)
            v_btn_container.addWidget(snippet_button)
        ...
        add_codeeditor_with_snippet(action_form_layout, "Action:", self.action_edit, self.action_snippet_btn)
...
class FindItemDialog(QDialog):
    item_selected_for_focus = pyqtSignal(str) # Emits the name of the state to focus on

    def __init__(self, parent=None, scene_ref=None): # scene_ref will be DiagramScene
        super().__init__(parent)
        self.scene_ref = scene_ref
        self.setWindowTitle("Find State")
        self.setWindowIcon(get_standard_icon(QStyle.SP_FileDialogContentsView, "Find"))
        # Remove Qt.WindowContextHelpButtonHint if it's there, add WindowStaysOnTopHint
        ...
        self.search_label = QLabel("Search for State:")
        layout.addWidget(self.search_label)

        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText("Start typing state name...")
        ...
        self.results_list.itemActivated.connect(self._on_item_activated) # Double-click or Enter
        layout.addWidget(self.results_list)

        self._populate_initial_list() # Populate with all states initially
    ...
    def _populate_initial_list(self):
        ...
        state_names = []
        # Assuming graphics_items.GraphicsStateItem is the correct type
        for item in self.scene_ref.items():
            if isinstance(item, GraphicsStateItem): # Check correct type
                state_names.append(item.text_label)
        
        state_names.sort() # Sort alphabetically
    ...
    def _update_results_list(self):
        ...
        for item in self.scene_ref.items():
            if isinstance(item, GraphicsStateItem): # Check correct type
                if search_term in item.text_label.lower():
                    matching_names.append(item.text_label)
    ...
    def _on_item_activated(self, item: QListWidgetItem):
        if item:
            self.item_selected_for_focus.emit(item.text())
            # self.accept() # Or self.hide() if you want it to stay reusable

    def _on_return_pressed(self):
        # If input has text and list has items, activate the first one
        if self.search_input.text() and self.results_list.count() > 0:
            first_item = self.results_list.item(0)
            if first_item:
                self._on_item_activated(first_item)
        elif not self.search_input.text() and self.results_list.count() > 0:
            pass # Do nothing if search is empty

    def refresh_list(self):
        """Call this if the scene items change while the dialog is open."""
        current_search = self.search_input.text() # Preserve current search
        self._populate_initial_list() # Repopulate all
        self.search_input.setText(current_search) # Re-apply search to filter
        self._update_results_list() # Ensure filtering happens


    def keyPressEvent(self, event: QKeyEvent): # Added type hint
        if event.key() == Qt.Key_Escape:
            self.reject() # or self.hide()
        elif event.key() in (Qt.Key_Up, Qt.Key_Down) and self.results_list.count() > 0:
            self.results_list.setFocus()
            # Manually select item if none is current, or let QListWidget handle
            if self.results_list.currentRow() == -1:
                ...
                # Default QListWidget key handling for up/down
