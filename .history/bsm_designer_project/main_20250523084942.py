...
class MainWindow(QMainWindow):
    ...
    def _update_ai_chat_status(self, status_text: str):
        print(f"--- MW_UPDATE_STATUS ({QTime.currentTime().toString('hh:mm:ss.zzz')}): status: '{status_text}' ---")
        if hasattr(self, 'ai_chat_status_label'):
            self.ai_chat_status_label.setText(status_text)
            
            is_thinking = "thinking..." in status_text.lower() or "sending..." in status_text.lower()
            is_error_or_key_needed = "error" in status_text.lower() or \
                                     "failed" in status_text.lower() or \
                                     "api key required" in status_text.lower()

            if is_error_or_key_needed:
                self.ai_chat_status_label.setStyleSheet("font-size: 8pt; color: red;")
            elif is_thinking:
                 self.ai_chat_status_label.setStyleSheet("font-size: 8pt; color: grey;")
            else: # Ready, cleared, added, cancelled etc.
            ...
            can_send = not is_thinking and not ("api key required" in status_text.lower())
