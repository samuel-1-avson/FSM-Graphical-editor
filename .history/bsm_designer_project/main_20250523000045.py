    ...
    def _append_to_py_simulation_log(self, log_entries: list[str]):
        for entry in log_entries:
            ...
            if "[Condition]" in entry or "[Eval Error]" in entry or "ERROR" in entry.upper():
                cleaned_entry = f"<span style='color:{COLOR_ACCENT_SECONDARY};'>{cleaned_entry}</span>" # Use .name() for QColor
        ...
        if log_entries: # Update main status from important PySim log entries
            last_log_short = log_entries[-1].split('\n')[0][:100]
            if "Transitioned from" in last_log_short or \
               "No eligible transition" in last_log_short or \
               "ERROR" in last_log_short.upper() or \
               "Reset to state" in last_log_short or \
               "Simulation started" in last_log_short:
                self.log_message(f"PySim: {last_log_short}", is_py_sim_log_source=False) # Update main status bar
