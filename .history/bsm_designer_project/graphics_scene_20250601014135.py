...
# Full content for graphics_scene.py with Improvement 2 applied.
...
from PyQt5.QtWidgets import (
    ...
    QGraphicsSceneDragDropEvent, QApplication, QGraphicsSceneContextMenuEvent # Added QGraphicsSceneContextMenuEvent
)
from PyQt5.QtGui import QWheelEvent,QMouseEvent, QDrag, QDropEvent, QPixmap
from PyQt5.QtGui import QKeyEvent, QKeySequence, QCursor, QPainter, QColor, QPen, QBrush, QTransform # Added QTransform
from PyQt5.QtCore import Qt, QRectF, QPointF, QLineF, pyqtSignal, QPoint, QMarginsF, QEvent, QMimeData
...
class DiagramScene(QGraphicsScene):
    ...
    def log_function(self, message: str, level: str = "ERROR"): # Kept for compatibility if used elsewhere
    ...
    def _update_transitions_for_renamed_state(self, old_name:str, new_name:str):
        self._log_to_parent("INFO", f"Scene notified: State '{old_name}' changed to '{new_name}'.")
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            ...
            pass # Qt will call contextMenuEvent
        else:
            super().mousePressEvent(event)

    def contextMenuEvent(self, event: QGraphicsSceneContextMenuEvent):
        """
        Handle right-click events on the scene.
        If on an item, delegate to MainWindow's context menu.
        If on canvas background, show an "Add Element" menu.
        """
        ...
        if item_at_pos and isinstance(item_at_pos, (GraphicsStateItem, GraphicsTransitionItem, GraphicsCommentItem)):
            if self.parent_window and hasattr(self.parent_window, '_show_context_menu_for_item_from_scene'):
                ...
                event.accept()
                return
            else: # Fallback for SubFSMEditorDialog's scene or if parent_window is not MainWindow
                if hasattr(self, '_show_context_menu'): # Check if DiagramScene itself has a _show_context_menu
                    self._show_context_menu(item_at_pos, event.screenPos()) # Call local _show_context_menu
                    event.accept()
                    return
                else:
                    super().contextMenuEvent(event) # Allow default or other handlers
        elif not item_at_pos: # Click was on the canvas background
            menu = QMenu()
            ...
            # Only add AI option if parent_window is MainWindow (which has AI capabilities)
            # Check class name to differentiate from SubFSMEditorDialog which also might be parent_window
            ...
            if action == add_state_action:
                grid_x = round(click_pos.x() / self.grid_size) * self.grid_size - 60 # Adjust for default state size
            ...
            event.accept()
        else: # Click was on some other type of item, or no item and not background
    ...
    def _show_context_menu(self, item, global_pos): # This is now a local method for SubFSMEditorDialog scenes
        ...
        if isinstance(item, GraphicsStateItem) and item.is_superstate:
            pass # Could add "Edit Sub-Machine" here if needed for sub-scene context menus
    ...
    def edit_item_properties(self, item):
        ...
        if dialog_executed_and_accepted and new_props_from_dialog is not None:
            ...
            if isinstance(item, GraphicsStateItem) and old_props.get('name') != final_new_props.get('name'):
                pass 
            elif isinstance(item, GraphicsCommentItem) and old_props.get('text') != final_new_props.get('text'):
                self.scene_content_changed_for_find.emit() 
        ...
        if event.key() == Qt.Key_Delete or (event.key() == Qt.Key_Backspace and sys.platform != 'darwin'):
            ...
            if active_editor_item:
                ...
                if editor_widget:
                    ...
                    QApplication.postEvent(editor_widget, esc_event)
                self.set_mode("select")
                event.accept()
                return
    ...
    def paste_items_from_clipboard(self):
        ...
        for i, item_info in enumerate(items_to_paste_data):
            ...
            current_paste_pos = QPointF(base_paste_pos.x() + i * offset_delta.x(),
                                         base_paste_pos.y() + i * offset_delta.y())
            new_item = None
            if item_type == "State":
    ...
    def _generate_unique_state_name(self, base_name: str) -> str:
        ...
        if match:
            ...
            if base_match_num and not self.get_state_by_name(f"{base_match_num.group(1)}{int(base_match_num.group(2))+1}"):
                 prefix = base_match_num.group(1)
                 num = int(base_match_num.group(2))

        if not match and base_match_num:
             next_num_base = num + 1
             while self.get_state_by_name(f"{prefix}{next_num_base}"):
                 next_num_base += 1
             return f"{prefix}{next_num_base}"
        else:
            next_num_copy = num + 1
    ...
    def _add_template_to_scene(self, template_data: dict, drop_pos: QPointF):
        ...
        first_state_name_in_template = template_data.get('states', [{}])[0].get('name', "State")
        if self.get_state_by_name(first_state_name_in_template):
            i = 1
    ...
    # --- End FSM Template Methods ---


class ZoomableView(QGraphicsView):
    zoomChanged = pyqtSignal(float) # <<<< ADDED SIGNAL
    ...
    def __init__(self, scene, parent=None):
        ...
        self.zoom_level_steps = 0 # Renamed from zoom_level to avoid confusion with scale factor
        self.setTransformationAnchor(QGraphicsView.AnchorUnderMouse)
        self.setResizeAnchor(QGraphicsView.AnchorViewCenter)
        self._is_panning_with_space = False
        self._is_panning_with_mouse_button = False
        self._last_pan_point = QPoint()
        self._emit_current_zoom() # Emit initial zoom
        
        
        
        
    def _emit_current_zoom(self): # <<<< ADDED HELPER
        # The transform().m11() gives the horizontal scale factor
    ...
    def wheelEvent(self, event: QWheelEvent):
        if event.modifiers() & Qt.ControlModifier:
            ...
            if min_zoom_level_steps <= new_zoom_level_steps <= max_zoom_level_steps:
                ...
                self._emit_current_zoom() # <<<< EMIT SIGNAL
            event.accept()
        else:
    ...
    def zoom_in(self):
        ...
        if new_zoom_level_steps <= 15: # Max zoom steps
            self.scale(factor, factor)
            self.zoom_level_steps = new_zoom_level_steps
            self._emit_current_zoom() # <<<< EMIT SIGNAL
    ...
    def zoom_out(self):
        factor = 1 / 1.12
        new_zoom_level_steps = self.zoom_level_steps - 1
        if new_zoom_level_steps >= -15: # Min zoom steps
            self.scale(factor, factor)
            self.zoom_level_steps = new_zoom_level_steps
            self._emit_current_zoom() # <<<< EMIT SIGNAL
    ...
    def fitInView(self, rect: QRectF, aspectRadioMode: Qt.AspectRatioMode = Qt.IgnoreAspectRatio):
        ...
        self._emit_current_zoom() # <<<< EMIT SIGNAL after fitInView might change scale

    def setTransform(self, matrix: QTransform, combine: bool = False):
        super().setTransform(matrix, combine)
        self._emit_current_zoom() # <<<< EMIT SIGNAL if transform is set directly
