...
from dialogs import (StatePropertiesDialog, TransitionPropertiesDialog, CommentPropertiesDialog,
                     MatlabSettingsDialog) # SubFSMEditorDialog is also in dialogs.py
...
class DiagramScene(QGraphicsScene):
    ...
    def mouseDoubleClickEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if item_to_edit:
            # If it's a superstate and select mode, maybe "drill down" instead of properties?
            # For now, always properties on double click.
            # if isinstance(item_to_edit, GraphicsStateItem) and item_to_edit.is_superstate:
            #    self.parent_window.open_sub_fsm_editor_for_state(item_to_edit) # Hypothetical
            # else:
    ...
    def _show_context_menu(self, item, global_pos):
        ...
        # Add "Edit Sub-Machine" option for superstates
        ...
        if action == edit_action:
            ...
            # This is where we'd trigger opening the proper sub-FSM editor.
            # For now, it might just open the StatePropertiesDialog which then opens its sub-editor.
            # A more direct approach would be:
            # self.parent_window.open_sub_fsm_editor_for_state(item) # Hypothetical
            # Or, if StatePropertiesDialog is the gateway:
            self.edit_item_properties(item) # This will open StateProperties, which has the button
    ...
    def edit_item_properties(self, item):
        ...
        # Pass self.parent_window (MainWindow instance) as parent to the dialog
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        ...
        if item_type == "State":
            ...
            # Pass self.parent_window as parent
...
class MainWindow(QMainWindow):
        ...
        if not clear_current_diagram and items_to_add_for_undo_command: 
            self.undo_stack.endMacro()
             ...
             if self.undo_stack.count() > 0 and self.undo_stack.command(self.undo_stack.count() -1).childCount() == 0:
                 pass # Potential empty macro if nothing was added
    ...
    def _append_to_py_simulation_log(self, log_entries: list[str]):
        for entry in log_entries:
            ...
            elif "[Safety Check Failed]" in entry:
        ...
        if log_entries:
            ...
            important_keywords = ["Transitioned from", "No eligible transition", "ERROR", "Reset to state", "Simulation started", "Simulation stopped", "SecurityError", "Safety Check Failed"]
    ...
    def on_start_py_simulation(self):
        ...
        if not diagram_data.get('states'):
            ...
            return
        try:
            # Add warning about code execution if this is the first time or from a loaded file.
            # For simplicity here, we assume the user is aware if they authored it.
            # A more robust solution would track if a "dangerous" file was loaded.
