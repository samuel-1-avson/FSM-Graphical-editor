...
class StateMachinePoweredSimulator:
    ...
    def _create_dynamic_callback(self, code_string, callback_type="action", original_name="dynamic_callback"):
        ...
        simulator_self = self # Capture the simulator instance for use in the wrapper

        def dynamic_callback_wrapper(sm_instance_arg, *args, **kwargs_from_sm_call):
            # Use the _variables dict from the captured simulator_self instance
            ...
            try:
                ...
                if callback_type == "condition": return False
                if simulator_self._halt_simulation_on_action_error and callback_type == "action": simulator_self.simulation_halted_flag = True; raise FSMError(err_msg)
            except TypeError as e: # Specifically catch TypeError which was the issue
                if "locals must be a mapping or None" in str(e):
