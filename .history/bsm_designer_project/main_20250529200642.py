    ...
    def _update_properties_dock(self):
        ...
        if len(selected_items) == 1:
            ...
            if isinstance(item, GraphicsStateItem):
                ...
                if props.get('action'): label_parts.append(f"<span style='font-family:Consolas,monospace;color:{QColor(COLOR_ACCENT_SECONDARY).darker(110).name()};'>/{{{fmt(props.get('action'),20,is_code=True)}}}</span>")
