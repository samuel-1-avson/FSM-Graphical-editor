...
class DiagramScene(QGraphicsScene):
    ...
    # Signals for FindItemDialog updates (emitted by MainWindow based on these)
    ...
    def _update_transitions_for_renamed_state(self, old_name:str, new_name:str):
        ...
        # Visual updates are handled by _update_connected_transitions
        # Data integrity for transitions (if they stored names) is handled by EditItemPropertiesCommand
        self.scene_content_changed_for_find.emit() # Notify FindItemDialog to refresh
    ...
    def edit_item_properties(self, item):
        ...
        if dialog_executed_and_accepted and new_props_from_dialog is not None:
            ...
            # If it was a state and the name changed, emit signal for Find dialog
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
            ...
            if props_dialog.exec() == QDialog.Accepted:
                ...
                if self.get_state_by_name(final_props['name']) and final_props['name'] != default_name:
                    ...
                    if isinstance(current_item, GraphicsStateItem) and self.parent_window:
                        if hasattr(self.parent_window, 'connect_state_item_signals'):
        ...
        if current_item:
            ...
            self.undo_stack.push(cmd) # AddItemCommand handles addItem and set_dirty
            self.scene_content_changed_for_find.emit() # Notify FindItemDialog
    ...
    def delete_selected_items(self):
        ...
        if items_to_delete_with_related:
            ...
            self.undo_stack.push(cmd) # RemoveItemsCommand handles removeItem and set_dirty
            self.scene_content_changed_for_find.emit() # Notify FindItemDialog
    ...
    def dropEvent(self, event: QGraphicsSceneDragDropEvent):
        ...
        if event.mimeData().hasFormat("application/x-bsm-tool"):
            ...
            self._add_item_interactive(QPointF(grid_x, grid_y),
                                       ...
                                       initial_data=initial_props_for_add) # This will emit scene_content_changed_for_find
            event.acceptProposedAction()
        else:
    ...
    def load_diagram_data(self, data):
        self.clear() # This clears items but not necessarily signal connections on them
        ...
        for state_data in data.get('states', []):
            ...
            if self.parent_window and hasattr(self.parent_window, 'connect_state_item_signals'): # Connect signal
        ...
        self.scene_content_changed_for_find.emit() # Notify FindItemDialog after load
    ...
    def paste_items_from_clipboard(self):
        ...
        for i, item_info in enumerate(items_to_paste_data):
            ...
            if item_type == "State":
                ...
                if isinstance(new_item, GraphicsStateItem) and self.parent_window: # Connect signal for pasted state
                    if hasattr(self.parent_window, 'connect_state_item_signals'):
