...
class GraphicsStateItem(QObject, QGraphicsRectItem):
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
                 ...
                 parent_item=None, q_object_parent=None): # Added q_object_parent
        # Initialize QObject part, potentially with a QObject parent
        super(GraphicsStateItem, self).__init__(q_object_parent) # This calls QObject.__init__

        # Initialize QGraphicsRectItem part with its specific arguments
        # We cannot use super() again here in the same way for QGraphicsRectItem
        # because QGraphicsRectItem is not next in MRO from QObject for __init__ in this diamond.
        # We directly call its __init__.
    ...
    # ... (rest of GraphicsStateItem methods) ...
    ...
    def itemChange(self, change, value):
        ...
        return QGraphicsRectItem.itemChange(self, change, value) # Use QGraphicsRectItem's itemChange
...
class GraphicsTransitionItem(QObject, QGraphicsPathItem):
    ...
    def __init__(self, start_item, end_item, event_str="", condition_str="", action_str="",
                 ...
                 parent_item=None, q_object_parent=None): # Added q_object_parent
        super(GraphicsTransitionItem, self).__init__(q_object_parent) # Calls QObject.__init__
        QGraphicsPathItem.__init__(self, parent_item) # Graphics parent
    ...
    def itemChange(self, change, value): # Add this if you need to handle ItemPositionHasChanged from QObject perspective
        return QGraphicsPathItem.itemChange(self, change, value) # Use QGraphicsPathItem's itemChange
...
class GraphicsCommentItem(QGraphicsTextItem):
    ...
    def itemChange(self, change, value):
        # For QGraphicsTextItem, super().itemChange() should refer to QGraphicsTextItem.itemChange
        # This ensures proper handling of text-related changes.
        if change == QGraphicsItem.ItemPositionHasChanged and self.scene():
            if hasattr(self.scene(), 'item_moved'):
                self.scene().item_moved.emit(self)
