...
class MainWindow(QMainWindow):
    ...
    def _clear_dock_editors(self):
        while self.properties_editor_layout.rowCount() > 0:
            ...
            # row_widgets = []
            # label_widget = self.properties_editor_layout.labelForField(self.properties_editor_layout.itemAt(0, QFormLayout.FieldRole).widget()) # <--- Accessing potentially deleted widget here
            # if label_widget: row_widgets.append(label_widget)
            # field_widget = self.properties_editor_layout.itemAt(0, QFormLayout.FieldRole).widget() # <--- And here
            # if field_widget: row_widgets.append(field_widget)

            self.properties_editor_layout.removeRow(0) # This might delete the widgets

            # for w in row_widgets:
            #     if w: w.deleteLater() # <--- Then trying to delete them again here
