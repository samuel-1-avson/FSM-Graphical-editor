...
class MainWindow(QMainWindow):
    ...
    def _setup_ide_dock_widget(self):
        ...
        else: # If tabs might exist from a previous state (though unlikely with current setup)
    ...
    def on_ide_open_file(self):
        ...
        if file_path:
            ...
            if should_use_active_tab_for_open and active_tab_idx != -1:
                ...
                self.ide_tab_widget.setTabText(active_tab_idx, new_tab_title) # Update title of reused tab
            else:
    ...
    def _save_specific_ide_tab(self, index: int) -> bool:
        ...
        if not file_to_save_path: 
            ...
            default_filename = f"{current_tab_text}.py" 
            
            lang = tab_data.get("language", "Python")
            if lang == "Python": default_filename = current_tab_text.rsplit('.',1)[0] + ".py" if '.' in current_tab_text else current_tab_text + ".py"
            elif "Arduino" in lang: default_filename = current_tab_text.rsplit('.',1)[0] + ".ino" if '.' in current_tab_text else current_tab_text + ".ino"
            elif "C/C++" in lang: default_filename = current_tab_text.rsplit('.',1)[0] + ".cpp" if '.' in current_tab_text else current_tab_text + ".cpp"
            else: default_filename = current_tab_text.rsplit('.',1)[0] + ".txt" if '.' in current_tab_text else current_tab_text + ".txt"
    ...
    def on_ide_save_as_file(self) -> bool:
        ...
        if lang == "Python": default_filename = default_filename.rsplit('.',1)[0] + ".py" if '.' in default_filename else default_filename + ".py"
        elif "Arduino" in lang: default_filename = default_filename.rsplit('.',1)[0] + ".ino" if '.' in default_filename else default_filename + ".ino"
        elif "C/C++" in lang: default_filename = default_filename.rsplit('.',1)[0] + ".cpp" if '.' in default_filename else default_filename + ".cpp"
        else: default_filename = default_filename.rsplit('.',1)[0] + ".txt" if '.' in default_filename else default_filename + ".txt"
            ...
            if self.ide_tab_widget:
                for i in range(self.ide_tab_widget.count()):
                    ...
                    if hasattr(widget, "_ide_tab_data") and widget._ide_tab_data.get("is_dirty", False):
                        any_tab_dirty = True
                        break
            self.ide_save_all_action.setEnabled(any_tab_dirty)
    ...
    def on_ide_save_all_files(self):
        ...
        for i in range(self.ide_tab_widget.count()):
            ...
            if hasattr(widget, "_ide_tab_data"):
                ...
                if tab_data["is_dirty"]:
                    ...
                    # self.ide_tab_widget.setCurrentIndex(i) # This might trigger _on_ide_current_tab_changed
                    
                    if not self._save_specific_ide_tab(i): 
                        # User might have cancelled a "Save As" dialog
                        # Ask if they want to continue saving other tabs
                        ...
                        if reply == QMessageBox.No:
                            ...
                            # self.ide_tab_widget.setCurrentIndex(current_active_idx) # Restore original active tab
                            break 
                    else: 
                        saved_count +=1
                    # self.ide_tab_widget.setCurrentIndex(current_active_idx) # Restore original active tab
        ...
        self._update_ide_related_actions() # Refresh button states
