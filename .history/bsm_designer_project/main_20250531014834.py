...
from PyQt5.QtCore import QTime, QTimer, QPointF, QMetaObject, QFile, QTemporaryFile, QDir, QIODevice, QFileInfo, QEvent, QSize, QUrl, pyqtSignal, pyqtSlot, QThread, QPoint, QMimeData, QOb ject
...
from PyQt5.QtGui import (
    ...
    QDesktopServices, QWheelEvent, QMouseEvent, QCloseEvent, QFontMetrics, QPalette, QRegion, QCursor
)
...
class MainWindow(QMainWindow):
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        if clear_current_diagram:
            if not self.on_new_file(silent=True): # This will also emit scene_content_changed_for_find
        ...
        state_items_map = {}
        items_to_add_for_undo_command = [] # Will hold QGraphicsItem instances for AddItemCommand
        ...
        if items_to_add_for_undo_command:
            ...
            # self.scene.set_dirty(True) # AddItemCommand sets dirty
            QTimer.singleShot(100, self._fit_view_to_new_ai_items)
        else:
            logger.info("MW: No valid AI-generated items to add.")

        if not clear_current_diagram: # End macro only if it was started
    ...
    def _update_properties_dock(self):
        ...
        # editor_font_size_css = f"font-size: {APP_FONT_SIZE_EDITOR};" # Not used here directly
