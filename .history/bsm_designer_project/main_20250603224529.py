...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.alignment_manager = AlignmentManager(self) # <--- INSTANTIATE AlignmentManager
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        # Add AlignmentManager cleanup
        if self.alignment_manager:
            self.alignment_manager.cleanup()
            self.alignment_manager.deleteLater()
        ...
        managers_to_cleanup_and_delete = [
            self.view_manager, self.file_op_manager, self.ide_manager,
            self.matlab_op_manager, self.alignment_manager, # Added alignment_manager
            self.ai_chat_ui_manager, self.py_sim_ui_manager
        ]
        for manager in managers_to_cleanup_and_delete:
            if manager:
                ...
                manager.deleteLater()
        # Clear references after scheduling deletion
        self.view_manager = None; self.file_op_manager = None; self.ide_manager = None
        self.matlab_op_manager = None; self.alignment_manager = None
        self.ai_chat_ui_manager = None; self.py_sim_ui_manager = None
...
# ... (The following methods are likely still in MainWindow or need to be decided where they fit best)
    ...
    def focus_on_item(self, item_to_focus: QGraphicsItem):
        if item_to_focus and item_to_focus.scene() == self.scene:
            ...
            self.log_message("WARNING", f"Could not find or focus on the provided item: {item_to_focusvi}")
