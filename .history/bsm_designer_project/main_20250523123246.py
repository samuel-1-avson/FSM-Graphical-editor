...
import socket
import re
import importlib.util # For loading environment modules
import inspect # For getting module name if needed
...
from fsm_simulator import FSMSimulator, FSMError # FSMSimulator now takes environment_module
...
class DiagramScene(QGraphicsScene):
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        # Prioritize GraphicsStateItem for clicks, then others
    ...
    def drawBackground(self, painter: QPainter, rect: QRectF):
        ...
        for x in range(left, right, self.grid_size):
            if x % (self.grid_size * 5) != 0: # Don't draw minor if it's a major line
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.py_fsm_environment_module = None # For loaded Python environment script
        self.py_fsm_environment_path = None   # Path to the environment script
    ...
    def _create_actions(self):
        ...
        self.load_py_env_action = QAction(QIcon.fromTheme("document-open-folder", get_standard_icon(QStyle.SP_DirLinkIcon, "EnvPy")), "Load Python Environment...", self, statusTip="Load a Python script to act as the FSM environment", triggered=self.on_load_py_environment)
        self.clear_py_env_action = QAction(QIcon.fromTheme("edit-clear", get_standard_icon(QStyle.SP_DialogResetButton, "ClrEnv")), "Clear Python Environment", self, statusTip="Unload the current Python environment script", triggered=self.on_clear_py_environment, enabled=False)
    ...
    def _create_menus(self):
        ...
        py_sim_menu.addSeparator()
        py_sim_menu.addAction(self.load_py_env_action)
        py_sim_menu.addAction(self.clear_py_env_action)
    ...
    def _create_py_simulation_dock(self):
        ...
        self.py_sim_env_status_label = QLabel("Environment: <i>None loaded</i>")
        self.py_sim_env_status_label.setToolTip("The Python script providing environment functions/variables for the FSM.")
        self.py_sim_env_status_label.setObjectName("PySimEnvStatusLabel") # For potential styling
        env_header_layout.addWidget(self.py_sim_env_status_label)
        env_header_layout.addStretch()
        load_env_button_small = QToolButton() # Smaller button for loading
        load_env_button_small.setDefaultAction(self.load_py_env_action)
        load_env_button_small.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
        load_env_button_small.setIconSize(QSize(16,16))
        clear_env_button_small = QToolButton()
        clear_env_button_small.setDefaultAction(self.clear_py_env_action)
        clear_env_button_small.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
        clear_env_button_small.setIconSize(QSize(16,16))
        env_header_layout.addWidget(load_env_button_small)
        env_header_layout.addWidget(clear_env_button_small)
        py_sim_main_layout.addLayout(env_header_layout)
        # --- End Environment Status ---
    ...
    def _create_ai_chatbot_dock(self):
        ...
        self.ai_chat_display.setObjectName("AIChatDisplay") # For QSS
        self.ai_chat_display.setReadOnly(True)
        self.ai_chat_display.setPlaceholderText("AI chat history will appear here...")
        chatbot_main_layout.addWidget(self.ai_chat_display, 1) # Display takes most space

        input_layout = QHBoxLayout()
        self.ai_chat_input = QLineEdit()
        self.ai_chat_input.setObjectName("AIChatInput") # For QSS
        ...
        self.ai_chat_status_label.setObjectName("AIChatStatusLabel") # For QSS
        chatbot_main_layout.addWidget(self.ai_chat_status_label)

        # Initial status, will be updated by manager signals
        self._update_ai_chat_status("Status: API Key required. Configure in Settings.")
        ...
        if hasattr(self, 'view_menu'):
            self.view_menu.addSeparator() # Visually separate from other dock toggles
            ai_dock_action = self.ai_chatbot_dock.toggleViewAction()
            ai_dock_action.setText("AI Assistant Panel") # More descriptive menu text
            ai_dock_action.setIcon(get_standard_icon(QStyle.SP_ComputerIcon, "AIChat")) # Example icon
            self.view_menu.addAction(ai_dock_action)
        else:
            print("Warning: view_menu not found for AI Chatbot dock toggle.")


    # ---- AI Chatbot Interaction Methods (Copied from previous, assumed correct) ----
    ...
    def _handle_ai_response(self, ai_message_text: str):
        ...
        fsm_data_description = None
        json_match = None
        match_markdown = re.search(r"```json\s*(\{.*?\})\s*```", ai_message_text, re.DOTALL | re.IGNORECASE)
        if match_markdown:
            json_string = match_markdown.group(1).strip(); json_match = True
        else:
            ...
            try:
                ...
                if isinstance(fsm_data, dict) and "states" in fsm_data and isinstance(fsm_data["states"], list) and \
                   "transitions" in fsm_data and isinstance(fsm_data["transitions"], list):
                    self.log_message("AI response contained valid FSM JSON data.", type_hint="AI_ACTION")
                    ...
                    if reply == QMessageBox.Yes:
                        ...
                        self._append_to_ai_chat_display("AI Assistant", fsm_data_description + " (See diagram)")
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False):
        ...
        state_items_map = {}; start_x, start_y = 100, 100; items_per_row = 3
        default_item_width, default_item_height = 120, 60; padding_x, padding_y = 150, 100
        ...
        for i, state_data in enumerate(fsm_data.get('states', [])):
            ...
            pos_x = state_data.get('x', start_x + (i % items_per_row) * (default_item_width + padding_x))
            pos_y = state_data.get('y', start_y + (i // items_per_row) * (default_item_height + padding_y))
            ...
            try:
                state_item = GraphicsStateItem(pos_x, pos_y, item_w, item_h, name,
                    ...
                    color=state_data.get('properties', {}).get('color'), entry_action=state_data.get('entry_action', ""),
                    during_action=state_data.get('during_action', ""), exit_action=state_data.get('exit_action', ""),
        ...
        for trans_data in fsm_data.get('transitions', []):
            ...
            if source_item and target_item:
                try:
                    trans_item = GraphicsTransitionItem(source_item, target_item,
        ...
        for i, comment_data in enumerate(fsm_data.get('comments', [])):
            ...
            comment_x = comment_data.get('x', comment_start_x + i * (150 + 20)); comment_y = comment_data.get('y', comment_start_y)
        ...
        if items_to_add_for_undo_command:
            for item in items_to_add_for_undo_command:
                ...
                cmd_text = f"Add AI {item_type_name}"; add_cmd = AddItemCommand(self.scene, item, cmd_text)
    ...
    def _append_to_ai_chat_display(self, sender: str, message: str):
        ...
        sender_color = COLOR_ACCENT_PRIMARY if sender == "AI Assistant" else (COLOR_ACCENT_SECONDARY if sender == "You" else "#D32F2F")
    ...
    def on_openai_settings(self):
        ...
        if ok:
            self.ai_chatbot_manager.set_api_key(key.strip() or None)
            self.log_message(f"OpenAI API Key {'set/updated' if key.strip() else 'cleared'}.", type_hint="AI_CONFIG")
    ...
    def _update_properties_dock(self):
        ...
        html_content = ""; edit_enabled = False; item_type_for_tooltip = "item"
        if len(selected_items) == 1:
            item = selected_items[0]; props = item.get_data()
            item_type_name = type(item).__name__.replace("Graphics", "").replace("Item", "")
            item_type_for_tooltip = item_type_name.lower(); edit_enabled = True
            ...
            rows_html = ""
            if isinstance(item, GraphicsStateItem):
                clr = QColor(props.get('color', COLOR_ITEM_STATE_DEFAULT_BG)); tc = 'black' if clr.lightnessF()>0.5 else 'white'
                style = f"background-color:{clr.name()}; color:{tc}; padding:1px 4px; border-radius:2px;"
                rows_html += f"<tr><td><b>Name:</b></td><td>{html.escape(props['name'])}</td></tr>"
                rows_html += f"<tr><td><b>Initial:</b></td><td>{'Yes' if props.get('is_initial') else 'No'}</td></tr><tr><td><b>Final:</b></td><td>{'Yes' if props.get('is_final') else 'No'}</td></tr>"
                rows_html += f"<tr><td><b>Color:</b></td><td><span style='{style}'>{html.escape(props.get('color','N/A'))}</span></td></tr>"
                for act_key in ['entry_action', 'during_action', 'exit_action']:
                    rows_html += f"<tr><td><b>{act_key.split('_')[0].capitalize()}:</b></td><td>{format_prop_text(props.get(act_key, ''))}</td></tr>"
                if props.get('description'): rows_html += f"<tr><td colspan='2'><b>Desc:</b> {format_prop_text(props.get('description'), 50)}</td></tr>"
            elif isinstance(item, GraphicsTransitionItem):
                clr = QColor(props.get('color', COLOR_ITEM_TRANSITION_DEFAULT)); tc = 'black' if clr.lightnessF()>0.5 else 'white'
                style = f"background-color:{clr.name()}; color:{tc}; padding:1px 4px; border-radius:2px;"
                evt = html.escape(props.get('event','')) if props.get('event') else ''
                cond = f"[{html.escape(props.get('condition',''))}]" if props.get('condition') else ''
                act = f"/{{{format_prop_text(props.get('action',''), 15)}}}" if props.get('action') else ''
                lbl = " ".join(p for p in [evt, cond, act] if p) or "<i>(No Label)</i>"
                rows_html += f"<tr><td><b>Label:</b></td><td style='font-size:8pt;'>{lbl}</td></tr>"
                rows_html += f"<tr><td><b>From:</b></td><td>{html.escape(props.get('source','N/A'))}</td></tr><tr><td><b>To:</b></td><td>{html.escape(props.get('target','N/A'))}</td></tr>"
                rows_html += f"<tr><td><b>Color:</b></td><td><span style='{style}'>{html.escape(props.get('color','N/A'))}</span></td></tr>"
            ...
            html_content = f"<i><b>{len(selected_items)} items selected.</b><br>Select single item to view/edit.</i>"
        ...
        self.properties_edit_button.setToolTip(f"Edit properties of {item_type_for_tooltip}" if edit_enabled else "Select single item to enable editing")
    ...
    def log_message(self, message: str, type_hint: str = "GENERAL"):
        ...
        else: formatted_log_entry += display_message # Default
        ...
        # Update main status bar for non-special messages
        if type_hint not in ["NETWATCH", "MATLAB_CONN", "AI_CONFIG", "AIChatError", "AI_ACTION"] and "worker:" not in message :
    ...
    def _update_window_title(self):
        ...
        title += "[*]" # For setWindowModified placeholder
    ...
    # --- MATLAB Related Methods (Keep for now, can be deprecated later) ---
    def _update_matlab_status_display(self, connected, message):
        ...
        color = COLOR_PY_SIM_STATE_ACTIVE if connected else '#C62828' # Green if connected, Red if not
        self.matlab_status_label.setStyleSheet(f"font-weight: bold; padding: 0px 5px; color: {color};")
        if "Initializing" not in message and "path cleared" not in message.lower() : # Avoid logging initial or trivial updates
    ...
    def _update_matlab_actions_enabled_state(self):
        # True if MATLAB connected AND Python sim is NOT active
        ...
        self.matlab_settings_action.setEnabled(not self.py_sim_active) # Settings always enabled if PySim not running
    ...
    def _start_matlab_operation(self, operation_name):
        ...
        self.set_ui_enabled_for_matlab_op(False) # Disable UI
    ...
    def set_ui_enabled_for_matlab_op(self, enabled: bool):
        ...
        for child in self.findChildren(QToolBar): child.setEnabled(enabled) # All toolbars
        ...
        # Ensure Python sim actions are updated based on MATLAB op state
        self._update_py_simulation_actions_enabled_state()
    ...
    def _handle_matlab_modelgen_or_sim_finished(self, success, message, data):
        ...
        if success:
            if "Model generation" in self.status_label.text() and data: # Check if it was model gen
            ...
            elif "Simulation" in self.status_label.text(): # Check if it was simulation
    ...
    def _handle_matlab_codegen_finished(self, success, message, output_dir):
        ...
        if success and output_dir:
            ...
            msg_box.setText(f"Code generation completed.<br>Output directory: <a href='file:///{abs_path_output_dir}'>{abs_path_output_dir}</a>")
            ...
            if msg_box.clickedButton() == open_dir_button:
                try: QDesktopServices.openUrl(QUrl.fromLocalFile(abs_path_output_dir))
                    ...
                    self.log_message(f"Error opening directory {abs_path_output_dir}: {e}", type_hint="GENERAL")
    ...
    def on_new_file(self, silent=False):
        ...
        self.scene.set_dirty(False) # setWindowModified(False) will be called by signal
        self._update_window_title()
        self._update_undo_redo_actions_enable_state()
        if not silent:
            self.log_message("New diagram created. Ready.", type_hint="GENERAL")
    ...
    def _load_from_path(self, file_path):
        try:
            ...
            if not isinstance(data, dict) or ('states' not in data or 'transitions' not in data):
                ...
                return False
            self.scene.load_diagram_data(data) # This clears scene, loads, sets dirty=False
            return True
        except Exception as e:
    ...
    def on_save_file_as(self) -> bool:
        ...
        if file_path:
            ...
            if self._save_to_path(file_path):
                ...
                self._update_window_title() # Update title with new name
                return True
        return False # Cancelled or save failed
    ...
    def _save_to_path(self, file_path) -> bool:
        ...
        try:
            ...
            if save_file.write(json_data.encode('utf-8')) == -1: # Error during write
                error_str = save_file.errorString()
                ...
                save_file.cancelWriting() # Discard temporary file
                return False
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        self.on_stop_py_simulation(silent=True) # Stop internal sim if active
        ...
        if self._prompt_save_if_dirty(): # Check for unsaved changes
            # Log active MATLAB threads if any (they might be orphaned)
            if self.matlab_connection and hasattr(self.matlab_connection, '_active_threads') and self.matlab_connection._active_threads:
                self.log_message(f"Closing. {len(self.matlab_connection._active_threads)} MATLAB-related worker threads may still be processing.", type_hint="GENERAL")
            ...
            print(f"--- MW ({current_time}): Close cancelled by user. ---")
    ...
    def _run_internet_check_job(self):
        host_to_check = "8.8.8.8" # Google DNS
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        ...
        try: check_host_name_for_tooltip = socket.getfqdn('8.8.8.8') # For tooltip
        ...
        color = COLOR_PY_SIM_STATE_ACTIVE if is_connected else '#D32F2F' # Green if connected, Red if not
        self.internet_status_label.setStyleSheet(f"font-weight: normal; padding: 0px 5px; color: {color};")
    ...
    def _update_py_sim_status_display(self):
        ...
        # Update environment status label in PySim dock
        if hasattr(self, 'py_sim_env_status_label'):
            if self.py_fsm_environment_path:
                env_name = os.path.basename(self.py_fsm_environment_path)
                self.py_sim_env_status_label.setText(f"Environment: <b>{env_name}</b>")
            else:
                self.py_sim_env_status_label.setText("Environment: <i>None loaded</i>")


    def _update_py_simulation_actions_enabled_state(self):
        # is_matlab_op_running = self.progress_bar.isVisible() # If using MATLAB progress
        is_other_op_running = self.progress_bar.isVisible() # Generalize for any blocking op
        ...
        # Actions/widgets enabled only when sim is active AND no other blocking op
        sim_active_and_free = self.py_sim_active and not is_other_op_running
        for widget_action in [self.stop_py_sim_action, self.reset_py_sim_action]:
            widget_action.setEnabled(sim_active_and_free)
        ...
        # Environment loading actions enabled if sim is NOT active and no other op
        can_load_env = sim_inactive and not is_other_op_running
        self.load_py_env_action.setEnabled(can_load_env)
        self.clear_py_env_action.setEnabled(can_load_env and self.py_fsm_environment_module is not None)


    def set_ui_enabled_for_py_sim(self, is_sim_running: bool):
        self.py_sim_active = is_sim_running
        self._update_window_title()
        is_editable = not is_sim_running # UI is editable if sim is NOT running

        # Scene interaction mode
        if is_editable and self.scene.current_mode != "select":
            self.scene.set_mode("select") # Revert to select mode if sim stops
        elif not is_editable: # Sim is starting
            self.scene.set_mode("select") # Force select mode during simulation
        ...
        # Enable/disable general editing actions
        actions_to_toggle = [
        ...
        self._update_matlab_actions_enabled_state() # MATLAB actions depend on PySim state
        self._update_py_simulation_actions_enabled_state() # Update PySim specific actions
        self._update_py_sim_status_display() # Update status labels


    def _highlight_sim_active_state(self, state_name_to_highlight: str | None):
        # Remove highlight from previously active state
        ...
        # Add highlight to new active state
    ...
    def _update_py_simulation_dock_ui(self):
        if not self.py_fsm_engine or not self.py_sim_active:
            ...
            self._highlight_sim_active_state(None) # No highlight
            return
        ...
        variables = self.py_fsm_engine.get_variables() # This now includes env vars
    ...
    def _append_to_py_simulation_log(self, log_entries: list[str]):
        for entry in log_entries:
            ...
            # Basic color coding for log entries
            if "[Condition]" in entry or "[Eval Error]" in entry or "ERROR" in entry.upper() or "[Env Error]" in entry :
                cleaned_entry = f"<span style='color:{COLOR_ACCENT_SECONDARY};'>{cleaned_entry}</span>"
            elif "Transitioned from" in entry or "Reset to state" in entry or "Simulation started" in entry or "Environment module" in entry or "Called environment" in entry:
        ...
        # Update main status bar with important PySim log messages
        if log_entries:
            last_log_short = log_entries[-1].split('\n')[0][:100]
            important_keywords = ["Transitioned from", "No eligible transition", "ERROR", "Reset to state",
                                  "Simulation started", "Simulation stopped", "Environment module", "[Env Error]"]
            if any(keyword in last_log_short for keyword in important_keywords):
                self.log_message(f"PySim: {last_log_short}", type_hint="PYSIM_STATUS_UPDATE")


    # --- Python FSM Simulation Slots (with Environment Integration) ---
    def on_load_py_environment(self):
        if self.py_sim_active:
            QMessageBox.warning(self, "Simulation Active", "Stop the current Python simulation before loading a new environment.")
            return

        filePath, _ = QFileDialog.getOpenFileName(self, "Load Python Environment Script",
                                                  self.py_fsm_environment_path or QDir.homePath(), # Start in last dir or home
                                                  "Python Files (*.py)")
        if filePath:
            try:
                # Create a unique module name to avoid conflicts
                base_name = os.path.splitext(os.path.basename(filePath))[0]
                # Add a hash of the path to ensure uniqueness even for same-named files from different dirs
                module_name = f"fsm_custom_env_{base_name}_{abs(hash(filePath))}"

                spec = importlib.util.spec_from_file_location(module_name, filePath)
                if spec is None or spec.loader is None: # Check if spec or loader is None
                    raise ImportError(f"Could not create module spec for '{filePath}'. Ensure the file exists and is accessible.")

                env_module = importlib.util.module_from_spec(spec)
                sys.modules[module_name] = env_module # Add to sys.modules BEFORE exec_module
                spec.loader.exec_module(env_module) # Execute the module to load its content

                self.py_fsm_environment_module = env_module
                self.py_fsm_environment_path = filePath
                self.log_message(f"Python environment script loaded: {os.path.basename(filePath)}", "PYSIM_STATUS_UPDATE")
                self._update_py_sim_status_display() # Update env label
                self.clear_py_env_action.setEnabled(True)
            except Exception as e:
                self.py_fsm_environment_module = None # Clear on error
                self.py_fsm_environment_path = None
                QMessageBox.critical(self, "Environment Load Error", f"Failed to load Python environment script:\n{type(e).__name__}: {e}")
                self.log_message(f"Error loading Python environment: {e}", "PYSIM_STATUS_UPDATE")
                self._update_py_sim_status_display()
                self.clear_py_env_action.setEnabled(False)


    def on_clear_py_environment(self):
        if self.py_sim_active:
            QMessageBox.warning(self, "Simulation Active", "Stop the current Python simulation before clearing the environment.")
            return

        if self.py_fsm_environment_module:
            # Clean up: remove from sys.modules if loaded this way
            module_name_to_remove = getattr(self.py_fsm_environment_module, '__name__', None)
            if module_name_to_remove and module_name_to_remove in sys.modules:
                del sys.modules[module_name_to_remove]

        self.py_fsm_environment_module = None
        self.py_fsm_environment_path = None
        self.log_message("Python environment cleared.", "PYSIM_STATUS_UPDATE")
        self._update_py_sim_status_display() # Update env label
        self.clear_py_env_action.setEnabled(False)
    ...
    def on_start_py_simulation(self):
        if self.py_sim_active:
            ...
            return
        if self.scene.is_dirty() and QMessageBox.question(
                self, "Unsaved Changes",
                "The diagram has unsaved changes that won't be reflected in the simulation if you don't save.\n"
        ...
        if not diagram_data.get('states'):
            ...
            return
        try:
            # Pass the loaded environment module to the FSMSimulator
            self.py_fsm_engine = FSMSimulator(diagram_data['states'], diagram_data['transitions'],
                                              environment_module=self.py_fsm_environment_module)
            self.set_ui_enabled_for_py_sim(True) # This updates py_sim_active
            self.py_sim_action_log_output.clear()
            initial_log = ["Python FSM Simulation started."]
            if self.py_fsm_environment_module:
                env_name = os.path.basename(self.py_fsm_environment_path) if self.py_fsm_environment_path else "loaded module"
                initial_log.append(f"Using environment: {env_name}")
            initial_log.extend(self.py_fsm_engine.get_last_executed_actions_log()) # Get logs from FSM init (and env init)
            self._append_to_py_simulation_log(initial_log)
            self._update_py_simulation_dock_ui() # Update variable table, current state, etc.
        except Exception as e:
            ...
            self.set_ui_enabled_for_py_sim(False) # Ensure UI is reset


    def on_stop_py_simulation(self, silent=False):
        if not self.py_sim_active: return

        # Call environment cleanup if defined
        if self.py_fsm_engine and self.py_fsm_engine.env and hasattr(self.py_fsm_engine.env, 'cleanup_environment'):
            try:
                self.py_fsm_engine.env.cleanup_environment()
                # Log action via engine so it's captured if needed
                self.py_fsm_engine._log_action("Called environment.cleanup_environment().")
                if not silent: # Append any logs from cleanup to the GUI log
                    self._append_to_py_simulation_log(self.py_fsm_engine.get_last_executed_actions_log())
            except Exception as e:
                log_msg = f"[Env Error] cleanup_environment: {e}"
                if self.py_fsm_engine: self.py_fsm_engine._log_action(log_msg) # Log to engine's internal log
                if not silent: self._append_to_py_simulation_log([log_msg]) # And to GUI

        self.py_fsm_engine = None # Release the engine
        self.set_ui_enabled_for_py_sim(False) # This updates py_sim_active and UI states
        self._update_py_simulation_dock_ui() # Clear variables table, etc.
    ...
    def on_reset_py_simulation(self):
        if not self.py_fsm_engine or not self.py_sim_active:
            ...
            return
        try:
            self.py_fsm_engine.reset() # This now calls env.reset_environment if it exists
    ...
    def on_step_py_simulation(self):
        if not self.py_fsm_engine or not self.py_sim_active:
            ...
            return
        try:
            # Step FSM (this now calls env.update_environment internally first)
            _, log_entries = self.py_fsm_engine.step(event_name=None)
            self._append_to_py_simulation_log(log_entries)
            self._update_py_simulation_dock_ui() # Update variables table, current state
        except Exception as e:
            msg = f"Simulation Step Error: {type(e).__name__}: {e}"
    ...
    def on_trigger_py_event(self):
        ...
        if not event_name: # If no event name, treat as a parameterless step
            self.on_step_py_simulation()
            return

        try:
            # Step FSM with event (this now calls env.update_environment internally first)
            _, log_entries = self.py_fsm_engine.step(event_name=event_name)
            self._append_to_py_simulation_log(log_entries)
            self._update_py_simulation_dock_ui() # Update variables table, current state
            self.py_sim_event_name_edit.clear() # Clear event input field
        except Exception as e:
            msg = f"Simulation Event Error ({html.escape(event_name)}): {type(e).__name__}: {e}"
            QMessageBox.warning(self, "Simulation Event Error", str(e))
            self._append_to_py_simulation_log([f"ERROR DURING EVENT '{html.escape(event_name)}': {msg}"])
...
if __name__ == '__main__':
    ...
    # Ensure icon directory for QSS exists
