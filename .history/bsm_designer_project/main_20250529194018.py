...
class DraggableToolButton(QPushButton):
    def __init__(self, text, mime_type, item_type_data, parent=None):
        ...
        # Text is set by constructor, QSS handles font, padding, etc.
        self.setMinimumHeight(38) # Match QToolButton in toolbox
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        # Drag pixmap appearance can still be customized if needed
        # For now, relying on the default Qt drag pixmap which shows button content.
        # If a more custom pixmap is desired (like before), that code can be reinstated here.
        # Default drag pixmap will be based on current button appearance.
        
        # To make the drag pixmap more "ghostly" or distinct:
        pixmap = QPixmap(self.size())
        self.render(pixmap, QPoint(), QRegion(), QWidget.RenderFlags(QWidget.DrawChildren)) # Render current button state
        painter = QPainter(pixmap)
        painter.setCompositionMode(QPainter.CompositionMode_DestinationIn) # For transparency effect
        painter.fillRect(pixmap.rect(), QColor(0,0,0,150)) # Make it semi-transparent
        painter.end()
        
        drag.setPixmap(pixmap)
        drag.setHotSpot(event.pos()) # Hotspot relative to button, not pixmap for default rendering
...
class MainWindow(QMainWindow):
    ...
    def _create_docks(self):
        ...
        # Text changed: removed leading space as QSS will handle padding
    ...
    def _update_properties_dock(self):
        ...
        # These are now available because APP_FONT_FAMILY is correctly imported from config
    ...
    def _update_ai_features_enabled_state(self, is_online_and_key_present: bool):
        ...
        if hasattr(self, 'ide_analyze_action') and hasattr(self, 'ide_language_combo'):
            ...
            elif not (language == "Python" or language.startswith("C/C++")): # Check if 'language' var exists in this scope
    ...
    @pyqtSlot(str)
    def _on_ide_language_changed(self, language: str): # Renamed 'language_param' to 'language' to avoid potential scope issue.
        if self.ide_code_editor:
