...
from PyQt5.QtGui import QColor # Keep this if COLOR_PY_SIM_STATE_ACTIVE is a QColor object
...
COLOR_ACCENT_PRIMARY = "#1976D2" # Primary Blue
COLOR_ACCENT_PRIMARY_LIGHT = "#BBDEFB" # Lighter primary blue
COLOR_ACCENT_SECONDARY = "#FF8F00" # Accent Orange
COLOR_ACCENT_SECONDARY_LIGHT = "#FFECB3" # Lighter Accent Orange
COLOR_BORDER_LIGHT = "#CFD8DC"
COLOR_BORDER_MEDIUM = "#90A4AE"
COLOR_BORDER_DARK = "#607D8B"

COLOR_ITEM_STATE_DEFAULT_BG = "#E3F2FD" # Light blue for states
COLOR_ITEM_STATE_DEFAULT_BORDER = "#90CAF9" # Medium blue border for states
COLOR_ITEM_STATE_SELECTION = "#FFD54F" # Amber/Yellow for selection
COLOR_ITEM_TRANSITION_DEFAULT = "#009688" # Teal for transitions
COLOR_ITEM_TRANSITION_SELECTION = "#80CBC4" # Lighter Teal for transition selection
COLOR_ITEM_COMMENT_BG = "#FFFDE7" # Light yellow for comments
COLOR_ITEM_COMMENT_BORDER = "#FFF59D" # Darker yellow border for comments

COLOR_GRID_MINOR = "#ECEFF1"
COLOR_GRID_MAJOR = "#CFD8DC"

# Note: The original config had COLOR_ACCENT_PRIMARY defined twice. I'm using the first one (#1976D2).
# Also, COLOR_PY_SIM_STATE_ACTIVE was defined as a string and then as a QColor.
# For QSS, it needs to be a string. For direct QPen/QBrush use, QColor is fine.
# Let's keep the string version for QSS and convert to QColor where needed in Python code.
COLOR_PY_SIM_STATE_ACTIVE_STR = "#4CAF50" # Green string for QSS
COLOR_PY_SIM_STATE_ACTIVE = QColor(COLOR_PY_SIM_STATE_ACTIVE_STR) # QColor object for Python use
...
STYLE_SHEET_GLOBAL = f"""
    ...
    QToolBar QToolButton:checked {{
        background-color: {COLOR_ACCENT_PRIMARY};
        color: {COLOR_TEXT_ON_ACCENT};
        border: 1px solid #0D47A1; /* Darker primary for checked border */
    }}
    QMenuBar {{
        background-color: {COLOR_BACKGROUND_DARK};
        color: {COLOR_TEXT_PRIMARY};
    ...
    QLabel#StatusLabel, QLabel#MatlabStatusLabel, QLabel#PySimStatusLabel, QLabel#InternetStatusLabel {{ /* Added InternetStatusLabel */
         padding: 0px 5px;
    }}
    QDialog {{
        background-color: {COLOR_BACKGROUND_DIALOG};
    ...
    /* --- QSS for Disabled States (Moved here) --- */
    QLineEdit:disabled, QTextEdit:disabled, QSpinBox:disabled, QComboBox:disabled {{
        background-color: #F0F0F0;
        color: #A0A0A0;
        border-color: #D0D0D0;
    }}
    QToolButton:disabled {{ /* General ToolButton disabled state */
        background-color: transparent;
        color: #BDBDBD;
        border-color: transparent;
    }}
    QToolBar QToolButton:disabled {{ /* Specific for ToolButtons in ToolBar if different behavior desired */
        color: #B0B0B0; /* Example: slightly different grey */
    }}
    QCheckBox:disabled {{
        color: #A0A0A0;
    }}
    /* QCheckBox::indicator:disabled {{ border: 1px solid #D0D0D0; background-color: #F0F0F0; }} */
    ...
    QComboBox::down-arrow {{
         image: url(./dependencies/icons/arrow_down.png); /* Ensure this path is correct relative to where app runs or use Qt resource system */
    ...
    QPushButton:disabled {{
        background-color: #F5F5F5; /* Default disabled button look */
        color: #BDBDBD;
        border-color: #EEEEEE;
    }}
    ...
    QDialogButtonBox QPushButton[text="OK"], QDialogButtonBox QPushButton[text="Apply & Close"] {{
        background-color: {COLOR_ACCENT_PRIMARY};
        color: {COLOR_TEXT_ON_ACCENT};
        border-color: #0D47A1; /* Darker primary for OK button border */
    }}
    QDialogButtonBox QPushButton[text="OK"]:hover, QDialogButtonBox QPushButton[text="Apply & Close"]:hover {{
        background-color: #1E88E5; /* Lighter primary on hover */
    }}
    QGroupBox {{
        background-color: {COLOR_BACKGROUND_LIGHT};
        border: 1px solid {COLOR_BORDER_LIGHT};
        border-radius: 5px;
        margin-top: 10px; /* Space for title to sit above the border */
        padding: 10px 8px 8px 8px; /* Top padding should accommodate title */
    }}
    QGroupBox::title {{
        subcontrol-origin: margin;
        subcontrol-position: top left; /* Position title at top-left corner of margin */
    ...
    QTabBar::tab {{
        ...
        border: 1px solid {COLOR_BORDER_LIGHT};
        border-bottom: none; /* Selected tab will "connect" to pane */
    ...
    QTabBar::tab:selected {{
        background: {COLOR_BACKGROUND_DIALOG}; /* Same as pane for connection */
        color: {COLOR_TEXT_PRIMARY};
        border-color: {COLOR_BORDER_LIGHT};
        /* border-bottom-color: {COLOR_BACKGROUND_DIALOG}; */ /* Make bottom border match pane */
        font-weight: bold;
    }}
    ...
    QCheckBox::indicator:checked {{
        ...
        border-radius: 3px;
        background-color: {COLOR_ACCENT_PRIMARY};
        /* image: url(./dependencies/icons/checkmark.png); */ /* Optional: custom checkmark image */
    }}
    QTextEdit#LogOutputWidget, QTextEdit#PySimActionLog {{
         font-family: Consolas, 'Courier New', monospace;
         background-color: #263238; /* Dark background for logs */
         color: #CFD8DC; /* Light text on dark background */
    ...
    QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical {{
         height: 0px; background: transparent; /* No visible add/sub lines */
    }}
    ...
    QPushButton#SnippetButton {{
        ...
        border: 1px solid #E65100; /* Darker accent for border */
        font-weight: normal; /* Normal weight for snippet buttons */
    }}
    QPushButton#SnippetButton:hover {{
        background-color: #FFA000; /* Lighter accent on hover */
    }}
    ...
    QPushButton#DraggableToolButton:hover {{
        background-color: #B9D9EB; /* Lighter blue on hover, using existing accent light */
        border-color: {COLOR_ACCENT_PRIMARY};
    }}
    QPushButton#DraggableToolButton:pressed {{ background-color: #98BAD6; }}

    #PropertiesDock QLabel {{ /* Specific styling for the properties label */
    ...
    #PropertiesDock QPushButton {{ /* Specific styling for the "Edit Details" button */
        background-color: {COLOR_ACCENT_PRIMARY}; color: {COLOR_TEXT_ON_ACCENT};
    }}
    #PropertiesDock QPushButton:hover {{ background-color: #1E88E5; }}

    QDockWidget#ToolsDock QToolButton {{ /* Styles for ToolButtons inside ToolsDock */
    ...
    QDockWidget#PySimDock QPushButton {{ /* PySim Dock specific button styling */
        padding: 5px 10px;
    }}
    QDockWidget#PySimDock QPushButton:disabled {{
        background-color: #E0E0E0; /* Custom disabled for PySim dock if needed */
        color: #9E9E9E;
    }}
    ...
    /* Add specific styling for AIChatbotDock if needed, e.g., input field or status label */
    /*
    QDockWidget#AIChatbotDock QLineEdit#AIChatInput {{ ... }}
    QDockWidget#AIChatbotDock QLabel#AIChatStatusLabel {{ ... }}
    */
"""