...
class AppearanceSettingsTab(QWidget):
    ...
    def _choose_grid_color(self, grid_type_str):
        button, current_color_attr_name = (
            (self.minor_grid_color_button, 'current_minor_grid_color') 
            if grid_type_str == 'minor'
    ...
    def apply_specific_settings(self, data):
        ...
        if current_app_font_size <= 0:
            current_app_font_size = 9  # Default fallback
        ...
        if current_app_font_size != new_font_size:
            ...
            # Update graphics items font sizes
            for item in scene.items():
                if hasattr(item, 'font') and callable(item.setFont):
                    item_font = item.font()
                    if isinstance(item, GraphicsStateItem):
                        item_font.setPointSize(new_font_size - 1 if new_font_size > 8 else 7)
                    elif isinstance(item, GraphicsTransitionItem):
                        item_font.setPointSize(new_font_size - 2 if new_font_size > 9 else 7)
                    else:
                        item_font.setPointSize(new_font_size)
