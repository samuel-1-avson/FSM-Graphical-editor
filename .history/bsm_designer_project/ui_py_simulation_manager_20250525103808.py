...
class PySimulationUIManager(QObject):
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        # Ensure mw.start_py_sim_action etc. are created before this is called if they are to be used here
        ...
        else: self.py_sim_start_btn.setText("Start"); self.py_sim_start_btn.setIcon(get_standard_icon(self.mw.style(), QStyle.SP_MediaPlay, "Py▶")); self.py_sim_start_btn.clicked.connect(self.on_start_py_simulation)
        ...
        else: self.py_sim_stop_btn.setText("Stop"); self.py_sim_stop_btn.setIcon(get_standard_icon(self.mw.style(), QStyle.SP_MediaStop, "Py■")); self.py_sim_stop_btn.clicked.connect(lambda: self.on_stop_py_simulation(silent=False))
        ...
        else: self.py_sim_reset_btn.setText("Reset"); self.py_sim_reset_btn.setIcon(get_standard_icon(self.mw.style(), QStyle.SP_MediaSkipBackward, "Py«")); self.py_sim_reset_btn.clicked.connect(self.on_reset_py_simulation)
    ...
    def on_start_py_simulation(self):
        ...
        try:
            ...
            QMessageBox.critical(self.mw, "Sim Start Error", f"Unexpected error starting simulation:\n{type(e).__name__}: {e}")
    ...
    def on_step_py_simulation(self):
        ...
        try:
            ...
            if self.mw.py_fsm_engine.simulation_halted_flag:
                self.append_to_action_log(["[HALTED] Sim halted due to error. Reset."]); QMessageBox.warning(self.mw, "Sim Halted", "Sim halted due to FSM action error. Reset.")
        except FSMError as e:
