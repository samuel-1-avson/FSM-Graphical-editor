...
class AIChatUIManager(QObject):
    ...
    def create_dock_widget_contents(self):
        ...
        self.ai_chat_send_button = QPushButton(get_standard_icon(self.mw.style(), QStyle.SP_ArrowForward, "Snd"), "Send")
    ...
    def update_status_display(self, status_text: str):
        # Replaces MainWindow._update_ai_chat_status
        ...
        elif is_thinking: self.ai_chat_status_label.setStyleSheet(f"font-size: 8pt; color: {COLOR_ACCENT_SECONDARY.name()};")
        ...
        if self.ai_chat_input:
            ...
            if can_send and self.mw.ai_chatbot_dock and self.mw.ai_chatbot_dock.isVisible() and self.mw.isActiveWindow():
                self.ai_chat_input.setFocus()
        # Update global action enabled state directly if needed (or emit signal to MainWindow)
        self.mw.ask_ai_to_generate_fsm_action.setEnabled(can_send)


    def _append_to_chat_display(self, sender: str, message: str):
        # Replaces MainWindow._append_to_ai_chat_display
        if not self.ai_chat_display: return
        timestamp = QTime.currentTime().toString('hh:mm')
        sender_color = COLOR_ACCENT_PRIMARY.name()
        if sender == "You": sender_color = COLOR_ACCENT_SECONDARY.name()
    ...
    def handle_ai_error(self, error_message: str):
        # Replaces MainWindow._handle_ai_error
    ...
    def handle_fsm_data_from_ai(self, fsm_data: dict, source_message: str):
        # Replaces MainWindow._handle_fsm_data_from_ai
        ...
        msg_box.setText("AI has generated an FSM. Clear current diagram or add to existing?")
    ...
    def handle_plain_ai_response(self, ai_message: str):
        # Replaces MainWindow._handle_plain_ai_response
    ...
    def on_send_ai_chat_message(self):
        # Replaces MainWindow.on_send_ai_chat_message
    ...
    def on_ask_ai_to_generate_fsm(self):
        # Replaces MainWindow.on_ask_ai_to_generate_fsm
        description, ok = QInputDialog.getMultiLineText(self.mw, "Generate FSM", "Describe FSM:", "Traffic light...")
        ...
        elif ok: QMessageBox.warning(self.mw, "Empty Description", "Provide description.")

    @pyqtSlot()
    def on_openai_settings(self):
        # Replaces MainWindow.on_openai_settings
        current_key = self.mw.ai_chatbot_manager.api_key or ""
        key, ok = QInputDialog.getText(self.mw, "OpenAI API Key", "Enter API Key:", QLineEdit.PasswordEchoOnEdit, current_key)
        if ok:
            self.mw.ai_chatbot_manager.set_api_key(key.strip() or None)
            logger.info("AIChatUI: OpenAI API Key " + ("set/updated." if key.strip() else "cleared."))

    @pyqtSlot()
    def on_clear_ai_chat_history(self):
        # Replaces MainWindow.on_clear_ai_chat_history
        if self.mw.ai_chatbot_manager:
            self.mw.ai_chatbot_manager.clear_conversation_history()
            if self.ai_chat_display: self.ai_chat_display.clear(); self.ai_chat_display.setPlaceholderText("AI chat history...")
