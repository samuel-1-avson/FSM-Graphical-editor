...
# from config import COLOR_ACCENT_PRIMARY_LIGHT # Already imported below
...
from config import (
    ...
    COLOR_BACKGROUND_LIGHT, COLOR_ACCENT_PRIMARY_LIGHT # Moved here
)
...
try:
    ...
    # This print will go to console, which is fine for this kind of import error
...
class SubFSMEditorDialog(QDialog): 
    def __init__(self, sub_fsm_data_initial: dict, parent_state_name: str, parent_window_ref=None):
        ...
        if IMPORTS_SUCCESSFUL: # This global variable determines which editor to show
            ...
            # --- Using CodeEditor for JSON fallback ---
            self.json_text_edit = CodeEditor() # Use CodeEditor
            ...
            # Check if "JSON" language/highlighter exists in CodeEditor, otherwise use "Text"
            # This assumes CodeEditor.set_language handles unknown languages gracefully or defaults to plain.
            try:
                # Attempt to set to JSON, if JsonHighlighter was added to CodeEditor
                self.json_text_edit.set_language("JSON")
            except Exception: # Fallback if "JSON" language isn't defined in CodeEditor
            ...
            # --- End CodeEditor for JSON fallback ---
                        ...
                        if not has_initial_json:
                            ...
                            if reply == set_first_btn:
                                if sub_states_list_json:
                                    ...
                                    # Update editor text if we modified the data for "Set First as Initial"
    ...
    def log_message(self, level, message): 
        # This simple logger is fine for dialog-specific messages.
        # For more complex logging, it could integrate with MainWindow's logger.
        print(f"SubFSMEditor Log ({level}): {message}")
        # Pass to parent if parent has a log_message method (like MainWindow)
...
class StatePropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_state=False, scene_ref=None):
        ...
        self.parent_window_ref = parent # Could be MainWindow or another dialog like SubFSMEditor
        ...
        else: # Ensure a valid empty structure if input is malformed or None
            self.current_sub_fsm_data = {'states': [], 'transitions': [], 'comments': []}
            if raw_sub_fsm_data: # Log if we had to correct it
            ...
            if self.current_sub_fsm_data and \
               (self.current_sub_fsm_data.get('states') or self.current_sub_fsm_data.get('transitions')):
                ...
                if reply == QMessageBox.Yes:
                    ...
                    # Prevent signal recursion if user cancels, by blocking signals temporarily
                    self.is_superstate_cb.blockSignals(True)
                    self.is_superstate_cb.setChecked(True)
                    self.is_superstate_cb.blockSignals(False)
                    self.edit_sub_fsm_button.setEnabled(True) # Ensure button state is correct


    def _on_edit_sub_fsm(self): 
        parent_state_name = self.name_edit.text().strip() or "Unnamed Superstate"
        
        # Determine the correct top-level parent for the SubFSMEditorDialog
        # This ensures modality behaves as expected, especially if StatePropertiesDialog
        # is itself a child of MainWindow.
        dialog_parent = self.parent() # Start with immediate parent
        while dialog_parent and not isinstance(dialog_parent, QMainWindow) and hasattr(dialog_parent, 'parent') and callable(dialog_parent.parent):
            if dialog_parent.parent() == dialog_parent: # Safety break for odd cases
                break
            dialog_parent = dialog_parent.parent()
        
        if not dialog_parent: # Fallback if no QMainWindow found up the chain
            dialog_parent = self # Use current dialog as parent
...
class MatlabSettingsDialog(QDialog): 
    ...
    def _update_test_label_from_signal(self, success, message): 
        ...
        # from config import COLOR_ACCENT_SUCCESS, COLOR_ACCENT_ERROR # Already imported at module level
        if success:
