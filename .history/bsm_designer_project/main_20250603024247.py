...
from PyQt5.QtWidgets import (
    ...
    QHeaderView, QTableWidget, QTableWidgetItem,QLayout, QAbstractItemView, QScrollArea # Added QScrollArea
)
...
class MainWindow(QMainWindow):
    ...
    def _clear_dock_editors(self):
        layout = self.properties_editor_layout
        if layout is None:
            return

        items_to_delete_later = []

        # First, collect all widgets and layouts to be removed and detached.
        # Iterate backwards when modifying the layout's structure.
        for i in range(layout.rowCount() - 1, -1, -1):
            ...
            if label_item:
                ...
                if widget:
                    items_to_delete_later.append(widget)
                # No need to check label_item.layout() as labels in QFormLayout are typically QWidgets.

            if field_item:
                widget = field_item.widget()
                if widget:
                    items_to_delete_later.append(widget)
                else: # If not a widget, it must be a layout
                    inner_layout = field_item.layout()
                    if inner_layout:
                        # Mark child widgets/layouts of this inner_layout for deletion
                        while inner_layout.count():
                            child_layout_item = inner_layout.takeAt(0) # takeAt(0) removes the item
                            if child_layout_item:
                                if child_layout_item.widget():
                                    items_to_delete_later.append(child_layout_item.widget())
                                elif child_layout_item.layout():
                                    # If it's a layout within a layout, mark it too
                                    # and then clear its children (basic one-level recursion for clarity)
                                    sub_inner_layout = child_layout_item.layout()
                                    while sub_inner_layout and sub_inner_layout.count():
                                        sub_child_item = sub_inner_layout.takeAt(0)
                                        if sub_child_item and sub_child_item.widget():
                                            items_to_delete_later.append(sub_child_item.widget())
                                        elif sub_child_item and sub_child_item.layout():
                                            items_to_delete_later.append(sub_child_item.layout()) # Mark for deletion
                                    if sub_inner_layout: # Mark the sub_inner_layout itself
                                        items_to_delete_later.append(sub_inner_layout)
                        items_to_delete_later.append(inner_layout) # Mark the inner_layout itself

            # Remove the row from the form layout. This detaches the items.
            layout.removeRow(i)

        # Now, schedule all collected QObjects (widgets or layouts) for deletion.
        for item_obj in items_to_delete_later:
            if item_obj: # Ensure it's not None
                try:
                    # QWidget and QLayout (and its subclasses) are QObjects and have deleteLater
                    item_obj.deleteLater()
                except RuntimeError:
                    # This can happen if the object was already deleted due to parent ownership
                    # when a containing layout or widget was deleted.
                    logger.debug(f"Dock Clear: Object {type(item_obj).__name__} was already deleted or invalid, skipping deleteLater.")
                except AttributeError:
                    # Should not happen if item_obj is always a QWidget or QLayout
                    logger.warning(f"Dock Clear: item_obj of type {type(item_obj).__name__} does not have deleteLater.")


        self._dock_property_editors.clear()
        self._dock_color_button_current_color = None
        self._dock_sub_fsm_data_pending = None
        logger.debug("Properties Dock: All editors cleared.")
    ...
    def _update_matlab_actions_enabled_state(self):
        can_run_matlab_ops = self.matlab_connection.connected and not self.py_sim_active # CORRECTED: Use self.py_sim_active
        ...
        if hasattr(self, 'matlab_settings_action'): self.matlab_settings_action.setEnabled(not self.py_sim_active) # CORRECTED
