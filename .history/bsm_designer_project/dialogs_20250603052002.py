...
import pygraphviz as pgv # Keep for potential future direct use, though AI handles Graphviz now
import psutil
try:
    import pynvml
...
from dialogs import FindItemDialog, MatlabSettingsDialog # Specific dialogs used by MainWindow/Managers
...
class DraggableToolButton(QPushButton):
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        drag = QDrag(self); mime_data = QMimeData(); mime_data.setData(self.mime_type, self.item_type_data_str.encode('utf-8'))
        ...
        drag.setMimeData(mime_data); pixmap = QPixmap(self.size()); pixmap.fill(Qt.transparent)
...
class ResourceMonitorWorker(QObject):
    ...
    @pyqtSlot()
    def stop_monitoring(self): logger.info("ResourceMonitorWorker: stop_monitoring_slot called."); self._stop_requested = True
    ...
    def _monitor_resources(self):
        ...
        last_data_emit_time = 0; worker_thread = self.thread()
        if not worker_thread: logger.error("ResourceMonitorWorker: CRITICAL - Worker not associated with QThread."); self.finished_signal.emit(); return
        loop_count = 0
        while not worker_thread.isInterruptionRequested() and not self._stop_requested:
            loop_count += 1
            if loop_count % 50 == 0: logger.debug(f"ResourceMonitorWorker: Loop iteration {loop_count}")
            ...
            if (current_loop_time - last_data_emit_time) >= self.data_collection_interval_ms or last_data_emit_time == 0:
                ...
                try:
                    ...
                    if self._nvml_initialized and self._gpu_handle:
                        ...
                        except pynvml.NVMLError as e: logger.warning(f"NVML error getting GPU util: {e}"); gpu_util = -2.0; gpu_name_to_emit = f"NVML Read Err (Code: {e.value if hasattr(e, 'value') else 'N/A'})"; self._nvml_initialized = False; self._attempt_nvml_init(from_worker_loop=True)
                ...
                except Exception as e: logger.error(f"ResourceMonitorWorker: Error in data collection: {e}", exc_info=False); (not (worker_thread.isInterruptionRequested() or self._stop_requested) and self.resourceUpdate.emit(-1.0, -1.0, -3.0, "Data Error"))
            inner_loop_checks = int(self.data_collection_interval_ms / self.WORKER_LOOP_CHECK_INTERVAL_MS) + 1; inner_loop_checks = max(1, inner_loop_checks)
            for _i in range(inner_loop_checks):
                if worker_thread.isInterruptionRequested() or self._stop_requested: break
                QThread.msleep(self.WORKER_LOOP_CHECK_INTERVAL_MS)
            if worker_thread.isInterruptionRequested() or self._stop_requested: break
        logger.info(f"ResourceMonitorWorker: _monitor_resources loop EXITED."); self.finished_signal.emit()

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"{APP_NAME} - Untitled [*]")
        self.current_file_path = None; self.undo_stack = QUndoStack(self); self.scene = DiagramScene(self.undo_stack, self); self.view: ZoomableView | None = None
        self.alignment_manager = AlignmentManager(self)
        self.py_fsm_engine: FSMSimulator | None = None; self.py_sim_active = False
        self.find_item_dialog: FindItemDialog | None = None
        self.matlab_connection = MatlabConnection(); self.file_op_manager = FileOperationsManager(self); self.ide_manager = IDEManager(self)
        self.matlab_op_manager = MatlabOperationsManager(self, self.matlab_connection); self.view_manager = ViewManager(self)
        self.ai_chatbot_manager = AIChatbotManager(self); self.ai_chat_ui_manager = AIChatUIManager(self); self.py_sim_ui_manager = PySimulationUIManager(self)
        self._internet_connected: bool | None = None; self.internet_check_timer = QTimer(self); self.resource_monitor_worker: ResourceMonitorWorker | None = None; self.resource_monitor_thread: QThread | None = None
        self.init_ui()
        try:
            if not hasattr(self, 'log_output') or not self.log_output: self.log_output = QTextEdit(); logger.warning("MainWindow: log_output fallback used.")
            setup_global_logging(self.log_output); logger.info("Main window initialized and logging configured.")
        except Exception as e: print(f"ERROR: Failed setup_global_logging: {e}"); logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s')
        self._populate_dynamic_docks(); self._connect_signals(); self._init_resource_monitor(); self._init_internet_status_check()
        self._set_status_label_object_names(); self._update_ui_element_states()
        QTimer.singleShot(0, lambda: self.on_new_file_orchestrator(silent=True))
        if self.ai_chat_ui_manager and self.ai_chatbot_manager: QTimer.singleShot(250, lambda: self.ai_chatbot_manager.set_online_status(self._internet_connected if self._internet_connected is not None else False))
        else: logger.warning("AI managers not fully initialized for final status update.")
    ...
    def _connect_signals(self):
        if hasattr(self, 'scene') and self.scene:
            ...
            self.scene.modifiedStatusChanged.connect(self.setWindowModified) # Also calls self.update_window_title
            ...
            # Connect modifiedStatusChanged to update_window_title to handle the [*]
            self.scene.modifiedStatusChanged.connect(self.update_window_title)
        else: logger.error("Scene not initialized for signal connections.")
        ...
        else: logger.error("PySimUIManager not initialized for signal connections.")
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
            ...
            if self.resource_monitor_worker:
                QMetaObject.invokeMethod(self.resource_monitor_worker, "stop_monitoring", Qt.BlockingQueuedConnection if QThread.currentThread() != self.resource_monitor_thread else Qt.DirectConnection) # Ensure thread-safe call
            self.resource_monitor_thread.requestInterruption() # Request interruption
        ...
        elif self.resource_monitor_worker: worker_ref_for_nvml_shutdown = self.resource_monitor_worker # Still need to shutdown NVML if worker exists
        ...
        for manager in managers_to_cleanup_and_delete:
            if manager:
                ...
                if isinstance(manager, QObject): manager.deleteLater() # Only QObjects can be deleteLater'd
        # Clear references
    ...
    def _update_undo_redo_actions_enable_state(self):
        ...
        else: logger.warning("undo_action not found.")
        ...
        else: logger.warning("redo_action not found.")
        
    @pyqtSlot()
    def on_select_all(self):
        if hasattr(self, 'scene') and self.scene: self.scene.select_all()
        else: logger.warning("Scene not available for select_all.")
    ...
    def _update_save_actions_enable_state(self):
        ...
        else: logger.warning("save_action or scene not available for state update.")
            
    @pyqtSlot()
    def on_delete_selected(self):
        if hasattr(self, 'scene') and self.scene: self.scene.delete_selected_items()
        else: logger.warning("Scene not available for delete_selected.")
    ...
    def on_show_find_item_dialog(self):
        if not self.find_item_dialog:
            if hasattr(self, 'scene') and self.scene:
                ...
                self.scene.scene_content_changed_for_find.connect(self._refresh_find_dialog_if_visible)
            else: logger.error("Scene not available for FindItemDialog."); QMessageBox.warning(self, "Error", "Cannot open Find Item: Diagram scene not initialized."); return
    ...
    @pyqtSlot() # This method remains in MainWindow as it updates UI elements directly owned by it.
        ...
        if self._current_edited_item_in_dock:
            if self.properties_apply_button.isEnabled():
                ...
                if reply == QMessageBox.Apply: self._on_apply_dock_properties();
                                             if self.properties_apply_button.isEnabled(): return # Apply failed or reverted
    ...
    def _handle_py_sim_global_ui_enable_by_manager(self, enable: bool):
        ...
        diagram_editing_actions = [self.new_action, self.open_action, self.save_action, self.save_as_action, self.undo_action, self.redo_action, self.delete_action, self.select_all_action, self.add_state_mode_action, self.add_transition_mode_action, self.add_comment_mode_action, self.add_history_shallow_mode_action] # Added H-action
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram=True, original_user_prompt="AI Generated FSM"):
        if clear_current_diagram:
            if not self.on_new_file_orchestrator(silent=True): return # Prompt for save if needed
        if self.scene: self.scene._add_template_to_scene(fsm_data, self.view.mapToScene(self.view.viewport().rect().center())) # Use scene's method
    ...
    def log_message(self, level_str: str, message: str): # Central logging from MainWindow
