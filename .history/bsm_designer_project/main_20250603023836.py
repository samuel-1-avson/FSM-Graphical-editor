...
class MainWindow(QMainWindow):
    ...
    def _clear_dock_editors(self):
        # Iterate backwards when removing items from a layout to avoid index shifting issues
        for i in range(self.properties_editor_layout.rowCount() - 1, -1, -1):
            # Get the layout item for the label and field
            label_item = self.properties_editor_layout.itemAt(i, QFormLayout.LabelRole)
            field_item = self.properties_editor_layout.itemAt(i, QFormLayout.FieldRole)

            # Get the widgets from the layout items
            label_widget = label_item.widget() if label_item else None
            field_widget = field_item.widget() if field_item else None # Field can be a layout too

            # Remove the row from the layout. This detaches the widgets from the layout.
            self.properties_editor_layout.removeRow(i)

            # Now, explicitly delete the widgets if they exist.
            # Qt's deleteLater is safe to call even if the object was already deleted
            # by parent ownership, but it's good practice to check.
            if label_widget:
                label_widget.deleteLater()
            if field_widget:
                # If the field item was actually a layout, we need to clear that layout too.
                if isinstance(field_item, QHBoxLayout) or isinstance(field_item, QVBoxLayout) or isinstance(field_item, QFormLayout): # Check if it's a layout
                    # Recursively delete widgets in nested layouts
                    layout_in_item = field_item # field_item itself is the layout
                    while layout_in_item.count():
                        nested_child_item = layout_in_item.takeAt(0)
                        if nested_child_item:
                            nested_widget = nested_child_item.widget()
                            if nested_widget:
                                nested_widget.deleteLater()
                            # If the nested item is also a layout, clear it (though not fully recursive here for simplicity)
                            nested_layout = nested_child_item.layout()
                            if nested_layout:
                                while nested_layout.count():
                                    sub_nested_item = nested_layout.takeAt(0)
                                    if sub_nested_item and sub_nested_item.widget():
                                        sub_nested_item.widget().deleteLater()
                elif field_widget: # It's a simple widget
