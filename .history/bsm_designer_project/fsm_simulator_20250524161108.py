        ...
        for t_data in self._transitions_input_data:
            ...
            if event_name not in fsm_class_attrs:
                ...
                if isinstance(fsm_class_attrs[event_name], SMTransitionList):
                    ...
                    if isinstance(current_val, SMTransitionList):
                        new_list.add_transitions(current_val)
                    else:
                        new_list.add_transitions(SMTransitionList([current_val]))
        ...
        if event_name:
            try:
                ...
                if hasattr(self.sm, event_name) and callable(getattr(self.sm, event_name)):
                    self._log_action(f"AttributeError processing event '{event_name}': {e}")
                else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("After step:", next_state, log)
