...
from PyQt5.QtWidgets import (QGraphicsRectItem, QGraphicsPathItem, QGraphicsTextItem,
                             ...
                             QStyle) # Added QStyle for standard icons
...
from config import (COLOR_ITEM_STATE_DEFAULT_BG, COLOR_ITEM_STATE_DEFAULT_BORDER, APP_FONT_FAMILY,
                    ...
                    DEFAULT_EXECUTION_ENV, COLOR_TEXT_SECONDARY, COLOR_BACKGROUND_DIALOG, COLOR_BORDER_MEDIUM) # Added COLOR_BORDER_MEDIUM

from utils import get_standard_icon # For consistent icon fetching
...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
        ...
        self._superstate_border_pen_width = 2.5 # Slightly thicker for superstates
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        current_pen_to_use = QPen(self.pen()) # Start with a copy of the current pen
        
        if self.is_superstate:
            # Apply a slightly thicker border for superstates
            current_pen_to_use.setWidthF(self._superstate_border_pen_width)
            # Optionally, change color or style too, e.g., a slightly darker border
            # current_pen_to_use.setColor(self.border_color.darker(110)) 
        else:
        ...
        if self.is_py_sim_active:
            ...
            current_pen_to_use = py_sim_pen # Override if PySim active
        ...
        # Adjust text rect slightly if superstate icon is present to avoid overlap
        if self.is_superstate:
            text_rect.setRight(text_rect.right() - 18) # Make space for icon on the right
        ...
        if self.is_final:
            # Use the current_pen_to_use's color, but potentially darker, and standard width for inner border
        ...
        # Superstate icon (more prominent)
        if self.is_superstate:
            icon_size = 16 # Larger icon
            margin = 5    # Smaller margin from corner
            icon_rect = QRectF(
                current_rect.right() - icon_size - margin,
                current_rect.top() + margin, # Place in top-right
                icon_size,
                icon_size
            )
            # Using a standard "folder" or "collection" like icon
            # SP_DirIcon or SP_FileDialogDetailedView could work
            superstate_icon = get_standard_icon(QStyle.SP_FileDialogDetailedView, "Superstate") 
            if not superstate_icon.isNull():
                 # Icon color should contrast with base_color
                icon_color = QColor(COLOR_TEXT_PRIMARY)
                if self.base_color.lightnessF() < 0.5: # Dark background
                    icon_color = QColor(COLOR_BACKGROUND_LIGHT)
                
                # Create a pixmap, color it, then draw
                pixmap = superstate_icon.pixmap(icon_size, icon_size)
                # If you need to change the icon color dynamically (QIcon often uses system theme):
                # painter_temp = QPainter(pixmap)
                # painter_temp.setCompositionMode(QPainter.CompositionMode_SourceIn)
                # painter_temp.fillRect(pixmap.rect(), icon_color)
                # painter_temp.end()
                painter.drawPixmap(icon_rect.topLeft(), pixmap)
            else: # Fallback to drawing simple stack if icon fails
    ...
    def set_py_sim_active_style(self, active: bool):
        ...
        else: self.setPen(self.original_pen_for_py_sim_restore) # Restore potentially modified pen
        self.update()
    ...
    def get_data(self):
        # ... (no changes needed here for this step) ...
        return {
    ...
    def set_properties(self, name, is_initial, is_final, color_hex=None,
        ...
        if self.base_color != new_base_color:
            ...
            # Pen will be set in paint based on is_superstate
            changed = True
        ...
        if changed:
            self.prepareGeometryChange()
            # Reset pen to base, paint() will adjust for superstate
...
# ... (Rest of GraphicsTransitionItem and GraphicsCommentItem remain unchanged for this step) ...
class GraphicsTransitionItem(QGraphicsPathItem): # Unchanged from previous step
