...
from dialogs import (StatePropertiesDialog, TransitionPropertiesDialog, CommentPropertiesDialog,
                     MatlabSettingsDialog, PreferencesDialog) # UPDATED
...
from ai_chatbot import AIChatbotManager, ChatbotState # ADDED ChatbotState
...
from PyQt5.QtCore import (
    ...
    QSaveFile, QIODevice, QSettings # ADDED QSettings
)

# --- FSM SIMULATOR IMPORT ---
from fsm_simulator import EnhancedFSMSimulator as FSMSimulator, FSMError # UPDATED to EnhancedFSMSimulator
...
# Removed duplicate dialog import, PreferencesDialog is now imported
...
class DiagramScene(QGraphicsScene):
    ...
    def __init__(self, undo_stack, parent_window=None):
        ...
        self.last_right_click_pos = QPointF() # Store last right-click position

        self.item_moved.connect(self._handle_item_moved)

        self.grid_size = 20
        self.grid_visible = True # Added attribute for grid visibility
    ...
    def set_mode(self, mode: str):
        ...
        # The QActionGroup in MainWindow handles UI button checked states.
        # If this method is called programmatically, MainWindow should also setChecked on the action.
        ...
        if event.button() == Qt.LeftButton:
            if self.current_mode == "state":
                ...
                if isinstance(top_item_at_pos, GraphicsStateItem):
                    ...
                    if self.parent_window: self.parent_window.select_mode_action.setChecked(True)
            ...
            if top_item_at_pos and isinstance(top_item_at_pos, (GraphicsStateItem, GraphicsTransitionItem, GraphicsCommentItem)):
                ...
                self._show_item_context_menu(top_item_at_pos, event.screenPos())
            else:
                self.clearSelection()
    ...
    def _show_scene_context_menu(self, global_pos):
        menu = QMenu()
        ...
        action = menu.exec_(global_pos)
        click_pos_scene = self.last_right_click_pos

        if action == add_state_action:
            grid_x = round(click_pos_scene.x() / self.grid_size) * self.grid_size - 60
            grid_y = round(click_pos_scene.y() / self.grid_size) * self.grid_size - 30
            self._add_item_interactive(QPointF(grid_x, grid_y), item_type="State")
        elif action == add_comment_action:
            grid_x = round(click_pos_scene.x() / self.grid_size) * self.grid_size
            grid_y = round(click_pos_scene.y() / self.grid_size) * self.grid_size
            self._add_item_interactive(QPointF(grid_x, grid_y), item_type="Comment")
        ...
        if event.button() == Qt.LeftButton and self.current_mode == "select":
            if self._mouse_press_items_positions:
                ...
                if moved_items_data_for_command:
                    ...
                    cmd = MoveItemsCommand(cmd_data_redo)
        ...
        if item_type == "State":
            ...
            if props_dialog.exec_() == QDialog.Accepted:
                ...
                if self.get_state_by_name(final_props['name']) and final_props['name'] != default_name :
                    ...
                    current_item = GraphicsStateItem(pos.x(), pos.y(), 120, 60,
                                                     final_props['name'],
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_Delete or \
           (event.key() == Qt.Key_Backspace and sys.platform != 'darwin'):
    ...
    def drawBackground(self, painter: QPainter, rect: QRectF):
        ...
        if not self.grid_visible: return # Check if grid should be drawn
    ...
    def set_grid_visible(self, visible: bool): # Method to control grid visibility
        if self.grid_visible != visible:
            self.grid_visible = visible
            self.update() # Force redraw

    def update_grid_colors(self, minor_color_hex: str, major_color_hex: str):
        self.grid_pen_light.setColor(QColor(minor_color_hex))
        self.grid_pen_dark.setColor(QColor(major_color_hex))
        self.update()
...
class ZoomableView(QGraphicsView):
    def __init__(self, scene, parent=None):
        ...
        self.zoom_sensitivity_factor = 1.12 # Default, can be adjusted by settings
    ...
    def wheelEvent(self, event: QWheelEvent):
        if event.modifiers() & Qt.ControlModifier: # Zoom with Ctrl + Wheel
            ...
            factor = self.zoom_sensitivity_factor if delta > 0 else 1 / self.zoom_sensitivity_factor
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_Space and not self._is_panning_with_space and not event.isAutoRepeat():
            ...
            self.scale(self.zoom_sensitivity_factor, self.zoom_sensitivity_factor); self.zoom_level +=1
        elif event.key() == Qt.Key_Minus: # Zoom Out
            self.scale(1/self.zoom_sensitivity_factor, 1/self.zoom_sensitivity_factor); self.zoom_level -=1
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.global_style_sheet_template = STYLE_SHEET_GLOBAL # STORE THE TEMPLATE
        ...
        # Connect AI Manager signals (Manager's own status/errors)
        self.ai_chatbot_manager.statusChanged.connect(self._update_ai_chat_status)
        self.ai_chatbot_manager.errorOccurred.connect(self._handle_ai_error)
        # Worker signals for response, error, status are connected inside AIChatbotManager to slots in this MainWindow instance
        ...
        # ObjectNames for QSS or direct access
    ...
    def _create_actions(self):
        ...
        self.preferences_action = QAction(get_standard_icon(QStyle.SP_FileDialogDetailedView, "Prefs"),
                                         "&Preferences...", self,
                                         statusTip="Configure application settings",
                                         triggered=self.on_preferences) # CHANGED from on_general_settings
    ...
    def _load_persistent_settings(self):
        settings = QSettings(APP_NAME, "Preferences") # Use a general name

        # Load Appearance Settings
        grid_visible = settings.value("appearance/grid_visible", self.scene.grid_visible, type=bool)
        minor_color = settings.value("appearance/minor_grid_color", self.scene.grid_pen_light.color().name(), type=str)
        major_color = settings.value("appearance/major_grid_color", self.scene.grid_pen_dark.color().name(), type=str)
        font_size = settings.value("appearance/app_font_size", QApplication.instance().font().pointSize(), type=int)
        if font_size <= 0: font_size = 9

        self.scene.set_grid_visible(grid_visible)
        self.scene.update_grid_colors(minor_color, major_color)

        app_font = QApplication.instance().font()
        if app_font.pointSize() != font_size:
            app_font.setPointSize(font_size)
            QApplication.instance().setFont(app_font)
            self.setStyleSheet("") 
            self.setStyleSheet(self.global_style_sheet_template)

        # Load General Settings
        general_settings_to_apply = {}
        general_settings_to_apply['snap_to_grid'] = settings.value("general/snap_to_grid", self.scene.snap_to_grid_enabled, type=bool)
        general_settings_to_apply['zoom_sensitivity'] = settings.value("general/zoom_sensitivity", 3, type=int) # Default 3 maps to 1.12
        # Add more general settings to load here
        # e.g., general_settings_to_apply['auto_save_enabled'] = settings.value("general/auto_save_enabled", False, type=bool)

        if general_settings_to_apply:
            self.apply_general_settings(general_settings_to_apply)


        # Load AI Settings
        api_key = settings.value("ai/api_key", "", type=str)
        # Default to manager's current config model_name if not found in settings
        model_name = settings.value("ai/model_name", self.ai_chatbot_manager.config.model_name, type=str)
        
        if api_key:
            self.ai_chatbot_manager.set_api_key(api_key) # This might start the worker

        # Update config after API key is set (and worker potentially started)
        if model_name != self.ai_chatbot_manager.config.model_name:
            # This updates self.ai_chatbot_manager.config and worker if active
            self.ai_chatbot_manager.update_config({'model_name': model_name})


        # Load MATLAB Path
        if hasattr(self, 'matlab_connection'):
            matlab_path = settings.value("matlab/path", "", type=str)
            if matlab_path:
                self.matlab_connection.set_matlab_path(matlab_path)
    
        self.scene.update()
        self.log_message("Loaded persistent application settings.", type_hint="CONFIG")

    def apply_general_settings(self, settings_dict: dict):
        if 'snap_to_grid' in settings_dict:
            self.scene.snap_to_grid_enabled = settings_dict['snap_to_grid']
            self.log_message(f"Snap to grid {'enabled' if self.scene.snap_to_grid_enabled else 'disabled'}.", type_hint="CONFIG")

        if 'zoom_sensitivity' in settings_dict:
            # Map sensitivity (1-10) to a factor (e.g., 1.05 to 1.25)
            # Example mapping: 1->1.05, 2->1.08, 3->1.12 (default), ..., 10->1.25
            # This is just one way, can be linear or exponential.
            # Let's use a simple linear mapping for now: factor = 1.0 + (value * 0.02)
            # So, 1 -> 1.02, 3 -> 1.06, 5 -> 1.10, 10 -> 1.20.
            # Default in ZoomableView is 1.12. Let's make sensitivity 5 map to 1.12.
            # Factor = 1.0 + (sensitivity_value / 50.0 * 1.2) # (1 to 10) -> 1.024 to 1.24
            # For sensitivity 3 (original default for factor 1.12) -> 1.0 + (3 * 0.04) = 1.12 (incorrect mapping)
            # Let's make it simpler: factor = 1.05 + ( (sensitivity_value - 1) / 9 * 0.15 )
            # sensitivity_value 1 -> 1.05
            # sensitivity_value 5 -> 1.05 + (4/9 * 0.15) = 1.05 + 0.066 = 1.116 (close to 1.12)
            # sensitivity_value 10 -> 1.05 + (9/9 * 0.15) = 1.05 + 0.15 = 1.20
            sensitivity_value = settings_dict['zoom_sensitivity']
            self.view.zoom_sensitivity_factor = 1.05 + ((sensitivity_value - 1) / 9.0 * 0.15)
            self.log_message(f"Zoom sensitivity set to {sensitivity_value} (factor ~{self.view.zoom_sensitivity_factor:.3f}).", type_hint="CONFIG")


        # Apply other settings as they are implemented
        # e.g., self.setup_auto_save_timer(settings_dict.get('auto_save_interval'))

        self.scene.update()
        self.log_message("Applied general settings.", type_hint="CONFIG")
    ...
    def _create_menus(self):
        ...
        edit_menu.addSeparator()
        edit_menu.addAction(self.preferences_action) # CHANGED
    ...
    def _create_status_bar(self):
        ...
        self.matlab_status_label = QLabel("MATLAB: N/A")
    ...
    def on_preferences(self): # RENAMED from on_general_settings
        dialog = PreferencesDialog(self, self) # Pass main_window as first arg
        if dialog.exec_() == QDialog.Accepted or dialog.settings_changed:
             # Settings are applied via Apply button or OK in the dialog itself
             # We might need to refresh things if font changed globally
             self.setStyleSheet("") # Clear current to force re-evaluation
             self.setStyleSheet(self.global_style_sheet_template) # Re-apply global
             self.scene.update() # Redraw scene (e.g. for grid color changes)
             self.log_message("Preferences updated.", type_hint="CONFIG")
    ...
    def _create_py_simulation_dock(self):
        ...
        self.py_sim_variables_table.setFixedHeight(150)
        ...
        py_sim_main_layout.addWidget(log_group, 1)
    ...
    def _on_send_ai_chat_message(self):
        ...
        self._update_ai_chat_status(ChatbotState.PROCESSING.value, "Sending to AI...") # Use enum value
        self.ai_chatbot_manager.send_message(user_message)

    def _handle_ai_response(self, ai_message_text: str, metadata: dict): # Added metadata
        final_status_message = ChatbotState.READY.value; final_status_details = "Response received."
        message_processed_as_fsm = False; fsm_data_description = None
        
        is_generation_request = metadata.get("is_generation", False)
        
        json_string_to_parse = None

        if is_generation_request:
            # For generation requests, expect pure JSON.
            # The 'response_format={"type": "json_object"}' should ensure this.
            json_string_to_parse = ai_message_text.strip()
        else:
            # For non-generation, check for markdown-wrapped JSON first
            match_markdown = re.search(r"```json\s*(\{.*?\})\s*```", ai_message_text, re.DOTALL | re.IGNORECASE)
            if match_markdown:
                json_string_to_parse = match_markdown.group(1).strip()
            else:
                # Fallback: try to find JSON if not wrapped (less reliable for non-generation)
                ...
                if first_brace != -1:
                    ...
                    if last_brace_index != -1:
                        json_string_to_parse = ai_message_text[first_brace : last_brace_index + 1]

        if json_string_to_parse:
            try:
                fsm_data = json.loads(json_string_to_parse)
                if isinstance(fsm_data, dict) and "states" in fsm_data and isinstance(fsm_data["states"], list) and \
                   "transitions" in fsm_data and isinstance(fsm_data["transitions"], list):
                    ...
                    if reply == QMessageBox.Yes:
                        ...
                        # If it was a generation request, the description is the main content.
                        # Otherwise, the JSON might be part of a larger explanation.
                        display_msg = fsm_data_description + " (FSM elements added to diagram)"
                        if not is_generation_request and ai_message_text.strip() != json_string_to_parse:
                            # If JSON was embedded, show the original text too
                            display_msg = f"{ai_message_text}\n\n---\n{display_msg}"

                        self._append_to_ai_chat_display("AI Assistant", display_msg)
                        final_status_details = "FSM elements added."
                    else: 
                        self._append_to_ai_chat_display("AI Assistant", "[FSM data received, user chose not to add.]\n" + (ai_message_text if not is_generation_request else fsm_data_description))
                        final_status_details = "FSM generation cancelled."
                else: message_processed_as_fsm = False # JSON found but not valid FSM structure
            except json.JSONDecodeError: message_processed_as_fsm = False # Not valid JSON

        if not message_processed_as_fsm:
            self._append_to_ai_chat_display("AI Assistant", ai_message_text)
        
        self._update_ai_chat_status(final_status_message, final_status_details)
    ...
    def _handle_ai_error(self, error_message: str, error_code: str = "UNKNOWN"):
        self._append_to_ai_chat_display(f"System Error ({error_code})", error_message)
        self.log_message(f"AI Chatbot Error [{error_code}]: {error_message}", type_hint="AIChatError")
        short_error = error_message.split('\n')[0][:50]
        self._update_ai_chat_status(ChatbotState.ERROR.value, f"{error_code}: {short_error}") # Use enum value

    def _update_ai_chat_status(self, status: str, details: str = ""):
        if details and "State changed from" in details and status != ChatbotState.ERROR.value : # Don't log every state change unless it's an error
            pass # self.log_message(f"AI Chat Status: {status}, Detail: {details}", type_hint="AI_CONFIG")
        elif details: # Log other details
             self.log_message(f"AI Chat Status: {status}, Detail: {details}", type_hint="AI_CONFIG")


        display_status_text = status.replace("_", " ").capitalize()

        if status == ChatbotState.READY.value and "API key validated" in details:
            display_status_text = "Ready (API Key Validated)"
        elif status == ChatbotState.ERROR.value and ("AUTH_ERROR" in error_code if 'error_code' in locals() else "Invalid API key" in details): # Check error_code if passed
            display_status_text = "Error: Invalid API Key"
        elif status == ChatbotState.STOPPED.value and "No API key" in details:
            display_status_text = "Inactive: API Key Required"
        elif status == ChatbotState.ERROR.value : # Generic error
            display_status_text = f"Error: {details.split(':')[0]}" if details else "Error"


        if hasattr(self, 'ai_chat_status_label'):
            self.ai_chat_status_label.setText(f"Status: {display_status_text}")
            
            is_thinking = status in [ChatbotState.PROCESSING.value, ChatbotState.INITIALIZING.value]
            is_key_required = (status == ChatbotState.STOPPED.value and "No API key" in details) or \
                              (status == ChatbotState.ERROR.value and "AUTH_ERROR" in details)

            is_error_state = status == ChatbotState.ERROR.value or is_key_required

            current_style = "font-size: 8pt; "
            if is_error_state: current_style += f"color: {QColor(Qt.red).name()};"
            elif is_thinking: current_style += f"color: {QColor(Qt.gray).name()};"
            else: current_style += f"color: {QColor(Qt.darkGreen).name()};" # Ready, Idle

            self.ai_chat_status_label.setStyleSheet(current_style)
            
            can_send = status == ChatbotState.READY.value and not is_thinking
            ...
            if can_send and self.ai_chatbot_dock.isVisible(): self.ai_chat_input.setFocus()

    def _append_to_ai_chat_display(self, sender: str, message: str):
        timestamp = QTime.currentTime().toString('hh:mm'); 
        sender_color_map = {
            "AI Assistant": COLOR_ACCENT_PRIMARY,
            "You": COLOR_ACCENT_SECONDARY,
        }
        default_sender_color = "#D32F2F" # Red for system errors or unknown
        sender_color = sender_color_map.get(sender, default_sender_color)

        # Sanitize message for HTML display
        escaped_message = html.escape(message).replace('\n','<br>')

        formatted_message = f"<div style='margin-bottom:5px;'><span style='font-size:8pt;color:grey;'>[{timestamp}]</span> <strong style='color:{sender_color};'>{html.escape(sender)}:</strong> {escaped_message}</div>"
        self.ai_chat_display.append(formatted_message); self.ai_chat_display.ensureCursorVisible()

    def on_openai_settings(self):
        settings = QSettings(APP_NAME, "Preferences") # Use "Preferences" to align with dialog
        ...
        if not ok: # User cancelled API key input
            return

        new_api_key = key.strip() or None
        
        models = ["gpt-3.5-turbo", "gpt-4", "gpt-4-turbo", "gpt-4o"] 
        current_model = self.ai_chatbot_manager.config.model_name
        
        # Find current model index, default to 0 if not found
        try:
            current_model_idx = models.index(current_model)
        except ValueError:
            current_model_idx = 0
            if current_model not in models and current_model: # If current_model is custom and not in list, add it
                models.insert(0, current_model)
                current_model_idx = 0


        model_name, ok_model = QInputDialog.getItem(self, "Select Model", "Choose OpenAI Model:", models, current_model_idx, editable=True) # Allow editing for custom models
        
        if not ok_model: # User cancelled model selection
             # Still apply API key change if it was modified
            if new_api_key != current_key:
                 self.ai_chatbot_manager.set_api_key(new_api_key)
                 self.log_message(f"OpenAI API Key {'set/updated' if new_api_key else 'cleared'}.", type_hint="AI_CONFIG")
                 settings.setValue("ai/api_key", new_api_key if new_api_key else "")
            return

        api_key_changed = (new_api_key != current_key)
        model_name_changed = (model_name != current_model)
        ...
        if model_name_changed:
            self.ai_chatbot_manager.update_config({'model_name': model_name}) # Updates manager's config and worker
            self.log_message(f"OpenAI Model set to: {model_name}", type_hint="AI_CONFIG")
            settings.setValue("ai/model_name", model_name)
        elif api_key_changed and new_api_key: # If API key was set, ensure current model config is pushed
             self.ai_chatbot_manager.update_config({'model_name': model_name})
    ...
    def _update_window_title(self):
        ...
        if self.isWindowModified(): title+="[*]" # Use built-in isWindowModified
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.ai_chatbot_manager:self.ai_chatbot_manager.stop() # Changed from stop_chatbot to stop
        ...
        if self.internet_check_timer and not self.internet_check_timer.isActive() and not event.isAccepted(): # If close was cancelled, restart timer
             self.internet_check_timer.start();
             print(f"--- MW ({current_time}): Close cancelled by user. Internet timer restarted. ---")
    ...
    def _update_py_sim_status_display(self):
        if self.py_sim_active and self.py_fsm_engine:
            state_name = self.py_fsm_engine.get_current_state() # FSMSimulator has get_current_state
    ...
    def set_ui_enabled_for_py_sim(self, is_sim_running: bool):
        ...
        if is_editable and self.scene.current_mode != "select":
            ...
            self.scene.set_mode("select") # Force select mode
            self.select_mode_action.setChecked(True)
    ...
    def _update_py_simulation_dock_ui(self):
        ...
        current_state = self.py_fsm_engine.get_current_state(); self.py_sim_current_state_label.setText(f"<b>{html.escape(current_state or 'N/A')}</b>"); self._highlight_sim_active_state(current_state)
    ...
    def _append_to_py_simulation_log(self, log_entries: list): # Updated to handle LogEntry objects
        for entry_obj in log_entries:
            # Assuming entry_obj is either a string (old format) or LogEntry
            if isinstance(entry_obj, str):
                entry_msg = entry_obj
            elif hasattr(entry_obj, 'message'): # Assuming LogEntry object
                entry_msg = f"[{entry_obj.level.value}] {entry_obj.message}"
                if entry_obj.state: entry_msg += f" (State: {entry_obj.state})"
                if entry_obj.event: entry_msg += f" (Event: {entry_obj.event})"
            else:
                entry_msg = str(entry_obj)


            cleaned_entry = html.escape(entry_msg)
            if "[Condition]" in entry_msg or "[Eval Error]" in entry_msg or "ERROR" in entry_msg.upper() or "[Env Error]" in entry_msg : cleaned_entry = f"<span style='color:{COLOR_ACCENT_SECONDARY};'>{cleaned_entry}</span>"
            elif "Transitioned from" in entry_msg or "Reset to state" in entry_msg or "Simulation started" in entry_msg or "Environment module" in entry_msg or "Called environment" in entry_msg: cleaned_entry = f"<span style='color:{COLOR_ACCENT_PRIMARY}; font-weight:bold;'>{cleaned_entry}</span>"
            elif "No eligible transition" in entry_msg: cleaned_entry = f"<span style='color:{COLOR_TEXT_SECONDARY};'>{cleaned_entry}</span>"
        ...
        if log_entries:
            last_log_obj = log_entries[-1]
            last_log_short = (last_log_obj.message if hasattr(last_log_obj, 'message') else str(last_log_obj)).split('\n')[0][:100]
    ...
    def on_start_py_simulation(self):
        ...
        try:
            ...
            initial_log = [type('LogEntry', (), {'level': type('LogLevel', (), {'value':'INFO'}), 'message':'Python FSM Simulation started.', 'state':None, 'event':None, 'variables':None})()] # Mock LogEntry
            # If FSMSimulator now returns LogEntry objects from get_last_executed_actions_log or similar:
            ...
            QMessageBox.critical(self, "FSM Initialization Error", msg); self._append_to_py_simulation_log([type('LogEntry', (), {'level': type('LogLevel', (), {'value':'ERROR'}), 'message':f"ERROR Starting Sim: {msg}", 'state':None, 'event':None, 'variables':None})()])
    ...
    def on_stop_py_simulation(self, silent=False):
        ...
        if not silent: self._append_to_py_simulation_log([type('LogEntry', (), {'level': type('LogLevel', (), {'value':'INFO'}), 'message':'Python FSM Simulation stopped.', 'state':None, 'event':None, 'variables':None})()])
    ...
    def on_reset_py_simulation(self):
        ...
        try:
            ...
            # reset_logs = self.py_fsm_engine.get_last_executed_actions_log(); # Assuming this returns LogEntry
            # self._append_to_py_simulation_log(reset_logs);
            self._update_py_simulation_dock_ui()
        except Exception as e: msg = f"Failed to reset Python FSM simulation:\n{type(e).__name__}: {e}"; QMessageBox.critical(self, "FSM Reset Error", msg); self._append_to_py_simulation_log([type('LogEntry', (), {'level': type('LogLevel', (), {'value':'ERROR'}), 'message':f"ERROR DURING RESET: {msg}", 'state':None, 'event':None, 'variables':None})()])
    ...
    def on_step_py_simulation(self):
        ...
        except Exception as e: msg = f"Simulation Step Error: {type(e).__name__}: {e}"; QMessageBox.warning(self, "Simulation Step Error", str(e)); self._append_to_py_simulation_log([type('LogEntry', (), {'level': type('LogLevel', (), {'value':'ERROR'}), 'message':f"ERROR DURING STEP: {msg}", 'state':None, 'event':None, 'variables':None})()])
    ...
    def on_trigger_py_event(self):
        ...
        except Exception as e: msg = f"Simulation Event Error ({html.escape(event_name)}): {type(e).__name__}: {e}"; QMessageBox.warning(self, "Simulation Event Error", str(e)); self._append_to_py_simulation_log([type('LogEntry', (), {'level': type('LogLevel', (), {'value':'ERROR'}), 'message':f"ERROR DURING EVENT '{html.escape(event_name)}': {msg}", 'state':None, 'event':None, 'variables':None})()])
