...
class GraphicsStateItem(QGraphicsRectItem):
    # ... (No changes to GraphicsStateItem in this fix) ...
...
class GraphicsTransitionItem(QGraphicsPathItem):
           ...
           self.control_point_offset.x() == 0 and self.control_point_offset.y() == 0:
            if self.path().elementCount() > 0:
                ...
                if self.start_item and self.end_item: 
                    ...
                    if line_len_for_perp > 1e-6: # Check if start and end are not coincident
                        ...
                        cp_pos = mid_point + QPointF(0, default_offset_dist) # Fallback if centers are same
                else: 
    ...
    def _get_intersection_point(self, item: QGraphicsRectItem, line: QLineF):
        ...
        if not intersect_path.isEmpty() and intersect_path.elementCount() > 0:
            # For simple intersections, the first element is often one of the intersection points
            # However, for lines passing through, it might be complex.
            # We want the intersection point on the boundary that the line hits first.
            
            # Get all points from the intersection path
            points_on_boundary = []
            for i in range(intersect_path.elementCount()):
                el = intersect_path.elementAt(i)
                # Element types can be MoveTo, LineTo etc. We are interested in the end points of segments.
                # For simplicity, consider all element points.
                points_on_boundary.append(QPointF(el.x, el.y))
                # If LineTo, also consider previous point if it was MoveTo
            ...
            if points_on_boundary:
                # Corrected: We need the actual vector from line.p1() to line.p2() for unitVector
                ...
                if line_length < 1e-6: # Points are coincident
                    return line.p1()

                # The unit vector representing the direction of the line
                ...
                for pt_boundary in points_on_boundary:
                    ...
                    if 0 <= projection < min_proj : # Projection must be non-negative and smallest
                         min_proj = projection
                         best_point = pt_boundary
                if best_point:
                    return best_point
            # Fallback if the above logic doesn't find a suitable point (e.g. line starts inside)
            # or if intersection_path structure is not as expected.
            # Use the first element's point as a basic fallback.
            if intersect_path.elementCount() > 0:
                return QPointF(intersect_path.elementAt(0).x, intersect_path.elementAt(0).y)


        # Fallback: simple line-rect intersection if path intersection fails
        edges = [
        ...
        if not intersect_points: return item_rect.center() # Should not happen if line aims at rect
        
        # Return the intersection point closest to the start of the line (line.p1())
