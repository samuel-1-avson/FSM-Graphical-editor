...
from PyQt5.QtWidgets import (QGraphicsRectItem, QGraphicsPathItem, QGraphicsTextItem,
                             ...
                             QStyle, QLineEdit, QTextEdit, QGraphicsProxyWidget, QMessageBox) # Added QMessageBox
...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    def start_inline_edit(self):
        ...
        editor.setStyleSheet(f"""
            QLineEdit {{ 
                ...
                border-radius: {self.rect().height() / 6}px; 
                padding: 5px; 
            }}
        """)
        ...
        # editor.setGeometry(text_rect_local.toRect()) # This line is problematic for proxy widget
        editor.selectAll()
        
        editor.editingFinished.connect(lambda: self._finish_inline_edit(editor)) # Pass editor instance
        ...
        # Position the proxy so the editor aligns with the text area
        self._inline_editor_proxy.setPos(text_rect_local.topLeft())
        # Set the size of the editor widget itself
    ...
    def _handle_editor_key_press(self, event: QKeyEvent, editor_widget: QLineEdit):
        if event.key() == Qt.Key_Escape:
            self._is_editing_inline = False 
            editor_widget.setText(self.text_label) # Revert text
    ...
    def _finish_inline_edit(self, editor_widget: QLineEdit | None = None):
        # If editor_widget is None, it means we might be called from an abort path
        # where the proxy might already be cleaned up.
        if not self._inline_editor_proxy and not editor_widget : return
        ...
        if not actual_editor: return


        new_text = actual_editor.text().strip()
        old_text = self.text_label
        
        commit_changes = self._is_editing_inline # Use the flag that was set by Escape or not

        # Cleanup
        if self._inline_editor_proxy:
            self._inline_editor_proxy.setWidget(None)
            if self._inline_editor_proxy.scene(): # Only remove if it was added to a scene
        ...
        if commit_changes and new_text and new_text != old_text:
            if self.scene() and hasattr(self.scene(), 'get_state_by_name'):
                ...
                if existing_state and existing_state != self:
                    QMessageBox.warning(None, "Duplicate Name", f"A state named '{new_text}' already exists. Reverting.")
                    return 
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_F2 and self.isSelected() and self.flags() & QGraphicsItem.ItemIsFocusable:
            if not self._is_editing_inline: # Only start if not already editing
...
class GraphicsTransitionItem(QGraphicsPathItem): # Unchanged from before
    ...
    def _get_actual_control_point_scene_pos(self) -> QPointF:
        ...
        if self.start_item == self.end_item:
            ...
            base_cp_x = p1_scene.x() 
            base_cp_y = p1_scene.y() - loop_radius_y * 1.5 
           ...
           self.control_point_offset.x() == 0 and self.control_point_offset.y() == 0:
            if self.path().elementCount() > 0:
                ...
                if self.start_item and self.end_item: 
                    ...
                    if line_len_for_perp > 1e-6: 
                        perp_vec_normalized = QPointF(-line_vec.y(), line_vec.x()) / line_len_for_perp
    ...
    def _get_intersection_point(self, item: QGraphicsRectItem, line: QLineF):
        ...
        if not intersect_path.isEmpty() and intersect_path.elementCount() > 0:
            ...
            for i in range(intersect_path.elementCount()):
                ...
                if el.isLineTo() and i > 0 and intersect_path.elementAt(i-1).isMoveTo():
                    prev_el = intersect_path.elementAt(i-1)
                    points_on_boundary.append(QPointF(prev_el.x, prev_el.y))

            if points_on_boundary:
                original_line_actual_vector = line.p2() - line.p1()
                line_length = math.hypot(original_line_actual_vector.x(), original_line_actual_vector.y())
                
                if line_length < 1e-6: 
                    points_on_boundary.sort(key=lambda pt: QLineF(line.p1(), pt).length())
                    return points_on_boundary[0] if points_on_boundary else item_rect.center()

                direction_vector_qpointf = original_line_actual_vector / line_length

                min_proj = float('inf')
                best_point = None
                for pt_boundary in points_on_boundary:
                    vec_to_boundary = pt_boundary - line.p1()
                    projection = QPointF.dotProduct(vec_to_boundary, direction_vector_qpointf)
                    if 0 <= projection < min_proj : 
                         min_proj = projection
                         best_point = pt_boundary
                if best_point:
                    return best_point
            if intersect_path.elementCount() > 0: 
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent):
        if self._dragging_control_point:
            ...
            if self.start_item == self.end_item:
                ...
                tangent_dir = line_vec / line_len if line_len > 0 else QPointF(1,0) 
...
class GraphicsCommentItem(QGraphicsTextItem): 
    ...
    def paint(self, painter, option, widget):
        ...
        # Only draw text using super().paint if not inline editing
        if not self._is_editing_inline: 
    ...
    def start_inline_edit(self):
        ...
        editor = QTextEdit(self.toPlainText())
        editor.setFont(self.font()) 
        
        # Approximate current visual width and height of the text content for editor size
        ...
        editor_width = int(max(current_doc_width, self._default_width * 0.8)) + 10 # Add some padding
        editor_height = int(current_doc_height) + 10 # Add some padding
        ...
        editor.setStyleSheet(f"""
            QTextEdit {{ 
                ...
                border-radius: 3px; 
                padding: 4px; /* Match visual padding */
            }}
        """)
        ...
        self._inline_editor_proxy = QGraphicsProxyWidget(self)
        self._inline_editor_proxy.setWidget(editor)
        # Position proxy relative to item's origin.
        # Since comment's text is drawn from its origin, proxy at (0,0) is fine.
    ...
    def _handle_comment_editor_focus_out(self, event: QFocusEvent, editor_widget: QTextEdit):
        ...
        # QTextEdit.focusOutEvent(editor_widget, event) # Avoid calling super's focusOut if it causes issues with proxy
    ...
    def _finish_inline_edit(self):
        if not self._inline_editor_proxy: return 

        editor_widget = self._inline_editor_proxy.widget()
        if not editor_widget: # Check if widget is already gone
            self._is_editing_inline = False # Ensure flag is reset
            self.update()
            return

        new_text = editor_widget.toPlainText() # Don't strip yet for comments
        old_text = self.toPlainText() 

        commit_changes = self._is_editing_inline 

        # Cleanup editor and proxy
        self._inline_editor_proxy.setWidget(None) # Important to release widget
        editor_widget.deleteLater() # Schedule editor for deletion
        
        # It's safer to remove proxy from scene if it was added, or just delete if parented
        if self._inline_editor_proxy.scene():
            self.scene().removeItem(self._inline_editor_proxy)
        self._inline_editor_proxy.deleteLater()
        self._inline_editor_proxy = None
        
        self._is_editing_inline = False # Reset state *before* potential property update

        if commit_changes and new_text.strip() != old_text.strip(): # Compare stripped for change detection
            self.setPlainText(new_text) # Apply change to the QGraphicsTextItem
            
            # After setting text, re-evaluate properties for undo command
            old_props = self.get_data() # This will get old width before text change affected it
            old_props['text'] = old_text # Ensure old text is correct

            new_props = self.get_data() # This will get new width after text change
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_F2 and self.isSelected() and self.flags() & QGraphicsItem.ItemIsFocusable:
            ...
            # If QGraphicsTextItem is already editable (e.g. TextEditorInteraction and it has focus from a click)
            # let it handle typing. If not, pass to base.
            if self.textInteractionFlags() & Qt.TextEditorInteraction and self.hasFocus():
                 super().keyPressEvent(event)
            elif not event.isAccepted(): # Only if not handled by QGraphicsTextItem itself
                 QGraphicsItem.keyPressEvent(self,event) 
            
    def get_data(self):
        doc_width = self.textWidth()
        if doc_width < 0 : 
            doc_width = self.document().idealWidth() if self.document() else self._default_width
    ...
    def set_properties(self, text, width=None):
        ...
        target_width = float(width) if width is not None else self._default_width # Ensure width is float
        
        # Compare with current textWidth if it's not -1 (auto)
        if self.textWidth() >= 0 and abs(self.textWidth() - target_width) > 1e-3 :
             width_changed = True
        elif self.textWidth() < 0 and abs(self.document().idealWidth() - target_width) > 1e-3: # Compare idealWidth if auto
             width_changed = True
