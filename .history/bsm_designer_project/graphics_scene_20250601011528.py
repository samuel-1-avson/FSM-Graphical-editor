...
class DiagramScene(QGraphicsScene):
    ...
    def edit_item_properties(self, item):
        ...
        # Determine parent for dialog: if this scene belongs to SubFSMEditorDialog, use that, else MainWindow.
        dialog_parent = self.parent_window
        if self.parent_window and self.parent_window.__class__.__name__ != "MainWindow":
            # If parent_window is not MainWindow, it's likely the SubFSMEditorDialog itself.
            pass # dialog_parent is already correct (the SubFSMEditorDialog)
        elif self.parent_window and self.parent_window.__class__.__name__ == "MainWindow":
            pass # dialog_parent is already correct (MainWindow)
        elif self.views(): # Fallback if parent_window context is unclear
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        ...
        # Determine parent for dialog (same logic as in edit_item_properties)
        dialog_parent = self.parent_window
        if self.parent_window and self.parent_window.__class__.__name__ != "MainWindow":
            pass
        elif self.parent_window and self.parent_window.__class__.__name__ == "MainWindow":
