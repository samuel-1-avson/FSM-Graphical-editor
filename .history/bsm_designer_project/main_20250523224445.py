...
# ...
    ...
    def _handle_ai_response(self, ai_message_text: str): # SLOT for ChatbotWorker.responseReady
        ...
        fsm_data_description = None # To store the description from the FSM JSON

        # Try to find a JSON block. LLMs sometimes wrap it in ```json ... ``` or just output it.
        ...
        if match_markdown:
            ...
            # Pattern 2: Starts with { and ends with } (might be greedy and include trailing text)
        ...
        if json_match and json_string:
            ...
            # ...
                ...
                # ...
                    if not match_markdown and not ai_message_text.strip().startswith("{"): 
                    # If we got here via brace matching, but not strict JSON start/end, and no markdown
                    # it might indicate a slight deviation from pure JSON output.
                        self.log_message("AI response contained JSON that was extracted via brace matching from a larger text body.", type_hint="AI_ACTION")   
            ...
            if first_brace != -1:
                ...
                if last_brace_index != -1:
                    ...
                    json_string = None # No matching brace found
               ...
               "transitions" in fsm_data and isinstance(fsm_data["transitions"], list):
                ...
                if isinstance(fsm_data, dict) and \
                   ...
                   "transitions" in fsm_data and isinstance(fsm_data["transitions"], list):
                    ...
                    fsm_data_description = fsm_data.get("description", "Generated FSM.") # Get description
                    ...
                    if reply == QMessageBox.Yes:
                        ...
                        # Display only the FSM description (or generic message) in chat, not the whole raw AI output if it was processed
                ...
                message_processed_as_fsm = False
        # else: json_match was false or json_string was None
            # Fall through to treat ai_message_text (the whole thing) as text.
        
        if not message_processed_as_fsm:
            print(f"--- MW_HANDLE_RESP ({current_time}): Appending full AI response as REGULAR chat. ---")
            self._append_to_ai_chat_display("AI Assistant", ai_message_text) # Display the whole original message
