    ...
    def process_message_slot(self, user_message: str, force_fsm_generation: bool):
        ...
        if self.current_diagram_context_json_str:
            try:
                ...
                context_type = diagram.get("context_type", "full_diagram") # Check if context is partial
                ...
                if "error" not in diagram: # Check if the context itself is an error message
                    ...
                    context_prefix = "The current diagram"
                    if context_type == "partial_selection":
                        context_prefix = "The current selection from the diagram"
                        system_prompt_parts.append(" The following context is based on a partial selection of items from a larger diagram.")
                    ...
                    if state_names:
                        ...
                        max_state_names_summary_len = 150 
                        ...
                        context_summary = (
                            f" {context_prefix} has states: {state_names_list_str}."
                            f" It has {num_transitions} transition(s)."
                        )
                        system_prompt_parts.append(context_summary)
                    else:
                        system_prompt_parts.append(f" {context_prefix} is empty or contains no states.")
             ...
             system_prompt_parts.append(" No diagram context is currently active or provided for this request.")
        ...
        api_contents = []
        history_to_include = self.conversation_history[-12:] 
...
class AIChatbotManager(QObject):
            ...
            try:
                ...
                if match:
                    ...
                    if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
                        ...
                        try:
                            _ = json.loads(potential_json) # Test parse
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        partial_context_used = False 
        if self.parent_window and hasattr(self.parent_window, 'scene'):
            scene_ref = self.parent_window.scene
            # Check if there are selected items and if the scene has the method for selected data
            if scene_ref.selectedItems() and hasattr(scene_ref, 'get_selected_items_diagram_data'):
                try:
                    diagram_data = scene_ref.get_selected_items_diagram_data()
                    # Only use partial context if it actually contains states or transitions
                    if diagram_data.get('states') or diagram_data.get('transitions') or diagram_data.get('comments'):
                        diagram_json_str = json.dumps(diagram_data)
                        partial_context_used = True
                        logger.info("MGR_PREP_SEND: Using *selected items* for diagram context. Data length: %d", len(diagram_json_str))
                    else:
                        logger.info("MGR_PREP_SEND: Selected items yielded no data (empty states/transitions/comments), falling back to full diagram context.")
                except Exception as e:
                    logger.error(f"MGR_PREP_SEND: Error getting selected items diagram data: {e}", exc_info=True)
                    diagram_json_str = json.dumps({"error": "Error retrieving selected diagram context."}) # Send error as JSON

            
            if not partial_context_used: # If not using partial context (no selection, or error with selection, or empty selection data)
                if hasattr(scene_ref, 'get_diagram_data'):
                    try:
                        diagram_data = scene_ref.get_diagram_data()
                        diagram_json_str = json.dumps(diagram_data)
                        logger.info("MGR_PREP_SEND: Using *full diagram* for context. Data length: %d", len(diagram_json_str))
                    except Exception as e:
                        logger.error(f"MGR_PREP_SEND: Error getting full diagram data: {e}", exc_info=True)
                        diagram_json_str = json.dumps({"error": "Could not retrieve full diagram context from scene."})
                else: # Scene exists but no get_diagram_data method
                    logger.warning("MGR_PREP_SEND: Diagram context (scene.get_diagram_data) unavailable.")
                    diagram_json_str = json.dumps({"error": "Diagram context source method unavailable in manager (full)."})
        else: # No scene reference on parent_window
             logger.warning("MGR_PREP_SEND: Diagram scene (parent_window.scene) unavailable to manager.")
             diagram_json_str = json.dumps({"error": "Diagram scene unavailable to manager."})
