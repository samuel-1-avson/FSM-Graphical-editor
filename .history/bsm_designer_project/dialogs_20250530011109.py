...
from config import COLOR_ACCENT_PRIMARY_LIGHT
...
from config import (
    ...
    COLOR_BACKGROUND_LIGHT # For GroupBox background
)
...
class SubFSMEditorDialog(QDialog): 
    def __init__(self, sub_fsm_data_initial: dict, parent_state_name: str, parent_window_ref=None):
        ...
        if IMPORTS_SUCCESSFUL:
            ...
            self.json_edit_label.setObjectName("ErrorLabel") # For potential specific styling if needed
            ...
            self.status_label = QLabel("Edit the JSON data for the sub-machine. Click OK to save changes.") # Simplified status for JSON mode
        ...
        if IMPORTS_SUCCESSFUL and hasattr(self, 'sub_scene'):
            ...
            if isinstance(updated_data, dict) and \
               ...
               isinstance(updated_data.get('comments'), list):
                ...
                if sub_states_list: # Only check if there are states
                    has_initial = any(s.get('is_initial', False) for s in sub_states_list)
                    if not has_initial and sub_states_list: # And there's at least one state to make initial
                        ...
                        if reply == set_first_btn:
                            if sub_states_list: # Double check there's a state
            ...
            try:
                ...
                if isinstance(parsed_new_data, dict) and all(k in parsed_new_data for k in ['states', 'transitions', 'comments']):
                    ...
                    if sub_states_list_json: # Only check if there are states
                        has_initial_json = any(s.get('is_initial', False) for s in sub_states_list_json)
                        if not has_initial_json and sub_states_list_json:
                            msg_box = QMessageBox(self)
                            msg_box.setIcon(QMessageBox.Warning)
                            msg_box.setWindowTitle("No Initial Sub-State")
...
class StatePropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_state=False, scene_ref=None):
        ...
        self.setMinimumWidth(600) # Wider dialog
        ...
        def add_field_with_note(form_layout, label_text, code_editor_widget, snippet_button):
            ...
            v_btn_container.setSpacing(3) # Space between button and note
            ...
            v_btn_container.addStretch(1) # Pushes button and note up if space allows
            h_editor_btn_layout.addLayout(v_btn_container)
            form_layout.addRow(label_text, h_editor_btn_layout) # Add entire HBox as the field for the row
        ...
        desc_layout = QVBoxLayout(desc_group) # Use QVBoxLayout for direct widget add
    ...
    def _create_insert_snippet_button(self, target_widget: CodeEditor, snippet_category: str, button_text="Insert...", icon_size_px=16): # Keep icon size consistent
        button = QPushButton(button_text); button.setObjectName("SnippetButton") # QSS can target SnippetButton
    ...
    def _on_edit_sub_fsm(self): 
        ...
        # Traverse up to find the main window or top-level dialog if parent_window_ref is another dialog
        actual_main_window_or_top_dialog = dialog_parent_ref
        max_depth = 5 # Safety break for recursion
        depth_count = 0
        while hasattr(actual_main_window_or_top_dialog, 'parent_window_ref') and \
              actual_main_window_or_top_dialog.parent_window_ref is not None and \
              not isinstance(actual_main_window_or_top_dialog, QMainWindow) and \
              depth_count < max_depth :
            actual_main_window_or_top_dialog = actual_main_window_or_top_dialog.parent_window_ref
            depth_count +=1

        sub_editor_dialog = SubFSMEditorDialog(self.current_sub_fsm_data, parent_state_name, actual_main_window_or_top_dialog)
...
class TransitionPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_transition=False):
        ...
        def add_lineedit_with_snippet(form_layout, label_text, edit_widget, snippet_button, is_code_field=True):
            ...
            if is_code_field: v_btn_container.addStretch() # Only stretch for code fields to align notes better
            h_editor_btn_layout.addLayout(v_btn_container)
        ...
        action_form_layout = QFormLayout(action_group) # Using QFormLayout for consistency
        ...
        def add_codeeditor_with_snippet(form_layout, label_text, code_editor_widget, snippet_button): # Separate helper for CodeEditor for clarity
            h_editor_btn_layout = QHBoxLayout(); h_editor_btn_layout.setSpacing(6)
            h_editor_btn_layout.addWidget(code_editor_widget, 1)
...
class MatlabSettingsDialog(QDialog): 
    ...
    def _update_test_label_from_signal(self, success, message): 
        ...
        from config import COLOR_ACCENT_SUCCESS, COLOR_ACCENT_ERROR # Import here if not global
        if success:
