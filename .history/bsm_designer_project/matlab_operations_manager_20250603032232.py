...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QComboBox, QDialogButtonBox, QHBoxLayout, QStyle, QToolBar # Added QToolBar
)
from PyQt5.QtCore import QDir, QUrl, pyqtSlot, QObject # <--- ADD QObject HERE
from PyQt5.QtGui import QDesktopServices, QColor # <--- Added QColor

from matlab_integration import MatlabConnection
# from dialogs import MatlabSettingsDialog # This dialog is still created and used here
from utils import get_standard_icon
# Need to import MatlabSettingsDialog if it's used directly by this manager.
# It was previously in main.py. If it's purely a UI dialog shown by MainWindow,
# then this manager wouldn't import it. But on_matlab_settings creates it.
from dialogs import MatlabSettingsDialog # Ensure this import is correct

# For status label updates (colors)
from config import COLOR_ACCENT_SUCCESS, COLOR_ACCENT_ERROR


logger = logging.getLogger(__name__)

class MatlabOperationsManager(QObject): # Now QObject is defined
    def __init__(self, main_window, matlab_connection: MatlabConnection):
        super().__init__()
        self.mw = main_window
        self.matlab_connection = matlab_connection
        self.last_generated_model_path: str | None = None
    ...
    def _finish_matlab_operation(self):
        ...
        if hasattr(self.mw, 'status_label'):
            self.mw.status_label.setText("Ready")
    ...
    def set_ui_enabled_for_matlab_op(self, enabled: bool):
        ...
        for child in self.mw.findChildren(QToolBar): # Corrected to iterate QToolBar
        ...
        for dock_name in ["ToolsDock", "PropertiesDock", "LogDock", "PySimDock", "AIChatbotDock", "IDEDock", "ProblemsDock"]:
            # Ensure self.mw.ide_dock is accessed correctly if it's a direct attribute of mw,
            # or via self.mw.ide_manager.ide_dock if it's managed by IDEManager
            dock_widget_instance = None
            if dock_name == "IDEDock" and hasattr(self.mw, 'ide_manager') and self.mw.ide_manager:
                 dock_widget_instance = self.mw.ide_manager.ide_dock
            else:
                 dock_widget_instance = self.mw.findChild(QDockWidget, dock_name)

            if dock_widget_instance:
                 dock_widget_instance.setEnabled(enabled)

        self.mw._update_py_simulation_actions_enabled_state()

    @pyqtSlot(bool, str)
    def update_matlab_status_display(self, connected: bool, message: str):
        self.mw._update_matlab_status_display(connected, message)
        self.update_matlab_actions_enabled_state()
