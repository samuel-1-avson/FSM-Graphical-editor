...
# from PyQt5.QtCore import QTime, QTimer, QPointF, QMetaObject, QFile, QTemporaryFile, QDir, QIODevice, QFileInfo, QEvent, QSize, QUrl, pyqtSignal, pyqtSlot, QThread, QPoint, QMimeData # Duplicate removed
...
from ai_chatbot import AIChatbotManager, AIStatus, AIChatUIManager # Added AIStatus, AIChatUIManager
...
# AIChatUIManager is now imported from ai_chatbot
...
class ResourceMonitorWorker(QObject):
    ...
    NVML_REINIT_BACKOFF_SECONDS = 30 # Time to wait before retrying NVML init after max attempts
    ...
    def __init__(self, interval_ms=2000, parent=None):
        ...
        if PYNVML_AVAILABLE and pynvml:
            self._attempt_nvml_init() # Initial attempt
        elif not PYNVML_AVAILABLE:
            self._gpu_name_cache = "N/A (pynvml N/A)"

    def _attempt_nvml_init(self, from_worker_loop=False):
        if self._nvml_initialized: return # Already initialized

        current_time = QTime.currentTime().secsTo(QTime(23,59,59)) # Simple timer
        if self._nvml_reinit_attempts >= self.MAX_NVML_REINIT_ATTEMPTS and \
           (current_time - self._last_nvml_reinit_attempt_time) < self.NVML_REINIT_BACKOFF_SECONDS and \
           from_worker_loop : # Apply backoff only if called from worker loop after max attempts
            # logger.debug(f"NVML: In backoff period for re-initialization. Attempts: {self._nvml_reinit_attempts}")
            return

        try:
            pynvml.nvmlInit()
            self._nvml_initialized = True
            self._nvml_reinit_attempts = 0 # Reset attempts on success
            ...
            self._last_nvml_reinit_attempt_time = current_time
            if self._nvml_reinit_attempts <= self.MAX_NVML_REINIT_ATTEMPTS or not from_worker_loop: # Log more verbosely for initial attempts or direct calls
                 logger.warning(f"Could not initialize NVML (attempt {self._nvml_reinit_attempts}): {e_nvml}")
            else: # Log less verbosely after max attempts from worker loop
                 logger.debug(f"NVML init attempt {self._nvml_reinit_attempts} failed: {e_nvml.value}")
            self._nvml_initialized = False
            error_code_str = f" (Code: {e_nvml.value})" if hasattr(e_nvml, 'value') else ""
            self._gpu_name_cache = f"NVML Init Err{error_code_str}"
        except AttributeError as e_attr:
             logger.warning(f"NVML: Attribute error during init: {e_attr}")
            ...
            self._nvml_initialized = False; self._gpu_name_cache = "NVML Unexp. Err"
    ...
    def _monitor_resources(self):
        ...
        short_sleep_ms = 100 # Check monitoring flag more often

        while self._monitoring: # Primary loop condition
            # Attempt NVML re-initialization only if not already initialized
            if PYNVML_AVAILABLE and pynvml and not self._nvml_initialized:
                self._attempt_nvml_init(from_worker_loop=True)

            # Attempt to get GPU handle if NVML is initialized but handle is missing
            if PYNVML_AVAILABLE and pynvml and self._nvml_initialized and not self._gpu_handle:
                try:
                    if pynvml.nvmlDeviceGetCount() > 0:
                        ...
                        logger.info(f"NVML: GPU handle re-acquired for {self._gpu_name_cache} in worker loop.")
                    else:
                        self._gpu_name_cache = "NVIDIA GPU N/A (Poll)"
                except pynvml.NVMLError as e_nvml_poll:
                    if self._nvml_reinit_attempts < self.MAX_NVML_REINIT_ATTEMPTS + 2: # Log handle errors a bit more
                    ...
                    self._gpu_name_cache = f"NVML Poll Err{error_code_str}"
                    self._gpu_handle = None
                    if hasattr(pynvml, 'NVML_ERROR_UNINITIALIZED') and e_nvml_poll.value == pynvml.NVML_ERROR_UNINITIALIZED:
                        self._nvml_initialized = False # Mark for re-init attempt
                except Exception as e_poll_outer: # Broader catch for unforeseen issues
                    logger.error(f"NVML: Outer unexpected error during handle acquisition: {e_poll_outer}")
                    self._gpu_handle = None # Ensure handle is None on error
                    self._nvml_initialized = False # Force re-init attempt next cycle if something went very wrong


            # --- Data Collection ---
            try:
                # Ensure we break early if monitoring stopped during NVML ops
                ...
                if self._nvml_initialized and self._gpu_handle and PYNVML_AVAILABLE and pynvml:
                    # Ensure we break early if monitoring stopped
                    if not self._monitoring: break
                    try:
                        gpu_util = pynvml.nvmlDeviceGetUtilizationRates(self._gpu_handle).gpu
                    except pynvml.NVMLError as e_nvml_util:
                        if self._nvml_reinit_attempts < self.MAX_NVML_REINIT_ATTEMPTS + 2:
                            logger.debug(f"NVML: Error getting GPU utilization: {e_nvml_util}")
                        gpu_util = -2.0 # Specific error code for utilization failure
                        error_code_str = f" (Code: {e_nvml_util.value})" if hasattr(e_nvml_util, 'value') else ""
                        gpu_name_to_emit = f"NVML Util Err{error_code_str}"
                        # Check for errors that indicate a lost handle or uninitialized state
                        if hasattr(pynvml, 'NVML_ERROR_GPU_IS_LOST') and hasattr(pynvml, 'NVML_ERROR_INVALID_ARGUMENT') and hasattr(pynvml, 'NVML_ERROR_UNINITIALIZED') and \
                           e_nvml_util.value in (pynvml.NVML_ERROR_GPU_IS_LOST,
                            ...
                            self._gpu_handle = None # Mark handle as lost
                            if e_nvml_util.value == pynvml.NVML_ERROR_UNINITIALIZED:
                                self._nvml_initialized = False # Mark NVML for re-init
                            logger.warning(f"NVML: GPU handle lost or error {e_nvml_util.value} during util fetch. Will attempt re-acquire/re-init.")
                    except Exception as e_util_other:
                        if self._nvml_reinit_attempts < self.MAX_NVML_REINIT_ATTEMPTS + 2:
                            logger.debug(f"NVML: Unexpected error getting GPU utilization: {e_util_other}")
                        gpu_util = -2.0; gpu_name_to_emit = "NVML Util Unexp. Err"
                
                # Ensure we break early if monitoring stopped before emitting
                if not self._monitoring: break
                self.resourceUpdate.emit(cpu_usage, ram_percent, gpu_util, gpu_name_to_emit)
                 ...
                 if self._monitoring: self.resourceUpdate.emit(-1.0, -1.0, -3.0, f"PSUtil Err: {str(e_psutil)[:20]}")
            except Exception as e: # General catch for unexpected errors during data collection
                logger.error(f"Error in resource monitoring data collection: {e}", exc_info=True)
                if self._monitoring:
                    self.resourceUpdate.emit(-1.0, -1.0, -3.0, f"Monitor Err: {str(e)[:20]}")

            # --- Sleep with more frequent checks ---
            # Calculate how many short sleeps fit into the desired interval
            num_short_sleeps = self.interval_ms // short_sleep_ms
            if num_short_sleeps <= 0: num_short_sleeps = 1 # Ensure at least one sleep

            for _ in range(num_short_sleeps):
                if not self._monitoring: # Check before each small sleep
                    break
        ...
        logger.info("Resource monitor worker loop finished.")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        if self.ai_chat_ui_manager and self.ai_chatbot_manager:
            initial_ai_status, initial_ai_text = self.ai_chatbot_manager.get_current_ai_status(), "Status: Initializing AI..."
            if self.ai_chatbot_manager.get_current_ai_status() == AIStatus.API_KEY_REQUIRED:
                 initial_ai_text = "Status: API Key required. Configure in Settings."
            elif self.ai_chatbot_manager.get_current_ai_status() == AIStatus.OFFLINE:
                 initial_ai_text = "Status: Offline. AI features unavailable."
            elif self.ai_chatbot_manager.get_current_ai_status() == AIStatus.INACTIVE:
    ...
    def _create_actions(self):
        ...
        self.find_item_action = QAction(get_standard_icon(QStyle.SP_FileDialogContentsView, "Find"), "&Find State...", self, shortcut=QKeySequence.Find, statusTip="Find a state by name", triggered=self.on_show_find_item_dialog)
    ...
    def _create_docks(self):
        ...
        self.ai_chatbot_dock.setVisible(False)
    ...
    def _update_window_title(self):
        ...
        main_window_is_dirty = self.isWindowModified() or self.ide_editor_is_dirty
        modified_indicator = "[*]" if main_window_is_dirty else ""
        
        title_parts = [f"{APP_NAME} - {file_name}"]
        if sim_status_suffix: title_parts.append(sim_status_suffix)
        title_parts.append(modified_indicator)
    ...
    def _update_save_actions_enable_state(self):
        # Main diagram save action enable state should only depend on diagram's dirty status
        self.save_action.setEnabled(self.isWindowModified()) # isWindowModified reflects scene._dirty
    ...
    def _prompt_save_if_dirty(self) -> bool:
        # Check main diagram dirty status first
        if self.isWindowModified() and not self.ide_editor_is_dirty: # Only diagram dirty
            ...
            if reply == QMessageBox.Save:
                if not self.on_save_file(): return False # Save failed, cancel operation
            elif reply == QMessageBox.Cancel:
                return False # User cancelled
        # If diagram is saved or wasn't dirty, then check IDE (if it's also part of main window modified logic)
        # The self.isWindowModified() in MainWindow should ideally reflect combined state.
        # For now, assuming _prompt_ide_save_if_dirty handles IDE specific part.
        return True
    ...
    def on_new_file(self, silent=False):
        if not silent and not self._prompt_save_if_dirty():
            return False
        # Also ensure IDE is prompted if relevant before clearing everything for a new diagram
        if not silent and not self._prompt_ide_save_if_dirty(): # If IDE save is cancelled, stop new file op.
             return False
        ...
        # MainWindow's modified status will be set considering both diagram and IDE by _update_window_title
        self.setWindowModified(self.ide_editor_is_dirty)
        self._update_window_title()
        self._update_undo_redo_actions_enable_state()
        if not silent:
    ...
    def on_open_file(self):
        if not self._prompt_save_if_dirty(): 
            return
        if not self._prompt_ide_save_if_dirty(): # Also prompt for IDE
            return
        ...
        if file_path:
            if self._load_from_path(file_path):
                ...
                self.undo_stack.clear()
                self.scene.set_dirty(False)
                self.setWindowModified(self.ide_editor_is_dirty) # Consider IDE state
                self._update_window_title()
                self._update_undo_redo_actions_enable_state()
    ...
    def on_save_file(self) -> bool:
        ...
        # Save diagram if it's dirty
        if self.scene.is_dirty(): # Check scene specific dirty flag
             if not self._save_to_path(self.current_file_path):
                 return False # Diagram save failed
        
        # If IDE is also dirty, user should save it via IDE save action
        # This function primarily handles diagram saving.
        return True
    ...
    def _save_to_path(self, file_path) -> bool:
        ...
        try:
            ...
            self.scene.set_dirty(False) 
            self.setWindowModified(self.ide_editor_is_dirty) # Update based on IDE state
            self._update_window_title()
            return True
        except Exception as e:
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
            ...
            self.resource_monitor_thread.quit() # Ask the thread to quit
            logger.debug("MW_CLOSE: resource_monitor_thread.quit() called.")

            # Wait for the thread to finish.
            # The worker's finished_signal is connected to thread.quit() and worker.deleteLater().
            # The thread's finished signal is connected to thread.deleteLater().
            # So, waiting for the thread should be sufficient.
            wait_time_ms = 2500 # Increased timeout slightly

            logger.debug(f"MW_CLOSE: Waiting up to {wait_time_ms}ms for resource_monitor_thread to finish...")
            if not self.resource_monitor_thread.wait(wait_time_ms): # Wait for thread to finish
                logger.warning(f"MW_CLOSE: Resource monitor thread did not quit gracefully after {wait_time_ms}ms. Attempting to terminate.")
                self.resource_monitor_thread.terminate() # Force termination if not quitting
                if not self.resource_monitor_thread.wait(500): # Wait for termination
        ...
        # Set references to None after ensuring thread cleanup attempt
        self.resource_monitor_worker = None
        self.resource_monitor_thread = None
        logger.debug("MW_CLOSE: Resource monitor worker and thread references (potentially) cleared.")
