...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        QTimer.singleShot(0, lambda: self.on_new_file(silent=True)) # Deferred to ensure full UI setup
    ...
    def _populate_dynamic_docks(self):
        ...
        for dock1_ref, dock2_ref in docks_to_tabify:
            if dock1_ref and dock2_ref:
                ...
                except Exception as e: # Catch specific Qt error if possible
    ...
    def _show_context_menu_for_item_from_scene(self, item, global_pos):
        ...
        # --- NEW: Add Breakpoint Toggle to Context Menu ---
        if isinstance(item, GraphicsStateItem) and self.py_sim_active and self.py_fsm_engine:
            menu.addSeparator()
            is_bp_set = item.text_label in self.py_fsm_engine.breakpoints["states"]
            # Use a checkable action
            bp_action = QAction(f"{'Clear' if is_bp_set else 'Set'} Breakpoint on Entry", self)
            bp_action.setCheckable(True)
            bp_action.setChecked(is_bp_set)
            # Connect its toggled signal (which passes the new checked state)
            bp_action.toggled.connect(lambda checked, s_item=item: self.on_toggle_state_breakpoint(s_item, checked))
...
if __name__ == '__main__':
    ...
    # Ensure the directory containing 'bsm_designer_project' is on sys.path
    # This is often needed if you run main.py directly from within its own folder.
    # For module execution (python -m bsm_designer_project.main), this is less critical.
    script_dir = os.path.dirname(os.path.abspath(__file__)) # bsm_designer_project
    parent_dir_of_script = os.path.dirname(script_dir)      # Project research
