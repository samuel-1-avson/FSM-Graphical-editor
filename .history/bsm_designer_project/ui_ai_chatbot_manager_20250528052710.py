    ...
    def _append_to_chat_display(self, sender: str, message: str):
        ...
        for i, part in enumerate(parts):
            if part.startswith("```") and part.endswith("```"):
                ...
                code_content = part[3:-3] # Remove ``` delimiters
                # Escape HTML within the code block but preserve newlines for <pre>
                ...
                # Basic markdown-like formatting (bold, italic, inline code)
                escaped_part = re.sub(r'\*\*(.*?)\*\*', r'<b>\1</b>', escaped_part)
                escaped_part = re.sub(r'(?<!\*)\*(?!\*)(.*?)(?<!\*)\*(?!\*)', r'<i>\1</i>', escaped_part) # Non-greedy italic
