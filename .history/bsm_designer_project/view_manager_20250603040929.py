...
class ViewManager(QObject):
    ...
    def cleanup(self):
        ...
        for signal_obj, slot_func in self._connected_signals:
            try:
                if signal_obj and not sip.isdeleted(signal_obj):
                    signal_obj.disconnect(slot_func)
            except (TypeError, RuntimeError) as e:
                logger.debug(f"ViewManager: Error/Warning disconnecting signal: {e}")
    ...
    def update_zoom_to_selection_action_enable_state(self):
        ...
        has_selection = False
        if hasattr(self.mw, 'scene') and self.mw.scene and not sip.isdeleted(self.mw.scene):
            try:
    ...
    def on_zoom_to_selection(self):
        if not self.mw or not hasattr(self.mw, 'view') or not self.mw.view or sip.isdeleted(self.mw.view) or \
           not hasattr(self.mw.view, 'zoom_to_selection'):
            logger.warning("ViewManager.on_zoom_to_selection: MainWindow, view, or view.zoom_to_selection not available/valid.")
            return
    ...
    def on_fit_diagram_in_view(self):
        if not self.mw or not hasattr(self.mw, 'view') or not self.mw.view or sip.isdeleted(self.mw.view) or \
           not hasattr(self.mw.view, 'fit_diagram_in_view'):
            logger.warning("ViewManager.on_fit_diagram_in_view: MainWindow, view, or view.fit_diagram_in_view not available/valid.")
            return
    ...
    def on_toggle_show_snap_guidelines(self, checked: bool):
        if not self.mw or not hasattr(self.mw, 'scene') or not self.mw.scene or sip.isdeleted(self.mw.scene) or \
           not hasattr(self.mw.scene, '_show_dynamic_snap_guidelines'):
            return
    ...
    def connect_zoom_actions(self):
        ...
        if hasattr(self.mw, 'zoom_to_selection_action'):
            self.mw.zoom_to_selection_action.triggered.connect(self.on_zoom_to_selection) # Connects to the method in this class
        if hasattr(self.mw, 'fit_diagram_action'):
            self.mw.fit_diagram_action.triggered.connect(self.on_fit_diagram_in_view) # Connects to the method in this class
