...
from dialogs import (StatePropertiesDialog, TransitionPropertiesDialog, CommentPropertiesDialog,
                     MatlabSettingsDialog, PreferencesDialog)

print("Python Path (sys.path):")
for p in sys.path:
    print(f"  - {p}")

from ai_chatbot import AIChatbotManager, ChatbotState # Ensure ai_chatbot.py is in the path or same directory
...
from PyQt5.QtCore import (
    ...
    QSaveFile, QIODevice, QSettings
)

from fsm_simulator import EnhancedFSMSimulator as FSMSimulator, FSMError, LogEntry, LogLevel # Ensure fsm_simulator.py is correct
...
from utils import get_standard_icon # Ensure utils.py is correct
from matlab_integration import MatlabConnection # Ensure matlab_integration.py is correct
from graphics_items import GraphicsStateItem, GraphicsTransitionItem, GraphicsCommentItem # Ensure graphics_items.py is correct
from undo_commands import (AddItemCommand, RemoveItemsCommand, MoveItemsCommand, # Ensure undo_commands.py is correct
...
class DiagramScene(QGraphicsScene):
    ...
    def __init__(self, undo_stack, parent_window=None):
        ...
        self.last_right_click_pos = QPointF()
...
class ZoomableView(QGraphicsView):
    def __init__(self, scene, parent=None):
        ...
        self.zoom_level = 0
        self.zoom_sensitivity_factor = 1.12
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._last_applied_font_size = QApplication.instance().font().pointSize()
        if self._last_applied_font_size <=0: self._last_applied_font_size = 9

        self.current_file_path = None
        self.last_generated_model_path = None
        self.matlab_connection = MatlabConnection()
        self.undo_stack = QUndoStack(self)
        self.global_style_sheet_template = STYLE_SHEET_GLOBAL
        self.ai_chatbot_manager = AIChatbotManager(parent=self) # Pass self as parent
    ...
    def _load_persistent_settings(self):
        settings = QSettings(APP_NAME, "Preferences")

        # Appearance Settings (from AppearanceSettingsTab in PreferencesDialog)
        ...
        font_size_val = settings.value("appearance/app_font_size", QApplication.instance().font().pointSize(), type=int)
        if font_size_val <= 0: font_size_val = 9 # Default if pointSize not reliable
        self._last_applied_font_size = font_size_val # Initialize this
        ...
        if app_font.pointSize() != font_size_val:
            app_font.setPointSize(font_size_val)
            QApplication.instance().setFont(app_font)
            # Global stylesheet will be applied if needed after dialog, or on startup via main
            # self.setStyleSheet("") 
            # self.setStyleSheet(self.global_style_sheet_template) # Avoid applying it directly here during load


        # General Settings (from GeneralSettingsTab in PreferencesDialog, if it existed)
        ...
        general_settings_to_apply['zoom_sensitivity'] = settings.value("general/zoom_sensitivity", 5, type=int) # Default 5 in dialog maps to 1.116 factor
        if general_settings_to_apply:
            self.apply_general_settings(general_settings_to_apply)

        # AI Settings (from AISettingsTab in PreferencesDialog)
        ...
        self.ai_chatbot_manager.set_api_key(api_key or None) # Handles empty string as None
        ...
        # MATLAB Path (from MATLABSettingsTab in PreferencesDialog)
        if hasattr(self, 'matlab_connection'):
            matlab_path = settings.value("matlab/path", "", type=str)
            if matlab_path:
                # Set path but defer connection test
                self.matlab_connection.set_matlab_path(matlab_path) 
                # Manually trigger a status update if needed, as set_matlab_path might not emit if path is "unchanged" from its perspective
                self._update_matlab_status_display(self.matlab_connection.connected, 
                                                   f"Path loaded: {matlab_path}" if matlab_path else "Path not set.")
    ...
    def apply_general_settings(self, settings_dict: dict):
        ...
        if 'zoom_sensitivity' in settings_dict:
            sensitivity_value = settings_dict['zoom_sensitivity'] # Value from 1 to 10
        ...
        self.scene.update()
        # self.log_message("Applied general settings.", type_hint="CONFIG") # Redundant if called from _load_persistent_settings
    ...
    def on_preferences(self):
        ...
        # settings_changed flag is internal to dialog, exec_() result is key
        if dialog.exec_() == QDialog.Accepted or getattr(dialog, 'settings_changed', False):
            # If font_change_pending was set by dialog's apply_all_settings, 
            # handle_pending_font_change would have been called from dialog._apply_and_accept
            # So, just updating scene should be enough here.
            self.scene.update()
            self.log_message("Preferences updated and dialog accepted/applied.", type_hint="CONFIG")
        else:
            self.log_message("Preferences dialog cancelled or no changes applied.", type_hint="CONFIG")
    ...
    def _on_send_ai_chat_message(self):
        ...
        self._update_ai_chat_status(ChatbotState.PROCESSING.value, "Sending to AI...")
        self.ai_chatbot_manager.send_message(user_message)

    def _handle_ai_response(self, ai_message_text: str, metadata: dict):
    ...
    def _handle_ai_error(self, error_message: str, error_code: str = "UNKNOWN"):
        ...
        self._update_ai_chat_status(ChatbotState.ERROR.value, f"{error_code}: {short_error}")

    def _update_ai_chat_status(self, status: str, details: str = ""):
        if details and "State changed from" in details and status != ChatbotState.ERROR.value:
            pass 
        elif details:
        ...
        is_auth_error = "AUTH_ERROR" in details or "Invalid API key" in details or \
                        (self.ai_chatbot_manager.state == ChatbotState.ERROR and "API_KEY_ERROR" in details) # Using .state property

        if status == ChatbotState.READY.value and "API key validated" in details:
            display_status_text = "Ready (API Key Validated)"
        elif status == ChatbotState.ERROR.value:
            if is_auth_error:
                display_status_text = "Error: Invalid API Key"
            else:
                detail_parts = details.split(":", 1)
                err_code_from_detail = detail_parts[0].strip() if len(detail_parts) > 0 and detail_parts[0].isupper() and "_" in detail_parts[0] else ""
                display_status_text = f"Error{f' ({err_code_from_detail})' if err_code_from_detail else ''}"
        ...
        if hasattr(self, 'ai_chat_status_label'):
            ...
            is_key_required = (status == ChatbotState.STOPPED.value and "No API key" in details) or is_auth_error
            ...
            else: current_style += f"color: {QColor(Qt.darkGreen).name()};"
    ...
    def _append_to_ai_chat_display(self, sender: str, message: str):
        ...
        default_sender_color = "#D32F2F" # A distinct error-like color
    ...
    def on_openai_settings(self):
        ...
        if dialog.exec_() == QDialog.Accepted or getattr(dialog, 'settings_changed', False):
            # self.setStyleSheet("") # Defer this to handle_pending_font_change if needed
            # self.setStyleSheet(self.global_style_sheet_template)
            self.scene.update() # Scene update is fine
            self.log_message("Preferences updated (via AI Settings menu action).", type_hint="CONFIG")
    ...
    def _update_window_title(self):
        ...
        if self.isWindowModified(): title+="[*]"
    ...
    def _update_matlab_status_display(self, connected, message):
        ...
        color_val = COLOR_PY_SIM_STATE_ACTIVE if connected else QColor('#C62828')
        self.matlab_status_label.setStyleSheet(f"font-weight: bold; padding: 0px 5px; color: {color_val.name()};")
    ...
    def _handle_matlab_modelgen_or_sim_finished(self, success, message, data):
        ...
        if success:
            if "Exporting" in self.status_label.text() and data: # Check if it was an export operation
            ...
            elif "Running Simulink simulation" in self.status_label.text(): # Check if it was a simulation
    ...
    def on_matlab_settings(self):
        dialog = PreferencesDialog(self, self)
        matlab_tab_index = -1
        if hasattr(dialog, 'matlab_settings_tab_widget'): # Check if MATLAB tab was created
            for i in range(dialog.tab_widget.count()):
                if dialog.tab_widget.tabText(i) == "MATLAB":
                    matlab_tab_index = i
                    break
        if matlab_tab_index != -1:
            dialog.tab_widget.setCurrentIndex(matlab_tab_index)
        # The old MatlabSettingsDialog is no longer needed as its functionality is in PreferencesDialog
        if dialog.exec_() == QDialog.Accepted or getattr(dialog, 'settings_changed', False):
             self.scene.update() # Scene update might be needed for grid, etc.
             self.log_message("Preferences updated (via MATLAB Settings menu action).", type_hint="CONFIG")
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.ai_chatbot_manager:self.ai_chatbot_manager.stop()
        ...
        if self.internet_check_timer and not self.internet_check_timer.isActive() and not event.isAccepted():
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        ...
        color_val = COLOR_PY_SIM_STATE_ACTIVE if is_connected else QColor('#D32F2F')
        self.internet_status_label.setStyleSheet(f"font-weight:normal;padding:0px 5px;color:{color_val.name()};")
    ...
    def _update_py_sim_status_display(self):
        if self.py_sim_active and self.py_fsm_engine:
            state_name = self.py_fsm_engine.get_current_state()
    ...
    def _append_to_py_simulation_log(self, log_entries: list[LogEntry]):
        for entry_obj in log_entries:
            ...
            if entry_obj.level in [LogLevel.ERROR, LogLevel.WARNING] or "[Condition]" in entry_msg or "[Eval Error]" in entry_msg or "[Env Error]" in entry_msg:
                cleaned_entry = f"<span style='color:{COLOR_ACCENT_SECONDARY};'>{cleaned_entry}</span>"
            elif "Transitioned from" in entry_msg or "Reset to state" in entry_msg or "Simulation started" in entry_msg or "Environment module" in entry_msg or "Called environment" in entry_msg:
                cleaned_entry = f"<span style='color:{COLOR_ACCENT_PRIMARY}; font-weight:bold;'>{cleaned_entry}</span>"
            elif "No eligible transition" in entry_msg:
        ...
        if log_entries:
            ...
            last_log_short = last_log_obj.message.split('\n')[0][:100]
            ...
            if any(keyword in last_log_short for keyword in important_keywords) or last_log_obj.level in [LogLevel.ERROR, LogLevel.WARNING]:
    ...
    def on_start_py_simulation(self):
        ...
        try:
            ...
            initial_log_entry = LogEntry(timestamp=time.time(), level=LogLevel.INFO, message="Python FSM Simulation started.")
            self._append_to_py_simulation_log([initial_log_entry])
            self._update_py_simulation_dock_ui()
        except Exception as e:
            msg = f"Failed to start Python FSM simulation:\n{type(e).__name__}: {e}"
            error_log_entry = LogEntry(timestamp=time.time(), level=LogLevel.ERROR, message=f"ERROR Starting Sim: {msg}")
            QMessageBox.critical(self, "FSM Initialization Error", msg); self._append_to_py_simulation_log([error_log_entry])
    ...
    def on_stop_py_simulation(self, silent=False):
        ...
        if not silent:
            stop_log_entry = LogEntry(timestamp=time.time(), level=LogLevel.INFO, message="Python FSM Simulation stopped.")
            self._append_to_py_simulation_log([stop_log_entry])
    ...
    def on_reset_py_simulation(self):
        ...
        try:
            ...
            error_log_entry = LogEntry(timestamp=time.time(), level=LogLevel.ERROR, message=f"ERROR DURING RESET: {msg}")
            QMessageBox.critical(self, "FSM Reset Error", msg); self._append_to_py_simulation_log([error_log_entry])
    ...
    def on_step_py_simulation(self):
        ...
        try:
            ...
            self._append_to_py_simulation_log(log_entries); self._update_py_simulation_dock_ui()
        except Exception as e:
            msg = f"Simulation Step Error: {type(e).__name__}: {e}"
            error_log_entry = LogEntry(timestamp=time.time(), level=LogLevel.ERROR, message=f"ERROR DURING STEP: {msg}")
            QMessageBox.warning(self, "Simulation Step Error", str(e)); self._append_to_py_simulation_log([error_log_entry])
    ...
    def on_trigger_py_event(self):
        ...
        try:
            ...
            self._append_to_py_simulation_log(log_entries); self._update_py_simulation_dock_ui(); self.py_sim_event_name_edit.clear()
        except Exception as e:
            msg = f"Simulation Event Error ({html.escape(event_name)}): {type(e).__name__}: {e}"
            error_log_entry = LogEntry(timestamp=time.time(), level=LogLevel.ERROR, message=f"ERROR DURING EVENT '{html.escape(event_name)}': {msg}", event=event_name)
            QMessageBox.warning(self, "Simulation Event Error", str(e)); self._append_to_py_simulation_log([error_log_entry])

    def handle_pending_font_change(self): # New method
        if self.font_change_pending:
            reply = QMessageBox.information(self, "Font Change Applied",
                                      "Application font size setting has changed.\n"
                                      "A restart is recommended for all UI elements to update correctly.\n\n"
                                      "Some elements may update now, but for full consistency, please restart the application.",
                                      QMessageBox.Ok)
            
            settings_qt = QSettings(APP_NAME, "AppearanceSettings") # Corrected from "Preferences"
            new_font_size = settings_qt.value("appearance/app_font_size", QApplication.instance().font().pointSize(), type=int)
            if new_font_size <= 0: new_font_size = 9
            ...
            if font.pointSize() != new_font_size:
                ...
                # This is the heavy operation. It's now done after the dialog is closed.
                self.setStyleSheet("") 
                self.setStyleSheet(self.global_style_sheet_template)
                self.log_message("Global font and stylesheet reapplied.", type_hint="CONFIG")
            
            self.font_change_pending = False
            self._last_applied_font_size = new_font_size
