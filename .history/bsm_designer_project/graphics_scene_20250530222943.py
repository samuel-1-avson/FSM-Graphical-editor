...
import logging
import math
import re # Added for _generate_unique_state_name
from PyQt5.QtWidgets import (
    ...
    QGraphicsSceneDragDropEvent, QApplication # Added QApplication
)
from PyQt5.QtGui import QKeyEvent, QKeySequence, QCursor # Added QKeySequence, QCursor
from PyQt5.QtGui import (
    QPainter, QColor, QPen, QMouseEvent,
    QWheelEvent, QBrush
)
from PyQt5.QtCore import Qt, QRectF, QPointF, QLineF, pyqtSignal, QPoint, QMarginsF, QEvent, QMimeData # Added QMimeData
...
MIME_TYPE_BSM_ITEMS = "application/x-bsm-designer-items" # For Copy/Paste
...
class DiagramScene(QGraphicsScene):
    ...
    def _update_transitions_for_renamed_state(self, old_name:str, new_name:str):
        ...
        # This method is more of a hook for now. The actual data update for transitions
        # (if they store names directly in their `get_data`) should be handled by the
        # EditItemPropertiesCommand for the state, which would know about the name change
        # and could iterate through transitions if necessary.
        # Visually, _update_connected_transitions handles the path redraw.
        for item in self.items():
            if isinstance(item, GraphicsTransitionItem):
                # If transition data stores source/target names, it needs update
                # However, GraphicsTransitionItem.get_data() gets names from linked items.
                # So, this might not be strictly necessary if transitions always refer to items,
                # but good for future-proofing if data model changes.
                if item.start_item and item.start_item.text_label == new_name: # This implies the state item's label is already updated
                    pass # Visually handled
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            if self.current_mode == "state":
                ...
                # Store positions if not dragging a transition control point
    ...
    def _show_context_menu(self, item, global_pos):
        ...
        if isinstance(item, GraphicsStateItem) and item.is_superstate:
            pass # Potentially add "Edit Sub-Machine" here later if not already easily accessible
    ...
    def edit_item_properties(self, item):
        from dialogs import StatePropertiesDialog, TransitionPropertiesDialog, CommentPropertiesDialog # Local import to avoid circular dependency
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        from dialogs import StatePropertiesDialog, CommentPropertiesDialog # Local import
        current_item = None
    ...
    def _handle_transition_click(self, clicked_state_item: GraphicsStateItem, click_pos: QPointF):
        from dialogs import TransitionPropertiesDialog # Local import
        dialog_parent = self.parent_window if self.parent_window else self.views()[0] if self.views() else None
    ...
    def copy_selected_items(self):
        ...
        if items_to_copy_data:
            ...
            try:
                ...
                mime_data_obj = QMimeData() # Renamed from mime_data to avoid conflict
    ...
    def paste_items_from_clipboard(self):
        clipboard = QApplication.clipboard()
        mime_data_clipboard = clipboard.mimeData() # Renamed
