...
from PyQt5.QtCore import Qt, QRectF, QPointF, QMimeData, QPoint, QLineF, QSize, pyqtSignal, QObject # Added QObject
...
# Ensure QObject is the first inherited class for proper MRO with signals/slots
class GraphicsStateItem(QObject, QGraphicsRectItem):
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
                 ...
                 parent_item=None): # QGraphicsItem can have a parent_item
        # Call QObject constructor first, then QGraphicsRectItem
        QObject.__init__(self)
        QGraphicsRectItem.__init__(self, x, y, w, h, parent_item) # Pass parent_item if provided
    ...
    # ... (rest of the GraphicsStateItem methods remain the same: paint, set_py_sim_active_style, etc.) ...
    # Make sure set_text and set_properties correctly emit nameChanged if it's defined
    ...
    def set_text(self, text):
        if self.text_label != text:
            ...
            self.text_label = text
            self.update()
            if hasattr(self, 'nameChanged'): # Check if signal exists (it should now)
...
class GraphicsTransitionItem(QObject, QGraphicsPathItem): # Inherit from QObject first
    ...
    # ... (rest of GraphicsTransitionItem methods: _compose_label_string, hoverEvents, boundingRect, shape, update_path, _get_intersection_point, paint, set_validation_highlight, get_data, set_properties, set_control_point_offset)
...
class GraphicsCommentItem(QObject, QGraphicsTextItem): # Inherit from QObject first
    ...
    def __init__(self, x, y, text="Comment", parent_item=None): # QGraphicsItem can have a parent_item
        QObject.__init__(self)
        QGraphicsTextItem.__init__(self, parent_item) # Pass parent_item
    ...
    # ... (rest of GraphicsCommentItem methods: _on_contents_changed, paint, set_validation_highlight, get_data, set_properties, itemChange)
