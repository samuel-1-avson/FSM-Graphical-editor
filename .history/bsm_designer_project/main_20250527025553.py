...
class MainWindow(QMainWindow):
    ...
    def _prompt_ide_save_if_dirty(self, tab_index: int = -1) -> bool:
        ...
        if tab_index != -1 and tab_index < self.ide_tab_widget.count(): 
            ...
            if widget and hasattr(widget, "_ide_tab_data"): # Check if widget is not None
    ...
    def _save_specific_ide_tab(self, index: int) -> bool:
        ...
        if not file_to_save_path: 
            ...
            if '.' in current_tab_text and not current_tab_text.endswith(default_ext): # Check if existing extension is different
    ...
    def on_ide_save_as_file(self) -> bool:
        ...
        if '.' in default_filename_base:
            name_part, _ = default_filename_base.rsplit('.',1) # Get name without existing extension
    ...
    def on_ide_save_all_files(self):
        ...
        for i in indices_to_save:
            ...
            if hasattr(widget, "_ide_tab_data"):
                tab_data = widget._ide_tab_data
                if tab_data["is_dirty"]:
                    if i != self.ide_tab_widget.currentIndex(): # Temporarily activate tab for dialog parenting
                        self.ide_tab_widget.setCurrentIndex(i)
