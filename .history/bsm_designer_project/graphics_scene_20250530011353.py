...
SNAP_THRESHOLD_PIXELS = 8 # Threshold for snapping to objects
...
class DiagramScene(QGraphicsScene):
    ...
    def __init__(self, undo_stack, parent_window=None):
        ...
        self.snap_to_objects_enabled = True # New flag for object snapping
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent):
        if self.current_mode == "transition" and self.transition_start_item and self._temp_transition_line:
            ...
            # TODO: Add logic here to draw temporary snap lines if snap_to_objects_enabled
            # This is for visual feedback during drag, actual snapping on release.
    ...
    def _calculate_object_snap_position(self, moving_item: QGraphicsItem, candidate_item_origin_pos: QPointF) -> QPointF:
        """
        Calculates the best snapped position for moving_item if it were placed at candidate_item_origin_pos.
        Returns a new QPointF for the item's origin.
        """
        ...
        min_offset_x = SNAP_THRESHOLD_PIXELS + 1 # Initialize with a value greater than threshold
        min_offset_y = SNAP_THRESHOLD_PIXELS + 1

        moving_item_br = moving_item.boundingRect() # Item's local bounding rect
        
        # This is the scene bounding rect if the item were placed at candidate_item_origin_pos
        ...
        for other_item in self.items():
            ...
            other_sbr = other_item.sceneBoundingRect() # Already in scene coordinates
            ...
            for moving_ref_name, moving_x_val in moving_item_refs_x.items():
                for other_x_val in other_item_snap_points_x:
                    ...
                    elif abs(diff_x) == min_offset_x: # Prefer snapping to existing current_best_x if equally close
                        pass
        ...
        # Only apply snap if it's within the threshold
        ...
        if event.button() == Qt.LeftButton and self.current_mode == "select":
            if self._mouse_press_items_positions:
                ...
                for item, old_pos in self._mouse_press_items_positions.items():
                    current_item_pos_after_drag = item.pos() # This is where the item is after QGraphicsScene handles move
                    ...
                    # 2. Then try grid snapping on the (potentially) object-snapped position
