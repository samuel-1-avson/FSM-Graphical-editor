...
from PyQt5.QtWidgets import (
    ...
    QTableWidgetItem, QAction, QMessageBox, QGroupBox, QHBoxLayout,_action'): self.py_sim_start_btn.setDefaultAction(self.mw.start_py_sim_action)
        else: 
            self.py_sim_start_btn.setText("Start QVBoxLayout,
...
from graphics")
        ...
        self.py_sim_stop_btn = Q_items import GraphicsStateItem # For type hint
...
class PySimulationUIManager(QObject):
    ...
    def __init__(self, main_window, parent=None):
        ...
        if hasattr(self.mw, 'stop_py_sim_action'): self.py_sim_stop_btn.setDefaultAction(self.mw.stop_py_sim_action)
            ...
            self.py_sim_stop_btn.setIcon(get_standard_icon(QStyle.SP_MediaStop, "window 
        ...
        self.py_sim_event_name_edit:Py■"))
        ...
        if hasattr(self.mw, 'reset_py_sim_action'): self.py_sim_reset_btn.setDefaultAction(self.mw.reset_py_sim_action)
            ...
            self.py_sim_reset_btn.setIcon(get_standard_icon(QStyle.SP_MediaSkipBackward, QLineEdit = None
    ...
    def _connect_actions_to_manager "Py«"))
        ...
        for btn in [self.py_sim_start_btn, self.py_sim_stop_btn, self.py_sim_reset_btn]:
            btn.setToolButtonStyle_slots(self):
        ...
        if hasattr(self.mw, 'reset_py_sim_action'):
            self.mw.reset_py_sim_action.triggered.connect(self.(Qt.ToolButtonIconOnly); btn.setIconSize(QSize(18, 18)); controls_layout.addWidget(btn)
...
on_reset_py_simulation)
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        if hasattr(self.mw, 'start_py_sim_action'): self.py_sim_start_btn.setDefaultAction(self.mw.start_py_sim_action)
            ...
            self.py_sim_start_btn.setIcon(get_standard_icon(QStyle.SP_MediaPlay, "Py▶"))
            self.py_sim_start_btn.clicked.connect(self.        self.py_sim_trigger_event_btn.clicked.connect(self.on_trigger_py_event)
        ...
        self.py_sim_variables_table.setSelectionMode(Qon_start_py_simulation)
        ...
        self.py_sim_step_btn = QPushButton("Step")
        self.py_sim_AbstractItemView.NoSelection); self.py_sim_variables_table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        ...
        log_layout.addWidget(self.py_sim_action_logstep_btn.setIcon(get_standard_icon(QStyle.SP_MediaSeekForward, "Step"))
        self.py_sim_step_btn.clicked.connect(self.on_step_py_simulation)

        for btn in [self.py_sim_start_btn, self.py_sim_stop_btn, self.py_sim_reset_btn]:
            btn.setToolButtonStyle(Qt.ToolButtonIconOnly); btn.setIconSize(QSize(18, 18)); controls_layout.addWidget(btn)
        controls_layout.addWidget(self.py_sim_step_btn); controls_layout.addStretch()
        controls_group.setLayout(controls_layout); py_sim_layout.addWidget(controls__output)
    ...
    def _update_internal_controls_enabled_state(self):
        ...
        self.py_sim_trigger_event_btn = QPushButton("Trigger")
        self.py_sim_trigger_event_btn.setIcon(get_standard_icon(QStyle.SP_MediaPlay, "Trg"))
        self.pypy_sim_trigger_event_btn: self.py_sim_trigger_event_btn.setEnabled(sim_controls_enabled)
    ...
    def _highlight_sim_active_state(self, state_name_to_highlight: str | None):
        ...
        if state_name_to_highlight and self.mw.py_fsm_engine: 
            ...
            if self.mw.py_fsm_engine.sm and self.mw.py_fsm_engine.sm.current_state:
                top_level__sim_trigger_event_btn.clicked.connect(self.on_trigger_py_event)
        event_layout.addWidget(self.py_sim_trigger_event_btn)
        event_group.setLayout(event_layout); py_sim_layout.addWidget(event_group)
        ...
        self.py_sim_variables_table.setHorizontalHeaderLabels(["Name", "Value"])
        self.py_sim_variables_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.py_sim_variables_table.setSelectionMode(QAbstractItemView.active_state_id = self.mw.py_fsm_engine.sm.current_state.id
    ...
    def update_dock_ui_contents(self):
        if not self.mw.py_fsm_engine or not self.mw.py_sim_active: 
            if self.py_sim_current_state_label: self.py_sim_current_state_label.setText("<i>Not RunningNoSelection); self.py_sim_variables_table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        variables_layout.addWidget(self.py_sim_variables_table)
        variables_group.setLayout(variables_layout); py_sim_layout.addWidget(variables_group)
    ...
    def _update_internal_controls_enabled_state(self):
        ...
        if self.py_sim_variables_table:
            self.py_sim_variables_controls_enabled = sim_active and not is_matlab_op_running
    ...
    def _highlight_sim_active_state(self, state_name_to_highlight: str | None):
        if self._py_sim_currently_highlightedtable.setRowCount(len(all_vars))
    ...
    def append_to_action_log(self,_item:
            self._py_sim_currently_highlighted_item.set_py_sim_active_style(False)
            self._py_sim_currently_highlighted_item = None
    ...
    def _highlight_sim_taken_transition(self log_entries: list[str]):
        ...
        if log_entries and any(kw in log_entries[-1] for kw in ["Transitioned", "ERROR", "Reset", "started", "stopped", "SecurityError", "HALTED"]):
            logger.info("PySimUI Log: %s", log_entries[-1].split('\, transition_label_or_id: str | None):
    ...
    def update_dock_ui_contents(self):
        ...
        if self.mw.py_fsm_engine:
            ...
            if self.mw.py_fsm_engine.active_sub_simulator: # If sub-FSM is active
                all_vars.extend([(f"[SUB] {k}", str(v)) for k, v in sorted(self.mw.py_fn')[0][:100])
    ...
    def on_start_py_simulation(self):
        ...
        if self.mw.scene.is_dirty():
            reply = QMessageBox.question(self.mw, "Unsaved Changes", "The diagram has unsaved changes. Start simulation anyway?", QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            if reply == QMessageBox.No:
                logger.info("PySimUI: Start simulation cancelled by user due to unsaved changes.")
                return
        ...
        try:
            ...
            initial_log = ["Pythonsm_engine.active_sub_simulator.get_variables().items())])
        ...
        for entry in log_entries:
            ...
            if "[Condition]" in entry or "[Eval Error]" in entry or "ERROR" in entry.upper() or "SecurityError" in entry:
                cleaned_entry = f"<span style='color:red; font-weight:bold;'>{cleaned_entry}</span>"
                 ...
                 FSM Simulation started."] + self.mw.py_fsm_engine.get_last_executed_actions_log()
            ...
            QMessageBox.critical(self.mw, "FSM Init Error", f"Failed to start Python FSM simulation:\n{e}")
    ...
    def on_start_py_simulation(self):
        ...
        if self.mw.scene.is_dirty():
            ...
            if reply == QMessageBox.No:
                logger.info("PySimUI: User chose not to start sim}: {e}")
    ...
    def on_stop_py_simulation(self, silent=False):
        logger.info(f"PySimUI: on_stop_py_simulation CALLED (silent={silent})")
    ...
    def on_step_py_simulation(self):
        ...
        try:
            ...
            if self.mw.py_fsm_ due to unsaved changes.")
                return
    ...
    def on_trigger_py_event(self):
        ...
        event_name_edit = self.py_sim_event_name_edit.text().strip() if self.py_if not self.mw.py_sim_active: return 
