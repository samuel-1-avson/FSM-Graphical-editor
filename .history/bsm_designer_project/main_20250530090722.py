...
class MainWindow(QMainWindow):
    ...
    def _create_actions(self):
        ...
        # Using placeholder icons, actual icons might need to be sourced or created for better representation
        self.distribute_h_action = QAction(get_standard_icon(QStyle.SP_ArrowLeft, "DstH"), "Distribute Horizontally", self, statusTip="Distribute selected items horizontally", triggered=lambda: self.on_distribute_items("horizontal")) # Placeholder icon
        self.distribute_v_action = QAction(get_standard_icon(QStyle.SP_ArrowUp, "DstV"), "Distribute Vertically", self, statusTip="Distribute selected items vertically", triggered=lambda: self.on_distribute_items("vertical")) # Placeholder icon
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
            ...
            if self.resource_monitor_worker:
                ...
                logger.debug("MW_CLOSE: Resource worker _monitoring flag set to False.")
                
            self.resource_monitor_thread.quit()
            logger.debug("MW_CLOSE: resource_monitor_thread.quit() called.")

            wait_time_ms = self.resource_monitor_worker.interval_ms + 500 if self.resource_monitor_worker and hasattr(self.resource_monitor_worker, 'interval_ms') else 2500
        ...
        if self.resource_monitor_worker and hasattr(self.resource_monitor_worker, '_nvml_initialized') and self.resource_monitor_worker._nvml_initialized:
            logger.debug("MW_CLOSE: Ensuring NVML shutdown by directly calling worker's stop_monitoring.")
    ...
    def on_align_items(self, mode: str):
        ...
        if mode == "left":
            ...
            for item in selected_items:
                item.setPos(ref_x, item.y()) # Set X, keep Y
        elif mode == "center_h":
            ref_x_center = overall_selection_rect.center().x()
            for item in selected_items:
                item_br = item.sceneBoundingRect() # Get SBR before changing pos
            ...
            for item in selected_items:
                item.setPos(item.x(), ref_y) # Set Y, keep X
        elif mode == "middle_v":
    ...
    def on_distribute_items(self, mode: str):
        ...
        if mode == "horizontal":
            ...
            # Use sceneBoundingRect for min/max to get actual positions
            min_x = selected_items[0].sceneBoundingRect().left()
            max_x_right_edge = selected_items[-1].sceneBoundingRect().right()
            
            total_width_of_items = sum(item.sceneBoundingRect().width() for item in selected_items)
            # Total span covered by the items from the leftmost point of the first to rightmost of last
            actual_span = max_x_right_edge - min_x
            
            if len(selected_items) <= 1: spacing = 0
            else: spacing = (actual_span - total_width_of_items) / (len(selected_items) - 1)

            if spacing < 0: # Overlapping, so distribute with minimal (e.g., 0) spacing
                spacing = 0 # Or a small positive value like self.scene.grid_size / 2
                logger.warning("Distribute Horizontal: Items wider than span, distributing with minimal spacing.")


            current_x_origin = selected_items[0].scenePos().x() # Keep the first item's origin x
            # The first item's y is its original y
            selected_items[0].setPos(current_x_origin, old_positions_map[selected_items[0]].y()) 

            for i in range(1, len(selected_items)):
                prev_item = selected_items[i-1]
                current_item = selected_items[i]
                # New origin for current_item is prev_item's right edge + spacing
                new_x_origin = prev_item.sceneBoundingRect().right() + spacing
                current_item.setPos(new_x_origin, old_positions_map[current_item].y()) # Keep original Y
        
        elif mode == "vertical":
            selected_items.sort(key=lambda item: item.sceneBoundingRect().top())
            min_y = selected_items[0].sceneBoundingRect().top()
            max_y_bottom_edge = selected_items[-1].sceneBoundingRect().bottom()

            total_height_of_items = sum(item.sceneBoundingRect().height() for item in selected_items)
            actual_span = max_y_bottom_edge - min_y

            if len(selected_items) <= 1: spacing = 0
            else: spacing = (actual_span - total_height_of_items) / (len(selected_items) - 1)

            if spacing < 0:
                spacing = 0
                logger.warning("Distribute Vertical: Items taller than span, distributing with minimal spacing.")

            current_y_origin = selected_items[0].scenePos().y() # Keep the first item's origin y
            selected_items[0].setPos(old_positions_map[selected_items[0]].x(), current_y_origin) # Keep original X

            for i in range(1, len(selected_items)):
                prev_item = selected_items[i-1]
                current_item = selected_items[i]
                new_y_origin = prev_item.sceneBoundingRect().bottom() + spacing
                current_item.setPos(old_positions_map[current_item].x(), new_y_origin) # Keep original X
