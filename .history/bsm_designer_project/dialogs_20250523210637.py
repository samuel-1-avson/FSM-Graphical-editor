...
from PyQt5.QtWidgets import (
    ...
    QTabWidget, QWidget, QApplication, QMessageBox
)
...
class MatlabSettingsDialog(QDialog):
    ...
    def _browse(self):
        ...
        if path:
            ...
            # Don't auto-test, just update status. User can click "Test" or "Apply"
    ...
    def _test_connection_and_update_label(self):
        ...
        QApplication.processEvents() # Allow UI to update before potentially long op
        
        # Set path first, then test. set_matlab_path might change self.connected
        current_path_valid = self.matlab_connection.set_matlab_path(path)
        if current_path_valid: # If path is structurally okay
            self.matlab_connection.test_connection() # This will emit connectionStatusChanged
        # else: set_matlab_path already emitted a failure status

    def _update_test_label_from_signal(self, success: bool, message: str):
        status_prefix = "Status: "
        style = "font-weight:bold;padding:3px;"
        if success:
            if "path set" in message.lower() and "test successful" not in message.lower(): 
                 status_prefix = "Status: Path valid. " # Path looks okay, but not fully tested yet
                 style += "color:#FFA000;" # Orange for "valid but not tested"
            elif "test successful" in message.lower() or "connected" in message.lower(): 
                status_prefix = "Status: Connected! "
                style += "color:#2E7D32;" # Green for connected
            else: # Other success messages
                style += "color:#2E7D32;"
        else:
            status_prefix = "Status: Error. "
            style += "color:#C62828;" # Red for error
        
        self.test_status_label.setText(status_prefix + message)
        self.test_status_label.setStyleSheet(style)
        
        # If auto-detect was successful and path_edit is empty, fill it
    ...
    def _apply_settings(self):
        ...
        # If the path text is different from the connection's current path,
        # or if the path text is not empty and the connection is not currently marked as 'connected'.
        if self.matlab_connection.matlab_path != path or (path and not self.matlab_connection.connected):
            self.matlab_connection.set_matlab_path(path) # Update path, might change self.connected based on basic validation
            # Only test if path seems valid and we are not already connected, or if path changed
            if path and self.matlab_connection.matlab_path == path: # set_matlab_path was successful
                 if not self.matlab_connection.connected or self.matlab_connection.matlab_path != path : # Test if not connected yet, or if path genuinely changed
                    self.matlab_connection.test_connection() # This will update self.connected and emit signal
        self.accept()


class AppearanceSettingsTab(QWidget):
    def __init__(self, main_window_instance, parent=None): # Changed main_window to main_window_instance
        super().__init__(parent)
        self.main_window_instance = main_window_instance # Changed main_window to main_window_instance
    ...
    def _get_current_app_settings(self):
        ...
        scene = self.main_window_instance.scene # Use main_window_instance
        default_font_size = 9
        app_font = QApplication.instance().font()
        if app_font.pointSize() > 0:
            default_font_size = app_font.pointSize()
        elif app_font.pixelSize() > 0 and self.logicalDpiY() > 0: # logicalDpiY needs self as a QWidget
             # Ensure self (AppearanceSettingsTab) is a QWidget for logicalDpiY to be available
    ...
    def apply_specific_settings(self, data, mainWindowInstance): # Pass main_window_instance
        scene = mainWindowInstance.scene # Use passed instance
        settings_qt = QSettings(APP_NAME, "AppearanceSettings")
        ...
        if current_app_font_size != new_font_size:
            mainWindowInstance.font_change_pending = True # Set flag
        
        # Update scene item fonts immediately as it's less disruptive
        # Only do this if font actually changed or if a global font change is pending from a previous apply
        if mainWindowInstance.font_change_pending or \
           (hasattr(mainWindowInstance, '_last_applied_font_size') and mainWindowInstance._last_applied_font_size != new_font_size):
            ...
            scene.update()
            if hasattr(mainWindowInstance, '_last_applied_font_size'):
                mainWindowInstance._last_applied_font_size = new_font_size
...
class PreferencesDialog(QDialog):
    def __init__(self, main_window, parent=None):
        ...
        self.settings_changed = False # Tracks if any "Apply" has happened
    ...
    def _create_matlab_settings_tab(self):
        ...
        # Connect signal for live updates if matlab_connection status changes from elsewhere
        self.main_window.matlab_connection.connectionStatusChanged.connect(
            lambda connected, msg: self.matlab_test_status_label_tab.setText(f"Status: {msg}")
        )
    ...
    def _browse_matlab_path_tab(self):
        ...
        if path: 
            ...
            self.matlab_test_status_label_tab.setText(f"Path changed to: {path}. Apply to save.")

    def _auto_detect_matlab_path_tab(self):
        self.matlab_test_status_label_tab.setText("Auto-detecting...")
        QApplication.processEvents()
        # Temporarily disconnect to avoid recursive signals if detect_matlab -> set_matlab_path -> connectionStatusChanged
        try:
            self.main_window.matlab_connection.connectionStatusChanged.disconnect()
        ...
        if self.main_window.matlab_connection.detect_matlab():
            ...
            self.matlab_test_status_label_tab.setText(f"Detected: {detected_path}. Apply to save.")
        else:
    ...
    def apply_all_settings(self):
        ...
        self.appearance_tab.apply_specific_settings(appearance_data, self.main_window) # Pass main_window
        ...
        if hasattr(self, 'matlab_settings_tab_widget'):
            ...
            # Only set the path in the connection object. Don't test here.
            if self.main_window.matlab_connection.matlab_path != new_matlab_path:
                 if self.main_window.matlab_connection.set_matlab_path(new_matlab_path): # This should be quick
                     self.matlab_test_status_label_tab.setText(f"Path set to: {new_matlab_path if new_matlab_path else 'None'}. Test in MATLAB Settings dialog if needed.")
                 else:
                     self.matlab_test_status_label_tab.setText(f"Invalid path: {new_matlab_path}. Path not set.")
        
        self.settings_changed = True # Mark that settings were applied
        self.main_window.log_message("Preferences applied.", type_hint="CONFIG")

    def _apply_and_accept(self):
        self.apply_all_settings()
        self.accept()
        # Check and handle pending font changes after the dialog is closed.
        if hasattr(self.main_window, 'font_change_pending') and self.main_window.font_change_pending:
            self.main_window.handle_pending_font_change()