...
class MatlabConnection(QObject):
    ...
    def set_matlab_path(self, path):
        old_path_attempt = path.strip() if path else "" # Store the path that was attempted
        ...
        if self.matlab_path and os.path.exists(self.matlab_path) and \
           (os.access(self.matlab_path, os.X_OK) or self.matlab_path.lower().endswith('.exe')):
            ...
            if old_path_attempt: # If a path was attempted and found invalid
...
class ResourceMonitorWorker(QObject):
    ...
    def __init__(self, interval_ms=2000, parent=None):
        ...
        if PYNVML_AVAILABLE and pynvml:
            ...
            except AttributeError as e_attr: # Specifically catch if .decode is the issue
                 logger.warning(f"NVML: Attribute error during init (possibly on .decode): {e_attr}")
                 self._nvml_initialized = False
                ...
                if PYNVML_AVAILABLE and pynvml and self._nvml_initialized and not self._gpu_handle:
                    try:
                        ...
                        if e_nvml_poll.value == pynvml.NVML_ERROR_UNINITIALIZED: self._nvml_initialized = False
                         ...
                         logger.warning(f"NVML: Attribute error getting GPU handle (possibly on .decode): {e_attr}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        if self.ai_chat_ui_manager: # Check if manager exists
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.resource_monitor_worker and self.resource_monitor_thread:
            ...
            if self.resource_monitor_thread.isRunning():
                ...
                wait_time = 200 # Default wait time if interval_ms not found
                if hasattr(self.resource_monitor_worker, 'interval_ms'):
                     wait_time = self.resource_monitor_worker.interval_ms + 200 # Wait for one cycle + buffer
                else: # Fallback if interval_ms is not on worker, use a reasonable default
