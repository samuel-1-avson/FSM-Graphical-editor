...
class ChatbotWorker(QObject):
    ...
    # Renaming the method slightly for invokeMethod, and using pyqtSlot name for original
    @pyqtSlot(str, name='setApiKeyInWorker') 
    def _set_api_key_slot(self, api_key: str): # Python method name can differ from slot name
        logger.info(f"WORKER: _set_api_key_slot called (new key {'SET' if api_key else 'NOT SET'}).")
        self.api_key = api_key
        self._initialize_client()

    @pyqtSlot(str, name='setCurrentDiagramContextInWorker')
    def _set_current_diagram_context_slot(self, diagram_json_str: str): 
    ...
    @pyqtSlot(str, bool, name='processMessageInWorker')
    def _process_message_slot(self, user_message: str, force_fsm_generation: bool = False):
        logger.info(f"WORKER_PROCESS: _process_message_slot CALLED for: '{user_message[:50]}...' (force_fsm_generation={force_fsm_generation})")
        
        # ... (rest of the process_message logic remains IDENTICAL) ...
        self._current_processing_had_error = False 
