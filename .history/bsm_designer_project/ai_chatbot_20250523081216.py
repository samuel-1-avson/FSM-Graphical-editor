import openai
from PyQt5.QtCore import QObject, pyqtSignal, QThread, QTimer, QTime # Added QTime for timestamped prints
import os 
# In ai_chatbot_manager.py (or wherever AIChatbotManager is defined)
...
class AIChatbotManager:
    ...
    errorOccurred = pyqtSignal(str)
    statusUpdate = pyqtSignal(str) # Add this if not present for generic status
    ...
    def __init__(self, parent_window):
        ...
        self.parent_window = parent # Store parent for easier access to scene later
        self.api_key = None
        self.conversation_history = []
        self.worker_thread = None
        self.worker = None
        # Default system message can be simple for now
    ...
    def send_message(self, user_message: str):
        current_time = QTime.currentTime().toString('hh:mm:ss.zzz')
        print(f"--- MGR_SEND_MSG ({current_time}): Request for: '{user_message}' ---")
        if not self.api_key:
            print(f"--- MGR_SEND_MSG ({current_time}): No API Key. ---")
            if hasattr(self.parent_window, '_handle_ai_error'):
                self.parent_window._handle_ai_error("API Key not set. Configure in AI Assistant Settings.")
            return
        
        if not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning():
            print(f"--- MGR_SEND_MSG ({current_time}): Worker/Thread not ready. Attempting setup. ---")
            self._setup_worker() 
            if not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning():
                 print(f"--- MGR_SEND_MSG ({current_time}): Worker/Thread setup FAILED. ---")
                 if hasattr(self.parent_window, '_handle_ai_error'):
                    self.parent_window._handle_ai_error("Chatbot is not ready. Try setting API key again.")
                 return
        
        print(f"--- MGR_SEND_MSG ({current_time}): Queuing worker.process_message. ---")
    ...
    def stop_chatbot(self):
        current_time = QTime.currentTime().toString('hh:mm:ss.zzz')
        print(f"--- MGR ({current_time}): stop_chatbot called. ---")
        self._cleanup_existing_worker_and_thread()