...
class ResourceMonitorWorker(QObject):
    ...
    def stop_monitoring(self):
        ...
        logger.debug("ResourceMonitorWorker: Stop event set.")
        ...
        while not self._stop_requested_event.is_set(): 
            ...
            if (current_loop_time - last_data_emit_time) >= self.data_collection_interval_ms or last_data_emit_time == 0:
                ...
                try:
                    ...
                    if self._stop_requested_event.is_set(): break
                    # Emit only if still monitoring (self._monitoring was removed, rely on event)
                    ...
                    last_data_emit_time = current_loop_time
                     ...
                     if not self._stop_requested_event.is_set(): self.resourceUpdate.emit(-1.0, -1.0, -3.0, "Data Error")
            
            if self._stop_requested_event.wait(timeout=self.WORKER_LOOP_CHECK_INTERVAL_MS / 1000.0):
                logger.debug("ResourceMonitorWorker: Stop event detected during short wait, breaking loop.")
                break
...
class MainWindow(QMainWindow):
    ...
    def _init_resource_monitor(self):
        ...
        self.resource_monitor_thread.finished.connect(self.resource_monitor_worker.deleteLater) # Worker deleted when thread finishes
