...
class MainWindow(QMainWindow):
    ...
    def _handle_fsm_data_from_ai(self, fsm_data: dict, source_message: str):
        ...
        reply = QMessageBox.question(self, "Add AI Generated FSM",
                                     ...
                                     "Clear and Add", "Add to Existing", "Cancel", 0, 2) 
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        # For a fully undoable "Clear and Add", a more complex command structure is needed.
        # This simplified version clears the undo stack if `clear_current_diagram` is true.
        ...
        # Start a macro command if we are not clearing (or if clear was handled and stack is fresh)
        # If clearing, on_new_file already cleared the stack.
        # If not clearing, we want to group all additions.
        ...
        if not clear_current_diagram and items_to_add_for_undo_command: # Only end macro if we started one and added items
            self.undo_stack.endMacro()
        elif not clear_current_diagram and not items_to_add_for_undo_command: # No items added, cancel empty macro
             self.undo_stack.endMacro() # End it, then check if it's empty to potentially remove
             if self.undo_stack.count() > 0 and self.undo_stack.command(self.undo_stack.count() -1).childCount() == 0:
                 # This is a bit hacky. QUndoStack doesn't have a direct "remove last if empty macro"
                 # We might need to live with an empty macro in undo history or implement custom macro command.
                 # For now, an empty macro is benign.
                 pass
    ...
    def _update_ai_chat_status(self, status_text: str):
        if hasattr(self, 'ai_chat_status_label'):
            ...
            is_key_required = "api key required" in status_text.lower() or \
                              ...
                              "api key error" in status_text.lower() # Added check for key error
    ...
    def _get_bundled_file_path(self, filename: str) -> str | None:
        try:
            ...
            possible_paths = [
                ...
                os.path.join(base_path, 'examples', filename) # Added examples to search path
            ]
            for path_to_check in possible_paths:
                if os.path.exists(path_to_check):
                    return path_to_check
            # If filename already contains a path component (like 'examples/traffic_light.bsm')
            # check relative to base_path without adding another subdirectory
    ...
    def _open_example_file(self, filename: str):
        ...
        example_file_path = self._get_bundled_file_path(filename) # Filename might be "examples/traffic_light.bsm"
    ...
    def _update_py_simulation_dock_ui(self):
        ...
        elif not possible_events: # No events, default to "None"
