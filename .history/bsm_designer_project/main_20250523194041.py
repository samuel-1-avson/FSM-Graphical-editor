...
from config import (
    ...
    COLOR_PY_SIM_STATE_ACTIVE, COLOR_BACKGROUND_LIGHT, COLOR_GRID_MINOR, COLOR_GRID_MAJOR, # COLOR_PY_SIM_STATE_ACTIVE_STR is not needed if COLOR_PY_SIM_STATE_ACTIVE is QColor
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Worker signals (responseReady, errorOccurred, statusUpdate from worker) are connected
        # to MainWindow methods inside AIChatbotManager if parent_window is set.
    ...
    def _load_persistent_settings(self):
        ...
        # For now, these are directly loaded here.
        ...
        elif api_key: # If API key was set, ensure current model config is pushed to worker
             self.ai_chatbot_manager.update_config({'model_name': model_name})
    ...
    def on_preferences(self):
        ...
        # settings_changed attribute might not be standard, exec_() result is usually enough
    ...
    def _update_matlab_status_display(self, connected, message):
        ...
        color_str = COLOR_PY_SIM_STATE_ACTIVE.name() if connected else '#C62828' # Use .name() for QColor
        self.matlab_status_label.setStyleSheet(f"font-weight: bold; padding: 0px 5px; color: {color_str};")
    ...
    def on_matlab_settings(self):
        ...
        else: # Fallback to old dialog if MATLAB tab isn't in PreferencesDialog for some reason
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        ...
        color_str = COLOR_PY_SIM_STATE_ACTIVE.name() if is_connected else '#D32F2F' # Use .name() for QColor
        self.internet_status_label.setStyleSheet(f"font-weight:normal;padding:0px 5px;color:{color_str};")
    ...
    def _update_py_sim_status_display(self):
        if self.py_sim_active and self.py_fsm_engine:
            ...
            self.py_sim_status_label.setStyleSheet(f"font-weight: bold; padding: 0px 5px; color: {COLOR_PY_SIM_STATE_ACTIVE.name()};") # Use .name()
        else:
            self.py_sim_status_label.setText("PySim: Idle")
            self.py_sim_status_label.setStyleSheet("font-weight: normal; padding: 0px 5px;") # Standard, no dynamic QColor
