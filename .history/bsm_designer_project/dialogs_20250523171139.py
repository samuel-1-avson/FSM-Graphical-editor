...
class MatlabSettingsDialog(QDialog):
    def __init__(self, matlab_connection: MatlabConnection, parent=None):
        ...
        # Connect signals and set initial status
        ...
        if self.matlab_connection.matlab_path and self.matlab_connection.connected:
            ...
            self._update_test_label_from_signal(False, "Path previously set, connection unconfirmed.")
        else:
    ...
    def _browse(self):
        exe_filter = ("MATLAB Executable (matlab.exe)" 
                     if sys.platform == 'win32' 
                     else "MATLAB Executable (matlab);;All Files (*)")
        start_dir = QDir.homePath()
        
        if (self.path_edit.text() and 
            QDir(QDir.toNativeSeparators(self.path_edit.text())).exists()):
...
class GeneralSettingsDialog(QDialog):
    ...
    def _create_ai_settings_tab(self):
        ...
        current_api_key = settings_qt.value(
            "api_key", 
            self.main_window.ai_chatbot_manager.api_key or "", 
            type=str)
        ...
        self.model_name_combo_tab.addItems([
            "gpt-3.5-turbo", 
            "gpt-4",
            "gpt-4-turbo-preview",
            "gpt-4o"
        ...
        if self.main_window.ai_chatbot_manager.chatbot_worker:
        ...
        layout.addRow(QLabel(
            "<small><i>API key changes apply immediately. "
            "Model changes apply on next new chat or app restart.</i></small>"))
    ...
    def _create_matlab_settings_tab(self):
        ...
        current_matlab_path = settings_qt.value(
            "path", 
            self.main_window.matlab_connection.matlab_path or "", 
            type=str)
        ...
        path_browse_layout.addWidget(self.matlab_path_edit_tab, 1)
        browse_btn = QPushButton(get_standard_icon(QStyle.SP_DirOpenIcon, "Brw"), " Browse...")
    ...
    def _browse_matlab_path_tab(self):
        ...
        if (self.matlab_path_edit_tab.text() and 
            QDir(QDir.toNativeSeparators(self.matlab_path_edit_tab.text())).exists()):
            path_obj = QDir(self.matlab_path_edit_tab.text())
            path_obj.cdUp()
            start_dir = path_obj.absolutePath()
        
        path, _ = QFileDialog.getOpenFileName(self, "Select MATLAB Executable", start_dir, exe_filter)
        if path:
            self.matlab_path_edit_tab.setText(path)
            self.matlab_test_status_label_tab.setText(f"Path changed to: {path}")
    ...
    def apply_all_settings(self):
        ...
        if (self.main_window.ai_chatbot_manager.chatbot_worker and 
            ...
            self.main_window.log_message(
                f"AI Chat Model set to: {new_model_name}. "
                "(Effective on next new chat/restart)", 
                type_hint="AI_CONFIG")
        ...
        if hasattr(self, 'matlab_settings_tab_widget'):
            ...
            if self.main_window.matlab_connection.matlab_path != new_matlab_path:
                if self.main_window.matlab_connection.set_matlab_path(new_matlab_path):
                    self.matlab_test_status_label_tab.setText(
                        f"Path set to: {new_matlab_path if new_matlab_path else 'None'}. "
                        "Test if needed.")
                else:
                    self.matlab_test_status_label_tab.setText(
                        f"Invalid path: {new_matlab_path}. Path not set.")
