--- START OF FILE bsm_designer_project/ui_py_simulation_manager.py ---
...
from PyQt5.QtCore import QObject, pyqtSlot, pyqtSignal, QSize, Qt, QTime
...
class PySimulationUIManager(QObject):
                    ...
                    if isinstance(item, GraphicsStateItem) and item.text_label == top_level_active_state_id:
                        ...
                        if self.mw.view: 
                             if hasattr(self.mw.view, 'ensureVisible') and callable(self.mw.view.ensureVisible):
                                # Try to ensure visible with margin, if not possible, center on it.
                                # The QGraphicsView.ensureVisible method does not return a bool indicating success.
                                # We will just call it. If the item is already largely visible, it might not scroll.
                                self.mw.view.ensureVisible(item, 50, 50) # 50px margin
                                # Forcing a centerOn might be too disruptive if item is already visible.
                                # Consider only centering if ensureVisible causes no scroll or item is still mostly off-screen.
                                # This part is hard to perfect without more complex viewport calculations.
                                # A simple centerOn might be okay for now if ensureVisible isn't aggressive enough.
                                # self.mw.view.centerOn(item) # Potentially too aggressive
                             else: 
    ...
    def update_dock_ui_contents(self):
        ...
        if self.py_sim_current_state_label:
            ...
            # Determine a good text color based on background luminance for better contrast
            text_lum = QColor(active_text_color).lightnessF() if 'active_text_color' in locals() else 0.0
    ...
    def append_to_action_log(self, log_entries: list[str]):
        ...
        warning_color = QColor(COLOR_ACCENT_WARNING).darker(10).name() # Darken warning for better readability on light bg
        ...
        if log_entries and any(kw in log_entries[-1] for kw in ["Transitioned", "ERROR", "Reset", "started", "stopped", "SecurityError", "HALTED", "Entering state", "Exiting state"]):
    ...
    def on_trigger_py_event(self):
        ...
        if not event_to_trigger:
            self.on_step_py_simulation() # Treat as an internal step if no event specified
            return
        
        # Add the triggered event to the log manually before it's processed by the engine
...
--- END OF FILE bsm_designer_project/ui_py_simulation_manager.py ---