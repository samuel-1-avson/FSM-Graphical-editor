...
from PyQt5.QtCore import Qt, QDir, QSize, QTime, QObject, pyqtSlot
from PyQt5.QtGui import QIcon, QColor

from code_editor import CodeEditor # Assuming CodeEditor is in the same directory or correctly pathed
from config import (
...
class IDEManager(QObject):
    def __init__(self, main_window):
        ...
        # References to QActions created in MainWindow, for enabling/disabling
        self.ide_run_script_action_ref: QAction | None = None
        self.ide_analyze_action_ref: QAction | None = None
        self.ide_save_file_action_ref: QAction | None = None
        self.ide_save_as_file_action_ref: QAction | None = None


    @pyqtSlot()
    def setup_ide_dock_widget(self) -> QDockWidget | None:
        ...
        if hasattr(self.mw, 'ide_save_file_action'):
            self.ide_save_file_action_ref = self.mw.ide_save_file_action # Store ref
            ide_toolbar.addAction(self.ide_save_file_action_ref)
        if hasattr(self.mw, 'ide_save_as_file_action'):
            self.ide_save_as_file_action_ref = self.mw.ide_save_as_file_action # Store ref
            ide_toolbar.addAction(self.ide_save_as_file_action_ref)
        ...
        # self.mw.addDockWidget(Qt.RightDockWidgetArea, self.ide_dock) # MainWindow adds it

        self.update_ide_save_actions_enable_state() # Call after actions are potentially referenced
    ...
    def on_ide_open_file(self):
        ...
        if file_path and self.ide_code_editor:
            try:
                ...
                # Calling on_ide_language_changed will update window title via its own logic
                elif hasattr(self.mw, 'update_window_title'):
    ...
    def on_ide_text_changed(self):
        if not self.ide_editor_is_dirty:
            ...
            self.update_ide_save_actions_enable_state()
            if hasattr(self.mw, 'update_window_title') and callable(self.mw.update_window_title):
                self.mw.update_window_title()
            else:
                logger.error("IDEManager: MainWindow instance does not have 'update_window_title' method.")
    ...
    def on_ide_language_changed(self, language_param: str):
        ...
        if self.ide_run_script_action_ref: # Use the stored reference
        ...
        ai_ready = self.mw.ai_chatbot_manager is not None and \
                   ...
                   hasattr(self.mw, '_internet_connected') and self.mw._internet_connected is True


        if self.ide_analyze_action_ref: # Use the stored reference
    ...
    def on_ide_run_python_script(self):
        ...
        try:
            ...
            if err_output:
                error_color_hex = COLOR_ACCENT_ERROR.name() if isinstance(COLOR_ACCENT_ERROR, QColor) else str(COLOR_ACCENT_ERROR)
            ...
            if self.ide_output_console: self.ide_output_console.ensureCursorVisible()
    ...
    def on_ide_analyze_with_ai(self):
        ...
        if not self.mw.ai_chatbot_manager or not self.mw.ai_chatbot_manager.api_key:
            ...
            return
        if not hasattr(self.mw, '_internet_connected') or not self.mw._internet_connected:
    ...
    def update_ide_save_actions_enable_state(self):
        ...
        if self.ide_save_file_action_ref: # Use stored reference
            self.ide_save_file_action_ref.setEnabled(self.ide_editor_is_dirty)
        elif hasattr(self.mw, 'ide_save_file_action'): # Fallback if ref not set yet
        ...
        if self.ide_save_as_file_action_ref: # Use stored reference
            self.ide_save_as_file_action_ref.setEnabled(can_save_as)
        elif hasattr(self.mw, 'ide_save_as_file_action'): # Fallback
