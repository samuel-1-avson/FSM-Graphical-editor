...
# from matlab_integration import MatlabConnection # Removed, functionality embedded
...
class ResourceMonitorWorker(QObject):
    """Worker class for monitoring system resources (CPU, RAM, GPU)"""
    resourceUpdate = pyqtSignal(float, float, float, str)  # cpu%, ram%, gpu%, gpu_name
    ...
    def __init__(self, interval_ms=2000, parent=None):
        ...
        if PYNVML_AVAILABLE and pynvml:
            try:
                pynvml.nvmlInit()
                self._nvml_initialized = True # Defer handle acquisition to _monitor_resources to avoid issues if GPU not present at init
            except Exception as e:
    ...
    def stop_monitoring(self):
        ...
        self._monitoring = False
        if self._nvml_initialized and PYNVML_AVAILABLE and pynvml: # Check PYNVML_AVAILABLE again
            try:
                pynvml.nvmlShutdown()
            except Exception as e:
    ...
    def _monitor_resources(self):
        ...
        while self._monitoring:
            # GPU Handle acquisition moved here, attempt each cycle if not already acquired
            if self._nvml_initialized and not self._gpu_handle and PYNVML_AVAILABLE and pynvml:
                ...
                except Exception as e_nvml_handle: logger.debug(f"NVML: Could not get GPU handle/name: {e_nvml_handle}")
            try:
                # CPU Usage
                ...
                if self._nvml_initialized and self._gpu_handle and PYNVML_AVAILABLE and pynvml:
                    try:
                        ...
                        gpu_util = -2.0  # Error reading GPU
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # log_function is now part of DiagramScene, no monkey-patch needed
    ...
    # --- Start of copy-pasted methods from previous valid MainWindow ---
    def init_ui(self):
    ...
    def _update_resource_display(self, cpu_usage, ram_usage, gpu_util, gpu_name):
        ...
        if hasattr(self, 'gpu_status_label'):
            if gpu_util == -1.0: # N/A or NVML init error before handle
                self.gpu_status_label.setText(f"GPU: {gpu_name}")
            elif gpu_util == -2.0: # Error polling GPU after handle
                 self.gpu_status_label.setText(f"GPU: {gpu_name}")
            else: # Valid GPU util
        ...
        if not clear_current_diagram and items_to_add_for_undo_command:
            self.undo_stack.endMacro()
             ...
             if self.undo_stack.count() > 0 and self.undo_stack.command(self.undo_stack.count() -1).childCount() == 0:
                 pass # If macro was started but nothing added, it's okay, endMacro will handle it.
             self.undo_stack.endMacro()
    ...
    def on_matlab_settings(self):
        """Handle MATLAB settings dialog."""
        # The dialog itself will modify self.matlab_connection settings
        # and call test_connection or detect_matlab as needed via its internal logic.
        dialog = MatlabSettingsDialog(matlab_connection=self.matlab_connection, parent=self)
        dialog.exec() # Use exec() not exec_() for QDialog
        logger.info("MATLAB settings dialog closed. Status should be updated via signals if changed.")
    ...
    def _update_window_title(self):
        ...
        self.setWindowTitle(title + "[*]") # Always include the placeholder
    ...
    def _update_matlab_status_display(self, connected, message):
        ...
        if hasattr(self, 'matlab_status_label'):
            ...
            style_sheet = f"font-weight: bold; padding: 0px 5px; color: {COLOR_PY_SIM_STATE_ACTIVE.name() if connected else '#C62828'};"
    ...
    def on_new_file(self, silent=False):
        ...
        self.setWindowModified(False) # This should make Qt hide the [*] if title contains it
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.resource_monitor_worker and self.resource_monitor_thread:
            ...
            if self.resource_monitor_thread.isRunning():
                ...
                if not self.resource_monitor_thread.wait(500): # Reduced wait time
                    logger.warning("Resource monitor thread did not quit gracefully. Terminating.")
                    self.resource_monitor_thread.terminate()
                    self.resource_monitor_thread.wait(200) # Brief wait after terminate
                else:
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        ...
        if hasattr(self, 'internet_status_label'):
            ...
            active_color_name = COLOR_PY_SIM_STATE_ACTIVE.name() # Get the color name
            style_sheet = f"font-weight: normal; padding: 0px 5px; color: {active_color_name if is_connected else '#D32F2F'};"
            ...
            if top_level_active_state_id:
                for item in self.scene.items():
                    if isinstance(item, GraphicsStateItem) and item.text_label == top_level_active_state_id:
                        ...
                        if self.view and hasattr(self.view, 'ensureVisible'): # ensureVisible might not exist on basic QGraphicsView
                            if not self.view.ensureVisible(item, 50, 50):
