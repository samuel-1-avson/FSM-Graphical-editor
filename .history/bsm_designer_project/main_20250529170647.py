...
from config import (
    ...
    COLOR_DRAGGABLE_BUTTON_PRESSED_BG, APP_FONT_SIZE_SMALL,
...
class DraggableToolButton(QPushButton):
    def __init__(self, text, mime_type, item_type_data, parent=None):
        ...
        self.setObjectName("DraggableToolButton") # Targeted by QSS
        self.mime_type = mime_type
        self.item_type_data = item_type_data
        self.setText(text) # Text set here, QSS will handle font weight
        self.setMinimumHeight(42) # Slightly taller for better touch/drag
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self.drag_start_position = QPoint()
        self.setIconSize(QSize(20,20)) # Ensure icon size matches visual intent
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        pixmap_size = QSize(max(160, self.width()), max(42, self.height())) # Match min height
        ...
        # Use colors from config for consistency if possible, or defaults
        bg_color = QColor(COLOR_DRAGGABLE_BUTTON_HOVER_BG) # Use hover state for drag preview
        border_color_qcolor = QColor(COLOR_DRAGGABLE_BUTTON_HOVER_BORDER)

        painter.setBrush(bg_color)
        painter.setPen(QPen(border_color_qcolor, 1.8)) # Slightly thicker border for drag preview
        painter.drawRoundedRect(button_rect.adjusted(0.5,0.5,-0.5,-0.5), 5, 5)

        icon_pixmap = self.icon().pixmap(self.iconSize(), QIcon.Normal, QIcon.On)
        text_x_offset = 12 # Increased padding
        ...
        if not icon_pixmap.isNull():
            ...
            text_x_offset += icon_pixmap.width() + 10 # More space between icon and text

        text_color_qcolor = QColor(COLOR_TEXT_PRIMARY)
        painter.setPen(text_color_qcolor)
        
        font = self.font()
        font.setWeight(QFont.DemiBold) # Make text slightly bolder for drag
        painter.setFont(font)

        text_rect = QRectF(text_x_offset, 0, pixmap_size.width() - text_x_offset - 8, pixmap_size.height())
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Object name assignment moved here to ensure labels exist.
    ...
    def _create_central_widget(self):
        ...
        self.view.setObjectName("MainDiagramView") # For specific styling if needed
    ...
    def _create_toolbars(self):
        ...
        tb_style = Qt.ToolButtonIconOnly # Change to IconOnly for a cleaner look
        ...
        tools_tb.setToolButtonStyle(tb_style) # Also icon only
    ...
    def _create_docks(self):
        ...
        # Tools Dock
        self.tools_dock = QDockWidget("Elements & Modes", self) # Changed title
        ...
        for btn in [self.toolbox_select_button, self.toolbox_add_state_button, self.toolbox_transition_button, self.toolbox_add_comment_button]:
            btn.setToolButtonStyle(Qt.ToolButtonTextBesideIcon); btn.setIconSize(QSize(20,20)); mode_layout.addWidget(btn) # Consistent icon size
        ...
        drag_initial_state_btn.setIcon(get_standard_icon(QStyle.SP_ToolBarHorizontalExtensionButton, "ISt")) # SP_ToolBarHorizontalExtensionButton can look like a start marker
        ...
        for btn in [drag_state_btn, drag_initial_state_btn, drag_final_state_btn, drag_comment_btn]:
             # Icons already set on DraggableToolButton construction
        ...
        # Properties Dock 
        self.properties_dock = QDockWidget("Item Properties", self) # Changed title
        ...
        self.properties_editor_label.setObjectName("PropertiesLabel") # For specific QSS
        ...
        self.properties_edit_button = QPushButton(get_standard_icon(QStyle.SP_DialogApplyButton, "Edt"),"Edit Properties...") # Added "..."
        ...
        # Log Dock 
        self.log_dock = QDockWidget("Application Log", self) # Changed title
        self.log_dock.setObjectName("LogDock")
        self.log_dock.setAllowedAreas(Qt.BottomDockWidgetArea | Qt.LeftDockWidgetArea | Qt.RightDockWidgetArea)
        log_widget = QWidget()
        log_layout = QVBoxLayout(log_widget)
        log_layout.setContentsMargins(0,0,0,0) # QTextEdit will have padding via QSS
        ...
        # AI Chatbot Dock 
        self.ai_chatbot_dock = QDockWidget("AI Assistant", self) # Changed title
    ...
    def _setup_ide_dock_widget(self):
        ...
        ide_toolbar.setToolButtonStyle(Qt.ToolButtonIconOnly) # Icon only for IDE toolbar
        ...
        self.ide_output_console.setFixedHeight(160) # Slightly taller console
    ...
    def _create_status_bar(self):
        ...
        self.status_bar.addWidget(self.status_label, 1) # Give status label more space

        # Resource Monitors (CPU, RAM, GPU)
        ...
        for label in [self.cpu_status_label, self.ram_status_label, self.gpu_status_label]:
            label.setMinimumWidth(80) # Standardized min width
    ...
    def _update_resource_display(self, cpu_usage, ram_usage, gpu_util, gpu_name):
        if hasattr(self, 'cpu_status_label'): self.cpu_status_label.setText(f"CPU: {cpu_usage:.0f}%") # No decimal for CPU
        if hasattr(self, 'ram_status_label'): self.ram_status_label.setText(f"RAM: {ram_usage:.0f}%") # No decimal for RAM
        if hasattr(self, 'gpu_status_label'):
            if gpu_util == -1.0: self.gpu_status_label.setText(f"GPU: {gpu_name}") # NVML available, no device or error
            elif gpu_util == -2.0: self.gpu_status_label.setText(f"GPU: Error") # Util Error
            elif gpu_util == -3.0: self.gpu_status_label.setText(f"GPU: Mon Error") # Monitor error
            elif PYNVML_AVAILABLE and self._resource_monitor_worker and self._resource_monitor_worker._nvml_initialized:
                self.gpu_status_label.setText(f"GPU: {gpu_util:.0f}%")
                self.gpu_status_label.setToolTip(f"GPU: {gpu_util:.0f}% ({gpu_name})")
            else: # pynvml not available or not initialized
                 self.gpu_status_label.setText(f"GPU: N/A")
                 self.gpu_status_label.setToolTip(gpu_name) # Show error message in tooltip
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        GV_SCALE = 1.3 # Slightly increased scale for more spread
        NODE_SEP = 0.8 # Increased node separation
        RANK_SEP = 1.5 # Increased rank separation
        ...
        G.node_attr['fontname'] = "Arial" # Match app font if possible
        ...
        for state_data in fsm_data.get('states', []):
            ...
            # Truncate long names for Graphviz label, but use full name for node ID
        ...
        for trans_data in fsm_data.get('transitions', []):
            ...
            if len(event_label) > 15: event_label = event_label[:12] + '...' # Truncate edge label
        ...
        for i, state_data in enumerate(fsm_data.get('states', [])):
            ...
            # Increased spacing for grid fallback
        ...
        for i, comment_data in enumerate(fsm_data.get('comments', [])):
            ...
            pos_x = comment_data.get('x', layout_start_x + i * (180 + 25)) # Increased spacing
            pos_y = comment_data.get('y', max_y_items + 120) # Increased spacing
            try:
    ...
    def _update_properties_dock(self):
        ...
        if len(selected_items) == 1:
            ...
            def fmt(txt, max_chars=25):
                if not txt: return "<i style='color:#999;'>(none)</i>" # More distinct 'none'
                txt_str = str(txt)
            ...
            rows = ""
            # Table styling for properties
            table_style = "width:100%; border-collapse:collapse; font-size:9pt;"
            td_key_style = "padding:4px 6px; text-align:right; font-weight:bold; border-right:1px solid #ddd; border-bottom:1px solid #eee; width:35%;"
            td_val_style = "padding:4px 6px; border-bottom:1px solid #eee;"
            
            if isinstance(item, GraphicsStateItem):
                color_obj = QColor(props.get('color', COLOR_ITEM_STATE_DEFAULT_BG))
                # Ensure text on color swatch is readable
                lum = color_obj.lightnessF()
                swatch_text_color = COLOR_TEXT_ON_ACCENT if lum < 0.5 else COLOR_TEXT_PRIMARY
                color_style = f"background-color:{color_obj.name()}; color:{swatch_text_color}; padding:2px 5px; border-radius:3px; font-size:8pt;"
                
                rows += f"<tr><td style='{td_key_style}'>Name:</td><td style='{td_val_style}'>{html.escape(props.get('name', 'N/A'))}</td></tr>"
                rows += f"<tr><td style='{td_key_style}'>Initial / Final:</td><td style='{td_val_style}'>{'Yes' if props.get('is_initial') else 'No'} / {'Yes' if props.get('is_final') else 'No'}</td></tr>"
                if props.get('is_superstate'):
                    sub_states_count = len(props.get('sub_fsm_data',{}).get('states',[]))
                    rows += f"<tr><td style='{td_key_style}'>Superstate:</td><td style='{td_val_style}'>Yes ({sub_states_count} sub-state{'s' if sub_states_count != 1 else ''})</td></tr>"

                rows += f"<tr><td style='{td_key_style}'>Color:</td><td style='{td_val_style}'><span style='{color_style}'>{html.escape(color_obj.name())}</span></td></tr>"
                for act_key in ['entry_action', 'during_action', 'exit_action']:
                    act_label = act_key.replace('_action','').capitalize()
                    rows += f"<tr><td style='{td_key_style}'>{act_label}:</td><td style='{td_val_style}'>{fmt(props.get(act_key, ''), 35)}</td></tr>"
                if props.get('description'): rows += f"<tr><td style='{td_key_style}'>Desc:</td><td style='{td_val_style}white-space:normal;'>{fmt(props.get('description'), 100)}</td></tr>"
                ...
                if props.get('action'): label_parts.append(f"/{{{fmt(props.get('action'),18)}}}") # Shorten action for display
                full_label = " ".join(p for p in label_parts if p) or "<i style='color:#999;'>(No Label)</i>"

                rows += f"<tr><td style='{td_key_style}'>Label:</td><td style='{td_val_style}font-size:8.5pt;'>{full_label}</td></tr>"
                rows += f"<tr><td style='{td_key_style}'>From / To:</td><td style='{td_val_style}'>{html.escape(props.get('source','N/A'))} &rarr; {html.escape(props.get('target','N/A'))}</td></tr>"
                rows += f"<tr><td style='{td_key_style}'>Color:</td><td style='{td_val_style}'><span style='{color_style}'>{html.escape(color_obj.name())}</span></td></tr>"
                rows += f"<tr><td style='{td_key_style}'>Curve:</td><td style='{td_val_style}'>Bend={props.get('control_offset_x',0):.0f}, Shift={props.get('control_offset_y',0):.0f}</td></tr>"
                if props.get('description'): rows += f"<tr><td style='{td_key_style}'>Desc:</td><td style='{td_val_style}white-space:normal;'>{fmt(props.get('description'), 100)}</td></tr>"

            elif isinstance(item, GraphicsCommentItem):
                rows += f"<tr><td style='{td_key_style}'>Text:</td><td style='{td_val_style}white-space:normal;'>{fmt(props.get('text', ''), 100)}</td></tr>"
            else:
                rows = f"<tr><td colspan='2' style='{td_val_style}text-align:center;'>Unknown Item Type</td></tr>"
                item_type_name = "Unknown"

            html_content = f"""
                <div style='font-family:"Segoe UI",Arial,sans-serif; font-size:9pt; line-height:1.4;'>
                    <h4 style='margin:0 0 8px 0; padding:3px 0; color:{COLOR_ACCENT_PRIMARY}; border-bottom:1.5px solid {COLOR_BORDER_LIGHT};'>
                        {item_type_name} Properties
                    </h4>
                    <table style='{table_style}'>{rows}</table>
                </div>"""
        elif len(selected_items) > 1:
            html_content = f"<div style='padding:8px;text-align:center;'><i><b>{len(selected_items)} items selected.</b><br><small>Select a single item to view or edit its properties.</small></i></div>"
            item_type_tooltip = f"{len(selected_items)} items"
        else:
            html_content = "<div style='padding:8px;text-align:center;'><i>No item selected.</i><br><small>Click an item in the diagram or use the tools to add new elements.</small></div>"
    ...
    def _update_window_title(self):
        ...
        title = f"{APP_NAME} - {file_name}{sim_status_suffix}{ide_suffix} {modified_indicator}".strip() # Remove trailing space if no modified_indicator
        self.setWindowTitle(title)
        
        # Also update main status label for combined status
    ...
    def _update_matlab_status_display(self, connected, message):
        ...
        if hasattr(self, 'matlab_status_label'):
            ...
            bg_color = QColor(text_color).lighter(180).name() # Lighter background for status
    ...
    def _save_to_path(self, file_path) -> bool:
        ...
        try:
            ...
            self.scene.set_dirty(False)
            self.setWindowModified(False) # Clears '*' from window title via connected slot
            self._update_window_title() # Ensure title is correct post-save
            return True
        except Exception as e:
    ...
    def _run_internet_check_job(self):
        ...
        try:
            ...
            status_detail = "Timeout" # More concise for status bar
        except (socket.gaierror, OSError):
            status_detail = "Net Issue" # More concise
    ...
    def _update_py_sim_status_display(self):
        if hasattr(self, 'py_sim_status_label'):
            if self.py_sim_active and self.py_fsm_engine:
                ...
                # Truncate long hierarchical state names for status bar
    ...
    def _on_ide_language_changed(self, language: str):
        ...
        if self.ide_analyze_action:
            ...
            if not (self.ai_chatbot_manager and self.ai_chatbot_manager.api_key and self._internet_connected):
                tooltip += " (Requires Internet & Gemini API Key)"
            elif not (language == "Python" or language.startswith("C/C++")):
                 tooltip += " (Best for Python or C/C++)"
            self.ide_analyze_action.setToolTip(tooltip)
    ...
    def log_message(self, level_str: str, message: str):
        ...
        logger.log(level, message) # Let the logger format it
...
if __name__ == '__main__':
    ...
    if not os.path.exists(deps_icons_dir):
        try:
            ...
            # Create a dummy check.png if it doesn't exist, as it's referenced in QSS
            check_png_path = os.path.join(deps_icons_dir, "check.png")
            if not os.path.exists(check_png_path):
                try:
                    from PIL import Image, ImageDraw # Try to use PIL for a simple checkmark
                    img = Image.new('RGBA', (16, 16), (255, 0, 0, 0)) # Transparent background
                    draw = ImageDraw.Draw(img)
                    # Draw a simple green checkmark
