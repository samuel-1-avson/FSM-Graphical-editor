...
from bsm_designer_project.test_graphviz import FSMSimulator, FSMError
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.init_ui() # Creates all UI elements including AI dock
        
        
        
        
         # Connect Manager's own status/error signals if needed for manager-level issues
        ...
        # Note: Worker signals (responseReady, errorOccurred, statusUpdate from ChatbotWorker)
        # are connected directly to MainWindow slots inside AIChatbotManager._setup_worker()

        # Initial status based on whether API key might be loaded from settings (not implemented here)
        # For now, assume no key initially.
        # set_api_key will trigger relevant status updates
        self.ai_chatbot_manager.set_api_key(None) # Explicitly initialize, set_api_key handles logic
    ...
    def _create_actions(self):
        ...
        # ... after self.exit_action ...
        self.open_example_menu_action = QAction("Open E&xample...", self) # This will be a menu trigger
...
# ...
    ...
    def _create_menus(self):
        ...
        matlab_sim_menu = sim_menu.addMenu(get_standard_icon(QStyle.SP_ComputerIcon, "M"), "MATLAB/Simulink"); matlab_sim_menu.addAction(self.run_simulation_action); matlab_sim_menu.addAction(self.generate_code_action); matlab_sim_menu.addSeparator(); 
        ...
        help_menu.addAction(self.quick_start_action) # Add before about
        help_menu.addAction(self.about_action)
        
        
    def on_show_quick_start(self):
        guide_filename = "QUICK_START.html" # Or .md if you have a markdown viewer/browser handles it
        guide_path = self._get_bundled_file_path(guide_filename) # Use the same helper
        if guide_path:
        ...
        # _get_bundled_file_path already shows a message
            pass    
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False):
          ...
          """
      Adds states, transitions, and comments from AI-generated data to the scene using Graphviz for layout.
      'fsm_data' should have 'states', 'transitions', and 'comments' keys.
      """
        current_time = QTime.currentTime().toString('hh:mm:ss.zzz')
        print(f"--- MW_ADD_FSM ({current_time}): CALLED. clear_current_diagram={clear_current_diagram} ---")
        print(f"--- MW_ADD_FSM ({current_time}): Received FSM Data (states: {len(fsm_data.get('states',[]))}, transitions: {len(fsm_data.get('transitions',[]))}) ---")
      ...
      # Initialize layout parameters for fallback
      ...
      # Add nodes (states)
        for state in fsm_data.get('states', []):
            name = state.get('name')
            if name:
                G.add_node(name)
            else:
                self.log_message(f"AI Gen Warning: State data missing 'name'. Skipping: {state}", type_hint="AIChatError")

      # Add edges (transitions)
        for trans in fsm_data.get('transitions', []):
      ...
      # Apply layout algorithm
        ...
        try:
            G.layout(prog=layout_algorithm)
          # Extract node positions
            for node in G.nodes():
                pos = node.attr['pos'].split(',')
                try:
                    node_positions[node.name] = (float(pos[0]), float(pos[1]))
                except (ValueError, IndexError):
                    node_positions[node.name] = (0, 0)
                    self.log_message(f"AI Gen Warning: Invalid position for node '{node.name}'. Using (0, 0).", type_hint="AIChatError")
        except Exception as e:
            error_msg = str(e).strip() or "Could not execute Graphviz 'dot' (check PATH or compatibility)"
            self.log_message(
                f"Graphviz layout failed: {error_msg}. Using fallback grid layout. "
                f"Graphviz version: {os.popen('dot -V').read().strip() or 'Not found'}. "
                f"Ensured PATH includes: {graphviz_path}. "
                "Verify with 'dot -V' in Command Prompt. Try 'conda install -c conda-forge pygraphviz=1.12'.",
                type_hint="AIChatError"
            )
            print(f"--- MW_ADD_FSM_GRAPHVIZ_ERROR: {error_msg} ---")
            use_fallback = True
          # Assign grid positions
            for i, state in enumerate(fsm_data.get('states', [])):
                if state.get('name'):
                    state['x'] = start_x + (i % items_per_row) * (default_item_width + padding_x)
                    state['y'] = start_y + (i // items_per_row) * (default_item_height + padding_y)

      # Scale and translate positions if using Graphviz
        if not use_fallback and node_positions:
            min_x = min(pos[0] for pos in node_positions.values()) if node_positions else 0
            min_y = min(pos[1] for pos in node_positions.values()) if node_positions else 0
            scale_factor = 100
            start_x, start_y = 100, 100
      ...
      # Add States
        for state in fsm_data.get('states', []):
            ...
            if not name:
                continue
            pos_x = state.get('x', start_x + (node_positions.get(name, (0, 0))[0] - min_x) * scale_factor if not use_fallback else start_x)
            pos_y = state.get('y', start_y + (node_positions.get(name, (0, 0))[1] - min_y) * scale_factor if not use_fallback else start_y)
            item_w = state.get('width', 120)
            item_h = state.get('height', 60)
            try:
                state_item = GraphicsStateItem(
                    pos_x, pos_y, item_w, item_h, name,
                    is_initial=state.get('is_initial', False),
                    is_final=state.get('is_final', False),
                    color=state.get('properties', {}).get('color'),
                    entry_action=state.get('entry_action', ""),
                    during_action=state.get('during_action', ""),
                    exit_action=state.get('exit_action', ""),
                    description=state.get('description', "")
                )
                items_to_add_for_undo_command.append(state_item)
                state_items_map[name] = state_item
                self.log_message(f"AI Gen: Queued state '{name}' at ({pos_x:.0f}, {pos_y:.0f}).", type_hint="AI_ACTION")
            except Exception as e:
                self.log_message(f"Error creating GraphicsStateItem '{name}': {e}", type_hint="AIChatError")
                print(f"--- MW_ADD_FSM_STATE_ERROR: {e} for state: {state} ---")
      ...
      # Add Transitions
        for trans in fsm_data.get('transitions', []):
            ...
            if not source_name or not target_name:
                self.log_message(f"AI Gen Warning: Transition missing source/target. Skipping: {trans}", type_hint="AIChatError")
                continue
            ...
            if source_item and target_item:
                try:
                    trans_item = GraphicsTransitionItem(
                        ...
                        event_str=trans.get('event', ""),
                        condition_str=trans.get('condition', ""),
                        action_str=trans.get('action', ""),
                        color=trans.get('properties', {}).get('color'),
                        description=trans.get('description', "")
                    )
                    offset_x = trans.get('control_offset_x')
                    offset_y = trans.get('control_offset_y')
                    if offset_x is not None and offset_y is not None:
                        try:
                            trans_item.set_control_point_offset(QPointF(float(offset_x), float(offset_y)))
                        except ValueError:
                            self.log_message(f"AI Gen Warning: Invalid control offsets for transition {source_name}->{target_name}.", type_hint="AIChatError")
                    items_to_add_for_undo_command.append(trans_item)
                    self.log_message(f"AI Gen: Queued transition '{source_name}'->'{target_name}'.", type_hint="AI_ACTION")
                except Exception as e:
                    self.log_message(f"Error creating GraphicsTransitionItem from '{source_name}' to '{target_name}': {e}", type_hint="AIChatError")
                    print(f"--- MW_ADD_FSM_TRANS_ERROR: {e} for transition: {trans} ---")
            else:
                self.log_message(f"AI Gen Warning: Could not find source ('{source_name}') or target ('{target_name}') for transition.", type_hint="AIChatError")

      # Add Comments
        max_y_of_states = start_y
        if state_items_map:
            max_y_of_states = max((item.scenePos().y() + item.rect().height() for item in state_items_map.values()), default=start_y)
        comment_start_y = max_y_of_states + 100
        comment_start_x = start_x
        for i, comment in enumerate(fsm_data.get('comments', [])):
            text = comment.get('text')
            if not text:
                continue
            comment_x = comment.get('x', comment_start_x + i * (150 + 20))
            comment_y = comment.get('y', comment_start_y)
            comment_width = comment.get('width')
            try:
                comment_item = GraphicsCommentItem(comment_x, comment_y, text)
                if comment_width is not None:
                    try:
                        comment_item.setTextWidth(float(comment_width))
                    except ValueError:
                        self.log_message(f"AI Gen Warning: Invalid width '{comment_width}' for comment. Using default.", type_hint="AIChatError")
                items_to_add_for_undo_command.append(comment_item)
                self.log_message(f"AI Gen: Queued comment '{text[:20]}...'.", type_hint="AI_ACTION")
            except Exception as e:
                self.log_message(f"Error creating GraphicsCommentItem: {e}", type_hint="AIChatError")
                print(f"--- MW_ADD_FSM_COMMENT_ERROR: {e} for comment: {comment} ---")

      # Add all items via UndoCommands
        if items_to_add_for_undo_command:
            for item in items_to_add_for_undo_command:
                ...
                if hasattr(item, 'text_label'):
                    cmd_text += f": {item.text_label}"
                elif hasattr(item, 'event_str'):
                    cmd_text += f" {item.start_item.text_label}->{item.end_item.text_label}"
                add_cmd = AddItemCommand(self.scene, item, cmd_text)
                self.undo_stack.push(add_cmd)

            self.scene.set_dirty(True)
            QTimer.singleShot(0, self._fit_view_to_new_ai_items)

        print(f"--- MW_ADD_FSM ({QTime.currentTime().toString('hh:mm:ss.zzz')}): Processing finished. Items queued: {len(items_to_add_for_undo_command)} ---")
    ...
    def _on_send_ai_chat_message(self):
        ...
        # UI updates to indicate processing are now mostly handled by signals from ChatbotWorker
        # but we can set an initial "Sending..." state from MainWindow side.
        self._update_ai_chat_status("Status: Sending...") # Manager will then queue to worker.
        # Buttons will be re-enabled by _update_ai_chat_status based on subsequent signals.
    ...
    def _get_bundled_file_path(self, filename: str) -> str | None:
        """Gets the path to a file bundled with the application."""
        try:
            # PyInstaller temporary directory
            base_path = sys._MEIPASS # type: ignore
        except AttributeError:
            # Not bundled, running from source
            base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "examples")) # Assume 'examples' subdir

        file_path = os.path.join(base_path, filename)
        if not os.path.exists(file_path) and base_path == os.path.abspath(os.path.join(os.path.dirname(__file__), "examples")):
            # Try one level up if 'examples' is not directly in the same dir as main.py (e.g. bsm_designer_project/examples)
            base_path_alt = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "examples"))
            file_path_alt = os.path.join(base_path_alt, filename)
            if os.path.exists(file_path_alt):
                return file_path_alt
            else: # Fallback if "examples" is sibling to main.py's dir (e.g. src/main.py, examples/)
                base_path_alt2 = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "bsm_designer_project", "examples"))
                file_path_alt2 = os.path.join(base_path_alt2, filename)
                if os.path.exists(file_path_alt2): return file_path_alt2


        if os.path.exists(file_path):
            return file_path
        else:
            self.log_message(f"Example file '{filename}' not found at expected path: {file_path}", type_hint="GENERAL")
            QMessageBox.warning(self, "Example Not Found", f"Could not find the example file: {filename}")
            return None
    ...
    def _open_example_file(self, example_filename: str):
        ...
        self.on_stop_py_simulation(silent=True) # Stop any active simulation
        self.log_message(f"Opening example file: {file_path}", type_hint="GENERAL")
        if self._load_from_path(file_path):
            self.current_file_path = file_path # Set current_file_path to the example path
            self.setWindowModified(False) # Examples are not "dirty" initially
            self.last_generated_model_path = None
            self.undo_stack.clear()
            self._update_window_title()
            self._update_undo_redo_actions_enable_state()
            self.log_message(f"Successfully opened example: {example_filename}", type_hint="GENERAL")
            ...
            QMessageBox.critical(self, "Error Opening Example", f"Could not load or parse example file: {example_filename}")
            self.log_message(f"Failed to open example file: {example_filename}", type_hint="GENERAL")
    ...
    def _handle_ai_response(self, ai_message_text: str):
        current_time = QTime.currentTime().toString('hh:mm:ss.zzz')
        print(f"--- MW_HANDLE_RESP ({current_time}): CALLED. Raw AI message length: {len(ai_message_text)} ---")
        print(f"--- MW_HANDLE_RESP ({current_time}): Raw AI (first 100): '{ai_message_text[:100].replace('\n',' ')}...' ---")
    ...
    # Pattern 1: ```json ... ```
        ...
        if match_markdown:
            json_string = match_markdown.group(1).strip()
            json_match = True
            print(f"--- MW_HANDLE_RESP ({current_time}): Found JSON block in markdown. AI might not be in strict JSON-only mode. ---")
            self.log_message("AI response contained JSON wrapped in markdown. Processing it.", type_hint="AI_ACTION")
        else:
        # Pattern 2: Starts with { and ends with }
            ...
            if first_brace != -1:
                ...
                for i, char in enumerate(ai_message_text[first_brace:]):
                    if char == '{':
                        ...
                        balance -= 1
                        if balance == 0:
                            last_brace_index = first_brace + i
                            break
                if last_brace_index != -1:
                    json_string = ai_message_text[first_brace : last_brace_index + 1]
                    json_match = True
                    print(f"--- MW_HANDLE_RESP ({current_time}): Extracted potential JSON block by brace matching. ---")
                else:
                    json_string = None
                    print(f"--- MW_HANDLE_RESP ({current_time}): Found opening brace but no matching closing brace. ---")
            else:
                json_string = None
                print(f"--- MW_HANDLE_RESP ({current_time}): No JSON-like block found. ---")
        ...
        if json_match and json_string:
            try:
                ...
                "transitions" in fsm_data and isinstance(fsm_data["transitions"], list):
                    print(f"--- MW_HANDLE_RESP ({current_time}): Successfully parsed extracted JSON string. ---")
                    print(f"--- MW_HANDLE_RESP ({current_time}): Extracted JSON IS valid FSM data. ---")
                    ...
                    print(f"--- MW_HANDLE_RESP ({current_time}): Extracted JSON is valid, but not FSM structure. Keys: {list(fsm_data.keys  ())}. Treating as text. ---")
                    self.log_message(f"AI response contained valid JSON, but it did not match the expected FSM structure (missing 'states' or   'transitions' list). Displaying as text.", type_hint="AI_ACTION")
                    message_processed_as_fsm = False
            except json.JSONDecodeError as e:
                print(f"--- MW_HANDLE_RESP ({current_time}): Extracted block LOOKED like JSON but failed to parse: {e}. Treating original as    text. ---")
                self.log_message(f"AI response contained a block that looked like JSON, but it could not be parsed. Error: {e}. Displaying as   text.", type_hint="AIChatError")
                message_processed_as_fsm = False

        if not message_processed_as_fsm:
            print(f"--- MW_HANDLE_RESP ({current_time}): Appending full AI response as REGULAR chat. ---")
    ...
    def _update_ai_chat_status(self, status_text: str):
        ...
        if hasattr(self, 'ai_chat_status_label'):
            ...
            is_thinking = "thinking..." in status_text.lower() or \
                          ...
                          "preparing..." in status_text.lower() # Added "preparing..."
            ...
            else: # Ready, cleared, added, cancelled etc.
                 self.ai_chat_status_label.setStyleSheet("font-size: 8pt; color: grey;")
            
            # Determine if user can send new message
            can_send_new_message = not is_thinking and not is_key_required 
            # However, AI Manager could still be busy with a previous valid request
            # A better check might involve checking if self.ai_chatbot_manager.worker_thread is running
            # For now, this simpler logic relies on the status_text string.
            if hasattr(self, 'ai_chat_send_button'):
                self.ai_chat_send_button.setEnabled(can_send_new_message)
            if hasattr(self, 'ai_chat_input'):
                self.ai_chat_input.setEnabled(can_send_new_message)
                if can_send_new_message: # If ready, focus input
                    self.ai_chat_input.setFocus()

              
        
            
    # In MainWindow class (main.py)
    ...
    #     if clear_current_diagram:
    #         # To make this operation fully undoable as one block, you'd need a command
    #         # that stores all current items, clears, then adds new ones, and its undo
    #         # would restore the old items. For now, clear is destructive to undo stack.
    #         self.scene.clear()
    #         self.undo_stack.clear()
    #         self.log_message("Cleared diagram before AI generation.", type_hint="AI_ACTION")
    ...
    #     start_x, start_y = 100, 100
    #     items_per_row = 3
    #     default_item_width, default_item_height = 120, 60 # Your default sizes
    #     padding_x, padding_y = 150, 100
    ...
    #         # Auto-positioning (application-controlled)
    #         # AI could provide 'x', 'y', 'width', 'height' in state_data,
    #         # you can decide whether to use them or your app's layout/defaults.
    ...
    #         try:
    #             # Using your GraphicsStateItem.__init__ signature:
    #             # (self, x, y, w, h, text, is_initial=False, is_final=False,
    ...
    #                 item_w,                     # w
    #                 item_h,                     # h
    #                 name,                       # text (this is 'text_label' inside your GSI)
    #                 is_initial=state_data.get('is_initial', False),
    #                 is_final=state_data.get('is_final', False),
    #                 color=state_data.get('properties', {}).get('color'), # GSI takes 'color'
    ...
    #             items_to_add_for_undo_command.append(state_item)
    #             state_items_map[name] = state_item
    #             self.log_message(f"AI Gen: Queued state '{name}'.", type_hint="AI_ACTION")
    #         except Exception as e:
    #             self.log_message(f"Error creating GraphicsStateItem '{name}' from AI data: {e}", type_hint="AIChatError")
    #             print(f"--- MW_ADD_FSM_STATE_ERROR: {e} for state: {state_data} ---")
    ...
    #         if source_item and target_item:
    #             try:
    #                 # Using your GraphicsTransitionItem.__init__ signature:
    #                 # (self, start_item, end_item, event_str="", condition_str="", action_str="",
    #                 #  color=None, description="")
    #                 trans_item = GraphicsTransitionItem(
    #                     source_item,                # start_item
    #                     target_item,                # end_item
    #                     event_str=trans_data.get('event', ""),
    #                     condition_str=trans_data.get('condition', ""),
    #                     action_str=trans_data.get('action', ""),
    #                     color=trans_data.get('properties', {}).get('color'), # GTI takes 'color'
    #                     description=trans_data.get('description', "")
    #                 )

    #                 # Handle control point offsets if provided by AI
    #                 offset_x = trans_data.get('control_offset_x')
    #                 offset_y = trans_data.get('control_offset_y')
    #                 if offset_x is not None and offset_y is not None:
    #                     try:
    #                         # GfxTransitionItem has set_control_point_offset(QPointF)
    #                         trans_item.set_control_point_offset(QPointF(float(offset_x), float(offset_y)))
    #                     except ValueError:
    ...
    #             except Exception as e:
    #                 self.log_message(f"Error creating GraphicsTransitionItem from '{source_name}' to '{target_name}': {e}", type_hint="AIChatError")
    #                 print(f"--- MW_ADD_FSM_TRANS_ERROR: {e} for transition: {trans_data} ---")
    #         else:
    ...
    #     # Add Comments
    #     # Simple layout for comments, below states.
    #     max_y_of_states = start_y
    #     if state_items_map: # Check if any states were added
    #         max_y_of_states = max((item.scenePos().y() + item.rect().height() for item in state_items_map.values()), default=start_y)

    #     comment_start_y = max_y_of_states + padding_y # Place comments below the lowest state
    #     comment_start_x = start_x
    ...
    #         comment_x = comment_data.get('x', comment_start_x + i * (150 + 20)) # Simple horizontal list
    #         comment_y = comment_data.get('y', comment_start_y)
    #         comment_width = comment_data.get('width') # GraphicsCommentItem handles default internally if None

    #         try:
    #             # Using your GraphicsCommentItem.__init__ signature:
    #             # (self, x, y, text="Comment")
    #             comment_item = GraphicsCommentItem(comment_x, comment_y, text)
    #             if comment_width is not None:
    #                 try:
    #                     comment_item.setTextWidth(float(comment_width))
    ...
    #         except Exception as e:
    #             self.log_message(f"Error creating GraphicsCommentItem: {e}", type_hint="AIChatError")
    #             print(f"--- MW_ADD_FSM_COMMENT_ERROR: {e} for comment: {comment_data} ---")

    #     # Add all items via UndoCommands
    #     if items_to_add_for_undo_command:
    #         # Option: If you had an AddItemsCommand (plural)
    #         # cmd = AddItemsCommand(self.scene, items_to_add_for_undo_command, "Add AI Generated FSM")
    #         # self.undo_stack.push(cmd)
    ...
    #             elif hasattr(item, 'event_str'): cmd_text += f" {item.start_item.text_label}->{item.end_item.text_label}"
    #             add_cmd = AddItemCommand(self.scene, item, cmd_text)
    #             self.undo_stack.push(add_cmd)

    #         self.scene.set_dirty(True)
    #         QTimer.singleShot(0, self._fit_view_to_new_ai_items) # Fit view after items are on scene
    ...
    def _fit_view_to_new_ai_items(self):
        """Helper to fit view after items are processed and added to the scene."""
        ...
        elif self.view: # Fallback if bounds are null for some reason but items might exist
             self.view.centerOn(self.scene.sceneRect().center())    
        

    def _handle_ai_error(self, error_message: str): # SLOT for ChatbotWorker.errorOccurred & AIChatbotManager.errorOccurred
    ...
    def _update_ai_chat_status(self, status_text: str): # SLOT for ChatbotWorker.statusUpdate & AIChatbotManager.statusUpdate
        current_time = QTime.currentTime().toString('hh:mm:ss.zzz')
        print(f"--- MW_UPDATE_STATUS ({current_time}): status: '{status_text}' ---")
        if hasattr(self, 'ai_chat_status_label'): # Ensure UI element exists
            self.ai_chat_status_label.setText(status_text)
            
            is_thinking = "thinking..." in status_text.lower() or \
                          "sending..." in status_text.lower()
            
            is_key_required = "api key required" in status_text.lower() or \
                              "inactive" in status_text.lower() # Consider "inactive" as needing key
            ...
            if hasattr(self, 'ai_chat_send_button'):
                self.ai_chat_send_button.setEnabled(can_send)
            if hasattr(self, 'ai_chat_input'):
                self.ai_chat_input.setEnabled(can_send)
                if can_send: # If ready to send new message, focus input
                    self.ai_chat_input.setFocus()
    ...
    def _append_to_ai_chat_display(self, sender: str, message: str):
        ...
        formatted_message = (
            ...
            f"{html.escape(message).replace(r'\n', '<br>')}" # Basic newline handling for display
            f"</div>"
        )
    ...
    def on_openai_settings(self):
        ...
        if ok:
            ...
            # Status updates will now come from the manager/worker signals.
            if new_key:
    ...
    def on_clear_ai_chat_history(self):
        ...
        if self.ai_chatbot_manager:
            self.ai_chatbot_manager.clear_conversation_history() # This will trigger worker.clear_history()
            # Clear GUI display immediately
            if hasattr(self, 'ai_chat_display'):
                self.ai_chat_display.clear()
                self.ai_chat_display.setPlaceholderText("AI chat history will appear here...")
        # Worker's statusUpdate signal will update the status label
    ...
    def _update_properties_dock(self):
        ...
        if len(selected_items) == 1:
            ...
            # CORRECTED format_prop_text helper function
            def format_prop_text(text_content, max_chars=25):
                if not text_content: # Handles None or empty string
                    return "<i>(none)</i>"

                escaped = html.escape(str(text_content)) # Ensure it's a string
                first_line = escaped.split('\n')[0]

                if len(first_line) > max_chars or '\n' in escaped: # Check original 'escaped' for other newlines
                    return first_line[:max_chars] + "…"
                else:
                    return first_line
            # END CORRECTED format_prop_text
            ...
            if isinstance(item, GraphicsStateItem):
                ...
                rows_html += f"<tr><td><b>Name:</b></td><td>{html.escape(props['name'])}</td></tr>" # props.get('name', 'N/A')
                ...
                # Use props.get with a default for safety, even though get_data should provide them
                ...
                # Using props.get for event, condition, action for safety
                ...
                action_text = f"/{{{format_prop_text(props.get('action', ''), 15)}}}" if props.get('action') else '' # Uses corrected format_prop_text
                ...
                rows_html += f"<tr><td><b>Label:</b></td><td style='font-size:8pt;'>{full_label}</td></tr><tr><td><b>From:</b></td><td>{html.escape(props.get('source','N/A'))}</td></tr><tr><td><b>To:</b></td><td>{html.escape(props.get('target','N/A'))}</td></tr>"
                rows_html += f"<tr><td><b>Color:</b></td><td><span style='{color_style}'>{html.escape(props.get('color','N/A'))}</span></td></tr>"
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        self.on_stop_py_simulation(silent=True) # Stop internal sim
        ...
        if self._prompt_save_if_dirty():
            ...
            event.ignore()
            if self.internet_check_timer: # Restart if close was cancelled
