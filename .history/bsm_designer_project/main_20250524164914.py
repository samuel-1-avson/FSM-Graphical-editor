...
# DraggableToolButton class (unchanged)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._py_sim_currently_highlighted_transition: GraphicsTransitionItem | None = None  # NEW: Track highlighted transition
    ...
    def _create_toolbars(self):
        ...
        edit_toolbar = self.addToolBar("Edit"); edit_toolbar.setObjectName("EditToolBar"); edit_toolbar.setIconSize(icon_size); file_toolbar.setToolButtonStyle(tb_style); edit_toolbar.addAction(self.undo_action); edit_toolbar.addAction(self.redo_action); edit_toolbar.addSeparator(); edit_toolbar.addAction(self.delete_action)
    ...
    def _create_docks(self):
        ...
        # Properties Dock
        self.properties_dock = QDockWidget("Properties", self); self.properties_dock.setObjectName("PropertiesDock"); self.properties_dock.setAllowedAreas(Qt.LeftDockWidgetArea | Qt.RightDockWidgetArea)
        properties_widget = QWidget(); properties_layout = QVBoxLayout(properties_widget); properties_layout.setContentsMargins(5,5,5,5); properties_layout.setSpacing(5)
        self.properties_editor_label = QLabel("<i>No item selected.</i><br><small>Click an item in the diagram or use tools to add new items.</small>"); self.properties_editor_label.setWordWrap(True); self.properties_editor_label.setTextFormat(Qt.RichText); self.properties_editor_label.setAlignment(Qt.AlignTop); self.properties_editor_label.setStyleSheet(f"padding: 5px; background-color: {COLOR_BACKGROUND_LIGHT}; border: 1px solid {COLOR_BORDER_MEDIUM};"); properties_layout.addWidget(self.properties_editor_label, 1)
        ...
        self.properties_dock.setWidget(properties_widget); self.addDockWidget(Qt.RightDockWidgetArea, self.properties_dock); self.view_menu.addAction(self.properties_dock.toggleViewAction())
        
        # Log Dock
        self.log_dock = QDockWidget("Log", self); self.log_dock.setObjectName("LogDock"); self.log_dock.setAllowedAreas(Qt.BottomDockWidgetArea | Qt.LeftDockWidgetArea | Qt.RightDockWidgetArea)
        log_widget = QWidget(); log_layout = QVBoxLayout(log_widget); log_layout.setContentsMargins(5,5,5,5)
        self.log_output = QTextEdit(); self.log_output.setObjectName("LogOutput"); self.log_output.setReadOnly(True); self.log_output.setStyleSheet("font-family: Consolas, monospace; font-size: 9pt;"); self.log_output.setHtml(f"<span style='color:{COLOR_TEXT_SECONDARY};'>[Starting log...]</span>"); log_layout.addWidget(self.log_output)
        self.log_dock.setWidget(log_widget); self.addDockWidget(Qt.BottomDockWidgetArea, self.log_dock); self.view_menu.addAction(self.log_dock.toggleViewAction())
        
        # Python Simulation Dock (UPDATED)
        self.py_sim_dock = QDockWidget("Python Simulation", self); self.py_sim_dock.setObjectName("PySimDock"); self.py_sim_dock.setAllowedAreas(Qt.LeftDockWidgetArea | Qt.RightDockWidgetArea | Qt.BottomDockWidgetArea)
        py_sim_widget = QWidget(); py_sim_layout = QVBoxLayout(py_sim_widget); py_sim_layout.setContentsMargins(5,5,5,5); py_sim_layout.setSpacing(5)
        controls_group = QGroupBox("Controls"); controls_layout = QHBoxLayout(); controls_layout.setSpacing(5)
        self.py_sim_start_btn = QToolButton(); self.py_sim_start_btn.setDefaultAction(self.start_py_sim_action); self.py_sim_stop_btn = QToolButton(); self.py_sim_stop_btn.setDefaultAction(self.stop_py_sim_action); self.py_sim_reset_btn = QToolButton(); self.py_sim_reset_btn.setDefaultAction(self.reset_py_sim_action); self.py_sim_step_btn = QPushButton(get_standard_icon(QStyle.SP_MediaSeekForward, "Step"),"Step"); self.py_sim_step_btn.clicked.connect(self.on_step_py_simulation)
        for btn in [self.py_sim_start_btn, self.py_sim_stop_btn, self.py_sim_reset_btn]: btn.setToolButtonStyle(Qt.ToolButtonIconOnly); btn.setIconSize(QSize(18,18)); controls_layout.addWidget(btn)
        controls_layout.addWidget(self.py_sim_step_btn); controls_layout.addStretch(); controls_group.setLayout(controls_layout); py_sim_layout.addWidget(controls_group)
        event_group = QGroupBox("Event Trigger"); event_layout = QHBoxLayout(); event_layout.setSpacing(5)
        self.py_sim_event_combo = QComboBox(); self.py_sim_event_combo.addItem("None (Internal)"); self.py_sim_event_combo.setEditable(False); self.py_sim_event_combo.currentTextChanged.connect(self.on_trigger_py_event); event_layout.addWidget(self.py_sim_event_combo, 1)  # NEW: Event combo box
        self.py_sim_event_name_edit = QLineEdit(); self.py_sim_event_name_edit.setPlaceholderText("Custom event name"); event_layout.addWidget(self.py_sim_event_name_edit, 1); self.py_sim_trigger_event_btn = QPushButton(get_standard_icon(QStyle.SP_MediaPlay, "Trg"),"Trigger"); self.py_sim_trigger_event_btn.clicked.connect(self.on_trigger_py_event); event_layout.addWidget(self.py_sim_trigger_event_btn); event_group.setLayout(event_layout); py_sim_layout.addWidget(event_group)
        state_group = QGroupBox("Current State"); state_layout = QVBoxLayout(); self.py_sim_current_state_label = QLabel("<i>Not Running</i>"); self.py_sim_current_state_label.setStyleSheet("font-size: 9pt; padding: 3px;"); state_layout.addWidget(self.py_sim_current_state_label); state_group.setLayout(state_layout); py_sim_layout.addWidget(state_group)
        variables_group = QGroupBox("Variables"); variables_layout = QVBoxLayout(); self.py_sim_variables_table = QTableWidget(); self.py_sim_variables_table.setRowCount(0); self.py_sim_variables_table.setColumnCount(2); self.py_sim_variables_table.setHorizontalHeaderLabels(["Name", "Value"]); self.py_sim_variables_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch); self.py_sim_variables_table.setSelectionMode(QAbstractItemView.NoSelection); self.py_sim_variables_table.setEditTriggers(QAbstractItemView.NoEditTriggers); variables_layout.addWidget(self.py_sim_variables_table); variables_group.setLayout(variables_layout); py_sim_layout.addWidget(variables_group)
        log_group = QGroupBox("Action Log"); log_layout = QVBoxLayout(); self.py_sim_action_log_output = QTextEdit(); self.py_sim_action_log_output.setReadOnly(True); self.py_sim_action_log_output.setStyleSheet("font-family: Consolas, monospace; font-size: 9pt;"); self.py_sim_action_log_output.setHtml("<i>Simulation log will appear here...</i>"); log_layout.addWidget(self.py_sim_action_log_output); log_group.setLayout(log_layout); py_sim_layout.addWidget(log_group, 1)
        self.py_sim_dock.setWidget(py_sim_widget); self.addDockWidget(Qt.RightDockWidgetArea, self.py_sim_dock); self.view_menu.addAction(self.py_sim_dock.toggleViewAction())
        
        # AI Chatbot Dock
        self.ai_chatbot_dock = QDockWidget("AI Chatbot", self); self.ai_chatbot_dock.setObjectName("AIChatbotDock"); self.ai_chatbot_dock.setAllowedAreas(Qt.LeftDockWidgetArea | Qt.RightDockWidgetArea | Qt.BottomDockWidgetArea)
        ai_chat_widget = QWidget(); ai_chat_layout = QVBoxLayout(ai_chat_widget); ai_chat_layout.setContentsMargins(5,5,5,5); ai_chat_layout.setSpacing(5)
        self.ai_chat_display = QTextEdit(); self.ai_chat_display.setReadOnly(True); self.ai_chat_display.setStyleSheet("font-size: 9pt; padding: 5px;"); self.ai_chat_display.setPlaceholderText("AI chat history will appear here..."); ai_chat_layout.addWidget(self.ai_chat_display, 1)
        input_layout = QHBoxLayout(); self.ai_chat_input = QLineEdit(); self.ai_chat_input.setPlaceholderText("Type your message to the AI..."); self.ai_chat_input.returnPressed.connect(self.on_send_ai_chat_message); input_layout.addWidget(self.ai_chat_input, 1); self.ai_chat_send_button = QPushButton(get_standard_icon(QStyle.SP_ArrowForward, "Snd"),"Send"); self.ai_chat_send_button.clicked.connect(self.on_send_ai_chat_message); input_layout.addWidget(self.ai_chat_send_button); ai_chat_layout.addLayout(input_layout)
        self.ai_chat_status_label = QLabel("Status: Initializing..."); self.ai_chat_status_label.setStyleSheet("font-size: 8pt; color: grey;"); ai_chat_layout.addWidget(self.ai_chat_status_label)
        self.ai_chatbot_dock.setWidget(ai_chat_widget); self.addDockWidget(Qt.RightDockWidgetArea, self.ai_chatbot_dock); self.view_menu.addAction(self.ai_chatbot_dock.toggleViewAction())
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False):
        ...
        # Add Graphviz PATH for layout
        graphviz_path = r"C:\Program Files\Graphviz\bin"
        os.environ["PATH"] += os.pathsep + graphviz_path

        # Create Graphviz graph for layout
        ...
        try:
            G.layout(prog="dot")
            print(f"--- MW_ADD_FSM ({current_time}): Graphviz layout successful ---")
        except Exception as e:
            self.log_message(f"Graphviz layout error: {str(e).strip() or 'Unknown error'}", type_hint="AIChatError")
            print(f"--- MW_ADD_FSM ({current_time}): Graphviz error: {e} ---")
        ...
        for i, state_data in enumerate(fsm_data.get('states', [])):
            ...
            # Use Graphviz positions if available
            ...
            try:
                ...
                pos_x = start_x + (i % items_per_row) * (default_item_width + padding_x)
                pos_y = start_y + (i // items_per_row) * (default_item_height + padding_y)
        ...
        comment_start_y = max_y_of_states + padding_y
        ...
        for i, comment_data in enumerate(fsm_data.get('comments', [])):
            ...
            if not text:
                continue
            comment_x = comment_data.get('x', comment_start_x + i * (150 + 20))
        ...
        # Initialize simulator for new FSM
        if self.py_sim_active:
            try:
                self.py_fsm_engine = FSMSimulator(fsm_data['states'], fsm_data['transitions'])
                self._append_to_py_simulation_log(["Python FSM Simulation reinitialized for new diagram."])
                self._update_py_simulation_dock_ui()
            except FSMError as e:
                self._append_to_py_simulation_log([f"ERROR Initializing Sim: {e}"])
                self.on_stop_py_simulation(silent=True)

        print(f"--- MW_ADD_FSM ({current_time}): Processing finished. Items queued: {len(items_to_add_for_undo_command)} ---")
