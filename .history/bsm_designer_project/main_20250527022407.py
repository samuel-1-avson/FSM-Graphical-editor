...
class MainWindow(QMainWindow):
    ...
    def _update_ai_features_enabled_state(self, is_online_and_key_present: bool):
        ...
        if self.ide_generate_code_action:
            can_ide_generate_code = is_online_and_key_present and \
            ...
            self.ide_generate_code_action.setToolTip(
                "Generate code snippet using AI (requires Internet & API Key)" if can_ide_generate_code
                else "Requires active IDE tab, Internet & API Key"
            )
    ...
    def _add_ide_tab(self, title="Untitled", content="", file_path=None, language="Python"):
        ...
        tab_content_widget._ide_tab_data = {
            ...
            "path": file_path,
            "is_dirty": False if file_path and not content else (True if not file_path else False), # New untitled are dirty, loaded files are not, new with content is dirty
            "language": language
        }
    ...
    def on_ide_open_file(self):
        ...
        if file_path:
            for i in range(self.ide_tab_widget.count()):
                tab_data_iter = self.ide_tab_widget.widget(i)._ide_tab_data
    ...
    def _save_specific_ide_tab(self, index: int) -> bool:
        ...
        if not file_to_save_path: 
            ...
            default_filename = current_tab_text if '.' in current_tab_text else current_tab_text + default_ext
    ...
    def on_ide_save_as_file(self) -> bool:
        ...
        if '.' in default_filename_base:
             default_filename = default_filename_base.rsplit('.',1)[0] + default_ext
        else:
             default_filename = default_filename_base + default_ext
    ...
    def _update_ide_related_actions(self):
        ...
        if self.ide_tab_widget:
            for i in range(self.ide_tab_widget.count()):
                ...
                if hasattr(widget, "_ide_tab_data") and widget._ide_tab_data.get("is_dirty", False):
        ...
        if self.ide_generate_code_action:
             can_ide_generate_code = is_online_and_key_present and \
                                self.ide_tab_widget is not None and self.ide_tab_widget.count() > 0
             self.ide_generate_code_action.setEnabled(can_ide_generate_code)
             self.ide_generate_code_action.setToolTip("Generate code snippet using AI (requires Internet & API Key)" if can_ide_generate_code else "Requires active IDE tab, Internet & API Key")
    ...
    def on_ide_save_all_files(self):
        ...
        # Iterate a copy of indices in case tabs are removed during save (though less likely with modal dialogs)
        indices_to_save = list(range(self.ide_tab_widget.count()))

        for i in indices_to_save:
            if i >= self.ide_tab_widget.count(): # Tab might have been removed by a prior save_as cancel
                continue
            ...
            if hasattr(widget, "_ide_tab_data"):
                tab_data = widget._ide_tab_data
                if tab_data["is_dirty"]:
                    # Temporarily make this tab active to ensure dialogs are parented correctly if needed by _save_specific_ide_tab
                    current_active_idx = self.ide_tab_widget.currentIndex()
                    ...
                    if not self._save_specific_ide_tab(i): 
                        ...
                        if reply == QMessageBox.No:
                            ...
                            if current_active_idx < self.ide_tab_widget.count(): # Restore original active tab if still valid
                                self.ide_tab_widget.setCurrentIndex(current_active_idx) 
                            break 
                    else: 
                        saved_count +=1
                    
                    if current_active_idx < self.ide_tab_widget.count() and current_active_idx != self.ide_tab_widget.currentIndex():
                         self.ide_tab_widget.setCurrentIndex(current_active_idx) 
