    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Name):
            ...
            if func_name in ('eval', 'exec', 'compile', 'open', 'input',
                ...
                # Heuristic: if not recognized, might be an attempt to call something from an unsafe scope
                # This is less critical than blocking specific dangerous calls, as `SAFE_BUILTINS` limits globals.
                # It could also be a typo by the user.
                # self.violations.append(f"Warning: Call to unrecognized function '{func_name}'. Ensure it's an FSM variable or allowed builtin.")
                pass # Keep it less noisy for now, focus on hard blocks
        super().generic_visit(node)
...
class StateMachinePoweredSimulator:
    def __init__(self, states_data, transitions_data, parent_simulator=None, log_prefix=""):
        ...
        self._halt_simulation_on_action_error = False # Future: make this configurable
        ...
        try:
            ...
            elif not self.sm: # Should be caught by FSMClass creation, but as a fallback
        ...
        def dynamic_callback_wrapper(*args, **kwargs):
            ...
            try:
                if callback_type == "action":
                    ...
                    result = eval(code_string, {"__builtins__": SAFE_BUILTINS}, exec_eval_locals_dict.copy()) # Pass a copy for eval
                ...
                if self._halt_simulation_on_action_error and callback_type == "action": raise FSMError(err_msg)
    ...
    def _build_fsm_class_and_instance(self):
        ...
        for t_data in self._transitions_input_data:
            ...
            if not event_name:
                event_name = f"_internal_{source_name}_to_{target_name}"
    ...
    def step(self, event_name=None):
        ...
        if main_state_data and main_state_data.get('during_action'):
            ...
            try: self._create_dynamic_callback(action_str, "action", f"during_{main_state_id}")()
            except Exception as e: self._log_action(f"Error in 'during' action '{action_str}': {e}"); logger.error(f"{self.log_prefix}During action error:", exc_info=True)
        ...
        if self.active_sub_simulator:
            ...
            except Exception as e_sub: self._log_action(f"Error in sub-machine step: {e_sub}"); logger.error(f"{self.log_prefix}Sub-machine step error:", exc_info=True)

        if event_name:
            try: self.sm.send(event_name)
                ...
                if hasattr(self.sm, event_name) and callable(getattr(self.sm, event_name)): log_msg = f"AttributeError processing '{event_name}': {e}. Internal setup issue?"
        ...
        elif not self.active_sub_simulator: self._log_action(f"No event. 'During' actions done. State remains '{main_state_id}'.")
        return self.get_current_state_name(), self.get_last_executed_actions_log()
...
FSMSimulator = StateMachinePoweredSimulator # Alias

if __name__ == "__main__": # Unchanged test block from previous version
    main_states_data = [
    ...
    try:
        ...
        unsafe_states = [{"name": "A", "is_initial": True, "entry_action": "import os; os.system('echo UNSAFE MALICIOUS CODE')"}]
        unsafe_sim = FSMSimulator(unsafe_states, []) # Should log safety violation during on_enter_A
        print_status(unsafe_sim, "Unsafe Sim Start (check logs for blocking)")
