...
class DiagramScene(QGraphicsScene):
    ...
    def __init__(self, undo_stack, parent_window=None):
        ...
        self.item_moved.connect(self._handle_item_moved_visual_update) # This line caused the error
    ...
    def edit_item_properties(self, item):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if isinstance(item, GraphicsHistoryPseudoStateItem) and 'default_target_substate_name' in new_props_from_dialog:
                # Convert name to UUID for the command data
                target_name = new_props_from_dialog.pop('default_target_substate_name')
                if target_name:
                    target_state = self.get_state_by_name(target_name) # Assumes sub-states are findable by name in this context
                    if target_state:
                        new_props_from_dialog['default_target_substate_uuid'] = target_state.uuid
                    else:
                        new_props_from_dialog['default_target_substate_uuid'] = None
                        logger.warning(f"Could not find state named '{target_name}' to set as default target for H-item {item.uuid}.")
                else:
                    new_props_from_dialog['default_target_substate_uuid'] = None
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None, parent_item:GraphicsStateItem=None):
        ...
        if item_type == "State":
            ...
            if h_props_dialog.exec() == QDialog.Accepted:
                ...
                default_target_item = None
                if final_h_props.get('default_target_substate_name'):
                    # In sub-editor, self is the DiagramScene for the sub-machine
                    # In main editor, self is the main DiagramScene
                    # We need to find the substate by name within the parent_item's sub_fsm_data
                    # or if the sub-editor is open, within its scene.
                    # For simplicity, assume parent_item.sub_fsm_data contains names,
                    # but for robustness, it should contain UUIDs.
                    # Here, we try to get the actual GraphicsStateItem if possible.
                    sub_states_in_parent = [s_data for s_data in parent_item.sub_fsm_data.get('states', [])]
                    target_s_data = next((s for s in sub_states_in_parent if s.get('name') == final_h_props['default_target_substate_name']), None)
                    if target_s_data and target_s_data.get('uuid'): # If UUID is available in sub_fsm_data
                        default_target_item = self.get_item_by_uuid(target_s_data['uuid']) # Assuming sub-states are in the same scene if sub-editor
                    if not default_target_item: # Fallback to name if UUID not found or not in sub_fsm_data
                        default_target_item = self.get_state_by_name(final_h_props.get('default_target_substate_name'))
    ...
    # ... (Remaining methods: keyPressEvent, delete_selected_items, drag/drop events, drawBackground/Foreground, ZoomableView)
    # These generally don't need changes for UUIDs beyond what's already covered by item/command updates.
    # For instance, delete_selected_items passes item references to RemoveItemsCommand, which now uses UUIDs.
    # copy_selected_items gets data via item.get_data() which now includes UUID.
    # paste_items_from_clipboard and _add_template_to_scene are the main areas that need careful UUID handling,
    # which has been addressed by generating new UUIDs and using maps for internal linking.
