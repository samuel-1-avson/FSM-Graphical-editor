...
class ChatbotWorker(QObject):
    ...
    def set_api_key(self, api_key: str):
        logger.info(f"WORKER: set_api_key called (new key {'SET' if api_key else 'NOT SET'}).")
        self.api_key = api_key
        self._initialize_client()

    @pyqtSlot(str) # Changed signature to always expect str
    def set_current_diagram_context(self, diagram_json_str: str): 
        if not diagram_json_str: # Treat empty string as if None was intended
            logger.debug(f"WORKER: Setting diagram context to None (received empty or null-equivalent string).")
            self.current_diagram_context_json_str = None
        else:
    ...
    def process_message(self, user_message: str, force_fsm_generation: bool = False):
        ...
        # current_diagram_context_json_str is now guaranteed to be a string or None by the time it's used here by the slot logic
        if self.current_diagram_context_json_str: # This will be true if it's a non-empty string
            try:
        ...
        else: # current_diagram_context_json_str is None (was an empty string passed to the slot)
             system_prompt_content += " No diagram context was provided for this request."


        if is_fsm_generation_attempt:
            system_prompt_content += (
                " When asked to generate an FSM, you MUST respond with ONLY a valid JSON object that directly represents the FSM data. " # ... (rest of the long prompt)
...
class AIChatbotManager(QObject):
    ...
    def set_api_key(self, api_key: str | None):
        ...
        self.api_key = api_key # This is the string or None
        ...
        if old_api_key_val != self.api_key or (self.api_key and (not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning())):
            ...
            if self.api_key: # This checks if self.api_key is not None and not empty
                self._setup_worker()
            else:
             ...
             # Pass the actual api_key string. If it was None and cleared, this branch isn't hit.
             QMetaObject.invokeMethod(self.chatbot_worker, "set_api_key", Qt.QueuedConnection, 
                                      self.api_key) 
    ...
    def _setup_worker(self):
        if not self.api_key: # Should be true if api_key is None or empty
        ...
        self.chatbot_worker = ChatbotWorker(self.api_key) # Pass the api_key string
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        if self.chatbot_worker: 
            # Prepare the string for the slot, ensuring it's never None for a @pyqtSlot(str)
            ...
            QMetaObject.invokeMethod(self.chatbot_worker, "set_current_diagram_context", Qt.QueuedConnection, 
                                     effective_diagram_json_str)

            QMetaObject.invokeMethod(self.chatbot_worker, "process_message", Qt.QueuedConnection,
                                     user_message_text, is_fsm_gen_specific) # Python bools are fine
