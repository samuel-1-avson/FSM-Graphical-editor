...
class MatlabConnection(QObject):
    ...
    simulinkStatusChanged = pyqtSignal(bool, str)
    ...
    def test_connection(self):
        ...
        try:
            ...
            self.connectionStatusChanged.emit(
                False, f"MATLAB error during test: {e.stderr or e.stdout or str(e)}".splitlines()[0])
            return False
                ...
                try:
                    ...
                    if test_process.returncode == 0:
                        if self.set_matlab_path(path_candidate):
                            self.test_simulink_available()  # <-- auto-test Simulink immediately
                            return True
    ...
    def test_simulink_available(self):
        """
        Checks if Simulink is installed and licensed.
        Emits simulinkStatusChanged with details and returns True/False.
        """
        if not self.connected:
            self.simulink_available = False
            self.simulinkStatusChanged.emit(False, "MATLAB not connected, can't check Simulink.")
            return False

        try:
            # Compose one-line script to test for Simulink and license
            script = (
                "try, "
                "si=ver('simulink'); "
                "if isempty(si), error('Simulink not installed.'); end; "
                "if ~license('test','Simulink'), error('Simulink license not available.'); end; "
                "disp('SIMULINK_AVAILABLE'), "
                "catch e, disp(['SIMULINK_CHECK_ERROR:',e.message]), end"
            )
            cmd = [
                self.matlab_path, "-nodisplay", "-nosplash", "-nodesktop", "-batch", script
            ]
            process = subprocess.run(cmd, capture_output=True, text=True, timeout=40, check=False,
                                     creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0)
            if "SIMULINK_AVAILABLE" in process.stdout:
                self.simulink_available = True
                self.simulinkStatusChanged.emit(True, "Simulink detected and licensed.")
                return True
            elif "SIMULINK_CHECK_ERROR:" in process.stdout:
                self.simulink_available = False
                msg = process.stdout.split("SIMULINK_CHECK_ERROR:")[-1].splitlines()[0]
                self.simulinkStatusChanged.emit(False, f"Simulink check error: {msg.strip()}")
                return False
            else:
                self.simulink_available = False
                msg = process.stderr or "Undetermined error."
                self.simulinkStatusChanged.emit(False, f"Unknown Simulink detection error: {msg[:200]}")
                return False
        except Exception as e:
            self.simulink_available = False
            self.simulinkStatusChanged.emit(False, f"Error running Simulink check: {str(e)}")
            return False

    # (all previous methods for _run_matlab_script, generate_simulink_model, run_simulation, generate_code, MatlabCommandWorker...)
    # (snipped, paste all previous methods below unchanged)
