...
class BasePropertiesDialog(QDialog):
    ...
    def create_color_button(self, initial_color: str) -> QPushButton:
        ...
        self._update_color_button_style(button, color) # Pass button and color
        return button, color # Return both button and QColor object
...
class StatePropertiesDialog(BasePropertiesDialog):
    ...
    def _choose_color(self):
        ...
        if color_dialog_result.isValid():
            ...
            self._update_color_button_style(self.color_button, self.current_color) # Pass button and new color
...
class AppearanceSettingsTab(QWidget):
    ...
    def _choose_grid_color(self, grid_type_str):
        ...
        color_dialog_result = QColorDialog.getColor(getattr(self, current_color_attr_name), self, title)
        if color_dialog_result.isValid():
            setattr(self, current_color_attr_name, color_dialog_result)
    ...
    def apply_specific_settings(self, data: Dict[str, Any]):
        scene = self.main_window.scene
        settings_qt = QSettings(APP_NAME, "AppearanceSettings")
        ...
        if current_app_font_size != new_font_size:
            ...
            from graphics_items import GraphicsStateItem, GraphicsTransitionItem # Local import for type check
...
# Renamed to PreferencesDialog in main.py and dialogs.py (where it's defined as GeneralSettingsDialog)
# This class definition is from dialogs.py
class PreferencesDialog(QDialog): # In dialogs.py, this is GeneralSettingsDialog
    def __init__(self, main_window, parent=None): # main_window needed for settings access
        super().__init__(parent)
        self.main_window = main_window
        self.setWindowTitle(f"{APP_NAME} - Preferences") # Updated title
        self.setWindowIcon(get_standard_icon(QStyle.SP_FileDialogDetailedView, "Settings"))
        self.setMinimumSize(550, 450)
        self.settings_changed = False # Flag to track if Apply was clicked
    ...
    def _create_ai_settings_tab(self):
        ...
        current_api_key = settings_qt.value("api_key", self.main_window.ai_chatbot_manager.config.api_key if self.main_window.ai_chatbot_manager.config else "", type=str)
        ...
        self.model_name_combo_tab.addItems(["gpt-3.5-turbo", "gpt-4", "gpt-4-turbo-preview", "gpt-4o"]) # Updated gpt-4-turbo
        default_model = self.main_window.ai_chatbot_manager.config.model_name
    ...
    def apply_all_settings(self):
        ...
        if (self.main_window.ai_chatbot_manager.config.api_key or "") != new_api_key : # Compare with internal config api_key
