...
from PyQt5.QtCore import Qt, QRectF, QPointF, QMimeData, QPoint, QLineF, QSize, pyqtSignal, QObject
from PyQt5.QtCore import QRectF # Ensure QRectF is explicitly imported
from config import (COLOR_ITEM_STATE_DEFAULT_BG, COLOR_ITEM_STATE_DEFAULT_BORDER, APP_FONT_FAMILY,
                    ...
                    DEFAULT_EXECUTION_ENV, COLOR_EDITOR_DARK_BACKGROUND)


class GraphicsStateItem(QObject, QGraphicsRectItem):
    Type = QGraphicsItem.UserType + 1
    nameChanged = pyqtSignal(QGraphicsItem, str, str)
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
                 ...
                 parent_item=None, q_object_parent=None): # q_object_parent for QObject, parent_item for QGraphicsItem
        # Initialize QGraphicsRectItem FIRST with its graphics parent.
        # Use QRectF overload to avoid potential signature ambiguity with floats and parent
        rect = QRectF(float(x), float(y), float(w), float(h))
        QGraphicsRectItem.__init__(self, rect, parent=parent_item) # Use named 'parent' argument
        # THEN initialize QObject for its distinct parenting (for signals/slots, event system)
        QObject.__init__(self, parent=q_object_parent) # Use named 'parent' argument
        
        self.text_label = text
        self.is_initial = is_initial
        self.is_final = is_final
        # ... (rest of GraphicsStateItem __init__ as before) ...
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        if self.is_superstate:
            ...
            painter.setBrush(self.border_color.lighter(140))
    ...
    def itemChange(self, change, value):
        ...
        # Use super() which will correctly call QGraphicsRectItem's itemChange
    ...
    def set_text(self, text):
        if self.text_label != text:
            old_name = self.text_label
            self.prepareGeometryChange()
            self.text_label = text
            self.update()
            if hasattr(self, 'nameChanged'):
                self.nameChanged.emit(self, old_name, self.text_label)
    ...
    def set_properties(self, name, is_initial, is_final, color_hex=None,
        ...
        changed = False
        old_name_prop = self.text_label
        if self.text_label != name:
            self.text_label = name
            changed = True
            if hasattr(self, 'nameChanged'):
                self.nameChanged.emit(self, old_name_prop, self.text_label)
...
class GraphicsTransitionItem(QObject, QGraphicsPathItem):
    ...
    def __init__(self, start_item, end_item, event_str="", condition_str="", action_str="",
                 ...
                 parent_item=None, q_object_parent=None):
        # Initialize QGraphicsPathItem FIRST with its graphics parent.
        QGraphicsPathItem.__init__(self, parent=parent_item)
        # THEN initialize QObject for its distinct parenting
        QObject.__init__(self, parent=q_object_parent)


        self.start_item: GraphicsStateItem | None = start_item
        self.end_item: GraphicsStateItem | None = end_item
        # ... (rest of GraphicsTransitionItem __init__ as before) ...
    ...
    def itemChange(self, change, value):
        return super().itemChange(change, value) # Use super() for QGraphicsPathItem's itemChange
    ...
    def hoverEnterEvent(self, event: QGraphicsSceneMouseEvent):
        if not self._validation_highlight_pen:
    ...
    def boundingRect(self):
        extra = (self.pen().widthF() + self.arrow_size) / 2.0 + 35
        ...
        if current_label:
            ...
            text_render_rect = QRectF(mid_point_on_path.x() - text_rect.width()/2 - 20,
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        if self._validation_highlight_pen:
            current_pen = self._validation_highlight_pen
        ...
        angle_at_end_rad = -self.path().angleAtPercent(percent_at_end_for_angle) * (math.pi / 180.0)
        ...
        if current_label:
            ...
            text_final_pos = QPointF(text_center_x - text_rect_original.width() / 2,
                                     text_center_y - text_rect_original.height() / 2 + fm.ascent()/2 -2 )

            bg_padding = 3
            bg_rect = QRectF(text_final_pos.x() - bg_padding, text_final_pos.y() - bg_padding - fm.ascent()/2 +2,
...
class GraphicsCommentItem(QGraphicsTextItem): # QGraphicsTextItem inherits QGraphicsObject which inherits QObject
    Type = QGraphicsItem.UserType + 3
    def type(self): return GraphicsCommentItem.Type

    def __init__(self, x, y, text="Comment", parent_item=None):
        # Initialize QGraphicsTextItem FIRST with its graphics parent.
        # Note: QGraphicsTextItem's constructor can take (parent) or (text, parent)
        # To avoid ambiguity if 'text' could be mistaken for a parent, it's safer
        # to set plain text after construction if not using named args (which __init__ often doesn't have for parent).
        super().__init__(parent=parent_item) # Call QGraphicsTextItem's constructor
        self.setPlainText(text) # Set text after initial construction.
    ...
    def _on_contents_changed(self):
        self.prepareGeometryChange()
        if self.scene() and hasattr(self.scene(), 'item_moved'):
             self.scene().item_moved.emit(self)
    ...
    def paint(self, painter, option, widget):
        ...
        if self._validation_highlight_pen:
            current_border_pen = self._validation_highlight_pen
        ...
        if self.isSelected() and not self._validation_highlight_pen:
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.ItemPositionHasChanged and self.scene():
            if hasattr(self.scene(), 'item_moved'):
                self.scene().item_moved.emit(self)
        return super().itemChange(change, value) # Correctly call super for QGraphicsTextItem