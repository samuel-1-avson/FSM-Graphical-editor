...
# from PyQt5.QtCore import QObject, pyqtSignal # Already imported via Qt
...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    # textChangedViaInlineEdit = pyqtSignal(str, str) # Moved to StateItemSignals
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
        ...
        self.signals = StateItemSignals() # Initialize signals object
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        border_radius = 12 
        # No explicit super().paint(), as we're custom drawing the rounded rect
        ...
        if self.is_superstate:
            ...
            if not superstate_icon.isNull():
                ...
                if self.base_color.lightnessF() < 0.3: pen_color = QColor(Qt.lightGray)
    ...
    def _handle_editor_key_press(self, event: QKeyEvent, editor_widget: QLineEdit):
        if event.key() == Qt.Key_Escape:
            self._inline_edit_aborted = True 
    ...
    def _finish_inline_edit(self, editor_widget: QLineEdit | None = None):
        ...
        if self._inline_edit_aborted: # If aborted, just redraw original and exit
            self.update() 
            return

        if not new_text: # Prevent empty names
            QMessageBox.warning(None, "Invalid Name", "State name cannot be empty.")
            self.update() # Redraw with original name
            return

        if new_text != old_text: # Only proceed if text actually changed and is not empty
            ...
            if hasattr(self.signals, 'textChangedViaInlineEdit'):
                 self.signals.textChangedViaInlineEdit.emit(old_text, new_text)
    ...
    def set_properties(self, name, is_initial, is_final, color_hex=None,
        ...
        if sub_fsm_data_prop is not None:
            ...
            elif self.is_superstate and not sub_fsm_data_prop: # Superstate but sub_fsm_data is cleared
                self.sub_fsm_data = {'states': [], 'transitions': [], 'comments': []}; changed = True
        ...
        if changed:
            ...
            self.original_pen_for_py_sim_restore = base_pen # Keep this updated
            self.update()
...
class GraphicsTransitionItem(QGraphicsPathItem): 
    ...
    def _get_intersection_point(self, item: QGraphicsRectItem, line: QLineF):
        ...
        intersect_points = []
        for edge in edges:
            intersection_point_var = QPointF(); intersect_type = line.intersect(edge, intersection_point_var) # intersection_type is deprecated, use intersectType
...
class GraphicsCommentItem(QGraphicsTextItem): 
    ...
    def _on_contents_changed(self): 
        self.prepareGeometryChange()
        self.update()
        # If scene available and has item_moved signal, could emit it if geometry really changed,
        # but this is usually for position. Size changes are handled by prepareGeometryChange/update.
        # if self.scene() and hasattr(self.scene(), 'item_moved'):
        #     self.scene().item_moved.emit(self) # This might be too frequent
    ...
    def start_inline_edit(self):
        ...
        # Estimate current size for editor, ensuring it's reasonable
        ...
        editor_width = int(max(current_doc_width, self._default_width * 0.8)) + 20 # Add padding
        editor_height = int(current_doc_height) + 20 # Add padding
        ...
        # Install an event filter on the editor to handle focus out and key presses
        # This is often more robust than subclassing or direct signal connections for QTextEdit's end-of-edit.
        editor.installEventFilter(self) # The item itself will filter events for the editor

        self._inline_editor_proxy = QGraphicsProxyWidget(self); self._inline_editor_proxy.setWidget(editor)
        self._inline_editor_proxy.setPos(0,0); # Position proxy at item's origin
        editor.setFocus(Qt.MouseFocusReason)
        cursor = editor.textCursor(); cursor.movePosition(cursor.End); editor.setTextCursor(cursor)

    def eventFilter(self, watched_object, event):
        # This event filter is specifically for the inline QTextEdit editor
        if self._is_editing_inline and self._inline_editor_proxy and watched_object == self._inline_editor_proxy.widget():
            if event.type() == QEvent.FocusOut:
                self._finish_inline_edit(self._inline_editor_proxy.widget())
                return True # Event handled
            elif event.type() == QEvent.KeyPress:
                key_event = QKeyEvent(event) # Cast to QKeyEvent
                if key_event.key() == Qt.Key_Escape:
                    self._inline_edit_aborted = True
                    self._inline_editor_proxy.widget().clearFocus() # Triggers FocusOut
                    return True # Event handled
                elif key_event.key() in (Qt.Key_Return, Qt.Key_Enter):
                    if not (key_event.modifiers() & Qt.ShiftModifier): # Enter without Shift
                        self._inline_edit_aborted = False
                        self._inline_editor_proxy.widget().clearFocus() # Triggers FocusOut
                        return True # Event handled
    ...
    def _finish_inline_edit(self, editor_widget: QTextEdit | None = None):
        ...
        new_text = actual_editor.toPlainText() 
        old_text = self.toPlainText() 
        
        # Cleanup proxy and editor first
        if self._inline_editor_proxy:
            actual_editor.removeEventFilter(self) # Remove event filter
            self._inline_editor_proxy.setWidget(None)
            actual_editor.deleteLater() # Schedule editor for deletion
        ...
        self._is_editing_inline = False # Reset flag *before* any potential QMessageBox
        
        if self._inline_edit_aborted:
            if new_text.strip() != old_text.strip(): # If text was changed before abort
                self.setPlainText(old_text) # Explicitly revert
            self.update()
            return

        if not new_text.strip() and old_text.strip(): # Text was cleared
             QMessageBox.warning(None, "Empty Comment", "Comment text cannot be empty if it previously had content. Edit cancelled.")
             self.setPlainText(old_text) # Revert to old text
             self.update()
             return
        elif not new_text.strip() and not old_text.strip(): # Was empty, still empty
            self.update()
            return
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_F2 and self.isSelected() and self.flags() & QGraphicsItem.ItemIsFocusable:
            ...
            return # Consume F2 event here
        
        # If item has focus and text interaction is enabled, allow QGraphicsTextItem to handle keys for selection, copy/paste etc.
        # BUT, do not let it handle typing if we are about to start inline editing or are already.
        if self.textInteractionFlags() & Qt.TextEditorInteraction and self.hasFocus() and not self._is_editing_inline:
            # Check if the key is a character key that would start typing
            if event.text() and not event.modifiers() & (Qt.ControlModifier | Qt.AltModifier | Qt.MetaModifier):
                 # If it's a character key, and we are not editing, initiate inline edit
                 self.start_inline_edit()
                 if self._inline_editor_proxy and self._inline_editor_proxy.widget():
                     QApplication.sendEvent(self._inline_editor_proxy.widget(), event) # Forward the event
                 event.accept()
                 return
            else: # For non-character keys like Ctrl+C, allow superclass
                super().keyPressEvent(event)
        elif not event.isAccepted(): # Fallback for other keys if not handled
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.ItemPositionHasChanged and self.scene():
            ...
            if not value: # If item is deselected
                if self._is_editing_inline and self._inline_editor_proxy and self._inline_editor_proxy.widget():
                    # If deselected while editing, finish the edit (commit)
                    self._inline_edit_aborted = False # Assume commit on deselection
                    self._inline_editor_proxy.widget().clearFocus() # This will trigger focusOut then _finish_inline_edit
        return super().itemChange(change, value)