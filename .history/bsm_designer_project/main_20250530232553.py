...
import re
import logging
from PyQt5.QtCore import QTime, QTimer, QPointF, QMetaObject, QFile, QTemporaryFile, QDir, QIODevice, QFileInfo
...
from config import (
    ...
    FSM_TEMPLATES_BUILTIN # Added for templates
)
...
class DraggableToolButton(QPushButton):
    def __init__(self, text, mime_type, item_type_data_str, parent=None): # item_type_data is now string
        super().__init__(text, parent)
        self.setObjectName("DraggableToolButton")
        self.mime_type = mime_type
        self.item_type_data_str = item_type_data_str # Store the string
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        drag = QDrag(self)
        mime_data = QMimeData()
        # For templates, item_type_data_str is the JSON string
        mime_data.setData(self.mime_type, self.item_type_data_str.encode('utf-8'))

        # Optional: set text for simple drag feedback or non-Qt targets
        if self.mime_type == "application/x-bsm-template":
            try:
                template_obj = json.loads(self.item_type_data_str)
                mime_data.setText(f"FSM Template: {template_obj.get('name', 'Custom')}")
            except: # Fallback if JSON parsing fails (should not happen for valid templates)
                mime_data.setText("FSM Template")
        else: # Existing logic for single elements like "State", "Comment"
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.scene.scene_content_changed_for_find.connect(self._refresh_find_dialog_if_visible) # Connect new signal
    ...
    def _create_docks(self):
        ...
        self._load_and_display_templates() # Populate templates after docks are created
    ...
    def _load_and_display_templates(self):
        ...
        templates_to_load = []
        # FSM_TEMPLATES_BUILTIN should be imported from config
        ...
        # TODO: Load user-defined templates
        ...
        for template_info in templates_to_load:
            ...
            template_btn = DraggableToolButton(
                template_info["name"],
                "application/x-bsm-template", # Custom MIME type for templates
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        for i, state_data in enumerate(fsm_data.get('states', [])):
            ...
            try:
                ...
                self.connect_state_item_signals(state_item) # Connect signals
    ...
    def on_new_file(self, silent=False):
        ...
        self.view.centerOn(self.scene.sceneRect().center())
        if hasattr(self, 'select_mode_action'): self.select_mode_action.trigger()
        self._refresh_find_dialog_if_visible() # Refresh find dialog
        return True
    ...
    def _load_from_path(self, file_path):
        try:
            ...
            # Clear existing scene first (load_diagram_data in scene also clears)
            self.scene.clear()
            self.scene.load_diagram_data(data) # This will create new items

            # After items are loaded by scene.load_diagram_data, connect signals for newly loaded state items
            for item in self.scene.items():
                if isinstance(item, GraphicsStateItem):
    ...
    def on_show_find_item_dialog(self):
        if not self.find_item_dialog:
            ...
            # Connect scene content changes after find_item_dialog is created
    ...
    def _handle_state_renamed_inline(self, old_name: str, new_name: str):
        ...
        self._refresh_find_dialog_if_visible()
        if self.scene.selectedItems() and isinstance(self.scene.selectedItems()[0], GraphicsStateItem) and self.scene.selectedItems()[0].text_label == new_name:
            self._update_properties_dock()
