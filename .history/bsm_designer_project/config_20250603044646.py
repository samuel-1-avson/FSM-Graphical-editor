...
MECHATRONICS_SNIPPETS = {
    EXECUTION_ENV_PYTHON_GENERIC: { # For simulation purposes
        "actions": {
        ...
        "events": { # Events often map to abstract signals or abstract conditions
        ...
        "conditions": { # Conditions might check the state of these virtual signals
    ...
    EXECUTION_ENV_ARDUINO_CPP: {
        ...
        "events": { # In C, events are often just symbolic names for the FSM logic
            "Button_Press_Event": "EVT_BUTTON_PRESS", # Will be used in fsm_run(EVT_BUTTON_PRESS)
            "Timer_Expired_Event": "EVT_TIMER_EXPIRED",
            "Sensor_Data_Ready_Event": "EVT_SENSOR_READY",
        },
        "conditions": { # These would be actual C expressions
            ...
            "Custom Flag is True": "my_global_flag == 1", # or true
            "Function Returns True": "checkMyHardwareCondition() == true /* Implement this function */",
        }
    },
    EXECUTION_ENV_RASPBERRYPI_PYTHON: { # Using RPi.GPIO often
        "actions": {
        ...
        "conditions": {
            "GPIO Input is High": "GPIO.input(MY_GPIO_IN) == GPIO.HIGH", # Assumes RPi.GPIO imported and set up
    ...
    EXECUTION_ENV_MICROPYTHON: {
        ...
        "events": {
             "IRQ Pin Event": "EVENT_PIN_IRQ", # Symbolic
             "Timer Callback Event": "EVENT_TIMER_CB" # Symbolic
        },
        "conditions": {
            "Pin Value is High": "my_input_pin.value() == 1", # Assumes my_input_pin is defined
...
COLOR_ITEM_HISTORY_PSEUDO_STATE_BG = "#E8EAF6"  # Light Indigo
COLOR_ITEM_HISTORY_PSEUDO_STATE_BORDER = "#7986CB" # Indigo
COLOR_ITEM_HISTORY_PSEUDO_STATE_TEXT = COLOR_TEXT_PRIMARY
DEFAULT_HISTORY_TRANSITION_COLOR = "#7986CB" # Indigo
DEFAULT_HISTORY_TRANSITION_STYLE = Qt.DashLine # Using Qt.DashLine for the enum value
...
STYLE_SHEET_GLOBAL = f"""
    ...
    QTextEdit#SubFSMJsonEditor /* Added for SubFSM JSON Editor */
    {{
    ...
    QDockWidget#ProblemsDock QListWidget::item {{ /* Added for Problems Dock */
        padding: 5px;
        border-bottom: 1px dotted {COLOR_BORDER_LIGHT};
    }}
    QDockWidget#ProblemsDock QListWidget::item:selected {{ /* Added for Problems Dock */
        background-color: {COLOR_ACCENT_PRIMARY_LIGHT};
        color: {COLOR_TEXT_PRIMARY};
    }}
