...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
        ...
        # --- NEW: Hover state attributes ---
        self._is_hovered = False
        self.hover_border_color = self.border_color.lighter(130) # Brighter border on hover
        self.hover_brush_color = self.base_color.lighter(105)   # Slightly lighter fill on hover
        # --- END NEW ---
        ...
        self.setAcceptHoverEvents(True) # Ensure this is True
        ...
        self.original_brush_for_py_sim_restore = self.brush() # Store original brush too
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        # Apply hover effect if not selected and not in PySim active mode
        if self._is_hovered and not self.isSelected() and not self.is_py_sim_active:
            current_pen_to_use = QPen(self.hover_border_color, self._border_pen_width + 0.5) # Slightly thicker
            current_brush_to_use = QBrush(self.hover_brush_color)
        # elif not self._is_hovered and not self.isSelected() and not self.is_py_sim_active: # Revert if not hovered/selected/sim
            # current_pen_to_use = QPen(self.border_color, self._border_pen_width)
            # current_brush_to_use = QBrush(self.base_color)
        ...
        if self.is_py_sim_active:
            ...
            # Optionally, you might want a specific brush for PySim active state too
            # current_brush_to_use = QBrush(COLOR_PY_SIM_STATE_ACTIVE.lighter(150))
        ...
        if self.isSelected() and not self.is_py_sim_active: # Selection highlight takes precedence over hover if not in sim
    ...
    def set_py_sim_active_style(self, active: bool):
        ...
        if active:
            self.original_pen_for_py_sim_restore = QPen(self.pen()) # Store current pen before changing
            self.original_brush_for_py_sim_restore = QBrush(self.brush()) # Store current brush
        else:
            self.setPen(self.original_pen_for_py_sim_restore) # Restore original pen
            self.setBrush(self.original_brush_for_py_sim_restore) # Restore original brush
        self.update()
    ...
    def set_text(self, text): # Primarily for direct name changes if ever needed outside full props
     ...
     def set_properties(self, name, is_initial, is_final, color_hex=None,
        ...
        if self.base_color != new_base_color:
            ...
            # --- UPDATE HOVER COLORS WHEN BASE COLOR CHANGES ---
            ...
            self.original_brush_for_py_sim_restore = QBrush(self.base_color) # Also update backup brush
            changed = True
