...
class MainWindow(QMainWindow):
    ...
    def _clear_dock_editors(self):
        # Iterate backwards when removing items from a layout
        for i in range(self.properties_editor_layout.rowCount() - 1, -1, -1):
            # Get the layout item for the field
            # We are less certain about the label's lifecycle if QFormLayout auto-creates it.
            field_layout_item = self.properties_editor_layout.itemAt(i, QFormLayout.FieldRole)

            # Remove the row. This action will also remove the label associated with this row
            # and potentially delete it if QFormLayout owns it.
            # It will also remove the field_layout_item from the form layout.
            self.properties_editor_layout.removeRow(i) # This should handle the label.

            if field_layout_item:
                # Check if the field item contains a widget or another layout
                field_widget = field_layout_item.widget()
                field_nested_layout = field_layout_item.layout()

                if field_widget:
                    # If it's a widget, schedule it for deletion.
                    # It's generally safe to call deleteLater multiple times or on already deleted items.
                    field_widget.deleteLater()
                elif field_nested_layout:
                    # If it's a nested layout, clear its contents and then delete the layout.
                    while field_nested_layout.count():
                        child_item = field_nested_layout.takeAt(0)
                        if child_item:
                            child_widget = child_item.widget()
                            if child_widget:
                                child_widget.deleteLater()
                            # If the child item itself is a layout, delete it
                            # (not handling further recursion here for simplicity, assuming max one level of nested layout in fields)
                            grandchild_layout = child_item.layout()
                            if grandchild_layout:
                                grandchild_layout.deleteLater()
                    field_nested_layout.deleteLater() # Delete the nested layout itself
