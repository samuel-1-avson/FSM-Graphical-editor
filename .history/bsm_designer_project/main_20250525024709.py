Okay, I've analyzed the provided codebase with a focus on `main.py`.

Here's the updated `main.py` with fixes and improvements:

1.  **Python Simulation Event Handling for Hierarchical States**:
    *   The `_update_py_simulation_dock_ui` method has been updated to populate the event combo box with events from both the active sub-simulator (if any) and the main FSM's current state. This provides a more complete list of potentially triggerable events in a hierarchical context. The actual event dispatch logic in `fsm_simulator.py` would determine how these events are processed.

2.  **Logging Workaround for `undo_commands.py`**:
    *   A "monkey-patch" has been added in `MainWindow.__init__` to define `self.scene.log_function`. This is a workaround because `undo_commands.py` (which is not part of this fix request) appears to call `self.scene.log_function(...)` which doesn't exist on `DiagramScene`. The proper fix would be to change `undo_commands.py` to use `self.scene._log_to_parent("ERROR", ...)`. This workaround makes `main.py` more robust to this external issue.

3.  **Minor Code Clarity/Style**:
    *   Ensured consistency and added comments where beneficial.

No other significant bugs were identified *within* `main.py` itself based on the provided code, assuming the fixes noted in `ai_chatbot.py` (for `QMetaObject.invokeMethod`) and `config.py` (closing triple quote) are in place. The `fsm_simulator.py` correctly handles hierarchical simulation setup and basic step/reset logic for sub-machines.
...
class DraggableToolButton(QPushButton): # Unchanged from previous correct version
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Monkey-patch for undo_commands.py if it calls log_function (better to fix undo_commands.py)
        if not hasattr(self.scene, 'log_function'):
            ...
            # This lambda captures self.scene from the MainWindow context.
            # When self.scene._log_to_parent is called, its 'self' is the DiagramScene instance.
        ...
        self._py_sim_currently_highlighted_transition: GraphicsTransitionItem | None = None # Not currently used but kept for future

        self._internet_connected: bool | None = None
        self.internet_check_timer = QTimer(self)

        self.init_ui() # self.log_output is created here

        try:
            setup_global_logging(self.log_output) # self.log_output must exist before this call
            logger.info("Main window initialized and logging configured.")
        except AttributeError as e: # e.g. if self.log_output doesn't exist due to init_ui order
            logger.error(f"Failed to run setup_global_logging: {e}. UI logs might not work correctly.")
        except NameError: # If setup_global_logging itself is not found
        ...
        # Ensure object names are set after UI creation
    ...
    def init_ui(self): # Unchanged from previous correct version
        ...
        self._create_status_bar() # Needs to be after docks if log_output is used by logging_setup
    ...
    def _create_actions(self): # Unchanged from previous correct version
        ...
        self.save_as_action = QAction(get_standard_icon(_safe_get_style_enum("SP_DriveHDIcon","SP_DialogSaveButton"),"Save &As...", self, shortcut=QKeySequence.SaveAs, statusTip="Save the current file with a new name", triggered=self.on_save_file_as) # Fallback for SP_DriveHDIcon
    ...
    def _create_menus(self): # Unchanged from previous correct version
    ...
    def _create_toolbars(self): # Unchanged from previous correct version
    ...
    def _create_status_bar(self): # Unchanged from previous correct version
    ...
    def _create_docks(self): # Unchanged from previous correct version
        ...
        self.log_output = QTextEdit() # QTextEdit for logs is created here
    ...
    # --- AI Chatbot Methods --- (Unchanged from previous correct version)
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        # Default layout parameters if Graphviz fails or is not used
        layout_start_x, layout_start_y = 100, 100
        items_per_row = 3
        default_item_width, default_item_height = 120, 60 # Must match GraphicsStateItem defaults if not in data
        padding_x, padding_y = 150, 100 # Spacing for grid layout
        GV_SCALE = 1.2 # Scale factor for Graphviz coordinates

        # Attempt Graphviz layout
        G = pgv.AGraph(directed=True, strict=False, rankdir='TB', ratio='auto', nodesep='0.75', ranksep='1.2 equally') # Added more layout hints

        for state_data in fsm_data.get('states', []):
            name = state_data.get('name')
            if name: 
                # Use fixedsize=false and estimate width/height if needed, or let Graphviz decide
                # For better control, one might calculate label width.
                label_for_gv = name # Could add actions here for better width estimation by Graphviz
        ...
        for trans_data in fsm_data.get('transitions', []):
            ...
            if source and target and G.has_node(source) and G.has_node(target):
                ...
                # Note: Graphviz doesn't directly support complex labels with conditions/actions easily on edges like our custom painter.
                # We'll keep it simple for GV, the custom painter handles the full label.
        ...
        graphviz_positions = {}
        try:
            # Ensure dot is in PATH or pygraphviz is configured.
            G.layout(prog="dot") # Use 'dot' for hierarchical layout
            logger.debug("AI: Graphviz layout ('dot') successful.")

            raw_gv_positions = []
            # Graphviz 'pos' attribute is typically in points (72 points per inch)
            # Y usually increases upwards in Graphviz, downwards in Qt.
            for node in G.nodes():
                try:
            ...
            if raw_gv_positions:
                ...
                max_y_gv = max(p['y'] for p in raw_gv_positions) # Graphviz Y increases upwards

                for p_gv in raw_gv_positions:
                    # Convert Graphviz points to Qt scene coordinates
                    qt_x = (p_gv['x'] - min_x_gv) * GV_SCALE + layout_start_x
                    qt_y = (max_y_gv - p_gv['y']) * GV_SCALE + layout_start_y # Flip Y for Qt
            ...
            self._append_to_ai_chat_display("System", f"Warning: AI FSM layout using Graphviz failed ({error_msg[:60]}...). Using basic grid layout.")
            graphviz_positions = {} # Ensure fallback

        # Create GraphicsStateItems
        for i, state_data in enumerate(fsm_data.get('states', [])):
            ...
            item_w = state_data.get('width', default_item_width) # Use default if not specified by AI
            ...
            if pos:
                ...
                pos_x = layout_start_x + (i % items_per_row) * (item_w + padding_x) # Use item_w for grid
            ...
            try:
                state_item = GraphicsStateItem(
                    ...
                    color=state_data.get('properties', {}).get('color', state_data.get('color', COLOR_ITEM_STATE_DEFAULT_BG)), # Check top-level color too
                    entry_action=state_data.get('entry_action', ""),
                    during_action=state_data.get('during_action', ""),
                    exit_action=state_data.get('exit_action', ""),
                    description=state_data.get('description', fsm_data.get('description', "") if i==0 else ""), # Add FSM desc to first state if available
        ...
        # Create GraphicsTransitionItems
        for trans_data in fsm_data.get('transitions', []):
        ...
        # Create GraphicsCommentItems
        # Determine a suitable Y position for comments below the states
        ...
        for i, comment_data in enumerate(fsm_data.get('comments', [])):
            ...
            # Use explicit x,y if provided by AI, otherwise fall back to grid-like placement
            comment_x = comment_data.get('x', comment_start_x_fallback + i * (150 + 20)) # 150 is default comment width
        ...
        # Add items to scene via Undo Commands
        if items_to_add_for_undo_command:
            for item in items_to_add_for_undo_command:
                ...
                elif hasattr(item, 'toPlainText') and item.toPlainText(): cmd_text += f": {item.toPlainText()[:20]}..." # For comments
                
                add_cmd = AddItemCommand(self.scene, item, cmd_text)
                self.undo_stack.push(add_cmd) # This calls redo() which adds to scene

            logger.info("AI: Added %d items to diagram via undo commands.", len(items_to_add_for_undo_command))
            QTimer.singleShot(100, self._fit_view_to_new_ai_items) # Adjust view after items are added
        else:
        ...
        elif not clear_current_diagram and not items_to_add_for_undo_command: # End macro even if empty to balance beginMacro
             if self.undo_stack.count() > 0 and self.undo_stack.command(self.undo_stack.count() -1).childCount() == 0:
                 # If the macro is empty, it might be good to pop it, but QUndoStack doesn't have a direct pop.
                 # An empty macro is usually harmless.
                 pass
             self.undo_stack.endMacro()


        # If Python simulation was active, reinitialize it with the new/modified diagram
        if self.py_sim_active and items_to_add_for_undo_command:
            ...
            current_diagram_data = self.scene.get_diagram_data() # Get fresh data
            try:
    ...
    def _fit_view_to_new_ai_items(self): # Unchanged from previous correct version
    ...
    def _append_to_ai_chat_display(self, sender: str, message: str): # Unchanged
        ...
        elif sender == "System Error" or sender == "System": sender_color_hex_str = "#D32F2F" # Red for system errors

        # Basic Markdown-like to HTML conversion
        escaped_message = html.escape(message)
        # Bold: **text**
        escaped_message = re.sub(r'\*\*(.*?)\*\*', r'<b>\1</b>', escaped_message)
        # Italics: *text* (ensure not part of bold)
    ...
    def on_openai_settings(self): # Unchanged
        ...
        if ok:
            ...
            if new_key:
                ...
                # self._append_to_ai_chat_display("System", "API Key set. AI Assistant ready.")
            else:
                logger.info("AI: OpenAI API Key cleared.")
                # self._append_to_ai_chat_display("System", "API Key cleared. AI Assistant inactive.")
    ...
    # --- Properties Dock Methods --- (Unchanged from previous correct version)
    ...
    # --- Window and File Management --- (Unchanged from previous correct version)
    ...
    def _update_matlab_status_display(self, connected, message):
        ...
        if "Initializing" not in message: # Avoid logging initial default message
    ...
    def set_ui_enabled_for_matlab_op(self, enabled: bool):
        ...
        self._update_py_simulation_actions_enabled_state() # Re-evaluate sim actions based on new state
    ...
    def on_new_file(self, silent=False):
        ...
        self.on_stop_py_simulation(silent=True) # Ensure sim is stopped
        self.scene.clear()
        self.scene.setSceneRect(0,0,6000,4500) # Reset scene rect
        self.current_file_path = None
        self.last_generated_model_path = None
        self.undo_stack.clear()
        self.scene.set_dirty(False) # Explicitly set dirty to false for a new file
        self.setWindowModified(False) # Update window modified status accordingly
        ...
        self.view.centerOn(self.scene.sceneRect().center()) # Center view
    ...
    def on_open_file(self):
        ...
        if file_path:
            ...
            if self._load_from_path(file_path):
                ...
                self.last_generated_model_path = None # Reset for new file context
                self.undo_stack.clear()
    ...
    def _load_from_path(self, file_path):
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            # Basic validation of BSM file structure
            if not isinstance(data, dict) or ('states' not in data or 'transitions' not in data):
                ...
                return False
            self.scene.load_diagram_data(data) # Scene handles detailed loading
            return True
    ...
    def on_save_file_as(self) -> bool:
        ...
        if file_path:
            ...
            if self._save_to_path(file_path):
                ...
                # _save_to_path already handles dirty state and window title
                return True
        return False

    def _save_to_path(self, file_path) -> bool:
        if self.py_sim_active: # Prevent saving during active simulation
            QMessageBox.warning(self, "Simulation Active", "Please stop the Python FSM simulation before saving.")
            return False

        save_file = QSaveFile(file_path) # Use QSaveFile for safer saving
        ...
        try:
            ...
            bytes_written = save_file.write(json_data.encode('utf-8')) # Encode to UTF-8
            ...
            if not save_file.commit(): # Finalize writing (rename temp to actual)
                error_str = save_file.errorString()
            ...
            self.scene.set_dirty(False)
            self.setWindowModified(False)
            self._update_window_title() # Update title to remove [*]
            return True
        except Exception as e: # Catch other errors like JSON encoding issues
    ...
    def _get_bundled_file_path(self, filename: str) -> str | None: # Unchanged
        try:
            ...
            # Check common locations relative to base_path
            possible_paths = [
                os.path.join(base_path, filename),                     # Direct
                os.path.join(base_path, 'docs', filename),             # In 'docs'
                os.path.join(base_path, 'resources', filename),        # In 'resources'
                os.path.join(base_path, 'examples', filename)          # In 'examples'
            ]
            for path_to_check in possible_paths:
                if os.path.exists(path_to_check):
                    return path_to_check
            
            # Fallback: if not found in subdirs, check directly in base_path again (already done, but for safety)
    ...
    def _open_example_file(self, filename: str): # Unchanged
        ...
        if example_file_path and os.path.exists(example_file_path):
            ...
            if self._load_from_path(example_file_path):
                self.current_file_path = example_file_path # Set path as if it were opened normally
                self.last_generated_model_path = None
                self.undo_stack.clear()
                self.scene.set_dirty(False)
                self.setWindowModified(False)
    ...
    def on_show_quick_start(self): # Unchanged
        guide_filename = "QUICK_START.html" # Assuming it's HTML
    ...
    def closeEvent(self, event: QCloseEvent): # Unchanged
        ...
        if self.ai_chatbot_manager:
            self.ai_chatbot_manager.stop_chatbot() # Clean up AI worker thread
        ...
        if self._prompt_save_if_dirty():
            if self.matlab_connection and hasattr(self.matlab_connection, '_active_threads') and self.matlab_connection._active_threads:
                ...
                # Note: We don't explicitly wait for MATLAB threads here as it could hang the UI.
                # They are QThreads and should terminate if their work finishes or on app exit.
            event.accept()
        else:
            event.ignore()
    ...
    def _run_internet_check_job(self): # Unchanged
        ...
        try:
            # Create a socket connection to test internet
        ...
        except OSError as e: # Other network errors
            status_message_detail = "Disconnected (Net Error)"

        if current_status != self._internet_connected or self._internet_connected is None: # Update only on change or first run
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str): # Unchanged
        ...
        if hasattr(self, 'internet_status_label'):
            ...
            try: # Attempt to get FQDN for tooltip for more info
            ...
            style_sheet = f"font-weight: normal; padding: 0px 5px; color: {COLOR_PY_SIM_STATE_ACTIVE if is_connected else '#D32F2F'};" # Green if connected, Red if not
    ...
    def _update_py_sim_status_display(self): # Updated for hierarchical state display
        if hasattr(self, 'py_sim_status_label'):
            if self.py_sim_active and self.py_fsm_engine:
                state_name_display = self.py_fsm_engine.get_current_state_name() # This now returns hierarchical name
    ...
    def set_ui_enabled_for_py_sim(self, is_sim_running: bool): # Unchanged
        ...
        # Force select mode if sim starts, restore previous mode if applicable (or default to select)
        if is_sim_running and self.scene.current_mode != "select":
            self.scene.set_mode("select") # Force select mode during sim
        elif not is_sim_running and self.scene.current_mode != "select":
            pass # Keep current mode if sim stops and it wasn't select

        # Enable/disable item movability based on editability
        for item in self.scene.items():
            if isinstance(item, (GraphicsStateItem, GraphicsCommentItem)): # Only these are normally movable
                item.setFlag(QGraphicsItem.ItemIsMovable, is_editable and self.scene.current_mode == "select")

        # Enable/disable editing actions
        actions_to_toggle = [
        ...
        if hasattr(self, 'properties_edit_button'): # Properties editing only if not simulating
        ...
        self._update_py_simulation_actions_enabled_state() # Update sim control buttons
    ...
    def _highlight_sim_active_state(self, state_name_to_highlight: str | None):
        ...
        if state_name_to_highlight and self.py_fsm_engine:
            # The state_name_to_highlight can be hierarchical, e.g., "Super (Sub)"
            # We need to find the top-level state item on the current diagram.
            # The FSMSimulator's current_state.id will give the top-level active state.
            ...
            if top_level_active_state_id:
                for item in self.scene.items():
                    if isinstance(item, GraphicsStateItem) and item.text_label == top_level_active_state_id:
                        ...
                        if self.view and not self.view.ensureVisible(item, 50, 50): # Try to ensure visible
                            self.view.centerOn(item)
                        break
        self.scene.update() # Ensure visual update

    def _highlight_sim_taken_transition(self, transition_label_or_id: str | None): # Unchanged
        # This method is placeholder; actual transition highlighting is complex
        # as it requires identifying the specific GraphicsTransitionItem instance.
        # For now, it just clears any previous transition highlight.
        if self._py_sim_currently_highlighted_transition:
            if hasattr(self._py_sim_currently_highlighted_transition, 'set_py_sim_active_style'): # Assuming a similar method exists or could be added
    ...
    def _update_py_simulation_dock_ui(self):
        ...
        # Get events from the active sub-simulator if one is active
        ...
        # Also get events from the main FSM's current state (which might be a superstate)
        ...
        elif not possible_events: # If list became empty, default to "None"
             self.py_sim_event_combo.setCurrentIndex(0)


    def _append_to_py_simulation_log(self, log_entries: list[str]): # Unchanged
        for entry in log_entries:
            cleaned_entry = html.escape(entry) # Basic HTML escape
            # Apply some color coding based on keywords
            if "[Condition]" in entry or "[Eval Error]" in entry or "ERROR" in entry.upper() or "SecurityError" in entry:
                ...
                cleaned_entry = f"<span style='color:orange; font-weight:bold;'>{cleaned_entry}</span>" # Orange for warnings/blocks
                ...
                cleaned_entry = f"<span style='color:{COLOR_ACCENT_PRIMARY}; font-weight:bold;'>{cleaned_entry}</span>" # Blue for key transitions
            elif "No eligible transition" in entry or "event is not allowed" in entry:
                cleaned_entry = f"<span style='color:{COLOR_TEXT_SECONDARY};'>{cleaned_entry}</span>" # Grey for non-critical info
        ...
        # Log important entries to main application log as well
        if log_entries:
            last_log_short = log_entries[-1].split('\n')[0][:100] # Log only first line, truncated
    ...
    def on_start_py_simulation(self): # Unchanged
        ...
        if not diagram_data.get('states'):
            ...
            return
        try:
            # Pass halt_on_action_error=True (or make it configurable)
            ...
            self.py_sim_action_log_output.clear() # Clear log on new start
            self.py_sim_action_log_output.setHtml("<i>Simulation log will appear here...</i>")
            ...
            self._update_py_simulation_dock_ui()
        except FSMError as e: # Catch specific FSMError from simulator
            ...
            self.set_ui_enabled_for_py_sim(False) # Ensure UI is re-enabled
    ...
    def on_stop_py_simulation(self, silent=False): # Unchanged
        ...
        self._highlight_sim_active_state(None) # Unhighlight
        self._highlight_sim_taken_transition(None)

        self.py_fsm_engine = None
        self.set_ui_enabled_for_py_sim(False) # Re-enable UI editing
        self._update_py_simulation_dock_ui() # Update dock to reflect stopped state
    ...
    def on_step_py_simulation(self): # Unchanged
        ...
        try:
            ...
            self._update_py_simulation_dock_ui()
            self._highlight_sim_taken_transition(None) # Clear transition highlight after step
    ...
    def on_trigger_py_event(self): # Unchanged
        ...
        if not event_to_trigger: # If neither is selected/entered, treat as internal step
    ...
    def log_message(self, level_str: str, message: str):
        """
        Logs a message using the application's logging system.
        This method can be called from other parts of the application (like DiagramScene)
        to ensure messages are consistently logged.
        """
        level = getattr(logging, level_str.upper(), logging.INFO) # Default to INFO if level_str is invalid
...
if __name__ == '__main__':
    ...
    # Ensure dependencies directory for QSS icons exists (if needed by stylesheet)
    ...
    if not os.path.exists(dependencies_dir):
        try:
            ...
            # This is not critical if icons are bundled or not used by QSS
