...
class MainWindow(QMainWindow):
    ...
    def _create_status_bar(self):
        ...
        self.internet_status_label = QLabel("Internet: Init...") # Added
        self.internet_status_label.setToolTip("Internet connectivity status. Checks every 15s.") # Added
        self.status_bar.addPermanentWidget(self.internet_status_label) # Added
    ...
    def log_message(self, message: str, is_py_sim_log_source=False):
        # Filter out internet status specific logging from updating the main status_label
        if is_py_sim_log_source or "Internet Status Update:" in message or "Internet check" in message :
            if "Internet Status Update:" in message or "Internet check" in message: # Log to console/text log but not main UI elements that are handled specifically
                print(f"Log (silent for status bar): {message}") # Or just pass
            # else: Python Sim log, handled below
        else: # Regular messages update main status bar label
            timestamp = QTime.currentTime().toString('hh:mm:ss.zzz')
            self.log_output.append(f"<span style='color:{COLOR_TEXT_SECONDARY.name()};'>[{timestamp}]</span> {html.escape(message)}")
            self.log_output.verticalScrollBar().setValue(self.log_output.verticalScrollBar().maximum())
            if "worker:" not in message: # Don't show worker thread messages in main status bar
                self.status_label.setText(message.split('\n')[0][:120])
        
        # Special handling for Python Simulation action log which is separate from general app log
        if is_py_sim_log_source and hasattr(self, 'py_sim_action_log_output') and not ("Internet Status Update:" in message or "Internet check" in message) :
             # This logic is better handled by _append_to_py_simulation_log now
            pass
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self._prompt_save_if_dirty():
            ...
            event.ignore()
            if timer_was_active: # If close was cancelled, and timer was running, restart it
    ...
    def _init_internet_status_check(self):
        ...
        self._run_internet_check_job()         # Initial check immediately

    def _run_internet_check_job(self):
        """
        Performs the internet connectivity check.
        This is called by a QTimer and will block briefly (timeout is 1.5s).
        """
        ...
        try:
            # create_connection handles both resolving (if hostname) and connecting.
            # Using an IP address is generally faster as it skips DNS resolution.
            s = socket.create_connection((host_to_check, port_to_check), timeout=connection_timeout)
            s.close() # Ensure the socket is closed after successful connection
            current_status = True
            status_message = "Connected"
        except socket.timeout:
            current_status = False
            status_message = "Disconnected (Timeout)"
        except socket.gaierror as e: # Address-related error (e.g. DNS failure if hostname used)
            current_status = False
            status_message = "Disconnected (DNS/Net Issue)"
            # Log specific error for diagnostics, but not to main status bar
            self.log_message(f"Internet check gaierror: {e}", is_py_sim_log_source=True) 
        except OSError as e: # Other network errors (e.g., "Network is unreachable")
            current_status = False
            status_message = "Disconnected (Net Error)"
             # Log specific error for diagnostics
            self.log_message(f"Internet check OSError: {e.strerror} (errno {e.errno})", is_py_sim_log_source=True)

        # Update UI only if status changed or it's the first check (_internet_connected is None)
        if current_status != self._internet_connected or self._internet_connected is None:
            self._internet_connected = current_status
            self._update_internet_status_display(current_status, status_message)
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        ...
        self.internet_status_label.setToolTip(f"{full_status_text} (Checks {socket.getfqdn('8.8.8.8')}:{53})") # Shows which host is checked

        style_sheet = "font-weight: normal; padding: 0px 5px;"
        if is_connected:
            style_sheet += f"color: {COLOR_PY_SIM_STATE_ACTIVE.name()};" # Greenish color for connected
        else:
            style_sheet += "color: #D32F2F;" # Reddish color for disconnected
        self.internet_status_label.setStyleSheet(style_sheet)
        
        # Log this status change silently (not to main status bar, only to console/log dock)
        self.log_message(f"Internet Status Update: {message_detail}", is_py_sim_log_source=True)
    # --- End Internet Connectivity Check ---
    ...
    def _append_to_py_simulation_log(self, log_entries: list[str]):
        for entry in log_entries:
            ...
            if "[Condition]" in entry or "[Eval Error]" in entry or "ERROR" in entry.upper():
                cleaned_entry = f"<span style='color:{COLOR_ACCENT_SECONDARY.name()};'>{cleaned_entry}</span>" # Use .name() for QColor
                ...
                cleaned_entry = f"<span style='color:{COLOR_TEXT_SECONDARY.name()};'>{cleaned_entry}</span>"
