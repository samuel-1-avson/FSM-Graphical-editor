from PyQt5.QtCore import QObject, pyqtSignal, QThread, QTime, QTimer, Qt, QMetaObject
import openai
import json
...
class ChatbotWorker(QObject):
    ...
    def __init__(self, api_key, model_name="gpt-4-turbo", parent=None):
    ...
    def process_message(self, user_message: str):
        ...
        is_generation_request = "generate fsm" in user_message.lower() or \
                         ...
                         "design a state machine" in user_message.lower() or \
                         "design it" in user_message.lower() or \
    ...
    # You might even slightly alter the user message for the AI to reinforce JSON-only
    # effective_user_message += "\n\nImportant: Provide your response as a single, complete JSON object only, with no other surrounding text or explanation."
    # However, the system prompt and response_format should mostly handle this.
            pass # System prompt already handles this for JSON requests

# Then use effective_user_message when building messages_for_api
        messages_for_api.append({"role": "user", "content": effective_user_message})
        
                         
                         
        keywords_for_generation = [
            "generate fsm", "create fsm",
            "generate state machine", "create state machine", "design state machine",
            "model fsm", "model state machine",
            "draw fsm", "draw state machine", # If you want these to trigger JSON
            "design it", # Keep this if it works, but it's broad
            "/generate_fsm"
        ]
        ...
        if self.current_diagram_context_json_str:
            try:
                ...
                if "error" not in diagram: # Avoid processing if context was an error placeholder
        ...
        history_context_limit = -6 # last 3 turns
        ...
        try:
            ...
            # Update history before emitting
            self.conversation_history.append({"role": "user", "content": user_message}) # Add user message that led to this
            ...
            self.errorOccurred.emit(f"API Connection Error: {str(e)[:200]}") # Keep error messages concise
