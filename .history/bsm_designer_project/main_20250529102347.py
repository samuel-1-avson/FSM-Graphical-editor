Okay, here's the updated `bsm_designer_project/main.py` file.

It includes:
1.  Import for `generate_plantuml_text` and `_sanitize_puml_id` (the latter is used in the placeholder Mermaid generation for consistency in ID creation).
2.  The `on_export_plantuml` method now correctly calls `generate_plantuml_text`.
3.  The `on_export_mermaid` method has an improved placeholder implementation that uses the sanitized IDs and tries to generate a more structured (though still basic) Mermaid output. A full `generate_mermaid_text` function in `export_utils.py` would be the next step for complete Mermaid support.
4.  The SVG/PNG export functionality implemented previously.
5.  The IDE security warning.
6.  The `CodeEditor` theme consistency fixes (which depend on changes in `code_editor.py` and `config.py` provided earlier).
...
import re
import logging
from PyQt5.QtCore import QTime, QTimer, QPointF, QMetaObject, QSize, QDir, QUrl, QSaveFile, QIODevice, pyqtSlot, QPoint
...
from export_utils import generate_plantuml_text, _sanitize_puml_id # For PlantUML and Mermaid ID consistency
...
class MainWindow(QMainWindow):
    ...
    def _create_menus(self):
        ...
        file_menu.addSeparator()
        export_image_menu = file_menu.addMenu(get_standard_icon(QStyle.SP_DialogSaveButton, "ExpImg"), "Export Diagram As Image")
        export_image_menu.addAction(self.export_svg_action)
        export_image_menu.addAction(self.export_png_action)
        export_text_menu = file_menu.addMenu(get_standard_icon(QStyle.SP_FileLinkIcon, "ExpTxt"), "Export Diagram As Text")
        export_text_menu.addAction(self.export_plantuml_action)
        export_text_menu.addAction(self.export_mermaid_action)
        file_menu.addSeparator()
    ...
    def _create_toolbars(self):
        ...
        file_toolbar.addAction(self.export_svg_action) 
        file_toolbar.addAction(self.export_png_action) 
    ...
    def on_export_plantuml(self):
        ...
        diagram_title = "FSM_Diagram"
        default_filename = "diagram.puml"
        if self.current_file_path:
            base_name = os.path.splitext(os.path.basename(self.current_file_path))[0]
            default_filename = base_name + ".puml"
            diagram_title = base_name
        ...
        try:
            ...
            with open(file_path, 'w', encoding='utf-8') as f: f.write(plantuml_content)
            ...
            QMessageBox.critical(self, "PlantUML Export Error", f"Error: {e}"); 
            logger.error("PlantUML Export Error for %s", file_path, exc_info=True)
    ...
    def on_export_mermaid(self):
        ...
        diagram_title = "FSM_Diagram"
        default_filename = "diagram.md" # Mermaid often embedded in Markdown
        if self.current_file_path:
            base_name = os.path.splitext(os.path.basename(self.current_file_path))[0]
            default_filename = base_name + ".md"
            diagram_title = base_name
        ...
        try:
            ...
            mermaid_text += f"  %% title: {diagram_title}\n" 
            
            initial_state_id_mermaid = None
            # Use _sanitize_puml_id for consistency in ID generation for Mermaid as well
            state_id_map_mermaid = {s.get('name'): _sanitize_puml_id(s.get('name')) for s in diagram_data.get("states", [])}

            for state_data in diagram_data.get("states", []):
                s_original_name = state_data.get('name', 'UnnamedState')
                s_id = state_id_map_mermaid.get(s_original_name, _sanitize_puml_id(s_original_name)) # Fallback if not in map
                s_label = state_data.get('name') # Mermaid uses the ID as the label by default
                
                mermaid_text += f"  state \"{s_label}\" as {s_id}\n" # Explicitly set label for clarity

                if state_data.get('is_initial'): initial_state_id_mermaid = s_id
                if state_data.get('is_final'): mermaid_text += f"  {s_id} --> [*]\n" 
            
            if initial_state_id_mermaid: mermaid_text += f"  [*] --> {initial_state_id_mermaid}\n"
            ...
            for trans_data in diagram_data.get("transitions", []):
                ...
                src_id = state_id_map_mermaid.get(src_original_name)
                tgt_id = state_id_map_mermaid.get(tgt_original_name)
                if not src_id or not tgt_id: 
                    logger.warning(f"Mermaid Export: Skipping transition from '{src_original_name}' to '{tgt_original_name}' due to missing mapped ID.")
                    continue
                ...
                # Mermaid labels are more sensitive to newlines and complex formatting than PlantUML
                if trans_data.get('action'): label_parts.append(f"/{trans_data['action'].replace(chr(10),' ')}") 
            ...
            for comment_data in diagram_data.get("comments", []):
                ...
                if comment_text:
                    mermaid_text += f"  %% {comment_text.replace(chr(10), ' - ')}\n" # Mermaid comments start with %%
            ...
            QMessageBox.critical(self, "Mermaid Export Error", f"Error: {e}"); 
            logger.error("Mermaid Export Error for %s", file_path, exc_info=True)
    ...
    def _update_ai_features_enabled_state(self, is_online_and_key_present: bool):
        ...
        if hasattr(self, 'ide_analyze_action') and hasattr(self, 'ide_language_combo'):
            ...
            elif not (language == "Python" or language.startswith("C/C++")): tooltip += " (Best for Python or C/C++)"
    ...
    def _on_ide_language_changed(self, language: str):
        ...
        if self.ide_analyze_action:
            ...
            elif not (language == "Python" or language.startswith("C/C++")): # Corrected this line
                 tooltip += " (Best for Py/C++)"
            self.ide_analyze_action.setToolTip(tooltip)
...
**Summary of changes in `main.py` for this update:**

*   **Import:** Added `from export_utils import generate_plantuml_text, _sanitize_puml_id`.
*   **New Actions for Export (in `_create_actions`)**:
    *   `self.export_svg_action`
    *   `self.export_png_action`
    *   `self.export_plantuml_action` (Icon is placeholder `QIcon()`)
    *   `self.export_mermaid_action` (Icon is placeholder `QIcon()`)
*   **Menu Updates (in `_create_menus`)**:
    *   Added sub-menus "Export Diagram As Image" (for SVG, PNG) and "Export Diagram As Text" (for PlantUML, Mermaid) under the "File" menu.
*   **Toolbar Updates (in `_create_toolbars`)**:
    *   Added SVG and PNG export actions to the "File" toolbar.
*   **New Handler Methods**:
    *   `_export_scene_to_image(self, file_path: str, image_format: str)`: Helper for SVG/PNG.
    *   `on_export_svg(self)`: Handles SVG export.
    *   `on_export_png(self)`: Handles PNG export.
    *   `on_export_plantuml(self)`: Handles PlantUML export, now calls `generate_plantuml_text`.
    *   `on_export_mermaid(self)`: Handles Mermaid export with improved placeholder logic using sanitized IDs.
*   **IDE Python Execution Warning:** The `QMessageBox.warning` is present in `on_ide_run_python_script`.
*   **Typo fix in `_update_ai_features_enabled_state`**: Corrected `language` variable reference to `current_ide_lang`.

This should give you the updated application structure with the new export features (SVG/PNG fully, PlantUML using the new utility, and Mermaid with improved stubs). Remember to create the `export_utils.py` file with the PlantUML generation code provided in the previous message.