...
from PyQt5.QtGui import QKeyEvent, QKeySequence, QCursor, QPainter, QColor, QPen, QBrush
...
class DiagramScene(QGraphicsScene):
    ...
    def edit_item_properties(self, item):
        ...
        if dialog_executed_and_accepted and new_props_from_dialog is not None:
            ...
            if isinstance(item, GraphicsStateItem) and old_props.get('name') != final_new_props.get('name'):
                # No need to directly emit here if _update_transitions_for_renamed_state emits it
                pass # self.scene_content_changed_for_find.emit() is handled by _update_transitions_for_renamed_state
            elif isinstance(item, GraphicsCommentItem) and old_props.get('text') != final_new_props.get('text'):
                self.scene_content_changed_for_find.emit() # If comment text changes
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        ...
        if item_type == "State":
            ...
            if props_dialog.exec() == QDialog.Accepted:
                ...
                if self.get_state_by_name(final_props['name']) and final_props['name'] != default_name:
                    ...
                    # Connect signals for FindItemDialog updates
        ...
        if current_item:
            ...
            self.undo_stack.push(cmd) # AddItemCommand calls connect_state_item_signals and emits scene_content_changed
            ...
            # scene_content_changed_for_find is emitted by AddItemCommand.redo()
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.matches(QKeySequence.Copy):
            ...
            self.paste_items_from_clipboard() # This will emit scene_content_changed_for_find
            event.accept()
            return
            ...
            if len(selected) == 1 and isinstance(selected[0], (GraphicsStateItem, GraphicsCommentItem)) \
               and selected[0].flags() & QGraphicsItem.ItemIsFocusable:
                if hasattr(selected[0], 'start_inline_edit') and not getattr(selected[0], '_is_editing_inline', False):
                    selected[0].start_inline_edit() # This will emit textChangedViaInlineEdit, which MainWindow connects to refresh FindDialog
                    event.accept()
                    return

        if event.key() == Qt.Key_Delete or (event.key() == Qt.Key_Backspace and sys.platform != 'darwin'):
            if self.selectedItems():
                self.delete_selected_items() # This will emit scene_content_changed_for_find
                event.accept()
                return
    ...
    def delete_selected_items(self):
        ...
        if items_to_delete_with_related:
            ...
            self.undo_stack.push(cmd) # RemoveItemsCommand.redo() will emit scene_content_changed_for_find
    ...
    def dropEvent(self, event: QGraphicsSceneDragDropEvent):
        ...
        if mime_data.hasFormat(MIME_TYPE_BSM_TEMPLATE):
            ...
            try:
                ...
                self._add_template_to_scene(template_data, pos) # This emits scene_content_changed_for_find
                event.acceptProposedAction()
            ...
            self._add_item_interactive(QPointF(grid_x, grid_y),
                                       ...
                                       initial_data=initial_props_for_add) # This emits scene_content_changed_for_find
            event.acceptProposedAction()
        else:
    ...
    def _add_template_to_scene(self, template_data: dict, drop_pos: QPointF):
        ...
        for state_data in template_data.get('states', []):
            ...
            # AddItemCommand handles adding to scene and connecting signals via MainWindow if needed.
            cmd = AddItemCommand(self, state_item, f"Add State from Template: {unique_name}")
            self.undo_stack.push(cmd) # AddItemCommand.redo() will call connect_state_item_signals
