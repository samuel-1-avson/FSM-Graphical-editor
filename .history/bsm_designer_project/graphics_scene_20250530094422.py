...
import math # For math operations in ZoomableView
...
from PyQt5.QtGui import (
    ...
    QWheelEvent, QBrush # Added QBrush
)
from PyQt5.QtCore import Qt, QRectF, QPointF, QLineF, pyqtSignal, QPoint, QMarginsF
...
class DiagramScene(QGraphicsScene):
    ...
    def _clear_dynamic_guidelines(self):
        if self._horizontal_snap_lines or self._vertical_snap_lines:
            ...
            self.update() # Request full scene update to clear old lines
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            if self.current_mode == "state":
                ...
                # If clicking on an unselected item, clear selection and select only that one
                # QGraphicsScene handles this if RubberBandDrag is not active for this click
                if top_item_at_pos and not top_item_at_pos.isSelected() and not (event.modifiers() & Qt.ControlModifier):
                    self.clearSelection()
                    top_item_at_pos.setSelected(True)
                    selected_items_list = [top_item_at_pos] # Update list for position storing
        ...
        if self.current_mode == "select" and event.buttons() & Qt.LeftButton and self._mouse_press_items_positions:
            if self.snap_to_objects_enabled and self._show_dynamic_snap_guidelines:
                ...
                if dragged_items:
                    ...
                    # Calculate where the item *would* be if moved by standard QGraphicsScene logic
                    # This requires knowing the delta from the item's original press position,
                    # not just event.lastScenePos() which can be jittery or affected by snapping.
                    original_item_press_pos = self._mouse_press_items_positions.get(primary_dragged_item, primary_dragged_item.scenePos())
            ...
            if self._mouse_press_items_positions:
                ...
                for item, old_pos in self._mouse_press_items_positions.items():
                    ...
                    if self.snap_to_grid_enabled:
                        snapped_x = round(final_snapped_pos.x() / self.grid_size) * self.grid_size
                        snapped_y = round(final_snapped_pos.y() / self.grid_size) * self.grid_size
                        final_snapped_pos = QPointF(snapped_x, snapped_y)
                    
                    if (final_snapped_pos - old_pos).manhattanLength() > 0.1 or \
                    ...
                    elif (current_item_pos_after_drag - old_pos).manhattanLength() > 0.1: 
    ...
    def edit_item_properties(self, item):
        from dialogs import StatePropertiesDialog, TransitionPropertiesDialog, CommentPropertiesDialog # Local import
    ...
    def drawForeground(self, painter: QPainter, rect: QRectF): # Changed from drawBackground
        super().drawForeground(painter, rect) # Call super's drawForeground
