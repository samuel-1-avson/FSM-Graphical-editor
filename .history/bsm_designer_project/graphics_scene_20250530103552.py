...
from PyQt5.QtWidgets import (
    ...
    QGraphicsSceneDragDropEvent, QKeyEvent, QApplication # Added QApplication here
)
from PyQt5.QtGui import (
    QPainter, QColor, QPen, QCursor, QMouseEvent, 
    QWheelEvent, QBrush 
)
from PyQt5.QtCore import Qt, QRectF, QPointF, QLineF, pyqtSignal, QPoint, QMarginsF, QEvent
...
class DiagramScene(QGraphicsScene):
    ...
    def _update_transitions_for_renamed_state(self, old_name:str, new_name:str):
        ...
        for item in self.items():
            if isinstance(item, GraphicsTransitionItem):
                needs_update = False
                if item.start_item and item.start_item.text_label == new_name: 
                    pass 
    ...
    def keyPressEvent(self, event: QKeyEvent): # Added type hint for event
        ...
        if event.key() == Qt.Key_Delete or (event.key() == Qt.Key_Backspace and sys.platform != 'darwin'): 
            ...
            if active_editor_item:
                ...
                if editor_widget:
                    # Simulate Escape on editor: this should trigger its keyPressEvent, then focusOut, then _finish_inline_edit
                    # This assumes editor's keyPressEvent sets an abort flag and clears focus.
                    esc_event = QKeyEvent(QEvent.KeyPress, Qt.Key_Escape, Qt.NoModifier)
                    QApplication.postEvent(editor_widget, esc_event) # Use postEvent for safety
                self.set_mode("select") 
                event.accept()
                return 
...
class ZoomableView(QGraphicsView):
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_Space and not self._is_panning_with_space and not event.isAutoRepeat():
            ...
            super().keyPressEvent(event) # Pass to scene first
