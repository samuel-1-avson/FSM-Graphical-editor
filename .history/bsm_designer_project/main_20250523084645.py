...
class MainWindow(QMainWindow):
    ...
    def _on_send_ai_chat_message(self):
        ...
        # Add user message to AI's conversation history (though manager now handles history internally mostly)
        # self.ai_chatbot_manager.conversation_history.append({"role": "user", "content": user_message}) # Can be removed if manager does it

        self.ai_chat_input.clear()
        
        # Update UI to indicate "thinking" - AI manager will emit "Sending..." then worker will do its thing.
        self._update_ai_chat_status("Status: Preparing to send...") # Or directly to "Thinking..."
        # Disable inputs here. They will be re-enabled when AI response/error occurs.
        ...
        self.ai_chatbot_manager.send_message(user_message) # This now handles context internally

    def _handle_ai_response(self, ai_message_text: str): # SLOT
        current_time = QTime.currentTime().toString('hh:mm:ss.zzz')
        print(f"--- MW_HANDLE_RESP ({current_time}): CALLED with (first 30): '{ai_message_text[:30].replace('\n',' ')}...' ---")
        
        final_status_message = "Status: Ready." # Default status after successful response
        message_processed_as_fsm = False # For Phase 1, this will likely remain False unless AI specifically formats.
       
        try:
            potential_json = ai_message_text.strip()
            if potential_json.startswith("{") and potential_json.endswith("}"):
                fsm_data = json.loads(potential_json) # Needs 'import json' at top of main.py
        ...
        # This _update_ai_chat_status now handles enabling/disabling UI elements.
