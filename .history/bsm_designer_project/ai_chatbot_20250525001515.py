...
class ChatbotWorker(QObject):
    ...
    @pyqtSlot(object) # Changed to object to allow None more gracefully
    def set_current_diagram_context(self, diagram_context_obj: object): 
        if diagram_context_obj is None:
            logger.debug(f"WORKER: Setting diagram context to None (received None object).")
            self.current_diagram_context_json_str = None
        elif isinstance(diagram_context_obj, str):
            # If it's an empty string, also treat as None for consistency
            if not diagram_context_obj:
                 logger.debug(f"WORKER: Setting diagram context to None (received empty string object).")
                 self.current_diagram_context_json_str = None
            else:
                logger.debug(f"WORKER: Setting diagram context (string). Length: {len(diagram_context_obj)}")
                self.current_diagram_context_json_str = diagram_context_obj
        else:
            logger.warning(f"WORKER: set_current_diagram_context received unexpected type: {type(diagram_context_obj)}. Setting context to None.")
            self.current_diagram_context_json_str = None
...
class AIChatbotManager(QObject):
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        diagram_json_str: str | None = None # Explicitly type hint
        ...
        if self.chatbot_worker: 
            # For the slot expecting 'object', we can now pass None directly or the string.
            QMetaObject.invokeMethod(self.chatbot_worker, "set_current_diagram_context", Qt.QueuedConnection, 
                                     diagram_json_str) # Pass str or None
