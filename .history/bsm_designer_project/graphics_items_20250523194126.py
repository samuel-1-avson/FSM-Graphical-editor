...
class BaseGraphicsItem: # Does not inherit QGraphicsItem directly
    """Base class providing common functionality for all graphics items via mixin."""
    
    def __init__(self):
        # These methods are called on 'self', which will be an instance of a QGraphicsItem derivative
        # Ensure derived classes call QGraphicsItem.__init__ before BaseGraphicsItem.__init__
        # or that BaseGraphicsItem.__init__ is called after self is fully a QGraphicsItem.
        if isinstance(self, QGraphicsItem):
            self._setup_common_flags()
            self._setup_shadow_effect()
        else:
            # This case should ideally not happen if used as a mixin correctly.
            # Consider raising an error or logging a warning.
            print(f"Warning: BaseGraphicsItem initialized on a non-QGraphicsItem: {type(self)}")


    def _setup_common_flags(self: QGraphicsItem): # Type hint self as QGraphicsItem
    ...
    def _setup_shadow_effect(self: QGraphicsItem, blur_radius: int = 10, offset: tuple = (2.5, 2.5), alpha: int = 60): # Type hint self
    ...
    # itemChange is a virtual method of QGraphicsItem.
    # It should be implemented in classes directly deriving from QGraphicsItem or its subclasses.
    # Removing it from BaseGraphicsItem to avoid the super() call error.
    # Derived classes will implement it as needed.
...
class GraphicsStateItem(QGraphicsRectItem, BaseGraphicsItem):
    ...
    def __init__(self, x: float, y: float, w: float, h: float, text: str, 
        ...
        QGraphicsRectItem.__init__(self, x, y, w, h) # Qt parent first
        BaseGraphicsItem.__init__(self) # Then mixin
    ...
    def itemChange(self, change, value):
        ...
        if change == QGraphicsItem.ItemPositionHasChanged and self.scene() and hasattr(self.scene(), 'item_moved'):
            self.scene().item_moved.emit(self)
        return QGraphicsRectItem.itemChange(self, change, value) # Call QGraphicsRectItem's method
...
class GraphicsTransitionItem(QGraphicsPathItem, BaseGraphicsItem):
    ...
    def __init__(self, start_item: Optional['GraphicsStateItem'], 
        ...
        QGraphicsPathItem.__init__(self) # Qt parent first
        BaseGraphicsItem.__init__(self) # Then mixin
    ...
    def itemChange(self, change, value):
        """Handle item changes and emit signals."""
        # Transitions are not typically 'moved' by direct dragging in this app,
        # their path is updated when connected states move.
        # If it were to be movable, and scene emits item_moved, this would handle it:
        # if change == QGraphicsItem.ItemPositionHasChanged and self.scene() and hasattr(self.scene(), 'item_moved'):
        #     self.scene().item_moved.emit(self) 
    ...
    def _setup_appearance(self):
        ...
        self.setFlag(QGraphicsItem.ItemIsMovable, False) # Transitions are not directly moved by user
        self.setZValue(-1)
    ...
    def boundingRect(self) -> QRectF:
        ...
        if current_label:
            ...
            if self.path().elementCount() > 0: # Ensure path is not empty
                mid_point = self.path().pointAtPercent(0.5)
            else: # Default point if path is empty (e.g. during initial creation)
                mid_point = QPointF(0,0) 
                if self.start_item and self.end_item: # Estimate midpoint if items exist
                    mid_point = (self.start_item.sceneBoundingRect().center() + self.end_item.sceneBoundingRect().center()) / 2.0

            
            text_render_rect = QRectF(
                mid_point.x() - text_rect.width()/2 - 10, # Center text_render_rect around midpoint
                mid_point.y() - text_rect.height()/2 - 10,
                text_rect.width() + 20,
                text_rect.height() + 20
            )
    ...
    def _create_path(self, start_point: QPointF, end_point: QPointF) -> QPainterPath:
        ...
        if self.start_item == self.end_item:
            # Self-loop
            path, end_point = self._create_self_loop_path(path) # end_point is updated for arrow drawing
        else:
            # Regular transition
    ...
    def _create_self_loop_path(self, path: QPainterPath) -> tuple[QPainterPath, QPointF]:
        ...
        return path, p2 # Return the actual end point of the loop for arrow
    ...
    def _get_intersection_point(self, item: QGraphicsRectItem, line: QLineF) -> Optional[QPointF]:
        ...
        intersect_points = []
        for edge in edges:
            # QLineF.intersects() in PyQt5 returns (IntersectType, QPointF)
            intersect_type, intersection_point = line.intersects(edge)
            
            if intersect_type == QLineF.BoundedIntersection:
                intersect_points.append(QPointF(intersection_point))
            elif hasattr(QLineF, "IntersectionType") and intersect_type == QLineF.IntersectionType.BoundedIntersection: # For newer enum style if applicable
                intersect_points.append(QPointF(intersection_point))

        if not intersect_points:
            return item_rect.center() # Fallback
        
        # Return closest intersection point to the line's starting point (p1 of the line argument)
    ...
    def _draw_arrow(self, painter: QPainter):
        ...
        # For self-loops, the path ends where it started visually, but the control points
        # determine the curve. The arrow should be at the visual end.
        if self.start_item == self.end_item:
            # For self-loops, calculate the arrow based on the cubicTo's end point
            rect = self.start_item.sceneBoundingRect()
            loop_radius_x = rect.width() * self.LOOP_RADIUS_FACTOR
            arrow_end_point = QPointF(rect.center().x() - loop_radius_x * 0.35, rect.top()) # This is p2 of the loop
            
            # Approximate angle for self-loop arrow
            # This is tricky. A simple horizontal angle pointing left.
            angle_at_end_rad = math.pi # Points left
            if self.control_point_offset.y() < 0 : # if loop goes "up"
                 angle_at_end_rad = math.pi # Default for loop on top
            # A more robust angle calculation would involve the derivative of the Bezier curve at the end.
            # For simplicity, using a fixed angle for loops for now.
            # For example, if the loop is on top, the arrow points left towards the state.
            # If the self.control_point_offset suggests a loop direction, it could be used here.

        else: # Regular transition
            percent_at_end = 0.999 if self.path().length() >= 1 else 0.9
            arrow_end_point = self.path().pointAtPercent(1.0)
        ...
        painter.drawPolygon(QPolygonF([arrow_end_point, arrow_p1, arrow_p2]))
    
    def _draw_label(self, painter: QPainter):
        """Draw transition label with background."""
        current_label = self._compose_label_string()
        if not current_label or self.path().elementCount() == 0: # Check path not empty
            return
        ...
        offset_angle_rad = (angle_at_mid_deg - 90.0) * (math.pi / 180.0) # Perpendicular offset
    ...
    def set_properties(self, event_str: str = "", condition_str: str = "", action_str: str = "",
        ...
        if offset is not None and self.control_point_offset != offset:
            ...
            changed = True # Path will be updated due to offset change
        
        if changed:
            self.update_path() # Always update path if any property affecting it or its label changed
            self.prepareGeometryChange() # For label changes
            self.update() # For visual changes
...
class GraphicsCommentItem(QGraphicsTextItem, BaseGraphicsItem):
    ...
    def __init__(self, x: float, y: float, text: str = "Comment"):
        QGraphicsTextItem.__init__(self) # Qt parent first
        BaseGraphicsItem.__init__(self) # Then mixin
    ...
    def itemChange(self, change, value):
        ...
        if change == QGraphicsItem.ItemPositionHasChanged and self.scene() and hasattr(self.scene(), 'item_moved'):
            self.scene().item_moved.emit(self)
        # Also handle ItemSelectedChange to ensure text interaction state is correct
        if change == QGraphicsItem.ItemSelectedChange:
            self.setTextInteractionFlags(Qt.TextEditorInteraction if value else Qt.NoTextInteraction)
    ...
    def _setup_interaction(self):
        ...
        # Initial state based on selection (usually not selected initially)
        self.setTextInteractionFlags(Qt.NoTextInteraction)
        # Other flags are set in BaseGraphicsItem.__init__()
    
    def _on_contents_changed(self):
        """Handle text content changes."""
        self.prepareGeometryChange()
        if self.scene() and hasattr(self.scene(), 'item_moved'): # Technically not 'moved' but 'changed'
            self.scene().item_moved.emit(self) # Signal that item content/bounds changed
    ...
    def set_properties(self, text: str, width: Optional[float] = None):
        ...
        if text_changed or width_changed:
            self.prepareGeometryChange() # Ensure bounds update if width or text content changes significantly
            self.update()
