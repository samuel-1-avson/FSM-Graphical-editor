...
class StateMachinePoweredSimulator:
    def __init__(self, states_data, transitions_data, parent_simulator=None, log_prefix="", halt_on_action_error=False):
        ...
        self._variables = {} # User-defined FSM variables
        self._action_log = []
        self.FSMClass = None
        ...
        # --- NEW: Timing ---
        self.current_tick = 0
        # self._fsm_delay_until_tick = 0 # For potential future non-blocking delay
    ...
    def _create_dynamic_callback(self, code_string, callback_type="action", original_name="dynamic_callback"):
        ...
        # Add 'current_tick' to the set of allowed names for safety check if needed,
        # though it's injected directly into the exec/eval scope later.
        # For BasicSafetyVisitor, it's more about what *user code* can access directly as a Name node.
        ...
        def dynamic_callback_wrapper(*args, **kwargs_from_sm_call):
            ...
            # --- MODIFIED: Prepare execution scope with current_tick ---
            exec_eval_locals_dict = simulator_self._variables.copy() # Start with a copy of main variables
            exec_eval_locals_dict['current_tick'] = simulator_self.current_tick
            # exec_eval_locals_dict['self_sm'] = simulator_self.sm # Potentially for advanced users, ensure safety implications
            ...
            # Log with current_tick already included by self._log_action
            ...
            try:
                if callback_type == "action":
                    ...
                    # Update simulator_self._variables from exec_eval_locals_dict
                    for key, value in exec_eval_locals_dict.items():
                        if key not in SAFE_BUILTINS and key != 'current_tick' and key != '__builtins__': # Don't overwrite injected vars
                    ...
                    # Conditions should not modify variables, so they operate on the prepared local dict
                ...
                logger.error(f"{simulator_self.log_prefix}{err_msg}", exc_info=False) # Keep exc_info False for cleaner logs unless debugging
                if callback_type == "condition": return False
            ...
            if state_def.get('is_superstate', False):
                ...
                if sub_fsm_data and sub_fsm_data.get('states'):
                    ...
                    try:
                        ...
                        # The sub-simulator's current_tick will start at 0 upon its init
    ...
    def reset(self):
        self._log_action("--- FSM Resetting ---") # Log action will use current_tick before reset
        self._variables.clear()
        self.simulation_halted_flag = False
        
        # --- NEW: Reset timing ---
        self.current_tick = 0
        # self._fsm_delay_until_tick = 0 # If implementing non-blocking delay
        
        if self.active_sub_simulator:
            self._log_action("Resetting active sub-machine...") # Parent's current_tick
        ...
        if self.FSMClass:
            try:
                ...
                self._log_action(f"FSM Reset. Current state: {current_state_id}") # Log will show tick 0
            except Exception as e:
    ...
    def step(self, event_name=None):
        ...
        # --- NEW: Increment tick at the start of every step ---
        self.current_tick += 1
        
        main_state_id = self.sm.current_state.id
        # Log action will now include the updated tick
        ...
        try:
            ...
            if self.active_sub_simulator:
                ...
                # Sub-simulator's step will also increment its own current_tick internally
                self._log_action(f"Stepping sub-machine in '{superstate_log_name}' with event: {event_name or 'None (Internal)'}.")
                _, sub_log = self.active_sub_simulator.step(event_name=event_name) # Pass event to sub if any
                self._action_log.extend(sub_log) # Sub-log will have its own tick context
            ...
            # Send event to the main FSM if an event_name is provided AND it wasn't consumed by an active sub-machine above.
            # The current python-statemachine model sends events to the current state. If current state is a superstate,
            # internal transitions of the superstate or transitions from the superstate itself are considered.
            # If the event is meant for the sub-machine, it has to be explicitly targeted or the sub-machine needs to
            # be the one processing events.
            # For now, if a sub-machine is active, we've already stepped it with the event.
            # If no sub-machine is active, or if the event is for the parent, process it here.
            if event_name and not self.active_sub_simulator: # Only send to parent if no active sub-machine took the event.
                self._log_action(f"Sending event '{event_name}' to FSM.")
                self.sm.send(event_name) 
            elif not event_name and not self.active_sub_simulator: 
                 self._log_action(f"No event. 'During' actions done. State remains '{main_state_id}'.")
            elif event_name and self.active_sub_simulator:
                 self._log_action(f"Event '{event_name}' was processed by active sub-machine. Parent FSM does not re-process it unless sub-machine signals parent.")
    ...
    def get_possible_events_from_current_state(self) -> list[str]:
        ...
        if self.active_sub_simulator and self.active_sub_simulator.sm:
            current_sm_to_query = self.active_sub_simulator.sm # Prioritize sub-machine events
        ...
        # If a sub-machine was active, also consider events that can transition the parent superstate
...
if __name__ == "__main__":
    # ... (main test code remains the same, it will now show ticks in logs) ...
    main_states_data = [
        {"name": "Idle", "is_initial": True, "entry_action": "print('Main: Idle Entered'); idle_counter = 0; Processing_sub_completed = False; my_timer = current_tick"},
        {"name": "Processing", "is_superstate": True,
         "sub_fsm_data": {
             ...
             "transitions": [
                 ...
                 {"source": "SubActive", "target": "SubDone", "event": "finish_sub_work", "condition": "sub_var > 11"} # Condition on sub_var
             ],
             "comments": []
         },
    ...
    main_transitions_data = [
        {"source": "Idle", "target": "Processing", "event": "start_processing", "condition": "current_tick >= my_timer + 2"}, # Example with tick
        {"source": "Processing", "target": "Done", "event": "auto_finish", "condition": "Processing_sub_completed == True and idle_counter > 1"}
    ]

    print("--- HIERARCHICAL SIMULATOR TEST (python-statemachine with Ticks) ---")
    try:
        ...
        print_status(simulator, "INITIAL STATE") # Tick 0 (init sets it)
        simulator.step(None); print_status(simulator, "AFTER main internal step 1 (Idle during)") # Tick 1
        simulator.step(None); print_status(simulator, "AFTER main internal step 2 (Idle during)") # Tick 2
        
        simulator.step("start_processing"); print_status(simulator, "AFTER 'start_processing'") # Tick 3, condition met
        
        if simulator.active_sub_simulator:
            print("\n>>> Trigger 'start_sub_work' on sub-machine (via parent's step) <<<")
            # Now, parent step propagates event to sub-machine IF sub-machine is active
            simulator.step("start_sub_work") # Tick 4 (parent), Sub-machine will get this event and step its own tick
            print_status(simulator, "AFTER sub-event 'start_sub_work'")
        
        simulator.step(None); print_status(simulator, "AFTER main internal step (sub during actions run)") # Tick 5 (parent)
        simulator.step(None); print_status(simulator, "AFTER main internal step (sub during actions run)") # Tick 6 (parent)
        
        if simulator.active_sub_simulator:
            print("\n>>> Trigger 'finish_sub_work' on sub-machine (via parent's step) <<<")
            simulator.step("finish_sub_work") # Tick 7 (parent)
            print_status(simulator, "AFTER sub-event 'finish_sub_work'")
        
        simulator.step("auto_finish"); print_status(simulator, "AFTER 'auto_finish'") # Tick 8 (parent)
