...
class CodeEditor(QPlainTextEdit):
    ...
    def set_language(self, language: str):
        ...
        # Mark the entire document content as dirty to force re-evaluation.
        # This is more robust than iterating and calling rehighlightBlock manually in some cases,
        # especially when switching from a highlighter to None.
        # However, for QPlainTextEdit, rehighlightBlock iteration is often necessary.

        if self.current_highlighter:
            self.current_highlighter.rehighlight() # New highlighter will handle it
        else:
            # If no highlighter is set (e.g., for "Text" language),
            # we need to ensure all blocks are re-rendered using default formatting.
            # Iterating through blocks and calling rehighlightBlock is a reliable way.
            block = doc.firstBlock()
            while block.isValid():
                # Create a dummy cursor, select the block, and apply a default format.
                # This is a bit more involved but ensures clearing previous formats.
                # cursor = QTextCursor(block)
                # cursor.select(QTextCursor.BlockUnderCursor)
                # default_format = QTextCharFormat() # Create an empty format
                # # You might want to set specific default colors from your palette if needed
                ...
                # Or, simpler: just tell Qt to rehighlight. It should use default if no highlighter.
                doc.rehighlightBlock(block)
                block = block.next()
        
        self.viewport().update() # Ensure viewport repaints
