...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ai_chatbot_manager.statusUpdate.connect(self._update_ai_chat_status) # If not already connected
        self.ai_chatbot_manager.responseReceived.connect(self._handle_ai_response)
    ...
    def _handle_ai_response(self, ai_message_text: str): # SLOT
        ...
        try:
            ...
            # For phase 1, we are not expecting actionable JSON for diagram modification yet.
            # So the FSM JSON processing part will likely not be triggered.
            # This part for handling FSM data can remain, it just won't be hit often in phase 1.
            if potential_json.startswith("{") and potential_json.endswith("}"):
                ...
                if isinstance(fsm_data, dict) and \
                   ...
                   "transitions" in fsm_data and isinstance(fsm_data["transitions"], list):
                    # ... (existing FSM JSON handling code) ...
                    print(f"--- MW_HANDLE_RESP ({current_time}): AI response IS valid FSM JSON. ---")
                    # ... (the rest of your _add_fsm_data_to_scene logic call) ...
            ...
            # self.log_message("AI response was not valid JSON.", type_hint="AI_DEBUG") # Already logged
    ...
    def _update_ai_chat_status(self, status_text: str): # SLOT
        current_time = QTime.currentTime().toString('hh:mm:ss.zzz')
        ...
        if hasattr(self, 'ai_chat_status_label'):
            ...
            # Allow sending even if previous send resulted in a non-blocking error.
            # API Key required is the main blocking error for sending.
