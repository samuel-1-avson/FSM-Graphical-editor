...
from fsm_simulator import FSMSimulator, FSMError # Assuming FSMSimulator is correctly in this path
...
class PySimulationUIManager(QObject):
    ...
    def __init__(self, main_window, parent=None):
        ...
        self.py_sim_continue_btn: QPushButton | None = None
        ...
        self.py_sim_current_tick_label: QLabel | None = None
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        for btn in [self.py_sim_start_btn, self.py_sim_stop_btn, self.py_sim_reset_btn]:
            if btn: # Ensure button was created
        ...
        self.py_sim_event_name_edit.setObjectName("PySimEventNameEdit") # For stylesheet
        event_layout.addWidget(self.py_sim_event_name_edit, 1)

        self.py_sim_trigger_event_btn = QPushButton("Trigger Event")
    ...
    def _update_internal_controls_enabled_state(self):
        ...
        sim_active = self.mw.py_sim_active # Correctly accesses MainWindow's attribute
        sim_controls_enabled = sim_active and not is_matlab_op_running

        is_paused_at_bp = False
        if sim_active and self.mw.py_fsm_engine: # Correctly accesses MainWindow's attribute
    ...
    def append_to_action_log(self, log_entries: list[str]):
        ...
        if self.py_sim_action_log_output.verticalScrollBar():
