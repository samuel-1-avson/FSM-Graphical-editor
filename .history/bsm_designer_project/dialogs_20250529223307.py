...
class StatePropertiesDialog(QDialog):
    ...
    def _on_edit_sub_fsm(self): 
        ...
        # Attempt to get the main window if deeply nested for dialog parent
        # This logic might need refinement based on actual parent structure
        if hasattr(sys, 'modules') and 'bsm_designer_project.main' in sys.modules:
            main_module = sys.modules['bsm_designer_project.main']
            if hasattr(main_module, 'MainWindow') and isinstance(dialog_parent_ref, main_module.MainWindow):
                 pass # Already the main window
            elif hasattr(dialog_parent_ref, 'parent') and callable(dialog_parent_ref.parent):
                 # Traverse up if current parent_window_ref is not MainWindow
                 temp_parent = dialog_parent_ref
                 while temp_parent and not isinstance(temp_parent, main_module.MainWindow):
                     if hasattr(temp_parent, 'parent') and callable(temp_parent.parent):
                         temp_parent = temp_parent.parent()
                     else: break # Stop if no further parent() method or not callable
                 if isinstance(temp_parent, main_module.MainWindow):
                     dialog_parent_ref = temp_parent
