...
class DiagramScene(QGraphicsScene):
    ...
    @pyqtSlot(QGraphicsItem) # <--- ENSURE THIS DECORATOR IS PRESENT
    def _handle_item_moved_visual_update(self, moved_item: QGraphicsItem):
        ...
        # Comments moving don't affect transitions directly.
    ...
    def edit_item_properties(self, item):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if isinstance(item, GraphicsHistoryPseudoStateItem) and 'default_target_substate_name' in new_props_from_dialog:
                target_name = new_props_from_dialog.pop('default_target_substate_name') # Remove name
                if target_name:
                    target_state_item = None
                    # If in sub-editor, parent_item's sub_fsm_data might not have live items
                    # We need to find the state by name in the *current scene* if it's a sub-scene
                    if isinstance(self.parent_window, QDialog) and self.parent_window.windowTitle().startswith("Sub-Machine Editor:"):
                         # In sub-editor, items are directly in `self` (DiagramScene instance for sub-machine)
                         for sub_item_candidate in self.items():
                             if isinstance(sub_item_candidate, GraphicsStateItem) and sub_item_candidate.text_label == target_name:
                                 target_state_item = sub_item_candidate
                                 break
                    elif item.parent_superstate_item: # Main scene context
                        # Default target must be a sub-state of the H-item's parent
                         for sub_state_data in item.parent_superstate_item.sub_fsm_data.get('states', []):
                             if sub_state_data.get('name') == target_name:
                                 # Now find this sub-state item instance if it were loaded/visualized
                                 # This part is tricky if sub-FSMs are not fully instantiated in main scene
                                 # For now, we assume we can get its UUID if it was loaded
                                 sub_state_uuid = sub_state_data.get('uuid')
                                 if sub_state_uuid:
                                     target_state_item = self.get_item_by_uuid(sub_state_uuid) # Find by UUID
                                 break
                         if not target_state_item: # Fallback by name in main scene if not found in sub_fsm_data by UUID
