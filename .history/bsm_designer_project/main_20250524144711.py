...
# ...
    ...
    # Apply layout algorithm (e.g., 'dot' for hierarchical layout)
        layout_algorithm = 'dot'  # Can be made configurable later
    ...
    # Scale and translate positions to fit the scene
        ...
        scale_factor = 100  # Adjust this for spacing
        start_x, start_y = 100, 100  # Starting offset in the scene

    # Add states to the scene
        for state in fsm_data.get('states', []):
            name = state.get('name')
            if name in node_positions:
                pos = node_positions[name]
                pos_x = start_x + (pos[0] - min_x) * scale_factor
                pos_y = start_y + (pos[1] - min_y) * scale_factor
                ...
                self.log_message(f"AI Gen: Added state '{name}' at ({pos_x:.0f}, {pos_y:.0f}).", type_hint="AI_ACTION")
    ...
    # Add transitions
        for trans in fsm_data.get('transitions', []):
            ...
            if source_item and target_item:
                ...
                self.log_message(f"AI Gen: Added transition '{source_name}' -> '{target_name}'.", type_hint="AI_ACTION")
