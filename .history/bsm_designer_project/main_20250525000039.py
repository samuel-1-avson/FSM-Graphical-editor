...
try:
    from logging_setup import setup_global_logging # Import your setup function
except ImportError:
    # Fallback or error if logging_setup.py is not found
...
from matlab_integration import MatlabConnection # logging changes would apply here too
...
# DraggableToolButton class (No logging changes needed here directly)
...
class DiagramScene(QGraphicsScene):
    ...
    def __init__(self, undo_stack, parent_window=None):
        ...
        # self.log_function = print # Replaced by standard logging
    ...
    def _update_transitions_for_renamed_state(self, old_name:str, new_name:str):
        logger.info("Scene notified: State '%s' changed to '%s'. Dependent transitions' data should reflect this.", old_name, new_name)
    ...
    # def set_log_function(self, log_function): # No longer needed
    ...
    def set_mode(self, mode: str):
        ...
        logger.info("Interaction mode changed to: %s", mode)
        self.transition_start_item = None
            ...
            if self.current_mode == "state":
                ...
                if isinstance(top_item_at_pos, GraphicsStateItem):
                    ...
                    if self.transition_start_item:
                        logger.info("Transition drawing cancelled (clicked non-state/empty space).")
                    self.transition_start_item = None
                    if self._temp_transition_line:
                        self.removeItem(self._temp_transition_line)
                        self._temp_transition_line = None
    ...
    def _show_context_menu(self, item, global_pos):
        ...
        if isinstance(item, GraphicsStateItem) and item.is_superstate:
            edit_sub_fsm_action = menu.addAction(get_standard_icon(QStyle.SP_FileDialogDetailedView, "Sub"), "Edit Sub-Machine...")
        else:
            edit_sub_fsm_action = None
        ...
        elif action == edit_sub_fsm_action and edit_sub_fsm_action is not None:
            self.edit_item_properties(item) 
    ...
    def edit_item_properties(self, item):
        ...
        dialog = DialogType(parent=self.parent_window, current_properties=old_props, scene_ref=self)
        ...
        if dialog_executed_and_accepted and new_props_from_dialog is not None:
            ...
            if final_new_props == old_props:
                logger.info("Properties unchanged.")
                return
            ...
            logger.info("Properties updated for: %s", item_name_for_log)
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        ...
        if item_type == "State":
            ...
            logger.warning("Unknown item type for addition: %s", item_type)
            return
        ...
        if current_item:
            ...
            logger.info("Added %s: %s at (%.0f,%.0f)", item_type, log_name, pos.x(), pos.y())
    ...
    def _handle_transition_click(self, clicked_state_item: GraphicsStateItem, click_pos: QPointF):
        if not self.transition_start_item: 
            ...
            logger.info("Transition started from: %s. Click target state.", clicked_state_item.text_label)
        else: 
            if self._temp_transition_line: 
                self.removeItem(self._temp_transition_line)
                self._temp_transition_line = None
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                self.undo_stack.push(cmd)
                logger.info("Added transition: %s -> %s [%s]", self.transition_start_item.text_label, clicked_state_item.text_label, new_transition._compose_label_string())
            else: 
                logger.info("Transition addition cancelled by user.")
        ...
        if event.key() == Qt.Key_Delete or (event.key() == Qt.Key_Backspace and sys.platform != 'darwin'): 
            ...
            if self.current_mode == "transition" and self.transition_start_item:
                ...
                if self._temp_transition_line:
                    self.removeItem(self._temp_transition_line)
                    self._temp_transition_line = None
                logger.info("Transition drawing cancelled by Escape.")
                self.set_mode("select")
    ...
    def delete_selected_items(self):
        ...
        if items_to_delete_with_related:
            ...
            self.undo_stack.push(cmd)
            logger.info("Queued deletion of %d item(s).", len(items_to_delete_with_related))
            self.clearSelection() 
    ...
    def dropEvent(self, event: QGraphicsSceneDragDropEvent):
        ...
        if event.mimeData().hasFormat("application/x-bsm-tool"):
            ...
            if item_type_data_str == "State":
                ...
                logger.warning("Unknown item type dropped: %s", item_type_data_str)
                event.ignore()
                return
        ...
        for item in self.items():
            if isinstance(item, GraphicsStateItem):
                ...
                if item.start_item and item.end_item: 
                    ...
                    logger.warning("Skipping save of orphaned/invalid transition: '%s'.", item._compose_label_string())
    ...
    def load_diagram_data(self, data):
        ...
        for trans_data in data.get('transitions', []):
            ...
            if src_item and tgt_item:
                ...
                logger.warning("Load Warning: Could not link transition '%s' due to missing states: Source='%s', Target='%s'.", 
                               label_info, trans_data['source'], trans_data['target'])
...
# ZoomableView class (No logging changes needed here directly)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.matlab_connection = MatlabConnection() # Will need logging updates
        self.undo_stack = QUndoStack(self)

        self.ai_chatbot_manager = AIChatbotManager(self)  # Will need logging updates
        self.scene = DiagramScene(self.undo_stack, self) 
        # self.scene.set_log_function(self.log_message) # Replaced by standard logging
        ...
        self.init_ui() # Creates log_output widget

        # Initialize global logging after UI elements (like self.log_output) are created
        try:
            setup_global_logging(self.log_output) # Pass the log widget
            logger.info("Main window initialized and logging configured.")
        except NameError: # Fallback if setup_global_logging wasn't imported
    ...
    def _create_docks(self):
        ...
        self.log_output = QTextEdit() # This is the widget for logs
        self.log_output.setObjectName("LogOutput")
        self.log_output.setReadOnly(True)
        # self.log_output.setStyleSheet("font-family: Consolas, monospace; font-size: 9pt;") # Style from QSS now
        # self.log_output.setHtml(f"<span style='color:{COLOR_TEXT_SECONDARY};'>[Starting log...]</span>") # Initial message handled by logger
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        try:
            ...
            if raw_gv_positions:
                ...
                max_y_shifted_scaled = 0 # Initialize
                # Check if raw_gv_positions is not empty before proceeding with min/max
                if raw_gv_positions: # Ensure this check
                    min_y_gv = min(p['y'] for p in raw_gv_positions)
                    shifted_scaled_positions = [{'name': p['name'],
        ...
        logger.debug("AI: ADD_FSM_TO_SCENE processing finished. Items queued: %d", len(items_to_add_for_undo_command))
    ...
    # Removed self.log_message - all logging now goes through standard `logging` module
    # The custom QTextEditHandler in logging_setup.py will handle UI display.
    # Status bar updates will be done explicitly where needed.
    ...
    def _update_window_title(self):
        ...
        # Update status label as well, if desired, as a general status
    ...
    def _update_matlab_status_display(self, connected, message):
        ...
        if "Initializing" not in message: 
            # Log this info using the standard logger
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        ...
        logging.debug("Internet Status: %s", message_detail) # DEBUG level for network checks
    ...
    def _append_to_py_simulation_log(self, log_entries: list[str]):
        ...
        if log_entries:
            ...
            if any(keyword in log_entries[-1] for keyword in important_keywords): 
                logging.info("PySim: %s", last_log_short) # Log important PySim events to main log
    ...
    def on_start_py_simulation(self):
        ...
        try:
            ...
            logging.error("PySim: Failed to start Python FSM simulation: %s", e, exc_info=True)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(dependencies_dir):
        try:
            ...
            # Use logging for this info now, if setup_global_logging happens before main_win
            # For now, print is fine as logging isn't fully set up until MainWindow instance
    ...
    main_win = MainWindow() # Logging will be fully set up inside MainWindow's __init__
    main_win.show()
    sys.exit(app.exec_())
