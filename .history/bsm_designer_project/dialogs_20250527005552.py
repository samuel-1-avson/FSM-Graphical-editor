...
class FindReplaceDialog(QDialog):
    def __init__(self, code_editor_ref, parent=None, replace_mode=False):
        ...
        self.whole_word_cb = QCheckBox("Whole word") # Basic implementation for now
    ...
    def on_find_next(self, search_backward=False):
        if not self.editor: return
        find_text = self.find_edit.text()
        if not find_text: return

        flags = self._get_find_flags()
        if search_backward: # This dialog doesn't have a "Find Previous" button yet
            flags |= QTextDocument.FindBackward

        found = self.editor.find(find_text, flags)
        if not found:
            # If not found, wrap around: move cursor to beginning (or end if searching backward) and search again
            cursor = self.editor.textCursor()
            initial_pos = cursor.position()
            cursor.movePosition(QTextDocument.Start if not search_backward else QTextDocument.End)
            self.editor.setTextCursor(cursor)
            found = self.editor.find(find_text, flags)
            if not found: # If still not found after wrapping
                QMessageBox.information(self, "Find", f"Cannot find '{find_text}'.")
                # Restore original cursor position if nothing was found after wrap
    ...
    def on_replace(self):
        ...
        cursor = self.editor.textCursor()
        # Check if current selection matches find_text, considering case sensitivity
    ...
    def on_replace_all(self):
        ...
        # Store original cursor to restore if nothing is replaced or to avoid jumping around
        original_cursor = self.editor.textCursor() 
        
        # Start search from the beginning of the document
        search_cursor = self.editor.textCursor()
        search_cursor.movePosition(QTextDocument.Start)
        self.editor.setTextCursor(search_cursor) # Move editor's cursor for find operation

        self.editor.textCursor().beginEditBlock() # Group all replacements into a single undo step
        
        while self.editor.find(find_text, flags): # find() moves the editor's main cursor
            self.editor.textCursor().insertText(replace_text)
            count += 1
        
        self.editor.textCursor().endEditBlock()

        # Restore original cursor position if desired, or leave it at the end of the last replacement
    ...
    def closeEvent(self, event):
        # Could store last search text/options here if desired for next time dialog opens
        # For example, save self.find_edit.text() to parent or settings
        if self.parent() and hasattr(self.parent(), '_find_replace_dialog_instance'):
             # Allow parent to know dialog is closing if it's managing a single instance
