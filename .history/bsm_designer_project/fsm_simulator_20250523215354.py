...
class FSMSimulator:
    ...
    def _evaluate_action(self, action_str):
        ...
        # Update the FSM's variables with any changes, logging only actual changes
        for var_name, new_value in local_vars_for_exec.items():
            old_value = original_vars_before_exec.get(var_name) # Get old value, or None if new
            if old_value != new_value or var_name not in original_vars_before_exec:
                self._log_action(f"Variable '{var_name}' changed from '{old_value}' to: '{new_value}'")
                self._variables[var_name] = new_value    
