    ...
    def process_message_slot(self, user_message: str, force_fsm_generation: bool):
        ...
        if is_fsm_generation_attempt:
            system_prompt_content += (
                ...
                "The JSON response MUST be a single object, starting with '{' and ending with '}'. "
                ...
                "It MUST have a key 'states' which is a list of state objects. Each state object in the 'states' list MUST have a 'name' (string, required, unique within the FSM). "
                "Optional state object keys are: 'is_initial' (boolean, default false, at most one state can be initial), 'is_final' (boolean, default false), "
                ...
                "Each transition object in the 'transitions' list MUST have 'source' (string, an existing state name from the 'states' list) and 'target' (string, an existing state name from the 'states' list). "
                "Optional transition object keys: 'event' (string), 'condition' (string), 'action' (string), "
                "'control_offset_x' (number), 'control_offset_y' (number), and 'properties' object (optional) for 'color'. "
                "Optionally, include a top-level key 'comments' which is a list of comment objects. Each comment object can have 'text' (string), 'x' (number, optional layout hint), 'y' (number, optional layout hint), 'width' (number, optional). "
                "Do not include any state positions (x, y, width, height for states) in the JSON, as the application will handle layout. "
                "ABSOLUTELY NO other text, greetings, explanations, or markdown formatting like ```json should be outside or inside this single JSON object response. The response MUST be parseable by json.loads(). Ensure all string values are properly quoted and escaped if necessary. Check for trailing commas in lists or objects as they are invalid in standard JSON."
                " An example of a minimal valid state is: {\"name\": \"S1\"}. An example of a minimal valid transition is: {\"source\": \"S1\", \"target\": \"S2\"}."
                " The 'states' and 'transitions' keys MUST be present, even if their lists are empty (e.g. {\"description\": \"empty\", \"states\": [], \"transitions\": []})."
            )
        else:
        ...
        # Combine dynamic system prompt with the user message for this turn
        # This ensures the system prompt, with its FSM generation instructions and context,
        # is always part of the current request to the LLM.
        effective_user_message = f"[System Instructions for this request:\n{system_prompt_content}\n]\n\nUser Question: {user_message}"
        
        gemini_contents.append({"role": "user", "parts": [{"text": effective_user_message}]})
        ...
        if is_fsm_generation_attempt:
            ...
            logger.info("WORKER_PROCESS: Requesting JSON object output format from Gemini.")

        try:
            if self._is_stopped:
                logger.info("WORKER_PROCESS: Worker stopped just before API call.")
                return

            # We are not using the 'system_instruction' parameter of GenerativeModel or generate_content
            # as the system prompt here is dynamic and contains conversation history context.
            # It's prepended to the user's message.
            generation_args = {
            ...
            ai_response_content = ""
            # Accessing response content based on Gemini API structure
            if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
                ai_response_content = response.candidates[0].content.parts[0].text
            elif hasattr(response, 'text'): # Fallback for simpler text attribute, if API changes
                ...
                if response.candidates and hasattr(response.candidates[0], 'finish_reason'):
                     finish_reason_info = str(response.candidates[0].finish_reason)
                
                error_msg = f"Gemini response was empty or malformed. Finish Reason: {finish_reason_info}. Prompt Feedback: {feedback_info}"
                logger.error(error_msg)
                self.errorOccurred.emit(error_msg)
                self._current_processing_had_error = True
                return

            # Add original user message (not the one with system prompt prepended) to history for cleaner display
            ...
            self._current_processing_had_error = True
        except (genai.types.BlockedPromptException, genai.types.StopCandidateException) as e: # These are specific to google.generativeai
            error_msg = f"Gemini content generation blocked or stopped: {e}"
            logger.error("WORKER_PROCESS: %s", error_msg, exc_info=True) # exc_info can be verbose for these
            self.errorOccurred.emit(error_msg)
            self._current_processing_had_error = True
        except Exception as e:
...
class AIChatbotManager(QObject):
    ...
    def _cleanup_existing_worker_and_thread(self):
        ...
        if self.chatbot_thread and self.chatbot_thread.isRunning():
            ...
            if not self.chatbot_thread.wait(200): # Reduced wait time for faster shutdown
    ...
    def _handle_worker_response(self, ai_response_content: str, was_fsm_generation_attempt: bool):
        ...
        if was_fsm_generation_attempt:
            # Attempt to strip markdown code block if present (Gemini sometimes adds it despite `response_mime_type`)
            cleaned_response_content = ai_response_content.strip()
            if cleaned_response_content.startswith("```json"):
                cleaned_response_content = cleaned_response_content[7:] # len("```json\n")
            ...
            elif cleaned_response_content.startswith("```"): # Generic code block start
                cleaned_response_content = cleaned_response_content[3:]
                if cleaned_response_content.endswith("```"):
                    cleaned_response_content = cleaned_response_content[:-3]
                cleaned_response_content = cleaned_response_content.strip()
            ...
            try:
                fsm_data = json.loads(cleaned_response_content)
                # More robust validation: check for 'states' and 'transitions' keys, and ensure they are lists.
                if isinstance(fsm_data, dict) and \
                   ...
                   'transitions' in fsm_data and isinstance(fsm_data['transitions'], list):
                    ...
                    logger.warning("MGR_HANDLE_WORKER_RESPONSE: JSON parsed but not valid FSM structure (missing/wrong type for states/transitions). Raw: %s", cleaned_response_content[:500])
                    self.errorOccurred.emit("AI returned JSON, but it's not a valid FSM structure (states/transitions keys missing or not lists). Displaying as text.")
            except json.JSONDecodeError:
                logger.warning("MGR_HANDLE_WORKER_RESPONSE: Failed to parse AI response as JSON. Treating as plain text. Raw: %s", ai_response_content[:500], exc_info=False) # Log raw for debugging
                self.statusUpdate.emit("Status: AI response was not valid FSM JSON.")
        
        # If not a valid FSM or not an FSM attempt, emit as plain response.
        logger.debug("MGR_HANDLE_WORKER_RESPONSE: Emitting plainResponseReady.")
        self.plainResponseReady.emit(ai_response_content) # Emit original if JSON parsing failed, or cleaned if it was just stripped
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        if is_fsm_gen_specific:
            ...
            self.last_fsm_request_description = None # Clear for non-FSM requests
