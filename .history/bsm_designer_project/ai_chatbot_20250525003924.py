...
class ChatbotWorker(QObject):
    ...
    @pyqtSlot(str, name='setApiKeyInWorker') 
    def _set_api_key_slot(self, api_key: str): 
        logger.info(f"WORKER: _set_api_key_slot called (new key {'SET' if api_key else 'NOT SET'}).")
        self.api_key = api_key
        self._initialize_client()

    # Try with no type in decorator for flexibility
    @pyqtSlot(name='setCurrentDiagramContextInWorker') 
    def _set_current_diagram_context_slot(self, diagram_context_arg): 
        if diagram_context_arg is None:
            logger.debug(f"WORKER: Setting diagram context to None (received None).")
            self.current_diagram_context_json_str = None
        elif isinstance(diagram_context_arg, str):
            if not diagram_context_arg: # Treat empty string as None too
                 logger.debug(f"WORKER: Setting diagram context to None (received empty string).")
                 self.current_diagram_context_json_str = None
            else:
                logger.debug(f"WORKER: Setting diagram context. Length: {len(diagram_context_arg)}")
                self.current_diagram_context_json_str = diagram_context_arg
        else:
            logger.warning(f"WORKER: _set_current_diagram_context_slot received UNEXPECTED type: {type(diagram_context_arg)}. Value: '{str(diagram_context_arg)[:50]}...' Setting context to None.")
            self.current_diagram_context_json_str = None


    # Try with no types in decorator for flexibility
    @pyqtSlot(name='processMessageInWorker') 
    def _process_message_slot(self, user_message_arg, force_fsm_generation_arg):
        # Explicitly cast/check types at the beginning of the method
        user_message = str(user_message_arg)
        force_fsm_generation = bool(force_fsm_generation_arg)

        logger.info(f"WORKER_PROCESS: _process_message_slot CALLED for: '{user_message[:50]}...' (force_fsm_generation={force_fsm_generation})")
    ...
    @pyqtSlot(name='clearHistoryInWorker')
    def _clear_history_slot(self):
        self.conversation_history = []
...
class AIChatbotManager(QObject):
    ...
    def set_api_key(self, api_key: str | None):
        ...
        if old_api_key_val != self.api_key or (self.api_key and (not self.chatbot_worker or not self.chatbot_thread or not self.chatbot_thread.isRunning())):
            ...
            if self.api_key:
             ...
             QMetaObject.invokeMethod(self.chatbot_worker, "setApiKeyInWorker", Qt.QueuedConnection, 
                                      self.api_key) 
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        if self.chatbot_worker: 
            # For slots without type hints in decorator, we can pass None directly
            QMetaObject.invokeMethod(self.chatbot_worker, "setCurrentDiagramContextInWorker", Qt.QueuedConnection, 
                                     diagram_json_str) 

            QMetaObject.invokeMethod(self.chatbot_worker, "processMessageInWorker", Qt.QueuedConnection,
                                     user_message_text, 
                                     is_fsm_gen_specific)
    ...
    def clear_conversation_history(self):
        ...
        if self.chatbot_worker and self.chatbot_thread and self.chatbot_thread.isRunning():
            QMetaObject.invokeMethod(self.chatbot_worker, "clearHistoryInWorker", Qt.QueuedConnection)
