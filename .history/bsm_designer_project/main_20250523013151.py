...
class MainWindow(QMainWindow):
    ...
    def _on_send_ai_chat_message(self):
        print(f"--- MW_SEND_MSG ({QTime.currentTime().toString('hh:mm:ss.zzz')}): _on_send_ai_chat_message TRIGGERED ---") # Timestamped
        user_message = self.ai_chat_input.text().strip()
        if not user_message:
            print(f"--- MW_SEND_MSG ({QTime.currentTime().toString('hh:mm:ss.zzz')}): Empty message, returning. ---")
            return

        self._append_to_ai_chat_display("You", user_message)
        self.ai_chat_input.clear()
        
        # print(f"--- MW_SEND_MSG ({QTime.currentTime().toString('hh:mm:ss.zzz')}): Disabling UI elements. ---")
        # self.ai_chat_send_button.setEnabled(False) # Handled by _update_ai_chat_status now
        # self.ai_chat_input.setEnabled(False)
        
        print(f"--- MW_SEND_MSG ({QTime.currentTime().toString('hh:mm:ss.zzz')}): Calling ai_chatbot_manager.send_message with: '{user_message}' ---")
        self.ai_chatbot_manager.send_message(user_message)

    def _handle_ai_response(self, ai_message_text: str): # SLOT
        print(f"--- MW_HANDLE_RESP ({QTime.currentTime().toString('hh:mm:ss.zzz')}): _handle_ai_response CALLED with: '{ai_message_text[:30]}...' ---")
        ...
        try:
            ...
            if potential_json.startswith("{") and potential_json.endswith("}"):
                ...
                if isinstance(fsm_data, dict) and \
                   ...
                   "transitions" in fsm_data and isinstance(fsm_data["transitions"], list):
                    ...
                    self.log_message("AI response is valid JSON and appears to be FSM data.", type_hint="AI_ACTION")
                    message_processed_as_fsm = True # Mark that we are handling it as FSM
                    
                    reply = QMessageBox.question(self, "Generate FSM from AI?",
                                                 "The AI provided FSM data. Add to diagram?",
                    ...
                    if reply == QMessageBox.Yes:
                        ...
                        self._append_to_ai_chat_display("AI Assistant", "[FSM data received, but not added by user.]")
                        final_status_message = "Status: FSM generation cancelled."
                # else: JSON, but not FSM structure
            ...
            # UI re-enablement happens at the end of this function now

        # If the message was NOT processed as FSM data, append it as a regular chat message.
        if not message_processed_as_fsm:
            print(f"--- MW_HANDLE_RESP ({QTime.currentTime().toString('hh:mm:ss.zzz')}): Appending as REGULAR chat message. ---")
            self._append_to_ai_chat_display("AI Assistant", ai_message_text) # THIS APPENDS IT
            # final_status_message is already "Status: Ready." by default
        
        # Centralized UI update and re-enabling
        self._update_ai_chat_status(final_status_message) 
        # Note: _update_ai_chat_status itself handles enabling/disabling buttons.
        # Ensure its logic correctly re-enables for "Ready", "Error", "Cancelled", "Added"
