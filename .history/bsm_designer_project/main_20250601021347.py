...
class MainWindow(QMainWindow):
    ...
    def _update_properties_dock(self):
        ...
        if len(selected_items) == 1:
            ...
            def fmt(txt, max_lines=2, max_line_chars=35, is_code=False): # MODIFIED fmt
                ...
                for i, line in enumerate(lines):
                    if i < max_lines:
                        ...
                        display_lines[-1] += " …" # Add ellipsis to the last shown line if more exist
                        break
                ...
                if is_code:
                    style = (f"font-family: Consolas, 'Courier New', monospace; {small_font_size_css} "
                             ...
                             f"display: block; white-space: pre-wrap; overflow: hidden; text-overflow: ellipsis;") # MODIFIED style
                else:
                    style = small_font_size_css # For non-code, less styling

                if is_code and not txt.strip(): # Handle empty code string
            ...
            # ... (rest of bool_fmt and table setup remains the same) ...
            # The calls to fmt for actions will now use the new parameters.
            # Example change for one action (apply to others):
            ...
            td_val_style = f"padding:5px 8px; border-bottom:1px solid {COLOR_BORDER_LIGHT}; vertical-align:top; word-break:break-all;" # Allow word break for long non-code text
            ...
            if isinstance(item, GraphicsStateItem):
                ...
                if props.get('condition'): label_parts.append(f"<span style='font-family:Consolas,monospace; color:{COLOR_TEXT_SECONDARY}; {small_font_size_css}'>[{fmt(props.get('condition'), max_lines=1, max_line_chars=25, is_code=True)}]</span>") # Condition likely shorter
                if props.get('action'): label_parts.append(f"<span style='font-family:Consolas,monospace;color:{QColor(COLOR_ACCENT_SECONDARY).darker(110).name()}; {small_font_size_css}'>/{{{fmt(props.get('action'), max_lines=1, max_line_chars=25, is_code=True)}}}</span>") # Action in label also short
                ...
                rows += f"<tr><td style='{td_key_style}'>Text:</td><td style='{td_val_style}white-space:pre-wrap; font-style:italic; color:{COLOR_TEXT_SECONDARY};'>{fmt(props.get('text', ''), max_lines=5, max_line_chars=50)}</td></tr>" # Allow more lines for comments
        ...
        # ... (rest of _update_properties_dock for multiple selection or no selection) ...
    ...
    def _on_ide_language_changed(self, language_param: str):
        ...
        self._update_window_title() # <--- ADDED to update IDE dock title on language change
