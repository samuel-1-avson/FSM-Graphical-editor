...
from export_utils import generate_plantuml_text # Add this import
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.scene.item_moved.connect(self._clear_validation_highlights_on_modify) # New connection
        self.scene.changed.connect(self._clear_validation_highlights_on_modify) # New connection
        ...
        if self.ai_chat_ui_manager:
            ...
            elif self._internet_connected : # Only show ready if also online
    ...
    def _create_actions(self):
        ...
        self.openai_settings_action = QAction(get_standard_icon(QStyle.SP_ComputerIcon, "AISet"), "AI Assistant Settings (Gemini)...", self) # Changed text
    ...
    def _create_menus(self):
        ...
        edit_menu.addSeparator()
        edit_menu.addAction(self.auto_layout_action) # Add Auto-Layout to Edit menu
        edit_menu.addSeparator()
        ...
        tools_menu.addAction(self.validate_diagram_action) # Add Validate Diagram to Tools menu
        ...
        ai_menu.addSeparator() # This separator seems fine
    ...
    def _create_toolbars(self):
        ...
        edit_toolbar.addSeparator() # New Separator
        edit_toolbar.addAction(self.auto_layout_action) # Add Auto-Layout to Edit Toolbar
        edit_toolbar.addAction(self.validate_diagram_action) # Add Validate Diagram to Edit Toolbar
    ...
    def _handle_py_sim_global_ui_enable_by_manager(self, enable: bool):
        ...
        diagram_editing_actions = [
            ...
            self.auto_layout_action, self.validate_diagram_action # Add new actions here
        ]
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        if clear_current_diagram:
            if not self.on_new_file(silent=True): # This now calls _clear_validation_highlights
        ...
        else: # If not clearing, still clear any existing validation highlights
        ...
        for i, state_data in enumerate(fsm_data.get('states', [])):
            ...
            try:
                state_item = GraphicsStateItem(pos_x, pos_y, item_w, item_h, name,
                    ...
                    action_language=state_data.get('action_language', DEFAULT_EXECUTION_ENV), # Pass action_language
            ...
            if src_item and tgt_item:
                try:
                    trans_item = GraphicsTransitionItem(src_item, tgt_item,
                        ...
                        action_language=trans_data.get('action_language', DEFAULT_EXECUTION_ENV), # Pass action_language
        ...
        elif not clear_current_diagram: # Ensure macro is ended even if no items added
             self.undo_stack.endMacro()
    ...
    def _update_properties_dock(self):
        ...
        if len(selected_items) == 1:
            ...
            if isinstance(item, GraphicsStateItem):
                ...
                rows += f"<tr><td><b>Language:</b></td><td>{html.escape(props.get('action_language', DEFAULT_EXECUTION_ENV))}</td></tr>" # Display language
    ...
    def on_new_file(self, silent=False):
        ...
        self._clear_validation_highlights_on_modify() # Clear validation if any
    ...
    def _load_from_path(self, file_path):
        self._clear_validation_highlights_on_modify() # Clear validation if loading
        try:
    ...
    def _save_to_path(self, file_path) -> bool:
        ...
        self._clear_validation_highlights_on_modify() # Clear validation before save
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        self._clear_validation_highlights_on_modify() # Clear before closing
        ...
        if hasattr(self, 'ai_chat_ui_manager') and self.ai_chat_ui_manager:
            ...
            if self.ai_chat_ui_manager.ai_chat_input:
                ...
                if not is_online_and_key_present:
                    if self.ai_chatbot_manager and not self.ai_chatbot_manager.api_key: # Check if manager exists
    ...
    def _on_ide_language_changed(self, language: str):
        ...
        if self.ide_analyze_action:
            ...
            can_analyze = (language == "Python" or language.startswith("C/C++")) and \
                          ...
                          self._internet_connected # Also check internet connection
    ...
    def on_export_mermaid(self):
        # ... (Mermaid export logic - can be similarly structured using a generate_mermaid_text function) ...
        ...
        diagram_data = self.scene.get_diagram_data()
        diagram_title = os.path.splitext(os.path.basename(self.current_file_path or "FSM Diagram"))[0]
        try:
            # Placeholder for mermaid_text = generate_mermaid_text(diagram_data, diagram_title)
            mermaid_text = "```mermaid\nstateDiagram-v2\n  direction LR\n"
            mermaid_text += f"  %% title: {diagram_title}\n" # Mermaid doesn't have a formal title like PlantUML diagram name
            
            initial_state_id_mermaid = None
            state_id_map_mermaid = {}

            for state_data in diagram_data.get("states", []):
                s_name = state_data.get('name', 'UnnamedState')
                s_id = _sanitize_puml_id(s_name) # Re-use sanitize for Mermaid IDs
                state_id_map_mermaid[s_name] = s_id
                s_label = state_data.get('name') # Mermaid uses the ID as the label by default or can set a label
                
                mermaid_text += f"  {s_id}"
                if s_label != s_id: # Only add label if different from ID or if you always want explicit labels
                     mermaid_text += f" : {s_label}"
                mermaid_text += "\n"
            ...
            for trans_data in diagram_data.get("transitions", []):
                ...
                src_id = state_id_map_mermaid.get(src_name)
                tgt_id = state_id_map_mermaid.get(tgt_name)
                ...
                if trans_data.get('action'): label_parts.append(f"/{trans_data['action'].replace(chr(10),' ')}") # Mermaid less tolerant of newlines in labels
                
                label = " : ".join(label_parts) if label_parts else ""
                mermaid_text += f"  {src_id} --> {tgt_id}{label}\n"

            # Mermaid comments are typically %%
    ...
    def on_auto_layout(self):
        ...
        self._clear_validation_highlights_on_modify() # Clear any previous validation
        ...
        G = pgv.AGraph(directed=True, strict=False, rankdir='TB', ratio='auto', nodesep='1.0', ranksep='1.5 equally') # Increased separation
        default_item_width, default_item_height = 120, 60
        ...
        for state_data in diagram_data.get('states', []):
            ...
            if name:
                # Add fixedsize=true to respect width/height more, but can lead to overlaps if not careful
        ...
        for trans_data in diagram_data.get('transitions', []):
            ...
            if source and target and G.has_node(source) and G.has_node(target):
                G.add_edge(source, target) # Label can make layout crowded, consider omitting for layout pass

        graphviz_positions = {}
        GV_SCALE = 1.3 # Increased scale for more spread
        layout_start_x, layout_start_y = 70, 70

        try:
            G.layout(prog="dot") # "dot" is good for hierarchical
            raw_gv_pos = [{'name': n.name, 'x': float(n.attr['pos'].split(',')[0]), 'y': float(n.attr['pos'].split(',')[1])}
                          for n in G.nodes() if 'pos' in n.attr]
            if raw_gv_pos:
                min_x_gv = min(p['x'] for p in raw_gv_pos)
                max_y_gv = max(p['y'] for p in raw_gv_pos)

                for p_gv in raw_gv_pos:
                    scene_x = (p_gv['x'] - min_x_gv) * GV_SCALE + layout_start_x
                    scene_y = (max_y_gv - p_gv['y']) * GV_SCALE + layout_start_y # Invert Y for Qt
            ...
            # Fallback to a very basic grid layout if Graphviz fails
            graphviz_positions.clear() # Ensure it's empty
            states = diagram_data.get('states', [])
            num_states = len(states)
            cols = int(math.sqrt(num_states)) + 1
            for i, state_data in enumerate(states):
                name = state_data.get('name')
                row, col = divmod(i, cols)
                scene_x = layout_start_x + col * (default_item_width + 100)
        ...
        for item_name, new_pos_qpoint in graphviz_positions.items():
            ...
            if item:
                old_pos = item.pos()
                if (new_pos_qpoint - old_pos).manhattanLength() > 1.0:
                    moved_items_data_for_command.append((item, old_pos, new_pos_qpoint))

        if moved_items_data_for_command:
            from undo_commands import MoveItemsCommand # Local import
    ...
    def on_validate_diagram(self):
        ...
        if not issues:
            ...
            self.scene.highlight_validation_issues(issues)
    ...
    def _on_validation_issue_selected(self, list_item: QListWidgetItem):
        ...
        if isinstance(item_ref, QGraphicsItem) and item_ref.scene() == self.scene:
            ...
            self.view.ensureVisible(item_ref, 50, 50)
        elif isinstance(item_ref, str): # If only name was stored (e.g. for duplicate name)
