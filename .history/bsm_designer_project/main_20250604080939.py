...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Managers
        self.alignment_manager = AlignmentManager(self) # Instantiated here
    ...
    def _connect_signals(self):
        if hasattr(self, 'scene') and self.scene: 
            # self.scene.selectionChanged.connect(self._update_align_distribute_actions_enable_state) # Removed, AlignmentManager handles this
    ...
    def _create_actions(self):
        ...
        # Alignment & Distribution Actions are now retrieved from AlignmentManager in _create_menus and _create_toolbars
    ...
    def _create_toolbars(self):
        ...
        # Add actions from AlignmentManager
    ...
    def closeEvent(self, event: QCloseEvent):
            ...
            # 1. Prompt for saves (IDE first, then main diagram)
            if self.ide_manager and not self.ide_manager._prompt_ide_save_if_dirty():
                event.ignore(); return
            if self.file_op_manager and not self.file_op_manager._prompt_save_if_dirty():
                event.ignore(); return
        
            # 2. Attempt to stop operations that might use threads or prevent clean shutdown
            ...
            # 3. Handle Resource Monitor Thread
            ...
            if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
                ...
                if self.resource_monitor_worker:
                    ...
                    else: # Already in the same thread or worker has no thread affinity (unlikely for QThread scenario)
            ...
            # 4. Shutdown NVML (if worker existed)
            ...
            managers_to_process = [
                ...
                ('ai_chatbot_manager', self.ai_chatbot_manager), # Ensure chatbot manager is also cleaned up
                ('resource_monitor_worker', self.resource_monitor_worker), # Worker first
                ('resource_monitor_thread', self.resource_monitor_thread), # Then thread
            ]
            ...
            for manager_name, manager_obj in managers_to_process:
                if manager_obj:
                    ...
                    # For QThreads, ensure they are not running before deleteLater
                    if isinstance(manager_obj, QThread) and manager_obj.isRunning():
                        ...
                        if not manager_obj.wait(500):
                             manager_obj.terminate() # Force if quit fails
                             manager_obj.wait(100)

                    if isinstance(manager_obj, QObject): # Only QObjects can be deleted later
                        manager_obj.deleteLater()
                        logger.debug(f"MW_CLOSE: Scheduled {manager_name} for deletion.")
                    
                    # Clear the attribute on MainWindow
            ...
            # 6. Other cleanup (MATLAB threads warning, temp dir)
    ...
    def _update_align_distribute_actions_enable_state(self):
        if self.alignment_manager:
            self.alignment_manager.update_align_distribute_actions_enable_state()
        else:
            logger.warning("MainWindow: AlignmentManager not available to update action states.")
