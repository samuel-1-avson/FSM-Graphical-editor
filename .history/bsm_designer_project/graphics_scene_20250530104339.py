...
class DiagramScene(QGraphicsScene):
    ...
    def _update_transitions_for_renamed_state(self, old_name:str, new_name:str):
        ...
        for item in self.items():
            if isinstance(item, GraphicsTransitionItem):
                # This is mostly for data integrity if transitions store names.
                # Visual updates are handled by _update_connected_transitions.
                # The EditItemPropertiesCommand should ensure data consistency.
        ...
        if self.current_mode == "select" and event.buttons() & Qt.LeftButton and self._mouse_press_items_positions:
            if self.snap_to_objects_enabled and self._show_dynamic_snap_guidelines:
                ...
                if dragged_items:
                    ...
                    # Use event.buttonDownScenePos for the consistent start of the drag gesture
