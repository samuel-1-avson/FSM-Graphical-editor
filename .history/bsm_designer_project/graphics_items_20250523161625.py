...
from PyQt5.QtGui import (QBrush, QColor, QFont, QPen, QPainterPath, QPolygonF, QPainter,
                         QPainterPathStroker, QPixmap, QMouseEvent, QDrag, QPalette) # Removed duplicate QMouseEvent
...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
                 color=None, entry_action="", during_action="", exit_action="", description=""): # Corrected indentation
        ...
        self.original_brush_for_py_sim_restore = self.brush()
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        current_brush_to_use = self.brush()

        if self._is_hovered and not self.isSelected() and not self.is_py_sim_active:
            current_pen_to_use = QPen(self.hover_border_color, self._border_pen_width + 0.5)
            current_brush_to_use = QBrush(self.hover_brush_color)
        # When not hovered, selected, or in sim, the original pen/brush set in init or set_properties will be used.
        # The self.pen() and self.brush() calls above will fetch these defaults.
        ...
        if self.is_py_sim_active:
            ...
            # current_brush_to_use = QBrush(COLOR_PY_SIM_STATE_ACTIVE.lighter(150)) # Optional

        painter.setPen(current_pen_to_use)
        painter.setBrush(current_brush_to_use)
    ...
    def set_py_sim_active_style(self, active: bool):
        ...
        if active:
            ...
            # No need to set the sim style pen/brush here, paint() handles it
        else:
    ...
    def set_properties(self, name, is_initial, is_final, color_hex=None,
                       entry_action="", during_action="", exit_action="", description=""): # CORRECTED INDENTATION
        changed = False
        ...
        if self.base_color != new_base_color:
            ...
            self.hover_border_color = self.border_color.lighter(130) # Update hover colors
            self.hover_brush_color = self.base_color.lighter(105)   # Update hover colors

            current_pen = QPen(self.border_color, self._border_pen_width)
            current_brush = QBrush(self.base_color)

            self.setPen(current_pen) # Set the actual pen
            self.setBrush(current_brush) # Set the actual brush

            # Update backup pens/brushes if not in PySim mode
            # If in PySim mode, original_... should reflect the true underlying style, not the sim style
            self.original_pen_for_py_sim_restore = QPen(current_pen)
            self.original_brush_for_py_sim_restore = QBrush(current_brush)
            changed = True

        if self.entry_action != entry_action: self.entry_action = entry_action; changed = True
        if self.during_action != during_action: self.during_action = during_action; changed = True
        if self.exit_action != exit_action: self.exit_action = exit_action; changed = True
...
class GraphicsTransitionItem(QGraphicsPathItem): # (Code for this class remains the same as previously provided)
    ...
    def _get_intersection_point(self, item: QGraphicsRectItem, line: QLineF):
        ...
        for edge in edges:
            ...
            intersect_type = line.intersect(edge, intersection_point_var) # intersect_type is QLineF.IntersectType
            if intersect_type == QLineF.BoundedIntersection: # Check for BoundedIntersection
                 # Check if point lies on the segment for edge as QLineF.intersect can be tricky
    ...
    def set_properties(self, event_str="", condition_str="", action_str="",
                       color_hex=None, description="", offset=None): # offset is QPointF for control_point_offset
        changed = False
        ...
        if offset is not None and self.control_point_offset != offset:
            self.control_point_offset = offset # QPointF passed directly
...
class GraphicsCommentItem(QGraphicsTextItem): # (Code for this class remains the same)
    ...
    def __init__(self, x, y, text="Comment"):
        ...
        self.shadow_effect = QGraphicsDropShadowEffect(); self.shadow_effect.setBlurRadius(8); self.shadow_effect.setColor(QColor(0,0,0,50)); self.shadow_effect.setOffset(2,2); self.setGraphicsEffect(self.shadow_effect)
    ...
    def set_properties(self, text, width=None):
        ...
        width_changed = False; current_text_width = self.textWidth(); target_width = width if width and width > 0 else self._default_width
