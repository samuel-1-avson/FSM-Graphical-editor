--- START OF FILE bsm_designer_project/graphics_items.py ---
...
from PyQt5.QtWidgets import (QGraphicsRectItem, QGraphicsPathItem, QGraphicsTextItem,
                             ...
                             QGraphicsEllipseItem # Added for HistoryPseudoState
                             )
...
from PyQt5.QtCore import Qt, QRectF, QPointF, QMimeData, QPoint, QLineF, QSize, pyqtSignal, QEvent, QObject, QTimer # Added QTimer
...
from config import (
    ...
    COLOR_ACCENT_WARNING, # For validation
    # History Pseudo-State Styling
    COLOR_ITEM_HISTORY_PSEUDO_STATE_BG, COLOR_ITEM_HISTORY_PSEUDO_STATE_BORDER,
    COLOR_ITEM_HISTORY_PSEUDO_STATE_TEXT, DEFAULT_HISTORY_TRANSITION_COLOR,
    DEFAULT_HISTORY_TRANSITION_STYLE
)
...
class GraphicsStateItem(QGraphicsRectItem):
    ...
    def __init__(self, x, y, w, h, text, is_initial=False, is_final=False,
                 ...
                 item_uuid=None): # <--- ADD UUID PARAMETER
        super().__init__(x, y, w, h)
        self.uuid = item_uuid if item_uuid else str(uuid.uuid4()) # <--- ASSIGN UUID
        ...
        self.original_pen = QPen(self.border_color, self._border_pen_width)
        ...
        self.contained_history_pseudo_states: list[GraphicsHistoryPseudoStateItem] = []
    ...
    def _determine_current_pen(self) -> QPen:
        ...
        if self.is_py_sim_active:
            ...
            pen_to_use.setColor(self.border_color)
            return pen_to_use
    ...
    def set_properties(self, name, is_initial, is_final, color_hex=None,
        ...
        if self.base_color != new_base_color or self.border_color != new_border_color:
            ...
            current_base_pen_width = self._superstate_border_pen_width if (is_superstate_prop if is_superstate_prop is not None else self.is_superstate) else self._border_pen_width
    ...
    def itemChange(self, change, value):
        ...
        elif change == QGraphicsItem.ItemSceneChange and not value: # Being removed from scene
            for h_item in list(self.contained_history_pseudo_states):
                h_item._cleanup_default_transition()
                if h_item.scene(): h_item.scene().removeItem(h_item)
                h_item.deleteLater()
            self.contained_history_pseudo_states.clear()
        return super().itemChange(change, value)

    def get_data(self):
        return {'uuid': self.uuid,
                'name': self.text_label, 'x': self.x(), 'y': self.y(), 'width': self.rect().width(), 'height': self.rect().height(),
    ...
    def add_history_pseudo_state(self, history_item: 'GraphicsHistoryPseudoStateItem'):
        if history_item not in self.contained_history_pseudo_states:
            self.contained_history_pseudo_states.append(history_item)

    def remove_history_pseudo_state(self, history_item: 'GraphicsHistoryPseudoStateItem'):
        if history_item in self.contained_history_pseudo_states:
            self.contained_history_pseudo_states.remove(history_item)
...
class GraphicsTransitionItem(QGraphicsPathItem):
    ...
    def __init__(self, start_item, end_item, event_str="", condition_str="", action_str="",
                 ...
                 item_uuid=None):
        super().__init__()
        self.uuid = item_uuid if item_uuid else str(uuid.uuid4())
        self.start_item: QGraphicsItem | None = start_item
        self.end_item: GraphicsStateItem | None = end_item # Target is always a GraphicsStateItem for regular transitions
        ...
        if color is None:
            self.base_color = QColor(COLOR_ITEM_TRANSITION_DEFAULT)
            self.original_pen = QPen(self.base_color, self._pen_width, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
        elif isinstance(color, QPen):
            self.original_pen = QPen(color)
            self.base_color = self.original_pen.color()
            self._pen_width = self.original_pen.widthF()
        else:
            self.base_color = QColor(color) if QColor(color).isValid() else QColor(COLOR_ITEM_TRANSITION_DEFAULT)
            self.original_pen = QPen(self.base_color, self._pen_width, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
    ...
    def _determine_current_pen(self) -> QPen:
        if self.is_py_sim_active:
            ...
            return QPen(QColor(COLOR_ACCENT_WARNING), self.original_pen.widthF() + 0.7, Qt.DashDotLine, Qt.RoundCap, Qt.RoundJoin)
        else:
            pen_to_use = QPen(self.original_pen)
    ...
    def set_problematic_style(self, is_problematic: bool, problem_description: str = ""):
        ...
        self._is_problematic = is_problematic
        self._problem_tooltip_text = problem_description if is_problematic else ""
        self.setToolTip(self._problem_tooltip_text or self.description or self._compose_label_string())
        self.update()
    ...
    def paint(self, painter: QPainter, option, widget):
        ...
        painter.setPen(current_pen_for_drawing); painter.setBrush(Qt.NoBrush)
        ...
        arrow_brush_color = current_pen_for_drawing.color()
        arrow_pen = QPen(arrow_brush_color, 1, Qt.SolidLine)
        painter.setPen(arrow_pen); painter.setBrush(arrow_brush_color)
        painter.drawPolygon(QPolygonF([line_end_point, arrow_p1, arrow_p2]))
    ...
    def set_properties(self, event_str="", condition_str="", action_str="", color_hex=None, description="", offset=None, action_language=DEFAULT_EXECUTION_ENV):
        ...
        if self.base_color != new_color:
            ...
            self.original_pen = QPen(self.base_color, self._pen_width, self.original_pen.style(), Qt.RoundCap, Qt.RoundJoin)
            changed = True
    ...
    def update_path(self):
        ...
        if isinstance(self.start_item, GraphicsStateItem) or isinstance(self.start_item, GraphicsHistoryPseudoStateItem):
            start_rect_center = self.start_item.sceneBoundingRect().center()
        else: start_rect_center = self.start_item.pos() 
    ...
    def _get_intersection_point(self, item: QGraphicsItem, line: QLineF):
        item_rect = item.sceneBoundingRect(); rect_path = QPainterPath()
        if isinstance(item, GraphicsStateItem): rect_path.addRoundedRect(item_rect, 12, 12)
        elif isinstance(item, GraphicsHistoryPseudoStateItem): rect_path.addEllipse(item_rect)
        else: rect_path.addRect(item_rect)
    ...
    def get_data(self):
        start_label = "None"
        start_uuid = None
        if isinstance(self.start_item, GraphicsStateItem):
            start_label = self.start_item.text_label
            start_uuid = self.start_item.uuid
        elif isinstance(self.start_item, GraphicsHistoryPseudoStateItem):
            start_label = f"H:{self.start_item.parent_superstate_item.text_label if self.start_item.parent_superstate_item else 'OrphanedH'}"
            start_uuid = self.start_item.uuid

        target_label = "None"
        target_uuid = None
        if isinstance(self.end_item, GraphicsStateItem):
            target_label = self.end_item.text_label
            target_uuid = self.end_item.uuid
        elif isinstance(self.end_item, GraphicsHistoryPseudoStateItem): # Transitions can target H-items
            target_label = f"H:{self.end_item.parent_superstate_item.text_label if self.end_item.parent_superstate_item else 'OrphanedH'}"
            target_uuid = self.end_item.uuid

        return {'uuid': self.uuid,
                'source': start_label,
                'source_uuid': start_uuid,
                'target': target_label,
...
class GraphicsCommentItem(QGraphicsTextItem):
    ...
    def __init__(self, x, y, text="Comment", item_uuid=None):
        super().__init__();
        self.uuid = item_uuid if item_uuid else str(uuid.uuid4())
    ...
    def set_problematic_style(self, is_problematic: bool, problem_description: str = ""):
        ...
        self._is_problematic = is_problematic
        self._problem_tooltip_text = problem_description if is_problematic else ""
        self.setToolTip(self._problem_tooltip_text or self.toPlainText())
        self.border_pen = self._determine_current_pen()
        self.update()
    ...
    def paint(self, painter, option, widget):
        painter.setRenderHint(QPainter.Antialiasing)
        current_pen_for_drawing = self._determine_current_pen()
        painter.setPen(current_pen_for_drawing); painter.setBrush(self.background_brush)
    ...
    def set_properties(self, text, width=None):
        ...
        width_changed = False; target_width = float(width) if width is not None and float(width) > 0 else -1
        ...
        if abs(current_actual_width - (target_width if target_width >0 else (self.document().idealWidth() if self.document() else self._default_width))) > 1e-3 : width_changed = True
...
class GraphicsHistoryPseudoStateItem(QGraphicsEllipseItem):
    Type = QGraphicsItem.UserType + 4

    def type(self):
        return GraphicsHistoryPseudoStateItem.Type

    RADIUS = 12

    def __init__(self, x_relative: float, y_relative: float,
                 parent_superstate_item: GraphicsStateItem,
                 default_target_substate_item: GraphicsStateItem | None = None,
                 history_type: str = "shallow",
                 item_uuid: str | None = None):
        super().__init__(-self.RADIUS, -self.RADIUS, 2 * self.RADIUS, 2 * self.RADIUS)
        self.uuid = item_uuid if item_uuid else str(uuid.uuid4())
        
        self.parent_superstate_item = parent_superstate_item
        self.default_target_substate_item = default_target_substate_item
        self.history_type = history_type
        self.text_label = "H" if history_type == "shallow" else "H*"

        self.setParentItem(parent_superstate_item) 
        parent_superstate_item.add_history_pseudo_state(self) 
        self.setPos(x_relative, y_relative)

        self.base_color_bg = QColor(COLOR_ITEM_HISTORY_PSEUDO_STATE_BG)
        self.base_color_border = QColor(COLOR_ITEM_HISTORY_PSEUDO_STATE_BORDER)
        self.text_color = QColor(COLOR_ITEM_HISTORY_PSEUDO_STATE_TEXT)
        self._pen_width = 1.5
        self._font = QFont(APP_FONT_FAMILY, 9, QFont.Bold)

        self.original_pen = QPen(self.base_color_border, self._pen_width)
        self.setPen(self.original_pen)
        self.setBrush(QBrush(self.base_color_bg))

        self.setFlags(QGraphicsItem.ItemIsSelectable | QGraphicsItem.ItemIsMovable |
                      QGraphicsItem.ItemSendsGeometryChanges | QGraphicsItem.ItemIsFocusable)
        self.setAcceptHoverEvents(True)
        self.setToolTip(f"{history_type.capitalize()} History Pseudo-State")
        ...
        self.default_history_transition_item: GraphicsTransitionItem | None = None
        if self.scene():
            self.update_default_history_transition()


    def _determine_current_pen(self) -> QPen:
        if self._is_problematic:
            return QPen(QColor(COLOR_ACCENT_WARNING), self._pen_width + 0.7, Qt.DashDotLine)
        else:
            pen_to_use = QPen(self.original_pen)
            pen_to_use.setColor(self.base_color_border)
            pen_to_use.setWidthF(self._pen_width)
            return pen_to_use

    def paint(self, painter: QPainter, option, widget):
        painter.setRenderHint(QPainter.Antialiasing)
        current_pen = self._determine_current_pen()
        ...
        if self.isSelected():
            selection_pen = QPen(QColor(COLOR_ITEM_STATE_SELECTION_BORDER), current_pen.widthF() + 0.5, Qt.DashLine)
            selection_brush_color = QColor(COLOR_ITEM_STATE_SELECTION_BG); selection_brush_color.setAlpha(80)
    ...
    def boundingRect(self) -> QRectF:
        pen_half_width = self._pen_width / 2.0 + 2
        return self.rect().adjusted(-pen_half_width, -pen_half_width, pen_half_width, pen_half_width)

    def shape(self) -> QPainterPath:
        path = QPainterPath(); path.addEllipse(self.rect()); return path

    def set_default_target_substate(self, target_substate_item: GraphicsStateItem | None):
        if self.default_target_substate_item != target_substate_item:
            self.default_target_substate_item = target_substate_item
            self.update_default_history_transition()
            if self.scene():
                self.scene().set_dirty(True)
                self.scene().run_all_validations("HistoryDefaultTargetChanged")

    def update_default_history_transition(self):
        if self.default_history_transition_item and self.default_history_transition_item.scene():
            self.scene().removeItem(self.default_history_transition_item)
            self.default_history_transition_item.deleteLater()
        self.default_history_transition_item = None

        if self.default_target_substate_item and self.scene():
            transition_pen = QPen(QColor(DEFAULT_HISTORY_TRANSITION_COLOR), 1.5, DEFAULT_HISTORY_TRANSITION_STYLE)
            transition = GraphicsTransitionItem(start_item=self,
                                                end_item=self.default_target_substate_item,
                                                color=transition_pen,
                                                item_uuid=str(uuid.uuid4())) # New UUID for this programmatic transition
            transition.setZValue(self.zValue() - 0.1)
            self.scene().addItem(transition)
            transition.update_path()
            self.default_history_transition_item = transition
            self.default_history_transition_item.setFlag(QGraphicsItem.ItemIsSelectable, False)
            self.default_history_transition_item.setFlag(QGraphicsItem.ItemIsFocusable, False)
            logger.debug(f"Default history transition: {self.text_label} -> {self.default_target_substate_item.text_label}")
        elif self.default_target_substate_item is None:
            logger.debug(f"Cleared default history transition for {self.text_label}.")

    def get_data(self) -> dict:
        parent_name = self.parent_superstate_item.text_label if self.parent_superstate_item else None
        parent_uuid = self.parent_superstate_item.uuid if self.parent_superstate_item else None
        default_target_name = self.default_target_substate_item.text_label if self.default_target_substate_item else None
        default_target_uuid = self.default_target_substate_item.uuid if self.default_target_substate_item else None
        relative_pos = self.pos()
        return {
            'uuid': self.uuid,
            'type': self.history_type,
            'parent_superstate_name': parent_name,
            'parent_superstate_uuid': parent_uuid,
            'default_target_substate_name': default_target_name,
            'default_target_substate_uuid': default_target_uuid,
            'x_relative': relative_pos.x(),
            'y_relative': relative_pos.y(),
        }

    def set_properties(self, history_type: str, default_target_substate_item: GraphicsStateItem | None):
        changed = False
        if self.history_type != history_type:
            self.history_type = history_type
            self.text_label = "H" if history_type == "shallow" else "H*"
            self.setToolTip(f"{history_type.capitalize()} History Pseudo-State")
            changed = True
        if self.default_target_substate_item != default_target_substate_item:
            self.default_target_substate_item = default_target_substate_item
            self.update_default_history_transition()
            changed = True
        if changed: self.prepareGeometryChange(); self.update()
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.ItemPositionChange and self.scene():
            if self.parent_superstate_item:
                parent_rect_local_coords = self.parent_superstate_item.mapRectFromScene(self.parent_superstate_item.sceneBoundingRect())
                # new_rect_in_parent = self.boundingRect().translated(value) # Not needed for ellipse constraint
                margin = self.RADIUS + 2 # Keep H-item within parent bounds
                parent_bounds = parent_rect_local_coords

                constrained_x = max(parent_bounds.left() + margin, min(value.x(), parent_bounds.right() - self.RADIUS -2))
                constrained_y = max(parent_bounds.top() + margin, min(value.y(), parent_bounds.bottom() - self.RADIUS -2))
                
                if abs(value.x() - constrained_x) > 0.1 or abs(value.y() - constrained_y) > 0.1:
                    value = QPointF(constrained_x, constrained_y)
            
            if self.default_history_transition_item:
                 QTimer.singleShot(0, self.default_history_transition_item.update_path)

        elif change == QGraphicsItem.ItemParentHasChanged:
            if not self.parentItem() and self.scene():
                 logger.warning(f"History pseudo-state '{self.text_label}' lost its parent.")
                 self._cleanup_default_transition()

        elif change == QGraphicsItem.ItemSceneChange: 
            if value and self.scene(): 
                if self.parent_superstate_item and self.parent_superstate_item not in self.scene().items():
                    logger.warning(f"History item {self.text_label} added to scene, but its parent superstate {getattr(self.parent_superstate_item, 'text_label', 'None')} is not. Orphaned?")
                self.update_default_history_transition()
            elif not value: 
                self._cleanup_default_transition()
        return super().itemChange(change, value)
    ...
    def set_problematic_style(self, is_problematic: bool, problem_description: str = ""):
        ...
        self._is_problematic = is_problematic
        self._problem_tooltip_text = problem_description if is_problematic else ""
        self.setToolTip(self._problem_tooltip_text or f"{self.history_type.capitalize()} History Pseudo-State")
        self.update()
    
    def mouseDoubleClickEvent(self, event: QGraphicsSceneMouseEvent):
        if self.scene() and hasattr(self.scene(), 'edit_item_properties'):
            self.scene().edit_item_properties(self); event.accept()
        else: super().mouseDoubleClickEvent(event)

    def _cleanup_default_transition(self):
        if self.default_history_transition_item and self.default_history_transition_item.scene():
            self.scene().removeItem(self.default_history_transition_item)
            self.default_history_transition_item.deleteLater()
        self.default_history_transition_item = None
--- END OF FILE bsm_designer_project/graphics_items.py ---