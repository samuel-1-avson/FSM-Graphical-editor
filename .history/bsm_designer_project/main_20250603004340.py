...
class MainWindow(QMainWindow):
    ...
    def _handle_py_sim_global_ui_enable_by_manager(self, enable: bool):
        ...
        if hasattr(self, 'properties_edit_dialog_button'): # Changed from properties_edit_button
    ...
    def _clear_dock_editors(self):
        while self.properties_editor_layout.count():
            item = self.properties_editor_layout.takeAt(0)
            widget = item.widget()
            if widget:
                widget.deleteLater()
            label = item.label()
    ...
    def _update_properties_dock(self):
        self._clear_dock_editors() # Clear previous editors
        selected_items = self.scene.selectedItems()    
        if len(selected_items) == 1:
            item = selected_items[0]
            self._current_edited_item_in_dock = item # Store reference
            ...
            self.properties_edit_dialog_button.setEnabled(True) # Enable advanced edit

            if isinstance(item, GraphicsStateItem):
                self._populate_dock_for_state(props)
            elif isinstance(item, GraphicsTransitionItem):
                self._populate_dock_for_transition(props)
            elif isinstance(item, GraphicsCommentItem):
                self._populate_dock_for_comment(props)
            else:
                self.properties_placeholder_label.setText("<i>Selected item type not directly editable in dock.</i>")
            ...
            self._dock_editors_changed() # Set initial state of apply/revert buttons
        else:
            self.properties_placeholder_label.setVisible(True)
            self.properties_editor_container.setVisible(False)
            ...
            if len(selected_items) > 1:
                self.properties_placeholder_label.setText(f"<i><b>{len(selected_items)} items selected.</b><br>Select a single item to edit     properties.</i>")
            else:
                self.properties_placeholder_label.setText("<i>Select a single item to view/edit its properties.</i>")
    ...
    def _populate_dock_for_state(self, props: dict):
        ...
        # Color button - this one will still open a dialog for picking
        color_val = props.get('color', COLOR_ITEM_STATE_DEFAULT_BG)
        # Store color as QColor for easy comparison, but display hex
        self._dock_property_editors['_current_color_obj'] = QColor(color_val) 
    
        color_button = QPushButton(color_val)
        color_button.setStyleSheet(f"background-color: {color_val}; color: {COLOR_TEXT_ON_ACCENT if QColor(color_val).lightnessF() < 0.5 else   COLOR_TEXT_PRIMARY};")
    ...
    def _populate_dock_for_transition(self, props: dict):
        ...
        condition_edit.setPlaceholderText("e.g., var == 10")
    ...
    def _populate_dock_for_comment(self, props: dict):
        # For comments, direct editing in the scene is usually better.
    # But we can show the text here and allow advanced edit.
        text_label = QLabel(html.escape(props.get('text', '')[:100] + ('...' if len(props.get('text', '')) > 100 else '')))
        text_label.setWordWrap(True)
        self.properties_editor_layout.addRow("Text:", text_label)
        self._dock_property_editors['text_display'] = text_label # Not editable here
    ...
    def _pick_dock_color(self, color_button_ref: QPushButton):
        if self._current_edited_item_in_dock and '_current_color_obj' in self._dock_property_editors:
            ...
            if new_color.isValid() and new_color != initial_color:
                ...
                color_button_ref.setStyleSheet(f"background-color: {new_color.name()}; color: {COLOR_TEXT_ON_ACCENT if new_color.lightnessF() < 0.5 else COLOR_TEXT_PRIMARY};")
            self._dock_editors_changed() # Signal that a change occurred    
    ...
    # This slot is connected to textChanged/stateChanged of dock editors
    # It enables the Apply/Revert buttons if there's a change from original.
        ...
        if 'name' in self._dock_property_editors and isinstance(self._current_edited_item_in_dock, GraphicsStateItem):
            if self._dock_property_editors['name'].text() != original_props.get('name'):
                changed = True
        if 'is_initial' in self._dock_property_editors and isinstance(self._current_edited_item_in_dock, GraphicsStateItem):
            if self._dock_property_editors['is_initial'].isChecked() != original_props.get('is_initial'):
                changed = True
        if 'is_final' in self._dock_property_editors and isinstance(self._current_edited_item_in_dock, GraphicsStateItem):
            if self._dock_property_editors['is_final'].isChecked() != original_props.get('is_final'):
                changed = True
    
        if 'event' in self._dock_property_editors and isinstance(self._current_edited_item_in_dock, GraphicsTransitionItem):
            if self._dock_property_editors['event'].text() != original_props.get('event'):
                changed = True
        if 'condition' in self._dock_property_editors and isinstance(self._current_edited_item_in_dock, GraphicsTransitionItem):
            if self._dock_property_editors['condition'].text() != original_props.get('condition'):
                changed = True

        if '_current_color_obj' in self._dock_property_editors:
    ...
    def _on_apply_dock_properties(self):
        ...
        new_props_from_dock = old_props.copy() # Start with old, update relevant fields
        ...
        if isinstance(self._current_edited_item_in_dock, GraphicsStateItem):
            ...
            if 'name' in self._dock_property_editors:
                ...
                if not new_name:
                    ...
                    self._populate_dock_for_state(old_props) # Revert editor
                    return
        ...
        self.properties_apply_button.setEnabled(False)
        self.properties_revert_button.setEnabled(False)
    # The _update_properties_dock will be called due to selection change or command execution,
    # which will refresh the dock with the new (now "original") properties.


    @pyqtSlot()
    def _on_revert_dock_properties(self):
        if self._current_edited_item_in_dock:
        # Force a refresh of the dock from the item's current (unmodified) data
            self._update_properties_dock() 
            self.properties_apply_button.setEnabled(False)
            self.properties_revert_button.setEnabled(False)
    ...
    def _on_edit_selected_item_properties_from_dock_button(self):
        # This is the old "Edit Properties..." button, now repurposed as "Advanced Edit..."
        if self._current_edited_item_in_dock:
            self.scene.edit_item_properties(self._current_edited_item_in_dock)
         # After modal dialog closes, refresh dock display
            self._update_properties_dock()  
    
    def _log_to_parent(self, level, message): # Helper
