...
class ResourceMonitorWorker(QObject):
    ...
    finished_signal = pyqtSignal() # This signal is crucial
    ...
    def __init__(self, interval_ms=2000, parent=None):
        ...
        self._stop_requested_event = threading.Event() # Use a threading Event
    ...
    def _attempt_nvml_init(self, from_worker_loop=False):
        if self._nvml_initialized: return 
    ...
    def start_monitoring(self):
        ...
        self._stop_requested_event.clear() # Ensure event is clear on start
    ...
    def stop_monitoring(self):
        ...
        self._monitoring = False
        self._stop_requested_event.set() # Signal the loop to stop
        # NVML shutdown is now handled by _shutdown_nvml called from MainWindow
    ...
    def _monitor_resources(self):
        ...
        while self._monitoring:
            if self._stop_requested_event.is_set(): 
                logger.debug("ResourceMonitorWorker: Stop requested event detected at loop start, breaking.")
                break

            # --- NVML Init/Handle Acquisition ---
            ...
            if PYNVML_AVAILABLE and pynvml and self._nvml_initialized and not self._gpu_handle:
                try:
                    ...
                    if hasattr(pynvml, 'NVML_ERROR_UNINITIALIZED') and e_nvml_poll.value == pynvml.NVML_ERROR_UNINITIALIZED:
                        self._nvml_initialized = False 
                except Exception as e_poll_outer: 
            ...
            # --- Data Collection ---
            try:
                if not self._monitoring: break # Check before potentially blocking calls
                ...
                if self._stop_requested_event.is_set(): break
                if self._monitoring: # Double check before emitting
                 ...
                 if self._monitoring: self.resourceUpdate.emit(-1.0, -1.0, -3.0, "Data Error")


            # --- Wait with interruption capability ---
            if self._stop_requested_event.wait(timeout=self.interval_ms / 1000.0): 
                logger.debug("ResourceMonitorWorker: Stop event was set during wait, breaking loop.")
                break 
            
            if not self._monitoring: # Final check after wait, if _monitoring was set directly
                logger.debug("ResourceMonitorWorker: _monitoring flag became false after wait, breaking loop.")
                break
...
class MainWindow(QMainWindow):
    ...
    def _init_resource_monitor(self):
        ...
        # self.resource_monitor_worker.finished_signal.connect(self.resource_monitor_worker.deleteLater) # Let thread manage worker lifetime
        # self.resource_monitor_thread.finished.connect(self.resource_monitor_thread.deleteLater) # Let main manage thread lifetime
    ...
    def _update_window_title(self):
        ...
        main_window_is_dirty = self.scene.is_dirty() or self.ide_editor_is_dirty # Correctly combine dirty states
        self.setWindowModified(main_window_is_dirty) # Set main window modified flag
        ...
        if hasattr(self, 'status_label'):
            main_file_status = f"File: {file_name}{' *' if self.scene.is_dirty() else ''}" # Diagram dirty
    ...
    def _update_save_actions_enable_state(self):
        self.save_action.setEnabled(self.scene.is_dirty()) # Diagram save action only depends on diagram dirty status
    ...
    def _prompt_save_if_dirty(self) -> bool:
        # This prompt is now specifically for the diagram. IDE save is separate.
        if not self.scene.is_dirty(): # Check diagram's dirty state
            return True
        ...
        if reply == QMessageBox.Save:
            return self.on_save_file() # This saves the diagram
    ...
    def on_new_file(self, silent=False):
        if not silent:
            ...
            if not self._prompt_ide_save_if_dirty(): return False
        ...
        self.setWindowModified(self.ide_editor_is_dirty) # If IDE was dirty, main window remains modified
        self._update_window_title()
        self._update_undo_redo_actions_enable_state()
        self._update_save_actions_enable_state() # Update save action based on new diagram state
        if not silent:
    ...
    def on_save_file(self) -> bool:
        ...
        # This action specifically saves the diagram.
        # IDE save is handled by on_ide_save_file.
        if self.scene.is_dirty():
             return self._save_to_path(self.current_file_path)
        return True # Nothing in diagram to save, so "success"
    ...
    def _save_to_path(self, file_path) -> bool:
        ...
        try:
            ...
            self.scene.set_dirty(False) 
            self.setWindowModified(self.ide_editor_is_dirty) # Update main window modified based on IDE state
            self._update_window_title()
            self._update_save_actions_enable_state() # Reflect that diagram is now clean
            return True
        except Exception as e:
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
            ...
            if not self.resource_monitor_thread.wait(3000): 
                ...
                self.resource_monitor_thread.wait(500) 
            else:
                logger.info("MW_CLOSE: Resource monitor thread stopped gracefully.")
            
            if self.resource_monitor_worker: # Shutdown NVML after thread handling
