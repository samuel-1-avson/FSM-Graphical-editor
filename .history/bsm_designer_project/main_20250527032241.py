...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event: QCloseEvent): 
        # Prompt to save all dirty IDE tabs before closing the application
        if self.ide_tab_widget:
            for i in range(self.ide_tab_widget.count() -1, -1, -1): # Iterate backwards to handle removals
                if not self._prompt_ide_save_if_dirty(i):
                    event.ignore()
                    return
    ...
    def _save_specific_ide_tab(self, index: int) -> bool:
        ...
        if not file_to_save_path: 
            ...
            if '.' in current_tab_text and not current_tab_text.endswith(default_ext): 
                 base_name_for_save = current_tab_text.rsplit('.',1)[0] + default_ext
            elif '.' not in current_tab_text:
    ...
    def on_ide_save_as_file(self) -> bool:
        ...
        if '.' in default_filename_base:
            name_part, _ = default_filename_base.rsplit('.',1) 
    ...
    def _update_ide_related_actions(self):
        ...
        if self.ide_analyze_action:
            self.ide_analyze_action.setEnabled(bool(can_analyze_with_ai and has_text_in_ide)) # Cast to bool
        ...
        if self.ide_generate_code_action:
             ...
             self.ide_generate_code_action.setEnabled(bool(can_ide_generate_code)) # Cast to bool
    ...
    def on_ide_save_output(self):
        active_tab_data = self._get_active_ide_tab_data()
        if not active_tab_data or not active_tab_data.get("output_console") or \
    ...
    def on_ide_save_all_files(self):
        ...
        for i in indices_to_save:
            ...
            if hasattr(widget, "_ide_tab_data"):
                tab_data = widget._ide_tab_data
                if tab_data["is_dirty"]:
                    # No need to change current index for saving, _save_specific_ide_tab handles it.
        ...
        if original_active_index < self.ide_tab_widget.count(): # Restore original active tab if still valid
        ...
        if index == -1 and self.ide_tab_widget and self.ide_tab_widget.count() == 0 : 
            ...
            if active_tab_data and self.ide_language_combo:
                ...
                if active_tab_data.get("editor"): 
                    active_tab_data["editor"].setFocus() # Ensure editor in new tab gets focus
            ...
            if hasattr(self, '_find_replace_dialog_instance') and self._find_replace_dialog_instance and self._find_replace_dialog_instance.isVisible():
                if active_tab_data and active_tab_data.get("editor"):
                    ...
                    self._find_replace_dialog_instance.editor = None 
        else: 
             self._update_window_title()
             self._update_ide_related_actions()
             if self.ide_language_combo: 
