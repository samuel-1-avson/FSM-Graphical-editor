...
from dialogs import (MatlabSettingsDialog, FindReplaceDialog) 
from config import (
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ide_save_all_action: QAction | None = None
        self.ide_analyze_action: QAction | None = None
        self.ide_generate_code_action: QAction | None = None
    ...
    def _create_actions(self): 
        ...
        self.ide_save_all_action = QAction(get_standard_icon(QStyle.SP_DialogSaveButton, "IDESvAll"), "Save All Scripts", self, triggered=self.on_ide_save_all_files)
        self.ide_generate_code_action = QAction(get_standard_icon(QStyle.SP_ArrowRight, "IDEGen"), "Generate Code with AI...", self, triggered=self.on_ide_generate_code_with_ai)
    ...
    def _create_menus(self): 
        ...
        ide_menu.addAction(self.ide_save_all_action) 
        ...
        ide_menu.addAction(self.ide_generate_code_action)
    ...
    def _setup_ide_dock_widget(self):
        ...
        ide_toolbar.addAction(self.ide_save_all_action)
        ...
        ide_toolbar.addSeparator()
        ide_toolbar.addAction(self.ide_generate_code_action)
        ...
        if self.ide_tab_widget.count() == 0: 
             ...
             self._on_ide_current_tab_changed(self.ide_tab_widget.currentIndex())
    ...
    def _update_window_title(self): 
        ...
        ide_suffix = ""
        active_ide_tab_data = self._get_active_ide_tab_data() 
        if active_ide_tab_data and active_ide_tab_data.get("path"):
            ide_file_name = os.path.basename(active_ide_tab_data["path"])
            ide_dirty_char = "*" if active_ide_tab_data.get("is_dirty", False) else ""
    ...
    def _update_ide_save_actions_enable_state(self):
        ...
        has_content = False
        if active_tab_data and active_tab_data.get("editor"): # Check if 'editor' key exists
            has_content = bool(active_tab_data["editor"].toPlainText())
    ...
    def _update_ai_features_enabled_state(self, is_online_and_key_present: bool):
        ...
        if hasattr(self, 'ai_chat_ui_manager') and self.ai_chat_ui_manager:
            ...
            if self.ai_chat_ui_manager.ai_chat_input:
                ...
                if not (self.ai_chatbot_manager and self.ai_chatbot_manager.api_key): 
    ...
    def _get_active_ide_tab_data(self) -> dict | None:
        ...
        # editor.set_language(language) # This will be set by _on_ide_current_tab_changed
        ...
        output_console.setFixedHeight(100) # Or whatever height you prefer

        # Create the main widget for the tab
        tab_content_widget = QWidget()
        tab_layout = QVBoxLayout(tab_content_widget)
        tab_layout.setContentsMargins(2, 2, 2, 2) # Small margins for content within the tab
        tab_layout.setSpacing(2)
        tab_layout.addWidget(editor)        # Add editor to this tab's layout
        tab_layout.addWidget(output_console) # Add console to this tab's layout
        # --- End create widgets for THIS tab ---

        # Store metadata with the tab_content_widget itself
        tab_content_widget._ide_tab_data = {
            "editor": editor,
            "output_console": output_console,
            "path": file_path,
            "is_dirty": False if file_path and not content else (True if not file_path else False), 
            "language": language
        }
        ...
        # Focus will be set by _on_ide_current_tab_changed if needed, or explicitly after
        editor.setFocus() 
    ...
    def on_ide_new_file(self):
        self._add_ide_tab() 
        logger.info(f"IDE: New script tab created.")
    ...
    def on_ide_open_file(self):
        ...
        if active_tab_data and not active_tab_data["path"] and \
           (not active_tab_data.get("editor") or not active_tab_data["editor"].toPlainText().strip()) and \
        ...
        if file_path:
            for i in range(self.ide_tab_widget.count()):
                # Ensure widget(i) is not None before accessing _ide_tab_data
                widget = self.ide_tab_widget.widget(i)
                if widget and hasattr(widget, "_ide_tab_data"):
                    tab_data_iter = widget._ide_tab_data
                    if tab_data_iter["path"] == file_path:
            ...
            if should_use_active_tab_for_open and active_tab_idx != -1:
                ...
                self.ide_tab_widget.setTabText(active_tab_idx, new_tab_title) 
            else:
            ...
            try:
                ...
                if self.ide_language_combo.currentText() != lang_to_set: 
                    ...
                    if current_tab_data.get("editor"):
    ...
    def _save_specific_ide_tab(self, index: int) -> bool:
        ...
        if not file_to_save_path: 
            ...
            default_ext = ".txt"
            if lang == "Python": default_ext = ".py"
            elif "Arduino" in lang: default_ext = ".ino"
            elif "C/C++" in lang: default_ext = ".cpp"
            
            base_name_for_save = current_tab_text
            if '.' in base_name_for_save and not base_name_for_save.endswith(default_ext):
                 base_name_for_save = base_name_for_save.rsplit('.',1)[0] + default_ext
            elif '.' not in base_name_for_save:
                 base_name_for_save += default_ext
            default_filename = base_name_for_save
        ...
        try:
            ...
            self.ide_tab_widget.setTabText(index, os.path.basename(file_to_save_path)) 
            self._update_ide_tab_title(index) 
    ...
    def on_ide_save_as_file(self) -> bool:
        ...
        default_filename_base = os.path.basename(active_tab_data["path"] or current_tab_text)
        
        # Ensure correct extension based on language, preferring existing name part if possible
        name_part = default_filename_base
        if '.' in default_filename_base:
            name_part = default_filename_base.rsplit('.',1)[0]
        default_filename = name_part + default_ext
        ...
        if file_path:
            ...
            ext = os.path.splitext(file_path)[1].lower()
            new_lang = "Python" # Default
            if ext == ".py": new_lang = "Python"
            elif ext == ".ino": new_lang = "C/C++ (Arduino)"
            elif ext in [".c", ".cpp", ".h"]: new_lang = "C/C++ (Generic)"
            else: new_lang = "Text" # Fallback for other extensions
    ...
    def _update_ide_related_actions(self):
        ...
        key_present_for_ide = self.ai_chatbot_manager is not None and bool(self.ai_chatbot_manager.api_key)
        ai_ready_for_ide = self._internet_connected and key_present_for_ide

        can_analyze_with_ai = (selected_language == "Python" or selected_language.startswith("C/C++")) and \
                              ai_ready_for_ide
        
        any_tab_is_dirty = False
        if self.ide_tab_widget:
            for i in range(self.ide_tab_widget.count()):
                widget = self.ide_tab_widget.widget(i)
                if widget and hasattr(widget, "_ide_tab_data") and widget._ide_tab_data.get("is_dirty", False):
                    any_tab_is_dirty = True
                    break
        ...
        if self.ide_analyze_action:
            self.ide_analyze_action.setEnabled(bool(can_analyze_with_ai and has_text_in_ide))
            tooltip = "Analyze the current code with AI"
            if not has_text_in_ide:
                tooltip += " (No code in editor)"
            elif not key_present_for_ide:
        ...
        if self.ide_save_all_action:
            self.ide_save_all_action.setEnabled(any_tab_is_dirty) 
        ...
        if self.ide_generate_code_action:
            can_ide_generate_code = ai_ready_for_ide and \
                self.ide_tab_widget is not None and self.ide_tab_widget.count() > 0
            self.ide_generate_code_action.setEnabled(bool(can_ide_generate_code))
            self.ide_generate_code_action.setToolTip(
                "Generate code snippet using AI" if can_ide_generate_code else "Requires active IDE tab, Internet & API Key"
            )
    ...
    @pyqtSlot()
    def on_ide_generate_code_with_ai(self):
        active_tab_data = self._get_active_ide_tab_data()
        if not active_tab_data or not active_tab_data.get("editor") or not active_tab_data.get("output_console"):
            logger.error("IDE: No active editor/console for AI code generation.")
            QMessageBox.warning(self, "IDE Error", "No active script tab found to generate code into.")
            return

        if not self.ai_chatbot_manager or not self.ai_chatbot_manager.api_key:
            QMessageBox.warning(self, "AI Assistant Not Ready", "Please configure your OpenAI API key in AI Assistant Settings.")
            return
        if not self._internet_connected:
            QMessageBox.warning(self, "Offline", "Internet connection is required for AI code generation.")
            return

        description, ok = QInputDialog.getMultiLineText(self, "Generate Code with AI",
                                                        "Describe the code snippet you need (e.g., 'Arduino function to blink an LED 5 times', 'Python class for a simple counter'):",
                                                        "")
        if not ok or not description.strip():
            return

        selected_language = active_tab_data["language"]
        language_context = f"for {selected_language}"
        if "Arduino" in selected_language: language_context = "for Arduino (provide complete .ino sketch if possible, or a function)"
        elif "C/C++" in selected_language: language_context = "for generic C/C++"

        prompt = f"Please generate a {selected_language} code snippet {language_context} for the following task: {description}. IMPORTANT: Respond with ONLY the code block itself. Do not include any surrounding explanations or markdown like ``` before or after the code, unless the code itself requires comments for clarity (e.g. // or #). The response should be directly usable as code."
        
        if active_tab_data.get("output_console"):
            active_tab_data["output_console"].append(f"<i style='color:grey;'>Requesting AI to generate {selected_language} code for: '{description[:50]}...' (Response will appear in main AI Chat window. Copy code from there.)</i><hr>")
        self.ai_chat_ui_manager._append_to_chat_display("IDE", f"Requesting AI to generate {selected_language} code for: {description}")
        self.ai_chatbot_manager.send_message(prompt)
    ...
    def on_ide_save_all_files(self):
        if not self.ide_tab_widget: return
        saved_count = 0
        cancelled_by_user = False
        
        indices_to_save = list(range(self.ide_tab_widget.count()))

        for i in indices_to_save:
            if i >= self.ide_tab_widget.count(): 
                continue
            ...
            if hasattr(widget, "_ide_tab_data"):
                tab_data = widget._ide_tab_data
                if tab_data["is_dirty"]:
                    # Make the tab to be saved active for the dialog context, then restore
                    original_active_index = self.ide_tab_widget.currentIndex()
                    if i != original_active_index:
                        self.ide_tab_widget.setCurrentIndex(i)
                    
                    if not self._save_specific_ide_tab(i): 
                        reply = QMessageBox.question(self, "Save All Cancelled",
                                                     f"Saving tab '{self.ide_tab_widget.tabText(i).replace('*','')}' was cancelled. Continue saving other modified scripts?",
                                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
                        if reply == QMessageBox.No:
                            cancelled_by_user = True
                            if original_active_index < self.ide_tab_widget.count():
                                self.ide_tab_widget.setCurrentIndex(original_active_index) 
                            break 
                    else: 
                        saved_count +=1
                    
                    if original_active_index < self.ide_tab_widget.count() and \
                       original_active_index != self.ide_tab_widget.currentIndex(): # Restore if it changed and still valid
                         self.ide_tab_widget.setCurrentIndex(original_active_index) 

        status_msg = f"IDE: Save All complete. Saved {saved_count} tabs."
        if cancelled_by_user:
            status_msg += " Process was cancelled by user."
        logger.info(status_msg)
        if hasattr(self, 'status_label'): self.status_label.setText(status_msg)
        self._update_ide_related_actions() 
    ...
    def _on_ide_current_tab_changed(self, index):
        if index == -1 and self.ide_tab_widget and self.ide_tab_widget.count() == 0 : 
             self._update_window_title()
             self._update_ide_related_actions()
             if self.ide_language_combo: # Ensure combo is reset if no tabs
                 self.ide_language_combo.setCurrentText("Python")
             return
        elif index != -1: 
            self._update_window_title()
            self._update_ide_related_actions()
            ...
            # Update Find/Replace dialog's editor reference if it's open
            if hasattr(self, '_find_replace_dialog_instance') and self._find_replace_dialog_instance and self._find_replace_dialog_instance.isVisible():
                if active_tab_data and active_tab_data.get("editor"):
                    self._find_replace_dialog_instance.editor = active_tab_data.get("editor")
                else: # No active editor, perhaps disable find/replace dialog or handle gracefully
                    self._find_replace_dialog_instance.editor = None # Or close it
        else: 
             self._update_window_title()
             self._update_ide_related_actions()
             if self.ide_language_combo: # Ensure combo is reset if no tabs (e.g. last tab closed)
                 self.ide_language_combo.setCurrentText("Python")
