...
from PyQt5.QtCore import Qt, QRectF, QPointF, QLineF, pyqtSignal, QPoint, QMarginsF, QEvent, QMimeData, QTimer, pyqtSlot # <--- CORRECTED IMPORT
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if isinstance(item, GraphicsHistoryPseudoStateItem) and 'default_target_substate_name' in new_props_from_dialog:
                ...
                if target_name:
                    ...
                    search_context = self # Default to current scene
                    if isinstance(self.parent_window, QDialog) and self.parent_window.windowTitle().startswith("Sub-Machine Editor:"):
                        # If we are in the sub-editor, 'self' IS the sub-scene.
                        pass # target_state_item will be searched in self (current scene)
                    elif item.parent_superstate_item:
                        # If in main scene, and H-item's parent has sub_fsm_data,
                        # this implies target should be a sub-state.
                        # This logic might be complex if sub-states are not directly in main scene.
                        # For now, get_state_by_name searches the current scene.
                        # A more robust solution would involve looking up sub-states within the parent's sub_fsm_data
                        # if sub-states are not flat in the main scene.
                        # However, if sub-editor instantiates sub-states in ITS OWN scene, then this should work.
                         pass


                    # Find state by name. If sub-editor, it's in the sub-scene.
                    # If main editor, and H-item default target is a sub-state, it's *not* in the main scene.
                    # The dialog should populate based on parent's sub_fsm_data names.
                    # The command needs UUID of that sub-state.
                    # This assumes that if a name is returned, we find its instance (or placeholder data)
                    # in the parent's sub_fsm_data and get its UUID.
                    # This logic is tricky if sub_fsm_data only stores names and not UUIDs for its sub-states.
                    # Assuming sub_fsm_data in GraphicsStateItem *does* store UUIDs for its sub-states for robustness.
                    ...
                    if item.parent_superstate_item and item.parent_superstate_item.sub_fsm_data:
                        ...
                        if found_sub_state_data and found_sub_state_data.get('uuid'):
                            ...
                            logger.warning(f"Could not find sub-state named '{target_name}' with UUID in parent '{item.parent_superstate_item.text_label}' for H-item {item.uuid}.")
                    else: # Fallback if parent has no sub_fsm_data (error) or target_name is for a top-level state (unlikely for H-item default)
                        target_state_item = self.get_state_by_name(target_name)
                        if target_state_item:
                            new_props_from_dialog['default_target_substate_uuid'] = target_state_item.uuid
                        else:
                            new_props_from_dialog['default_target_substate_uuid'] = None
