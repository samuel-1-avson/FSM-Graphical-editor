            ...
            if (current_loop_time - last_data_emit_time) >= self.data_collection_interval_ms or last_data_emit_time == 0:
                ...
                try:
                    ...
                    last_data_emit_time = current_loop_time
                     ...
                     if not self._stop_requested: self.resourceUpdate.emit(-1.0, -1.0, -3.0, "Data Error")
            
            # Use QThread.msleep. The loop condition `while not self._stop_requested` is the primary exit.
            # This msleep simply yields if the data_collection_interval hasn't been met yet.
            QThread.msleep(self.WORKER_LOOP_CHECK_INTERVAL_MS)
            # No need to break here based on msleep, the while condition handles it.
        
        logger.info("Resource monitor worker loop finished (stop_requested=%s).", self._stop_requested)
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
            ...
            if self.resource_monitor_worker:
                ...
                # Using QueuedConnection is generally safer for cross-thread slot calls
                # if the slot might do more than just set a flag.
                # For just setting a flag, direct call worker.stop_monitoring() is also often fine.
