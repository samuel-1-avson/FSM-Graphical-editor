...
class ChatbotWorker(QObject):
    ...
    @pyqtSlot(str)
    def set_api_key(self, api_key: str): # Ensure api_key is typed if using Q_ARG, but not needed with lambda
    ...
    def process_message(self, user_message: str):
        ...
        # print(f"--- WORKER_PROCESS ({current_time}): Diagram context length: {len(self.current_diagram_context_json_str) if self.current_diagram_context_json_str else 'None'} ---")
        ...
        try:
            ...
            # Ensure status always reverts unless a more specific error status was set by error handlers.
            # The error handlers might set "Status: API Key Required" or similar.
            # If no error, this sets it back to "Ready".
            # Consider if this is always the desired behavior vs. letting error status persist.
            if not any(s in self.statusUpdate.signal for s in ["API Key required", "Error"]): # Rough check
