...
from config import (
    ...
    COLOR_BACKGROUND_LIGHT, COLOR_ACCENT_PRIMARY_LIGHT, COLOR_BORDER_MEDIUM # Added COLOR_BORDER_MEDIUM
)
from code_editor import CodeEditor # Ensure CodeEditor is imported
from utils import get_standard_icon
from matlab_integration import MatlabConnection
# Removed duplicate graphics_items import
...
class SubFSMEditorDialog(QDialog):
    ...
    def __init__(self, sub_fsm_data_initial: dict, parent_state_name: str, parent_window_ref=None):
        ...
        if IMPORTS_SUCCESSFUL: 
            ...
            self.json_text_edit = CodeEditor() # Use your CodeEditor
            self.json_text_edit.setPlainText(json.dumps(self.current_sub_fsm_data, indent=2, ensure_ascii=False))
            self.json_text_edit.setLineWrapMode(CodeEditor.NoWrap) # Or CodeEditor.WidgetWidth
            try:
                self.json_text_edit.set_language("JSON") # Assuming CodeEditor supports "JSON"
            except Exception: # Fallback if JSON highlighting is not available
            ...
            self.json_text_edit.setObjectName("SubFSMJsonEditor") # For specific styling if needed
...
class StatePropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_state=False, scene_ref=None):
        ...
        self.setStyleSheet(f"QDialog {{ background-color: {COLOR_BACKGROUND_DIALOG}; }} QLabel#SafetyNote, QLabel#HardwareHintLabel {{ font-size: {APP_FONT_SIZE_SMALL}; color: {COLOR_TEXT_SECONDARY}; }} QGroupBox {{ background-color: {QColor(COLOR_BACKGROUND_LIGHT).lighter(102).name()}; }}") # Added HardwareHintLabel
        ...
        # --- MODIFIED: Use add_field_with_note_and_hw_hint ---
    ...
    # ... (rest of StatePropertiesDialog methods: _on_action_language_changed, _create_insert_snippet_button, _update_snippet_button_menu, _on_superstate_toggled, _on_edit_sub_fsm, _choose_color, _update_color_button_style, get_properties - remain the same)
...
class TransitionPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_transition=False):
        ...
        def add_lineedit_with_snippet(form_layout, label_text, edit_widget, snippet_button, is_code_field=True):
            ...
            if is_code_field: v_btn_container.addStretch() # Only add stretch if it's a code field without safety note
            h_editor_btn_layout.addLayout(v_btn_container)

            field_v_layout = QVBoxLayout(); field_v_layout.setSpacing(3) # Container for editor+button and safety note
            field_v_layout.addLayout(h_editor_btn_layout)
            if is_code_field: # Conditions are code-like and might need safety note
        ...
        # --- MODIFIED: Use add_codeeditor_with_snippet_and_hw_hint ---
    ...
    # ... (rest of TransitionPropertiesDialog methods: _on_action_language_changed, _create_insert_snippet_buttons, _update_snippet_button_menu, _choose_color, _update_color_button_style, get_properties - remain the same)
    def _on_action_language_changed(self, language_mode: str):
...
class CommentPropertiesDialog(QDialog):
    # ... (CommentPropertiesDialog code remains the same) ...
    def __init__(self, parent=None, current_properties=None):
        ...
        self.text_edit.setFontPointSize(10) # Example: Set a slightly larger font for comments
...
class MatlabSettingsDialog(QDialog):
    # ... (MatlabSettingsDialog code remains the same) ...
...
class FindItemDialog(QDialog):
    # ... (FindItemDialog code remains the same) ...
