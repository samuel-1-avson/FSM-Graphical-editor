...
from PyQt5.QtGui import QMovie, QIcon # Add QMovie for potential animation
...
class ChatbotWorker(QObject):
    ...
    def set_api_key_slot(self, api_key: str):
        ...
        if not self.api_key:
            ...
            self.errorOccurred.emit("Failed to initialize Gemini client with the new API key.")
    ...
    def process_message_slot(self, user_message: str, force_fsm_generation: bool):
        ...
        if not self.client:
            ...
            logger.warning("process_message: %s", error_msg)
            self.errorOccurred.emit(error_msg)
            self.statusUpdate.emit(AIStatus.API_KEY_ERROR, "Status: API Client Error.") # Or a more general ERROR
            self._current_processing_had_error = True
            return
...
class AIChatUIManager(QObject):
    ...
    def create_dock_widget_contents(self) -> QWidget:
        ...
        # Placeholder for spinner GIF loading - currently commented out
    ...
    def update_status_display(self, status_enum: AIStatus, status_text: str):
        ...
        if status_enum == AIStatus.API_KEY_REQUIRED or status_enum == AIStatus.API_KEY_ERROR or status_enum == AIStatus.INACTIVE:
            ...
            # If history cleared, AI might be ready or API key required, etc. Rely on next status.
            # For now, assume it might still be ready if it was before clearing.
            # This might need another status update from manager after clear if key is missing.
        ...
        is_thinking = (status_enum == AIStatus.THINKING or status_enum == AIStatus.INITIALIZING)
    ...
    # ... (rest of AIChatUIManager methods like _format_code_block, _append_to_chat_display, etc.) ...
...
class AIChatbotManager(QObject):
        ...
        if was_fsm_generation_attempt:
            try:
                ...
                if isinstance(fsm_data, dict) and ('states' in fsm_data or 'transitions' in fsm_data):
                    ...
                    # Status should be Ready now, handled by worker's finally block or here explicitly if needed
                    # self._update_current_ai_status(AIStatus.READY, "Status: Ready.") 
                    return 
                else:
                    ...
                    # self._update_current_ai_status(AIStatus.ERROR, "Status: Invalid FSM JSON.")
    ...
    def clear_conversation_history(self):
        ...
        if self.chatbot_worker and self.chatbot_thread and self.chatbot_thread.isRunning():
            ...
            # Worker will emit HISTORY_CLEARED status
            logger.debug("MGR: clear_history invoked on worker.")
        else:
            # self._update_current_ai_status(AIStatus.INACTIVE, "Status: Chatbot not active (history is in worker).")
