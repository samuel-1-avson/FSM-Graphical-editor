...
class MainWindow(QMainWindow):
    ...
    def _create_docks(self):
        ...
        drag_state_btn.setToolTip("Drag to add a new State onto the canvas.")
        ...
        drag_initial_state_btn.setToolTip("Drag to add a new Initial State onto the canvas.")
        ...
        drag_final_state_btn.setToolTip("Drag to add a new Final State onto the canvas.")
        ...
        drag_comment_btn.setToolTip("Drag to add a new Comment onto the canvas.")
    ...
    def _update_properties_dock(self):
        ...
        if len(selected_items) == 1:
            ...
            th_style = f"padding: 6px 8px; text-align:left; font-weight:bold; color:{COLOR_TEXT_ON_ACCENT}; background-color:{COLOR_ACCENT_PRIMARY}; border-bottom:1px solid {QColor(COLOR_ACCENT_PRIMARY).darker(120).name()};"
            ...
            if isinstance(item, GraphicsStateItem):
                ...
                rows += f"<tr><th colspan='2' style='{th_style}'>Identification & Hierarchy</th></tr>"
                ...
                if props.get('is_superstate'):
                    ...
                    sub_trans_count = len(props.get('sub_fsm_data',{}).get('transitions',[]))
                    rows += f"<tr><td style='{td_key_style}'>Superstate:</td><td style='{td_val_style}'><span style='color:{COLOR_ACCENT_PRIMARY}; font-weight:bold;'>Yes</span> <span style='{small_font_size_css}'>(Sub: {sub_states_count} states, {sub_trans_count} transitions)</span></td></tr>"

                rows += f"<tr><th colspan='2' style='{th_style}'>Appearance</th></tr>"
                ...
                rows += f"<tr><th colspan='2' style='{th_style}'>Actions ({html.escape(props.get('action_language','N/A'))})</th></tr>"
                ...
                if props.get('description'):
                    rows += f"<tr><th colspan='2' style='{th_style}'>Description</th></tr>"
                    rows += f"<tr><td colspan='2' style='{td_val_style}white-space:normal;{small_font_size_css}'>{html.escape(props.get('description'))}</td></tr>"
                ...
                if props.get('condition'): label_parts.append(f"<span style='font-family:Consolas,monospace; color:{COLOR_TEXT_SECONDARY};'>[{html.escape(props.get('condition'))}]</span>")
                action_code = props.get('action')
                if action_code:
                    action_display = fmt(action_code, 20, is_code=True)
                    label_parts.append(f"<span style='font-family:Consolas,monospace;color:{QColor(COLOR_ACCENT_SECONDARY).darker(110).name()};'>/{{{action_display}}}</span>")

                full_label = " ".join(p for p in label_parts if p) or f"<i style='color:{COLOR_TEXT_SECONDARY}; {small_font_size_css}'>(No Label)</i>"
                
                rows += f"<tr><th colspan='2' style='{th_style}'>Identification & Logic</th></tr>"
                ...
                rows += f"<tr><th colspan='2' style='{th_style}'>Action ({html.escape(props.get('action_language','N/A'))})</th></tr>"
                rows += f"<tr><td colspan='2' style='{td_val_style}'>{fmt(props.get('action', ''), 40, is_code=True)}</td></tr>"
                ...
                rows += f"<tr><th colspan='2' style='{th_style}'>Content</th></tr>"
                rows += f"<tr><td colspan='2' style='{td_val_style}white-space:normal; font-style:italic; color:{COLOR_TEXT_SECONDARY};'>{html.escape(props.get('text', ''))}</td></tr>"
