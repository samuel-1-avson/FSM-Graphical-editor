...
class ResourceMonitorWorker(QObject):
    ...
    def __init__(self, interval_ms=2000, parent=None):
        ...
        if PYNVML_AVAILABLE and pynvml: # Check if pynvml was successfully imported
            try:
                pynvml.nvmlInit()
                self._nvml_initialized = True
                # Try to get GPU name early if possible, otherwise on first monitor call
            ...
            except pynvml.NVMLError as e_nvml: # Catch specific NVML errors
                logger.warning(f"Could not initialize NVML (for NVIDIA GPU monitoring): {e_nvml}")
                self._nvml_initialized = False
                self._gpu_name_cache = f"NVIDIA NVML Err ({type(e_nvml).__name__})"
    ...
    def stop_monitoring(self):
        self._monitoring = False
        if self._nvml_initialized and PYNVML_AVAILABLE and pynvml: # Check again before shutdown
            try:
                pynvml.nvmlShutdown()
                logger.info("NVML shutdown successfully.")
            except Exception as e:
                logger.warning(f"Error shutting down NVML: {e}")
        self._nvml_initialized = False # Mark as not initialized after stopping
        self._gpu_handle = None


    def _monitor_resources(self):
        """The core monitoring loop. This will run in the QThread."""
        logger.debug("Resource monitor worker loop started.")
        while self._monitoring:
            # Ensure NVML is re-checked if it wasn't init'd or handle was lost
            if PYNVML_AVAILABLE and pynvml and self._nvml_initialized and not self._gpu_handle:
                try:
                    if pynvml.nvmlDeviceGetCount() > 0:
                        self._gpu_handle = pynvml.nvmlDeviceGetHandleByIndex(0)
                        # Update name cache if it was N/A before and now we have a handle
                        if "N/A" in self._gpu_name_cache or "Err" in self._gpu_name_cache :
                             self._gpu_name_cache = pynvml.nvmlDeviceGetName(self._gpu_handle).decode('utf-8')
                    else: # No devices found, update name cache
                        self._gpu_name_cache = "NVIDIA GPU N/A"
                except pynvml.NVMLError as e_nvml_poll:
                    logger.debug(f"NVML: Error getting GPU handle during poll: {e_nvml_poll}")
                    self._gpu_name_cache = f"NVIDIA Poll Err ({type(e_nvml_poll).__name__})"
                    self._gpu_handle = None # Ensure handle is cleared on error
            ...
            try:
                ...
                gpu_util = -1.0 # Default for N/A or error
                gpu_name_to_emit = self._gpu_name_cache # Use cached name
                ...
                if self._nvml_initialized and self._gpu_handle and PYNVML_AVAILABLE and pynvml:
                    try:
                        ...
                        gpu_util = -2.0 # Specific error code for polling failure
                        gpu_name_to_emit = f"NVIDIA Poll Err ({type(e_nvml_util).__name__})"
                        # Potentially invalidate handle if error persists
                        if e_nvml_util.value == pynvml.NVML_ERROR_GPU_IS_LOST or \
                           e_nvml_util.value == pynvml.NVML_ERROR_INVALID_ARGUMENT: # e.g. handle became invalid
                            self._gpu_handle = None 
                            self._nvml_initialized = False # Force re-init attempt
                            logger.warning("NVML: GPU handle lost or invalid. Attempting re-init on next cycle.")

                    except Exception as e_util_other: # Other unexpected errors
            ...
            except Exception as e: # Catch errors from psutil or general loop
                logger.error(f"Error in resource monitoring loop: {e}", exc_info=True)
                # Emit error values to UI to indicate a problem
                self.resourceUpdate.emit(-1.0, -1.0, -3.0, f"Monitor Error: {str(e)[:20]}")
            
            # Sleep within the loop. QThread.msleep is thread-safe.
