```python
--- START OF FILE bsm_designer_project/main.py ---
...
class MainWindow(QMainWindow):
    ...
    def _setup_ide_dock_widget(self):
        ...
        self._on_ide_language_changed(self.ide_language_combo.currentText() if self.ide_language_combo else "Python")
        self._add_ide_tab(title="Untitled 1") # Start with one empty tab
    ...
    def _update_window_title(self): 
        ...
        ide_suffix = ""
        active_ide_tab_widget = self._get_active_ide_tab_data() # Helper to get current tab's data
        if active_ide_tab_widget and active_ide_tab_widget.get("path"):
            ...
            ide_suffix = f" | IDE: {ide_file_name}{ide_dirty_char}"
             ...
             active_tab_title = self.ide_tab_widget.tabText(self.ide_tab_widget.currentIndex()) # Get tab text
             active_tab_data = self._get_active_ide_tab_data() # Get underlying data
             ide_dirty_char = "*" if active_tab_data and active_tab_data.get("is_dirty", False) else ""
             ide_suffix = f" | IDE: {active_tab_title.replace('*','')}{ide_dirty_char}" # Use tab text, ensure no double asterisk
    ...
    def _update_ide_tab_title(self, index: int):
        ...
        if hasattr(widget, "_ide_tab_data"):
            ...
            if not tab_data["path"]:
                ...
                num = 1
                while temp_title.replace("*","") in [t.replace("*","") for t in all_tab_titles]: # Compare without asterisk
                    num += 1
                    temp_title = f"Untitled {num}"
                base_title = temp_title
    ...
    def _prompt_ide_save_if_dirty(self, tab_index: int = -1) -> bool:
        ...
        if tab_index != -1 and tab_index < self.ide_tab_widget.count(): # Specific tab
            ...
            if target_tab_data:
                tab_text_for_prompt = self.ide_tab_widget.tabText(self.ide_tab_widget.currentIndex()).replace("*","")
        ...
        reply = QMessageBox.question(self, "Save IDE Script?",
                                     ...
                                     QMessageBox.Save)
        if reply == QMessageBox.Save:
            if tab_index != -1 and self.ide_tab_widget.currentIndex() != tab_index:
                # Save request is for a non-active tab being closed, so save that one
                return self._save_specific_ide_tab(tab_index)
            else: # Save active tab
    ...
    def on_ide_new_file(self):
        # New file always adds a new tab now
        num_tabs = self.ide_tab_widget.count() if self.ide_tab_widget else 0
        self._add_ide_tab(title=f"Untitled {num_tabs + 1}")
        logger.info("IDE: New script tab created.")
        # _update_ide_related_actions called by _on_ide_current_tab_changed via setCurrentIndex in _add_ide_tab


    def on_ide_open_file(self):
        # Prompt to save active tab if it's dirty
        if not self._prompt_ide_save_if_dirty(self.ide_tab_widget.currentIndex() if self.ide_tab_widget else -1):
            return
        ...
        if file_path:
            ...
            # File not open, add it in a new tab
            new_tab_widget = self._add_ide_tab(title=os.path.basename(file_path), file_path=file_path)
            if not new_tab_widget: return 
            
            current_tab_data = new_tab_widget._ide_tab_data
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                ...
                idx = self.ide_tab_widget.indexOf(new_tab_widget)
                ...
                # self.ide_language_combo.setCurrentText(lang_to_set) # This will be handled by _on_ide_current_tab_changed
                # current_tab_data["editor"].set_language(lang_to_set) # This will also be handled
                ...
                self.ide_tab_widget.removeTab(self.ide_tab_widget.indexOf(new_tab_widget))
    ...
    def _save_specific_ide_tab(self, index: int) -> bool:
        ...
        if not tab_data["path"]: # If no path, it's a "Save As" operation for this tab
            default_filename = f"{self.ide_tab_widget.tabText(index).replace('*','')}.py" # Default to .py
            start_dir = QDir.homePath()
            file_path, _ = QFileDialog.getSaveFileName(self, "Save Script As", os.path.join(start_dir, default_filename),
                                                       "Python Files (*.py);;C/C++ Files (*.c *.cpp *.h *.ino);;Text Files (*.txt);;All Files (*)")
            if not file_path: return False # User cancelled
            tab_data["path"] = file_path

        # Now save to tab_data["path"]
        try:
            ...
            QMessageBox.critical(self, "Error Saving Script", f"Could not save script to {tab_data['path']}:\n{e}")
            logger.error(f"IDE: Failed to save script {tab_data['path']}: {e}")
            return False
    ...
    def on_ide_save_as_file(self) -> bool:
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Script As", os.path.join(start_dir, default_filename),
                                                   "Python Files (*.py);;C/C++ Files (*.c *.cpp *.h *.ino);;Text Files (*.txt);;All Files (*)")
        if file_path:
            return self._save_ide_to_path(file_path) # _save_ide_to_path saves the active tab
        return False
    ...
    def _update_ide_related_actions(self):
        ...
        has_text_in_ide = active_tab_data and active_tab_data["editor"] and \
                          bool(active_tab_data["editor"].toPlainText().strip())
        has_console_output = active_tab_data and active_tab_data["output_console"] and \
...
if __name__ == '__main__':
    ...
    main_win.show()
    sys.exit(app.exec_())
--- END OF FILE bsm_designer_project/main.py ---
```