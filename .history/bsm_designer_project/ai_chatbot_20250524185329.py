...
class ChatbotWorker(QObject):
    ...
    # MODIFIED: Added force_fsm_generation parameter
    @pyqtSlot(str, bool)
    def process_message(self, user_message: str, force_fsm_generation: bool = False):
        ...
        print(f"--- WORKER_PROCESS ({current_time}): process_message CALLED for: '{user_message[:50]}...' (force_fsm_generation={force_fsm_generation}) ---")
...
class AIChatbotManager(QObject):
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        # MODIFIED: Pass is_fsm_gen_specific to the worker's process_message
        QTimer.singleShot(0, lambda: self.chatbot_worker.process_message(user_message_text, is_fsm_gen_specific) if self.chatbot_worker else None)
