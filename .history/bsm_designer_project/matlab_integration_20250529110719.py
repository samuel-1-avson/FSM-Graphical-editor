...
class MatlabCommandWorker(QObject):
    ...
    def __init__(self, matlab_path, script_file, original_signal, success_message_prefix, model_name_for_context=None):
        ...
        self.original_signal = original_signal # The signal to emit results to (e.g., simulationFinished)
        self.success_message_prefix = success_message_prefix
        self.model_name_for_context = model_name_for_context # For better error messages

    def run_command(self):
        output_data_for_signal = ""
        success = False
        message = ""
        timeout_seconds = 600 # 10 minutes timeout for MATLAB operations

        try:
            # Ensure script path uses forward slashes for MATLAB compatibility across OS
            ...
            # Use subprocess.run for simplicity if blocking behavior is acceptable for the worker
            # For non-blocking on the main thread, this worker runs in a QThread.
            ...
            logger.debug(f"MATLAB STDOUT (first 1k chars):\n{stdout_str[:1000]}...")
            if stderr_str:
                logger.debug(f"MATLAB STDERR (first 1k chars):\n{stderr_str[:1000]}...")


            # Check for specific success/failure markers from the MATLAB script
            if "MATLAB_SCRIPT_SUCCESS:" in stdout_str:
                ...
                # Attempt to add more context from stderr if it's different and provides more info
                if stderr_str and extracted_error_detail not in stderr_str:
                    message += f"\n\n[Additional MATLAB STDERR Context]:\n{stderr_str[:500]}" # Limit length
                
                # Add context from stdout if error detail is brief and stdout has relevant lines
                stdout_context_lines = [line for line in stdout_str.splitlines() 
                                        if ("ERROR" in line.upper() or "WARNING" in line.upper() or 
                                           (self.model_name_for_context and self.model_name_for_context in line)) and
                                           not line.startswith("MATLAB_SCRIPT_FAILURE:")] # Avoid re-including the failure line
                stdout_context_for_failure = "\n".join(stdout_context_lines[:10]).strip() # Limit lines
                if stdout_context_for_failure and extracted_error_detail not in stdout_context_for_failure:
                    message += f"\n\n[Additional MATLAB STDOUT Context]:\n{stdout_context_for_failure[:500]}"

            elif process.returncode != 0: # If no specific markers, check exit code
                success = False
                error_output_detail = stderr_str or stdout_str # Prefer stderr for errors
                # Try to find specific MATLAB error lines
                ...
                if matlab_error_lines:
                    specific_error = " ".join(matlab_error_lines[:2]) # Take first two error lines
            ...
            else: # Should be caught by SUCCESS or FAILURE markers, but as a fallback for exit code 0
            ...
            # Emit the original signal (e.g., simulationFinished, codeGenerationFinished)
        ...
        except FileNotFoundError: # MATLAB executable not found
            ...
            # Clean up temporary script file and directory (if empty)
            if os.path.exists(self.script_file):
                try:
                    os.remove(self.script_file)
                    script_dir = os.path.dirname(self.script_file)
                    # Only remove directory if it's one of our temp dirs and is empty
                    if script_dir.startswith(tempfile.gettempdir()) and "bsm_matlab_" in script_dir:
                        if not os.listdir(script_dir): # Check if directory is empty
                            os.rmdir(script_dir)
                        else:
            ...
            # Emit finished_signal to allow thread cleanup
            self.finished_signal.emit(success, message, output_data_for_signal if success else "")


class MatlabConnection(QObject): 
    connectionStatusChanged = pyqtSignal(bool, str) # connected_status, message
    simulationFinished = pyqtSignal(bool, str, str)  # success, message, output_data (e.g. model_path if gen)
    codeGenerationFinished = pyqtSignal(bool, str, str) # success, message, output_directory
    ...
    def set_matlab_path(self, path):
        old_path_attempt = path.strip() if path else "" # Store original attempt for logging
        self.matlab_path = old_path_attempt # Assume valid until checked

        # Basic validation: path exists and is executable (or .exe on Windows)
        if self.matlab_path and os.path.exists(self.matlab_path) and \
           (os.access(self.matlab_path, os.X_OK) or self.matlab_path.lower().endswith('.exe')):
            self.connected = True # Mark as potentially connected; test_connection confirms
    ...
    def test_connection(self):
        ...
        # Ensure path is marked as valid before attempting test, if not already
        if not self.connected: # If path was set but not validated via set_matlab_path (e.g. direct call)
            if not self.set_matlab_path(self.matlab_path): # Validate path first
                # set_matlab_path will emit status, so just return
                return False 
        ...
        except FileNotFoundError: # Executable not found
    ...
    def detect_matlab(self):
        ...
        if sys.platform == 'win32':
            ...
            if os.path.isdir(matlab_base): # If MATLAB directory exists
                # Get versions (e.g., R2023a), sort newest first
            ...
            # MATLAB apps are usually named like MATLAB_R2023a.app
            ...
            common_base_paths = ['/usr/local/MATLAB', '/opt/MATLAB'] # Common install locations
        ...
        for path_candidate in paths_to_check:
            ...
            if path_candidate == 'matlab' and sys.platform != 'win32': # For 'matlab' in PATH on Unix-like
                try: 
                    # Simple test to see if 'matlab -batch exit' runs
                    ...
                    if test_process.returncode == 0:
                        ...
                        if self.set_matlab_path(path_candidate): return True # set_matlab_path validates and emits signal
                except (FileNotFoundError, subprocess.TimeoutExpired): 
                    logger.debug(f"Auto-detect: 'matlab' in PATH check failed or timed out for {path_candidate}")
                    continue # Try next candidate
            elif os.path.exists(path_candidate) and os.access(path_candidate, os.X_OK): # For full paths
                logger.info(f"Auto-detect: Found MATLAB at: {path_candidate}")
                if self.set_matlab_path(path_candidate): return True 

        # If no path found after checking all candidates
    ...
    def _run_matlab_script(self, script_content, worker_signal, success_message_prefix, model_name_for_context=None):
        if not self.connected: # Pre-condition check
            worker_signal.emit(False, "MATLAB not connected or path invalid.", "")
            return

        # Create a temporary script file
        try:
            temp_dir = tempfile.mkdtemp(prefix="bsm_matlab_") # Create unique temp directory
        ...
        # Setup worker and thread for MATLAB command execution
        worker = MatlabCommandWorker(self.matlab_path, script_file_path, worker_signal, success_message_prefix, model_name_for_context)
        thread = QThread() # Create a new thread for each command
        worker.moveToThread(thread)

        # Connect signals for thread management and worker execution
        thread.started.connect(worker.run_command)
        worker.finished_signal.connect(thread.quit) # Worker signals its own completion
        ...
        self._active_threads.append(thread) # Keep track of the thread
        # Lambda to remove thread from list once it's finished
        thread.finished.connect(lambda t=thread: self._active_threads.remove(t) if t in self._active_threads else None)

        thread.start() # Start the thread, which will run worker.run_command
    ...
    def generate_simulink_model(self, states, transitions, output_dir, model_name="BrainStateMachine"):
        ...
        model_name_orig = model_name # Keep original name for messages
        ...
        script_lines = [
            ...
            "try",
            # Ensure model is not already loaded or file exists (to prevent errors)
            "    if bdIsLoaded(modelNameVar), close_system(modelNameVar, 0); end", # Close if loaded
            "    if exist(outputModelPath, 'file'), delete(outputModelPath); end", # Delete if exists
            "    hModel = new_system(modelNameVar);",
            "    open_system(hModel);", # Open the new model
            "    disp('Adding Stateflow chart...');",
            # Get Stateflow machine object for the new model
            ...
            "    end",
            "    chartSFObj = Stateflow.Chart(machine);", # Create a new chart in the machine
            "    chartSFObj.Name = 'BrainStateMachineLogic';", # Name the chart
            # Add chart block to Simulink model
            "    chartBlockSimulinkPath = [modelNameVar, '/', 'BSM_Chart'];", 
            "    add_block('stateflow/Chart', chartBlockSimulinkPath, 'Chart', chartSFObj.Path);", 
            "    set_param(chartBlockSimulinkPath, 'Position', [100 50 400 350]);", # Set position
            "    disp(['Stateflow chart block added at: ', chartBlockSimulinkPath]);",
            # Map to store state handles for linking transitions
        ...
        for i, state in enumerate(states):
            ...
            # Convert BSM coordinates/sizes to Stateflow's (approximate)
            sf_x = state.get('x', 20 + i*150) / 2.5 + 20
            sf_y = state.get('y', 20) / 2.5 + 20
            sf_w = max(60, state.get('width', 120) / 2.5) # Ensure min size
            sf_h = max(40, state.get('height', 60) / 2.5)


            # Construct state label string with actions
            state_label_parts = []
            for action_key, action_desc in [('entry_action', 'entry'), ('during_action', 'during'), ('exit_action', 'exit')]:
                action_code = state.get(action_key)
                if action_code:
                    # Escape single quotes and newlines for MATLAB label string
                    escaped_action_code = action_code.replace("'", "''").replace(chr(10), '; ')
                    state_label_parts.append(f"{action_desc}: {escaped_action_code}")
            
            s_label_string_matlab = "\\n".join(state_label_parts) # Use \n for newlines in Stateflow label

            script_lines.extend([
                f"    {s_id_matlab_safe} = Stateflow.State(chartSFObj);", # Create state in chart
                f"    {s_id_matlab_safe}.Name = '{s_name_matlab}';", # Set Stateflow state name
                f"    {s_id_matlab_safe}.Position = [{sf_x}, {sf_y}, {sf_w}, {sf_h}];", # Set position
            ])
            if s_label_string_matlab: # Set label string if actions exist
                 script_lines.append(f"    {s_id_matlab_safe}.LabelString = '{s_label_string_matlab}';")
            script_lines.append(f"    stateHandles('{s_name_matlab}') = {s_id_matlab_safe};") # Store handle
            
            # Handle initial state marker
            if state.get('is_initial', False):
                script_lines.extend([
                    f"    defaultTransition_{i} = Stateflow.Transition(chartSFObj);", # Create default transition
                    f"    defaultTransition_{i}.Destination = {s_id_matlab_safe};", # Point to this state
                    # Adjust OClock for visual placement if needed
                    f"    defaultTransition_{i}.SourceOClock = 9;", # From left (9 o'clock)
        ...
        for i, trans in enumerate(transitions):
            ...
            # Construct transition label string
            label_parts = []
            if trans.get('event'): label_parts.append(trans['event'])
            if trans.get('condition'): label_parts.append(f"[{trans['condition']}]") # Guard condition
            if trans.get('action'): label_parts.append(f"/{{{trans['action']}}}") # Transition action
            
            t_label_matlab = " ".join(label_parts).strip().replace("'", "''") # Escape for MATLAB
            ...
            script_lines.extend([
                ...
                f"        t{i} = Stateflow.Transition(chartSFObj);", # Create transition
                f"        t{i}.Source = srcStateHandle;",
                f"        t{i}.Destination = dstStateHandle;",
            ])
            if t_label_matlab: # Set label if exists
        ...
        script_lines.extend([
            "% --- Finalize and Save ---",
            # Attempt to auto-arrange the chart (might need Stateflow license features)
            "    Simulink.BlockDiagram.arrangeSystem(chartBlockSimulinkPath, 'FullLayout', 'true', 'Animation', 'false');", 
            "    sf('FitToView', chartSFObj.Id);", # Fit chart to view
            ...
            "    close_system(modelNameVar, 0);", # Close model without saving again (already saved)
            "    disp(['Simulink model saved successfully to: ', outputModelPath]);",
            "    fprintf('MATLAB_SCRIPT_SUCCESS:%s\\n', outputModelPath);", # Success marker with path
            "catch e",
            "    disp('ERROR during Simulink model generation:');",
            "    disp(getReport(e, 'extended', 'hyperlinks', 'off'));", # Detailed error report
            "    if bdIsLoaded(modelNameVar), close_system(modelNameVar, 0); end", # Ensure model closed on error
...
if bdIsLoaded(modelName), close_system(modelName, 0); end % Ensure model is closed
...
"""
    ...
    def generate_code(self, model_path, language="C++", output_dir_base=None):
        ...
        if not output_dir_base: # Default output to model's directory if not specified
...
modelDir = '{model_dir_matlab}'; % Directory of the model

try
    prevPath = path; addpath(modelDir); % Add model's dir to path
    disp(['Added to MATLAB path: ', modelDir]);

    load_system(modelPath); % Load the model

    % Check for necessary Coder licenses
    ...
    % Configure for Embedded Coder (ERT target)
    set_param(modelName,'SystemTargetFile','ert.tlc'); 
    set_param(modelName,'GenerateMakefile','on'); % Typically needed for ERT

    cfg = getActiveConfigSet(modelName); % Get active configuration set
    if strcmpi('{language}', 'C++')
        set_param(cfg, 'TargetLang', 'C++');
        % Configure for C++ class interface (ERT specific option)
        set_param(cfg.getComponent('Code Generation').getComponent('Interface'), 'CodeInterfacePackaging', 'C++ class');
        set_param(cfg.getComponent('Code Generation'),'TargetLangStandard', 'C++11 (ISO)'); % Or newer if desired
    ...
    % Other useful codegen settings
    set_param(cfg, 'GenerateReport', 'on'); % Generate HTML report
    set_param(cfg, 'GenCodeOnly', 'on'); % Generate code only, don't build
    ...
    % Trigger code generation using rtwbuild (Simulink Coder command)
    % 'CodeGenFolder' specifies the root for codegen artifacts.
    % The actual code is usually in a subfolder like 'modelName_ert_rtw'.
    rtwbuild(modelName, 'CodeGenFolder', codeGenBaseDir, 'GenCodeOnly', true);
    disp('Code generation command (rtwbuild) executed.');

    % Determine actual output directory (often modelName_ert_rtw)
    actualCodeDir = fullfile(codeGenBaseDir, [modelName '_ert_rtw']);
    if ~exist(actualCodeDir, 'dir') % If standard subdir not found, assume output is in base
    ...
    fprintf('MATLAB_SCRIPT_SUCCESS:%s\\n', actualCodeDir); % Success marker with actual output dir
catch e
