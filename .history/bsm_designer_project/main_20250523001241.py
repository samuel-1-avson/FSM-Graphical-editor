...
import sys
import os
import tempfile # Still needed if DraggableToolButton uses it or other direct uses remain
import subprocess # Check if still directly used by MainWindow or its direct components
import json
import html # Used for escaping in properties dock
import math # Used by DiagramScene
import socket # Added for internet connectivity check
import sys
...
from PyQt5.QtWidgets import (
    ...
    QInputDialog, QLineEdit, QColorDialog, QDialog, QFormLayout, # Keep Dialog for specific local dialogs if any
...
) # Add other specific colors if directly used by main.py components like DiagramScene/DraggableToolButton
...
# --- Draggable Toolbox Buttons --- (Keep in main.py or move to a ui_elements.py)
...
# --- Diagram Scene --- (Keep in main.py or move to diagram_scene.py)
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            if self.current_mode == "state":
                ...
                movable_items_cache = []
                if selected_items_list is not None:
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        ...
        if item_type == "State":
            ...
            initial_text = (initial_data.get('text', "Comment") if initial_data
                             else (name_prefix if name_prefix != "Item" else "Comment"))
...
# --- Zoomable Graphics View --- (Keep in main.py or move to ui_elements.py)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._internet_connected: bool | None = None # Stores the last known internet status
        self.internet_check_timer = QTimer(self) # Timer for periodic internet check
        self._internet_connected: bool | None = None # Stores the last known internet status
        self.internet_check_timer = QTimer(self) # Timer for periodic internet check
        self.init_ui()
        ...
        self.internet_status_label.setObjectName("InternetStatusLabel") # Added
        ...
        self._init_internet_status_check() # Initialize and start internet check
        self._init_internet_status_check() # Initialize internet check
    ...
    def init_ui(self):
        ...
        self._create_status_bar() # internet_status_label is created here
        self._create_docks()
    ...
    def _create_actions(self):
     ...
     def _create_ai_chatbot_dock(self):
            # ... (Dock setup remains similar) ...
        
        # Chat Display Area
        self.ai_chat_display = QTextEdit() # Keep this in MainWindow for direct access
        self.ai_chat_display.setReadOnly(True)
        self.ai_chat_display.setPlaceholderText("AI chat history will appear here...")
        # ...

        # Input Area
        self.ai_chat_input = QLineEdit() # Keep in MainWindow
        # ...
        self.ai_chat_send_button = QPushButton(...) # Keep in MainWindow
        # ...
        
        # Status Label
        self.ai_chat_status_label = QLabel() # Keep in MainWindow
        self.ai_chat_status_label.setObjectName("AIChatStatusLabel")
        # self.ai_chat_status_label.setText("Status: Ready. Configure API Key.") # Initial text
        self._update_ai_chat_status("Status: API Key required. Configure in Settings.")


        # ... (add widgets to layout and dock)

    # ---- AI Chatbot Interaction Methods in MainWindow ----
    def _on_send_ai_chat_message(self):
        ...
        # self._update_ai_chat_status("Status: Sending to AI...") # Manager will emit "Thinking..."
        
        self.ai_chatbot_manager.send_message(user_message) # Delegate to the manager


    def _handle_ai_response(self, ai_message_text: str): # SLOT for ChatbotWorker.responseReady
        self._append_to_ai_chat_display("AI Assistant", ai_message_text)
        # self._update_ai_chat_status("Status: Ready.") # Worker's statusUpdate will handle this
        self.ai_chat_send_button.setEnabled(True)
        self.ai_chat_input.setEnabled(True)
        self.ai_chat_input.setFocus()


    def _handle_ai_error(self, error_message: str): # SLOT for ChatbotWorker.errorOccurred
        self._append_to_ai_chat_display("System Error", error_message)
        self.log_message(f"AI Chatbot Error: {error_message}", type_hint="AIChatError") # Log detailed error
        self._update_ai_chat_status(f"Error: {error_message.split(':')[0]}") # Show concise error
        self.ai_chat_send_button.setEnabled(True)
        self.ai_chat_input.setEnabled(True)
        self.ai_chat_input.setFocus()


    def _update_ai_chat_status(self, status_text: str): # SLOT for ChatbotWorker.statusUpdate
        if hasattr(self, 'ai_chat_status_label'): # Check if UI element exists
            ...
            elif "ready" in status_text.lower() or "thinking" in status_text.lower() or "cleared" in status_text.lower():
                 self.ai_chat_status_label.setStyleSheet("font-size: 8pt; color: grey;")
            else: # Default
                 self.ai_chat_status_label.setStyleSheet("font-size: 8pt; color: grey;")
            
            # Enable/disable input based on status
            if "Thinking..." in status_text or "Sending..." in status_text:
                self.ai_chat_send_button.setEnabled(False)
                self.ai_chat_input.setEnabled(False)
            else: # Ready, Error, etc.
                self.ai_chat_send_button.setEnabled(True)
                self.ai_chat_input.setEnabled(True)


    def _append_to_ai_chat_display(self, sender: str, message: str): # Remains in MainWindow
        # ... (same as before, ensures UI updates happen on the main thread) ...
        timestamp = QTime.currentTime().toString('hh:mm') # shorter timestamp for chat
        
        sender_color_hex = COLOR_ACCENT_PRIMARY
        if sender == "You": sender_color_hex = COLOR_ACCENT_SECONDARY
        elif sender == "System Error": sender_color_hex = "#D32F2F" # Red for errors
            
        # Ensure color constants are strings (hex codes)
        if not isinstance(sender_color_hex, str):
            sender_color_hex = sender_color_hex.name()

        formatted_message = (
            f"<div style='margin-bottom: 5px;'>"
            f"<span style='font-size:8pt; color:grey;'>[{timestamp}]</span> "
            f"<strong style='color:{sender_color_hex};'>{html.escape(sender)}:</strong> "
    ...
    def on_openai_settings(self):
        ...
        key, ok = QInputDialog.getText(
            ...
            QLineEdit.PasswordEchoOnEdit, # Or Password, or Normal
            current_key
        )
        if ok:
            new_key = key.strip()
            ...
            if new_key:
                ...
                self._update_ai_chat_status(f"Status: Ready. API Key set.")
            else:
                self.log_message("OpenAI API Key cleared.", type_hint="AI_CONFIG")
                self._update_ai_chat_status(f"Status: API Key required.")

    def on_clear_ai_chat_history(self):
        if self.ai_chatbot_manager:
            self.ai_chatbot_manager.clear_conversation_history()
        # self._update_ai_chat_status("Chat history cleared.") # Manager will emit status or directly clear

    # In closeEvent:
    def closeEvent(self, event: QCloseEvent):
        # ... (other close logic) ...
        if self.ai_chatbot_manager:
            self.ai_chatbot_manager.stop_chatbot() # Cleanly stop the chatbot thread
        # ...
    ...
    def _create_menus(self):
        ...
        ai_menu.addAction(self.clear_ai_chat_action) # Add action to clear history
        ai_menu.addAction(self.openai_settings_action) # Add settings action
    ...
    def _create_status_bar(self):
        ...
        self.status_bar.addWidget(self.status_label, 1) # Main status label with stretch
        ...
        self.status_bar.addPermanentWidget(self.internet_status_label) # Add label to status bar
        ...
        self.status_bar.addPermanentWidget(self.progress_bar) # Progress bar is usually last
    ...
    def _create_docks(self):
        ...
        self.log_output = QTextEdit(); self.log_output.setObjectName("LogOutputWidget")
    ...
    def _create_py_simulation_dock(self):
        ...
        control_row2_layout.addWidget(self.py_sim_event_name_edit, 1)
        ...
        if len(selected_items) == 1:
            ...
            def format_prop_text(text_content, max_chars=25):
                ...
                if len(first_line) > max_chars or '\n' in escaped:
                    return first_line[:max_chars] + "&hellip;"
                return first_line
    ...
    def log_message(self, message: str, type_hint: str = "GENERAL"):
        ...
        # CORRECTED: Removed .name() call on color constants if they are already strings
        formatted_log_entry = f"<span style='color:{COLOR_TEXT_SECONDARY};'>[{timestamp}]</span> " # Assuming COLOR_TEXT_SECONDARY is a hex string
        
        # Type-specific formatting for the detailed log dock
        if type_hint == "NETWATCH":
            # Internet check logs are styled distinctly and less prominently
            formatted_log_entry += f"<span style='color:grey;'><i>(NetCheck)</i> {display_message}</span>"
        elif type_hint == "MATLAB_CONN":
            # CORRECTED: Removed .name() call on color constants if they are already strings
            formatted_log_entry += f"<span style='color:{COLOR_TEXT_SECONDARY};'><i>(MATLAB)</i> {display_message}</span>" # Example, you might have another color
        elif type_hint == "PYSIM_STATUS_UPDATE":
             # Specific styling for Python simulation status updates
             # If you have a PYSIM_LOG_COLOR constant that is a string, use it directly.
             # If it's a QColor, then .name() is correct for QColor.
             # For this example, assuming you might use COLOR_ACCENT_PRIMARY directly (if it's a string)
             pysim_log_color_hex = COLOR_ACCENT_PRIMARY # Use directly if it's a hex string
             # If COLOR_ACCENT_PRIMARY was a QColor object, you would do:
             # pysim_log_color_hex = QColor(COLOR_ACCENT_PRIMARY).name()
             formatted_log_entry += f"<span style='color:{pysim_log_color_hex};'><i>(PySim)</i> {display_message}</span>"
        else: # GENERAL messages
            formatted_log_entry += display_message

        self.log_output.append(formatted_log_entry)
        self.log_output.verticalScrollBar().setValue(self.log_output.verticalScrollBar().maximum())

        # Update the main status_label
        if type_hint not in ["NETWATCH", "MATLAB_CONN"] and "worker:" not in message : 
...
# ...existing code...
    ...
    def _update_matlab_status_display(self, connected, message):
        ...
        style_sheet += f"color: {COLOR_PY_SIM_STATE_ACTIVE.name()};" if connected else f"color: #C62828;"
        self.matlab_status_label.setStyleSheet(style_sheet)
        if "Initializing" not in message: # Avoid logging the very first default message repeatedly
             self.log_message(f"MATLAB Conn: {message}", is_py_sim_log_source=True) # Keep log less noisy
        self._update_matlab_actions_enabled_state()
    ...
    def _save_to_path(self, file_path) -> bool:
        ...
        try:
            ...
            if bytes_written == -1 :
                ...
                save_file.cancelWriting(); return False
    ...
    def on_about(self):
        QMessageBox.about(self, "About " + APP_NAME,
            f"<h3 style='color:{COLOR_ACCENT_PRIMARY.name()};'>{APP_NAME} v{APP_VERSION}</h3>" # Use .name() for QColor in HTML
            ...
            f"<p style='font-size:8pt; color:{COLOR_TEXT_SECONDARY.name()};'>This tool is intended for research and educational purposes.</p>")

    def closeEvent(self, event: QCloseEvent):
        self.on_stop_py_simulation(silent=True) # Stop Python sim first
        ...
        if self._prompt_save_if_dirty():
            # active_threads access might need review if matlab_connection._active_threads is truly private
            ...
            event.ignore()
            if timer_was_active and not event.isAccepted(): 
    ...
    def _init_internet_status_check(self):
        ...
        # Check every 15 seconds (15000 milliseconds)
        self.internet_check_timer.start(15000) 
        # Perform an initial check shortly after startup, once UI is responsive
        QTimer.singleShot(100, self._run_internet_check_job)      # Initial check immediately

    def _run_internet_check_job(self):
        """
        Performs the internet connectivity check in a blocking way.
        Called by a QTimer.
        """
        host_to_check = "8.8.8.8"  # Google's public DNS (reliable and fast)
        port_to_check = 53         # Standard DNS port
        connection_timeout = 1.5   # Seconds to wait for connection

        current_status = False
        status_message_detail = "Checking..." # More detailed part of the message
        try:
            # socket.create_connection will attempt to resolve and connect
            s = socket.create_connection((host_to_check, port_to_check), timeout=connection_timeout)
            s.close() # Important to close the socket
            current_status = True
            ...
            # Address-related error, e.g., DNS failure if a hostname was used
            current_status = False
            status_message_detail = "Disconnected (DNS/Net Issue)"
            # Log the specific error for diagnostics, but don't clutter the main status
            self.log_message(f"Internet check gaierror: {e}", type_hint="NETWATCH") 
        except OSError as e:
            # Other network errors, e.g., "Network is unreachable"
            current_status = False
            status_message_detail = "Disconnected (Net Error)"
            self.log_message(f"Internet check OSError: {e.strerror} (errno {e.errno})", type_hint="NETWATCH")
        
        # Update UI only if the status has changed or it's the very first check
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        """
        Updates the GUI label (self.internet_status_label) to show the current status.
        """
        full_status_text = f"Internet: {message_detail}"
        self.internet_status_label.setText(full_status_text)
        
        try: # Try to get FQDN for tooltip for better info, fallback if it fails
            check_host_name_for_tooltip = socket.getfqdn('8.8.8.8')
        except Exception:
            check_host_name_for_tooltip = '8.8.8.8' 
        
        self.internet_status_label.setToolTip(
            f"{full_status_text} (Checks connection to {check_host_name_for_tooltip}:{53})"
        )

        style_sheet = "font-weight: normal; padding: 0px 5px;"
        if is_connected:
            # Use a green-like color (e.g., same as PySim active)
            style_sheet += f"color: {COLOR_PY_SIM_STATE_ACTIVE.name()};" 
        else:
            # Use a red-like color
            style_sheet += "color: #D32F2F;" # A Material Design red
        self.internet_status_label.setStyleSheet(style_sheet)
        
        # Log this status change to the detailed log dock, not the main status bar
    ...
    def _append_to_py_simulation_log(self, log_entries: list[str]):
        # This method handles logging to the DEDICATED Python Simulation log dock.
        for entry in log_entries:
            cleaned_entry = html.escape(entry)
            if "[Condition]" in entry or "[Eval Error]" in entry or "ERROR" in entry.upper():
                cleaned_entry = f"<span style='color:{COLOR_ACCENT_SECONDARY};'>{cleaned_entry}</span>" # Assuming COLOR_ACCENT_SECONDARY is a hex string
            elif "Transitioned from" in entry or "Reset to state" in entry or "Simulation started" in entry:
                cleaned_entry = f"<span style='color:{COLOR_ACCENT_PRIMARY}; font-weight:bold;'>{cleaned_entry}</span>" # Assuming COLOR_ACCENT_PRIMARY is a hex string
            elif "No eligible transition" in entry:
                cleaned_entry = f"<span style='color:{COLOR_TEXT_SECONDARY};'>{cleaned_entry}</span>" # Assuming COLOR_TEXT_SECONDARY is a hex string
        ...
        # If specific PySim events should also update the main status bar and detailed log with a type hint.
        if log_entries:
            ...
            if any(keyword in last_log_short for keyword in important_keywords):
                # CORRECTED CALL to use type_hint
    ...
    def on_start_py_simulation(self):
        ...
        try:
            ...
            # Log the initial state entry actions and "Simulation started" message
            initial_log_entries = ["Python FSM Simulation started."] + self.py_fsm_engine.get_last_executed_actions_log()
            self._append_to_py_simulation_log(initial_log_entries)
            ...
            self._append_to_py_simulation_log([f"ERROR Starting Sim: {e}"]) # Log error to PySim log
        except Exception as e:
            QMessageBox.critical(self, "Error", f"An unexpected error occurred while starting simulation:\n{e}")
            self.py_fsm_engine = None; self.set_ui_enabled_for_py_sim(False)
            self._append_to_py_simulation_log([f"CRITICAL ERROR Starting Sim: {e}"]) # Log error


    def on_stop_py_simulation(self, silent=False):
        if not self.py_sim_active:
            if not silent:
                pass # Optionally log "Simulation not running"
            return
    ...
    def on_reset_py_simulation(self):
        ...
        try:
            ...
            # self.py_sim_action_log_output.clear() # Clear log on reset for clarity, or append a separator
            self.py_sim_action_log_output.append("<hr><i style='color:grey;'>Simulation Reset</i><hr>")
            initial_log_entries = self.py_fsm_engine.get_last_executed_actions_log()
            self._append_to_py_simulation_log(initial_log_entries)
            self._update_py_simulation_dock_ui()
        except FSMError as e:
    ...
    def on_trigger_py_event(self):
        ...
        if not event_name: # If no event name, treat as a regular step (time-based transition)
...
if __name__ == '__main__':
    ...
    app.setStyleSheet(STYLE_SHEET_GLOBAL) # STYLE_SHEET_GLOBAL from config.py
