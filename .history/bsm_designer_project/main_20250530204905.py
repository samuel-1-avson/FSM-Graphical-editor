...
import re
import logging
from PyQt5.QtCore import QTime, QTimer, QPointF, QMetaObject, QFile, QTemporaryFile, QDir, QIODevice # Added QFile etc.
import pygraphviz as pgv # Keep if AI FSM layout uses it
import psutil
try:
    import pynvml
...
try:
    ...
    # logger might not be configured yet if this is very early
...
from PyQt5.QtCore import (
    Qt, QRectF, QPointF, QMimeData, QPoint, QLineF, QObject, pyqtSignal, QThread, # QDir, QIODevice moved to top
    QEvent, QSize, QUrl,
    QSaveFile, # QIODevice already imported
    pyqtSlot
)
...
class ResourceMonitorWorker(QObject):
    ...
    finished_signal = pyqtSignal()  # <-- Add this line
    ...
    def _monitor_resources(self):
        ...
        self.finished_signal.emit()  # <-- Add this line
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            setup_global_logging(self.log_output) # self.log_output is created in _create_docks
            logger.info("Main window initialized and logging configured.")
        except Exception as e:
            # Use print because logger might not be fully up if setup_global_logging failed
            print(f"ERROR: Failed to run setup_global_logging: {e}. UI logs might not work.")
            if not logging.getLogger().hasHandlers(): # Basic fallback if all else fails
                 logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s')
        ...
        if self.ai_chat_ui_manager:
            ...
            elif self._internet_connected is None: # If internet status not yet known
    ...
    def _create_docks(self): 
        ...
        self.log_output = QTextEdit(); self.log_output.setObjectName("LogOutputWidget"); self.log_output.setReadOnly(True) # setup_global_logging needs this
        log_layout.addWidget(self.log_output); self.log_dock.setWidget(log_widget); self.addDockWidget(Qt.BottomDockWidgetArea, self.log_dock)
        ...
        if hasattr(self, 'view_menu'): # Ensure view_menu exists before adding actions
    ...
    def _setup_ide_dock_widget(self):
        ...
        self._on_ide_language_changed(self.ide_language_combo.currentText()) # Initialize button states
    ...
    def _create_status_bar(self):
        ...
        # self._update_ide_save_actions_enable_state() # Called in _setup_ide_dock_widget
    ...
    def _init_resource_monitor(self):
        ...
        # Ensure thread starts worker *after* thread has started its event loop
        ...
        self.resource_monitor_worker.finished_signal.connect(self.resource_monitor_thread.quit) # Assuming finished_signal exists in worker
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        elif not clear_current_diagram: # if no items were added but macro was started
             self.undo_stack.endMacro() # Still need to end macro
    ...
    def _update_properties_dock(self):
        ...
        editor_font_size_css = f"font-size: {APP_FONT_SIZE_EDITOR};" 
    ...
    def _load_from_path(self, file_path):
        try:
            # If path is a Qt resource path (starts with :/)
            if file_path.startswith(":/"):
                ...
                qfile.close()
                file_content_str = file_content_bytes.data().decode('utf-8') # Convert QByteArray to str
    ...
    # --- Updated _get_bundled_file_path ---
    def _get_bundled_file_path(self, filename: str, resource_prefix: str = "") -> str | None:
        # Try Qt Resource System first
        if RESOURCES_AVAILABLE:
            # Construct the resource path. Ensure prefix is handled correctly (e.g. "/examples" or empty for root)
            # If resource_prefix is "examples", path becomes ":/examples/file.bsm"
            # If resource_prefix is "", path becomes ":/file.bsm" (assuming root qresource prefix)
            # Our QRC uses prefixes like "/icons", "/examples", "/docs"
            actual_resource_path_prefix = f"/{resource_prefix}" if resource_prefix else ""
            resource_path = f":{actual_resource_path_prefix}/{filename}".replace("//", "/") # Clean double slashes
            
            if QFile.exists(resource_path):
                logger.debug(f"Found bundled file '{filename}' in Qt Resources at: {resource_path}")
                # QDesktopServices.openUrl needs a file system path.
                # Copy from resource to a temporary file on disk.
                
                # Create a unique temporary directory for this app session if it doesn't exist
                app_temp_root_dir = QDir(QDir.tempPath())
                app_temp_session_dir_name = f"BSMDesigner_Temp_{QApplication.applicationPid()}" # Unique per instance
                ...
                # Ensure target directory within session_temp_dir exists if filename includes subdirs
                temp_file_info = QFileInfo(temp_disk_path)
                QDir().mkpath(temp_file_info.absolutePath())


                # Overwrite if it exists from a previous call in the same session
                ...
                if QFile.copy(resource_path, temp_disk_path):
                    ...
                    # Keep track of this temp file to delete it on application exit (optional, OS usually cleans temp)
                    # For now, let OS handle cleanup or make it a session-based temp dir.
                    return temp_disk_path
                else:
                    source_file_for_error = QFile(resource_path) # For error string
                    source_file_for_error.open(QIODevice.ReadOnly) # Try to open to get error
        ...
        # Filesystem fallback (existing logic, adjusted for more clarity)
        logger.debug(f"File '{filename}' (prefix: '{resource_prefix}') not found in Qt Resources or copy failed, trying filesystem fallback.")
        
        # Determine base path for filesystem fallback
        if getattr(sys, 'frozen', False): # PyInstaller
            base_path = sys._MEIPASS if hasattr(sys, '_MEIPASS') else os.path.dirname(sys.executable)
        else: # Development
            base_path = os.path.dirname(os.path.abspath(__file__))

        # Map resource prefix to expected subdirectory for filesystem fallback
        prefix_to_subdir_map = {
            "examples": "examples",
            "docs": "docs",
            "icons": "dependencies/icons" # If you ever needed to load icons this way
        }
        
        # Determine search paths based on prefix
        search_paths = []
        ...
        # Always check relative to base_path as well if no specific subdir for prefix or as a general fallback
        search_paths.append(os.path.join(base_path, filename)) # Check directly in base_path
        
        # Add original broader search if needed (for development structures)
        # This part might be too broad now if resources are well-defined
        # for dev_subdir_structure in ['', 'docs', 'resources', 'examples', '_internal/bsm_designer_project/docs', '_internal/bsm_designer_project/examples']:
        #    search_paths.append(os.path.join(base_path, dev_subdir_structure, filename))
    ...
    def _open_example_file(self, filename: str):
        ...
        example_path = self._get_bundled_file_path(filename, resource_prefix="examples") # Use resource_prefix
        if example_path: # Path will be a filesystem path (either original or temp copy from resource)
            if self._load_from_path(example_path): # _load_from_path handles filesystem paths
                # If it was a temp file from resource, current_file_path shouldn't be the temp path
                # We want to associate it with the resource path for title, etc.
    ...
    def on_show_quick_start(self):
        guide_path = self._get_bundled_file_path("QUICK_START.html", resource_prefix="docs") # Use resource_prefix
        if guide_path: # This will be a filesystem path (temp copy from resource or direct)
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
            ...
            wait_time_ms = 2500 # Generic wait time
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        ...
        if hasattr(self, 'ai_chat_ui_manager') and self.ai_chat_ui_manager:
            if not key_present:
                ...
                self.ai_chat_ui_manager.update_status_display("Status: Offline. AI features unavailable.")
            # If online and key present, the manager's set_online_status will trigger worker status update
    ...
    def on_distribute_items(self, mode: str):
        ...
        if mode == "horizontal":
            ...
            if spacing < 0: 
                spacing = 10 # Default minimum spacing if overlapping
            ...
            if spacing < 0:
                spacing = 10 # Default minimum spacing
    ...
    def _on_ide_language_changed(self, language_param: str): 
        ...
        ai_ready = self.ai_chatbot_manager is not None and \
                   ...
                   self._internet_connected is True # Explicitly check True
    ...
    def on_ide_analyze_with_ai(self):
        ...
        if not self.ai_chatbot_manager or not self.ai_chatbot_manager.api_key:
            ...
            return
        if not self._internet_connected: # Check the cached internet status
    ...
    def log_message(self, level_str: str, message: str):
        # This method is called by other parts of the application to log messages
        # which are then handled by the logging_setup.py configuration.
...
if __name__ == '__main__':
    ...
    # PIL fallback for icons is no longer needed here if using Qt Resource System
    # and resources_rc.py is correctly imported.

    # Ensure QFileInfo is imported if used in _get_bundled_file_path for path manipulation
    from PyQt5.QtCore import QFileInfo


    app = QApplication(sys.argv)
    app.setStyleSheet(STYLE_SHEET_GLOBAL) # STYLE_SHEET_GLOBAL should now use resource paths
    main_win = MainWindow()
    main_win.show()
    sys.exit(app.exec_())
