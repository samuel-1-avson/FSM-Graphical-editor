...
class MainWindow(QMainWindow):
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        try:
            ...
            if raw_gv_positions:
                ...
                max_y_gv = max(p['y'] for p in raw_gv_positions) # dot Y increases upwards
        ...
        if items_to_add_for_undo_command:
            for item in items_to_add_for_undo_command:
                ...
                # AddItemCommand's redo() will add the item to the scene.
                # If we are clearing the diagram, push directly.
                # If we are adding to existing (macro active), push to the macro.
    ...
    def _update_py_sim_status_display(self): # Updated for hierarchical state display
        if self.py_sim_active and self.py_fsm_engine:
            # get_current_state_name() from FSMSimulator now returns hierarchical name if sub-FSM is active
    ...
    def _highlight_sim_active_state(self, state_name_to_highlight: str | None): # Updated for leaf state
        ...
        if state_name_to_highlight and self.py_fsm_engine:
            # For hierarchical, we want to highlight the *leaf* active state.
            # If the simulator itself is a sub-simulator, its state_name_to_highlight is already the leaf.
            # If it's the main simulator, get its leaf state.
            leaf_state_name = self.py_fsm_engine.get_current_leaf_state_name()

            if leaf_state_name:
                # Determine which scene to search: main scene or sub-scene (if a sub-editor were open and active for highlight)
                # For now, assume highlighting always happens on the main scene.
                # If SubFSMEditorDialog is open, it has its own scene. Highlighting there would be complex.
                # Let's keep it simple: highlight on the main scene. If the leaf state is a sub-state,
                # this won't find it directly on the main scene unless we flatten the concept for display.
                # Current FSMSimulator highlights only the top-level superstate.
                # To highlight a sub-state, the sub-FSM diagram would need to be visible and receive update.
                
                # For now, highlight the top-level state that contains the leaf state.
                top_level_active_state_name = self.py_fsm_engine.sm.current_state.id if self.py_fsm_engine.sm else None

                if top_level_active_state_name:
                    for item in self.scene.items():
                        if isinstance(item, GraphicsStateItem) and item.text_label == top_level_active_state_name:
                            logging.debug("PySim: Highlighting top-level active state '%s' (leaf: '%s')", top_level_active_state_name, leaf_state_name)
    ...
    def _update_py_simulation_dock_ui(self): # Updated for hierarchy
        ...
        # Display hierarchical state name
        ...
        self._highlight_sim_active_state(hierarchical_state_name) # Pass the full name for context

        # Variables: Display main FSM vars. If sub-FSM is active, display its vars too.
        ...
        if self.py_fsm_engine.active_sub_simulator:
            ...
            for name, value in sorted(sub_vars.items()):
                all_vars_display.append((f"[SUB] {name}", str(value))) # Prefix sub-vars
        ...
        # Events: Primarily for the main FSM
    ...
    def _append_to_py_simulation_log(self, log_entries: list[str]): # Unchanged
        ...
        if log_entries:
            ...
            important_keywords = ["Transitioned from", "No eligible transition", "ERROR", "Reset to state",
                                  ...
                                  "Entering state", "Exiting state"] # Added more keywords
