...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ai_chat_ui_manager = AIChatUIManager(self) # This class is now in ai_chatbot.py
    ...
    def _populate_dynamic_docks(self):
        ...
        if self.ai_chat_ui_manager and self.ai_chatbot_dock: # ai_chat_ui_manager is now in ai_chatbot.py
    ...
    def _create_docks(self):
        ...
        self.ai_chatbot_dock = QDockWidget("AI Assistant", self); self.ai_chatbot_dock.setObjectName("AIChatbotDock") # Now defined in ai_chatbot.py
    ...
    def _update_resource_display(self, cpu_usage, ram_usage, gpu_util, gpu_name):
        ...
        if hasattr(self, 'gpu_status_label'):
            if gpu_util == -1.0: self.gpu_status_label.setText(f"GPU: {gpu_name}") # N/A or specific info
            elif gpu_util == -2.0: self.gpu_status_label.setText(f"GPU: Error") # Util fetch error
            elif gpu_util == -3.0: self.gpu_status_label.setText(f"GPU: Mon Error") # General monitor error
            ...
            else: # Fallback if NVML isn't fully ready or not available
                 self.gpu_status_label.setText(f"GPU: N/A")
                 self.gpu_status_label.setToolTip(gpu_name) # Show detailed reason in tooltip
    ...
    def _update_window_title(self):
        ...
        # Combine diagram dirty state with IDE dirty state for main window modified indicator
        ...
        # if ide_simple_status_for_main_title: title_parts.append(f"| {ide_simple_status_for_main_title}") # Can be too long
        ...
        if hasattr(self, 'status_label'):
            ...
            # Use the more detailed IDE status for the status bar if available
            ide_status_for_bar = ide_simple_status_for_main_title # Use the simplified one for status bar too for consistency
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        ...
        # The AIChatUIManager will update its own status label based on the AIStatus enum
        # emitted by AIChatbotManager, so no direct call to ai_chat_ui_manager.update_status_display here.
    ...
    def on_ide_analyze_with_ai(self):
        ...
        if self.ai_chat_ui_manager: # Ensure UI manager is available
