...
class SubFSMEditorDialog(QDialog): 
    def __init__(self, sub_fsm_data_initial: dict, parent_state_name: str, parent_window_ref=None):
        ...
        if IMPORTS_SUCCESSFUL:
            ...
            # Pass self (SubFSMEditorDialog) as parent_window to DiagramScene
            ...
            toolbar.setIconSize(QSize(20,20)) # Slightly larger icons
            ...
            self.sub_redo_action = self.sub_undo_stack.createRedoAction(self, "ReSub")
            ...
            self.status_label = QLabel("Hint: A full visual editor for sub-machines would replace this JSON view if dependencies were met.")
            self.status_label.setStyleSheet(f"font-size: {APP_FONT_SIZE_SMALL}; color: {COLOR_TEXT_SECONDARY};")
...
class StatePropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_state=False, scene_ref=None):
        ...
        self.setMinimumWidth(550) # Increased width for language combo and better layout
        self.setStyleSheet(f"QDialog {{ background-color: {COLOR_BACKGROUND_DIALOG}; }} QLabel#SafetyNote {{ font-size: {APP_FONT_SIZE_SMALL}; color: {COLOR_TEXT_SECONDARY}; }}")
        ...
        self.entry_action_edit = CodeEditor(); self.entry_action_edit.setPlainText(p.get('entry_action', "")); self.entry_action_edit.setFixedHeight(110); self.entry_action_edit.setObjectName("ActionCodeEditor")
        self.during_action_edit = CodeEditor(); self.during_action_edit.setPlainText(p.get('during_action', "")); self.during_action_edit.setFixedHeight(110); self.during_action_edit.setObjectName("ActionCodeEditor")
        self.exit_action_edit = CodeEditor(); self.exit_action_edit.setPlainText(p.get('exit_action', "")); self.exit_action_edit.setFixedHeight(110); self.exit_action_edit.setObjectName("ActionCodeEditor")
        
        self.description_edit = QTextEdit(p.get('description', "")); self.description_edit.setFixedHeight(80)

        self.entry_action_snippet_btn = self._create_insert_snippet_button(self.entry_action_edit, "actions", " Action Snippet")
        self.during_action_snippet_btn = self._create_insert_snippet_button(self.during_action_edit, "actions", " Action Snippet")
        self.exit_action_snippet_btn = self._create_insert_snippet_button(self.exit_action_edit, "actions", " Action Snippet")
        ...
        layout.addRow("Display Color:", self.color_button)
        ...
        def add_field_with_note(form_layout, label_text, code_editor_widget, snippet_button):
            ...
            h_editor_btn_layout.setSpacing(6)
    ...
    def _create_insert_snippet_button(self, target_widget: CodeEditor, snippet_category: str, button_text="Insert...", icon_size_px=16):
        ...
        button.setToolTip("Insert common code snippets"); button.setIcon(get_standard_icon(QStyle.SP_FileDialogContentsView, "InsSnip"))
    ...
    def _update_snippet_button_menu(self, button: QPushButton, target_widget: CodeEditor, language_mode: str, snippet_category: str):
        menu = button.menu()
        menu.clear()
        menu.setStyleSheet("QMenu { font-size: 9pt; } QMenu::item { padding: 5px 20px; }") # Style menu
    ...
    def _on_edit_sub_fsm(self): 
        ...
        # Ensure the sub-editor's parent is the top-level window for modality
        dialog_parent_ref = self.parent_window_ref
        while hasattr(dialog_parent_ref, 'parent_window_ref') and dialog_parent_ref.parent_window_ref is not None and not isinstance(dialog_parent_ref.parent_window_ref, QMainWindow):
            dialog_parent_ref = dialog_parent_ref.parent_window_ref

        sub_editor_dialog = SubFSMEditorDialog(self.current_sub_fsm_data, parent_state_name, dialog_parent_ref)
...
class TransitionPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_transition=False):
        ...
        self.setMinimumWidth(580) # Increased width
        self.setStyleSheet(f"QDialog {{ background-color: {COLOR_BACKGROUND_DIALOG}; }} QLabel#SafetyNote {{ font-size: {APP_FONT_SIZE_SMALL}; color: {COLOR_TEXT_SECONDARY}; }}")


        layout = QFormLayout(self); layout.setSpacing(12); layout.setContentsMargins(15,15,15,15) 
        p = current_properties or {}
        ...
        self.action_edit = CodeEditor(); self.action_edit.setPlainText(p.get('action', "")); self.action_edit.setFixedHeight(110); self.action_edit.setObjectName("ActionCodeEditor")
        ...
        self.event_snippet_btn = self._create_insert_snippet_button_lineedit(self.event_edit, "events", " Event Snippet")
        self.condition_snippet_btn = self._create_insert_snippet_button_lineedit(self.condition_edit, "conditions", " Condition Snippet")
        self.action_snippet_btn = self._create_insert_snippet_button_codeeditor(self.action_edit, "actions", " Action Snippet")
        ...
        def add_field_with_button_and_note(form_layout, label_text, edit_widget, snippet_button, is_code_field=True, is_line_edit=False):
            ...
            v_btn_container.setSpacing(0)
            v_btn_container.addWidget(snippet_button)
            if not is_line_edit : v_btn_container.addStretch() 
            h_editor_btn_layout.addLayout(v_btn_container)
        ...
        curve_layout.addWidget(QLabel("Bend (Perpendicular):")); curve_layout.addWidget(self.offset_perp_spin,1); curve_layout.addSpacing(15)
        curve_layout.addWidget(QLabel("Mid Shift (Tangential):")); curve_layout.addWidget(self.offset_tang_spin,1); curve_layout.addStretch()
    ...
    def _on_action_language_changed(self, language_mode: str):
        ...
        # Conditions and events are usually simpler and might not need full code editor highlighting based on selected action language,
        # but snippets should update based on the overall context language.
    ...
    def _create_insert_snippet_button_codeeditor(self, target_code_editor: CodeEditor, snippet_category: str, button_text="Insert..."): 
        button = QPushButton(button_text); button.setObjectName("SnippetButton")
        button.setIcon(get_standard_icon(QStyle.SP_FileDialogContentsView,"InsSnip")); button.setIconSize(QSize(16,16))
        button.setToolTip("Insert common code snippets."); button.setMenu(QMenu(self))
        return button
...
class CommentPropertiesDialog(QDialog): 
    def __init__(self, parent=None, current_properties=None):
        ...
        layout.setSpacing(10); layout.setContentsMargins(15,15,15,15) # Increased margins and spacing
        ...
        self.text_edit.setFontPointSize(10) # Slightly larger font for comment editing
...
class MatlabSettingsDialog(QDialog): 
    def __init__(self, matlab_connection: MatlabConnection, parent=None):
        ...
        self.setStyleSheet(f"QDialog {{ background-color: {COLOR_BACKGROUND_DIALOG}; }} QLabel#TestStatusLabel {{ padding: 5px; border-radius: 3px; }}")
