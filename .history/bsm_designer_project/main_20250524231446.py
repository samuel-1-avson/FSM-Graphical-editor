...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.matlab = MatlabConnection()
        self.matlab.connectionStatusChanged.connect(self.on_matlab_status)
        self.matlab.simulinkStatusChanged.connect(self.on_simulink_status)
        self.matlab.simulationFinished.connect(self.on_matlab_simulation_finished)
        self.matlab.codeGenerationFinished.connect(self.on_matlab_codegen_finished)
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("FSM/Simulink/Matlab Integration Example")

        # Setup the Matlab menu
        menubar = self.menuBar()
        matlab_menu = menubar.addMenu("Matlab/Simulink")

        act_set_matlab = QAction("Set Matlab Path", self)
        act_set_matlab.triggered.connect(self.prompt_set_matlab_path)
        matlab_menu.addAction(act_set_matlab)

        act_auto_matlab = QAction("Auto-Detect Matlab", self)
        act_auto_matlab.triggered.connect(self.auto_detect_matlab)
        matlab_menu.addAction(act_auto_matlab)

        act_test_matlab = QAction("Test Matlab Connection", self)
        act_test_matlab.triggered.connect(self.test_matlab_connection)
        matlab_menu.addAction(act_test_matlab)

        act_check_simulink = QAction("Check Simulink Available", self)
        act_check_simulink.triggered.connect(self.check_simulink_available)
        matlab_menu.addAction(act_check_simulink)
        matlab_menu.addSeparator()

        act_generate_simulink = QAction("Export FSM to Simulink", self)
        act_generate_simulink.triggered.connect(self.export_to_simulink)
        matlab_menu.addAction(act_generate_simulink)

        act_run_sim = QAction("Run Simulink Simulation", self)
        act_run_sim.triggered.connect(self.run_simulink_simulation)
        matlab_menu.addAction(act_run_sim)

        act_codegen = QAction("Generate Embedded C/C++ Code", self)
        act_codegen.triggered.connect(self.generate_embedded_code)
        matlab_menu.addAction(act_codegen)
