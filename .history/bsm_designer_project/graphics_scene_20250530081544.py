...
from PyQt5.QtCore import Qt, QRectF, QPointF, QLineF, pyqtSignal, QPoint, QMarginsF # Added QMarginsF
...
class ZoomableView(QGraphicsView):
    ...
    def zoom_in(self):
        ...
        if new_zoom_level <= 15: # Max zoom level
            self.scale(factor, factor)
            self.zoom_level = new_zoom_level
    ...
    def zoom_out(self):
        factor = 1 / 1.12
        new_zoom_level = self.zoom_level - 1
        if new_zoom_level >= -15: # Min zoom level
            self.scale(factor, factor)
            self.zoom_level = new_zoom_level
    ...
    def zoom_to_rect(self, target_rect: QRectF, padding_factor: float = 0.1):
        ...
        # Approximate zoom_level update after fitInView (optional, might not be perfectly accurate)
        # transform = self.transform()
        # current_scale = math.sqrt(transform.m11() * transform.m22() - transform.m12() * transform.m21()) # Determinant for general affine
        # if transform.type() == QTransform.TxScale: # Simpler if only scale and translate
        #     current_scale = math.sqrt(transform.m11() * transform.m22())

        # if current_scale > 1e-6 : # Avoid log(0)
        #      self.zoom_level = round(math.log(current_scale, 1.12)) # 1.12 is our zoom factor base
