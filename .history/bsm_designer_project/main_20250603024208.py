...
class MainWindow(QMainWindow):
    ...
    def _clear_dock_editors(self):
        ...
        if layout is None: # Should not happen if initialized, but defensive
            return

        # Remove all items from the layout.
        # Iterating backwards is good practice when removing.
        # Clearing a layout means taking items out; actual deletion of widgets
        # depends on their parentage or explicit deleteLater calls.
        items_to_delete_later = []

        for i in range(layout.rowCount() - 1, -1, -1):
            # Get items for both label and field roles
            label_item = layout.itemAt(i, QFormLayout.LabelRole)
            field_item = layout.itemAt(i, QFormLayout.FieldRole)

            # Mark the widgets/layouts for deletion later
            if label_item:
                widget = label_item.widget()
                if widget:
                    items_to_delete_later.append(widget)
                # If it's a layout item containing another layout (less common for labels)
                # inner_layout = label_item.layout()
                # if inner_layout:
                # items_to_delete_later.append(inner_layout)
            ...
            if field_item:
                ...
                if inner_layout:
                    # Recursively mark widgets in this inner layout for deletion
                    while inner_layout.count():
                        child = inner_layout.takeAt(0)
                        if child:
                            if child.widget():
                                items_to_delete_later.append(child.widget())
                            elif child.layout(): # Handle layouts within layouts
                                items_to_delete_later.append(child.layout()) # Mark layout itself
                    items_to_delete_later.append(inner_layout) # Mark the inner layout itself

            # This removes the row from the form layout's management.
            # It does NOT necessarily delete the widgets/layouts themselves if they have other parents
            # or are not direct children of the form layout's parent widget.
            layout.removeRow(i)

        # Now, after all items are removed from the layout's management,
        # schedule them for deletion.
        for item_to_delete in items_to_delete_later:
            if item_to_delete: # Check if it's not None
                # Check if it's a QLayout or QWidget before calling deleteLater
                if isinstance(item_to_delete, (QWidget, QLayout)): # QLayout also inherits QObject
                     # Check if the C++ object still exists
                    try:
                        # A simple check, though not foolproof for all Qt deletion scenarios
                        if item_to_delete.parent() is not None or not hasattr(item_to_delete, 'isWidgetType'): # Crude check
                             item_to_delete.deleteLater()
                    except RuntimeError:
                        logger.debug(f"Dock Clear: Object {type(item_to_delete).__name__} was already deleted, skipping deleteLater.")
                        pass # Object already deleted
                else:
                    logger.warning(f"Dock Clear: Encountered an item in items_to_delete_later that is not a QWidget or QLayout: {type(item_to_delete)}")
