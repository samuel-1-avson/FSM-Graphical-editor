...
class StateMachinePoweredSimulator:
    def __init__(self, states_data, transitions_data, parent_simulator=None, log_prefix="", halt_on_action_error=False):
        ...
        try:
            ...
            # Handle cases for empty FSMs (especially sub-FSMs)
            elif not self._states_input_data and not self.parent_simulator: # Top-level empty FSM
                 # _build_fsm_class_and_instance should raise FSMError in this case.
                 # This path might not be reachable if build raises correctly.
                 raise FSMError("No states defined in the FSM.")
            elif not self._states_input_data and self.parent_simulator: # Empty sub-FSM
    ...
    def _create_dynamic_callback(self, code_string, callback_type="action", original_name="dynamic_callback"):
        ...
        if not is_safe:
            ...
            if callback_type == "condition":
                def unsafe_condition_wrapper(*args, **kwargs): # machine instance might be in kwargs
        ...
        # `self` here is the StateMachinePoweredSimulator instance.
        # The returned wrapper will be a method of the dynamic StateMachine class.
        # When python-statemachine calls it, `sm_instance_arg` will be the StateMachine instance.
        def dynamic_callback_wrapper(sm_instance_arg, *args, **kwargs_from_sm_call):
            # sm_instance_arg is the 'self' of the StateMachine dynamic class instance (our self.sm)
            # kwargs_from_sm_call contains event, source, target, machine, model, event_data etc.
            
            # For user code, the execution context (locals) should be the model's attributes.
            exec_eval_locals_dict = sm_instance_arg.model # This is self._variables
            ...
            # Use `self` (StateMachinePoweredSimulator's self) for logging and halt flag
            ...
            try:
                if callback_type == "action":
                    ...
                    return None # Actions don't typically return values that affect flow
                elif callback_type == "condition":
                    # For eval, provide a copy to prevent modification if some builtin allows it.
                ...
                if self._halt_simulation_on_action_error and callback_type == "action": self.simulation_halted_flag = True; raise FSMError(err_msg) # Will be caught by step()
                ...
                if callback_type == "condition": return False
                if self._halt_simulation_on_action_error and callback_type == "action": self.simulation_halted_flag = True; raise FSMError(err_msg)
            return None # Default for actions if no error, or if error and not halting
        
        dynamic_callback_wrapper.__name__ = f"{original_name}_{callback_type}_{hash(code_string)}"
        return dynamic_callback_wrapper

    def _master_on_enter_state(self, machine_self, target: State, **kwargs): # machine_self is the SM instance
        # `self` here is StateMachinePoweredSimulator
        target_state_name = target.id # python-statemachine uses 'id' for the unique identifier
            ...
            if state_def.get('is_superstate', False):
                ...
                if sub_fsm_data and sub_fsm_data.get('states'):
                    ...
                    try:
                        ...
                        # Append sub-simulator's init log to current log
        ...
        # Specific entry actions (on_enter_STATENAME) are handled by python-statemachine automatically if defined on the FSMClass.
        # We already added them to fsm_class_attrs in _build_fsm_class_and_instance.

    def _master_on_exit_state(self, machine_self, source: State, **kwargs): # machine_self is the SM instance
        # `self` here is StateMachinePoweredSimulator
        source_state_name = source.id
        self._log_action(f"Exiting state: {source_state_name}")

        # Specific exit actions (on_exit_STATENAME) are handled by python-statemachine.
        ...
        if self.active_sub_simulator and self.active_superstate_name == source_state_name:
            ...
            # Collect final logs from sub-simulator before destroying it
    ...
    def _sm_before_transition(self, machine_self, event: str, source: State, target: State, **kwargs):
        self._log_action(f"Before transition on '{kwargs.get('event_data').event if kwargs.get('event_data') else event}' from '{source.id}' to '{target.id}'")

    def _sm_after_transition(self, machine_self, event: str, source: State, target: State, **kwargs):
        self._log_action(f"After transition on '{kwargs.get('event_data').event if kwargs.get('event_data') else event}' from '{source.id}' to '{target.id}'")
    ...
    def _build_fsm_class_and_instance(self):
        ...
        if not initial_state_name_from_data and self._states_input_data:
            ...
            if first_state_name_from_data in sm_states_obj_map:
                self._log_action(f"No initial state set. Using '{first_state_name_from_data}' as initial.")
            ...
            else: self._log_action("Sub-FSM has no states. Inactive."); self.FSMClass = self.sm = None; return
        ...
        for t_idx, t_data in enumerate(self._transitions_input_data):
            ...
            if not event_name_str:
                ...
                self._log_action(f"Warning: Transition {source_name}->{target_name} has no event. Synthetic: {event_name_str}")

            source_state_obj, target_state_obj = sm_states_obj_map.get(source_name), sm_states_obj_map.get(target_name)
            if not source_state_obj or not target_state_obj:
                self._log_action(f"Warning: Skipping transition {source_name}->{target_name} (event '{event_name_str}'), missing state obj."); continue

            if event_name_str not in defined_events:
                event_obj = SMEvent(event_name_str); defined_events[event_name_str] = event_obj
            ...
            # This defines the transition and it's collected by the metaclass
        ...
        if self.sm and self.sm.current_state:
            self._log_action(f"FSM Class '{unique_class_name}' built. Initial state: {self.sm.current_state.id}")
        elif self.sm and not self.sm.current_state and self._states_input_data : # Should be caught by InvalidDefinition earlier
    ...
    def get_current_state_name(self):
        ...
        name = self.sm.current_state.id # .id is the sanitized name, .name is the original
    ...
    def get_last_executed_actions_log(self):
        log_snapshot = self._action_log[:] # Create a copy
        self._action_log = [] # Clear the instance log
        return log_snapshot
    ...
    def reset(self):
        ...
        if self.FSMClass: # If a class was successfully built
            try:
                self.sm = self.FSMClass(model=self._variables, allow_event_without_transition=True)
        ...
        elif not self.parent_simulator and not self._states_input_data: # Top-level FSM was empty
            logger.error(f"{self.log_prefix}FSM Class not built (no states), cannot reset properly.")
            # No FSMError raise here, as init might have determined it's an empty (but valid for sub) FSM
        # If it's an empty sub-FSM, FSMClass is None, sm is None, this is fine.
    ...
    def step(self, event_name=None):
        ...
        try:
            ...
            if main_state_data and main_state_data.get('during_action'):
                ...
                # For 'during' action, we call it directly, not through SM lifecycle.
                # _create_dynamic_callback's wrapper needs to handle this direct call context.
                # We pass `self.sm` as if it were the `machine_self` from a callback.
                during_cb = self._create_dynamic_callback(action_str, "action", f"during_{main_state_id}")
                during_cb(self.sm) # Pass the SM instance (self.sm) 
    ...
    def get_possible_events_from_current_state(self) -> list[str]:
        ...
        # Get events from the current FSM (main or leaf sub-FSM)
        ...
        if current_sm_to_query and current_sm_to_query.current_state:
            # python-statemachine's allowed_events returns Event objects
            possible_events_set.update(str(evt.id) for evt in current_sm_to_query.allowed_events)
        
        # If in a superstate, also consider events defined on the parent FSM that can transition *out* of this superstate
        if self.active_sub_simulator and self.sm and self.sm.current_state: # current_state here is the superstate
...
if __name__ == "__main__":
    main_states_data = [
        {"name": "Idle", "is_initial": True, "entry_action": "print('Main: Idle Entered'); idle_counter = 0; Processing_sub_completed = False"}, # Initialize flag
    ...
    main_transitions_data = [
        ...
        # Condition now checks the variable set by the sub-machine's completion logic
    ...
    try:
        ...
        # Now, events for sub-machine should be sent to the sub-machine instance directly
        ...
        # After sub-machine finishes, the variable 'Processing_sub_completed' should be True in parent.
        # The next step on parent with 'auto_finish' event should trigger the transition.
        ...
        unsafe_trans_s = []
        try:
            unsafe_sim_s = FSMSimulator(unsafe_states_s, unsafe_trans_s) # Error will be logged on entry
        ...
        try:
            # The error should be raised by the entry action during StateMachine instantiation.
            # So, the FSMSimulator constructor itself should raise it.
            ...
            # Manually get logs if simulator object exists but step failed or was part of init
            # (This part might be tricky if `halt_sim` is not fully initialized)
            # if 'halt_sim' in locals() and halt_sim and hasattr(halt_sim, 'get_last_executed_actions_log'):
            #      log = halt_sim.get_last_executed_actions_log()
            #      if log: print("Log from potentially partially init halted sim:"); [print(f"  {entry}") for entry in log]
