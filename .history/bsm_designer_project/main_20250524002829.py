...
from fsm_simulator import FSMSimulator, FSMError # This now points to StateMachinePoweredSimulator
...
class DiagramScene(QGraphicsScene):
    ...
    def _add_item_interactive(self, pos: QPointF, item_type: str, name_prefix:str="Item", initial_data:dict=None):
        ...
        if item_type == "State":
            ...
            initial_dialog_props = {
                'name': default_name, 'is_initial': is_initial_state_from_drag,
                'is_final': is_final_state_from_drag, 'color': initial_data.get('color', COLOR_ITEM_STATE_DEFAULT_BG),
                'entry_action':"", 'during_action':"", 'exit_action':"", 'description':"",
                'is_superstate': initial_data.get('is_superstate', False), # From draggable or default
    ...
    def dropEvent(self, event: QGraphicsSceneDragDropEvent):
        ...
        if event.mimeData().hasFormat("application/x-bsm-tool"):
            ...
            grid_x = round(pos.x() / self.grid_size) * self.grid_size
            grid_y = round(pos.y() / self.grid_size) * self.grid_size
            if "State" in item_type_data_str: grid_x -= 60; grid_y -= 30 # Adjust for state item center
            initial_props_for_add = {}; actual_item_type_to_add = "Item"; name_prefix_for_add = "Item"

            # Logic to determine item type and initial properties from dropped data
            ...
            # Could add 'Super State' here if you have a dedicated draggable for it
            # elif item_type_data_str == "Super State": actual_item_type_to_add = "State"; name_prefix_for_add = "SuperState"; initial_props_for_add['is_superstate'] = True
    ...
    def load_diagram_data(self, data):
        ...
        for state_data in data.get('states', []):
            state_item = GraphicsStateItem(
                ...
                state_data.get('is_superstate', False), # Load superstate flag
                state_data.get('sub_fsm_data', {'states':[], 'transitions':[], 'comments':[]}) # Load sub-FSM data
            )
...
class MainWindow(QMainWindow):
    ...
    def _create_menus(self):
        ...
        file_menu = menu_bar.addMenu("&File"); file_menu.addAction(self.new_action); file_menu.addAction(self.open_action); file_menu.addAction(self.save_action); file_menu.addAction(self.save_as_action); file_menu.addSeparator()
        ...
        file_menu.addSeparator(); file_menu.addAction(self.export_simulink_action); file_menu.addSeparator(); file_menu.addAction(self.exit_action)
        ...
        self.view_menu = menu_bar.addMenu("&View") # View menu for dock toggles
        ...
        if json_match and json_string:
            try:
                ...
                "transitions" in fsm_data and isinstance(fsm_data["transitions"], list):
                    ...
                    print(f"--- MW_HANDLE_RESP ({current_time}): Extracted JSON is valid, but not FSM structure. Keys: {list(fsm_data.keys()) if isinstance(fsm_data, dict) else 'Not a dict'}. Treating as text. ---")
                    self.log_message(f"AI response contained valid JSON, but it did not match the expected FSM structure. Displaying as text.", type_hint="AI_ACTION")
                    message_processed_as_fsm = False
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False):
        ...
        for i, state_data in enumerate(fsm_data.get('states', [])):
            ...
            try:
                state_item = GraphicsStateItem(
                    ...
                    is_superstate=state_data.get('is_superstate', False), # Added
                    sub_fsm_data=state_data.get('sub_fsm_data', {'states':[],'transitions':[],'comments':[]}) # Added
                )
            ...
            if source_item and target_item:
                try:
                    trans_item = GraphicsTransitionItem(
                        source_item, target_item, event_str=trans_data.get('event', ""),
                        condition_str=trans_data.get('condition', ""), action_str=trans_data.get('action', ""),
                        color=trans_data.get('properties', {}).get('color'), description=trans_data.get('description', "")
                    )
                    ...
                    if offset_x is not None and offset_y is not None:
                        ...
                        except ValueError: self.log_message(f"AI Gen Warning: Invalid control offsets for transition {source_name}->{target_name}. Skipping offset.", type_hint="AIChatError")
        ...
        if state_items_map: max_y_of_states = max((item.scenePos().y() + item.rect().height() for item in state_items_map.values()), default=start_y)
        ...
        if items_to_add_for_undo_command:
            for item in items_to_add_for_undo_command:
                ...
                elif hasattr(item, 'event_str'): cmd_text += f" {item.start_item.text_label if item.start_item else '?' }->{item.end_item.text_label if item.end_item else '?'}"
    ...
    def _append_to_ai_chat_display(self, sender: str, message: str):
        ...
        timestamp = QTime.currentTime().toString('hh:mm'); sender_color_hex_str = COLOR_ACCENT_PRIMARY
        if sender == "You": sender_color_hex_str = COLOR_ACCENT_SECONDARY
        elif sender == "System Error": sender_color_hex_str = "#D32F2F"
        formatted_message = (f"<div style='margin-bottom: 5px;'><span style='font-size:8pt; color:grey;'>[{timestamp}]</span> <strong style='color:{sender_color_hex_str};'>{html.escape(sender)}:</strong> {html.escape(message).replace(r'\n', '<br>')}</div>")
    ...
    def on_openai_settings(self):
        ...
        if ok:
            ...
            if new_key: self.log_message("OpenAI API Key set/updated.", type_hint="AI_CONFIG")
            else: self.log_message("OpenAI API Key cleared.", type_hint="AI_CONFIG")
    ...
    def _update_properties_dock(self):
        ...
        if len(selected_items) == 1:
            ...
            def format_prop_text(text_content, max_chars=25):
                ...
                escaped = html.escape(str(text_content)); first_line = escaped.split('\n')[0]
            ...
            if isinstance(item, GraphicsStateItem):
                ...
                rows_html += f"<tr><td><b>Superstate:</b></td><td>{'Yes' if props.get('is_superstate') else 'No'}</td></tr>" # Added for superstate
                ...
                event_text = html.escape(props.get('event', '')) if props.get('event') else ''; condition_text = f"[{html.escape(props.get('condition', ''))}]" if props.get('condition') else ''; action_text = f"/{{{format_prop_text(props.get('action', ''), 15)}}}" if props.get('action') else ''
                label_parts = [p for p in [event_text, condition_text, action_text] if p]; full_label = " ".join(label_parts) if label_parts else "<i>(No Label)</i>"
    ...
    def log_message(self, message: str, type_hint: str = "GENERAL"):
        ...
        elif type_hint == "AI_ACTION": formatted_log_entry += f"<span style='color:teal;'><i>(AI Act)</i> {display_message}</span>" # Added for AI actions
    ...
    def _update_matlab_status_display(self, connected, message):
        ...
        style_sheet = f"font-weight: bold; padding: 0px 5px; color: {COLOR_PY_SIM_STATE_ACTIVE if connected else '#C62828'};"; self.matlab_status_label.setStyleSheet(style_sheet)
    ...
    def _run_internet_check_job(self):
        ...
        try:
            s = socket.create_connection((host_to_check, port_to_check), timeout=connection_timeout); s.close(); current_status = True; status_message_detail = "Connected"
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        ...
        style_sheet = f"font-weight: normal; padding: 0px 5px; color: {COLOR_PY_SIM_STATE_ACTIVE if is_connected else '#D32F2F'};"; self.internet_status_label.setStyleSheet(style_sheet)
    ...
    def _update_py_simulation_dock_ui(self):
        ...
        variables = self.py_fsm_engine.get_variables(); self.py_sim_variables_table.setRowCount(0); self.py_sim_variables_table.setRowCount(len(variables))
    ...
    def _append_to_py_simulation_log(self, log_entries: list[str]):
        for entry in log_entries:
            ...
            elif "No eligible transition" in entry or "Event '" in entry and "' is not allowed" in entry or "Event '" in entry and "' is not defined" in entry : cleaned_entry = f"<span style='color:{COLOR_TEXT_SECONDARY};'>{cleaned_entry}</span>" # Added more specific checks for event issues
        ...
        if log_entries:
            ...
            important_keywords = ["Transitioned from", "No eligible transition", "ERROR", "Reset to state", "Simulation started", "Simulation stopped", "not allowed", "not defined"] # Added
    ...
    def on_start_py_simulation(self):
        ...
        try:
            ...
            msg = f"Unexpected error starting Python FSM simulation:\n{type(e).__name__}: {e}"; QMessageBox.critical(self, "Simulation Startup Error", msg); self._append_to_py_simulation_log([f"ERROR Starting Sim: {msg}"])
            self.py_fsm_engine = None; self.set_ui_enabled_for_py_sim(False)
            import traceback; self.log_message(f"Traceback for sim start error: {traceback.format_exc()}", type_hint="GENERAL")
