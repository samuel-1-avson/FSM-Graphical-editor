...
class MainWindow(QMainWindow):
    ...
    def _handle_ai_response(self, ai_message_text: str): # SLOT for ChatbotWorker.responseReady
        ...
        try:
            ...
            if potential_json.startswith("{") and potential_json.endswith("}"):
                try:
                    fsm_data = json.loads(potential_json) # Ensure json is imported in main.py
                    ...
                    print(f"--- MW_HANDLE_RESP ({current_time}): AI response LOOKED like JSON but failed to parse. Treating as text. ---")
                    # Fall through to treat as text if JSON parsing fails even if it looks like JSON
                    message_processed_as_fsm = False 
            else:
                 print(f"--- MW_HANDLE_RESP ({current_time}): AI response does NOT look like JSON. ---")
        except Exception as e: 
            print(f"--- MW_HANDLE_RESP ({current_time}): Exception processing FSM data or checking JSON: {type(e).__name__} - {e} ---")
            self.log_message(f"Error processing potential FSM data: {e}", type_hint="AIChatError")
            self._append_to_ai_chat_display("System Error", f"Could not process potential FSM data: {e}")
            final_status_message = "Status: Error processing response."
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False):
        ...
        if clear_current_diagram:
            # This part is risky as it's not undoable by itself.
            # Consider if this full clear + add should be one large UndoCommand.
            # For now, it behaves as it did.
            # --- Option: Create a "ClearAndLoadCommand" if full undo is needed ---
            # current_diagram_data_for_undo = self.scene.get_diagram_data() # Get data before clear
            # if self.scene.items(): # Only if there's something to clear
            #     cmd_clear = RemoveItemsCommand(self.scene, list(self.scene.items()), "Clear Diagram For AI")
            #     self.undo_stack.push(cmd_clear) # This makes clear undoable
            # --- End Option ---
            self.scene.clear() 
            if not clear_current_diagram: # if we are NOT clearing, we keep the undo stack
                pass # If selectively adding, user manages undos
            else: # If we ARE clearing, then the undo stack also effectively resets for the new content
        ...
        current_x_offset, current_y_offset = 0, 0 # For grid layout relative to start_x, start_y
        items_per_row = 3
        
        # Default dimensions from your application's settings for new states via AI
        default_new_item_width, default_new_item_height = 120, 60
        padding_x, padding_y = 150, 100

        actual_items_to_add_undo_list = [] # For batching into a single undo command if desired
        ...
        for i, state_data in enumerate(fsm_data.get('states', [])):
            ...
            # Auto-positioning based on your previous logic
            # The AI might provide x, y in state_data.get('properties'), decide if you want to use those
            # or always use your auto-layout. For now, using your auto-layout.
            pos_x = start_x + (i % items_per_row) * (default_new_item_width + padding_x)
            pos_y = start_y + (i // items_per_row) * (default_new_item_height + padding_y)

            # --- CRITICAL CHANGE HERE ---
            # Explicitly pass arguments to GraphicsStateItem constructor
            # The names for keyword arguments (text_label, item_color, etc.) MUST match your
            # GraphicsStateItem.__init__ parameter names.
            
            try:
                state_item = GraphicsStateItem(
                    x=pos_x,  # Positional or keyword, depends on GSI. Assume keyword for flexibility matching typical use.
                    y=pos_y,
                    # Width and height for the item's rectangle.
                    # These are likely what QGraphicsRectItem (if it's the base) takes as w, h
                    # Using your default_new_item_width/height from application logic.
                    # The AI *could* provide these in state_data.get('properties', {}).get('width')
                    # but the error said 'width' was unexpected for GraphicsStateItem init
                    # Let's assume they are part of the core constructor, separate from other 'properties'
                    w=state_data.get('width', default_new_item_width),   # Take from AI if provided, else default
                    h=state_data.get('height', default_new_item_height), # Take from AI if provided, else default
                    text_label=name,  # Assuming this is the parameter name in GSI for the state's name
                    is_initial=state_data.get('is_initial', False),
                    is_final=state_data.get('is_final', False),
                    item_color=state_data.get('properties', {}).get('color', COLOR_ITEM_STATE_DEFAULT_BG), # Assuming GSI takes 'item_color'
                    ...
                    # parent=None # Typically not passed if adding to scene directly
                )
                # --- END CRITICAL CHANGE ---
                
                # Instead of pushing one by one, collect them for a batch AddItemCommand
                actual_items_to_add_undo_list.append(state_item)
                state_items_map[name] = state_item
                self.log_message(f"AI Generated State: {name}", type_hint="AI_ACTION")

            except TypeError as te:
                 self.log_message(f"AI Gen Error creating state '{name}': TypeError - {te}. Check GraphicsStateItem constructor.", type_hint="AIChatError")
                 continue # Skip this state
            except Exception as e_state:
                 self.log_message(f"AI Gen Error creating state '{name}': {type(e_state).__name__} - {e_state}", type_hint="AIChatError")
                 continue # Skip this state
        ...
        for trans_data in fsm_data.get('transitions', []):
            ...
            if source_item and target_item:
                # --- ADJUSTMENT FOR GraphicsTransitionItem ---
                # Assuming its constructor takes start_item, end_item positionally,
                # and other properties as keywords.
                # The keywords (event_str, color etc.) MUST match your GraphicsTransitionItem.__init__
                try:
                    trans_item = GraphicsTransitionItem(
                        start_item=source_item, # Assuming parameter name
                        end_item=target_item,   # Assuming parameter name
                        ...
                        # For control points, AI might send 'control_offset_x', 'control_offset_y'
                        # Ensure GraphicsTransitionItem can handle these, possibly in set_control_point_offset
                        # For now, assume they are handled after creation if provided.
                    )
                    
                    # Handle control points if provided by AI
                    ctrl_x = trans_data.get('control_offset_x') 
                    ctrl_y = trans_data.get('control_offset_y')
                    if ctrl_x is not None and ctrl_y is not None and hasattr(trans_item, 'set_control_point_offset'):
                        trans_item.set_control_point_offset(QPointF(float(ctrl_x), float(ctrl_y)))
                    
                    # --- END ADJUSTMENT ---
                    actual_items_to_add_undo_list.append(trans_item)
                    self.log_message(f"AI Generated Transition: {source_name} -> {target_name}", type_hint="AI_ACTION")
                
                except TypeError as te_trans:
                    self.log_message(f"AI Gen Error creating transition '{source_name}->{target_name}': TypeError - {te_trans}. Check GraphicsTransitionItem constructor.", type_hint="AIChatError")
                    continue
                except Exception as e_trans:
                    self.log_message(f"AI Gen Error creating transition '{source_name}->{target_name}': {type(e_trans).__name__} - {e_trans}", type_hint="AIChatError")
                    continue
        ...
        # Add Comments (Optional) - Assuming GraphicsCommentItem constructor is (x, y, text)
        # Layout comments below states
        last_y_pos = start_y + ((len(fsm_data.get('states', [])) // items_per_row) +1 ) * (default_new_item_height + padding_y) if fsm_data.get('states') else start_y
        ...
        for i, comment_data in enumerate(fsm_data.get('comments', [])):
            ...
            # AI might provide 'x', 'y'. If not, use auto-layout.
            comment_x = comment_data.get('x', comment_start_x + i * (150 + 20) ) # Basic horizontal list
            comment_y = comment_data.get('y', last_y_pos)

            try:
                comment_item = GraphicsCommentItem(comment_x, comment_y, text)
                # If AI can specify width for comment:
                comment_width = comment_data.get('width')
                if comment_width and hasattr(comment_item, 'setTextWidth'):
                    try:
                ...
                actual_items_to_add_undo_list.append(comment_item)
                self.log_message(f"AI Generated Comment: {text[:30]}...", type_hint="AI_ACTION")
            except TypeError as te_comm:
                 self.log_message(f"AI Gen Error creating comment: TypeError - {te_comm}. Check GraphicsCommentItem constructor.", type_hint="AIChatError")
                 continue
            except Exception as e_comm:
                 self.log_message(f"AI Gen Error creating comment: {type(e_comm).__name__} - {e_comm}", type_hint="AIChatError")
                 continue

        # Add all collected items via a single Undo Command (or multiple if preferred)
        if actual_items_to_add_undo_list:
            # Option 1: Add all as a single command (if AddItemCommand supports list)
            # Assuming AddItemCommand might need to be modified to take a list or
            # you loop and create one command per item.
            # For now, assume one command for potentially multiple items.
            # A better AddItemsCommand (plural) would be good for this.
            
            # If AddItemCommand only takes one item, loop:
            for item_to_add in actual_items_to_add_undo_list:
                add_cmd = AddItemCommand(self.scene, item_to_add, f"Add AI {type(item_to_add).__name__.replace('Graphics','').replace('Item','')}")
                self.undo_stack.push(add_cmd)
            # Alternative (if you create a new AddItemsCommand for lists):
            # if actual_items_to_add_undo_list:
            #    batch_add_cmd = AddItemsCommand(self.scene, actual_items_to_add_undo_list, "Add AI Generated FSM")
            #    self.undo_stack.push(batch_add_cmd)

        if actual_items_to_add_undo_list: # Only set dirty and center if items were added
            self.scene.set_dirty(True)
            # Fit view to all items OR center on the new items' collective bounding box
            new_items_bounds = QRectF()
            for item in actual_items_to_add_undo_list:
                if new_items_bounds.isNull():
                    new_items_bounds = item.sceneBoundingRect()
                else:
                    new_items_bounds = new_items_bounds.united(item.sceneBoundingRect())
            
            if not new_items_bounds.isNull() and self.view :
                self.view.fitInView(new_items_bounds.adjusted(-50, -50, 50, 50), Qt.KeepAspectRatio)
            elif self.view: # Fallback if somehow bounds are null but items added
