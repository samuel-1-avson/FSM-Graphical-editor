...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # This should come AFTER all UI docks and managers are fully set up
        self._update_window_title() # Initial title set
        self.on_new_file(silent=True) # <<< MOVED HERE, this triggers validation
        ...
        # Initialize AI Chatbot status display (can stay here or move after on_new_file)
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        # ... (prompts and stopping other services as before) ...
        if not self._prompt_ide_save_if_dirty(): 
            pass# ...
        if not self._prompt_save_if_dirty():
            pass# ...
        ...
        if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
            ...
            # 1. Store worker reference for NVML shutdown if it exists
            if self.resource_monitor_worker:
                worker_ref_for_nvml_shutdown = self.resource_monitor_worker

            # 2. Request interruption for the worker's loop condition
            self.resource_monitor_thread.requestInterruption()
            logger.debug("MW_CLOSE: Interruption requested on resource monitor thread.")

            # 3. Signal the worker to set its internal _stop_requested flag
            #    This helps if the worker uses this flag in addition to isInterruptionRequested
            if self.resource_monitor_worker:
            ...
            # 4. Explicitly ask the thread's event loop to quit.
            #    This is important. The worker's finished_signal also does this,
            #    but calling it here ensures the request is made.
