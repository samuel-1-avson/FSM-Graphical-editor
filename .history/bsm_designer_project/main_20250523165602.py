...
from dialogs import (StatePropertiesDialog, TransitionPropertiesDialog, CommentPropertiesDialog,
                     MatlabSettingsDialog, GeneralSettingsDialog) # ADDED GeneralSettingsDialog
...
class DiagramScene(QGraphicsScene):
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            ...
            self.last_right_click_pos = pos # Store position for context menu actions
            if top_item_at_pos and isinstance(top_item_at_pos, (GraphicsStateItem, GraphicsTransitionItem, GraphicsCommentItem)):
                if not top_item_at_pos.isSelected():
                    self.clearSelection()
                    top_item_at_pos.setSelected(True)
                self._show_item_context_menu(top_item_at_pos, event.screenPos()) # Renamed for clarity
            else: # Right-click on empty space
                self.clearSelection()
                self._show_scene_context_menu(event.screenPos()) # NEW METHOD CALL
        else:
            super().mousePressEvent(event)        
                
       
       
    def _show_item_context_menu(self, item, global_pos): # Renamed from _show_context_menu
        menu = QMenu()
        ...
        # ... (potentially add Bring to Front/Send to Back here later)
    ...
    def _show_scene_context_menu(self, global_pos):
        ...
        menu.addSeparator()
        select_all_action = menu.addAction("Select All") # Could use an icon

        action = menu.exec_(global_pos)

        # Use self.last_right_click_pos for positioning new items
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._load_persistent_settings() # NEW METHOD CALL
    ...
    def _create_actions(self):
        ...
        self.general_settings_action = QAction(get_standard_icon(QStyle.SP_FileDialogDetailedView, "Prefs"), # Example icon
                                         "&Preferences...", self,
                                         statusTip="Configure application settings",
                                         triggered=self.on_general_settings)
    ...
    def _load_persistent_settings(self):
        settings = QSettings(APP_NAME, "SettingsDialog") # Use same identifiers as in dialog
    ...
    # Load Appearance Settings
        ...
        if font_size <= 0: font_size = 9 # Default

        self.scene.grid_visible = grid_visible
        self.scene.grid_pen_light.setColor(QColor(minor_color))
        self.scene.grid_pen_dark.setColor(QColor(major_color))

        app_font = QApplication.instance().font()
        if app_font.pointSize() != font_size:
            app_font.setPointSize(font_size)
            QApplication.instance().setFont(app_font)
        # Reapply stylesheet to reflect font changes on existing widgets
            self.setStyleSheet("") 
    ...
    # Load AI Settings
        ...
        model_name = settings.value("ai/model_name", "gpt-3.5-turbo", type=str) # Default model
        if api_key: # Only set if a key was actually saved
            self.ai_chatbot_manager.set_api_key(api_key)
        if self.ai_chatbot_manager.chatbot_worker:
            self.ai_chatbot_manager.chatbot_worker.model_name = model_name
        else: # If worker not yet created (e.g. no API key yet), store for when it is
            self.ai_chatbot_manager.default_model_name = model_name # Need to add this attribute to AIChatbotManager

    # Load MATLAB Path (if feature is used)
        if hasattr(self, 'matlab_connection'):
            ...
            # Optionally, trigger a connection test silently or based on another setting
            # self.matlab_connection.test_connection()
    
        self.scene.update() # Update scene to reflect grid changes
    ...
    def _create_menus(self):
        ...
        edit_menu.addSeparator()
        edit_menu.addSeparator()
        edit_menu.addAction(self.general_settings_action)
    ...
    def on_general_settings(self):
        from dialogs import GeneralSettingsDialog # Import here
        dialog = GeneralSettingsDialog(self, self)
        dialog.exec_()        
