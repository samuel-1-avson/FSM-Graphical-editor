Okay, I'll provide the full updated code for `bsm_designer_project/main.py` reflecting **Improvement 1: Reduce Initial Clutter (Dock Management)**.

The other files (`graphics_scene.py` and `ui_py_simulation_manager.py`) are not directly modified by Improvement 1, so their content from the initial prompt remains the same for this specific step.
