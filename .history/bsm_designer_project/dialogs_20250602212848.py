...
from config import (
    ...
    APP_FONT_SIZE_SMALL, COLOR_TEXT_SECONDARY, COLOR_BACKGROUND_DIALOG,COLOR_ACCENT_SUCCESS, COLOR_ACCENT_ERROR, COLOR_ACCENT_PRIMARY,
...
class FindItemDialog(QDialog):
    item_selected_for_focus = pyqtSignal(QGraphicsItem) # Emit the actual item for more flexible focusing

    def __init__(self, parent=None, scene_ref=None): 
        super().__init__(parent)
        self.scene_ref = scene_ref
        self.setWindowTitle("Find Item") # MODIFIED title
        ...
        self.setMinimumWidth(350) # Slightly wider for more info
        self.setStyleSheet(f"QDialog {{ background-color: {COLOR_BACKGROUND_DIALOG}; }}")

        layout = QVBoxLayout(self)
        layout.setContentsMargins(10,10,10,10); layout.setSpacing(8)

        self.search_label = QLabel("Search for FSM Element (Text in Name, Event, Action, Comment, etc.):") # MODIFIED label
    ...
    def _get_item_display_text(self, item: QGraphicsItem) -> str:
        if isinstance(item, GraphicsStateItem):
            return f"State: {item.text_label}"
    ...
    def _populate_initial_list(self): # Shows all items initially, could be filtered too
        ...
        for item in self.scene_ref.items():
            if isinstance(item, (GraphicsStateItem, GraphicsTransitionItem, GraphicsCommentItem)):
                ...
                list_item.setData(Qt.UserRole, QVariant(item)) # Store the actual item reference
    ...
    def _update_results_list(self):
        ...
        if not search_term: # If search is empty, show all items (or initial list)
            self._populate_initial_list()
            return
        ...
        for item in self.scene_ref.items():
            ...
            if isinstance(item, GraphicsStateItem):
                props = item.get_data()
                searchable_text = (f"{props.get('name','')} "
                                   f"{props.get('entry_action','')} "
                                   f"{props.get('during_action','')} "
                                   f"{props.get('exit_action','')} "
                                   f"{props.get('description','')}").lower()
            elif isinstance(item, GraphicsTransitionItem):
                props = item.get_data()
                searchable_text = (f"{props.get('event','')} "
                                   f"{props.get('condition','')} "
                                   f"{props.get('action','')} "
                                   f"{props.get('description','')}").lower()
            elif isinstance(item, GraphicsCommentItem):
            ...
            if item_matches:
                list_item = QListWidgetItem(self._get_item_display_text(item))
                list_item.setData(Qt.UserRole, QVariant(item)) # Store actual item
    ...
    def _on_item_activated(self, list_item_widget: QListWidgetItem):
        if list_item_widget:
            ...
            if stored_item_variant is not None:
                ...
                if actual_item:
                    self.item_selected_for_focus.emit(actual_item)
                    # self.accept() # Or self.hide()
                    # For this extended find, let's keep it open after focusing
                    # self.hide() 
    ...
    def _on_return_pressed(self):
        if self.results_list.count() > 0:
            ...
            if current_or_first_item:
                self._on_item_activated(current_or_first_item)

    def refresh_list(self):
        """Call this if the scene items change while the dialog is open."""
        current_search = self.search_input.text() 
        # _populate_initial_list() # This will clear and re-add all
        # self.search_input.setText(current_search) # Re-apply search to filter
        self._update_results_list() # This will re-filter based on current_search
    ...
    def keyPressEvent(self, event: QKeyEvent): 
        if event.key() == Qt.Key_Escape:
            ...
            # Let QListWidget handle the actual movement after focus is set
            # QListWidget.keyPressEvent(self.results_list, event) # This might be problematic if not subclassing.
            # Instead, rely on default QListWidget behavior once it has focus.
        else:
