...
class DraggableToolButton(QPushButton):
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self.mime_type == MIME_TYPE_BSM_TEMPLATE:
            try: template_obj = json.loads(self.item_type_data_str); mime_data.setText(f"FSM Template: {template_obj.get('name', 'Custom Template')}")
            except json.JSONDecodeError: mime_data.setText("FSM Template (Invalid JSON)")
        else: mime_data.setText(self.item_type_data_str)
...
class MainWindow(QMainWindow):
    ...
    def _connect_signals(self):
        if hasattr(self, 'scene') and self.scene:
            ...
            self.scene.modifiedStatusChanged.connect(self.update_window_title) # Corrected name
    ...
    def init_ui(self):
        ...
        if self.view_manager: # Connect after actions are created
        ...
        self._update_save_actions_enable_state() # Call after status bar elements are created
    ...
    def _create_actions(self):
        ...
        if self.file_op_manager: self.save_action.triggered.connect(self.file_op_manager.on_save_file)
        ...
        if self.file_op_manager: self.save_as_action.triggered.connect(self.file_op_manager.on_save_file_as)
        ...
        if self.view_manager: # Connect after actions are created
            self.view_manager.connect_zoom_actions()
            self.view_manager.connect_snap_actions()
        else: logger.error("ViewManager not initialized when trying to connect actions in _create_actions.")
    ...
    def _create_menus(self):
        ...
        if hasattr(self, 'find_item_action'): edit_menu.addAction(self.find_item_action); edit_menu.addSeparator()
        ...
        if hasattr(self, 'align_right_action'): align_menu.addAction(self.align_right_action); align_menu.addSeparator()
        ...
        if hasattr(self, 'reset_py_sim_action'): py_sim_menu.addAction(self.reset_py_sim_action); sim_menu.addSeparator()
        ...
        if hasattr(self, 'generate_matlab_code_action'): matlab_sim_menu.addAction(self.generate_matlab_code_action); matlab_sim_menu.addSeparator()
        ...
        if hasattr(self, 'show_snap_guidelines_action'): snap_menu.addAction(self.show_snap_guidelines_action); self.view_menu.addSeparator()
        ...
        if hasattr(self, 'clear_ai_chat_action'): ai_menu.addAction(self.clear_ai_chat_action); ai_menu.addSeparator()
    ...
    def _create_toolbars(self):
        ...
        edit_toolbar = self.addToolBar("Edit"); edit_toolbar.setObjectName("EditToolBar"); edit_toolbar.setIconSize(icon_size); edit_toolbar.setToolButtonStyle(tb_style)
        if hasattr(self, 'undo_action'): edit_toolbar.addAction(self.undo_action)
        if hasattr(self, 'redo_action'): edit_toolbar.addAction(self.redo_action); edit_toolbar.addSeparator()
        ...
        tools_tb = self.addToolBar("Interaction Tools"); tools_tb.setObjectName("ToolsToolBar"); tools_tb.setIconSize(icon_size); tools_tb.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
        ...
        code_gen_export_toolbar = self.addToolBar("Code Generation & Export"); code_gen_export_toolbar.setObjectName("CodeGenExportToolBar"); code_gen_export_toolbar.setIconSize(icon_size); code_gen_export_toolbar.setToolButtonStyle(tb_style)
        ...
        sim_toolbar = self.addToolBar("Simulation Tools"); sim_toolbar.setObjectName("SimulationToolBar"); sim_toolbar.setIconSize(icon_size); sim_toolbar.setToolButtonStyle(tb_style)
        ...
        if hasattr(self, 'reset_py_sim_action'): sim_toolbar.addAction(self.reset_py_sim_action); sim_toolbar.addSeparator()
        if hasattr(self, 'run_simulation_action'): sim_toolbar.addAction(self.run_simulation_action)
        view_toolbar = self.addToolBar("View Tools"); view_toolbar.setObjectName("ViewToolBar"); view_toolbar.setIconSize(icon_size); view_toolbar.setToolButtonStyle(tb_style)
        ...
        if hasattr(self, 'reset_zoom_action'): view_toolbar.addAction(self.reset_zoom_action); view_toolbar.addSeparator()
        ...
        align_toolbar = self.addToolBar("Alignment & Distribution"); align_toolbar.setObjectName("AlignDistributeToolBar"); align_toolbar.setIconSize(icon_size); align_toolbar.setToolButtonStyle(tb_style)
        ...
        if hasattr(self, 'align_right_action'): align_toolbar.addAction(self.align_right_action); align_toolbar.addSeparator()
        ...
        if hasattr(self, 'align_bottom_action'): align_toolbar.addAction(self.align_bottom_action); align_toolbar.addSeparator()
    ...
    def _create_docks(self):
        ...
        self.properties_dock_main_layout = QVBoxLayout(self.properties_dock_widget_main); self.properties_dock_main_layout.setContentsMargins(8,8,8,8); self.properties_dock_main_layout.setSpacing(6)
        ...
        if self.ide_manager:
            ...
            else: logger.error("Failed to setup IDE dock via IDEManager in _create_docks.")
    ...
    def _add_fsm_data_to_scene(self, fsm_data: dict, clear_current_diagram: bool = False, original_user_prompt: str = "AI Generated FSM"):
        ...
        if clear_current_diagram:
            if not self.on_new_file_orchestrator(silent=True): logger.warning("MW: Clearing diagram cancelled by user (save prompt). Cannot add AI FSM."); return
            logger.info("MW: Cleared diagram before AI generation.")
        if not clear_current_diagram and self.undo_stack : self.undo_stack.beginMacro(f"Add AI FSM: {original_user_prompt[:30]}...")
        ...
        if not clear_current_diagram and self.undo_stack: self.undo_stack.endMacro()
        if self.py_sim_active and items_to_add_for_undo_command:
            ...
            try: (self.py_sim_ui_manager.on_stop_py_simulation(silent=True) if self.py_sim_ui_manager else None); (self.py_sim_ui_manager.on_start_py_simulation() if self.py_sim_ui_manager else None); (self.py_sim_ui_manager.append_to_action_log(["Python FSM Simulation reinitialized for new diagram from AI."]) if self.py_sim_ui_manager else None)
            except FSMError as e: (self.py_sim_ui_manager.append_to_action_log([f"ERROR Re-initializing Sim after AI: {e}"]) if self.py_sim_ui_manager else None); (self.py_sim_ui_manager.on_stop_py_simulation(silent=True) if self.py_sim_ui_manager else None)
        logger.debug("MW: ADD_FSM_TO_SCENE processing finished. Items involved: %d", len(items_to_add_for_undo_command)); (self.scene.scene_content_changed_for_find.emit() if self.scene else None)

    def _fit_view_to_new_ai_items(self):
        if not self.scene or not self.scene.items(): return
    ...
    def _update_properties_dock(self):
        if not hasattr(self, 'properties_editor_layout') or not hasattr(self, 'properties_placeholder_label') or not hasattr(self, 'properties_editor_container') or not hasattr(self, 'properties_edit_dialog_button') or not hasattr(self, '_dock_property_editors'): logger.warning("MainWindow._update_properties_dock: Properties dock UI elements not fully initialized."); return
        selected_items = []
        if hasattr(self, 'scene') and self.scene: selected_items = self.scene.selectedItems()
        else: logger.warning("MainWindow._update_properties_dock: Scene not available."); self.properties_editor_container.setVisible(False); self.properties_placeholder_label.setText("<i>Diagram scene not available.</i>"); self.properties_placeholder_label.setVisible(True); self.properties_edit_dialog_button.setEnabled(False); self.properties_apply_button.setEnabled(False); self.properties_revert_button.setEnabled(False); self._current_edited_item_in_dock = None; return
        ...
        if len(selected_items) == 1:
            ...
            else: self.properties_placeholder_label.setText(f"<i>Editing: {type(self._current_edited_item_in_dock).__name__}.<br><span style='font-size:{APP_FONT_SIZE_SMALL}; color:{COLOR_TEXT_SECONDARY};'>Dock editor not fully implemented. Use 'Advanced Edit...'.</span></i>"); self.properties_editor_container.setVisible(False); self.properties_placeholder_label.setVisible(True); self.properties_edit_dialog_button.setEnabled(True)
        elif len(selected_items) > 1: self.properties_placeholder_label.setText(f"<i><b>{len(selected_items)} items selected.</b><br><span style='font-size:{APP_FONT_SIZE_SMALL}; color:{COLOR_TEXT_SECONDARY};'>Select a single item to edit.</span></i>"); self.properties_editor_container.setVisible(False); self.properties_placeholder_label.setVisible(True); self.properties_edit_dialog_button.setEnabled(False)
        else: self.properties_placeholder_label.setText(f"<i>No item selected.</i><br><span style='font-size:{APP_FONT_SIZE_SMALL}; color:{COLOR_TEXT_SECONDARY};'>Click an item or use tools.</span>"); self.properties_editor_container.setVisible(False); self.properties_placeholder_label.setVisible(True); self.properties_edit_dialog_button.setEnabled(False)
        if hasattr(self, 'properties_apply_button'): self.properties_apply_button.setEnabled(False)
        if hasattr(self, 'properties_revert_button'): self.properties_revert_button.setEnabled(False)
    ...
    def _on_apply_dock_properties(self):
        ...
        else: logger.warning("Properties Dock: Apply clicked but no item is currently being edited in the dock.")
    ...
    def _update_save_actions_enable_state(self):
        if hasattr(self, 'save_action') and self.save_action is not None and hasattr(self, 'scene') and self.scene is not None:
            self.save_action.setEnabled(self.scene.is_dirty())
        else: logger.warning("MainWindow._update_save_actions_enable_state: save_action or scene not available.")

    @pyqtSlot()
    def _update_undo_redo_actions_enable_state(self):
        if hasattr(self, 'undo_action') and self.undo_action is not None:
            self.undo_action.setEnabled(self.undo_stack.canUndo()); undo_text = self.undo_stack.undoText(); self.undo_action.setText(f"&Undo{(' ' + undo_text) if undo_text else ''}"); self.undo_action.setToolTip(f"Undo: {undo_text}" if undo_text else "Undo")
        else: logger.warning("MainWindow._update_undo_redo_actions_enable_state: self.undo_action not found or not initialized.")
        if hasattr(self, 'redo_action') and self.redo_action is not None:
            self.redo_action.setEnabled(self.undo_stack.canRedo()); redo_text = self.undo_stack.redoText(); self.redo_action.setText(f"&Redo{(' ' + redo_text) if redo_text else ''}"); self.redo_action.setToolTip(f"Redo: {redo_text}" if redo_text else "Redo")
        else: logger.warning("MainWindow._update_undo_redo_actions_enable_state: self.redo_action not found or not initialized.")
    ...
    def on_delete_selected(self):
        ...
        else: logger.warning("MainWindow.on_delete_selected: Scene not available.")

    @pyqtSlot()
    def on_show_find_item_dialog(self):
        if not self.find_item_dialog:
            if hasattr(self, 'scene') and self.scene:
                self.find_item_dialog = FindItemDialog(parent=self, scene_ref=self.scene); self.find_item_dialog.item_selected_for_focus.connect(self.focus_on_item); self.scene.scene_content_changed_for_find.connect(self._refresh_find_dialog_if_visible)
    ...
    def on_toggle_state_breakpoint(self, state_item: GraphicsStateItem, set_bp: bool):
        if not self.py_fsm_engine or not self.py_sim_active: QMessageBox.information(self, "Simulation Not Active", "Breakpoints can only be managed during an active Python simulation."); (self.sender() and isinstance(self.sender(), QAction) and self.sender().setChecked(not set_bp)); return
        state_name = state_item.text_label; action_text = ""
        if set_bp: self.py_fsm_engine.add_state_breakpoint(state_name); current_tooltip = state_item.toolTip(); (not "[BP]" in current_tooltip and state_item.setToolTip(f"{current_tooltip}\n[Breakpoint Set]" if current_tooltip else f"State: {state_name}\n[Breakpoint Set]")); action_text = f"Breakpoint SET for state: {state_name}"
        else: self.py_fsm_engine.remove_state_breakpoint(state_name); state_item.setToolTip(state_item.toolTip().replace("\n[Breakpoint Set]", "")); action_text = f"Breakpoint CLEARED for state: {state_name}"
        state_item.update()
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
            logger.info("MW_CLOSE: Attempting to stop resource monitor worker and thread..."); worker_ref_for_nvml_shutdown = self.resource_monitor_worker
            ...
            if not self.resource_monitor_thread.wait(2000): logger.warning("MW_CLOSE: Resource monitor thread did not finish gracefully. Terminating."); self.resource_monitor_thread.terminate(); self.resource_monitor_thread.wait(500)
        ...
        for manager_attr in ['view_manager', 'file_op_manager', 'ide_manager', 'matlab_op_manager', 'ai_chat_ui_manager', 'py_sim_ui_manager', 'resource_monitor_worker', 'resource_monitor_thread']:
            manager = getattr(self, manager_attr, None)
            if manager and isinstance(manager, QObject): # Check if it's a QObject before calling deleteLater
                if hasattr(manager, 'cleanup') and callable(manager.cleanup): manager.cleanup()
                manager.deleteLater(); setattr(self, manager_attr, None)
    ...
    def update_window_title(self):
        ...
        if self.ide_manager and self.ide_manager.ide_dock:
            ...
            if self.ide_manager.current_ide_file_path: ide_fn = os.path.basename(self.ide_manager.current_ide_file_path); ide_dock_title = f"IDE: {ide_fn}{'*' if self.ide_manager.ide_editor_is_dirty else ''}{lang_info}"; ide_simple_status_for_main_title = f"IDE: {ide_fn}{'*' if self.ide_manager.ide_editor_is_dirty else ''}"
        ...
        main_window_is_dirty = (hasattr(self.scene, 'is_dirty') and self.scene.is_dirty()) or (self.ide_manager and self.ide_manager.ide_editor_is_dirty)
        self.setWindowModified(main_window_is_dirty)
        title = f"{APP_NAME} - {file_name}{sim_status_suffix} [*]"; self.setWindowTitle(title)
        if hasattr(self, 'status_label'):
            main_file_status = f"File: {file_name}{' *' if hasattr(self.scene, 'is_dirty') and self.scene.is_dirty() else ''}"; pysim_status = f"PySim: {'Active' if self.py_sim_active else 'Idle'}"
    ...
    def _update_ai_features_enabled_state(self, is_online_and_key_present: bool):
        ...
        if hasattr(self, 'ide_analyze_action') and self.ide_manager and self.ide_manager.ide_language_combo:
            current_ide_lang = self.ide_manager.ide_language_combo.currentText(); can_analyze_ide = (current_ide_lang == "Python" or current_ide_lang.startswith("C/C++")) and is_online_and_key_present; self.ide_analyze_action.setEnabled(can_analyze_ide)
    ...
    def _update_internet_status_display(self, is_connected: bool, message_detail: str):
        ...
        if hasattr(self, 'internet_status_label'):
            self.internet_status_label.setText(full_status_text); self.internet_status_label.setToolTip(f"Internet Status: {message_detail} (Checks connection to 8.8.8.8:53)")
    ...
    def _update_align_distribute_actions_enable_state(self):
        selected_count = len(self.scene.selectedItems()) if hasattr(self.scene, 'selectedItems') else 0
    ...
    def on_align_items(self, mode: str):
        selected_items = [item for item in self.scene.selectedItems() if isinstance(item, (GraphicsStateItem, GraphicsCommentItem))] if hasattr(self.scene, 'selectedItems') else []
        ...
        for item in selected_items:
            ...
            if isinstance(item, GraphicsStateItem) and hasattr(self.scene, '_update_connected_transitions'): self.scene._update_connected_transitions(item)
        if moved_items_data_for_command and self.undo_stack: cmd = MoveItemsCommand(moved_items_data_for_command, f"Align {mode.replace('_', ' ').title()}"); self.undo_stack.push(cmd); (self.scene.set_dirty(True) if self.scene else None)
    ...
    def on_distribute_items(self, mode: str):
        ...
        for item in selected_items:
            ...
            if (new_pos - old_pos).manhattanLength() > 0.1: moved_items_data_for_command.append((item, old_pos, new_pos))
            if isinstance(item, GraphicsStateItem) and hasattr(self.scene, '_update_connected_transitions'): self.scene._update_connected_transitions(item)
        if moved_items_data_for_command and self.undo_stack: cmd_text = "Distribute Horizontally" if mode == "horizontal" else "Distribute Vertically"; cmd = MoveItemsCommand(moved_items_data_for_command, cmd_text); self.undo_stack.push(cmd); (self.scene.set_dirty(True) if self.scene else None)

    def log_message(self, level_str: str, message: str): # Centralized logging call
        level = getattr(logging, level_str.upper(), logging.INFO)
        logger.log(level, message)

    @pyqtSlot(str) # For PySimUIManager to call
    def focus_on_state_by_name(self, state_name: str):
        item_to_focus = self.scene.get_state_by_name(state_name) if self.scene else None
    ...
    @pyqtSlot(list) # Connected to scene.validation_issues_updated
    def update_problems_dock(self, issues_with_items: list):
        if not hasattr(self, 'problems_list_widget') or self.problems_list_widget is None:
            # This can happen if this slot is called before _create_docks fully initializes problems_list_widget
            # For example, if initial validation runs very early.
            logger.warning("MainWindow.update_problems_dock: self.problems_list_widget is not yet initialized. Update deferred.")
            # QTimer.singleShot(100, lambda: self.update_problems_dock(issues_with_items)) # Optionally retry
            return
        ...
        if issues_with_items:
            for issue_msg, item_ref in issues_with_items:
                ...
                if item_ref and isinstance(item_ref, QGraphicsItem): # Ensure item_ref is a QGraphicsItem
                    list_item_widget.setData(Qt.UserRole, item_ref)
                else:
                    # Store a special value or None if item_ref is not a QGraphicsItem
                    list_item_widget.setData(Qt.UserRole, None)
            ...
            # Optionally show/raise the dock if it's hidden and has issues
            if hasattr(self, 'problems_dock') and self.problems_dock.isHidden() and len(issues_with_items) > 0:
    ...
    def on_problem_item_double_clicked(self, list_item: QListWidgetItem):
        item_ref = list_item.data(Qt.UserRole) # Retrieve the stored QGraphicsItem or None
        if item_ref and isinstance(item_ref, QGraphicsItem) and hasattr(self.scene, 'items') and item_ref in self.scene.items(): # Check if item is still in scene
        ...
        elif item_ref and not (isinstance(item_ref, QGraphicsItem) and hasattr(self.scene, 'items') and item_ref in self.scene.items()):
            logger.warning(f"Problem item '{list_item.text()}' reference is invalid or no longer in scene. Cannot focus.")
            QMessageBox.information(self, "Item Not Found", "The item related to this issue might have been removed or changed.")
        else: # item_ref was None
            logger.debug(f"No QGraphicsItem reference stored for validation issue: '{list_item.text()}'")


    @pyqtSlot(QGraphicsItem) # Make sure this slot is also present
    def focus_on_item(self, item_to_focus: QGraphicsItem):
        if not (hasattr(self, 'scene') and self.scene):
            logger.warning("MainWindow.focus_on_item: Scene not available.")
            return
        if item_to_focus and item_to_focus.scene() == self.scene:
            ...
            if isinstance(item_to_focus, GraphicsStateItem):
                ...
                text_content = item_to_focus.toPlainText()
                display_name = f"Comment: {text_content[:30] + '...' if len(text_content) > 30 else text_content}"


            self.log_message("INFO", f"Focused on {display_name}")
            # if self.find_item_dialog and not self.find_item_dialog.isHidden():
            #      self.find_item_dialog.hide() # Optionally hide find dialog if open
        else:
            self.log_message("WARNING", f"Could not find or focus on the provided item in the current scene: {item_to_focus}")

    @pyqtSlot()
    def on_generate_c_code(self): # This is the direct C code generator
        if not hasattr(self, 'scene') or not self.scene or not self.scene.items():
            QMessageBox.information(self, "Empty Diagram", "Cannot generate code for an empty diagram.")
            return

        default_filename_base = "fsm_generated"
        if self.current_file_path: # Use MainWindow's current_file_path
            default_filename_base = os.path.splitext(os.path.basename(self.current_file_path))[0]
        
        # Sanitize for C filename
        default_filename_base = "".join(c if c.isalnum() or c == '_' else '_' for c in default_filename_base)
        if not default_filename_base or not default_filename_base[0].isalpha(): # Ensure starts with letter
        ...
        if output_dir:
            ...
            if ok and filename_base.strip():
                filename_base_stripped = filename_base.strip()
                # Further sanitize the user input
                filename_base_sanitized = "".join(c if c.isalnum() or c == '_' else '_' for c in filename_base_stripped)
                if not filename_base_sanitized or not filename_base_sanitized[0].isalpha():
                ...
                try:
                    c_file_path, h_file_path = generate_c_code_files(diagram_data, output_dir, filename_base_sanitized)
                    QMessageBox.information(self, "C Code Generation Successful", 
                                            f"Generated files:\n{c_file_path}\n{h_file_path}")
                    logger.info(f"C code generated successfully to {output_dir} with base name {filename_base_sanitized}")
                except ValueError as ve: # Catch specific errors from generator if defined
                    QMessageBox.critical(self, "C Code Generation Error", f"Failed to generate C code: {ve}")
                    logger.error(f"Error generating C code (ValueError): {ve}", exc_info=True)
                except Exception as e:
                    QMessageBox.critical(self, "C Code Generation Error", f"An unexpected error occurred during C code generation: {e}")
                    logger.error(f"Error generating C code (Exception): {e}", exc_info=True)
            elif ok: # User pressed OK but left the field empty
                QMessageBox.warning(self, "Invalid Filename", "Base filename cannot be empty.")
        # else: User cancelled directory selection, do nothing

    @pyqtSlot(str, str) # For GraphicsStateItem.signals.textChangedViaInlineEdit
    def _handle_state_renamed_inline(self, old_name: str, new_name: str):
        ...
        if hasattr(self, '_refresh_find_dialog_if_visible'): self._refresh_find_dialog_if_visible()
        if hasattr(self.scene, 'selectedItems') and self.scene.selectedItems() and len(self.scene.selectedItems()) == 1 and isinstance(self.scene.selectedItems()[0], GraphicsStateItem) and self.scene.selectedItems()[0].text_label == new_name:
            if hasattr(self, '_update_properties_dock'): self._update_properties_dock()

    def connect_state_item_signals(self, state_item: GraphicsStateItem): # Called by FileOpManager and Scene
    ...
    # ... (The methods that were added back correctly in previous steps, like on_problem_item_double_clicked, _update_properties_dock, etc., should be here)
    # ... (The methods that are now correctly in managers should be removed from here)
