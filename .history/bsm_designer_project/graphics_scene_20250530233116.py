...
from PyQt5.QtGui import QKeyEvent, QKeySequence, QCursor
...
MIME_TYPE_BSM_TEMPLATE = "application/x-bsm-template" # For FSM Templates
...
class DiagramScene(QGraphicsScene):
    ...
    def dragEnterEvent(self, event: QGraphicsSceneDragDropEvent):
        if event.mimeData().hasFormat("application/x-bsm-tool") or \
           event.mimeData().hasFormat(MIME_TYPE_BSM_TEMPLATE): # Accept templates too
    ...
    def dropEvent(self, event: QGraphicsSceneDragDropEvent):
        ...
        if mime_data.hasFormat(MIME_TYPE_BSM_TEMPLATE):
            ...
            try:
                ...
                self._add_template_to_scene(template_data, pos) # This will emit scene_content_changed_for_find
                event.acceptProposedAction()
    ...
    def _add_template_to_scene(self, template_data: dict, drop_pos: QPointF):
        ...
        if self.get_state_by_name(first_state_name_in_template):
            i = 1
            # Check if any state name from the template with the current suffix already exists
