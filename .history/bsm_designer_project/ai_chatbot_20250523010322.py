...
class AIChatbotManager:
    def __init__(self, parent_window):
        ...
        self.chatbot_thread = None # Initialize as None
        self.chatbot_worker = None # Initialize as None
    ...
    def _cleanup_existing_worker_and_thread(self):
        if self.chatbot_thread and self.chatbot_thread.isRunning():
            print("--- MGR: Cleaning up existing worker and thread ---")
            self.chatbot_thread.quit()
            if not self.chatbot_thread.wait(3000):  # Wait 3 seconds
                print("--- MGR WARN: Chatbot thread did not quit gracefully. Terminating. ---")
                self.chatbot_thread.terminate()
                self.chatbot_thread.wait()
            print("--- MGR: Existing thread stopped. ---")
        
        if self.chatbot_worker:
            # Explicitly try to disconnect signals to prevent old connections lingering
            # This is a safeguard; ideally, a new worker shouldn't reuse old signal slots.
            try:
                ...
                print("--- MGR: Disconnected signals from old worker. ---")
            except TypeError: # Happens if a signal was not connected
                pass # Or already disconnected
            except RuntimeError as e: # Can happen if object is being deleted
                 print(f"--- MGR WARN: Error disconnecting signals from old worker: {e} ---")

            self.chatbot_worker.deleteLater() # Schedule the C++ object for deletion
            print("--- MGR: Old worker scheduled for deletion. ---")
