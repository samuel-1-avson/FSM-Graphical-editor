...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # self.ide_code_editor: CodeEditor | None = None # Will be per-tab
        # self.current_ide_file_path: str | None = None # Will be per-tab
        # self.ide_output_console: QTextEdit | None = None # Will be per-tab
        # self.ide_editor_is_dirty = False # Will be per-tab
        ...
        # self.ide_editor_is_dirty is now per-tab
        self._find_replace_dialog_instance: FindReplaceDialog | None = None # To manage a single instance
        # --- CRITICAL ORDERING ---
        self.init_ui() 
        ...
        # self.on_new_file(silent=True) # This will be handled by _add_ide_tab if no tabs initially
    ...
    def _update_window_title(self): 
        ...
        ide_suffix = ""
        active_ide_tab_data = self._get_active_ide_tab_data() # Helper to get current tab's data
        ...
        elif self.ide_tab_widget and self.ide_tab_widget.count() > 0: # If untitled tabs exist
             active_tab_index = self.ide_tab_widget.currentIndex()
             if active_tab_index != -1 : # Check if a tab is actually selected
    ...
    def _update_ide_save_actions_enable_state(self):
        ...
        if hasattr(self, 'ide_save_as_file_action'):
             self.ide_save_as_file_action.setEnabled(bool(has_content))
    ...
    def _add_ide_tab(self, title="Untitled", content="", file_path=None, language="Python"):
        ...
        # editor.set_language(language) # Will be set by _on_ide_current_tab_changed or when lang combo changes
        ...
        tab_content_widget._ide_tab_data = {
            ...
            "path": file_path,
            "is_dirty": False if file_path and content else True, 
            "language": language
        }
        editor.textChanged.connect(lambda widget=tab_content_widget: self._on_ide_text_changed_for_tab(widget))

        index = self.ide_tab_widget.addTab(tab_content_widget, title)
        self.ide_tab_widget.setCurrentIndex(index) # This will trigger _on_ide_current_tab_changed
        # _update_ide_tab_title and _update_ide_related_actions will be called by _on_ide_current_tab_changed
        editor.setFocus()
    ...
    def _update_ide_tab_title(self, index: int):
        ...
        if hasattr(widget, "_ide_tab_data"):
            ...
            base_title = os.path.basename(tab_data["path"]) if tab_data["path"] else f"Untitled {index + 1}"
            # Ensure tab titles are unique if multiple "Untitled" exist
            if not tab_data["path"]:
                all_tab_titles = [self.ide_tab_widget.tabText(i) for i in range(self.ide_tab_widget.count()) if i != index]
                temp_title = base_title
                num = index + 1 # Start with current index as a base for uniqueness
                # This loop for unique untitled names might need refinement if tabs are reordered or many are opened/closed.
                # A simpler approach might be to always use a globally incrementing counter for "Untitled X".
                # For now, this aims to make "Untitled 1", "Untitled 2", etc. based on current visual order + 1.
                # Let's refine: Use a simple incrementing counter for new untitled tabs.
                # This part needs a more robust unique naming if many untitled tabs are common.
                # For simplicity now, we'll stick to "Untitled {index + 1}"
                # A better unique name generation can be added if it becomes an issue.
                # current_untitled_numbers = []
                # for i in range(self.ide_tab_widget.count()):
                #     if i == index: continue
                #     text = self.ide_tab_widget.tabText(i).replace("*","")
                #     if text.startswith("Untitled ") and text.replace("Untitled ","").isdigit():
                #         current_untitled_numbers.append(int(text.replace("Untitled ","")))
                # next_num = 1
                # while next_num in current_untitled_numbers:
                #     next_num +=1
                # base_title = f"Untitled {next_num}"
                pass # Current base_title logic is okay for now
            ...
            if self.ide_tab_widget.currentIndex() == index:
                self._update_window_title() # Update main window title which might include active IDE tab info
    ...
    def _prompt_ide_save_if_dirty(self, tab_index: int = -1) -> bool:
        ...
        if tab_index != -1 and tab_index < self.ide_tab_widget.count(): # Specific tab
        ...
        else: # Active tab if no index specified
        ...
        reply = QMessageBox.question(self, "Save IDE Script?",
                                     ...
                                     QMessageBox.Save)
        if reply == QMessageBox.Save:
            # Ensure we are trying to save the correct tab (the one being prompted for)
            # This is tricky if the active tab changed while the dialog was up.
            # The _save_specific_ide_tab makes this more robust.
    ...
    def on_ide_new_file(self):
        # For "New", always add a new tab now
        num_tabs = self.ide_tab_widget.count() if self.ide_tab_widget else 0
    ...
    def on_ide_open_file(self):
        ...
        # If current active tab is untitled, empty, and not dirty, offer to use it.
        # Otherwise, prompt to save it if dirty before opening new file in new tab.
        should_use_active_tab_for_open = False
        if active_tab_data and not active_tab_data["path"] and \
           not active_tab_data["editor"].toPlainText().strip() and \
           not active_tab_data["is_dirty"]:
            should_use_active_tab_for_open = True
             ...
             if not self._prompt_ide_save_if_dirty(active_tab_idx):
                 return # User cancelled saving current dirty tab
        ...
        if file_path:
            ...
            if should_use_active_tab_for_open and active_tab_idx != -1:
                ...
                target_tab_widget = self._add_ide_tab(title=os.path.basename(file_path), file_path=file_path)
            ...
            try:
                ...
                self._update_ide_tab_title(idx) # This also updates main window title
                self.ide_tab_widget.setCurrentIndex(idx) # Ensure this new/updated tab is active
                ...
                if self.ide_language_combo.currentText() != lang_to_set: # Avoid redundant signal if already correct
                    self.ide_language_combo.setCurrentText(lang_to_set) 
                else: # If language is already correct, manually update editor's highlighter
                ...
                if target_tab_widget and not should_use_active_tab_for_open: # If it was a newly added tab that failed
                    self.ide_tab_widget.removeTab(self.ide_tab_widget.indexOf(target_tab_widget))
                elif should_use_active_tab_for_open and current_tab_data : # If reused tab failed, reset it
    ...
    def _save_specific_ide_tab(self, index: int) -> bool:
        ...
        if not file_to_save_path: # If no path, it's a "Save As" operation for this tab
            current_tab_text = self.ide_tab_widget.tabText(index).replace("*","")
            default_filename = f"{current_tab_text}.py" if not current_tab_text.startswith("Untitled") else "untitled_script.py"
            
            # Infer extension based on language
            ...
            file_path_dialog, _ = QFileDialog.getSaveFileName(self, f"Save Script '{current_tab_text}' As", os.path.join(start_dir, default_filename),
                                                       "Python Files (*.py);;C/C++ Files (*.c *.cpp *.h *.ino);;Text Files (*.txt);;All Files (*)")
            if not file_path_dialog: return False # User cancelled
            file_to_save_path = file_path_dialog

        # Now save to file_to_save_path
        try:
            with open(file_to_save_path, 'w', encoding='utf-8') as f:
                f.write(tab_data["editor"].toPlainText())
            tab_data["path"] = file_to_save_path # Update path in tab data
    ...
    def on_ide_save_as_file(self) -> bool:
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Script As", os.path.join(start_dir, default_filename),
                                                   "Python Files (*.py);;C/C++ Files (*.c *.cpp *.h *.ino);;Text Files (*.txt);;All Files (*)")
        if file_path:
            # If saving with a new name, current active tab's path should be updated
            active_tab_data["path"] = file_path
            # Update the language based on new extension if necessary
            ext = os.path.splitext(file_path)[1].lower()
            new_lang = "Python"
            ...
            if active_tab_data["language"] != new_lang:
                ...
                self.ide_language_combo.setCurrentText(new_lang) # Triggers _on_ide_language_changed
    ...
    def _on_ide_text_changed(self):
        # This slot is no longer directly connected globally.
        # It's handled by _on_ide_text_changed_for_tab.
        pass
    ...
    def _on_ide_text_changed_for_tab(self, tab_widget_item):
        if hasattr(tab_widget_item, "_ide_tab_data"):
            ...
            if not tab_data["is_dirty"]:
                ...
                if idx != -1: # Ensure tab still exists
                    self._update_ide_tab_title(idx)
        self._update_ide_related_actions() # Update global save/find etc. buttons


    @pyqtSlot(str)
    def _on_ide_language_changed(self, language: str):
        active_tab_data = self._get_active_ide_tab_data()
        if active_tab_data and active_tab_data["editor"]:
            active_tab_data["editor"].set_language(language)
            active_tab_data["language"] = language # Store it per tab
    ...
    def _update_ide_related_actions(self):
        # This method centralizes enabling/disabling of IDE-specific actions
        # based on current language, content, and AI status.
        active_tab_data = self._get_active_ide_tab_data()
        
        selected_language = active_tab_data["language"] if active_tab_data else "Python"

        has_text_in_ide = bool(active_tab_data and active_tab_data.get("editor") and
                           bool(active_tab_data["editor"].toPlainText().strip()))
        has_console_output = bool(active_tab_data and active_tab_data.get("output_console") and
                              bool(active_tab_data["output_console"].toPlainText().strip()))
        ...
        if self.ide_run_script_action:
            self.ide_run_script_action.setEnabled(bool(is_python and has_text_in_ide))
        ...
        self._update_ide_save_actions_enable_state() # For Save/SaveAs which depends on dirty and content
    ...
    def on_ide_find(self):
        active_tab_data = self._get_active_ide_tab_data()
        if not active_tab_data or not active_tab_data["editor"]: return
    ...
    def _on_ide_tab_close_requested(self, index):
        ...
        if not self._prompt_ide_save_if_dirty(index): # Prompt for the specific tab being closed
            return

        widget_to_remove = self.ide_tab_widget.widget(index)
        self.ide_tab_widget.removeTab(index)
        if widget_to_remove: # Ensure it's not already gone
             widget_to_remove.deleteLater() # Clean up the tab's content widget
    ...
    def _on_ide_current_tab_changed(self, index):
        if index == -1 and self.ide_tab_widget.count() == 0 : # All tabs closed, about to add a new one
             pass # _add_ide_tab will handle updates
        elif index != -1: # A valid tab is now current
            self._update_window_title()
            self._update_ide_related_actions()
            active_tab_data = self._get_active_ide_tab_data()
            if active_tab_data and self.ide_language_combo:
                # Block signals temporarily to prevent recursive calls or unwanted effects
                ...
                if active_tab_data.get("editor"): # Ensure editor exists
