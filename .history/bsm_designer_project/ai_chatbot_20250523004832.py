...
class ChatbotWorker(QObject):
    ...
    def process_message(self, user_message: str):
        ...
        current_conversation = [{"role": "system", "content": system_prompt_content}] 
        history_to_send = self.conversation_history[-6:] # Keep conversation somewhat short for generation
        ...
        try:
            chat_completion = self.client.chat.completions.create(
                ...
                # If using newer models, you might specify response_format for JSON output directly
                # response_format={"type": "json_object"} # Requires specific model versions
            )
            ai_response_content = chat_completion.choices[0].message.content
            
            
            if is_generation_request:
                # Attempt to parse the AI's response as JSON directly
                # This is where the strict system prompt is crucial.
                # We'll pass the raw string to the main thread to parse and handle.
                self.conversation_history.append({"role": "user", "content": user_message})
                self.conversation_history.append({"role": "assistant", "content": ai_response_content}) # Store the raw JSON as the response
                self.responseReady.emit(ai_response_content) # Emit the raw string (JSON or error message from AI)
            else:
