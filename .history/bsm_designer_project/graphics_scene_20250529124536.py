...
from PyQt5.QtCore import Qt, QRectF, QPointF, QLineF, pyqtSignal, QPoint, pyqtSlot # Added pyqtSlot
...
class DiagramScene(QGraphicsScene):
    ...
    def __init__(self, undo_stack, parent_window=None):
        ...
        # Cache for state items by name
        self._state_item_by_name_map = {}
    ...
    def addItem(self, item: QGraphicsItem):
        super().addItem(item) # Add to scene first
        if isinstance(item, GraphicsStateItem):
            if item.text_label in self._state_item_by_name_map:
                 # This could happen if a state with a duplicate name is added programmatically (e.g. by AI)
                 # The interactive add already tries to prevent duplicates.
                 logger.warning(f"Scene: addItem - State '{item.text_label}' already in name map. Overwriting. Ensure names are unique.")
            self._state_item_by_name_map[item.text_label] = item
            if hasattr(item, 'nameChanged') and callable(item.nameChanged.connect): # Check before connecting
                try:
                    item.nameChanged.connect(self._handle_state_item_name_change)
                except Exception as e:
                    logger.error(f"Scene: Error connecting nameChanged signal for {item.text_label}: {e}")

    def removeItem(self, item: QGraphicsItem):
        if isinstance(item, GraphicsStateItem):
            if item.text_label in self._state_item_by_name_map and self._state_item_by_name_map[item.text_label] == item:
                del self._state_item_by_name_map[item.text_label]
            if hasattr(item, 'nameChanged') and callable(item.nameChanged.disconnect): # Check before disconnecting
                try:
                    item.nameChanged.disconnect(self._handle_state_item_name_change)
                except TypeError: # If not connected or already disconnected
                    pass
                except Exception as e:
                    logger.error(f"Scene: Error disconnecting nameChanged signal for {item.text_label}: {e}")
        super().removeItem(item) # Remove from scene after internal cleanup


    @pyqtSlot(QGraphicsItem, str, str)
    def _handle_state_item_name_change(self, item: QGraphicsItem, old_name: str, new_name: str):
        if not isinstance(item, GraphicsStateItem): return

        logger.debug(f"Scene: Handling state name change from '{old_name}' to '{new_name}' for item {item}.")
        
        # Remove old name entry if it exists and points to this item
        if old_name in self._state_item_by_name_map and self._state_item_by_name_map[old_name] == item:
            del self._state_item_by_name_map[old_name]
            logger.debug(f"Scene: Removed '{old_name}' from name map.")
        elif old_name in self._state_item_by_name_map:
             logger.warning(f"Scene: Old name '{old_name}' was in map but pointed to a different item. Not removing.")

        # Add new name entry
        if new_name in self._state_item_by_name_map and self._state_item_by_name_map[new_name] != item:
            logger.error(f"Scene: CRITICAL - New name '{new_name}' for item {item} conflicts with existing item {self._state_item_by_name_map[new_name]} in map! This indicates a duplicate name was allowed.")
            # This situation should ideally be prevented by UI logic (e.g., in StatePropertiesDialog)
            # Forcing an overwrite here, but it's a symptom of a problem elsewhere if it happens.
        self._state_item_by_name_map[new_name] = item
        logger.debug(f"Scene: Added '{new_name}' to name map, pointing to {item}.")

        # Notify about potential transition updates (if transitions rely on names, which they shouldn't for linking but might for display/data)
        if hasattr(self, '_update_transitions_for_renamed_state'):
            self._update_transitions_for_renamed_state(old_name, new_name)
        self.set_dirty(True)
    ...
    def get_state_by_name(self, name: str) -> GraphicsStateItem | None:
        return self._state_item_by_name_map.get(name)
    ...
    def edit_item_properties(self, item):
        ...
        if dialog.exec() == QDialog.Accepted: 
            ...
            if isinstance(item, GraphicsStateItem): 
                ...
                existing_state_with_new_name = self.get_state_by_name(current_new_name) # Uses the new optimized map
    ...
    def clear(self):
        # Disconnect signals from items before removing them
        for item in self.items():
            if isinstance(item, GraphicsStateItem) and hasattr(item, 'nameChanged'):
                try:
                    item.nameChanged.disconnect(self._handle_state_item_name_change)
                except TypeError: # Already disconnected or never connected
                    pass
                except Exception as e:
                    logger.error(f"Error disconnecting nameChanged for {item.text_label} during clear: {e}")
        
        super().clear() # Call QGraphicsScene.clear()
        self._state_item_by_name_map.clear() # Clear the name map
        self.set_dirty(False) # Usually, clear makes it non-dirty (like new file)
        logger.debug("DiagramScene cleared and state name map reset.")





    def get_selected_items_diagram_data(self) -> dict:
        """
        Returns diagram data for the currently selected items.
        - Includes selected states and comments directly.
        - Includes selected transitions.
        - For selected transitions, also includes their source and target states.
        - For selected states, also includes transitions that connect *only between* the selected states.
        """
        data = {'states': [], 'transitions': [], 'comments': []}
        selected_graphics_items = self.selectedItems()

        if not selected_graphics_items:
            return data # Return empty structure if nothing selected

        # Sets to store unique items to include
        states_to_include = set()
        transitions_to_include = set()
        comments_to_include = set()

        # Pass 1: Collect directly selected items and related states from transitions
        for item in selected_graphics_items:
            if isinstance(item, GraphicsStateItem):
                states_to_include.add(item)
            elif isinstance(item, GraphicsTransitionItem):
                transitions_to_include.add(item)
                if item.start_item: states_to_include.add(item.start_item)
                if item.end_item: states_to_include.add(item.end_item)
            elif isinstance(item, GraphicsCommentItem):
                comments_to_include.add(item)
        
        # Pass 2: For selected states, add transitions that are fully contained within the selected state set
        if any(isinstance(si, GraphicsStateItem) for si in selected_graphics_items): # Only if states were part of selection
            for scene_item in self.items(): # Iterate all scene items to find relevant transitions
                if isinstance(scene_item, GraphicsTransitionItem):
                    if scene_item.start_item in states_to_include and scene_item.end_item in states_to_include:
                        transitions_to_include.add(scene_item)
        
        # Serialize the collected items
        for state_item in states_to_include:
            data['states'].append(state_item.get_data())
        
        for trans_item in transitions_to_include:
            if trans_item.start_item and trans_item.end_item: # Ensure valid transition
                 data['transitions'].append(trans_item.get_data())
            else: # Should not happen if states_to_include logic is correct
                self._log_to_parent("WARNING", f"Skipping save of orphaned/invalid selected transition: '{trans_item._compose_label_string()}'.")

        for comment_item in comments_to_include:
            data['comments'].append(comment_item.get_data())
            
        # Add a note if context is partial
        if data['states'] or data['transitions'] or data['comments']:
             data['context_type'] = "partial_selection"
             data['description'] = data.get('description', "") + " (Context: Based on selected diagram items)"


        logger.debug(f"Generated selected items diagram data. States: {len(data['states'])}, Transitions: {len(data['transitions'])}")
        return data
    ...
    def load_diagram_data(self, data):
        self.clear() # Clear existing scene content (also clears name map)
        self.set_dirty(False) 
        # state_items_map is local to this function, for linking transitions during load
        # The scene's _state_item_by_name_map will be populated by self.addItem below
        ...
        for state_data in data.get('states', []):
            ...
            self.addItem(state_item) # This will also populate self._state_item_by_name_map

        for trans_data in data.get('transitions', []):
            src_item = self.get_state_by_name(trans_data['source']) # Use optimized getter
            tgt_item = self.get_state_by_name(trans_data['target']) # Use optimized getter
            if src_item and tgt_item: 
                ...
                self.addItem(trans_item) # AddItem doesn't need to special-case transitions for the map
            else:
    ...
    def validate_diagram(self) -> list:
        issues = []
        states_from_map = list(self._state_item_by_name_map.values()) # Use the map for states
        transitions = [item for item in self.items() if isinstance(item, GraphicsTransitionItem)]

        initial_states = [s for s in states_from_map if s.is_initial]
        ...
        # Duplicate name check is inherently handled if _state_item_by_name_map is correctly maintained
        # and UI prevents adding duplicate names. If duplicates manage to get in, it's a deeper issue.
        # For safety, can iterate self.items() too, but the map should be source of truth.
        # This validation now implicitly checks map integrity.
        all_state_items_in_scene = [item for item in self.items() if isinstance(item, GraphicsStateItem)]
        if len(all_state_items_in_scene) != len(self._state_item_by_name_map):
            issues.append(("error", "Internal state name map inconsistent with scene items. Possible duplicate names not caught.", None))
        
        # Check for states with no incoming/outgoing transitions
        for state in states_from_map:
        ...
        for i, transition in enumerate(transitions):
            ...
            elif not self.get_state_by_name(transition.start_item.text_label) or \
                 not self.get_state_by_name(transition.end_item.text_label):
                 issues.append(("error", f"Transition {i+1} (Label: '{transition._compose_label_string()}') references non-existent source/target state (map lookup failed).", transition))
    ...
    def highlight_validation_issues(self, issues: list):
        ...
        for issue_type, message, item_ref_or_name in issues:
            ...
            elif isinstance(item_ref_or_name, str): 
    ...
    def clear_validation_highlights(self):
        for item in self._validation_highlighted_items:
            if hasattr(item, 'set_validation_highlight'):
                item.set_validation_highlight(None) 
        self._validation_highlighted_items = []
        self.update() 
