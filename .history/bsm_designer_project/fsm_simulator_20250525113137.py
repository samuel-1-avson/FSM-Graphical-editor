...
class StateMachinePoweredSimulator:
    ...
    def _create_dynamic_callback(self, code_string, callback_type="action", original_name="dynamic_callback"):
        ...
        if not is_safe:
            ...
            if callback_type == "condition":
                def unsafe_condition_wrapper(sm_instance_arg, *args, **kwargs):
                ...
                def unsafe_action_wrapper(sm_instance_arg, *args, **kwargs):
        ...
        def dynamic_callback_wrapper(sm_instance_arg, *args, **kwargs_from_sm_call):
            exec_eval_locals_dict = sm_instance_arg.model
    ...
    def _build_fsm_class_and_instance(self):
        ...
        simulator_self = self # For lambdas
        ...
        for s_name, s_data in self._states_input_data.items():
            ...
            if s_data.get('entry_action'):
                # Need to ensure the lambda captures the correct s_data for *this* iteration
                current_s_data = s_data # Capture current s_data
                fsm_class_attrs[f"on_enter_{s_name}"] = lambda sm_instance, *args, sd=current_s_data, sn=s_name, **kwargs: \
            ...
            if s_data.get('exit_action'):
                current_s_data_exit = s_data # Capture current s_data
                fsm_class_attrs[f"on_exit_{s_name}"] = lambda sm_instance, *args, sd=current_s_data_exit, sn=s_name, **kwargs: \
                    simulator_self._create_dynamic_callback(
        ...
        for t_idx, t_data in enumerate(self._transitions_input_data):
            ...
            # Must capture t_data for these lambdas too if they are to be used inside the lambda body directly
            current_t_data_cond = t_data # Capture current t_data for condition
            cond_cb = (lambda sm_instance, *args, td=current_t_data_cond, **kwargs: \
                simulator_self._create_dynamic_callback(td['condition'], "condition", f"cond_t{t_idx}_{event_name_str}")(sm_instance, *args, **kwargs)) \
                if t_data.get('condition') else None

            current_t_data_action = t_data # Capture current t_data for action
            action_cb = (lambda sm_instance, *args, td=current_t_data_action, **kwargs: \
                simulator_self._create_dynamic_callback(td['action'], "action", f"action_t{t_idx}_{event_name_str}")(sm_instance, *args, **kwargs)) \
