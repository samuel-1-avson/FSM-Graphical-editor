...
class ResourceMonitorWorker(QObject):
    ...
    def __init__(self, interval_ms=2000, parent=None):
        ...
        self._stop_requested = False # Use a simple boolean flag
    ...
    def _attempt_nvml_init(self, from_worker_loop=False):
        if self._stop_requested: return # Check if stop was requested before attempting init
    ...
    def stop_monitoring(self): # This slot is called from the main thread
        ...
        logger.debug("ResourceMonitorWorker: Stop flag set for worker loop.")
    ...
    def _monitor_resources(self):
        ...
        # Get a reference to the QThread managing this worker, if any
        # This is crucial for checking isInterruptionRequested()
        worker_thread = self.thread() 
        if not worker_thread:
            logger.error("ResourceMonitorWorker: Worker is not associated with a QThread. Cannot reliably stop.")
            # Fallback to old flag if no thread context, though this shouldn't happen if moved correctly
            while not self._stop_requested: # This is a fallback
                 QThread.msleep(self.WORKER_LOOP_CHECK_INTERVAL_MS) # Basic sleep
        ...
        while not worker_thread.isInterruptionRequested():
                ...
                try:
                    ...
                    if self._nvml_initialized and self._gpu_handle and PYNVML_AVAILABLE and pynvml:
                        ...
                        try:
                            ...
                            gpu_util = -2.0 
                            if hasattr(pynvml, 'NVML_ERROR_UNINITIALIZED') and e_nvml_util.value == pynvml.NVML_ERROR_UNINITIALIZED:
                                self._nvml_initialized = False
                            self._gpu_handle = None
                        except Exception: gpu_util = -2.0 
                    
                    if worker_thread.isInterruptionRequested(): break
                    if not worker_thread.isInterruptionRequested(): 
                        self.resourceUpdate.emit(cpu_usage, ram_percent, gpu_util, gpu_name_to_emit)
                    last_data_emit_time = current_loop_time

                except Exception as e:
                     logger.error(f"Error in resource monitoring data collection: {e}", exc_info=False)
                     if not worker_thread.isInterruptionRequested(): self.resourceUpdate.emit(-1.0, -1.0, -3.0, "Data Error")
            
            # Yield control and check for interruption
            # This is a more Qt-idiomatic way for a QThread's loop to be responsive
            # It will process events in the thread's event loop if any are pending
            # and allows isInterruptionRequested() to be effective.
            for _i in range(int(self.data_collection_interval_ms / self.WORKER_LOOP_CHECK_INTERVAL_MS) + 1):
                if worker_thread.isInterruptionRequested():
                    break
                QThread.msleep(self.WORKER_LOOP_CHECK_INTERVAL_MS) # Sleep for a short duration
            if worker_thread.isInterruptionRequested():
                break
        
        logger.info(f"Resource monitor worker loop finished (Interruption requested: {worker_thread.isInterruptionRequested()}).")
        self.finished_signal.emit()
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if self.resource_monitor_thread and self.resource_monitor_thread.isRunning():
            ...
            if self.resource_monitor_worker:
                ...
                # Request the worker to stop its loop.
                # QMetaObject.invokeMethod is good for thread-safe slot calls.
                # Direct call self.resource_monitor_worker.stop_monitoring() is also fine if worker is robust.
            ...
            # Ask the thread's event loop to quit. This is important if the thread
            # is processing its own events. The worker's finished_signal is connected to this.
            # However, calling it here too ensures the quit request is made.
        ...
        if worker_ref_for_nvml_shutdown: 
            ...
            # This case might occur if thread was never started or already stopped
            # but worker object somehow still exists.
            logger.info("MW_CLOSE: Shutting down NVML via direct worker attribute (thread might not have been running/found).")
            self.resource_monitor_worker._shutdown_nvml() # Ensure NVML is shut down
