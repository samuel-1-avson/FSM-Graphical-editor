...
class ChatbotWorker(QObject):
    ...
    def process_message_slot(self, user_message: str, force_fsm_generation: bool):
        ...
        history_context_limit = -6 # Max 3 pairs of user/model messages + current user message
        ...
        try:
            ...
            if not self.conversation_history: # If no history, system prompt goes first (as a "user" message contextually)
                ...
                if system_prompt_content:
                    ...
                    if not self.conversation_history:
                         ...
                         # gemini_contents was already prepared: history + current user message.
                         # The system prompt should come *before* any history.
                         # So, this needs adjustment. If no history, it's [system, current_user_message].
                         # If history exists, it's [history..., system_as_user_context, current_user_message] which is not ideal.
                         # The current `gemini_contents` already contains the current user message.
                         # So we need to insert the system prompt *before* that final user message.
                         if gemini_contents and gemini_contents[-1]["role"] == "user":
                             final_contents_for_api.extend(gemini_contents[:-1]) # Add history
                             final_contents_for_api.append({"role": "user", "parts": [{"text": system_prompt_content}]}) # System as user
                             final_contents_for_api.append(gemini_contents[-1]) # Actual current user message
                         else: # Should not happen if gemini_contents is built correctly
                             final_contents_for_api.append({"role": "user", "parts": [{"text": system_prompt_content}]})
                             final_contents_for_api.extend(gemini_contents)

                    else:
                        # If history exists, we inject the system prompt just before the current user's message.
                        # This is effectively making the system_prompt_content part of the context for this turn.
                        effective_user_message = f"{system_prompt_content}\n\nHuman: {user_message}"
                        # Rebuild gemini_contents for this call only.
                        # `gemini_contents` currently holds [history..., {role: user, parts: [user_message]}]
                        # We need to replace that last user message part.
                        call_contents = []
                        if self.conversation_history: # history_context_limit already applied
                            call_contents.extend(self.conversation_history[history_context_limit:])
                        
                        # Replace the user_message in the last entry of gemini_contents if it's a user message
                        # This is complex. Simpler:
                        # The `system_prompt_content` is dynamic and includes diagram context.
                        # It *must* be provided with each call.
                        # The Gemini API documentation suggests that for `generateContent` (non-chat),
                        # system instructions can be the first "user" role message in `contents`.
                        # Or for `startChat`, it's `system_instruction` or the first history message.
                        #
                        # Correct approach for `generate_content` without `system_instruction` kwarg:
                        # `contents` should be `[{"role": "user", "parts": [system_prompt]}, {"role": "model", "parts": [empty_or_primer]}, {"role": "user", "parts": [first_actual_user_msg]}, ...]`
                        # Or, simply: `[{"role": "user", "parts": [system_prompt + "\n\n" + actual_user_message_1]}, ...]`
                        # Given our current structure, where `gemini_contents` = [history..., {current_user_msg}],
                        # We'll make the system_prompt_content the first "user" message of the *entire sequence for this API call*.
                        
                        call_contents_for_api = [{"role": "user", "parts": [{"text": system_prompt_content}]}]
                        # Now add the existing conversation history + current user message
                        call_contents_for_api.extend(gemini_contents) # gemini_contents has history + current user message
                        
                        generation_args["contents"] = call_contents_for_api
                    
                    final_contents_for_api = generation_args["contents"]
            ...
            if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
                ...
                # Check for specific block reasons if available in prompt_feedback
            ...
            self._current_processing_had_error = True
        except (genai.types.BlockedPromptException, genai.types.StopCandidateException) as e: # These are more specific Gemini exceptions
...
class AIChatbotManager(QObject):
    ...
    def _setup_worker(self):
        ...
        self.chatbot_worker.errorOccurred.connect(self.errorOccurred) # Connect to manager's slot
        self.chatbot_worker.statusUpdate.connect(self.statusUpdate) # Connect to manager's slot
    ...
    def _handle_worker_response(self, ai_response_content: str, was_fsm_generation_attempt: bool):
        ...
        if was_fsm_generation_attempt:
            try:
                ...
                # More robust cleaning might be needed if Gemini varies its markdown
                cleaned_json_str = ai_response_content
                if ai_response_content.strip().startswith("```json"):
                    cleaned_json_str = ai_response_content.strip()[7:] # Remove ```json\n
                elif ai_response_content.strip().startswith("```"):
                     cleaned_json_str = ai_response_content.strip()[3:] # Remove ```\n
                
                if cleaned_json_str.strip().endswith("```"):
                    cleaned_json_str = cleaned_json_str.strip()[:-3] # Remove trailing ```
                ...
                # Emitting plain text response if JSON parsing fails for an FSM attempt
                # This allows the user to see what the AI returned, even if it's not valid JSON.
                # self.plainResponseReady.emit(ai_response_content) 
                # Decided against emitting as plain if it was an FSM attempt and failed parsing,
                # better to show the error and let user retry or adjust.
                # The raw content will be in the logs.
                # We can emit the errorOccurred signal for UI manager to display the raw output if desired.
                self.errorOccurred.emit(f"AI response for FSM generation was not valid JSON. Raw response:\n{ai_response_content}")
                return # Important: return here after emitting error for failed FSM JSON
    ...
    def _prepare_and_send_to_worker(self, user_message_text: str, is_fsm_gen_specific: bool = False):
        ...
        if is_fsm_gen_specific:
            ...
            self.last_fsm_request_description = None # Clear if it's a general message
        ...
        if self.parent_window and hasattr(self.parent_window, 'scene') and hasattr(self.parent_window.scene, 'get_diagram_data'):
            try:
                ...
                # For AI, only send states and transitions to keep context lean. Comments less relevant.
                # Positions are also not relevant for AI context.
                lean_diagram_data = {
        ...
        if self.chatbot_worker:
            ...
            # Status update is now handled by the worker itself or the UI manager based on signals
            # if self.parent_window and hasattr(self.parent_window, 'ai_chat_ui_manager') and self.parent_window.ai_chat_ui_manager:
            #     self.parent_window.ai_chat_ui_manager.update_status_display("Status: Sending to AI...")
            self.statusUpdate.emit("Status: Sending to AI...") # Manager can still emit this
        else:
    ...
    def clear_conversation_history(self):
        ...
        if self.chatbot_worker and self.chatbot_thread and self.chatbot_thread.isRunning():
            ...
            # Status update will come from worker after history is cleared
        else:
    ...
    def set_online_status(self, is_online: bool):
        ...
        if self.api_key:
            if is_online:
                # Don't emit "Ready" if worker setup might fail due to API key issue reported by worker itself
                if not self.chatbot_thread or not self.chatbot_thread.isRunning():
                    ...
                    self._setup_worker() # This will emit its own status (Ready or Error)
                else:
                    self.statusUpdate.emit("Status: Online and Ready.") # Already running
            else:
