...
class DiagramScene(QGraphicsScene):
    ...
    def edit_item_properties(self, item):
        ...
        if isinstance(item, GraphicsStateItem):
            ...
            dlg = TransitionPropertiesDialog(self.parent_window, old_props);
        if dlg.exec_()==QDialog.Accepted: 
            accepted=True; new_props=dlg.get_properties()
        elif isinstance(item, GraphicsCommentItem):
            dlg = CommentPropertiesDialog(self.parent_window, old_props);
        if dlg.exec_()==QDialog.Accepted: 
            accepted=True; new_props=dlg.get_properties()
        else: return
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() in [Qt.Key_Delete, Qt.Key_Backspace]:
            ...
            if self._temp_transition_line:
                ...
                self.log_function("Transition cancelled (Esc)."); self.set_mode("select")
...
class ZoomableView(QGraphicsView):
    ...
    def keyPressEvent(self, event:QKeyEvent):
        ...
        if self.scene(): 
            cr=self.scene().itemsBoundingRect();
...
class TransitionPropertiesDialog(QDialog):
    def __init__(self, parent=None, current_properties=None, is_new_transition=False):
        ...
        self.condition_edit.setPlaceholderText("e.g., var_x > 10")
        ...
        self.action_edit.setPlaceholderText("MATLAB actions...")
        ...
        self.offset_perp_spin.setToolTip("Perpendicular bend.")
        ...
        self.offset_tang_spin.setToolTip("Tangential mid shift.")
        self.description_edit = QTextEdit(p.get('description', ""))
        self.description_edit.setFixedHeight(75)
        self.description_edit.setPlaceholderText("Optional notes")
        def add_field(lbl, ed, btn):
            h = QHBoxLayout()
            h.setSpacing(5)
            h.addWidget(ed,1)
            v = QVBoxLayout()
        ...
        layout.addRow("Color:",self.color_button)
        cl = QHBoxLayout()
        cl.addWidget(QLabel("Bend (Perp):"))
        cl.addWidget(self.offset_perp_spin)
        cl.addSpacing(10)
        cl.addWidget(QLabel("Mid Shift (Tang):"))
        cl.addWidget(self.offset_tang_spin)
        cl.addStretch()
        layout.addRow("Curve Shape:",cl)
        layout.addRow("Description:",self.description_edit)
        btns = QDialogButtonBox(QDialogButtonBox.Ok|QDialogButtonBox.Cancel)
        btns.accepted.connect(self.accept)
        btns.rejected.connect(self.reject)
...
class MainWindow(QMainWindow):
    ...
    def _update_properties_dock(self):
        ...
        if len(sel) == 1:
            self.scene.edit_item_properties(sel[0])
    def log_message(self,message:str):
        ts=QTime.currentTime().toString('hh:mm:ss.zzz');
        self.lo.append(f"<span style='color:{COLOR_TEXT_SECONDARY};'>[{ts}]</span> {html.escape(message)}");self.lo.verticalScrollBar().setValue(self.lo.verticalScrollBar().maximum());self.sl.setText(message.split('\n')[0][:120])
    ...
    def on_export_diagram(self): # NEW METHOD
        ...
        def on_export_simulink(self):
            if not self.matlab_connection.connected:
                QMessageBox.warning(self, "MATLAB Not Connected", "Configure MATLAB.")
            return
        dlg = QDialog(self)
        dlg.setWindowTitle("Export to Simulink")
        dlg.setWindowIcon(get_standard_icon(QStyle.SP_ArrowUp, "->M", target_size=self.dialog_button_icon_size))
        layout = QFormLayout(dlg)
        layout.setSpacing(8)
        layout.setContentsMargins(10, 10, 10, 10)
        name_def = "BSM_Model"
        bis = self.dialog_button_icon_size
        if self.current_file_path:
            ...
            if not name_def or not name_def[0].isalpha():
                name_def = "Model_" + name_def
        ne = QLineEdit(name_def)
        ne.setPlaceholderText("Simulink model name")
        layout.addRow("Model Name:", ne)
        def_out = os.path.dirname(self.current_file_path) if self.current_file_path else QDir.homePath()
        oe = QLineEdit(def_out)
        br = QPushButton(get_standard_icon(QStyle.SP_DirOpenIcon, "Brw", target_size=bis), " Browse...")
        br.setIconSize(bis)
        br.clicked.connect(lambda: oe.setText(QFileDialog.getExistingDirectory(dlg, "Select Output Dir", oe.text()) or oe.text()))
        dl = QHBoxLayout()
        dl.addWidget(oe, 1)
        dl.addWidget(br)
        layout.addRow("Output Dir:", dl)
        btns = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        ...
        if dlg.exec_() == QDialog.Accepted:
            name = ne.text().strip()
            out_dir = oe.text().strip()
            if not name or not out_dir:
                QMessageBox.warning(self, "Input Error", "Model name & output dir required.")
                return
            if not name[0].isalpha() or not all(c.isalnum() or c == '_' for c in name):
                QMessageBox.warning(self, "Invalid Name", "Model name: letter start, alnum/underscores.")
                return
            if not os.path.exists(out_dir):
                try:
                    os.makedirs(out_dir, exist_ok=True)
                except OSError as e:
                    QMessageBox.critical(self, "Dir Error", f"Could not create dir:\n{e}")
                    return
            data = self.scene.get_diagram_data()
            if not data['states']:
                QMessageBox.information(self, "Empty Diagram", "No states to export.")
                return
